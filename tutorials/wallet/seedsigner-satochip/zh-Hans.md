...
Satochip 是一种智能卡格式的 Hardware Wallet，具有最高安全标准之一的 EAL6+ 认证安全元件。它由比利时同名公司 Satochip 设计和生产。



Satochip 的售价约为 25 欧元，因其物美价廉而在竞争中脱颖而出。由于采用了安全芯片，它可以抵御物理攻击。此外，它的小程序源代码是完全开源的，根据*AGPLv3*授权。
...
- Satochip 的价格约为 25 欧元，由于小程序是开源的，你可以在空白的智能卡上自行安装。然后，您还得加上 SeedSigner 组件和读取智能卡的扩展功能的费用：根据您购买硬件的地点不同，总费用应该在 70 欧元到 100 欧元之间。
...
如果您已经拥有了Satochip，可以跳过这一步，直接进入第4步。在本节中，我们将介绍如何在空白智能卡上安装Satochip小程序（DIY方法）。小程序只是运行在智能卡上的一个小程序，它能让我们管理特定的功能。
...
现在，您的所有硬件和软件都已正常运行，您可以开始创建 Bitcoin 投资组合了。为此，请插入 SeedSigner，然后像使用传统 SeedSigner 一样，通过掷骰子或拍照来创建 generate 和 seed：
...
一旦生成了 seed，这是它在 SeedSigner 的 RAM 中唯一的停留时间。就我而言，我想把它保存在[Seedkeeper](https://satochip.io/product/seedkeeper/)上，这是另一个 Satochip 产品，专门用于存储秘密。如果我的 Satochip 丢失，我将把它作为最后的手段。



这里选择的备份策略取决于您的偏好，但必须至少有一份 Mnemonic 短语的副本，可以是物理介质（纸质或金属），也可以是 Seedkeeper。您还可以根据需要增加备份数量。有关组合备份策略的更多信息，我建议您阅读本教程 ：



https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

要在 Seedkeeper 上备份 seed，请直接进入 "备份 seed "菜单。
...
选择一个 "标签"，以便轻松识别存储在 Seedkeeper 上的不同秘密。例如，您可以简单地保留 Wallet 的指纹，也可以明确标明 `seed`。选择取决于您的偏好和风险。
...
如果你的备份策略完全依赖于 Seedkeeper，我强烈建议你现在就运行空恢复测试，然后比较指纹来检查备份是否正常。



https://planb.academy/tutorials/wallet/backup/recovery-test-5a75db51-a6a1-4338-a02a-164a8d91b895

Seedkeeper 的 PIN 码应尽可能长且随机，以防在卡片受到物理破坏时被暴力破解。您还应保存该 PIN 码的备份副本，与 Seedkeeper 分开存放。没有这个 PIN 码，您将无法访问存储在 Seedkeeper 中的 Mnemonic，您的比特币也将永远丢失。
...
现在，您的投资组合已经生成、保存和检查完毕，我们将把它传输到 Satochip 上。为此，请确保 seed 已加载到 SeedSigner 的 RAM 中。然后进入 "工具 > 智能卡工具 > Satochip 功能"。
...
将 Satochip 插入智能卡读卡器，然后选择 "用 seed 初始化"。
...
然后，您可以设置 Satochip PIN 码，从 4 个字符到 16 个字符不等。为了加强 Wallet 的安全性，请选择一个随机的长密码：这是防止对 Mnemonic 短语进行物理访问的唯一保护措施。



根据您的个人策略，请记住在创建密码后立即将其保存在安全的密码管理器或物理介质中。在后一种情况下，请务必不要将包含 PIN 码的介质与 Satochip 存放在同一个地方，否则保护将失去作用。备份副本非常重要： **没有这个 PIN 码，您将无法访问您的 seed，您的比特币也将永久丢失**。
...
最后，选择 "协调器"，即您希望使用的投资组合管理软件。这里，我们将使用 Sparrow wallet。
...
为您的投资组合指定一个名称，然后点击 "创建 Wallet"。然后，Sparrow 会要求您设置一个密码，以锁定本地对该 Wallet 的访问。选择一个强大的密码：它可以保护您在 Sparrow 中的数据（公钥、地址、标签和交易历史）。不过，将来恢复 Wallet 时不需要该密码：只需要您的 Mnemonic 短语（可能还有 passphrase）。
...
然后，SeedSigner 会要求您从 Sparrow wallet 中随机扫描一个接收 Address，以确认导入的有效性。
...
您的 Satochip（通过 SeedSigner）和 Sparrow wallet 现在已安全连接。Sparrow 可以完全管理 Interface，而 Satochip 仍然是唯一能够签署交易的设备。现在，您已准备好在完全隔绝空气的配置下接收和发送比特币。
...
您的 Satochip 和 Sparrow wallet 现在已经配置好，可以一起工作了。在本节中，我们将逐步讲解如何在这种模式下接收和发送比特币。
...
在电脑上打开 Sparrow wallet，使用密码解锁您的 "Satochip-SeedSigner" Wallet。检查软件是否已连接到服务器（右下角的指示器）。然后在侧边栏点击 "接收"。
...
出现新的 Bitcoin Address。您会发现 ：




- 文本格式的 Address（如果使用的是 P2WPKH，则以 "bc1q... "开头）；
- 相关的二维码 ；
- 标签 "字段，用于跟踪交易。



我强烈建议您在 Wallet 中的每张 Bitcoin 收据上添加一个标签。这将帮助您轻松识别每张 UTXO 的来源，更好地管理您的隐私。要了解有关这一重要主题的更多信息，请查看 Plan ₿ Academy 上的专门培训：
...
现在，您的 Address 在所有 Sparrow 分区中都与此标签相关联。
...
在向付款人发送接收的 Address 之前，必须检查它是否属于您的 seed。这一步骤可确保 Satochip 能够签署与该 Address 相关的交易。它还能防止 Sparrow 显示欺诈性 Address 的潜在攻击。请记住，Sparrow 是在不安全的环境（您的电脑）中运行的，其攻击面远远大于完全隔离的 Satochip。因此，在 Hardware Wallet 上检查地址之前，千万不要盲目相信 Sparrow 中显示的地址。



在 Sparrow 中，点击 Address 的二维码可将其放大：然后将全屏显示。
...
如果扫描的 Address 与您的 seed 所生成的 Address 一致，SeedSigner 将显示信息：已验证 Address。
...
现在，您可以将 Address 以文本形式或通过二维码传送给需要向您发送 Satss 的个人或服务机构。一旦交易在网络上广播，它就会出现在 Sparrow wallet 的 "交易 "选项卡中。
...
在 Sparrow wallet 中，点击左侧边栏的 "发送 "选项卡即可创建交易。不过，我更喜欢使用 "UTXOs "选项卡，通过它可以练习*Coin 控制*。这种方法可以精确控制所花费的 UTXOs，从而限制交易过程中泄露的信息。
...
- 在 "付款至 "中，粘贴收件人的 Address 或使用相机图标扫描其 QR 码；
...
现在交易已签名并通过验证，剩下的就是在 Bitcoin 网络上广播，这样 Miner 就可以将其纳入 BLOCK。在 Sparrow 中，点击 "扫描 QR"。
...
将 SeedSigner 上显示的二维码（包含已签署交易的二维码）对准网络摄像头。Sparrow 将显示所有交易详情。最后检查所有信息是否正确，然后点击 "广播交易 "在 Bitcoin 网络上广播。
...
如果您的 Satochip 还在，但您的 SeedSigner 坏了或丢失了，这种情况处理起来相当简单，因为您的 Wallet 还在 Satochip 中。
...
如果您不想重建一个，也可以用传统方式使用 Satochip，即直接从电脑使用，而无需通过 SeedSigner。这种方法非常有效，但却大大降低了 Bitcoin Wallet 的安全性：您失去了 "*air-gapped*"隔离功能，现在只能盲签，因为 SeedSigner 充当了可信屏幕的角色。不过，在紧急情况下，或者在无法重建 SeedSigner 的情况下，这也不失为一种临时解决方案。
...
最后，输入智能卡密码解锁。然后，您就可以访问 Wallet，创建交易，并直接使用连接的 Satochip 进行签名。
...
另一种更微妙的情况是，您失去了使用包含 seed 的 Satochip 的权限：无论是它坏了、丢了、被偷了，还是您忘记了它的 PIN 码。如果您的 Satochip 被盗或遗失，我们强烈建议您在恢复资金访问权限后，立即将比特币转移到用不同的 seed 生成的全新 Wallet 中。这样可以确保潜在的攻击者永远无法访问您的 Sats。



要重新访问您的投资组合并转移资金，只需将您的 seed 载入 SeedSigner 即可。根据您使用的备份介质，您有几种选择：





- 在 `Seeds > Enter 12-word seed` 菜单中手动输入 Mnemonic 短语。
