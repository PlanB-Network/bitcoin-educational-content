...
Questo contenuto educativo è adattato dal libro e repo [JWBurgers](https://github.com/JWBurgers/An_Introduction_to_Cryptography). Sebbene l'autore ne abbia gentilmente concesso l'uso educativo, tutti i diritti di proprietà intellettuale rimangono del creatore originale.
...
Iniziamo la nostra indagine sul campo della [crittografia](https://planb.academy/resources/glossary/cryptography) con uno degli episodi più affascinanti e divertenti della sua storia: quello dei cifrari di Beale. [1]
...
È importante notare che la crittografia moderna non è esclusivamente lo studio degli schemi crittografici a chiave simmetrica e assimetrica (anche se questo copre gran parte del campo). Per esempio, la crittografia si occupa anche di [funzioni hash](https://planb.academy/resources/glossary/hash-function) e generatori di numeri pseudorandom, e su queste primitive si possono costruire applicazioni che non sono legate alla crittografia a chiave simmetrica o assimetrica.
...
La prima soluzione offerta è stata un *protocollo di scambio di chiavi* a distanza, ovvero un insieme di regole per lo scambio di una o più chiavi simmetriche su un canale di comunicazione non sicuro. Questo protocollo è oggi noto come *scambio di chiavi di [Diffie-Helmann](https://planb.academy/resources/glossary/diffiehellman)* o *scambio di chiavi di Diffie-Helmann-Merkle*. [2]
...
Gli schemi basati sui logaritmi discreti si basano su due tipi principali di gruppi ciclici: i gruppi moltiplicativi di numeri interi e i gruppi che includono i punti delle [curve ellittiche](https://planb.academy/resources/glossary/elliptic-curve). Lo scambio di chiavi Diffie Helmann originale, presentato in "New Directions in Cryptography", funziona con un gruppo ciclico moltiplicativo di numeri interi. L'algoritmo di [firma digitale](https://planb.academy/resources/glossary/digital-signature) di Bitcoin e lo schema di [firma Schnorr](https://planb.academy/resources/glossary/schnorr-protocol) recentemente introdotto (2021) si basano entrambi sul problema del logaritmo discreto per un particolare gruppo ciclico di curve ellittiche.
...
I Bitcoin esistono in lotti chiamati output di transazione non spesi (o **[UTXO](https://planb.academy/resources/glossary/utxo)**). Supponiamo di aver ricevuto tre pagamenti su un particolare indirizzo Bitcoin per 2 bitcoin ciascuno. Tecnicamente non avete 6 bitcoin su quell'indirizzo. Al contrario, avete tre lotti di 2 bitcoin che sono bloccati da un problema crittografico associato a quell'indirizzo. Per ogni pagamento effettuato, è possibile utilizzare uno, due o tutti e tre questi lotti, a seconda della quantità necessaria per la transazione.
...
Un esempio di funzione hash molto diffusa è **[SHA-256](https://planb.academy/resources/glossary/sha256)** (secure hash algorithm 256). Indipendentemente dalla dimensione dell'input (ad esempio, 15 bit, 100 bit o 10.000 bit), questa funzione produce un valore hash a 256 bit. Di seguito sono riportati alcuni esempi di output della funzione SHA-256.
