...
**Importante da capire**: il server BTCPay su Umbrel funziona per impostazione predefinita solo sulla rete locale. È possibile creare invoice, accettare pagamenti Lightning e Bitcoin e gestire la contabilità da qualsiasi dispositivo connesso alla rete domestica (computer, smartphone, tablet). Questa configurazione è ideale per la fatturazione di servizi di persona, per la gestione di pagamenti faccia a faccia o per l'utilizzo di BTCPay Server dalla rete locale. D'altra parte, per integrare BTCPay Server in un negozio online accessibile pubblicamente su Internet, sarà necessaria un'ulteriore configurazione con esposizione pubblica (tratteremo questo argomento alla fine del tutorial).

Questo tutorial ti guida attraverso l'installazione completa di BTCPay Server su Umbrel, la configurazione del Bitcoin wallet e nodo Lightning, la creazione e il pagamento delle invoice e la gestione dei rapporti contabili. Scoprirai come utilizzare BTCPay Server in modo efficiente sulla tua rete locale, e poi esamineremo le soluzioni per la visualizzazione pubblica se vuoi integrarlo con un sito di e-commerce.


## Prerequisiti

Per seguire questo tutorial, è necessario che Umbrel sia installato e configurato correttamente. Se non l'hai ancora fatto, consulta il nostro tutorial sull'installazione di Umbrel.

https://planb.academy/tutorials/node/bitcoin/umbrel-8b0e3b5b-d3cf-4a1e-8bb8-1ad2db4dd848

Il tuo nodo Bitcoin Core deve essere completamente sincronizzato con la blockchain (100% nell'applicazione Bitcoin di Umbrel). Questa sincronizzazione iniziale richiede solitamente tra i 3 giorni e le 2 settimane, a seconda dell'hardware e della connessione a Internet.

Per accettare pagamenti istantanei Lightning, è necessario installare LND (Lightning Network Daemon) su Umbrel. Se vuoi abilitare questa funzione, consulta il nostro tutorial sull'installazione e la configurazione di LND su Umbrel.

https://planb.academy/tutorials/node/lightning-network/umbrel-lnd-b12e0b5b-12ff-45f1-978e-62f4b4a8ba16

Prevedi almeno 50 GB di spazio libero su disco per il server BTCPay, i suoi database e i dati Lightning. Si consiglia vivamente di disporre di una connessione Internet stabile tramite cavo Ethernet per evitare disconnessioni.


## Installazione del server BTCPay su Umbrel

Dall'interfaccia Umbrel (`umbrel.local`), accedi all'App Store e cercare "BTCPay Server" nella categoria Bitcoin.

![Interface Umbrel App Store avec BTCPay Server](assets/fr/01.webp)

Fai clic su "Installa". Umbrel controlla automaticamente che Bitcoin Core e LND siano installati, quindi avvia la distribuzione (2-5 minuti).

![Dépendances requises pour BTCPay Server](assets/fr/02.webp)

Una volta installata, apri l'applicazione. È necessario creare un account amministratore con credenziali forti.

![Création du compte administrateur BTCPay Server](assets/fr/03.webp)

Una volta creato il tuo conto, il server BTCPay ti chiederà immediatamente di creare il tuo primo negozio. Scegli un nome commerciale e seleziona una valuta di riferimento (EUR, USD o BTC).
...
Il server BTCPay è accessibile da qualsiasi dispositivo sulla tua rete locale (WiFi o Ethernet). Accedi dal browser a:
...
**Accesso remoto con Tailscale**: per accedere al server BTCPay da qualsiasi parte del mondo, utilizza Tailscale. Questa VPN sicura ti consente di connetterti a Umbrel come se fossi sulla tua rete locale. Consulta il nostro tutorial dedicato a Tailscale su Umbrel.
...
Per configurare il wallet Bitcoin, vai su "Portafogli" > "Bitcoin".

Hai due opzioni: creare un nuovo portafoglio direttamente in BTCPay o importare un portafoglio esistente. Per l'importazione sono disponibili diversi metodi:
- **Collega l'hardware wallet** (consigliato): importare le chiavi pubbliche tramite l'applicazione Vault.
- **Importazione del file del wallet** (consigliato): carica un file esportato dal tuo wallet.
- **Inserisci la chiave pubblica estesa**: inserisci manualmente la tua XPub/YPub/ZPub.
- **Scansiona il codice QR wallet** : scansiona un codice QR da BlueWallet, Cobo Vault, Passport o Specter DIY.
- **Inserisci il seed del wallet** (non consigliato) : immetti la frase di recupero di 12 o 24 parole.

![Options de création de portefeuille](assets/fr/06.webp)

Per questo tutorial, creeremo un nuovo Hot wallet: la chiave privata sarà quindi memorizzata sul nostro server Umbrel. In questo caso, consigliamo vivamente di spostare regolarmente i fondi in un Cold wallet per evitare di immagazzinare grandi quantità sul server.

![Choix entre Hot wallet et Watch-only wallet](assets/fr/07.webp)

Una volta configurato, il server BTCPay conferma che il tuo wallet è pronto ad accettare i pagamenti on-chain.

![Portefeuille Bitcoin configuré avec succès](assets/fr/08.webp)


## Attivare Lightning Network

Per accettare pagamenti istantanei con Lightning, andare su "Portafogli > Lightning". Quindi, poiché il tuo nodo LND è già presente su Umbrel, è sufficiente fare clic sul pulsante "Salva" per convalidare la connessione tra il tuo server BTCPay e il tuo nodo Lightning.

![Configuration du nœud Lightning](assets/fr/09.webp)


## Creare e pagare le invoice

Nell'interfaccia del server BTCPay, vai su "Invoice > Crea Invoice". Immetti l'importo, aggiungi una descrizione opzionale e fai clic su "Crea".

![Création d'une nouvelle facture](assets/fr/10.webp)

È quindi possibile fare clic sul pulsante "Checkout" per visualizzare l'invoice. BTCPay genera quindi un'invoice con un codice QR unificato (BIP21) contenente l'indirizzo Bitcoin e l'invoice Lightning.
...
Una volta pagata, l'invoice diventa "Liquidata" in pochi secondi per Lightning.
...
Nella sezione "Reporting", scheda "Invoice", troverai uno storico completo delle tue fatture con data, importo, stato e metodo di pagamento. Se necessario, è possibile esportarlo.
...
- **Impostazioni generali**: nome del negozio, valuta di riferimento (BTC, EUR, USD), tempo di scadenza dell'invoice (predefinito 15 minuti), numero di conferme blockchain richieste.
- **Tassi di conversione**: configurazione delle fonti dei tassi di cambio e delle conversioni fiat/bitcoin.
- **Aspetto della cassa**: personalizza l'aspetto delle pagine di pagamento (logo, colori, messaggi personalizzati).
...
- Gestione della liquidità Lightning: apertura e gestione di canali con sufficiente capacità in entrata.
...
Questa limitazione della rete locale è il principale ostacolo all'integrazione di BTCPay Server in un negozio di e-commerce, dove i clienti devono poter accedere alle pagine di pagamento da qualsiasi punto su Internet.


## Migliori pratiche e sicurezza

Attiva i backup automatici di Umbrel e conservarne una copia su un supporto esterno (chiavetta USB, disco rigido, cloud crittografato). Conserva i seed Bitcoin (frasi di ripristino) in un luogo sicuro e fisicamente separato. Esegui il backup del file channel.backup di LND per il recupero su Lightning.

Monitora regolarmente la sincronizzazione di Bitcoin Core, i canali Lightning e la risposta del server BTCPay. Un semplice test settimanale: genera e paga un'invoice di qualche satoshi. Mantieni Umbrel aggiornato (patch di sicurezza, miglioramenti). Esegui un backup prima degli aggiornamenti più importanti. Per un uso professionale, considera un monitoraggio esterno (UptimeRobot) con avvisi via e-mail/SMS.
...
Questa configurazione espone il server su Internet e richiede una maggiore vigilanza (password forti, 2FA, aggiornamenti regolari). Prepareremo un tutorial dedicato che illustrerà nel dettaglio questa procedura completa.
...
Che si tratti di un'azienda, di un progetto in fase di avviamento o di un semplice esperimento, il server BTCPay su Umbrel offre una completa autonomia finanziaria. Il percorso inizia con il tuo primo negozio, la tua prima invoice, il tuo primo pagamento ricevuto direttamente nella tua infrastruttura sovrana.
