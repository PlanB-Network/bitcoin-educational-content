...
definition: 量化单个矿工在矿池中贡献的指标。
---
就矿池而言，份额是用于计算单个矿工在矿池中贡献的指标。该指标是计算矿池重新分配给每个矿工的奖励的基础。每个份额对应于一个哈希值，该哈希值满足的难度目标低于比特币网络的难度目标。
...
- 如果哈希值大于或等于矿池为某个份额设定的难度目标，那么这个哈希值就没有用了。矿工就会更改其 nonce，尝试寻找新的哈希值：`哈希 > 份额 > 区块`。
