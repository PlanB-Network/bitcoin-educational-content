...
Seedkeeper es una tarjeta inteligente desarrollada por Satochip, una empresa belga especializada en soluciones de hardware para la gestión y protección de secretos digitales. Conocida por su gama de tarjetas inteligentes para el ecosistema Bitcoin, Satochip diseñó la Seedkeeper como alternativa a los métodos tradicionales de almacenamiento de frases Mnemonic.



En concreto, Seedkeeper adopta la forma de una tarjeta inteligente multifuncional, certificada EAL6, con un procesador seguro y una memoria a prueba de manipulaciones (es decir, una "secure element*"). Como su nombre indica, su función es almacenar frases y contraseñas Bitcoin Mnemonic de forma encriptada y protegida. Con Seedkeeper, puedes generate, importar, organizar y guardar tus secretos directamente en el componente seguro de la tarjeta.
...
Seedkeeper también es muy interesante cuando se combina con un Hardware Wallet sin estado como SeedSigner o Specter DIY. En este caso, no es necesario utilizar el ordenador o el cliente móvil de Satochip. El Seedkeeper guarda el seed en su secure element y se puede utilizar directamente con el dispositivo de firma, eliminando la necesidad de un código QR en papel. No voy a desarrollar este caso de uso particular en este tutorial, ya que es el tema de otro tutorial dedicado :
...
En comparación con una simple copia de seguridad en papel de la frase Mnemonic, utilizar un Seedkeeper tiene varias ventajas:
...
- Repartir el riesgo en dos factores:** puedes dividir la seguridad entre un factor digital y un factor físico. Por ejemplo, si almacenas el PIN de Seedkeeper en tu gestor de contraseñas, necesitarás tanto el acceso a este gestor como la posesión física de la tarjeta inteligente para obtener seed (una probabilidad de ataque significativamente reducida).
...
Sin embargo, hay una serie de desventajas en comparación con una simple copia de seguridad en papel de su seed:
...
- Dependencia de un dispositivo informático de uso general:** Para entrar y gestionar la seed se necesita un ordenador o un teléfono inteligente, lo que significa que su Mnemonic pasa por una máquina con una superficie de ataque mucho más amplia que una Hardware Wallet. Esto puede representar un riesgo si la estación de trabajo se ve comprometida. Por eso no recomiendo utilizar Seedkeeper para almacenar la seed de una Hardware Wallet (excepto para un uso sin ordenador, como con SeedSigner). El papel de la Hardware Wallet es precisamente almacenar la seed en un entorno minimalista y altamente seguro. Al introducir manualmente su seed en su ordenador habitual, ya no queda confinado en Hardware Wallet: también acaba en una máquina de uso general, expuesta a múltiples vectores de ataque. Así que es mejor utilizar Seedkeeper para una Hot Wallet que para una Cold (excepto SeedSigner / stateless Hardware Wallet).





- El riesgo de pérdida vinculado al PIN:** la inaccesibilidad directa del seed, a diferencia de una copia de seguridad en papel, ofrece efectivamente una protección contra el robo físico. Pero como siempre, la seguridad es un acto de equilibrio entre el riesgo de robo y el riesgo de pérdida. Si su copia de seguridad requiere un PIN, la pérdida de este código hará imposible recuperar su frase Mnemonic, y por tanto acceder a sus bitcoins.



A la vista de estas ventajas y desventajas, considero que los mejores usos de Seedkeeper (aparte de su función de gestor de contraseñas) son, por un lado, almacenar semillas de tus **carteras de software**, puesto que ya residen en tu teléfono u ordenador, o de tus Hardware Wallets sin pantalla como el Satochip, y por otro lado, usarlo en combinación con un Hardware Wallet sin estado como el SeedSigner, donde realmente destaca.
...
Seleccione "Importar secreto*". La opción "*Secreto generate*" le permite crear una nueva frase Mnemonic directamente desde la aplicación.
...
*El Mnemonic que aparece en esta imagen es ficticio y no pertenece a nadie. Es sólo un ejemplo. Nunca reveles tu propia Mnemonic a nadie, o te robarán tus bitcoins
...
Si desea comprobar su frase Mnemonic, coja su Seedkeeper y haga clic en el botón "*Click & Scan*".
...
Esto le lleva a una lista de todos sus secretos registrados. En este ejemplo, quiero mostrar la seed de mi cartera "*BLOCKSTREAM App*", así que hago clic en ella.
...
Su frase Mnemonic previamente grabada aparece ahora en la pantalla.
...
¡Eso es todo! Ahora ya sabes como usar Seedkeeper para almacenar la frase Mnemonic de un Bitcoin Wallet. En un próximo tutorial, veremos cómo utilizar Seedkeeper para almacenar tus contraseñas. También te invito a descubrir su uso combinado con SeedSigner :
