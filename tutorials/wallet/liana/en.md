...
Liana is an open-source software package designed for the creation and management of advanced wallets, notably as part of an automated inheritance system or a robust backup mechanism. The project has been developed since 2022 by Wizardsardine, a company co-founded by Kévin Loaec and Antoine Poinsot. On the official website, Liana is presented as "a simple wallet for self-custody, with recovery and inheritance functionalities". The software runs on computers -- Linux, MacOS, Windows -- and its (open) source code is available [on GitHub](https://github.com/wizardsardine/liana).

Liana builds on Bitcoin's programmability to create an advanced wallet. In particular, it takes advantage of *timelocks*, which allow funds to be spent only once a given period of time has passed, and which are involved in the recovery of Bitcoins. A Liana wallet is thus made up of several spending paths:
...
- An enhanced backup with a recovery time, giving the user the possibility of using his wallet without having to keep the corresponding secret phrase and risk having it stolen, during a burglary for example.
...
The main strength of Liana is that it introduces a standardized way to ensure fund recovery in case of loss of the primary key used for everyday spending. This represents a major innovation for self-custody, which involves numerous risks, especially for those less informed on the subject. Liana could encourage more cautious users to stop relying on custodians (such as an exchange platform) to hold their funds and regain ownership of their money, in line with Bitcoin’s cypherpunk ethos.

Of course, Liana does have its drawbacks. The first is that you have to update your wallet on a regular basis by making a transaction on the Bitcoin blockchain. This can be a hassle (depending on how often you use the software) and costly (depending on the level of fees at the time), but it's the price you have to pay for extra security.

A second negative point may be privacy. When you involve another person in the configuration, he or she is privy to all your addresses and can therefore monitor your activity. However, this won't be a problem if you opt for an enhanced backup, or for a succession plan in which your heir has no immediate knowledge of the wallet's details.
...
Visit the official Wizardsardine website and download Liana at https://wizardsardine.com/liana/. You can also download the latest version [from the GitHub repository](https://github.com/wizardsardine/liana/releases), where you can verify the authenticity of the software. The version used in this tutorial is 0.9.
...
Choose your wallet type. If you want to set up an enhanced backup with recovery time, you can select the "*Build your own*" option and opt for the default scheme. The operation will be much the same, except that you won't need to retain the hardware wallet recovery phrase.
...
Next, set the payback delay, i.e. the time after which the funds can be spent by the *inheritance key*. This delay is defined in blocks, with each block separated by an average of 10 minutes. It can range from 10 minutes (1 block) to around 15 months (65,535 blocks). This upper limit is a limitation of the Bitcoin protocol, as the locking time is encoded on 16 bits.
...
The next step is to save your wallet descriptor. This is the information you need to find the funds in your account. Contrary to the mnemonic phrase, the descriptor does not allow you to spend funds, so disclosing it will only pose a privacy problem (the person will know all your transactions).
...
For added security, you can place the components used for recovery in a tamper-evident bag and write down its serial number somewhere. This ensures that no one has accessed it, and that your device remains valid.
...
To better understand the UTXO representation model used in Bitcoin, you can consult the first part of the course on privacy in Bitcoin written by Loïc Morel :
...
To make a charge, go to the "*Send*" tab and enter the essential information: the recipient's BTC address, the amount to be sent and the desired fee rate. A description (saved locally) can also be added for your personal convenience. In our example, we sent 10,000 satoshis to a certain Bob, for a fee rate of 4 sat/vb, or $0.67 at the time of the transaction.
...
The transaction will appear on the home screen and your balance will be updated.
...
A warning will be displayed before your funds (or part of them) expire and can be spent by the recovery key. It will indicate that your "recovery path" will soon be available. Given the shortness of our recovery time (one hour), this message is displayed directly in our case.
...
To update your coins, go to the "*Coins*" tab and click on "*Refresh coin*" in the corresponding coin box. If you have several coins, you should refresh them one by one, and at relatively short intervals, for reasons of privacy. To keep costs down, you can consolidate your funds by sending the entire wallet to a new receiving address, but this will affect your privacy.
...
In our case, we use the Blockstream Jade hardware wallet as a recovery device and choose to skip this step.
...
The transaction should appear on the home screen. Once confirmed, recovery is complete!
...
The descriptor is a human-readable character string that exhaustively describes a set of addresses. It combines a number of essential pieces of information for retrieving the coins (UTXO) of an advanced wallet. The way the descriptor is written is based on [Miniscript syntax](https://bitbox.swiss/blog/understanding-bitcoin-miniscript-part-2/), the script language developed by Andrew Poelstra, Pieter Wuille and Sanket Kanjalkar in 2019.
...
- `[42e629dd/48'/0'/0'/2']`: This is the same element as above (consisting of the fingerprint and the derivation path), except that the fingerprint of the master key of the hardware recovery wallet (in this case the Jade) is indicated.
...
The scripts created by this wallet will take the following form:
