...
这是由Alekos和Bas编写的关于BTCPay服务器操作员的入门课程，由melontwist和asi0按照PlanB课程格式进行改编。

一个未完成的故事

“这是谎言，我对你的信任已破碎，我将让你过时”。
...
让我们从BTCPay服务器是什么以及它的起源开始讲起。我们重视透明度和某些标准，以在比特币领域形成信任。
一个项目违背了这些价值。BTCPay服务器的主要开发者Nicolas Dorier将此事视为个人攻击，并承诺让他们过时。多年后的今天，我们正在朝着这个未来努力，每天都在全力以赴地开源。

> 这是谎言，我对你的信任已破碎，我将让你过时。
> Nicolas Dorier

在Nicolas发表这番话之后，是时候开始建设了。我们现在称之为BTCPay服务器的东西，投入了大量的工作。更多的人想要帮助推动这一进程。最为人所熟知的是r0ckstardev、MrKukks、Pavlenex，以及第一个使用该软件的商家astupidmoose。

什么是开源，以及这样一个项目需要投入什么？

FOSS代表自由和开源软件。前者指的是允许任何人复制、修改甚至分发软件版本（甚至用于盈利）的条款。后者指的是公开分享源代码，鼓励公众贡献和改进它。
这吸引了对贡献他们已经使用并从中获得价值的软件感兴趣的经验丰富的用户，证明了随着时间的推移，相对于专有软件，开源软件在采用方面更胜一筹。这与比特币的理念“信息渴望自由”是一致的。它汇集了充满激情的人们，形成了一个社区，而且简直更有趣。就像比特币，FOSS是不可避免的。

### 在我们开始之前

本课程由多个部分组成。许多部分将由您的课堂老师负责，您将获得访问权限的演示环境，一个为您自己托管的服务器，可能还有一个域名。如果您独立完成此课程，请注意标记为DEMO的环境将不可用。
注意：如果您通过课堂跟随本课程，服务器名称可能会因您的课堂设置而有所不同。由于这个原因，服务器名称中的变量可能会有所不同。

### 课程结构

每个章节都有目标和知识评估。在本课程中，我们将覆盖每个目标，并在每个课程块（即章节）的结尾总结关键特性。为了提供视觉反馈并以视觉方面强化关键概念，特别添加了插图。每个课程块的开始都设定了目标。这些目标不仅仅是一个清单。它们为您提供了进入新技能集的指南。知识评估逐渐增加挑战，设置您的BTCPay服务器。

### 学生在课程中会收到什么？
通过BTCPay Server课程，学生可以理解比特币的基本原理，无论是技术上的还是非技术上的。通过BTCPay Server进行的深入培训将使学生能够操作自己的比特币基础设施。

### 重要的网址或联系机会

允许Alekos和Bas编写此课程的BTCPay Server基金会位于日本东京。可以通过下列网站联系BTCPay Server基金会：
...
这是一个课堂练习，所以如果你自己参加这个课程，你不能执行它，但你仍然可以通过这个练习。完成这项任务的最少人数在9到11人之间。

练习在观看BBC的介绍“比特币和区块链是如何工作的”之后开始。

![比特币和区块链是如何工作的](https://youtu.be/mhE_vvwAiRc)

这个练习至少需要九个人参加。这个练习旨在物理上了解比特币是如何工作的。通过在网络中扮演每个角色，你将以互动和有趣的方式学习。这个练习不涉及闪电网络。
...
- 设置：
  - 见附图/解释器中的Figjam - [活动示意图](https://www.figma.com/file/ckmvMq02Jm2MegSsVCDFhc/Day-1-Classroom-Activity?type=whiteboard&node-id=0-1&t=KR31ofMaJX6S95UL-0)。
  - 获取三名学生志愿者扮演顾客（Alice）、商家（Bob）和银行的角色。
- 演绎事件序列：
  - 顾客- 在线浏览商店并找到一件他们想要的、价值25美元的商品，并告知商家他们想要购买
  - 商家- 请求付款。
  - 顾客- 将卡信息发送给商家
  - 商家- 将信息转发给银行（识别他们自己和身份/信息），请求支付
  - 银行收集有关顾客和商家（Alice和Bob）的信息，并检查顾客的余额是否充足。
  - 从Alice的账户扣除25美元，向Bob的账户添加24美元，收取1美元的服务费
  - 商家从银行获得点头，然后将商品寄给顾客。
- 评论：
  - Bob和Alice必须与银行有关系。
  - 银行收集有关Bob和Alice的识别信息。
  - 银行收取费用。
  - 必须一直信任银行来保管每个参与者的钱。
...
- 设置：
  - 见附图/解释器中的Figjam - [活动示意图](https://www.figma.com/file/ckmvMq02Jm2MegSsVCDFhc/Day-1-Classroom-Activity?type=whiteboard&node-id=0-1&t=KR31ofMaJX6S95UL-0)。
- 用九名学生代替银行，他们将扮演网络中的计算机（比特币节点/矿工）的角色，以取代银行。
- 这九台计算机每一台都拥有所有过去交易的完整历史记录（因此账户余额准确无伪造），以及一套规则：
  - 验证交易是否正确签名（钥匙适合锁）
  - 在网络中向同伴广播和接收有效交易，丢弃无效的（包括任何尝试双重支付的交易）
