...
توجه: در حال حاضر، برج‌های دیده‌بانی فقط خروجی‌های `to_local` و `to_remote` تعهدات لغو شده را ذخیره می‌کنند؛ ذخیره خروجی HTLC در نسخه آینده پیاده‌سازی خواهد شد، زیرا پروتکل می‌تواند برای شامل کردن داده‌های امضای اضافی در بلوک‌های رمزگذاری شده گسترش یابد.
...
- **teos**: شامل یک خط فرمان Interface (CLI) و ویژگی‌های سرور ضروری Watchtower است. دو فایل باینری - **teosd** و **teos-CLI** - زمانی که این **crate** کامپایل می‌شود تولید می‌شوند.
