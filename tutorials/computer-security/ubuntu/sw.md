---
name: Ubuntu
description: Mwongozo kamili wa kusakinisha na kutumia Ubuntu kama njia mbadala ya Windows
---
![cover](assets/cover.webp)

## Utangulizi

Mfumo wa uendeshaji (OS) ni programu kuu inayosimamia rasilimali zote za kompyuta yako. Kuchagua mfumo mbadala wa uendeshaji kama Ubuntu kunaweza kutoa faida nyingi katika masuala ya usalama, gharama na faragha.

### Kwa nini Ubuntu?

- **Usalama ulioimarishwa**: Usambazaji wa Linux unajulikana kwa usalama na uimara wao
- **Gharama sifuri**: Ubuntu na usambazaji mwingi wa Linux ni bila malipo
- **Jumuiya kubwa**: Jumuiya ya watumiaji walio tayari kusaidia kupitia vikao na mafunzo
- **Kuheshimu faragha**: Mfumo wa chanzo wazi kwa uwazi zaidi
- **Urahisi**: Interface ifaayo kwa mtumiaji na urahisi wa kutumia
- **Mfumo tajiri wa ikolojia**: Katalogi pana ya programu huria
- **Usaidizi wa mara kwa mara**: Linda masasisho kutoka kwa Canonical

## Ufungaji na usanidi

### 1. Mahitaji

**Vifaa vinavyohitajika:**

- Kitufe cha USB cha angalau GB 12
- Kompyuta yenye angalau GB 25 inapatikana
  

### 2. Pakua

- Nenda kwa [ubuntu.com/download](https://ubuntu.com/download)
- Chagua toleo thabiti (LTS inapendekezwa)
- Pakua picha ya ISO

![Page de tÃ©lÃ©chargement Ubuntu](assets/fr/01.webp)

![SÃ©lection de la version Ubuntu](assets/fr/02.webp)

### 3. Unda kitufe cha USB cha bootable

Unaweza kutumia zana kadhaa, kama vile Balena Etcher:

- Pakua na usakinishe [Balena Etcher](https://etcher.balena.io/)

![Page de tÃ©lÃ©chargement Balena Etcher](assets/fr/03.webp)

![Installation de Balena Etcher](assets/fr/04.webp)

- Fungua Balena Etcher, kisha uchague picha ya Ubuntu ISO
- Chagua kitufe cha USB kama media lengwa
- Bofya kwenye Flash na usubiri mchakato ukamilike

![Utilisation de Balena Etcher](assets/fr/05.webp)

### 4. Kusakinisha na kulinda Ubuntu

**4.1 Kuwasha kutoka kwa fimbo ya kumbukumbu ya USB** (kwa Kifaransa)

- Zima kompyuta
- Chomeka kitufe cha USB (kilicho na Ubuntu)
- Washa kompyuta yako. Kwenye Kompyuta za hivi karibuni, mfumo unapaswa kutambua kiotomati ufunguo wa boot wa USB. Ikiwa sivyo, fungua upya kwa kushikilia kitufe cha ufikiaji cha BIOS/UEFI (kawaida F2, F12, au Futa, kulingana na chapa)
  
- Katika menyu ya BIOS/UEFI, chagua kitufe chako cha USB kama kifaa cha kuwasha
- Hifadhi na uanze upya

**4.2 Kuzindua usakinishaji** (kwa Kifaransa)

**Skrini ya kuanza**

Unapoanzisha kutoka kwa ufunguo wa USB, utaona skrini hii, ambayo inakuwezesha kuanza Ubuntu.

![Ã‰cran de dÃ©marrage Ubuntu](assets/fr/06.webp)

**Chaguo la lugha**

Chagua lugha unayopendelea kwa usakinishaji na mfumo.

![SÃ©lection de la langue](assets/fr/07.webp)

**Chaguo za ufikivu**

Sanidi chaguo za ufikivu ikihitajika (kisoma skrini, utofautishaji wa juu, n.k.).

![Options d'accessibilitÃ©](assets/fr/08.webp)

**Mipangilio ya kibodi**

Chagua mpangilio wa kibodi yako. Sehemu ya majaribio inapatikana ili kuangalia kama funguo zinalingana na usanidi wako.

![Configuration du clavier](assets/fr/09.webp)

**Muunganisho wa mtandao**

Unganisha kwenye Wi-Fi yako au mtandao wa waya ili kupakua masasisho wakati wa usakinishaji.

![Configuration rÃ©seau](assets/fr/10.webp)

**Aina ya ufungaji**

Chagua kati ya "Jaribu Ubuntu" (kujaribu bila kusakinisha) au "Sakinisha Ubuntu".

![Choix du type d'installation](assets/fr/11.webp)

**Njia ya usakinishaji**

Chagua usakinishaji unaoingiliana.

![Mode d'installation](assets/fr/12.webp)

**Uteuzi wa maombi**

Chagua kati ya usakinishaji chaguo-msingi au uteuzi uliopanuliwa wa programu.

![SÃ©lection des applications](assets/fr/13.webp)

**Programu za wahusika wengine**

Amua ikiwa utasakinisha au kutosakinisha viendeshi vya ziada na programu inayomilikiwa.

![Installation applications tierces](assets/fr/14.webp)

**Aina ya kugawa**

Una chaguzi kuu mbili:

- "Futa diski na usakinishe Ubuntu": hutumia diski nzima kwa Ubuntu
- "Usakinishaji wa mwongozo: unda buti mbili na Windows au ubinafsishe kizigeu chako"

![Choix du partitionnement](assets/fr/15.webp)

**Uundaji wa akaunti ya mtumiaji**

Weka jina lako la mtumiaji na nenosiri kwa akaunti yako ya Ubuntu.


![CrÃ©ation du compte](assets/fr/16.webp)

**Saa za eneo**

Chagua eneo lako la kijiografia ili kuweka wakati wa mfumo.

![SÃ©lection du fuseau horaire](assets/fr/17.webp)

**Muhtasari wa usakinishaji**

Angalia chaguo zako zote kabla ya kuanza usakinishaji wa mwisho. Mara baada ya kubofya "Sakinisha", mchakato huanza.


![RÃ©sumÃ© de l'installation](assets/fr/18.webp)

**4.3 Kuboresha Ubuntu baada ya usakinishaji** (kwa Kifaransa)


Kusasisha mfumo wako ni hatua muhimu baada ya usakinishaji mpya. Una chaguzi mbili:


**Chaguo la 1: Kupitia mtumiaji wa picha Interface**

- Tafuta "Programu na masasisho" kwenye menyu ya programu
- Programu itaangalia kiotomatiki masasisho yanayopatikana
- Fuata maagizo kwenye skrini ili kusakinisha masasisho

**Chaguo la 2: Kupitia Kituo**

- Fungua Kituo (Ctrl + Alt + T)
- Andika amri ifuatayo ili kuangalia sasisho zinazopatikana:

```bash
sudo apt update
```

- Weka nenosiri lako unapoombwa
- Ili kusakinisha masasisho, chapa:

```bash
sudo apt upgrade
```

- Thibitisha usakinishaji kwa kuandika 'Y' kisha Ingiza

### 5. Kugundua kazi za msingi

**5.1 Kuvinjari Mtandao**

Kwa chaguo-msingi, mara nyingi utapata Firefox kwenye upau wa uzinduzi.


Zindua Firefox na uanze kuvinjari.


Vivinjari vingine (Chrome, Jasiri, n.k.) vinaweza kusakinishwa kupitia Kidhibiti Programu au kupitia vifurushi vya .deb.

**5.2 Usindikaji wa maneno**

Ubuntu inakuja na Suite ya LibreOffice (Mwandishi wa usindikaji wa maneno).


Ili kuifungua: Shughuli > Tafuta "Mwandishi wa LibreOffice" au bonyeza kwenye ikoni ikiwa inaonekana kwenye upau.


Unaweza kuunda, kuhariri na kuhifadhi hati katika miundo mbalimbali (ikiwa ni pamoja na .docx).


**5.3 Kusakinisha programu**

Kidhibiti programu (kinachoitwa "Ubuntu Software"): Interface ya picha ya kutafuta na kusakinisha programu.

Kutoka kwa terminal, tumia amri:

```bash
sudo apt install nom-du-paquet
```

Mfano:

```bash
sudo apt install vlc
```

### 6. Hitimisho na rasilimali za ziada

Sasa uko tayari kutumia Ubuntu kila siku: linda mfumo wako, vinjari, fanya kazi za ofisi, sakinisha programu na usasishe Mfumo wako wa Uendeshaji!

Ili kupiga hatua zaidi usalama wa maisha yako ya kidijitali, tunapendekeza uangalie huduma yetu ya ujumbe iliyosimbwa kwa njia fiche, ambayo inafaa kikamilifu kulinda faragha yako na inayosaidia usakinishaji wako wa Ubuntu:

https://planb.network/tutorials/computer-security/communication/proton-mail-c3b010ce-254d-4546-b382-19ab9261c6a2
