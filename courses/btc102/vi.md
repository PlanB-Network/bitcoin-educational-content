---
name: Nhận bitcoin đầu tiên của bạn
goal: Tìm hiểu cách mua, bảo mật và quản lý số bitcoin đầu tiên của bạn một cách độc lập.
objectives: 

  - Nhận diện và phòng tránh những rủi ro phổ biến liên quan đến Bitcoin
  - Hiểu rõ các nguyên lý cơ bản của Bitcoin
  - Chọn chiến lược bảo mật phù hợp nhất với nhu cầu của bạn
  - Tìm hiểu cách thức và địa điểm để kiếm được bitcoin đầu tiên của bạn
  - Chuẩn bị kế hoạch thừa kế để chuyển giao bitcoin của bạn

---
# Hành trình đến với những bitcoin đầu tiên của bạn

Hệ sinh thái Bitcoin đang trải qua sự tăng trưởng ngoạn mục và chưa từng có. Những chuyển đổi về công nghệ, kinh tế và xã hội được thúc đẩy bởi phát minh của Satoshi Nakamoto đang ngày càng mạnh mẽ hơn, và đang dần mở ra cánh cửa đến một thế giới mới mà giờ đây bạn có thể tận dụng tối đa lợi thế của nó.

Khám phá "Hang Thỏ" Bitcoin (Bitcoin Rabbit Hole) là một cuộc phiêu lưu trí tuệ thú vị, kích thích tư duy phản biện và thách thức nhiều niềm tin bấy lâu nay của bạn. Hành trình này không chỉ mang đến cho bạn hiểu biết sâu sắc về cách thức hoạt động của Bitcoin mà còn là cơ hội thực sự để giành lại tự do cá nhân, bảo vệ quyền riêng tư và đạt được chủ quyền tài chính thực sự.

Để hướng dẫn bạn hiệu quả trong suốt hành trình này, tôi đã xây dựng khóa học hoàn toàn miễn phí này. Khóa học này tập trung hoàn toàn vào Bitcoin. Không thảo luận lan man, không đề cập đến các loại tiền điện tử khác, chỉ có nội dung rõ ràng, chính xác và dễ hiểu.

Khóa học này được thiết kế để giúp bạn toàn quyền kiểm soát cách học của mình, cho phép bạn khám phá Bitcoin theo tốc độ của riêng mình, và lựa chọn con đường phù hợp nhất với sở thích và mục tiêu của bạn.

+++
# Giới thiệu

<partId>008c49b7-5e17-5973-87f2-ba28429b2697</partId>

## Tổng quan về khóa học

<chapterId>bfc96999-0ee1-5c41-8297-1b629f50cffc</chapterId>

:::video id=58e578ef-bb3c-423d-8431-0c16db8e5f29:::

Chào mừng bạn đến với BTC102! Nếu bạn đã hoàn thành BTC101, bạn đã khám phá những khái niệm lý thuyết then chốt đằng sau lý do tại sao Bitcoin lại là một công nghệ mang tính cách mạng. Giờ đây, với BTC102, đã đến lúc áp dụng kiến thức đó vào thực tiễn. Khóa học này được thiết kế để giúp bạn xây dựng kế hoạch Bitcoin cá nhân, từng bước một.

Trong các chương tiếp theo, chúng tôi sẽ hướng dẫn bạn các bước thực tế để **nhận được bitcoin đầu tiên**, **bảo mật** chúng đúng cách và tự tin bước vào hệ sinh thái đang phát triển nhanh chóng này.

https://planb.academy/courses/2b7dc507-81e3-4b70-88e6-41ed44239966

Mặc dù Bitcoin đã tồn tại hơn 16 năm, ngành công nghiệp này vẫn còn non trẻ, năng động và gắn liền với tự do. Giao thức cơ bản của nó hoàn toàn phi tập trung và không chịu sự kiểm soát của bất kỳ cơ quan trung ương nào, cho phép hệ sinh thái phát triển một cách tự nhiên.

Mặc dù sự tự do này thúc đẩy một làn sóng đổi mới và cơ hội đáng kinh ngạc, nhưng nó cũng đi kèm với một số rủi ro nhất định, bao gồm lừa đảo, những sai lầm phổ biến và cạm bẫy thường xuất phát từ việc thiếu kiến thức. Mục tiêu chính của khóa học này là giúp bạn định hướng hệ sinh thái mới này một cách tự tin và an toàn.

Để đạt được điều này, khóa học BTC102 được chia thành nhiều phần, mỗi phần tập trung vào một khía cạnh quan trọng trong hành trình của bạn với Bitcoin:

Phần đầu tiên, "Điều kiện tiên quyết để hiểu Bitcoin", sẽ giúp bạn chuẩn bị tốt cho chặng đường phía trước. Bạn sẽ học cách nhận diện những rủi ro chính liên quan đến lừa đảo và gian lận tài chính có thể tồn tại trong hệ sinh thái Bitcoin. Sau đó, chúng ta sẽ tìm hiểu những kiến thức cơ bản thiết yếu về bảo mật trực tuyến.

Cuối cùng, tôi sẽ chia sẻ với bạn một số mẹo thực tế dành riêng cho người mới bắt đầu, nhằm giúp bạn tránh những sai lầm phổ biến khi lần đầu tiếp cận với Bitcoin.

![image](assets/vi/001.webp)

Phần thứ hai, "Hiểu rõ những gì bạn sắp học", sẽ giúp bạn hiểu sâu hơn về Bitcoin. Chúng ta sẽ bắt đầu bằng phần ôn tập để đảm bảo bạn nắm vững những khái niệm cốt lõi.

Sau đó, chúng tôi sẽ giải thích lý do tại sao Bitcoin lại mang ý nghĩa nền tảng từ góc độ công nghệ, kinh tế và xã hội, qua đó củng cố niềm tin của bạn vào giá trị của nó. Bạn cũng sẽ khám phá toàn bộ ngành công nghiệp Bitcoin: các bên tham gia chính, cách hệ sinh thái này được tổ chức, cũng như quá trình phát triển của nó qua nhiều năm. Cuối cùng, chúng ta sẽ tìm hiểu kiến trúc phân lớp của hệ sinh thái Bitcoin - một khái niệm quan trọng để hiểu cách hệ thống này tiếp tục đổi mới mà không ảnh hưởng đến tính toàn vẹn của giao thức cốt lõi.

![image](assets/vi/027.webp)

Phần thứ ba, "Thiết lập kế hoạch của bạn", tập trung vào cách tiếp cận cá nhân của bạn với Bitcoin. Chúng tôi sẽ bắt đầu bằng cách giúp bạn xác định hồ sơ người dùng của mình từ bốn danh mục chính:

- *người nắm giữ* (holder), tập trung vào việc nắm giữ Bitcoin trong dài hạn;
- *người xếp chồng* (stacker), người mua đều đặn và có phương pháp;
- *người dùng* (user), người ưu tiên sử dụng Bitcoin hàng ngày;
- và cuối cùng là *người hoang tưởng* (paranoid), người hướng đến mức độ bảo mật tối đa nhằm bảo vệ tài sản và quyền riêng tư của mình một cách hiệu quả.

Chúng tôi sẽ hướng dẫn bạn xây dựng và triển khai chiến lược phù hợp với hồ sơ cá nhân của mình và giúp bạn đạt được mục tiêu đã đặt ra.

![image](assets/vi/067.webp)

Cuối cùng, phần thứ tư, "Bảo vệ người thừa kế và tài sản của bạn", đề cập đến một chủ đề quan trọng thường bị bỏ qua: làm thế nào để đảm bảo Bitcoin của bạn có thể được chuyển giao cho những người thân yêu trong trường hợp xảy ra sự cố ngoài ý muốn. Bạn sẽ học cách xây dựng và thiết lập một kế hoạch thừa kế Bitcoin, qua đó đảm bảo tài sản của bạn được bảo toàn và tiếp nối ngay cả sau khi bạn qua đời.

![image](assets/vi/095.webp)

Khi hoàn thành khóa học này, bạn sẽ sở hữu đầy đủ các công cụ thực tiễn cần thiết để tự tin tham gia và làm chủ hệ sinh thái Bitcoin, đồng thời trở thành một trong những người dùng am hiểu nhất.

Bạn đã sẵn sàng bắt đầu hành trình khám phá thế giới Bitcoin chưa? Bắt đầu thôi!



# Điều kiện tiên quyết để hiểu Bitcoin

<partId>4481fc4b-0f83-489e-ad07-81517c5d6bad</partId>

## Lừa đảo và gian lận tài chính

<chapterId>8af2948b-2ab5-54c4-862c-3414b8a285a2</chapterId>

:::video id=9f3a7b2e-2c4d-4c1e-8b1f-3a2c1d4e5f6a:::

Hệ sinh thái Bitcoin và môi trường xung quanh vẫn còn khá non trẻ và được quản lý lỏng lẻo, tùy thuộc vào từng quốc gia. Mặc dù sự tự do này mở ra nhiều cơ hội, nhưng nó cũng tạo ra mảnh đất màu mỡ cho các hành vi gian lận tài chính, lừa đảo và nhiều hình thức thao túng khác nhau. Chính vì vậy, chương đầu tiên đóng vai trò đặc biệt quan trọng: việc hiểu rõ những cạm bẫy thường gặp sẽ giúp bạn chủ động tránh xa chúng. Đảm bảo an toàn tài chính của bạn là ưu tiên hàng đầu, bởi một trải nghiệm tiêu cực không chỉ ảnh hưởng đến cá nhân bạn mà còn tác động đến toàn bộ cộng đồng Bitcoin.

### Bitcoin so với các loại tiền điện tử: hiểu rõ sự khác biệt

Trước khi đi sâu hơn, điều quan trọng là phải phân biệt rõ ràng hai "thế giới" có bản chất hoàn toàn khác nhau:

- **Hệ sinh thái Bitcoin**: Tập trung vào ý tưởng về một loại tiền tệ vững chắc (sound money), được xây dựng trên nền tảng phi tập trung mạnh mẽ, khả năng phục hồi dài hạn, quyền riêng tư và chủ quyền cá nhân. Kể từ khi ra mắt vào năm 2009, Bitcoin đã vận hành một cách đáng tin cậy và an toàn, được hỗ trợ bởi một cộng đồng các nhà phát triển tận tâm trên toàn cầu. Đây không phải là một xu hướng nhất thời, mà là một giao thức ổn định và được thiết lập tốt, và được thiết kế để bảo toàn giá trị theo thời gian.

- **Ngành công nghiệp [tiền điện tử](https://planb.academy/resources/glossary/cryptocurrency) (Cryptocurrency)**: Mặt khác, có quy mô lớn hơn rất nhiều và bao gồm hàng chục nghìn dự án khác nhau, mỗi dự án đi kèm với token riêng. Chúng thường bị chi phối bởi tốc độ đổi mới chóng vánh, sự thổi phồng và đầu cơ tài chính ngắn hạn. Nhiều dự án trong số đó mang tính tập trung cao, kém an toàn và không tạo ra nhiều giá trị thực — dù được quảng bá bằng những lời hứa táo bạo và chiến dịch marketing hào nhoáng.

![image](assets/vi/001.webp)

Nếu bạn muốn hiểu rõ hơn về nguồn gốc của Bitcoin và điều gì thực sự khiến nó khác biệt so với các dự án khác, bạn có thể tham khảo khóa học miễn phí chuyên sâu về lịch sử Bitcoin tại đây:

https://planb.academy/courses/a51c7ceb-e079-4ac3-bf69-6700b985a082

Như bạn đã biết, nền tảng Plan ₿ Academy chỉ tập trung vào Bitcoin. Tuy nhiên, việc hiểu rõ sự khác biệt giữa Bitcoin với các loại tiền điện tử khác sẽ giúp bạn tránh được những cạm bẫy liên quan đến các dự án vô ích và đôi khi là lừa đảo.

![image](assets/vi/002.webp)

### Những trò lừa đảo chính cần tránh

Sau đây là những trò lừa đảo phổ biến nhất mà bạn có thể gặp phải trên hành trình của mình:

#### Mô hình kim tự tháp và mô hình Ponzi

Đây là 2 kiểu lừa đảo phổ biến nhất trong thế giới tiền điện tử. Trong mô hình Ponzi, tiền của người tham gia sau được dùng để trả lợi nhuận cho người tham gia trước, chứ không đến từ bất kỳ khoản đầu tư hay sản phẩm thực tế nào. Không có giá trị thực sự nào được tạo ra. Hệ thống chỉ tồn tại khi còn người mới đổ tiền vào, và sẽ sụp đổ khi dòng tiền này dừng lại.

![image](assets/vi/003.webp)

Những trò lừa đảo này thường có đặc điểm:

- Những lời hứa hẹn không thực tế về lợi nhuận được đảm bảo (ví dụ: lợi nhuận được đảm bảo 20%);
- Sự chậm trễ hoặc khó khăn khi bạn cố gắng rút tiền đã đầu tư;
- Tạo động lực mạnh mẽ để người tham gia đi lôi kéo thêm người mới, nhằm duy trì hoạt động của hệ thống;
- Hoàn toàn thiếu minh bạch về nguồn gốc thực sự của lợi nhuận đã hứa.

Sau cùng thì các mô hình kim tự tháp và Ponzi đều sẽ thất bại. Điểm yếu cơ bản của chúng nằm ở việc phải liên tục thu hút các nhà đầu tư mới để trả lợi nhuận cho những người tham gia trước đó. Điều này trở nên bất khả thi về mặt toán học theo thời gian vì số lượng người mới cần tuyển dụng tăng theo cấp số nhân khi hệ thống phát triển. Khi đạt đến điểm giới hạn, những người tham gia bắt đầu nghi ngờ, mất niềm tin, và toàn bộ kim tự tháp sụp đổ. Ở giai đoạn này, những người tham gia cuối cùng, thường là những người ít hiểu biết nhất, sẽ mất toàn bộ khoản đầu tư mà không có cách nào thu hồi lại, trong khi những người tổ chức hoặc nhà đầu tư ban đầu thường đã rút tiền và rời khỏi hệ thống.

Trong thế giới tiền điện tử, các mô hình Ponzi có thể xuất hiện dưới nhiều hình thức, thường được thiết kế để che giấu bản chất gian lận của chúng dưới vỏ bọc công nghệ hoặc tài chính. Những trò lừa đảo này có thể xuất hiện dưới dạng chào bán token mới hoặc "Chào bán Coin Ban đầu" (ICO). Chúng là các hoạt động gây quỹ bằng cách bán một loại tiền điện tử mới ra công chúng. Đằng sau các thuật ngữ kỹ thuật như "[Blockchain](https://planb.academy/resources/glossary/blockchain)", "hợp đồng thông minh" hoặc "staking", một số dự án thực chất đang che giấu các mô hình kim tự tháp phức tạp. Một số dự án khác lại hứa hẹn lợi nhuận cao bằng cách kết hợp các tài sản tiền điện tử không rõ ràng với mô hình trả thưởng chỉ tồn tại khi liên tục có người mới tham gia.

Gần đây hơn, các mô hình Ponzi cũng đã lan rộng sang lĩnh vực Tài chính Phi tập trung (DeFi). DeFi được thiết kế để cung cấp các dịch vụ tài chính không qua trung gian, một số dự án lại lợi dụng nó để tạo ra vẻ ngoài hợp pháp giả tạo cho các trò lừa đảo của chúng. Một số nền tảng DeFi hứa hẹn lợi nhuận cao, và được đảm bảo khi người dùng gửi tiền điện tử vào các giao thức tự động. Những lời hứa hấp dẫn này thường được hỗ trợ bởi các cơ chế mờ ám và không thể xác minh, với các token được tạo ra dành riêng cho mục đích lừa đảo. Trên thực tế, các hệ thống này không có mô hình kinh doanh bền vững—lợi nhuận chỉ đơn giản được trả từ tiền của người dùng mới, giống như một mô hình Ponzi truyền thống. Khi niềm tin bắt đầu bị xói mòn hoặc số lượng người tham gia mới giảm dần, các hệ thống này chắc chắn sẽ sụp đổ, dẫn đến tổn thất đáng kể cho các nhà đầu tư thiếu cảnh giác.

Xin lưu ý rằng nội dung của khóa học này chỉ nhằm mục đích giáo dục và không phải là lời khuyên tài chính. Sự an toàn tài chính của bạn phụ thuộc vào việc bạn luôn thận trọng, biết hoài nghi và nắm rõ thông tin trong mọi quyết định tài chính của bạn.

Cách bảo vệ tốt nhất là luôn đặt câu hỏi đơn giản này: Khoản lợi nhuận được hứa hẹn thực sự đến từ đâu? Nếu câu trả lời không rõ ràng, hãy tránh xa ngay lập tức.

####  Pump & Dump (Thổi giá rồi xả)

Loại lừa đảo này liên quan đến việc thổi phồng giá một tài sản một cách giả tạo - thường là một loại token tiền điện tử có tính thanh khoản thấp - thông qua các chiến dịch quảng bá có tổ chức, thường do một nhóm nhà đầu tư đứng sau điều khiển. Mô hình Pump & Dump điển hình diễn ra như sau:

- Một nhóm người trong nội bộ hoặc những người có ảnh hưởng âm thầm mua vào số lượng lớn tài sản mục tiêu.
- Sau đó họ đẩy mạnh quảng bá, tạo tin đồn, hô hào trên mạng xã hội để gây chú ý và thổi phồng giá.
- Công chúng vì sợ bỏ lỡ cơ hội (FOMO – Fear of Missing Out) đổ xô mua vào, khiến giá tăng cao hơn nữa.
- Khi giá đạt đỉnh, nhóm đứng sau bán tháo toàn bộ số tài sản họ nắm giữ.
- Giá lập tức sụp đổ, và những người mua sau cùng phải gánh chịu thua lỗ nặng.

![image](assets/vi/004.webp)

Điều quan trọng cần lưu ý là chiến lược Pump & Dump là bất hợp pháp ở nhiều quốc gia và bị coi là một hình thức thao túng thị trường. Tuy nhiên, những chiêu trò này vẫn tiếp tục phát triển mạnh, đặc biệt là trong lĩnh vực tiền điện tử, nơi mà việc điều chỉnh pháp luật vẫn đang được tiến hành.

Hãy đặc biệt cẩn thận với các nhóm "tín hiệu" riêng tư trên các nền tảng như Telegram, Discord hoặc các kênh mạng xã hội khác. Những nhóm này thường được điều hành bởi những người có sức ảnh hưởng hoặc tự xưng là chuyên gia, một số thậm chí còn thu phí tham gia. Mặc dù các nhóm này tuyên bố cung cấp các cơ hội đầu tư độc quyền, nhưng thực tế lại không như vậy: chỉ có người tổ chức được hưởng lợi, trong khi hầu hết người tham gia đều mất tiền.

![image](assets/vi/005.webp)

Đúng là một số người tham gia có thể tạm thời hưởng lợi từ những kiểu thao túng thị trường này, nhưng thành công của họ thường chỉ dựa vào may mắn và thời điểm hoàn hảo. Về lâu dài, những kế hoạch này không bền vững. Chúng đòi hỏi sự tham gia liên tục với rủi ro cao và lặp đi lặp lại vào các mô hình gian lận mà chắc chắn sẽ sụp đổ.

Tệ hơn nữa, chúng còn nuôi dưỡng một ảo tưởng nguy hiểm: niềm tin rằng có thể kiếm tiền nhanh chóng và dễ dàng mà không cần hiểu rõ cách thức hoạt động thực sự của hệ thống tài chính. Tư duy này không chỉ đặt cá nhân vào nguy cơ mà còn làm suy yếu uy tín của toàn bộ hệ sinh thái tiền điện tử.

Vì tất cả những lý do này, chiến lược tốt nhất là áp dụng phương pháp đầu tư nghiêm túc và chu đáo, dựa trên nền tảng giáo dục tài chính, hiểu biết vững chắc về các nguyên tắc cơ bản và có tầm nhìn dài hạn.

Bằng cách kiên nhẫn xây dựng kiến thức, bạn sẽ ít bị tổn thương hơn trước sự thao túng cảm xúc và những lời hứa không thực tế; và được trang bị tốt hơn để tránh những cạm bẫy tài chính có thể dẫn đến thua lỗ thực sự.

#### Lừa đảo quyên góp, xổ số và tặng quà giả mạo

Loại lừa đảo này hứa hẹn tặng Bitcoin miễn phí hoặc các phần thưởng khác đổi lại việc bạn gửi trước một khoản tiền nhỏ. Điều quan trọng cần nhớ là: không có cá nhân hoặc tổ chức hợp pháp nào lại yêu cầu bạn gửi tiền điện tử trước với lời hứa sẽ gửi lại cho bạn nhiều hơn.

Những kẻ lừa đảo thường mạo danh những nhân vật nổi tiếng (như Elon Musk hoặc những người nổi tiếng khác) để dụ dỗ nạn nhân thông qua mạng xã hội. Luôn kiểm tra kỹ tính xác thực của các tài khoản và trang web trước khi giao dịch, và đừng bao giờ tin vào những lời đề nghị có vẻ quá hào phóng hoặc quá tốt đến mức khó tin .

Đôi khi, những trò lừa đảo này xuất hiện dưới dạng lừa đảo "trả trước phí". Bạn được hứa hẹn một giải thưởng hoặc phần thưởng (tiền, sản phẩm hoặc dịch vụ) nhưng trước tiên phải trả một khoản phí, được cho là để trang trải các chi phí như vận chuyển, thuế hoặc chi phí giao dịch. Sau khi thanh toán xong, kẻ lừa đảo biến mất và phần thưởng đã hứa sẽ không bao giờ đến tay bạn.

![image](assets/vi/006.webp)

#### Shitcoin và tiền điện tử được cung cấp

Các dự án tiền điện tử tập trung đôi khi cung cấp token miễn phí ("*airdrop*") để thu hút người dùng. Những token này thường có giá trị rất thấp hoặc không có giá trị thực tế và chủ yếu được sử dụng để tạo ảo giác về sự phổ biến hoặc kích thích đầu cơ. Hãy hết sức thận trọng với những loại khuyến mại này; chúng thường là bẫy tiếp thị hơn là cơ hội thực sự.

#### Trộm cắp danh tính và lừa đảo

Kẻ tấn công thường sử dụng các trang web giả mạo, tài khoản mạng xã hội hoặc email lừa đảo để cố gắng đánh cắp tiền của bạn. Những trò lừa đảo này có thể đến từ bất kỳ kênh giao tiếp nào: email, mạng xã hội, cuộc gọi điện thoại, hoặc thậm chí là thư tín truyền thống...


Trước khi nhấp vào liên kết hoặc thực hiện bất kỳ hành động nào, hãy luôn kiểm tra kỹ danh tính người gửi. Nếu nghi ngờ, hãy truy cập trang web theo cách thủ công thay vì sử dụng liên kết được cung cấp. Quan trọng nhất, đừng bao giờ chia sẻ [khóa riêng tư](https://planb.academy/resources/glossary/private-key) hoặc mật khẩu của bạn với bất kỳ ai.

#### Bitcoin Hardfork

Trong những năm qua, Bitcoin đã trải qua một số lần *hard forks*, dẫn đến việc tạo ra các phiên bản thay thế của đồng tiền điện tử gốc. Nói một cách đơn giản, *hard fork* là một sự phân tách trong mạng lưới dẫn đến hai blockchain riêng biệt, cả hai đều có cùng lịch sử cho đến thời điểm phân tách. Những đợt phân tách này thường xảy ra khi một phần của cộng đồng nhà phát triển hoặc hệ sinh thái Bitcoin nói chung muốn đưa ra những thay đổi lớn cho giao thức gốc nhưng không đạt được sự đồng thuận rộng rãi. Thay vì từ bỏ ý tưởng của mình, họ quyết định ra mắt phiên bản Bitcoin mới (với các quy tắc được thay đổi) với hy vọng rằng người dùng và thợ đào sẽ chọn làm theo Fork của họ.

![image](assets/vi/007.webp)

Không phải tất cả các đợt *hard forks* đều là gian lận, vì một số xuất phát từ những bất đồng về kỹ thuật hoặc ý thức hệ trong cộng đồng. Tuy nhiên, một số khác lại xuất phát từ lợi ích thương mại hoặc thậm chí là động cơ không trung thực. Ví dụ nổi tiếng nhất về các đợt hard forks này là **Bitcoin Cash (BCH)** và **Bitcoin Satoshi Vision (BSV)**. Ra mắt lần lượt vào năm 2017 và 2018, các đồng tiền thay thế này thường tự nhận là "phiên bản tốt hơn" của Bitcoin ban đầu. Chúng quảng bá những lợi thế được cho là như phí giao dịch thấp hơn hoặc giao dịch nhanh hơn do kích thước khối tăng lên. Tuy nhiên, những thay đổi kỹ thuật này đi kèm với những đánh đổi đáng kể về bảo mật, tính phi tập trung và độ bền vững; Elements có thể xung đột với các nguyên tắc nền tảng của Bitcoin.

Ngoài những khác biệt về mặt kỹ thuật, các loại tiền tệ thay thế này thường lợi dụng sự nhầm lẫn để thu hút các nhà đầu tư thiếu hiểu biết. Họ có thể sử dụng các chiến thuật tiếp thị được thiết kế để cố tình đánh lừa những người mới tham gia, khiến họ tin rằng họ đang mua Bitcoin (BTC) chính hãng.

Để tránh rơi vào bẫy này, hãy luôn xác minh loại tiền bạn đang mua. Bitcoin ban đầu sử dụng mã giao dịch **BTC**, trong khi Bitcoin Cash và các sản phẩm phái sinh của nó sử dụng các chữ viết tắt riêng biệt, chẳng hạn như BCH hoặc BSV.

#### Những người có ảnh hưởng không trung thực và những bậc thầy giả mạo

Khi tiền điện tử ngày càng được chú ý rộng rãi, mạng xã hội đã chứng kiến sự gia tăng đột biến của những người có sức ảnh hưởng, những chuyên gia tự xưng và những người được gọi là "chuyên gia tiền điện tử". Mặc dù một số ít có thể cung cấp những hiểu biết sâu sắc, nhiều người khác lại lợi dụng sự nổi tiếng của mình để quảng bá các dự án đáng ngờ hoặc các chiến lược giao dịch rủi ro cao (và đôi khi hoàn toàn là lừa đảo). Những cá nhân này thường bị thúc đẩy bởi lợi ích tài chính cá nhân, thường nhận được thù lao trực tiếp hoặc gián tiếp để quảng bá một số token hoặc nền tảng nhất định.

![image](assets/vi/008.webp)

Những người có sức ảnh hưởng này thường dựa vào các chiến thuật đã được chứng minh để thu hút người mới bắt đầu: họ khoe khoang những kết quả tài chính ấn tượng (thường là giả mạo hoặc không thể kiểm chứng), phô trương lối sống xa hoa như một bằng chứng cho thấy thành công của họ, và quảng bá các chiến lược đầu tư "thần kỳ". Mục đích là kích hoạt FOMO - nỗi sợ bỏ lỡ - và thúc đẩy khán giả đưa ra những quyết định bốc đồng và đầu tư liều lĩnh.

Điều quan trọng cần hiểu là hầu hết lời khuyên "miễn phí" từ những người này không bao giờ thực sự miễn phí. Đằng sau vẻ hào phóng thường ẩn chứa một chiến lược được tính toán kỹ lưỡng để dẫn dắt mọi người mua những tài sản đáng ngờ. Và ngay cả khi một số người có sức ảnh hưởng thành thật, việc sao chép kết quả của họ cũng gần như bất khả thi; thành công của họ thường phụ thuộc vào thời điểm cụ thể, kiến thức nội bộ, hoặc những hoàn cảnh đặc biệt mà bạn không thể tiếp cận.

Một số người có sức ảnh hưởng có thể mời bạn tham gia các nhóm riêng tư, trả phí, nơi họ tuyên bố chia sẻ các tín hiệu giao dịch độc quyền hoặc lời khuyên từ người trong cuộc hứa hẹn lợi nhuận nhanh chóng và dễ dàng. Trên thực tế, những nhóm này chủ yếu mang lại lợi ích cho chính những người tổ chức. Họ thường sử dụng người theo dõi như một nguồn thanh khoản, về cơ bản là bán tháo tài sản để trục lợi cá nhân. Người đăng ký thường bị mất tiền vì họ không thể phản ứng nhanh như những người trong cuộc điều phối các giao dịch (xem phần về các chiêu trò bơm & xả).

Trước thực tế này, sau đây là một số thói quen tốt cần áp dụng để tránh rơi vào bẫy của những người có sức ảnh hưởng không trung thực:

- **Hãy hết sức thận trọng với bất kỳ khuyến nghị đầu tư tiền điện tử nào.**
Một người đáng tin cậy và hiểu biết sẽ không bao giờ gây áp lực buộc bạn mua tiền điện tử mà không khuyến khích bạn tự nghiên cứu trước.
- **Các khóa học giao dịch hoặc đầu tư trả phí không phải lúc nào cũng là dấu hiệu của chất lượng.**
Nhiều khóa học này đưa ra các chiến lược rủi ro hoặc quá đơn giản; thường là những thông tin bạn có thể tìm thấy miễn phí trực tuyến.
- **Không có cách nào đảm bảo bạn có thể sao chép giao dịch của người khác và có được kết quả giống hệt.**
Mỗi chiến lược đầu tư đều phụ thuộc vào bối cảnh cá nhân, thời điểm, kiến thức và các yếu tố khác không thể sao chép chính xác được.
- **Đặc biệt hãy cảnh giác với những lời khuyên nghe có vẻ quá tốt để có thể là sự thật.**
Những lời hứa hẹn phi thực tế hoặc lợi nhuận được đảm bảo gần như luôn là dấu hiệu của sự thao túng. Hãy nhớ rằng: không phải cứ ai đó nói điều gì đó một cách tự tin là điều đó sẽ đúng.

Hãy luôn nhớ rằng, bất kỳ ai bạn gặp trong hệ sinh thái tiền điện tử Bitcoin hay rộng hơn đều có mục đích cá nhân; dù được nêu rõ hay ẩn ý. Không có thông tin nào hoàn toàn trung lập. Ngay cả khóa học này, BTC102, cũng có một mục tiêu rõ ràng, *là thúc đẩy sự hiểu biết sâu sắc hơn về Bitcoin.*

Đó là lý do tại sao việc nhận thức rõ động cơ thực sự đằng sau mỗi nội dung bạn tiếp nhận là vô cùng quan trọng. Và đừng bao giờ quên nguyên tắc vàng này của Internet:

*Nếu một sản phẩm hoặc dịch vụ có vẻ miễn phí thì rất có thể bạn chính là sản phẩm đó.*

Cuối cùng, cách phòng thủ tốt nhất của bạn là giáo dục bản thân, thái độ hoài nghi lành mạnh và quan trọng nhất là thói quen tự mình xác minh sự thật.

### Làm thế nào để tránh lừa đảo

**Để đảm bảo an toàn khi trực tuyến, đặc biệt là trong thế giới tiền điện tử, hãy ghi nhớ những nguyên tắc quan trọng sau:**

- Đừng bao giờ tin tưởng một cách mù quáng: "*Đừng tin tưởng, hãy kiểm chứng*";
- Hãy nghi ngờ những khoản lợi nhuận được đảm bảo hoặc cao bất thường;
- Không bao giờ chia sẻ khóa riêng tư của bạn với bất kỳ ai;
- Không gửi Bitcoin (hoặc bất kỳ loại tiền điện tử nào) đến các địa chỉ không xác định;
- Nếu còn nghi ngờ, hãy dừng lại. Hãy lùi lại và suy nghĩ trước khi hành động. Nỗi sợ bỏ lỡ (FOMO) chính là kẻ thù tồi tệ nhất của bạn;
- Tránh các nhóm hoặc cộng đồng giao dịch trả phí đưa ra những lời hứa không thực tế;
- Hầu hết các chương trình tặng thưởng hoặc xổ số "miễn phí" trực tuyến đều là lừa đảo hoặc gian lận nghiêm trọng;
- Về lâu dài, bạn sẽ luôn đạt được nhiều lợi ích hơn bằng cách học hỏi thay vì đánh bạc.

Bảo vệ bản thân khỏi lừa đảo là bước đầu tiên quan trọng để bảo vệ tài sản của bạn; nhưng điều đó là chưa đủ. Duy trì an toàn kỹ thuật số cũng quan trọng không kém. Đó là lý do tại sao, trong chương tiếp theo, chúng ta sẽ tìm hiểu cách tăng cường bảo mật trực tuyến và những biện pháp tốt nhất bạn nên áp dụng để bảo vệ bản thân trong thế giới số.

## Bảo mật trực tuyến

<chapterId>f0873bf2-6a6f-5485-bb7a-d84be14f404d</chapterId>

:::video id=2a1b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d:::

:::id video=2a1b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d:::

Ngay khi bắt đầu tìm hiểu về Bitcoin, bạn sẽ nhanh chóng nhận ra rằng an ninh mạng cơ bản không phải là tùy chọn mà là thiết yếu. Lừa đảo và gian lận chỉ là phần nổi của tảng băng chìm. Việc vô tình mất dữ liệu, phần mềm độc hại và mật khẩu bị xâm phạm cũng có thể gây ra hậu quả nghiêm trọng không kém, đặc biệt là khi bạn tự quản lý khóa của mình.

Chương này hướng dẫn bạn một số bước đơn giản và thiết thực để nâng cao khả năng vệ sinh kỹ thuật số và bảo vệ bản thân trong không gian mạng.

Nếu bạn muốn tìm hiểu sâu hơn, khóa học SEC101 của Plan ₿ Academy cung cấp thông tin chi tiết về các chiến lược an ninh mạng được thiết kế riêng cho người dùng Bitcoin.

https://planb.academy/courses/99c46148-7080-4915-a7e0-9df0e145cd47

### Tại sao an ninh mạng lại quan trọng

Bitcoin cho phép bạn nắm giữ tiền trực tiếp (không cần ngân hàng), không cần trung gian. Tuy nhiên, quyền tự chủ tài chính này đi kèm với trách nhiệm nghiêm trọng: nếu BTC của bạn bị đánh cắp do lỗi bảo mật, sẽ không có cách nào để đảo ngược giao dịch. Không giống như ngân hàng truyền thống, Bitcoin không có bộ phận hỗ trợ, không có khiếu nại gian lận và không có bảo hiểm để bảo lãnh cho bạn.

Hơn nữa, giá trị tài chính của Bitcoin khiến nó trở thành mục tiêu có giá trị cao. Một hacker xâm nhập vào Wallet của bạn có thể ngay lập tức chuyển tiền của bạn sang Address mà chúng kiểm soát (không cần hỏi han, không có cách nào lấy lại được).

### Một máy tính sạch sẽ, cập nhật

Một trong những khía cạnh bị bỏ qua nhiều nhất trong bảo mật cá nhân là luôn cập nhật hệ điều hành và phần mềm. Nhiều bản cập nhật vá các lỗ hổng đã biết có thể bị kẻ tấn công khai thác. Mặc dù một số người tránh cập nhật vì lo ngại vấn đề hiệu suất, nhưng rủi ro thực sự nằm ở việc chạy phần mềm lỗi thời mà tin tặc đã biết cách khai thác.

Bạn cũng nên sử dụng một phần mềm diệt virus đáng tin cậy. Trên Windows, Windows Defender tích hợp thường đủ dùng cho hầu hết người dùng. Nếu bạn thích các tính năng bổ sung hoặc muốn yên tâm hơn, các lựa chọn thương mại như Kaspersky cũng có sẵn. Trên macOS, các mối đe dọa phần mềm độc hại thường ít phổ biến hơn **nhưng không phải là không tồn tại**, vì vậy bạn vẫn nên cẩn thận.

![image](assets/vi/012.webp)

Ngoài việc cập nhật hệ thống và bảo vệ chống vi-rút, hãy hết sức cảnh giác khi tải xuống phần mềm từ các trang web đáng ngờ hoặc các cổng tải xuống được gọi là "phổ thông". Khi cần một công cụ hoặc ứng dụng, hãy luôn truy cập trực tiếp vào nguồn chính thức; điều này giúp giảm đáng kể nguy cơ cài đặt phần mềm độc hại ngụy trang thành phần mềm hợp pháp.

Một thói quen thông minh khác là kiểm tra tính xác thực và tính toàn vẹn của bất kỳ phần mềm nào trước khi cài đặt nó trên máy tính. Nếu bạn không chắc chắn cách thực hiện, đừng lo lắng, chúng tôi có hướng dẫn chi tiết để giúp bạn thực hiện quy trình này:

https://planb.academy/tutorials/computer-security/data/integrity-authenticity-21d0420a-be02-4663-94a3-8d487f23becc

Cuối cùng, hãy sao lưu thường xuyên dữ liệu quan trọng của bạn. Ổ cứng Hard hoặc SSD ngoài là một lựa chọn đáng tin cậy để lưu trữ bản sao dữ liệu phòng trường hợp máy tính bị hỏng đột ngột, bị hack hoặc vô tình xóa. Bạn sẽ phải cảm ơn chính mình sau.

Nếu bạn thích giải pháp đám mây, hãy cân nhắc sử dụng dịch vụ bảo mật như Proton Drive. Chỉ cần đảm bảo rằng bất kỳ tùy chọn nào bạn chọn đều tôn trọng quyền riêng tư của bạn và cung cấp mã hóa mạnh mẽ.

https://planb.academy/tutorials/computer-security/data/proton-drive-03cbe49f-6ddc-491f-8786-bc20d98ebb16

Một chiến lược sao lưu được khuyến nghị rộng rãi là "quy tắc 3-2-1". Quy tắc này được thiết kế để bảo vệ dữ liệu của bạn khỏi mất mát do tai nạn, tấn công mạng hoặc thậm chí là thiên tai.

Ý tưởng này rất đơn giản:

- Giữ **ít nhất 3 bản sao** dữ liệu quan trọng của bạn,
- Lưu trữ chúng trên **ít nhất 2 loại phương tiện khác nhau** (ví dụ: ổ đĩa Hard ngoài và bộ nhớ đám mây),
- Và đảm bảo **1 trong những bản sao đó được lưu trữ ngoài địa điểm** (tách biệt về mặt vật lý với địa điểm chính của bạn).

Phương pháp này mang lại khả năng phục hồi mạnh mẽ và giúp đảm bảo dữ liệu của bạn vẫn tồn tại ngay cả khi có sự cố nghiêm trọng xảy ra.

![image](assets/vi/013.webp)

### Giải pháp cho cơn ác mộng ID

Một trong những lý do lớn nhất khiến mọi người bị hack là sử dụng mật khẩu yếu. Một số lượng lớn người dùng vẫn sử dụng cùng một mật khẩu cho nhiều tài khoản, hoặc chọn các biến thể dễ đoán. Trình quản lý mật khẩu là giải pháp hoàn hảo cho vấn đề này.

Trình quản lý mật khẩu cho phép bạn:

- **Lưu trữ tất cả mật khẩu của bạn một cách an toàn** trong kho lưu trữ được mã hóa
- **Tạo mật khẩu dài, phức tạp và duy nhất** tự động cho mỗi tài khoản
- **Chỉ sử dụng một mật khẩu chính** để truy cập mọi thứ một cách an toàn

Với trình quản lý mật khẩu, bạn sẽ không bao giờ phải nhấp vào nút "Quên mật khẩu" nữa hoặc phải dựa vào thông tin đăng nhập yếu, được sử dụng lại. Hơn nữa, hầu hết các trình quản lý mật khẩu đều đồng bộ hóa liền mạch trên các thiết bị của bạn (máy tính để bàn, điện thoại, máy tính bảng) và thậm chí tự động điền biểu mẫu đăng nhập, giúp việc truy cập an toàn vừa dễ dàng vừa hiệu quả.

![image](assets/vi/011.webp)

Có rất nhiều trình quản lý mật khẩu, nhưng tôi có thể giới thiệu hai lựa chọn đáng tin cậy tùy thuộc vào nhu cầu của bạn. Nếu bạn đang tìm kiếm một trình quản lý mật khẩu dễ sử dụng và đồng bộ hóa liền mạch trên nhiều thiết bị, Bitwarden là một lựa chọn tuyệt vời:

https://planb.academy/tutorials/computer-security/authentication/bitwarden-0532f569-fb00-4fad-acba-2fcb1bf05de9

Nếu bạn muốn lưu trữ mọi thứ cục bộ trên thiết bị của mình, KeePass là một lựa chọn tuyệt vời:

https://planb.academy/tutorials/computer-security/authentication/keepass-f8073bb7-5b4a-4664-9246-228e307be246

### 2FA: bảo vệ kép

Trong Bitcoin, bạn là ngân hàng của chính mình. Điều đó có nghĩa là bạn cũng là đội ngũ bảo mật của chính mình. Ngay cả khi sử dụng mật khẩu mạnh, vẫn không có gì đảm bảo an toàn tuyệt đối - đó là lý do tại sao việc bật xác thực hai yếu tố (2FA) là điều cần thiết.

![image](assets/vi/014.webp)

2FA bổ sung thêm lớp bảo vệ Layer thứ hai bằng cách yêu cầu mã xác thực một lần theo thời gian (thường là 6 chữ số) được tạo bởi các ứng dụng như Google Authenticator hoặc Authy. Vì vậy, ngay cả khi ai đó lấy được mật khẩu của bạn, họ vẫn không thể truy cập tài khoản của bạn nếu không có quyền truy cập vật lý vào điện thoại.

https://planb.academy/tutorials/computer-security/authentication/authy-a76ab26b-71b0-473c-aa7c-c49153705eb7

![image](assets/vi/009.webp)

Khi bật 2FA, hãy đảm bảo lưu khóa khôi phục cho ứng dụng ở nơi an toàn. Điều này sẽ cho phép bạn khôi phục mã nếu bị mất hoặc đổi điện thoại. Mặc dù 2FA qua SMS hoặc email tốt hơn là không có gì, nhưng nó kém an toàn hơn nhiều. Một cuộc tấn công hoán đổi SIM, khi ai đó kiểm soát số điện thoại của bạn, có thể dễ dàng vượt qua lớp bảo vệ này.

![image](assets/vi/010.webp)

Đối với những người muốn nâng cao mức độ bảo mật hơn nữa, các loại khóa vật lý như YubiKey cung cấp mức độ bảo vệ thậm chí còn cao hơn.

https://planb.academy/tutorials/computer-security/authentication/security-key-61438267-74db-4f1a-87e4-97c8e673533e

### Bảo vệ quyền riêng tư của bạn

Quyền riêng tư và an ninh mạng có mối liên hệ chặt chẽ: bạn càng để nhiều thông tin được truy cập miễn phí thì khả năng bạn trở thành mục tiêu càng cao.

**VPN** (*Virtual Private Network - Mạng ảo riêng*) là một bước đơn giản nhưng hiệu quả để che giấu địa chỉ IP Address và mã hóa lưu lượng truy cập internet của bạn. Mặc dù nó không hoàn toàn ẩn danh (vì nhà cung cấp VPN vẫn có thể thấy hoạt động của bạn), nhưng nó sẽ khiến những kẻ muốn theo dõi hoặc theo dõi thói quen duyệt web của bạn gặp khó khăn hơn đáng kể.

Điều quan trọng là chọn một nhà cung cấp VPN đáng tin cậy:

- Không yêu cầu thông tin cá nhân của bạn
- Cho phép thanh toán qua BTC
- Có chính sách nghiêm ngặt không ghi nhật ký

![image](assets/vi/015.webp)

Chúng tôi có một số hướng dẫn về Plan ₿ Academy có thể hướng dẫn bạn thiết lập VPN một cách dễ dàng. Tôi đặc biệt khuyên dùng IVPN hoặc Mullvad:

https://planb.academy/tutorials/computer-security/communication/ivpn-5a0cd5df-29f1-4382-a817-975a96646e68

https://planb.academy/tutorials/computer-security/communication/mullvad-968ec5f5-b3f0-4d23-a9e0-c07a3e85aaa8

Các bước thiết yếu để bảo vệ quyền riêng tư trực tuyến của bạn cũng bao gồm:

- Sử dụng **nền tảng nhắn tin được mã hóa** như Signal, SimpleX hoặc Session;
- Sử dụng các trình duyệt chú trọng quyền riêng tư như Firefox, Brave hoặc Tor (để tăng cường tính ẩn danh);

https://planb.academy/tutorials/computer-security/communication/tor-browser-a847e83c-31ef-4439-9eac-742b255129bb

- Sử dụng **hộp thư an toàn** như ProtonMail;

https://planb.academy/tutorials/computer-security/communication/proton-mail-c3b010ce-254d-4546-b382-19ab9261c6a2

- **Mã hóa** các tệp của bạn bằng các công cụ như Bitlocker (dành cho Windows) hoặc VeraCrypt (có sẵn trên nhiều nền tảng).

https://planb.academy/tutorials/computer-security/data/veracrypt-d5ed4c83-7c1c-4181-95ea-963fdf2d83c5

### Tiến trình từng bước

An ninh mạng có vẻ như là một công việc khổng lồ, và người mới bắt đầu dễ bị choáng ngợp và bỏ cuộc vì nó có vẻ quá phức tạp. Bí quyết là hãy tiếp cận từng bước một. Hãy bắt đầu với những điều đơn giản, chẳng hạn như cài đặt trình quản lý mật khẩu. Hãy dành vài tuần để làm quen với nó, sau đó chuyển sang bước tiếp theo: chẳng hạn như bật xác thực hai yếu tố (2FA) trên một trong các tài khoản của bạn.

Khi bạn tự tin hơn với những công cụ này, bạn sẽ sẵn sàng thêm các phương pháp nâng cao hơn, như sử dụng email phụ, chuyển sang ProtonMail, thiết lập VPN hoặc duyệt web bằng Tor khi cần thiết.

Khi bạn tìm hiểu sâu hơn về thế giới Bitcoin, bạn sẽ nhận thấy rủi ro tăng lên khi giá trị của Wallet tăng lên. Việc xây dựng thói quen bảo mật vững chắc, bảo vệ quyền riêng tư và thiết lập các công cụ phù hợp không chỉ mang lại cho bạn sự an tâm mà còn củng cố chủ quyền mà Bitcoin hướng đến.

Tóm lại: đừng đánh giá thấp an ninh mạng, hãy dành thời gian thiết lập những điều cơ bản và nhớ rằng tính nhất quán là chìa khóa. Nếu không có hệ thống bảo mật số tốt, ngay cả những công cụ tốt nhất cũng sẽ không giúp ích được gì nhiều.

Ngoài ra, hãy nhớ xem [hướng dẫn bảo mật máy tính của chúng tôi](https://planb.academy/tutorials/computer-security) trên Plan ₿ Academy.

![image](assets/vi/016.webp)

Trong chương tiếp theo, tôi sẽ chia sẻ thêm một số mẹo để giúp bạn bắt đầu hành trình Bitcoin một cách tự tin; và tránh xa nhiều cạm bẫy có thể khiến người mới bắt đầu mất cảnh giác.

## Mẹo cho người mới bắt đầu

<chapterId>33134b3f-92c1-5185-afb6-88599e47e801</chapterId>

:::video id=7e9f3b2c-4d1a-4c5e-8b0d-3a2c1d4e5f6b:::

Tham gia Bitcoin có thể rất thú vị; nhưng cũng đi kèm với những rủi ro không hề nhỏ. Thế giới tiền điện tử không giống bất kỳ thế giới nào khác; biến động giá mạnh, công nghệ chưa quen thuộc, và khả năng mất tiền vĩnh viễn do những sai lầm nhỏ hoặc lựa chọn đầu tư kém là những cạm bẫy mà người dùng mới nên lưu ý.

Trong chương này, tôi sẽ chia sẻ một số lời khuyên thực tế và hướng dẫn chung cho bất kỳ ai đang thực hiện những bước đầu tiên, đặc biệt là những người thực hiện giao dịch mua đầu tiên hoặc khám phá thế giới đầu tư tài chính rộng lớn hơn.

Sau đây là những điểm chính mà chúng ta sẽ cùng nhau xem xét:



- Hãy cẩn thận với **[Shitcoin](https://planb.academy/resources/glossary/shitcoin)** và các loại tiền điện tử vô dụng khác;
- Chỉ đầu tư **số tiền bạn có thể chấp nhận mất**;
- Biết sự khác biệt giữa **giao dịch** và **đầu tư**;
- Hãy lưu ý đến **những tác động về thuế** của khoản đầu tư của bạn;
- Bảo vệ **cụm từ khôi phục** của bạn một cách cẩn thận;
- Hãy khiêm tốn và giữ thái độ **kín đáo** (sự kín đáo là một phần của an ninh);
- **Hãy suy nghĩ dài hạn**, nhìn xa hơn và kiên nhẫn (hãy nghĩ về nó như một cuộc chạy marathon chứ không phải chạy nước rút).

![image](assets/vi/017.webp)

### Những sai lầm thường gặp cần tránh

Bitcoin mở cửa cho tất cả mọi người, nhưng điều đó không có nghĩa là bạn nên lao vào mà không chuẩn bị. Dưới đây là một số sai lầm kinh điển mà người mới thường mắc phải:

**Lỗi công nghệ:**

- **Mất cụm từ seed:** Cụm từ khôi phục (thường là 12 hoặc 24 từ) là cách duy nhất để truy cập Bitcoin nếu Wallet của bạn gặp sự cố. Nếu bạn mất cụm từ này, tiền của bạn sẽ mất vĩnh viễn;
- **Lưu trữ bitcoin của bạn trên nền tảng của bên thứ ba:** Nếu tiền của bạn nằm trên một nền tảng tập trung, bạn không thực sự sở hữu chúng. Bạn phải đối mặt với các rủi ro như bị hack, nền tảng bị lỗi, hoặc thậm chí là bị tịch thu quỹ;
- **Bỏ bê quyền riêng tư:** Bảo vệ quyền riêng tư là một phần cốt lõi trong việc bảo vệ tài sản của bạn. Việc công khai số lượng Bitcoin bạn nắm giữ có thể khiến bạn trở thành mục tiêu;
- **Bảo mật trực tuyến không đủ:** Không bảo vệ thiết bị của bạn bằng các biện pháp bảo vệ cơ bản (như cập nhật, mật khẩu mạnh hoặc 2FA) khiến bạn dễ dàng trở thành mục tiêu cho kẻ tấn công; và có thể khiến bạn mất tất cả.

**Sai lầm về tài chính:**

- **Đầu tư nhiều hơn khả năng mất mát của bạn**: Đừng bao giờ vay nợ hoặc dồn tiền thuê nhà vào Bitcoin. Sự ổn định tài chính cơ bản của bạn luôn phải được đặt lên hàng đầu.

- **Không biết sự khác biệt giữa giao dịch và đầu tư**: Giao dịch đòi hỏi thời gian, kỹ năng và sự kỷ luật cảm xúc nghiêm túc. Đầu tư dài hạn dễ dàng hơn nhiều với người mới bắt đầu.

- **Quên chuyện thuế đi**: Mỗi quốc gia đều có luật thuế riêng cho tiền điện tử. Việc bỏ qua chúng có thể dẫn đến những bất ngờ đau đớn về sau.

- **Mắc phải hội chứng FOMO**: Mua hàng bốc đồng vì sợ bỏ lỡ cơ hội thường dẫn đến sai thời điểm và quyết định sai lầm. Kiên nhẫn là đồng minh tốt nhất của bạn.

![image](assets/vi/019.webp)

### Xác định chiến lược đầu tư

Trước khi mua Satoshi đầu tiên, điều quan trọng là phải hiểu lý do tại sao bạn nên đầu tư vào Bitcoin và cách thức đầu tư. Điều này có nghĩa là bạn cần có một kế hoạch tài chính rõ ràng, phù hợp với tình hình cá nhân và mục tiêu dài hạn của mình.

Hãy bắt đầu bằng cách xác định **ngân sách** của bạn một cách chính xác. Đừng chỉ chọn một con số ngẫu nhiên. Hãy dành thời gian tính toán thu nhập hàng tháng, trừ đi các chi phí cố định (như tiền thuê nhà, vay vốn, thuế, tiện ích), cũng như chi phí sinh hoạt hàng ngày (thực phẩm, đi lại, giải trí, v.v.). Phần còn lại chính là khoản tiết kiệm của bạn và bạn chỉ nên cân nhắc đầu tư từ phần này.

Tiếp cận theo cách này đảm bảo bạn không đặt sự an toàn tài chính của mình vào tình thế rủi ro, đặc biệt là trong trường hợp thị trường suy thoái. Một chiến lược chu đáo là nền tảng cho khả năng phục hồi lâu dài.

![image](assets/vi/020.webp)

Sau khi xác định được ngân sách, hãy suy nghĩ về cách bạn muốn đầu tư. Một trong những phương pháp dễ sử dụng nhất cho người mới bắt đầu và được khuyến nghị rộng rãi là Trung bình Chi phí Đô la (DCA), mua một lượng Bitcoin cố định theo các khoảng thời gian đều đặn (hàng tuần, hàng tháng, v.v.). Chiến lược này giúp cân bằng giá mua trung bình theo thời gian và giảm thiểu tác động cảm xúc của biến động giá. Đây là một cách tiếp cận thông minh cho hầu hết mọi người, đặc biệt là người mới bắt đầu.

Sau đó, hãy tự hỏi: Tôi có tầm nhìn xa đến đâu?

Bạn đang muốn nhanh chóng tham gia và thoát khỏi thị trường (giao dịch)? Hay bạn thiên về tư duy dài hạn là nắm giữ Bitcoin trong nhiều năm (hodler)? Nếu bạn là hodler, có lẽ bạn ít quan tâm đến biến động giá hàng ngày và tập trung hơn vào an toàn dài hạn và tự quản lý. Nếu bạn là người giao dịch, bạn sẽ dễ bị ảnh hưởng bởi nhiễu động thị trường ngắn hạn, rủi ro cao hơn và áp lực thường đi kèm với việc ra quyết định nhanh chóng. Không có câu trả lời nào phù hợp cho tất cả mọi người, nhưng việc hiểu rõ phương pháp tiếp cận của riêng bạn sẽ giúp định hướng quyết định của bạn.

Quan trọng nhất là đừng đưa ra quyết định đầu tư dựa trên cảm xúc hay nỗi sợ hãi. Hãy lập kế hoạch trước, viết ra và tuân thủ theo.

Nếu bạn vẫn chưa chắc chắn, **hãy bắt đầu bằng việc học.**

Hãy dành vài giờ khám phá Bitcoin, tham khảo các tài nguyên miễn phí về Plan ₿ Academy, đọc vài cuốn sách, bỏ ra năm euro để dùng thử và xem một số nội dung chất lượng trực tuyến. Hãy luôn tò mò. Bạn càng thoải mái, việc xem lại chiến lược, điều chỉnh cách tiếp cận và tiến về phía trước một cách tự tin sẽ càng dễ dàng.

### Hiểu về sự biến động của BTC

Bitcoin nổi tiếng với những biến động giá mạnh. Biến động 10%, 20%, thậm chí 50% chỉ trong vài ngày không phải là điều bất thường. Đối với người mới, sự biến động này có thể gây hoang mang. Rất dễ bị cuốn theo cơn sốt trong các đợt tăng giá hoặc hoảng loạn trong các đợt suy thoái; cả hai đều thường dẫn đến những quyết định sai lầm, chẳng hạn như bán lỗ.

![image](assets/vi/021.webp)

Đó là lý do tại sao việc **hiểu và chấp nhận sự biến động của Bitcoin** trước khi đầu tư là rất quan trọng. Những biến động giá này không phải là một lỗi, mà là một đặc điểm của một tài sản vẫn đang trong quá trình trưởng thành. Nếu những thăng trầm đột ngột khiến bạn mất ngủ hoặc đưa ra những quyết định dựa trên cảm xúc, rất có thể bạn đã đầu tư quá mức rủi ro mà bạn có thể chấp nhận. Trong trường hợp đó, hãy lùi lại một bước và đánh giá lại chiến lược cũng như khả năng chịu đựng rủi ro của mình. Đừng ngần ngại giảm quy mô vị thế cho đến khi bạn cảm thấy thoải mái hơn.

Trên hết, đừng bao giờ đầu tư nhiều hơn mức bạn có thể chấp nhận mất. Tránh vay tiền để mua Bitcoin (đặc biệt nếu bạn vẫn đang học những kiến thức cơ bản). Một nền tảng vững chắc bắt đầu từ những bước đi được tính toán kỹ lưỡng, chứ không phải những canh bạc liều lĩnh.

### Quản lý và bảo mật Bitcoin Wallet của bạn

Một trong những tính năng mạnh mẽ nhất (và thường bị đánh giá thấp) của Bitcoin là **tự quản lý**. Với Wallet tự quản lý, bạn hoàn toàn chịu trách nhiệm về tiền của mình. Những ví này thường được tạo từ một **cụm từ khôi phục** (còn được gọi là cụm từ seed), một chuỗi gồm 12 hoặc 24 từ cho phép truy cập đầy đủ vào BTC của bạn. Nếu bạn làm mất cụm từ này (hoặc nếu người khác nắm giữ nó), số bitcoin của bạn sẽ mất vĩnh viễn. **Không có hỗ trợ khách hàng. Không có nút đặt lại.**

![image](assets/vi/023.webp)

Đó là lý do tại sao quy tắc vàng trong Bitcoin là:

"***Không phải khóa của bạn, không phải tiền của bạn***". Nếu bạn không tự mình kiểm soát khóa riêng, bạn không thực sự sở hữu Bitcoin của mình. Mặc dù các sàn giao dịch có thể thuận tiện (đặc biệt là khi mới bắt đầu), nhưng họ vẫn giữ khóa cho bạn. Điều đó có nghĩa là tiền của bạn sẽ gặp rủi ro nếu nền tảng bị hack, đóng băng tài khoản hoặc phá sản.

![image](assets/vi/024.webp)

Để tránh rủi ro này, chúng tôi đặc biệt khuyến nghị bạn nên thiết lập **Wallet** của riêng mình, nơi chỉ bạn mới có quyền truy cập vào cụm từ khôi phục. Cụm từ này phải luôn được viết tay và lưu trữ **ngoại tuyến** ở một nơi an toàn. Một số người dùng thậm chí còn **duy trì nhiều bản sao lưu**, được lưu trữ ở các vị trí địa lý riêng biệt để tăng cường bảo mật.

**Không bao giờ lưu trữ cụm từ khôi phục của bạn trên thiết bị được kết nối internet hoặc trên đám mây**.

**Một vụ tấn công hoặc vi phạm dữ liệu có thể dẫn đến mất mát không thể khắc phục.**

Nếu bạn đã sẵn sàng thực hiện Ownership trong Bitcoin và muốn tìm hiểu sâu hơn về các phương pháp hay nhất để đảm bảo cụm từ khôi phục của mình, tôi thực sự khuyên bạn nên xem bài viết này:

https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### Tính bảo mật và kín đáo

Trong thế giới số ngày nay, **sự thận trọng thường bị bỏ qua**; nhưng đó lại là một phần thiết yếu để giữ an toàn, đặc biệt là khi nói đến Bitcoin. Bạn càng chia sẻ cởi mở về tài sản của mình, bạn càng có nhiều khả năng trở thành mục tiêu của những kẻ lừa đảo, tội phạm mạng, hoặc thậm chí là các mối đe dọa truyền thống hơn như tống tiền hoặc đe dọa.

Đã có rất nhiều trường hợp trên khắp thế giới mà những cá nhân được biết là nắm giữ một lượng lớn BTC đã bị bắt cóc hoặc tấn công.

**Tránh khoe khoang về Bitcoin Stash của bạn**; dù là trên mạng xã hội hay thậm chí trong các cuộc trò chuyện thông thường. Việc tiết lộ thông tin tài chính nhạy cảm không mang lại lợi ích gì, và rủi ro là có thật.

Cũng nên **phân chia hoạt động trực tuyến của bạn**. Ví dụ:

- Sử dụng email Address riêng cho mọi thông tin liên quan đến Bitcoin, khác với tài khoản cá nhân hoặc công việc của bạn.
- Hãy cẩn thận với các nỗ lực lừa đảo, liên kết đáng ngờ và các trang web giả mạo bắt chước các nền tảng đáng tin cậy.
- Hãy cảnh giác! Sự thận trọng và cảnh giác thường là biện pháp phòng thủ tốt nhất của bạn.

Nếu bạn đã sẵn sàng tìm hiểu sâu hơn về chủ đề quyền riêng tư Bitcoin, chúng tôi khuyên bạn nên tiếp tục với Khóa học Quyền riêng tư Năm 2 của chúng tôi, nơi bạn sẽ học các kỹ thuật nâng cao hơn để giữ an toàn cho danh tính và hoạt động của mình:

https://planb.academy/courses/65c138b0-4161-4958-bbe3-c12916bc959c

### Tác động về thuế

Mặc dù là một loại tiền tệ phi tập trung, **Bitcoin không được miễn trừ khỏi luật và quy định thuế** của quốc gia bạn. Mỗi khu vực pháp lý có cách tiếp cận riêng về cách đánh thuế lợi nhuận từ tiền điện tử.

Ở một số nơi, lợi nhuận được đánh thuế như thuế thu nhập vốn khi bán. Những nơi khác có thể yêu cầu bạn khai báo mọi giao dịch, và một số nơi áp dụng các quy tắc ít phổ biến hơn, chẳng hạn như thuế tài sản hoặc đóng góp xã hội.

![image](assets/vi/018.webp)

Trước khi thực hiện bất kỳ giao dịch quan trọng nào, bạn nên tham khảo ý kiến chuyên gia thuế hoặc xem lại hướng dẫn chính thức của chính phủ. Việc dành thời gian tìm hiểu trước các nghĩa vụ thuế có thể giúp bạn tránh được những vấn đề bất ngờ sau này (như tiền phạt, kiểm toán hoặc hình phạt), đặc biệt nếu bạn đang lên kế hoạch bán hàng quy mô lớn hoặc tái phân bổ danh mục đầu tư.

![image](assets/vi/022.webp)

### Sự khác biệt giữa giao dịch, đầu tư và nắm giữ

Bitcoin thường bị bao quanh bởi những quan niệm sai lầm phổ biến; một trong những quan niệm phổ biến nhất là ý tưởng rằng đây là con đường nhanh chóng để làm giàu thông qua giao dịch. Tuy nhiên, điều quan trọng là phải hiểu rõ sự khác biệt giữa giao dịch, đầu tư và nắm giữ, vì mỗi phương pháp đều đi kèm với tư duy, kỹ năng và mức độ rủi ro riêng.

- **Giao dịch:**

Nói một cách trung thực thì: **có lẽ bạn không nên giao dịch.**

Giao dịch liên quan đến việc đầu cơ ngắn hạn (đôi khi có đòn bẩy) nhằm mục đích kiếm lời từ biến động giá của Bitcoin. Nghe có vẻ hấp dẫn, nhưng giao dịch thành công đòi hỏi kiến thức kỹ thuật chuyên sâu (như phân tích biểu đồ và quản lý rủi ro), kỷ luật cảm xúc và sự chú ý liên tục đến thị trường. Việc này rất tốn thời gian và gây áp lực tinh thần, và sự thật là **hầu hết người mới bắt đầu đều thua lỗ** vì họ đánh giá thấp mức độ khó khăn thực sự của nó. Như Warren Buffett đã từng nói: "**Nếu bạn không muốn nắm giữ một cổ phiếu trong mười năm, thì đừng nghĩ đến việc nắm giữ nó trong mười phút**".

Bitcoin không phải là một công cụ làm giàu nhanh chóng.

- **Sự đầu tư:**

Các nhà đầu tư có tầm nhìn trung và dài hạn, mua Bitcoin với niềm tin rằng giá trị của nó sẽ tăng theo thời gian (vài tháng, vài năm, thậm chí vài thập kỷ). Tất nhiên, vẫn có rủi ro, vì giá của Bitcoin có thể biến động đáng kể. Tuy nhiên, cách tiếp cận này nhìn chung bình tĩnh và thực tế hơn nhiều đối với hầu hết mọi người, đặc biệt là những người không muốn dành hàng giờ đồng hồ dán mắt vào biểu đồ mỗi ngày.

- **Giữ (HODL):**

"HODL" ban đầu là lỗi đánh máy của từ "hold" (giữ) và nhanh chóng trở thành một phần của văn hóa Bitcoin. Ngày nay, nó là một huy hiệu danh dự.

Những người nắm giữ luôn tham gia vào cuộc chơi dài hơi; đôi khi là mười năm hoặc hơn. Họ cất giữ Bitcoin một cách an toàn và chỉ đơn giản là chờ đợi, với niềm tin mạnh mẽ vào tiềm năng dài hạn của Bitcoin. Họ không hề nao núng trước những biến động giá hàng ngày hay thị trường giá xuống. Tư duy của họ rất đơn giản: tích lũy, bảo toàn và giữ vững.

|          | Giao dịch | Đầu tư | Nắm giữ |
| ---------------------- | ----------- | -------------- | --------------- |
| Đòn bẩy | Có | Không | Không |
| Khung thời gian | Ngắn hạn | Trung hạn | Rất dài hạn |
| Loại tài sản | Hợp đồng | BTC thực | BTC thực |
| Mức độ rủi ro | Rất cao | Cao | Cao |
| Độ khó | Rất khó | Khó | Khó |
| Đường cong học tập | Dài | Dài | Dài |
| Khả năng thua lỗ | Không giới hạn | Có giới hạn | Có giới hạn |
| Phù hợp với | Một số rất ít người có kinh nghiệm | Phần lớn mọi người | Người tin vào dài hạn |

### Tiếp tục học hỏi

Không bao giờ là quá sớm (hay quá muộn) để bắt đầu tìm hiểu về tiền bạc, đầu tư và cách thức vận hành thực sự của hệ thống tài chính. Bạn không cần phải trở thành chuyên gia hay đào sâu vào mọi chi tiết kỹ thuật; chỉ cần có một cái nhìn tổng quan, vững chắc là đủ để đưa ra quyết định sáng suốt và tránh bị đánh lừa bởi các sản phẩm tài chính không phục vụ lợi ích của bạn (thường được quảng cáo bởi các ngân hàng hoặc cố vấn).

Một điểm khởi đầu tuyệt vời là cuốn sách "Cha Giàu, Cha Nghèo" của Robert T. Kiyosaki. Cuốn sách nổi tiếng với phong cách dễ hiểu và những bài học nền tảng; chẳng hạn như hiểu được sự khác biệt giữa tài sản và nợ phải trả, và tại sao giáo dục tài chính lại là chìa khóa cho sự độc lập lâu dài.

Nếu bạn đã sẵn sàng tìm hiểu sâu hơn, các podcast như *The Investors Podcast* cung cấp những cuộc thảo luận sâu sắc về đầu tư, thị trường và các nguyên tắc kinh tế. Thỉnh thoảng, họ cũng đề cập đến Bitcoin, biến nó thành một bước tiếp theo vững chắc cho những ai tò mò muốn hiểu Bitcoin phù hợp với bối cảnh tài chính rộng lớn hơn như thế nào.

![image](assets/vi/025.webp)

### Những quy tắc vàng cần ghi nhớ

Tóm lại, đây là một số nguyên tắc vượt thời gian mà mọi người dùng Bitcoin (đặc biệt là người mới bắt đầu) nên ghi nhớ:

- **Nguyên tắc số 1**: Đừng bao giờ đầu tư nhiều hơn mức bạn có thể chấp nhận mất. Bitcoin là một tài sản biến động. Đừng mạo hiểm sự ổn định tài chính của bạn để theo đuổi lợi nhuận. Nhu cầu thiết yếu và sự an tâm của bạn luôn phải được đặt lên hàng đầu.
- **Nguyên tắc số 2**: Đừng mù quáng chạy theo những lời quảng cáo thổi phồng hay tin vào những lời khuyên thần kỳ. Hãy bỏ qua các xu hướng và những lời hứa hẹn hào nhoáng. Thay vào đó, hãy tập trung vào việc đưa ra những quyết định sáng suốt và hợp lý. Khi còn phân vân, hãy suy nghĩ kỹ; hãy nói chuyện với những người bạn tin tưởng. Tốt hơn hết là hành động chậm rãi và thận trọng hơn là vội vàng mắc phải những sai lầm tốn kém.
- **Nguyên tắc số 3**: Xây dựng kế hoạch và bám sát tầm nhìn dài hạn.

Sự kiên trì, nhẫn nại và kỷ luật sẽ đưa bạn tiến xa hơn những hứng thú nhất thời. Đừng đặt mục tiêu quá cao xa; hãy hướng đến sự tăng trưởng bền vững. Tránh những sai lầm chết người và để những thành quả nhỏ tích lũy theo thời gian.

Bằng cách tuân thủ những nguyên tắc này, bạn sẽ có thể tiếp cận đầu tư theo mô hình Bitcoin một cách rõ ràng và an tâm hơn. Đúng vậy, Bitcoin khá biến động và ban đầu có thể khiến bạn e ngại; nhưng khi tiếp cận với sự thận trọng, kiên nhẫn và tư duy thực tế, nó sở hữu tiềm năng không thể phủ nhận. Hãy dành thời gian để tích lũy kiến thức, xem xét lại chiến lược khi cần thiết, và trên hết, hãy nhớ rằng: tiến triển chậm mà chắc sẽ luôn tốt hơn là vội vàng vì sợ hãi hoặc thiếu kiên nhẫn.

# Hiểu rõ những gì bạn đang làm

<partId>a42355a3-9dd8-57ed-b590-32a333fe09ea</partId>

## Bitcoin trong 5 phút

<chapterId>ae122ad9-9b4d-5229-9038-e1b99d5cfc83</chapterId>

:::video id=3c2d1e4f-5a6b-7c8d-9e0f-1a2b3c4d5e6f:::

Trong khóa học này, mục tiêu chính là hướng dẫn bạn cách mua và bảo mật những đồng Bitcoin đầu tiên. Tuy nhiên, trước khi đi sâu vào các bước thực tế (cách mua BTC, nên sử dụng ví nào, v.v.), điều quan trọng là hãy lùi lại một bước và hiểu rõ Bitcoin thực sự là gì. Việc nắm bắt được bản chất sâu sắc hơn của Bitcoin sẽ giúp bạn hiểu tại sao hàng triệu người đang chuyển sang công nghệ này, và tại sao việc hiểu rõ những kiến thức cơ bản trước khi đầu tư tiền tiết kiệm vào một loại tài sản mới mẻ và biến động như vậy lại vô cùng quan trọng.

Qua nhiều năm, Bitcoin đã được công nhận là một mạng lưới tiền tệ toàn cầu. Nó được gọi là "vàng kỹ thuật số", "giao thức tin cậy", và thậm chí là "hệ thống tài chính song song". Nhưng những thuật ngữ này thực sự có nghĩa là gì? Để trả lời câu hỏi này, chúng ta sẽ xem xét kỹ hơn cốt lõi của Bitcoin: câu chuyện khởi nguồn, nền tảng kỹ thuật, đặc tính tiền tệ và tác động tiềm tàng của nó; không chỉ đối với cá nhân mà còn đối với toàn bộ hệ thống tài chính toàn cầu.

### Nguồn gốc: Một dự án ra đời từ một hành trình dài

#### Cypherpunks và việc phát minh ra một hệ thống vượt ra ngoài tầm kiểm soát của ngân hàng

Bitcoin không phải sinh ra trong một sớm một chiều. Nó là kết quả của nhiều thập kỷ nghiên cứu và thử nghiệm về [mật mã học](https://planb.academy/resources/glossary/cryptography), khoa học máy tính và lý thuyết tiền tệ. Trước khi ra mắt vào năm 2009, một số dự án (như eCash, b-money, Bit Gold và RPOW) đã cố gắng tạo ra tiền kỹ thuật số. Tất cả đều phải đối mặt với cùng một vấn đề cốt lõi: làm thế nào để ngăn chặn chi tiêu gấp đôi trong một hệ thống phi tập trung mà không cần dựa vào một cơ quan quản lý trung ương.

Thách thức đó vẫn chưa được giải quyết cho đến cuối năm 2008, khi một người dùng tên Satoshi Nakamoto công bố [sách trắng](https://planb.academy/resources/glossary/white-paper) Bitcoin. Vài tháng sau, phiên bản đầu tiên của phần mềm nguồn mở Bitcoin đã ra mắt, khởi động một hệ thống có thể hoạt động độc lập với các ngân hàng hoặc chính phủ.

Dự án chịu ảnh hưởng sâu sắc từ tinh thần của [Cypherpunk](https://planb.academy/resources/glossary/cypherpunks); một cộng đồng các nhà phát triển và tư tưởng gia tin tưởng vào việc sử dụng mật mã để bảo vệ quyền tự do cá nhân trực tuyến. Đối với họ, quyền riêng tư và phi tập trung không phải là những ưu tiên kỹ thuật, mà là những nhu cầu về mặt tư tưởng. Bitcoin nổi lên như hiện thân thành công nhất của những lý tưởng đó: một mạng lưới tiền tệ ngang hàng mà bất kỳ ai cũng có thể sử dụng, không ai có thể kiểm soát, và mọi người đều có thể xác minh.

![image](assets/vi/035.webp)

Cypherpunks là một cộng đồng quốc tế phi chính thức, ủng hộ việc sử dụng mật mã để bảo vệ quyền tự do cá nhân trực tuyến. Họ tin tưởng mạnh mẽ vào quyền riêng tư của mỗi cá nhân; đặc biệt là trong một thế giới ngày càng bị chi phối bởi sự giám sát của chính phủ và khai thác dữ liệu của doanh nghiệp.

Nguồn gốc của phong trào Cypherpunk bắt nguồn từ đầu những năm 1990, khi các nhóm mật mã học, lập trình viên và những người theo chủ nghĩa tự do bắt đầu khám phá những hàm ý chính trị của mật mã trong các buổi gặp mặt ở Thung lũng Silicon. Một trong những tiếng nói nổi bật nhất trong cộng đồng là Tim May, tác giả của Tuyên ngôn Vô chính phủ Mật mã năm 1988; một văn bản nền tảng phác thảo tầm nhìn về một thế giới nơi mã hóa sẽ trao quyền cho các cá nhân hoạt động ngoài tầm với của chính phủ và sự kiểm soát tập trung.

Một cột mốc quan trọng trong phong trào này là vào năm 1992 với việc thành lập danh sách gửi thư Cypherpunks, một diễn đàn nơi các ý tưởng, dự án và thảo luận chính trị về quyền riêng tư và mật mã có thể phát triển mạnh mẽ. Sau đó, vào năm 1993, Eric Hughes đã công bố Tuyên ngôn của Cypherpunk, một tuyên bố ngắn gọn nhưng mạnh mẽ, thể hiện rõ ràng sứ mệnh và niềm tin của cộng đồng.

![image](assets/vi/032.webp)

Ý tưởng về một loại tiền kỹ thuật số hoạt động độc lập với bất kỳ cơ quan trung ương nào (như Bitcoin) bắt nguồn sâu xa từ triết lý của Cypherpunk.

#### Khoảnh khắc hậu khủng hoảng tài chính

Bitcoin không phải tự nhiên mà có. Nó được tạo ra vào một thời điểm rất cụ thể - ngay sau cuộc khủng hoảng tài chính toàn cầu năm 2008. Sự sụp đổ của thị trường nhà đất Hoa Kỳ và cuộc khủng hoảng cho vay dưới chuẩn đã khiến các ngân hàng lớn phá sản và làm lung lay niềm tin của mọi người vào toàn bộ hệ thống tài chính.

Chính trong bối cảnh đầy sợ hãi và bất định này, Bitcoin đã ra đời. Người sáng tạo, được biết đến với cái tên Satoshi Nakamoto, đã đưa vào một thông điệp mang tính biểu tượng ngay trong khối đầu tiên của Bitcoin, Blockchain, hay còn gọi là [khối Genesis](https://planb.academy/resources/glossary/genesis-block). Thông điệp đó là:

> **"The Times 03/01/2009 Bộ trưởng Tài chính sắp tung ra gói cứu trợ thứ hai cho các ngân hàng"**

Đây không chỉ là một ngày tháng hay một ghi chú kỹ thuật; đó là một cuộc phản đối lặng lẽ nhưng mạnh mẽ. Nó cho thấy Bitcoin được thiết kế để trở thành một điều gì đó hoàn toàn khác biệt: một hệ thống tài chính không phụ thuộc vào ngân hàng, các gói cứu trợ hay quyết định của chính phủ.

![image](assets/vi/034.webp)

Nhiều người hiểu đây là mục tiêu của Bitcoin: cung cấp một phương thức chuyển giao giá trị mà không cần đến trung gian, được kiểm soát bằng các quy tắc rõ ràng thay vì các quyết định thường không rõ ràng của các ngân hàng trung ương hoặc chính phủ.

![image](assets/vi/033.webp)

Để giúp bạn hiểu sâu hơn về nguồn gốc của Bitcoin, chúng tôi cung cấp khóa đào tạo miễn phí, toàn diện và có tài liệu đầy đủ về chủ đề này:

https://planb.academy/courses/a51c7ceb-e079-4ac3-bf69-6700b985a082

### Một mạng lưới phi tập trung để chuyển giao giá trị

#### Ngang hàng và không có cơ quan trung ương

Bitcoin được định nghĩa là "hệ thống tiền điện tử ngang hàng". Điều này có nghĩa là bất kỳ ai cũng có thể kết nối với mạng bằng phần mềm phù hợp (một [nút](https://planb.academy/resources/glossary/node) Bitcoin) và tương tác trực tiếp với những người dùng khác mà không cần dựa vào máy chủ trung tâm. Mục tiêu của sự phân quyền này là ngăn chặn bất kỳ thực thể đơn lẻ nào (chẳng hạn như ngân hàng, chính phủ hoặc tập đoàn lớn) kiểm soát, kiểm duyệt hoặc dừng hệ thống. Bitcoin hoạt động 24/7, trên toàn cầu và mọi người đều có thể truy cập mà không cần bất kỳ điều kiện nào.

![image](assets/vi/036.webp)

Nói một cách đơn giản, mỗi người tham gia mạng lưới Bitcoin (được gọi là "nút") đều có một bản sao đầy đủ của sổ cái giao dịch, được gọi là Blockchain. Khi một giao dịch mới xảy ra, nó sẽ được phát sóng lên mạng lưới. Sau đó, các [thợ đào](https://planb.academy/resources/glossary/miner) xác nhận các giao dịch này bằng cách nhóm chúng thành các khối, sau đó được thêm vào cuối chuỗi (do đó có tên là "Blockchain").

![image](assets/vi/029.webp)

#### Blockchain: một sổ kế toán (accounting ledger)

Hãy tưởng tượng Blockchain như một sổ kế toán khổng lồ, trong đó mỗi dòng đại diện cho một giao dịch. Trong hệ thống ngân hàng truyền thống, cơ sở dữ liệu được lưu trữ trên máy chủ của ngân hàng, nơi có thể thực hiện thay đổi bất cứ lúc nào họ muốn. Mặt khác, trong Bitcoin, **tất cả các thay đổi đều được xác thực trên toàn bộ mạng**: một khi một khối giao dịch mới được thêm vào Blockchain, gần như không thể thay đổi nó sau này. Cơ chế xác thực phi tập trung này giúp sổ cái của Bitcoin an toàn và minh bạch.

![image](assets/vi/027.webp)

### Vai trò của thợ mỏ và Proof-of-Work

#### Cách tạo khối: Mining

[Mining](https://planb.academy/resources/glossary/mining) là quá trình mà máy tính (hoặc các trang trại Mining lớn) đóng góp **sức mạnh tính toán** để bảo mật lịch sử giao dịch của Bitcoin và tạo ra các khối mới. Các thợ đào cạnh tranh để giải một bài toán—cụ thể là tìm ra một va chạm Hash cục bộ. Quá trình này đòi hỏi rất nhiều năng lượng và tài nguyên. Khi Miner tìm ra lời giải hợp lệ, chúng sẽ phát khối đó lên mạng lưới, mạng lưới sẽ xác minh và chấp nhận nó là hợp lệ.

Như một phần thưởng, Miner sẽ nhận được bitcoin mới được tạo ra (gọi là trợ cấp theo khối) cùng với phí giao dịch từ tất cả các giao dịch có trong khối đó.

#### Halving: Giảm trợ cấp khối

Để đảm bảo tính khan hiếm của Bitcoin, phần thưởng khối được lập trình để giảm một nửa sau mỗi 210.000 khối; khoảng bốn năm một lần. Sự kiện này được gọi là "Halving". Khi Bitcoin ra mắt, thợ đào kiếm được 50 BTC cho mỗi khối. Vào năm 2025, phần thưởng đó đã giảm xuống còn 3,125 BTC và sẽ tiếp tục giảm theo thời gian.

Cuối cùng, vào khoảng năm 2140, khoản trợ cấp sẽ bằng 0, vì tổng số Bitcoin sẽ đạt mức tối đa là 21 triệu đồng. Đường cong phát hành có thể dự đoán được này mô phỏng sự khan hiếm của các hàng hóa vật chất như vàng; một lý do khiến Bitcoin thường được gọi là **vàng kỹ thuật số**.

![image](assets/vi/031.webp)

### Bitcoin Thuộc tính tiền tệ

#### Sự khan hiếm và chính sách tiền tệ cố định

Một trong những tính năng mạnh mẽ nhất của Bitcoin là *chính sách tiền tệ có thể dự đoán và không thể thay đổi*. Không giống như các loại tiền tệ truyền thống (như đô la, euro hoặc yên), vốn có thể được các ngân hàng trung ương in ấn tùy ý (thường dẫn đến [lạm phát](https://planb.academy/resources/glossary/inflation) hoặc biến động kinh tế), Bitcoin hoạt động theo một bộ quy tắc minh bạch được nhúng trong mã nguồn của nó.

Sẽ chỉ có 21 triệu bitcoin và tốc độ phát hành tiền mới sẽ được mọi người trong mạng lưới biết trước.

Không chính phủ, tổ chức hay cá nhân nào có thể đơn phương thay đổi giới hạn Supply này hoặc các quy tắc phân phối. Cách duy nhất để thay đổi các thông số này là thay đổi giao thức Bitcoin; và ngay cả điều đó cũng cần sự [đồng thuận](https://planb.academy/resources/glossary/consensus) từ đa số các bên tham gia kinh tế của mạng lưới.

Sự khan hiếm cố hữu này là một điểm thu hút lớn đối với những người muốn tránh các chính sách tiền tệ khó lường hoặc tránh sự xói mòn dần dần sức mua của họ do lạm phát. Theo thời gian, điều này có thể đại diện cho một sự thay đổi trong tư duy tài chính, khi việc tiết kiệm bằng một tài sản giảm phát như Bitcoin trở nên hấp dẫn hơn so với việc dựa vào các loại tiền tệ truyền thống dễ bị lạm phát.

#### Khả năng chia hết và khả năng tiếp cận

Một trong những điểm mạnh bị đánh giá thấp nhất của Bitcoin là khả năng phân chia. Mỗi Bitcoin có thể được chia nhỏ thành 100 triệu đơn vị, được gọi là satoshi (hay gọi tắt là Sats). Điều này có nghĩa là bạn không cần phải chi hàng chục nghìn euro hay đô la để bắt đầu; bạn có thể mua Bitcoin chỉ với vài euro, chia nhỏ thành các phần nhỏ.

![image](assets/vi/026.webp)

### Sự cởi mở và minh bạch

#### Một giao thức công khai, có thể xác minh được bởi tất cả mọi người

Bitcoin chạy trên một giao thức [mã nguồn mở](https://planb.academy/resources/glossary/foss) công khai (đáng chú ý nhất là thông qua [Bitcoin core](https://github.com/Bitcoin/Bitcoin)). Điều này có nghĩa là mã nguồn của nó được cung cấp miễn phí cho bất kỳ ai muốn kiểm tra, đánh giá và cải thiện. Không có cơ chế ẩn hay hệ thống đóng nào; mọi thứ về cách thức hoạt động của Bitcoin đều được công khai.

Mức độ minh bạch này khiến việc cài đặt backdoor hoặc thực hiện các thay đổi bí mật trở nên cực kỳ khó khăn. Bất kỳ ai có kỹ năng kỹ thuật đều có thể vận hành một nút, đóng góp vào quá trình phát triển hoặc xây dựng các công cụ tương thích. Trong Bitcoin, niềm tin được xây dựng thông qua mã nguồn và sự đồng thuận, chứ không phải thông qua kiểm soát tập trung.

![image](assets/vi/030.webp)

Tính minh bạch này là một trong những lý do chính khiến mọi người tin tưởng giao thức Bitcoin; nó ngăn chặn một nhóm nhỏ các nhà phát triển thao túng mạng lưới vì lợi ích riêng. Bitcoin hoạt động dựa trên một nguyên tắc đơn giản nhưng mạnh mẽ: nếu bạn không đồng ý với những thay đổi được đề xuất, bạn có quyền không cập nhật phần mềm của mình. Trong một số trường hợp, điều này sẽ không gây ra bất kỳ gián đoạn nào; bạn vẫn sẽ đồng bộ với phần còn lại của mạng lưới. Nhưng trong những trường hợp khác, điều này có thể dẫn đến sự kiện được gọi là Hard Fork, khi mạng lưới bị chia tách làm hai và một phiên bản Bitcoin mới được tạo ra. Đó chính xác là những gì đã xảy ra vào năm 2017 với sự chia tách giữa Bitcoin (BTC) và Bitcoin Cash (BCH).

Mặc dù loại quản trị này có thể chậm và đôi khi lộn xộn, nhưng nó cũng là một điểm mạnh; nó đảm bảo rằng không có thực thể đơn lẻ nào có thể đơn phương kiểm soát, giúp Bitcoin duy trì sự ổn định, trung lập và chống lại sự tập trung hóa.

#### Xác thực cá nhân: các nút

Bitcoin cho phép bất kỳ ai kiểm tra độ chính xác của Blockchain bằng cách chạy một "node" trên máy tính hoặc máy chủ của họ. Điều này có nghĩa là tải xuống phần mềm Bitcoin core (hoặc phiên bản khác của giao thức Bitcoin) và xác minh tất cả các giao dịch và khối kể từ năm 2009. Sau khi node của bạn được thiết lập và đồng bộ hóa, nó sẽ trở thành bản sao đầy đủ của Blockchain và giúp hỗ trợ mạng.

Mặc dù phương pháp này mang tính kỹ thuật hơn, nhưng nó cung cấp cho những người dùng khó tính nhất khả năng từ chối tin tưởng bên thứ ba. Việc vận hành một nút đảm bảo rằng người dùng có thể tham gia vào quá trình đồng thuận và không bị kiểm duyệt, góp phần trực tiếp vào tính bảo mật và phi tập trung của mạng lưới.

### Các trường hợp sử dụng

#### Một phương thức thanh toán xuyên biên giới linh hoạt

Nhờ tính chất phi tập trung, Bitcoin hoạt động 24/7, không bị ảnh hưởng bởi biên giới hay múi giờ. Tại những khu vực còn thiếu cơ sở hạ tầng ngân hàng truyền thống, Bitcoin thường được sử dụng như một giải pháp nhanh chóng, chi phí thấp để gửi hoặc nhận tiền mà không cần phụ thuộc vào các trung gian đắt đỏ. Mặc dù phí giao dịch có thể thay đổi tùy theo tình trạng tắc nghẽn mạng, nhưng nhìn chung vẫn thấp hơn nhiều so với phí mà các ngân hàng áp dụng cho các giao dịch chuyển khoản quốc tế. Ngoài ra, các giải pháp Layer-2 như [Lightning Network](https://planb.academy/resources/glossary/lightning-network) cho phép giao dịch Bitcoin nhanh hơn và rẻ hơn.

Nhờ tính chất phi tập trung, Bitcoin hoạt động 24/7, không bị ảnh hưởng bởi biên giới hay múi giờ. Tại những khu vực còn thiếu cơ sở hạ tầng ngân hàng truyền thống, Bitcoin thường được sử dụng như một giải pháp nhanh chóng, chi phí thấp để gửi hoặc nhận tiền mà không cần phụ thuộc vào các trung gian đắt đỏ. Mặc dù phí giao dịch có thể thay đổi tùy theo tình trạng tắc nghẽn mạng, nhưng nhìn chung vẫn thấp hơn nhiều so với phí mà các ngân hàng áp dụng cho các giao dịch chuyển khoản quốc tế. Ngoài ra, các giải pháp Layer-2 như Lightning Network cho phép giao dịch Bitcoin nhanh hơn và rẻ hơn.

#### Một kho lưu trữ giá trị

Nhờ tính khan hiếm (giới hạn ở mức 21 triệu BTC) và khả năng phục hồi vốn có, Bitcoin thường được xem là một kênh tiết kiệm an toàn dài hạn. Mặc dù giá có thể biến động trong ngắn hạn, Bitcoin nhìn chung vẫn duy trì xu hướng tăng trong những năm qua kể từ khi ra đời. Một số nhà đầu tư mua BTC với niềm tin rằng nó có thể đóng vai trò như một kho lưu trữ giá trị, đặc biệt là trong bối cảnh lạm phát hoặc khủng hoảng tài chính.

#### Một công cụ cho sự tự do và khả năng phục hồi tài chính

Ngoài đầu tư, Bitcoin còn cung cấp một giải pháp bảo vệ chủ quyền tài chính. Ở các quốc gia dưới chế độ độc tài hoặc phải đối mặt với những hạn chế tiền tệ nghiêm ngặt, việc sở hữu Bitcoin Wallet (có khóa riêng) mang lại một hình thức tự do. Không ai có thể chặn hoặc tịch thu số BTC này, miễn là người nắm giữ bảo đảm cụm từ khôi phục của chúng.

Đặc điểm này đặc biệt hấp dẫn đối với những người lo ngại về kiểm duyệt hoặc việc đóng băng tài khoản ngân hàng. Nó cũng phù hợp với những người dân đang phải chịu đựng tình trạng siêu lạm phát, như đã thấy ở Venezuela hoặc Zimbabwe, nơi việc nắm giữ BTC tỏ ra ổn định hơn so với việc giữ đồng nội tệ, vốn đang mất giá nhanh chóng.

### Một chặng đường dài phía trước

Bitcoin có thể được xem là một bước đột phá "Từ Không Đến Một": một sự phá vỡ triệt để các mô hình tài chính đã được thiết lập. Lần đầu tiên trong lịch sử, một mạng lưới tiền tệ toàn cầu, có thể tiếp cận được với tất cả mọi người, hoạt động mà không cần một cơ quan trung ương, cho phép các giao dịch riêng tư và không bị kiểm duyệt.

Tuy nhiên, sau hơn một thập kỷ tồn tại, Bitcoin vẫn tiếp tục khơi dậy những cuộc tranh luận và đam mê. Việc áp dụng nó ngày càng tăng, các giải pháp Layer thế hệ thứ hai (như Lightning Network) đang nổi lên để cải thiện tốc độ giao dịch và giảm phí, và các doanh nghiệp trên toàn thế giới đang thử nghiệm các trường hợp sử dụng mới. Nhiều khả năng Bitcoin sẽ tiếp tục ảnh hưởng đến các hệ thống thanh toán và thậm chí cả cách chúng ta nhận thức về tiền tệ trong nhiều thập kỷ tới.

![image](assets/vi/028.webp)

Nếu bạn muốn mở rộng kiến thức hơn nữa, bạn có thể tham gia khóa học BTC101 về Plan ₿ Academy, khóa học này cung cấp kiến thức chuyên sâu hơn về các nguyên tắc kinh tế và kỹ thuật cơ bản của Bitcoin.

https://planb.academy/courses/2b7dc507-81e3-4b70-88e6-41ed44239966

Sau phần giới thiệu về Bitcoin (có thể dài hơn 5 phút một chút!), giờ đây bạn đã được trang bị tốt hơn để cân nhắc mua và bảo mật bitcoin. Trong các chương tiếp theo của khóa học, chúng ta sẽ đi sâu hơn vào tầm quan trọng của Bitcoin, hoạt động của ngành công nghiệp này và sự phát triển của các lớp khác nhau. Tiếp theo, trong phần tiếp theo, chúng ta sẽ thảo luận về cách thiết lập kế hoạch cá nhân của riêng bạn.

## Tại sao Bitcoin lại quan trọng?

<chapterId>d4327ac4-9ff8-5192-b542-cb78c0bd0aa7</chapterId>

:::video id=5f6e7d8c-9b0a-1c2d-3e4f-5a6b7c8d9e0f:::

Tại sao Bitcoin lại quan trọng đến vậy? Đó chính là câu hỏi trọng tâm của khóa học này. Cho dù liên quan đến việc học hay chiến lược đầu tư của bạn, nếu không hiểu rõ tầm quan trọng của Bitcoin, bạn có nguy cơ đi chệch khỏi kế hoạch. Mục tiêu là luôn ghi nhớ các nguyên tắc cơ bản của Bitcoin để đảm bảo chiến lược của bạn luôn phù hợp với niềm tin của bạn.

### Một loại tiền tệ phổ quát

Barack Obama từng gọi Bitcoin là "ngân hàng Thụy Sĩ trong túi bạn", và điều này hoàn toàn có lý. Bitcoin mang đến những cơ hội như nhau cho tất cả mọi người, bất kể họ là ai. Dù bạn là một thiếu niên, một tổng thống, một người biểu tình ở Hồng Kông, hay một "Yellow Vest" ở Pháp, tất cả mọi người đều có quyền tiếp cận bình đẳng với cùng một giao thức và công cụ:

- Tạo ví miễn phí và không giới hạn (với Bitcoin, chúng ta không thực sự nói về "tài khoản" mà là "ví").
- Gửi tiền đến bất kỳ đâu, cho bất kỳ ai.
- Không cần phải xác minh danh tính hay bất kỳ thủ tục hành chính nào.
- Có thể tiếp cận với tất cả mọi người, bất kể tuổi tác, giới tính, tôn giáo, quốc gia hoặc mức thu nhập.
- Quyền riêng tư và minh bạch có thể được quyết định tùy theo quyết định của bạn.
- Không có trung gian hoặc phí ẩn.
- Bitcoin là phiên bản gốc của Internet, nghĩa là bất kỳ ai có quyền truy cập web đều có thể sử dụng.

![image](assets/vi/037.webp)

Bitcoin có thể được xem là "đồng tiền của nhân dân" đích thực, một hệ thống tiền tệ thay thế không phụ thuộc vào bất kỳ cơ quan trung ương nào và dựa trên các quy tắc bất biến thay vì các quyết định tùy tiện. Tính chất cởi mở và dễ tiếp cận của nó khiến nó trở thành một công cụ tiềm năng mang tính cách mạng cho hàng tỷ người trên toàn thế giới, dù họ bị loại khỏi hệ thống ngân hàng truyền thống hay chỉ đơn giản là đang tìm kiếm một giải pháp thay thế mang tính chủ quyền hơn.

Điều này dẫn chúng ta đến một câu hỏi cơ bản, gần như mang tính triết lý, chia rẽ những người ủng hộ Bitcoin thành hai thế giới quan chính. Một bên, một số người coi Bitcoin là giải pháp thúc đẩy hòa nhập tài chính, cho phép hàng tỷ người chưa có tài khoản ngân hàng cuối cùng cũng được tiếp cận cơ sở hạ tầng tiền tệ toàn cầu. Bên kia, một số người coi Bitcoin là một công cụ giải phóng tài chính, nhằm mục đích mở ra một lối thoát cho hàng tỷ người đã gắn bó với hệ thống ngân hàng, nhưng muốn thoát khỏi sự phụ thuộc vào nó và giành lại toàn quyền kiểm soát tiền bạc. Suy ngẫm này đáng được chúng ta lưu ý, và chúng ta sẽ quay lại chi tiết hơn sau.

![image](assets/vi/044.webp)

### Bảo vệ chống lại khủng hoảng tiền tệ

Trong nhiều thế kỷ, thế giới đã trải qua những cuộc khủng hoảng tiền tệ gây ra những hậu quả tàn khốc cho dân số. Hàng tỷ người vẫn đang phải gánh chịu hậu quả của các chính sách tiền tệ được quản lý kém, nơi việc thao túng tiền tệ (Supply) và lãi suất tạo ra sự mất cân bằng hệ thống. Những cuộc khủng hoảng này không chỉ là những sự kiện ngẫu nhiên—chúng là kết quả của một hệ thống được xây dựng dựa trên sự can thiệp và thao túng giá trị tiền tệ và thời gian.

![image](assets/vi/045.webp)

Những cuộc khủng hoảng này có thể diễn ra dưới nhiều hình thức khác nhau. Ví dụ, siêu lạm phát sẽ xóa sổ đồng tiền bằng cách dần dần phá hủy sức mua của người dân; như đã thấy ở các quốc gia như Zimbabwe và Venezuela. Mặt khác, các biện pháp kiểm soát tiền tệ nghiêm ngặt có thể hạn chế khả năng tiếp cận vốn và tước đi quyền tự do kinh tế của cá nhân, như đã xảy ra với các hạn chế ngân hàng ở Hy Lạp và Lebanon.

![image](assets/vi/047.webp)

Và cuối cùng, khi chính phủ phá giá đồng tiền quốc gia, nó sẽ dần dần làm xói mòn tiền tiết kiệm của người dân; một sự hao hụt tài sản vô hình nhưng liên tục. Theo nhiều cách, nó hoạt động như một loại thuế ẩn. Chừng nào chính sách tiền tệ vẫn nằm trong tay các cơ quan tập trung, những chu kỳ này chắc chắn sẽ lặp lại.

![image](assets/vi/038.webp)

Bitcoin là một giải pháp thay thế táo bạo cho chu kỳ bất ổn tiền tệ dai dẳng này. Không giống như các loại tiền tệ do nhà nước phát hành, Bitcoin được xây dựng dựa trên các quy tắc toán học bất biến, được thực thi bởi sự đồng thuận; chứ không phải bởi chính phủ hay ngân hàng trung ương. Việc phát hành Bitcoin có thể dự đoán được và được giới hạn ở khoảng 21 triệu đồng, khiến nó trở thành một hình thức tiền tệ vững chắc được thiết kế để giữ giá trị theo thời gian. Nhờ khả năng chống kiểm duyệt, bất kỳ ai cũng có thể lưu trữ và chuyển giao giá trị mà không cần dựa vào bất kỳ tổ chức nào. Và nhờ tính phân chia và tính di động, Bitcoin vừa dễ tiếp cận vừa thiết thực; một cơ sở hạ tầng tài chính cho bất kỳ ai, ở bất kỳ đâu.

**Bạn có biết?** Trong suốt lịch sử, đã có ít nhất 56 trường hợp siêu lạm phát được ghi nhận trên toàn thế giới. Trong nhiều trường hợp, toàn bộ nền kinh tế sụp đổ, tiền tiết kiệm cả đời bị xóa sổ, và hàng triệu người bị đẩy vào cảnh nghèo đói cùng cực. Tệ hơn nữa, những thất bại tiền tệ này thường là bàn đạp cho các cuộc biến động chính trị; đôi khi dẫn đến các chế độ độc tài, như đã xảy ra ở Đức vào những năm 1920 và Chile vào những năm 1970.

![image](assets/vi/039.webp)

Hanke, S. H., & Krus, N. (2013). *Siêu lạm phát thế giới*. Trong R. Parker & R. Whaples (Biên tập), Cẩm nang về các sự kiện lớn trong lịch sử kinh tế. Nhà xuất bản Routledge. Truy cập từ https://ssrn.com/abstract=2130109

Sự sụp đổ của tiền tệ fiat không phải là một sự may rủi lịch sử; đó là một mô hình lặp lại. Ngày nay, Bitcoin mang đến một lối thoát: một cơ hội độc nhất để bảo vệ tài sản của bạn bên ngoài các hệ thống tiền tệ do chính phủ kiểm soát. Tại thời điểm này, câu hỏi không phải là liệu một cuộc khủng hoảng khác có xảy ra hay không, mà là khi nào. Với Bitcoin, giờ đây bạn có thể lựa chọn thoát khỏi những chu kỳ hủy diệt này và lựa chọn một hệ thống tiền tệ được xây dựng dựa trên sự minh bạch, khả năng dự đoán và chủ quyền cá nhân.

### Một phản ứng đối với sự kiểm soát của nhà nước và sự bất công

Bất bình đẳng kinh tế ngày càng gia tăng trên toàn thế giới luôn là mảnh đất màu mỡ cho bất ổn xã hội và sự trỗi dậy của chủ nghĩa cực đoan chính trị. Lịch sử cho thấy khi khoảng cách giàu nghèo quá lớn, nó thường dẫn đến căng thẳng, khủng hoảng, và thậm chí là sự trỗi dậy của các chế độ độc tài. Trước những rủi ro này, việc bảo vệ tự do tài chính không chỉ là một điều xa xỉ; đó là điều cần thiết cho bất kỳ ai muốn duy trì quyền tự chủ và bảo vệ tương lai gia đình.

![image](assets/vi/046.webp)

Nhưng trong một thế giới mà nhà nước có thể kiểm soát hoàn toàn tài sản và giao dịch, thì có những lựa chọn thực sự nào để bảo vệ khoản tiết kiệm của bạn?

- **Tài khoản ngân hàng** có thể bị đóng băng ngay lập tức, bị tịch thu theo lệnh đơn giản của chính phủ hoặc bị rút hết tiền thông qua các hạn chế tiền tệ quá mức.

- **Vàng**, mặc dù đã được sử dụng làm phương tiện lưu trữ giá trị trong hàng thiên niên kỷ, nhưng lại khó phân chia, bất tiện khi vận chuyển và không thực tế để sử dụng trong các tình huống khủng hoảng khẩn cấp.

- **Tiền mặt**, mặc dù vô danh, nhưng cồng kềnh, dễ bị tịch thu và liên tục mất giá do lạm phát.

Nhưng Bitcoin không chỉ là một công cụ thiết thực. Nó còn là **một hình thức phản kháng hòa bình**; một tuyên ngôn độc lập khỏi hệ thống tài chính dựa trên quyền lực tùy tiện, tập trung quyền lực và bất bình đẳng hệ thống. Lựa chọn Bitcoin đồng nghĩa với việc từ chối thao túng, phá giá và giám sát. Nó liên quan đến việc giành lại **chủ quyền** của bạn, đảm bảo tương lai của bạn và bảo vệ quyền kiểm soát tài sản của chính bạn.

Dưới góc nhìn này, Bitcoin không chỉ là công nghệ. Nó là một công cụ của luật tự nhiên, một cách để cá nhân khẳng định các quyền cơ bản của mình, ngay cả khi những quyền đó bị luật pháp quốc gia phủ nhận. Nó trao lại quyền lực cho người dân, không phải thông qua cách mạng, mà thông qua luật lệ.

![image](assets/vi/040.webp)

**Bạn có biết** không? Bitcoin là tài khoản giả danh, không phải tài khoản ẩn danh. Người dùng có thể tạo địa chỉ Wallet mà không cần tiết lộ danh tính thực, cho phép họ gửi và nhận tiền bên ngoài hệ thống ngân hàng truyền thống.

Tuy nhiên, trái với quan niệm phổ biến, Bitcoin không đảm bảo tính ẩn danh hoàn toàn. Mọi giao dịch đều được ghi lại trên Ledger công khai (Blockchain) mà bất kỳ ai cũng có thể truy cập và xác minh. Mặc dù địa chỉ Wallet không gắn liền với tên, hoạt động tài chính của người dùng vẫn có thể bị theo dõi và phân tích nếu không tuân thủ các quy tắc bảo mật phù hợp.

### Giải pháp cho nạn tham nhũng tiền tệ và ngân hàng

Các ngân hàng trung ương, thông qua chính sách tiền tệ mở rộng, liên tục làm xói mòn sức mua của bạn. Thông qua lạm phát và in tiền quá mức (thường được ngụy trang dưới dạng Nới lỏng Định lượng), họ liên tục làm loãng giá trị đồng tiền đang lưu hành. Điều này hoạt động như một loại thuế vô hình, năm này qua năm khác, làm giảm tài sản của những người tiết kiệm bằng tiền do chính phủ phát hành.

Trái ngược với niềm tin phổ biến rằng lạm phát là một hiện tượng kinh tế tự nhiên, thực tế đây là một công cụ kiểm soát tiền tệ; một công cụ làm cho dân chúng nói chung trở nên nghèo đói dần dần trong khi lại mang lại lợi ích cho những người nắm giữ tài sản tài chính.

Nếu tài sản của bạn không được đảm bảo bằng các tài sản phi tiền tệ (như bất động sản, trái phiếu hoặc cổ phiếu), tiền tiết kiệm của bạn chắc chắn sẽ mất giá theo thời gian. Trong khi đó, những người có khả năng tiếp cận các công cụ tài chính sẽ tiếp tục gia tăng tài sản, làm gia tăng khoảng cách giữa giới tinh hoa kinh tế và phần còn lại của xã hội.

Đây không phải là một sai sót trong hệ thống; mà là một cơ chế cố ý. Các ngân hàng trung ương và chính phủ sử dụng nó để kích thích tăng trưởng kinh tế một cách giả tạo và thúc đẩy người dân tiêu dùng liên tục và tăng nợ.

![image](assets/vi/041.webp)

Hệ thống tài chính hiện đại của chúng ta được xây dựng trên một chu kỳ nợ nần; một chu kỳ mà việc vay mượn không chỉ được khuyến khích mà còn gần như không thể tránh khỏi. Cá nhân vay mượn để duy trì cuộc sống, chỉ để rồi thấy mình bị mắc kẹt trong một hệ thống mà họ phải trả lãi cho các ngân hàng tạo ra tiền từ hư không. Điều này không phải là ngẫu nhiên; đó là một thiết kế cấu trúc nhằm mang lại lợi ích cho các tổ chức tài chính bằng cách gây thiệt hại cho người dân thường.

Hệ thống bị phá hoại bởi ảnh hưởng của ngân hàng trung ương và quyền lực không thể kiểm soát của họ trong việc thao túng tiền tệ Supply. **Bitcoin là giải pháp thay thế.**

Không giống như tiền pháp định, Bitcoin được điều chỉnh bởi các quy tắc được thực thi dựa trên sự đồng thuận. Supply của nó bị giới hạn; sẽ không bao giờ có quá 21 triệu Bitcoin tồn tại (thực tế, ít hơn một chút do cách thức phát hành được cấu trúc). Không chính phủ, ngân hàng trung ương hay bất kỳ tác nhân kinh tế nào có thể thay đổi giới hạn này.

Điều này có nghĩa là Bitcoin hoạt động theo khuôn khổ tiền tệ có thể dự đoán được; khuôn khổ mà lạm phát không chỉ minh bạch mà còn được thiết kế để giảm dần hoàn toàn sau khi khai thác được Bitcoin cuối cùng.

Trước đây, vàng đóng vai trò như một công cụ kiểm soát sự bành trướng tiền tệ không được kiểm soát. Nhưng kể từ khi chế độ bản vị vàng sụp đổ năm 1971, không một đồng tiền quốc gia nào (dù là đô la, euro hay yên) được đảm bảo bằng một tài sản hữu hình. Sự tách biệt này đã cho phép các ngân hàng trung ương tự do in tiền không hạn chế, mở đường cho hàng thập kỷ bành trướng tiền tệ mạnh mẽ, bong bóng tài sản liên tục xuất hiện và các cuộc khủng hoảng tài chính tái diễn.

![image](assets/vi/042.webp)

Khi bạn gửi tiền vào ngân hàng, số tiền đó không còn thực sự thuộc về bạn nữa.

Hầu hết mọi người không nhận ra điều này: số tiền bạn giữ trong tài khoản ngân hàng về mặt kỹ thuật không phải là tài sản của bạn. Về mặt pháp lý và thực tế, đó là khoản vay bạn cho ngân hàng; khoản vay mà ngân hàng được toàn quyền sử dụng cho các hoạt động và đầu tư của mình.

Hệ thống này được xây dựng dựa trên lòng tin mù quáng của công chúng vào các tổ chức tài chính, nhưng nó mang lại những rủi ro nghiêm trọng:

- **Nếu ngân hàng của bạn sụp đổ, tiền của bạn có thể biến mất.** Ngay cả với các chương trình bảo hiểm tiền gửi, lịch sử đã chỉ ra rằng những bảo đảm này có thể thất bại trong các cuộc khủng hoảng hệ thống.
- **Nếu ngân hàng hạn chế quyền truy cập vào tiền của bạn, bạn có thể không thể rút hoặc sử dụng tiền của mình**. Điều này đã xảy ra nhiều lần; trong các cuộc khủng hoảng kinh tế ở Hy Lạp, Lebanon và Argentina, hoặc trong bối cảnh đàn áp chính trị như các cuộc biểu tình của tài xế xe tải ở Canada.

Bitcoin cung cấp một chế độ hoàn toàn khác biệt: cởi mở, trung lập và không thể bị thao túng. Các quy tắc của nó được mã hóa cứng bằng sự đồng thuận và áp dụng bình đẳng cho tất cả người tham gia mạng.

Đây chính là nơi mà nguyên tắc cốt lõi được đưa ra:

**"Không phải chìa khóa của anh, không phải Bitcoin của anh."**

Nếu bạn không kiểm soát được khóa riêng tư của Bitcoin, thì bạn không thực sự sở hữu chúng. Chúng nằm trong tay một bên thứ ba; giống như tiền pháp định trong ngân hàng vậy. Nhưng nếu bạn nắm giữ khóa riêng tư, bạn và chỉ mình bạn có toàn quyền kiểm soát tiền của mình. Không một tổ chức, chính phủ hay cơ quan nào có thể đóng băng, tịch thu hay hạn chế quyền truy cập của bạn.

Đây chính là lý do khiến Bitcoin trở thành giải pháp thay thế mạnh mẽ cho những điểm yếu và sự lạm quyền của hệ thống tài chính truyền thống: chủ quyền tiền tệ.

### Bitcoin: Một phong trào chính trị?

Bitcoin định hình lại cán cân quyền lực giữa cá nhân và các tổ chức tài chính. Nó trao quyền cho bất kỳ ai nắm toàn quyền kiểm soát tiền bạc, bảo vệ tiền tiết kiệm khỏi lạm phát và thoát khỏi các hạn chế tiền tệ do nhà nước áp đặt. Là một hệ thống mở và không biên giới, Bitcoin mang đến một giải pháp thay thế công bằng hơn; dễ tiếp cận cho tất cả mọi người, bất kể địa vị xã hội, quốc tịch hay nguồn gốc.

Chấp nhận Bitcoin chính là lựa chọn đồng tiền lành mạnh. Đó là sự từ chối tiếp tục là một mắt xích trong cỗ máy lạm phát, nợ nần của hệ thống tài chính hiện tại. Đó là một hành động thể hiện chủ quyền cá nhân và sự phản kháng ôn hòa chống lại nạn tham nhũng tiền tệ và sự xói mòn của cải.

Những người dùng Bitcoin đến từ mọi tầng lớp xã hội, nhưng họ đều có chung một tầm nhìn: một thế giới mà chủ quyền tiền tệ nằm trong tay cá nhân chứ không phải các tổ chức. Trong số đó có:

- **Cypherpunks**, những người ủng hộ quyền riêng tư và phản đối sự giám sát;
- **Những công dân bị áp bức**, tìm kiếm nơi ẩn náu khỏi chế độ độc tài và kiểm soát vốn;
- **Những người theo chủ nghĩa vô chính phủ**, coi Bitcoin là công cụ giải phóng khỏi sự kiểm soát của nhà nước;
- **Các nhà kinh tế học người Áo**, ủng hộ đồng tiền lành mạnh và thoát khỏi sự thao túng của chính phủ;
- **Các kỹ sư, nhà tài chính và những người ủng hộ quyền tự do ngôn luận**, những người nhận ra những tác động sâu sắc đến xã hội của mô hình tiền tệ mới này.

![image](assets/vi/043.webp)

Bitcoin, về bản chất, đã vượt qua mọi rào cản chính trị và tư tưởng. Nó không thiên tả hay thiên hữu, không tự do hay tập thể. Nó là một giao thức trung lập, được quản lý bởi các quy tắc (không phải quy tắc của người cai trị) được áp dụng bình đẳng cho tất cả mọi người. Tuy nhiên, sự tồn tại của nó đã thách thức hiện trạng tài chính toàn cầu. Bitcoin đã trở thành biểu tượng của sự phản kháng bởi vì mọi người đã chấp nhận nó như một giải pháp thay thế cho tiền tệ fiat và cơ sở hạ tầng tài chính tập trung; những hệ thống ngày càng bị coi là bất công, dễ bị thao túng và loại trừ.

Đối với những người theo chủ nghĩa Cypherpunk, Bitcoin không chỉ là một tài sản kỹ thuật số. Nó chống lại sự xói mòn quyền riêng tư đang diễn ra trong một thế giới mà việc biến mất của tiền mặt thường được biện minh dưới vỏ bọc "an ninh".

Bitcoin cho phép thực hiện các giao dịch kỹ thuật số ngang hàng, chống kiểm duyệt; không cần đến trung gian hay người gác cổng. Như Satoshi Nakamoto đã hình dung, nó cung cấp một hình thức tương đương kỹ thuật số của tiền mặt: một cách để trao đổi giá trị tự do, không cần sự cho phép.

![image](assets/vi/048.webp)

Bitcoin không phải là một tổ chức hay một đảng phái chính trị, nhưng nó chắc chắn mang một thông điệp triết học mạnh mẽ. Nó định nghĩa lại mối quan hệ giữa cá nhân và nhà nước, thách thức sự độc quyền của các ngân hàng trung ương trong việc tạo ra tiền tệ và kiểm soát kinh tế.

Cho dù được những người đấu tranh vì tự do hay những người chỉ muốn duy trì sức mua của mình thông qua, Bitcoin đánh dấu sự khởi đầu của một kỷ nguyên mới; kỷ nguyên mà chủ quyền tài chính trở thành quyền cơ bản của con người, dành cho tất cả mọi người.

![image](assets/vi/049.webp)

Bây giờ chúng ta đã khám phá ý nghĩa sâu sắc của phát minh Satoshi Nakamoto, chương tiếp theo sẽ đưa chúng ta đến với ngành công nghiệp phi thường đã phát triển xung quanh giao thức này; toàn bộ hệ sinh thái đang định hình lại tài chính, công nghệ và xã hội.

## Hiểu về ngành công nghiệp Bitcoin

<chapterId>e106c6f1-d75b-5a62-b245-0ea2e4d02ef8</chapterId>

:::video id=8a9b0c1d-2e3f-4a5b-6c7d-8e9f0a1b2c3d:::

Kể từ khi ra mắt vào năm 2009 bởi nhà sáng lập ẩn danh Satoshi Nakamoto, Bitcoin đã khơi mào cho sự trỗi dậy của một ngành công nghiệp hoàn toàn mới; hiện được định giá hàng trăm tỷ đô la. Mặc dù có lịch sử tương đối ngắn, hệ sinh thái này đã trải qua sự tăng trưởng bùng nổ, phát triển với tốc độ cấp số nhân trong thập kỷ qua. Mỗi ngày, những người chơi mới (từ các nhà đầu tư tổ chức, các công ty khởi nghiệp linh hoạt đến các gã khổng lồ công nghệ) đều đổ vốn và nguồn lực đáng kể vào việc khẳng định vị thế của mình trong lĩnh vực đang phát triển nhanh chóng này.

Ngày nay, Bitcoin đã đạt đến ngưỡng quan trọng; một điểm không thể quay đầu. Chính phủ, ngân hàng trung ương, các tập đoàn công nghệ tài chính và các tổ chức tài chính truyền thống không còn có thể tiếp tục phớt lờ nó. Dù thông qua quy định, áp dụng thận trọng hay đối đầu công khai, giờ đây họ đều nhận ra tác động tất yếu của Bitcoin đối với nền kinh tế toàn cầu.

### Sự ra đời của một ngành công nghiệp toàn cầu

Bitcoin là một cải tiến đột phá, một bước nhảy vọt từ con số không lên con số một. Nó đại diện cho một sự thay đổi hoàn toàn so với mô hình tiền tệ truyền thống. Đối với một số người, sự gián đoạn này là một mối đe dọa; một thách thức hiện hữu đối với quyền lực và đặc quyền đã được thiết lập của họ. Đối với họ, Bitcoin là chiếc hộp Pandora không bao giờ nên được mở ra, và họ sẽ dùng mọi cách để chống lại nó.

![image](assets/vi/050.webp)

Tuy nhiên, những người khác lại coi Bitcoin là cơ hội ngàn năm có một: một công cụ cho tự do cá nhân, một chất xúc tác để chuyển đổi hệ thống tài chính toàn cầu, và một con đường hướng tới một giải pháp thay thế minh bạch và công bằng hơn. Họ là những người xây dựng, người áp dụng và người đóng góp (những người định hình tương lai).

**Bitcoin** tự nó vẫn giữ thái độ trung lập. Nó không xin phép hay xin phê duyệt.

**Nó đơn giản là tồn tại.**

Trong chương này, chúng ta sẽ khám phá những nhân tố chủ chốt thúc đẩy sự phát triển của ngành công nghiệp Bitcoin. Việc hiểu rõ vai trò, động lực và tương tác của họ là điều cần thiết để nắm bắt được động lực của hệ sinh thái đang phát triển này và định hướng tốt hơn các cơ hội cũng như thách thức mà nó mang lại.

### Sự gia tăng của các altcoin

Về mặt kỹ thuật, việc tạo ra một loại tiền điện tử mới cực kỳ dễ dàng; chỉ mất vài phút và hầu như không cần bất kỳ sự đổi mới thực sự nào. Thách thức thực sự không nằm ở việc tạo ra nó, mà nằm ở giá trị. Và giá trị, trong thế giới tài sản kỹ thuật số, hoàn toàn được xác định bởi thị trường; bởi niềm tin và nhu cầu của người dùng.

Vào tháng 12 năm 2019, CoinMarketCap đã niêm yết hơn 5.000 token. Đến năm 2025, con số đó đã bùng nổ lên đến hàng triệu, nhờ sự phát triển của NFT, tài chính phi tập trung (DeFi) và vô số ứng dụng khác (một số hợp pháp, nhiều ứng dụng còn đang gây tranh cãi). Những token này tồn tại dưới đủ mọi hình thức: một số tự nhận là tiền tệ, một số khác hoạt động như chứng khoán, tiện ích nền tảng, chuỗi phụ hoặc đại diện token hóa của nghệ thuật kỹ thuật số.

Nhưng chúng ta cần hiểu rõ: **hầu hết các loại tiền điện tử này đều không khác gì lừa đảo.**.

Đằng sau lớp vỏ công nghệ hào nhoáng và thương hiệu bóng bẩy, nhiều dự án trong số này được thúc đẩy bởi các chiến lược tiếp thị mạnh mẽ được thiết kế để thực hiện một mục đích duy nhất: **trích xuất Bitcoin của bạn**. Chúng lợi dụng lòng tham và sự thiếu hiểu biết của nhà đầu tư, thêu dệt những câu chuyện hấp dẫn về công nghệ mang tính cách mạng hoặc lợi nhuận được đảm bảo; những tuyên bố hiếm khi đứng vững khi bị kiểm tra kỹ lưỡng.

Tất nhiên, giữa biển thông tin hỗn loạn này, một số ít dự án thực sự nỗ lực vượt qua giới hạn. Một số tập trung vào việc giải quyết các thách thức kỹ thuật thực tế (khả năng mở rộng, quyền riêng tư, khả năng lập trình) và có thể đóng góp những ý tưởng giá trị cho không gian rộng lớn hơn. Rất có thể theo thời gian, một vài trong số những thử nghiệm này sẽ dẫn đến những đổi mới hữu ích.

Nhưng câu hỏi cơ bản vẫn còn đó:

**Những đổi mới này có thể phát triển bên ngoài Bitcoin không?**

Cho đến nay, một sự thật đã được chứng minh: Bitcoin vẫn là đồng tiền kỹ thuật số phi tập trung duy nhất thực sự, chống kiểm duyệt, được hỗ trợ bởi mạng lưới toàn cầu và sự chấp nhận ngày càng tăng. Không giống như các altcoin, Bitcoin không được hỗ trợ bởi các công ty tập trung hay được quản lý bởi một nhóm nhỏ các nhà phát triển và nhà đầu tư ban đầu. Đây là dự án duy nhất đã đạt được thành quả của hàng ngàn giờ nghiên cứu, phát triển và tinh chỉnh không ngừng nghỉ.

| Tính năng            | Bitcoin                  | Altcoins (99.9% trong số đó)   |
| ---------------------|--------------------------|--------------------------------|
| **Thanh khoản**       | Cao                      | Thấp                           |
| **Mức độ chấp nhận (thực tế)** | Toàn cầu và đang tăng | Rất hạn chế                    |
| **Đội ngũ**           | Phi tập trung và vững chắc | Tập trung và thiếu minh bạch   |
| **Danh tiếng**        | Mạnh và được công nhận trên toàn cầu | Không đồng đều, thường đáng ngờ |
| **Hạ tầng**           | Ổn định và an toàn       | Thiếu ổn định và dễ tổn thương |
| **Phi tập trung**     | Có                       | Hiếm khi                       |
| **Rủi ro lừa đảo**    | Không                    | Rất cao                        |
| **Giá trị sử dụng thực tế?** | Có                       | Còn tranh cãi                  |

**Hãy cẩn thận với những tuyên bố gây hiểu lầm như:**

- "Blockchain, không phải Bitcoin"
- "XRP là Bitcoin tiếp theo"
- "Libra sẽ thay thế Bitcoin
- "Dự án của tôi là phiên bản tốt hơn của Bitcoin"
- "Tiền kỹ thuật số của ngân hàng trung ương sẽ khiến Bitcoin trở nên lỗi thời"

Trước khi đầu tư thời gian hoặc nguồn lực vào bất kỳ sản phẩm Altcoin nào, hãy tự nghiên cứu vì đó không phải là nội dung chúng tôi muốn đề cập đến.

**Chúng tôi chỉ đề cập đến Bitcoin và Bitcoin.**

### Việc áp dụng của các tổ chức lớn

Sau cơn sốt ICO năm 2017, các tổ chức bắt đầu thể hiện sự quan tâm nghiêm túc đến Blockchain; nhưng thường không nắm bắt được điều gì thực sự làm nên tính cách mạng của nó. Các ngân hàng trung ương và chính phủ hiện đang nghiên cứu Tiền tệ Kỹ thuật số Ngân hàng Trung ương (CBDC), với hy vọng hiện đại hóa cơ sở hạ tầng tài chính đồng thời duy trì quyền kiểm soát hoàn toàn đối với các giao dịch của người dùng. Các dự án đã và đang được triển khai ở các quốc gia như Thụy Điển, EU, Nga và Trung Quốc.

Các gã khổng lồ công nghệ cũng đã tham gia cuộc đua. Facebook (nay là Meta) đã ra mắt sáng kiến stablecoin Libra, với mục tiêu tạo ra một loại tiền kỹ thuật số được hỗ trợ bởi một rổ tiền tệ fiat. Tuy nhiên, dự án đã nhanh chóng vấp phải sự phản đối của các cơ quan quản lý và cuối cùng đã bị hủy bỏ.

| Feature                    | Bitcoin | Altcoins | Facebook-Coin | FedCoin |
|----------------------------|---------|----------|---------------|---------|
| **Công khai**              | Có      | Tuỳ loại | Không         | Không   |
| **Mã nguồn mở**            | Có      | Tuỳ loại | Không         | Không   |
| **Không biên giới**        | Có      | Tuỳ loại | Không         | Không   |
| **Trung lập**              | Có      | Tuỳ loại | Không         | Không   |
| **Chống kiểm duyệt**       | Có      | Tuỳ loại | Không         | Không   |


Mặc dù được tiếp thị táo bạo, những sáng kiến này không cạnh tranh được với Bitcoin; chúng bắt chước ngôn ngữ của nó trong khi bác bỏ các nguyên tắc cốt lõi. Chúng được xây dựng để tuân thủ chứ không phải để tự do. Chúng được thiết kế để mở rộng giám sát chứ không phải để bảo vệ quyền riêng tư. Chúng củng cố quyền kiểm soát thay vì phân phối nó.

Libra của Facebook không bao giờ có ý định thách thức hiện trạng; nó được xây dựng để hoạt động với hệ thống. Ngược lại, Bitcoin tồn tại hoàn toàn bên ngoài hệ thống đó. Nó không cần xin phép. Nó không dựa vào lòng tin. Và nó đã hoạt động hoàn hảo (không cần người lãnh đạo, thời gian ngừng hoạt động hoặc kiểm soát tập trung) trong hơn một thập kỷ.

![image](assets/vi/051.webp)

### Quy định và cách tiếp cận của chính phủ

Về bản chất, Bitcoin hoạt động ngoài khuôn khổ truyền thống. Nó không phụ thuộc vào một cơ quan trung ương nào và không thể bị kiểm soát hay thay đổi bởi bất kỳ thực thể đơn lẻ nào. Tuy nhiên, mặc dù giao thức này không bị quản lý, những người tham gia tương tác với nó (sàn giao dịch, doanh nghiệp và người dùng) vẫn phải tuân thủ luật pháp quốc gia.

![image](assets/vi/052.webp)

Vì Bitcoin là một mạng lưới toàn cầu nên các quốc gia đã phản ứng theo nhiều cách khác nhau:

- **Một số nước áp dụng các hạn chế nghiêm ngặt**, như Trung Quốc, cố gắng hạn chế việc sử dụng mà không thực sự ngăn chặn hoàn toàn.

- **Một số quốc gia khác cung cấp môi trường chào đón hơn**, chẳng hạn như Thụy Sĩ hoặc Canada, coi Bitcoin là cơ hội chứ không phải là mối đe dọa.

- **Hầu hết vẫn chưa quyết định**, đang thử nghiệm các quy định trong khi cố gắng cân bằng giữa đổi mới và giám sát.

![image](assets/vi/056.webp)

Chính phủ và các tổ chức thường gặp khó khăn trong việc phân loại chính xác Bitcoin (là tiền, tài sản hay một thứ gì đó hoàn toàn mới?). Do đó, các quy định thường mang tính phản ứng, thiếu nhất quán và liên tục thay đổi. Nếu bạn tham gia vào Bitcoin, điều cần thiết là phải luôn cập nhật thông tin về bối cảnh địa phương; đặc biệt là khi nói đến thuế, quyền truy cập ngân hàng và các quy tắc tuân thủ.


### Quan điểm của Banks về Bitcoin

Là nền tảng của hệ thống tài chính truyền thống, các ngân hàng coi Bitcoin là mối đe dọa trực tiếp đến mô hình kinh tế của họ, vốn tập trung vào việc trung gian và kiểm soát dòng tiền. Điều này giải thích tại sao nhiều ngân hàng trên thế giới đã áp đặt các hạn chế đối với doanh nghiệp và cá nhân sử dụng Bitcoin. Một số ngân hàng thậm chí còn đóng tài khoản hoặc hạn chế quyền truy cập vào các dịch vụ của các công ty hoạt động trong lĩnh vực tiền điện tử, thường viện dẫn các lo ngại về chống rửa tiền (AML) và chống tài trợ khủng bố (CTF).

Tuy nhiên, mặc dù Bitcoin được xem là đối thủ cạnh tranh, nhiều ngân hàng trong số này đang tích cực đầu tư vào nghiên cứu và phát triển Bitcoin, cố gắng tận dụng những đổi mới của Bitcoin mà không từ bỏ quyền kiểm soát. Họ hiểu rằng Bitcoin mang lại tiềm năng đáng kể, nhưng họ muốn kiểm soát cách thức tích hợp Bitcoin vào cơ sở hạ tầng hiện có của mình, thay vì áp dụng mô hình mở, phi tập trung của Bitcoin.

### Sàn giao dịch tiền điện tử và lưu ký Bitcoin

Các sàn giao dịch đóng vai trò quan trọng trong hệ sinh thái Bitcoin, hoạt động như cầu nối giữa tiền tệ pháp định và Bitcoin. Chúng cho phép người dùng mua, bán và đôi khi giao dịch Bitcoin lấy các tài sản kỹ thuật số khác. Tuy nhiên, không phải tất cả các sàn giao dịch đều giống nhau, và điều cần thiết là phải chọn sàn phù hợp với nhu cầu của bạn đồng thời giảm thiểu rủi ro. Dưới đây là các yếu tố chính cần xem xét trước khi sử dụng sàn giao dịch:

- uy tín vững chắc về độ an toàn;
- tính thanh khoản đủ để đảm bảo giao dịch nhanh chóng mà không có biến động giá quá lớn;
- dịch vụ khách hàng nhanh chóng và hiệu quả;
- Interface thân thiện với người dùng giúp việc điều hướng giao dịch dễ dàng hơn;
- tùy chọn mua hàng định kỳ tự động (ARP);
- Rút bitcoin dễ dàng và miễn phí vào ví cá nhân Wallet.

Các sàn giao dịch tuân thủ quy định địa phương thường phải tuân thủ nghiêm ngặt các giao thức **"[Know Your Customer](https://planb.academy/resources/glossary/kyc-know-your-customer)"** (KYC), yêu cầu người dùng cung cấp giấy tờ tùy thân trước khi truy cập dịch vụ. Mặc dù các quy trình này được thiết kế để ngăn chặn hoạt động bất hợp pháp, nhưng chúng có thể làm ảnh hưởng đến quyền riêng tư mà Bitcoin vốn có.

![image](assets/vi/053.webp)

Nền tảng KYC thu thập thông tin cá nhân của bạn dưới chiêu bài bảo mật. Chính phủ có thể khai thác dữ liệu này để theo dõi các giao dịch tài chính của bạn và hạn chế quyền truy cập của bạn vào một số hoạt động nhất định.

Tuy nhiên, vẫn có những lựa chọn thay thế để mua bitcoin mà không cần phải thực hiện KYC:

- Các nền tảng mua P2P như Bisq, Robosat, LNP2PBot, Peach, HODL HODL, v.v.;
- Mua tiền mặt trực tiếp, ví dụ như tại các buổi gặp mặt Bitcoin tại địa phương;
- Nền tảng mua hàng được quản lý không cần KYC, khá hiếm nhưng vẫn có ở một số quốc gia.;
- Máy ATM Bitcoin;
- Làm việc để kiếm bitcoin;
- Mining bitcoin.

Có một số loại nền tảng, mỗi loại phù hợp với mục đích sử dụng cụ thể:

- **Nền tảng trao đổi ngang hàng (P2P)**

Các nền tảng này cho phép người dùng mua và bán Bitcoin trực tiếp với nhau mà không cần thông qua trung gian tập trung. Chúng mang lại sự riêng tư cao hơn, đặc biệt là vì chúng hoạt động mà không cần KYC. Bạn có thể tìm thấy những người bán địa phương để thực hiện giao dịch trực tiếp hoặc sử dụng nhiều phương thức thanh toán trực tuyến khác nhau (SEPA, Revolut, Wise, v.v.).

**Hãy cẩn thận:** Đối với bất kỳ giao dịch thực tế nào, hãy chọn địa điểm công khai và an toàn để tránh các vụ lừa đảo tiềm ẩn.

https://planb.academy/tutorials/exchange/peer-to-peer/bisq-v2-c1c6a702-6c16-4101-8b90-62c424017b80

https://planb.academy/tutorials/exchange/peer-to-peer/hodlhodl-d7344cd5-6b18-40f5-8e78-2574a93a3879

https://planb.academy/tutorials/exchange/peer-to-peer/lnp2pbot-v2-e6bcb210-610b-487d-970c-7cce85273e3c

https://planb.academy/tutorials/exchange/peer-to-peer/peach-c6143241-d900-4047-9b73-1caba5e1f874

https://planb.academy/tutorials/exchange/peer-to-peer/robosats-b60e4f7c-533a-4295-9f6d-5368152e8c06

- **Nền tảng giao dịch chỉ dành cho Bitcoin**

Các nền tảng này áp dụng phương pháp tiếp cận thân thiện với người dùng, cung cấp dịch vụ đơn giản, minh bạch. Chúng chỉ dành cho Bitcoin. Chúng thường triển khai các giải pháp mua bitcoin thông qua phương pháp Trung bình Chi phí Đô la (DCA) và cung cấp dịch vụ rút tiền tự động vào tài khoản Wallet cá nhân. Chúng đặc biệt phù hợp với người mới bắt đầu muốn tích lũy bitcoin một cách an toàn và liên tục. Ví dụ: Relai, Bull Bitcoin, StackinSat, Bitstack...

https://planb.academy/tutorials/exchange/centralized/bitstack-29fd71be-9570-42c6-8f6f-cd355d62e746

https://planb.academy/tutorials/exchange/centralized/bull-bitcoin-europe-0ccf713e-efcd-44ec-8205-211f49ac7d53

https://planb.academy/tutorials/exchange/centralized/relai-v2-30a9671d-e407-459d-9203-4c3eae15b30e

https://planb.academy/tutorials/exchange/centralized/stackinsat-5af6a380-f3c6-4246-9f81-9957a16ab066

- **Nền tảng giao dịch đa năng hoặc chuyên dụng**

Các nền tảng này cung cấp các tính năng nâng cao vượt xa việc chỉ mua Bitcoin, bao gồm đòn bẩy và các sản phẩm phái sinh. Tuy nhiên, chúng tôi khuyến nghị bạn không nên giao dịch. Thay vào đó, chúng tôi khuyên bạn nên mua Bitcoin và chuyển nó sang Wallet của riêng bạn. Giao dịch là một hoạt động rủi ro cao và thường không phù hợp với những người tập trung vào tích lũy dài hạn. Tránh xa trò chơi giao dịch thường là lựa chọn thông minh hơn.

https://planb.academy/tutorials/exchange/centralized/bitfinex-dc306d39-bd96-4ab9-a278-a322316716db

https://planb.academy/tutorials/exchange/centralized/bitstamp-5a36c896-bff5-46d7-b505-ff069c3ac47c

https://planb.academy/tutorials/exchange/centralized/kraken-1ef03e25-9b42-49bd-a47d-249e1a13cfc6

https://planb.academy/tutorials/exchange/centralized/paymium-92603f76-b985-49ce-81e5-f4fa0df776e5

**Các sàn giao dịch không phải là ví an toàn.** Để bitcoin của bạn trên sàn giao dịch sẽ khiến bạn gặp rủi ro đáng kể. Có nhiều trường hợp có thể dẫn đến việc mất tiền của bạn:

- **Tấn công**: Nhiều bitcoin đã bị đánh cắp từ các nền tảng bị xâm phạm (ví dụ: MtGox);
- **Chính phủ tịch thu**: Chính phủ có thể đóng cửa một nền tảng và đóng băng tiền của người dùng;
- **Phá sản hoặc gian lận**: Nhiều nền tảng đã biến mất cùng với tiền của khách hàng (ví dụ: FTX).

Nguyên tắc vàng rất đơn giản: **Nếu bạn không sở hữu khóa riêng tư, bạn không thực sự sở hữu Bitcoin của mình**. Hãy luôn rút tiền về tài khoản cá nhân Wallet càng sớm càng tốt để đảm bảo quyền sở hữu hoàn toàn đối với số tiền của bạn.

### Ví, Mining và phát triển: trụ cột của hệ sinh thái

#### Ví Bitcoin

Cốt lõi của Bitcoin là Ownership - một công cụ chuyên dụng lưu trữ an toàn các khóa riêng tư cần thiết để truy cập và quản lý bitcoin của bạn. Wallet có thể có nhiều dạng: thiết bị phần cứng chuyên dụng, ứng dụng di động hoặc máy tính để bàn, hoặc thậm chí là một tờ giấy có ghi khóa. Những chiếc ví này kết nối tài sản kỹ thuật số của bạn với thế giới thực, giúp Bitcoin trở nên hữu ích trong cuộc sống hàng ngày.

Mỗi loại Wallet cung cấp sự cân bằng khác nhau:

- Sự riêng tư
- Bảo vệ
- Dễ sử dụng
- Trị giá

Ngành công nghiệp Bitcoin Wallet được chia thành nhiều loại, mỗi loại đáp ứng các nhu cầu và trình độ chuyên môn kỹ thuật khác nhau:

- **Các nhà sản xuất ví cứng (Hardware Wallet)**: Các công ty này phát triển các thiết bị vật lý được thiết kế để lưu trữ khóa an toàn. Một số là mã nguồn mở, trong khi những công ty khác cung cấp các giải pháp độc quyền với nhiều tính năng và mức độ bảo mật khác nhau. Những cái tên đáng chú ý bao gồm Ledger, Trezor, Coinkite, Foundation và Shiftcrypto.
- **Software Wallet Developers**: Bao gồm các công ty và nhà phát triển độc lập chuyên tạo ứng dụng di động và máy tính để bàn. Các sản phẩm của họ đa dạng về trải nghiệm người dùng, bảo mật và tính năng. Ví dụ bao gồm Sparrow, Wizard Sardine, Galoy, Synonym và Blockstream.
- **Ví DIY (*Tự làm*)**: Các giải pháp mã nguồn mở này được thiết kế cho người dùng nâng cao muốn kiểm soát hoàn toàn và ít phụ thuộc vào bên thứ ba. Việc tự xây dựng Wallet giúp giảm thiểu sự phụ thuộc vào độ tin cậy và có thể tăng cường khả năng bảo mật của bạn. Các tùy chọn DIY đáng chú ý bao gồm Seedsigner và Spectre DIY.

Ví đóng vai trò cơ bản trong Bitcoin và sẽ được tìm hiểu sâu hơn ở phần sau của khóa học này.

#### Bitcoin Mining

Mining là chức năng cốt lõi của mạng lưới Bitcoin. Nó đảm bảo an ninh hệ thống và duy trì hoạt động của Blockchain. Thợ đào xác thực các giao dịch và bảo mật mạng lưới bằng cách thực hiện các phép tính tốn nhiều năng lượng, được gọi là [Proof of Work](https://planb.academy/resources/glossary/proof-of-work). Mỗi khối mới được khai thác sẽ thêm một loạt giao dịch vào Blockchain và giải phóng bitcoin mới theo lịch trình phát hành của giao thức.

Trong những ngày đầu của Bitcoin, Mining có thể được thực hiện từ máy tính cá nhân. Ngày nay, đây là một ngành công nghiệp toàn cầu, cạnh tranh, do các công ty có nguồn lực tài chính và kỹ thuật dồi dào thống trị. Việc tìm kiếm các nguồn năng lượng giá rẻ đã trở thành trọng tâm chính, khi các thợ mỏ hướng đến việc tối ưu hóa chi phí vận hành và lợi nhuận. Hoạt động của Mining hiện nay trải rộng từ các cơ sở công nghiệp quy mô lớn đến các cơ sở quy mô nhỏ hoạt động trong nhà hoặc gara.

Hệ sinh thái Mining bao gồm một số thành phần chính:

- **Nhà sản xuất phần cứng**: Các công ty như Bitmain thiết kế và sản xuất ASIC (Mạch tích hợp ứng dụng chuyên biệt), chip siêu chuyên dụng được tạo riêng cho Mining Bitcoin.
- **Nhóm Mining**: Đây là tập hợp các thợ đào kết hợp sức mạnh tính toán của mình để cải thiện cơ hội nhận phần thưởng. Do độ khó của Mining ngày càng tăng, các nhóm này cung cấp mức thanh toán dễ dự đoán hơn bằng cách phân phối phần thưởng khối (bitcoin mới đào và phí giao dịch) cho những người tham gia dựa trên đóng góp của họ. Ví dụ bao gồm Foundry USA, AntPool, F2Pool, MARA Pool và Braiins Pool.
- **Thợ đào**: Đây là những cá nhân hoặc tổ chức vận hành phần cứng và phần mềm Mining. Một bên là các thợ đào quy mô nhỏ sử dụng các máy móc như Antminer S9, và bên kia là các hoạt động công nghiệp như Galaxy Digital, đơn vị quản lý các cơ sở vật chất khổng lồ dành riêng cho Mining.

Mining là một thế giới riêng biệt, với nhiều tầng lớp để khám phá; những thách thức kỹ thuật, các ưu đãi kinh tế và các cân nhắc về năng lượng đều được đưa vào. Đối với những ai quan tâm đến việc tìm hiểu sâu hơn về lĩnh vực này và thực sự hiểu cách thức hoạt động của nó, khóa học MIN201 của chúng tôi sẽ hướng dẫn bạn mọi kiến thức cần thiết.

https://planb.academy/courses/ce272232-0d97-4482-884a-0f77a2ebc036

#### Sự phát triển trong hệ sinh thái Bitcoin

Trọng tâm của sự phát triển kỹ thuật của Bitcoin nằm ở Bitcoin core, phần mềm máy khách được sử dụng rộng rãi nhất để chạy một nút Bitcoin. Đây là một dự án mã nguồn mở, hoàn toàn minh bạch và có sẵn công khai trên GitHub: [https://github.com/Bitcoin/Bitcoin](https://github.com/Bitcoin/Bitcoin). nơi bất kỳ ai cũng có thể xem lại mã, theo dõi các cuộc thảo luận và xem giao thức phát triển như thế nào. Mặc dù các bản cập nhật được đề xuất và tranh luận, không ai bị ép buộc phải áp dụng chúng và người dùng vẫn kiểm soát được phiên bản họ chạy.

Sự phát triển của Bitcoin có thể được hiểu thông qua một số nhóm cộng tác viên riêng biệt:

- **Bitcoin core developer**: Đây là những cá nhân bảo trì và cải thiện phần mềm máy khách chính. Trong số đó có những người bảo trì, nắm giữ chìa khóa quản lý kho lưu trữ. Năm 2025, có năm người trong số họ: Hennadii Stepanov, Michael Ford, Ava Chow, Gloria Zhao và Ryan Ofsky. Ngoài ra còn có những người đóng góp, những người gửi các thay đổi mã, sửa lỗi hoặc cải tiến. Các đề xuất này sẽ được bình duyệt ngang hàng và thảo luận trong cộng đồng trước khi được chấp nhận.
- **Các nhà phát triển giao thức phân lớp**, Nhóm này nghiên cứu các công nghệ được xây dựng dựa trên Bitcoin, như Lightning Network hoặc RGB, nhằm mục đích mở rộng khả năng của Bitcoin mà không thay đổi lõi của nó.
- **Các nhà phát triển độc lập**, Những nhà phát triển này tập trung vào việc tạo ra các công cụ và ứng dụng để cải thiện trải nghiệm của người dùng, chẳng hạn như Mempool.space (Interface trực quan để theo dõi hoạt động giao dịch) hoặc Alby (các công cụ để sử dụng thanh toán Lightning trong trình duyệt và ứng dụng).

Bất kỳ ai cũng có thể đề xuất thay đổi đối với Bitcoin core, nhưng quy trình này được thiết kế rất nghiêm ngặt. Các ý tưởng mới thường mất nhiều năm để hoàn thiện và đòi hỏi sự hiểu biết chuyên môn sâu rộng, sự tham gia rộng rãi của cộng đồng và nhiều lớp đánh giá. Các đề xuất thường được nộp dưới dạng **Đề xuất Cải tiến Bitcoin ([BIPs](https://planb.academy/resources/glossary/bip))**; một số trong đó không bao giờ được đưa vào giao thức.

Sự đổi mới được hoan nghênh, nhưng chỉ khi nó được hỗ trợ bởi lý lẽ vững chắc, sự đồng thuận của cộng đồng và thử nghiệm cẩn thận.

![image](assets/vi/054.webp)

Bất chấp những gì một số người có thể nghĩ, không ai có quyền kiểm soát đơn phương đối với Bitcoin; ngay cả những người bảo trì Bitcoin core cũng vậy. Vai trò của họ là quản lý kho phần mềm, chứ không phải bản thân giao thức.

![image](assets/vi/055.webp)

Ngay cả khi người bảo trì chấp thuận một thay đổi gây tranh cãi, nó cũng sẽ không ảnh hưởng đến mạng trừ khi **các nút (do người dùng vận hành)** thực sự áp dụng và chạy phiên bản đó. Cuối cùng, mã của Bitcoin chỉ có giá trị nếu mọi người chọn chạy nó.

Cũng cần lưu ý rằng **Bitcoin core không phải là máy khách duy nhất**. Các lựa chọn thay thế như Bitcoin Knots cũng triển khai giao thức Bitcoin, mang đến cho người dùng nhiều lựa chọn hơn và củng cố tính phi tập trung của hệ thống:

https://planb.academy/tutorials/node/bitcoin/bitcoin-knots-e04b2196-4df2-4246-86ef-c02269c29098

## Kiến trúc phân lớp của Bitcoin

<chapterId>03017765-53cf-5f14-9682-e99ca02d2241</chapterId>

:::video id=6b7c8d9e-0f1a-2b3c-4d5e-6f7a8b9c0d1e:::

Bitcoin là một hệ thống mở được thiết kế tối giản, mạnh mẽ và bảo mật ngay từ đầu. Để bổ sung chức năng mà không làm thay đổi nền tảng, các cải tiến thường được thực hiện bằng cách thêm **các lớp giao thức** và các ứng dụng bổ sung, làm phong phú hệ sinh thái mà không ảnh hưởng đến tính phi tập trung và khả năng phục hồi của hệ thống chính. Tính linh hoạt này đã cho phép nhiều công ty và nhà phát triển độc lập xây dựng cơ sở hạ tầng xung quanh Bitcoin, bổ sung các cải tiến phù hợp với nhiều trường hợp sử dụng khác nhau.

### Phần mở rộng Bitcoin với các lớp bổ sung

Phương pháp tiếp cận phân lớp cho phép cải tiến Bitcoin mà không cần thay đổi giao thức cốt lõi, đảm bảo tính ổn định và bảo mật của hệ thống chính. Phương pháp này tương tự như cách thức hoạt động của Internet, nơi nhiều giao thức được xây dựng dựa trên nhau để cung cấp các chức năng riêng biệt trong khi vẫn duy trì khả năng tương tác mượt mà.

Trong số các hệ thống lớp phủ chính làm phong phú thêm hệ sinh thái Bitcoin là:

- **Lightning Network**:

Lightning Network, được Thaddeus Dryja và Joseph Poon tạo ra vào năm 2016, là giải pháp Layer thứ hai được thiết kế để cho phép thanh toán tức thì và chi phí thấp. Hai người dùng có thể mở một [kênh](https://planb.academy/resources/glossary/payment-channel) riêng, nơi họ có thể giao dịch và số dư chỉ được cập nhật trên Blockchain khi kênh được mở hoặc đóng. Các giao dịch trong kênh được thực hiện trên off-chain, nghĩa là chúng không cần được ghi lại riêng lẻ trên Bitcoin Blockchain. Cấu trúc này cho phép giao dịch tức thì với mức phí tối thiểu, lý tưởng cho các giao dịch giá trị thấp cần xác nhận nhanh chóng.

![image](assets/vi/059.webp)

Giả sử bạn mua cà phê bằng Bitcoin sử dụng mã cơ sở Layer. Để xác nhận thanh toán (và để quán cà phê chắc chắn rằng bạn đã thực sự thanh toán), giao dịch cần được đưa vào một khối. Quá trình này có thể mất vài phút, tùy thuộc vào mức phí bạn đã chọn. Về mặt kỹ thuật, người bán hàng phải đợi sáu lần xác nhận (khoảng một giờ) để hoàn toàn tin tưởng rằng khoản thanh toán đã hoàn tất. Rõ ràng, việc chờ đợi như vậy sẽ không hiệu quả khi bạn đang đứng tại quầy. Với Lightning Network, giao dịch được xử lý chỉ trong vài giây; vì vậy, cà phê của bạn đã được thanh toán và phục vụ trước cả khi kịp nguội.

Nếu bạn muốn tìm hiểu thêm về cách thức hoạt động của Lightning, chúng tôi cung cấp khóa học năm thứ hai tuyệt vời dành riêng cho chủ đề này:

https://planb.academy/courses/34bd43ef-6683-4a5c-b239-7cb1e40a4aeb

- **Sidechains**:

Sidechain là các blockchain chạy song song với Blockchain chính của Bitcoin. Chúng được kết nối thông qua một chốt hai chiều, đảm bảo rằng tài sản di chuyển giữa các chuỗi vẫn giữ nguyên giá trị; nghĩa là một Bitcoin trên Sidechain vẫn có giá trị bằng một Bitcoin trên chuỗi chính. Mỗi Sidechain có cơ chế đồng thuận riêng, có thể hoàn toàn tách biệt hoặc phụ thuộc một phần vào Bitcoin.

Ưu điểm chính của sidechain là chúng có thể cung cấp các tính năng không có trên Bitcoin cơ sở Layer; hoặc cung cấp chúng theo những cách cải tiến hơn. Điều này bao gồm tính linh hoạt hơn cho các nhà phát triển, giao dịch nhanh hơn và/hoặc riêng tư hơn, và thông lượng giao dịch lớn hơn. Tuy nhiên, để mang lại những lợi ích này, sidechain thường phải đánh đổi những thứ khác so với chuỗi chính của Bitcoin.

Khái niệm sidechain được giới thiệu vào năm 2014 bởi Adam Back, Matt Corallo, Luke Dashjr, Mark Friedenbach, Gregory Maxwell, Andrew Miller, Andrew Poelstra, Jorge Timon và Pieter Wuille. Tính đến năm 2025, các sidechain nổi tiếng nhất trong hệ sinh thái Bitcoin là Liquid và RSK (Rootstock).

Nếu bạn muốn tìm hiểu chi tiết hơn về Liquid, chúng tôi cung cấp khóa học nâng cao năm thứ ba về chủ đề này:

https://planb.academy/courses/6d26bcff-51a3-405f-bcdd-9af8297ce727

- **RGB**:

RGB là một hệ thống Hợp đồng thông minh (Smart contract) phi tập trung và tập trung vào quyền riêng tư, được thiết kế để hoạt động trên nền tảng Bitcoin và Lightning Network. Không giống như các nền tảng Smart contract truyền thống, RGB sử dụng mô hình Client-side Validation (nghĩa là toàn bộ Contract State được lưu trữ trên off-chain, và chỉ các cam kết mật mã được công bố trên Bitcoin Blockchain). Thiết kế này cải thiện cả khả năng mở rộng và quyền riêng tư. Với RGB, người dùng có thể tạo các hợp đồng thông minh nâng cao để phát hành token, NFT, danh tính phi tập trung, hoặc thậm chí cả các ứng dụng DeFi, trực tiếp trên Bitcoin hoặc Lightning.

Một tính năng quan trọng của RGB là khả năng bảo vệ chống lại Double-spending, đạt được bằng kỹ thuật mã hóa gọi là Dấu niêm phong sử dụng một lần. Cơ chế này dựa trên thực tế là [UTXO](https://planb.academy/resources/glossary/utxo) (Đầu ra giao dịch chưa sử dụng) của Bitcoin chỉ có thể được sử dụng một lần. Tính xác thực của mã thông báo được đảm bảo bằng việc xác thực phía người dùng về toàn bộ lịch sử của Contract (từ khi tạo ra đến trạng thái hiện tại).

![image](assets/vi/060.webp)

Để nâng cao kiến thức của bạn về RGB, chúng tôi cung cấp khóa đào tạo năm thứ tư (vui lòng lưu ý rằng khóa học này có tính kỹ thuật cao):

https://planb.academy/courses/3ce1d37c-05ba-4f54-aa15-7586d37b2bb7

RGB chỉ là một trong nhiều giao thức được xây dựng dựa trên Bitcoin. Mặc dù một số giao thức được áp dụng rộng rãi hơn những giao thức khác, nhưng những giao thức mới vẫn tiếp tục xuất hiện. Điểm chung là ý tưởng tối ưu hóa từng giao thức Layer cho một tác vụ cụ thể, đồng thời vẫn duy trì tính toàn vẹn và tính bất biến của giao thức cơ sở Bitcoin.

Thiết kế phân lớp này trái ngược với phần lớn ngành công nghiệp tiền điện tử nói chung, vốn thường tìm cách tích hợp nhiều tính năng vào một giao thức duy nhất. Bằng cách giữ cho Bitcoin đơn giản và tập trung vào một phạm vi hẹp, chúng tôi giảm thiểu bề mặt tấn công; đồng nghĩa với việc bảo mật cao hơn. Một giao thức tinh gọn dễ bảo mật, duy trì và mở rộng hơn. Bitcoin được thiết kế để thực hiện một mục tiêu cực kỳ hiệu quả: cung cấp tiền tệ phi tập trung, an toàn. Mọi thứ khác (hợp đồng thông minh, token, thanh toán, v.v.) đều có thể được tích hợp lên trên, cho phép đổi mới mà không ảnh hưởng đến cốt lõi.

![image](assets/vi/058.webp)

**Bạn có biết**? Internet không được xây dựng cùng một lúc; nó phát triển như một chồng giao thức có thể tương tác. Ví dụ, TCP/IP xử lý giao tiếp mạng, HTTP hỗ trợ web, và nhiều lớp khác đảm nhiệm các chức năng cụ thể. Mỗi Layer được tối ưu hóa cho từng chức năng, tạo nên một hệ thống mạnh mẽ và mô-đun. Bitcoin cũng tuân theo triết lý này. Layer cơ bản của nó mạnh mẽ và tối giản, và các chức năng bổ sung được bổ sung thông qua các giao thức phân lớp như Lightning, Liquid hoặc RGB; mỗi giao thức tập trung vào việc giải quyết các nhu cầu khác nhau của người dùng trong khi vẫn giữ nguyên nền tảng.

### Công cụ thương mại để chấp nhận Bitcoin

Ngày nay, có rất nhiều công cụ dành cho các thương gia muốn chấp nhận Bitcoin làm phương thức thanh toán. Đối với các doanh nghiệp nhỏ đang tìm kiếm một thiết lập đơn giản, việc sử dụng [ví nóng (hot wallet)](https://planb.academy/resources/glossary/hot-wallet--software-wallet) (hoặc thậm chí là Lightning Wallet) thường là đủ để bắt đầu chấp nhận thanh toán trực tiếp. Các doanh nghiệp lớn hơn yêu cầu hệ thống kế toán và báo cáo phù hợp thường sẽ ưu tiên các hệ thống xử lý thanh toán tiên tiến hơn. May mắn thay, có một số tùy chọn có sẵn tùy thuộc vào nhu cầu của bạn.

Nếu bạn thích giải pháp không cần can thiệp và muốn nhận tiền pháp định trực tiếp vào tài khoản ngân hàng của mình, các dịch vụ lưu ký như OpenNode cung cấp trải nghiệm hợp lý:

https://planb.academy/tutorials/business/point-of-sale/open-node-e69a0c1c-47f7-4932-8494-e6f26c3c9784

Đối với các thương nhân có thiên hướng kỹ thuật hơn và muốn kiểm soát toàn bộ quy trình, BTCPay Server là một lựa chọn mã nguồn mở tuyệt vời. Nhược điểm chính là cần thời gian để thiết lập và bảo trì, cùng với một số kiến thức kỹ thuật:

https://planb.academy/tutorials/business/point-of-sale/btcpay-server-928eb01e-824b-4b57-a3e8-8727633beddc

Ở đâu đó giữa hai lựa chọn này, bạn sẽ tìm thấy Swiss Bitcoin Pay, một giải pháp thân thiện với người dùng nhưng mạnh mẽ, cân bằng tốt giữa tính dễ sử dụng, chức năng và bảo mật. Giải pháp này phù hợp cho cả các nhà bán lẻ nhỏ và doanh nghiệp lớn:

https://planb.academy/tutorials/business/point-of-sale/swiss-bitcoin-pay-2-a78b057e-ed11-47ac-860c-71019fcb451a

Việc chấp nhận Bitcoin có thể mang lại nhiều lợi ích thiết thực và tài chính cho doanh nghiệp. Giống như tiền mặt, Bitcoin cho phép thanh toán trực tiếp giữa khách hàng và nhà cung cấp (không cần thông qua ngân hàng truyền thống). Các khoản thanh toán được thực hiện thông qua Lightning Network là ngay lập tức và cuối cùng, giúp giảm thiểu rủi ro bị hoàn tiền. Và khi nhà cung cấp nắm giữ Bitcoin (tự lưu giữ), họ sẽ có được quyền tự chủ tài chính lớn hơn.

Nó cũng có thể giúp cắt giảm chi phí bằng cách loại bỏ phí ngân hàng và nhu cầu sử dụng thiết bị đầu cuối thanh toán truyền thống; thường thì bạn chỉ cần một chiếc điện thoại thông minh hoặc máy tính xách tay. Ngay cả khi có bộ xử lý thanh toán, phí thường thấp hơn phí ngân hàng.

Không giống như các loại tiền tệ truyền thống mất giá theo thời gian do lạm phát, Bitcoin có số lượng Supply cố định là 21 triệu đồng. Điều này khiến nó trở thành một tài sản giá trị để bảo toàn và đa dạng hóa nguồn vốn kinh doanh trong dài hạn.

Trong hoạt động thường ngày, Bitcoin đơn giản hóa thanh toán bằng cách loại bỏ nhu cầu sử dụng tiền mặt, giảm thiểu rủi ro trộm cắp và loại bỏ khả năng tiền giả. Đây là loại tiền tệ toàn cầu, lý tưởng cho khách hàng quốc tế vì không cần chuyển đổi tiền tệ. Đối với các cửa hàng trực tuyến, Bitcoin đặc biệt an toàn và hiệu quả.

Hơn nữa, việc chấp nhận Bitcoin có thể là một bước đi tiếp thị thông minh. Điều này cho thấy doanh nghiệp của bạn có tư duy tiến bộ và có thể thu hút khách hàng mới (đặc biệt là thế hệ trẻ như Gen Z). Đây là một cơ hội chiến lược, ít rủi ro với chi phí tối thiểu, chủ yếu giới hạn ở khâu thiết lập ban đầu; giờ đây việc này trở nên dễ dàng hơn bao giờ hết với các công cụ phù hợp.

Nếu bạn muốn tìm hiểu cách tích hợp Bitcoin vào doanh nghiệp của mình (dù là phương thức thanh toán, tài sản kho bạc hay cả hai), chúng tôi cung cấp khóa học dành cho người mới bắt đầu phù hợp với nhu cầu đó:

https://planb.academy/courses/a804c4b6-9ff5-4a29-a530-7d2f5d04bb7a

Bitcoin đang ngày càng được sử dụng rộng rãi như một phương tiện trao đổi, với sự chấp nhận ngày càng tăng trong nhiều ngành công nghiệp. Mạng lưới Lightning Network đã giúp thanh toán nhanh hơn và rẻ hơn, càng làm tăng sức hấp dẫn của Bitcoin đối với các nhà bán lẻ.

Chúng tôi đã đạt đến điểm mà bất kỳ ai cũng có thể tham gia vào hệ sinh thái Bitcoin; cho dù là sử dụng trong cuộc sống hàng ngày, áp dụng trong kinh doanh, đóng góp vào giáo dục, giúp cải thiện mã hay xây dựng các ứng dụng mới.

Bitcoin hiện không thể ngăn cản được nữa.

![image](assets/vi/057.webp)

### Quan điểm cá nhân của tôi

Tôi luôn thấy phép ẩn dụ "xa lộ Bitcoin" là một trong những cách chính xác và thuyết phục nhất để hiểu hệ sinh thái đang phát triển như thế nào; và nó đang hướng đến đâu. Bitcoin không chỉ là tiền kỹ thuật số; nó là **một hệ thống tài chính thay thế đang phát triển**, với những điểm mạnh và điểm yếu riêng. Tuy còn non trẻ và đang đối mặt với nhiều thách thức, nhưng khả năng phục hồi của nó là không thể phủ nhận. Nó sẽ không biến mất. Ngược lại, giống như một hố đen, nó sẽ dần dần hấp thụ mọi thứ xung quanh cho đến khi trở thành một tiêu chuẩn tiền tệ không thể phủ nhận.

Hãy hình dung Bitcoin như một con đường bạn đang lái xe. Hiện tại, để lo liệu những nhu cầu thiết yếu hàng ngày (mua sắm, thanh toán dịch vụ, hoặc sửa xe), đôi khi bạn phải rời khỏi con đường này; nghĩa là bạn tạm thời quay lại hệ thống tài chính cũ. Đó là bởi vì cơ sở hạ tầng của Bitcoin vẫn đang được xây dựng, và một số khía cạnh của cuộc sống hàng ngày vẫn phụ thuộc vào tiền tệ fiat và ngân hàng.

Nhưng theo thời gian, con đường này sẽ trở thành đường cao tốc được xây dựng hoàn chỉnh.

Đó là cách tôi hình dung tương lai của Bitcoin. Nó có thể không thay thế hoàn toàn tài chính truyền thống, nhưng sẽ vượt trội hơn ở những khía cạnh then chốt (hiệu quả, bảo mật và khả năng ứng dụng của người dùng) cho đến khi trở thành tiêu chuẩn cho hầu hết các quốc gia trên thế giới.

Nếu tôi nhớ không nhầm thì ẩn dụ về đường cao tốc Bitcoin này được Andreas Antonopoulos giới thiệu lần đầu tiên. Tầm nhìn của ông vẫn còn giá trị đến ngày nay, và với mỗi bước tiến, chúng ta lại càng tiến gần hơn đến việc hiện thực hóa nó.[@aantonop](https://x.com/aantonop)

Tiếp theo trong khóa học: chúng ta sẽ đi vào thực tế và giúp bạn xác định hồ sơ bitcoiner của mình dựa trên tình hình và mục tiêu cá nhân của bạn.

# Thiết lập kế hoạch của bạn

<partId>3801faf6-7915-56fa-baf5-ee63ad03b7cf</partId>

## Chọn hồ sơ của bạn

<chapterId>c5d87903-a5f2-5eec-887a-f662734ce49b</chapterId>

:::video id=1a2b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d:::

:::id video=1a2b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d:::

Giờ bạn đã nắm được nền tảng lý thuyết của Bitcoin, đã đến lúc chuyển sang phần thực hành. Phần này sẽ giúp bạn hiểu cách kiếm những đồng bitcoin đầu tiên, cách lưu trữ chúng một cách an toàn và cách điều chỉnh phương pháp tiếp cận dựa trên tình hình cá nhân. Mục tiêu là giúp bạn xác định mình thuộc loại người dùng nào để có thể áp dụng chiến lược phù hợp với nhu cầu và hạn chế của mình.

Bitcoin không giống như các loại tài sản truyền thống. Nó đòi hỏi một cách tiếp cận thận trọng và sự hiểu biết sâu sắc về cả tác động kỹ thuật lẫn kinh tế của việc sử dụng nó. Không giống như tiền tệ fiat, Bitcoin hoạt động theo mô hình chủ quyền cá nhân; nghĩa là bạn hoàn toàn chịu trách nhiệm về việc quản lý và bảo mật tiền của mình. Như đã thảo luận trong các phần trước, quyền tự chủ tài chính này là một lợi thế lớn, nhưng nó cũng đi kèm với các phương pháp hay nhất cụ thể, mà chúng ta sẽ tìm hiểu trong các chương tiếp theo.

Khóa học này giả định rằng bạn mới bắt đầu với Bitcoin. Vì vậy, chúng tôi sẽ tập trung vào các giải pháp đơn giản, dễ sử dụng cho người mới bắt đầu và dễ thiết lập. Chúng tôi sẽ không đề cập đến các phương pháp tiếp xúc gián tiếp như Mining, mua cổ phiếu của các công ty liên quan đến Bitcoin, hoặc các chiến lược tài chính phức tạp. Mục tiêu ở đây là giúp bạn chọn Wallet phù hợp với tình hình của mình và đầu tư Bitcoin đầu tiên một cách an toàn.

![image](assets/vi/063.webp)

### Tại sao phải xác định hồ sơ người dùng của bạn?

Không có phương pháp chung nào để mua và quản lý BTC. Mỗi người dùng đều có bối cảnh riêng, được hình thành bởi tình hình tài chính, kiến thức kỹ thuật và kỳ vọng của họ về Bitcoin. Đó là lý do tại sao việc lựa chọn một chiến lược phù hợp với hồ sơ cá nhân của bạn là rất quan trọng. Hiểu rõ phương pháp của riêng mình sẽ giúp bạn đưa ra quyết định tốt hơn và tránh những sai lầm thường gặp do đánh giá sai nhu cầu.

Sau đây là một số yếu tố cần cân nhắc khi tìm ra cách tốt nhất để mua và lưu trữ bitcoin:

- **Số tiền bạn muốn đầu tư**: Bạn đang mua một khoản tiền nhỏ chỉ để thử nghiệm và học hỏi mà không phải chịu nhiều rủi ro? Hay bạn đang lên kế hoạch đầu tư lớn hơn để nắm giữ lâu dài?
- **Tần suất đầu tư của bạn**: Bạn muốn mua một lần và đảm bảo tiền của mình ngay lập tức, hay bạn muốn phân bổ số tiền mua theo thời gian bằng cách sử dụng chiến lược như Trung bình chi phí đô la (DCA) để làm phẳng biến động giá?
- **Bạn muốn làm gì với số bitcoin của mình**: Bạn chỉ giữ nó như một khoản tiết kiệm dài hạn? Dùng nó để thanh toán hàng ngày? Hay có thể là cả hai?
- **Quan điểm của bạn về quyền riêng tư**: Bạn có thoải mái cung cấp thông tin cá nhân và sử dụng các nền tảng tập trung để mua Bitcoin không? Hay bạn thích các phương pháp ưu tiên quyền riêng tư như giao dịch ngang hàng, không cần KYC?
- **Bối cảnh cá nhân, pháp lý và địa lý của bạn**: Tùy thuộc vào nơi bạn sống, quyền truy cập vào một số sàn giao dịch nhất định có thể bị hạn chế. Luật pháp và quy định thuế địa phương cũng có thể ảnh hưởng đến cách bạn lưu trữ và sử dụng mẫu Bitcoin.

Mỗi yếu tố này đều đóng vai trò trong việc định hình chiến lược phù hợp cho bạn. Một số người dùng sẽ coi trọng sự đơn giản và dễ sử dụng thông qua các dịch vụ được quản lý, trong khi những người khác lại thích phương pháp tiếp cận tự chủ hơn và tập trung vào quyền riêng tư.

![image](assets/vi/064.webp)

Đó là những gì chúng ta sẽ khám phá tiếp theo: những cách khác nhau để sở hữu và bảo mật bitcoin của bạn, để bạn có thể đưa ra quyết định sáng suốt dựa trên hồ sơ và mục tiêu của mình.

### Tiêu chí chính để xác định hồ sơ của bạn

Việc lựa chọn chiến lược phù hợp để sở hữu và bảo mật Bitcoin của bạn bắt đầu bằng việc đánh giá trung thực tình hình cá nhân. Có nhiều yếu tố ảnh hưởng đến những quyết định này, bao gồm rủi ro bạn có thể gặp phải, lối sống của bạn và cách bạn dự định sử dụng Bitcoin.

![image](assets/vi/065.webp)

#### Rủi ro và mối đe dọa tiềm ẩn

Việc lựa chọn chiến lược phù hợp để sở hữu và bảo mật Bitcoin của bạn bắt đầu bằng việc đánh giá trung thực tình hình cá nhân. Có nhiều yếu tố ảnh hưởng đến những quyết định này, bao gồm rủi ro bạn có thể gặp phải, lối sống của bạn và cách bạn dự định sử dụng Bitcoin:

- **Tấn công mạng**: Nếu bạn không am hiểu về an ninh mạng hoặc đang sử dụng các thiết bị dễ bị tấn công, bạn có thể trở thành mục tiêu của các cuộc tấn công trực tuyến. Trong trường hợp đó, lưu trữ bitcoin ngoại tuyến (lưu trữ Cold) có thể là một lựa chọn an toàn hơn.

- **Đe dọa vật lý**: Thật không may, các vụ tấn công vật lý vẫn xảy ra; đột nhập, cướp bóc, thậm chí bắt cóc đều nhắm vào người sở hữu thẻ Bitcoin. Mặc dù những sự cố này vẫn hiếm gặp và chủ yếu ảnh hưởng đến những cá nhân có địa vị cao, nhưng bạn nên cân nhắc chúng ngay từ đầu khi thiết kế hệ thống an ninh.

- **Thảm họa thiên nhiên**: Nếu bạn sống ở khu vực dễ xảy ra cháy rừng, lũ lụt hoặc các hiện tượng thời tiết khắc nghiệt khác, điều quan trọng là phải có hệ thống sao lưu mạnh mẽ để bảo vệ khóa riêng của bạn.

- **Tịch thu của chính phủ**: Nếu quốc gia của bạn có các quy định tài chính hạn chế hoặc không ổn định, bạn có thể phải đối mặt với các hạn chế về việc mua hoặc sử dụng Bitcoin. Trong những trường hợp như vậy, bạn nên ưu tiên các giải pháp tư nhân, không lưu ký.

- **Mất trí nhớ hoặc mất dữ liệu**: Việc quản lý khóa riêng tư đòi hỏi bạn phải lưu trữ và truy xuất thông tin nhạy cảm. Nếu bạn lo lắng về chứng hay quên hoặc các vấn đề sức khỏe tiềm ẩn, hãy triển khai các hệ thống sao lưu mạnh mẽ; hoặc thậm chí nhờ một người đáng tin cậy tham gia vào kế hoạch bảo mật của bạn.

Để hiểu rõ hơn về các điểm yếu của mình, hãy đánh giá từng rủi ro này theo thang điểm từ 0 (thấp) đến 10 (cao). Việc tự đánh giá này sẽ giúp bạn ưu tiên các biện pháp bảo vệ phù hợp trong tương lai.

![image](assets/vi/061.webp)

#### Phong cách sống và thói quen cá nhân

Cuộc sống hàng ngày của bạn cũng đóng vai trò quan trọng trong việc quản lý bitcoin. Việc lựa chọn một thiết lập phù hợp với lối sống của bạn sẽ giúp mọi việc dễ dàng hơn và tránh những rắc rối không đáng có.

**Thời gian có hạn hay lãi suất có hạn?** Hãy chọn các giải pháp tự động, đơn giản; chẳng hạn như các giao dịch mua theo lịch trình được tự động chuyển sang kho lưu trữ an toàn.

**Am hiểu công nghệ hay thực hành?** Bạn có thể thích các giải pháp tiên tiến hơn như ví Multisig mang lại khả năng kiểm soát và bảo mật tốt hơn.

**Đầu tư cho người thân hoặc thế hệ tương lai?** Hãy cân nhắc sớm về việc thừa kế. Có nhiều cách để đảm bảo Bitcoin của bạn có thể được truyền lại một cách an toàn nếu có chuyện gì xảy ra với bạn.

**Quan tâm đến quyền riêng tư?** Một số phương thức mua hàng yêu cầu thông tin cá nhân (nền tảng KYC), trong khi những phương thức khác (như ngang hàng hoặc Mining) mang lại sự kín đáo hơn.

#### Sử dụng bitcoin của bạn

Chiến lược lưu trữ và quản lý của bạn nên phản ánh đúng mục đích sử dụng Bitcoin của bạn. Việc xác định rõ ràng ngay từ đầu sẽ giúp bạn đưa ra quyết định chính xác.

- **Tiết kiệm dài hạn**: Bạn đang mua Bitcoin như một khoản đầu tư và không có kế hoạch sử dụng nó trong thời gian tới. Trong trường hợp này, bảo mật tối đa và rủi ro truy cập được giảm thiểu là chìa khóa.
- **Tích lũy dần dần**: Bạn mua số lượng nhỏ thường xuyên. Một chiến lược như Trung bình Chi phí Đô la (DCA), kết hợp với Wallet an toàn, sẽ hiệu quả.
- **Chi phí hàng ngày**: Bạn sử dụng Bitcoin như tiền mặt. Hãy sử dụng Wallet di động cho các khoản thanh toán nhỏ hàng ngày (có thể dùng Lightning), và giữ phần lớn số tiền Bitcoin ở nơi an toàn hơn.
- **Chuẩn bị nghỉ hưu hoặc chuyển giao tài sản**: Bạn muốn chuyển giao tài sản theo mẫu Bitcoin cho người khác sau này. Hãy sử dụng các công cụ giúp việc chuyển giao tài sản dễ dàng và an toàn khi đến thời điểm thích hợp. Các chiến lược như Multisig với khóa thời gian hoặc kế hoạch thừa kế cụ thể có thể được áp dụng để đảm bảo tài sản của bạn không bị mất hoặc bị truy cập quá sớm.

Sau khi đã cân nhắc kỹ lưỡng những rủi ro, lối sống và mục tiêu của mình, bạn sẽ sẵn sàng đưa ra những lựa chọn thông minh cho hành trình Bitcoin của mình.

Sau đây là bốn điều quan trọng cần quyết định:

- Sử dụng Ví nào?
- Cách bảo vệ Bitcoin của bạn
- Bạn sẽ mua Bitcoin như thế nào
- Bạn sẽ giao dịch bao nhiêu

![image](assets/vi/062.webp)

### Ví và Chiến lược Bảo mật

Bảo mật khóa riêng tư (khóa cho phép truy cập vào bitcoin của bạn) là phần quan trọng nhất khi sở hữu và sử dụng Bitcoin. Không giống như tài khoản ngân hàng truyền thống, nơi bên thứ ba quản lý tiền của bạn, Bitcoin trao cho bạn toàn quyền kiểm soát. Nhưng sự tự do đó đi kèm với trách nhiệm: nếu bạn làm mất khóa, Bitcoin của bạn sẽ biến mất mãi mãi. Có một số loại ví bạn có thể sử dụng. Mỗi loại có ưu và nhược điểm riêng tùy thuộc vào nhu cầu và trình độ kinh nghiệm của bạn.

![image](assets/vi/067.webp)

#### Ví Nóng (Hot Wallet)

Ví Nóng là các ứng dụng hoặc phần mềm được kết nối với internet. Chúng lưu trữ khóa riêng của bạn trên cùng một thiết bị đã cài đặt. Những ví này rất phù hợp để sử dụng hàng ngày hoặc lưu trữ một lượng nhỏ Bitcoin.

Ví dụ: Blue Wallet, Green Wallet, Sparrow wallet
Với sự hỗ trợ của Lightning: Phoenix, Wallet của Satoshi, BitKit

**Ưu điểm**:

- Dễ sử dụng và truy cập nhanh vào tiền của bạn.
- Thích hợp cho các khoản thanh toán nhỏ và sử dụng hàng ngày.
- Một số hỗ trợ Lightning Network để giao dịch nhanh chóng và rẻ.

**Nhược điểm**:

- Ít an toàn hơn: chìa khóa của bạn nằm trên một thiết bị được kết nối với Internet, làm tăng nguy cơ bị hack.
- Không thích hợp để lưu trữ số lượng lớn trong thời gian dài.

Phù hợp nhất cho: Người mới bắt đầu, số dư nhỏ và giao dịch thường xuyên.

#### Ví phần cứng

Ví phần cứng là thiết bị vật lý lưu trữ khóa riêng của bạn hoàn toàn ngoại tuyến. Chúng an toàn hơn nhiều so với ví Hot vì chúng giảm thiểu nguy cơ bị tấn công trực tuyến.

Ví dụ: Ledger, Trezor, Coldcard, Jade, BitBox

**Ưu điểm**:

- Khóa ngoại tuyến = tin tặc khó truy cập hơn nhiều.
- Được thiết kế đặc biệt cho mục đích bảo mật.

**Nhược điểm**:

- Chậm hơn khi sử dụng; bạn cần kết nối thiết bị và xác nhận giao dịch bằng tay.
- Bạn sẽ cần phải mua thiết bị này và nó có thể tốn của bạn một khoản tiền kha khá.

Phù hợp nhất cho: Người nắm giữ lâu dài và bất kỳ ai muốn đảm bảo số tiền lớn hơn.

#### Ví đa chữ ký

Ví Multisig (Đa chữ ký) yêu cầu nhiều hơn một khóa để phê duyệt một giao dịch. Hãy tưởng tượng nó giống như một két sắt cần hai hoặc ba tổ hợp để mở. Những thiết lập này lý tưởng cho người dùng nâng cao, doanh nghiệp hoặc bất kỳ ai đang tìm kiếm bảo mật hàng đầu.

Ví dụ: Liana, Casa, Spectre, Sparrow

**Ưu điểm**:

- Khả năng bảo vệ rất mạnh mẽ; nếu một khóa bị xâm phạm, tiền của bạn vẫn an toàn.
- Có thể giảm thiểu rủi ro do trộm cắp, mất mát hoặc cưỡng ép.
- Bạn có thể phân phối chìa khóa giữa mọi người hoặc địa điểm.

**Nhược điểm**:

- Thiết lập và quản lý phức tạp hơn.
- Có thể cần sự phối hợp giữa nhiều người/thiết bị.
- Nguy cơ mất tiền nếu Multisig bị quản lý kém;
- Chi phí mua ban đầu cao hơn, có khả năng phải mua nhiều ví phần cứng.

Phù hợp nhất cho: Người dùng có nhu cầu cao, gia đình, công ty và lưu trữ lâu dài với tính bảo mật cao.

#### Dịch vụ lưu ký và ETF

Một số người chọn lưu trữ Bitcoin thông qua các dịch vụ của bên thứ ba như sàn giao dịch, đơn vị lưu ký hoặc ETF. Điều này cho phép bạn tiếp cận Bitcoin mà không cần tự quản lý khóa riêng. Tuy nhiên, điều này đi kèm với những đánh đổi lớn.

Ví dụ: Các sàn giao dịch như Coinbase hoặc Binance, các sản phẩm tài chính như Bitcoin ETF của BlackRock.

**Ưu điểm**:

- Dễ sử dụng; không cần phải tự mình cầm chìa khóa hoặc ví.
- Hữu ích cho những người chỉ muốn đầu tư, không muốn sử dụng trực tiếp Bitcoin.

**Nhược điểm**:

- Bạn không thực sự sở hữu Bitcoin của mình; đơn vị lưu ký mới là người sở hữu. Nếu chúng sụp đổ, tiền của bạn cũng có thể sụp đổ (hãy hỏi người dùng Mt. Gox hoặc FTX).
- Có khả năng phí cao hơn và ít quyền riêng tư hơn.
- Sự phụ thuộc vào một bên thứ ba đáng tin cậy, có nghĩa là từ bỏ chủ quyền, một trong những lợi ích chính của Bitcoin;
- Bạn không nhận được tất cả lợi ích của Bitcoin về quyền riêng tư và chủ quyền tài chính.

**Nguyên tắc chung: Không phải chìa khóa, không phải tiền xu.**

Chúng tôi không khuyến nghị sử dụng dịch vụ lưu ký ngay cả với người mới bắt đầu. Tốt hơn hết là bạn nên dành thời gian tìm hiểu cách bảo mật Bitcoin đúng cách thay vì mạo hiểm mất tất cả chỉ vì người khác làm sai. Nhưng nếu bạn đang đọc về ví và quản lý khóa ở đây, có lẽ bạn đã hiểu tại sao nguyên tắc đó lại quan trọng.

![image](assets/vi/068.webp)

Việc lựa chọn Wallet và phương pháp bảo mật phụ thuộc vào khối lượng dữ liệu bạn lưu trữ, trình độ chuyên môn và những rủi ro tiềm ẩn mà bạn nhận thấy. Tuy nhiên, phương pháp kết hợp có thể là một lựa chọn thông minh:

- Sử dụng Hot Wallet trên điện thoại hoặc Lightning Wallet cho các giao dịch hàng ngày;
- Sử dụng Hardware Wallet để bảo mật phần lớn số bitcoin của bạn, vốn được dùng để tiết kiệm dài hạn.

Nếu Hot Wallet của bạn tích lũy quá nhiều bitcoin, bạn có thể chuyển một ít sang kho lưu trữ Cold. Mặt khác, nếu bạn cần thêm bitcoin để chi tiêu hàng ngày, bạn có thể rút từ Cold Wallet.

![image](assets/vi/066.webp)

Cuối cùng, bảo mật phần lớn phụ thuộc vào cách bạn sử dụng các công cụ. Hardware Wallet sẽ chẳng giúp ích gì nếu bạn làm mất, tiết lộ hoặc xâm phạm cụm từ Mnemonic. Thận trọng và kỷ luật là biện pháp bảo vệ tốt nhất của bạn khỏi việc mất bitcoin. Để biết thêm về vấn đề này, tôi thực sự khuyên bạn nên đọc bài viết này:

https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### Phương pháp mua lại và khối lượng giao dịch

Có nhiều cách khác nhau để sở hữu Bitcoin, tùy thuộc vào mức độ riêng tư bạn cần, quy định của quốc gia bạn và số lượng bạn muốn sở hữu. Có hai cách chính:

![image](assets/vi/070.webp)

#### Thu thập thông tin bằng Xác minh danh tính (KYC)

Các nền tảng được quản lý thường yêu cầu bạn xác minh danh tính trước khi mua Bitcoin. Điều này có nghĩa là bạn sẽ cần cung cấp các giấy tờ chính thức (hộ chiếu, CCCD, giấy tờ chứng minh địa chỉ, v.v.) để tạo tài khoản và truy cập các dịch vụ mua bán.

**Ưu điểm**:

- Quy trình mua hàng đơn giản, thường phù hợp với người mới bắt đầu;
- Khả năng mua số lượng lớn cùng một lúc;
- Truy cập vào các dịch vụ bổ sung (DCA, chuyển đổi tự động, rút tiền fiat dễ dàng);
- Tính thanh khoản cao với mức chênh lệch nhỏ giữa giá mua và giá bán.

**Nhược điểm**:

- Phí chuyển đổi và rút tiền cao;
- Một số nền tảng phức tạp và có thể khiến bạn choáng ngợp với các tính năng giao dịch nâng cao hoặc các loại tiền điện tử thay thế;
- Các vấn đề lớn về quyền riêng tư.

Nhược điểm lớn nhất của các sàn giao dịch KYC là rủi ro về quyền riêng tư, điều này cũng có thể ảnh hưởng đến bảo mật cá nhân của bạn. KYC cho phép các sàn giao dịch liên kết danh tính thực của bạn với địa chỉ rút tiền Bitcoin. Thông tin này trở thành điểm khởi đầu lý tưởng cho việc phân tích Blockchain. Khi xác định được Address, tất cả các giao dịch liên quan đều có thể bị theo dõi, gây ảnh hưởng đến quyền riêng tư On-Chain của bạn. Nếu không có các biện pháp bảo vệ bổ sung, tất cả hoạt động Bitcoin của bạn đều có thể bị truy ngược về bạn.

Một rủi ro lớn khác là chính phủ, với quyền truy cập vào cơ sở dữ liệu của các nền tảng được quản lý, có thể dễ dàng xác định những cá nhân đã mua BTC. Mặc dù điều này có vẻ vô hại hiện nay, nhưng nó có thể trở thành vấn đề nếu các hạn chế kinh tế hoặc chính trị được áp đặt. Hơn nữa, các nền tảng được quản lý phải báo cáo bất kỳ giao dịch đáng ngờ nào, làm tăng nguy cơ bị các cơ quan công quyền giám sát.

Ngoài ra, các nền tảng KYC lưu trữ rất nhiều dữ liệu nhạy cảm, bao gồm ID và bằng chứng Address. Dữ liệu này thường được thuê ngoài cho các nhà cung cấp bên thứ ba xử lý việc xác minh danh tính. Nếu một nền tảng bị tấn công hoặc gặp sự cố, thông tin này có thể bị lộ và sử dụng cho mục đích lừa đảo, tống tiền, hoặc thậm chí là trộm cắp vật lý. Không giống như các vụ vi phạm dữ liệu khác, rò rỉ liên quan đến Bitcoin đặc biệt nguy hiểm vì chúng có thể tiết lộ bạn sở hữu tài sản có giá trị.

Vì vậy, điều quan trọng là phải hiểu rằng việc thực hiện KYC không chỉ là một bước nhỏ; **nó ảnh hưởng đến an ninh cá nhân của bạn**. Việc chính phủ yêu cầu kiểm tra danh tính này không chỉ vi phạm các quyền tự nhiên của bạn mà còn khiến bạn gặp rủi ro. Mặc dù việc sử dụng các nền tảng này có thể giúp bạn bắt đầu với Bitcoin, bạn nên nhận thức được những rủi ro liên quan và cân nhắc tìm hiểu các phương pháp không cần KYC để có được Bitcoin sau này.

#### Mua lại mà không cần xác minh danh tính (No-KYC)

Việc mua Bitcoin mà không tiết lộ danh tính là một lựa chọn dành cho những ai coi trọng quyền riêng tư. Phương pháp này có thể bao gồm một số cách tiếp cận sau:

- **Giao dịch ngang hàng (P2P) giữa các cá nhân**: Mua trực tiếp bằng tiền mặt từ người bạn tin tưởng (bạn bè, gia đình, mạng lưới địa phương, các cuộc gặp mặt...) sẽ tránh được mọi hồ sơ giao dịch chính thức và giúp bạn không phải tiếp xúc với hệ thống ngân hàng. Mặc dù đây có lẽ là cách tốt nhất để mua hoặc bán Bitcoin, nhưng nó đi kèm với rủi ro bị tấn công vật lý vì đây là giao dịch trực tiếp.
- **Nền tảng P2P**: Các nền tảng như Bisq, RoboSats và Peach tạo điều kiện thuận lợi cho việc trao đổi giữa các cá nhân mà không cần xác minh danh tính, sử dụng hệ thống ký quỹ để bảo mật giao dịch. Các nền tảng này vẫn tiềm ẩn rủi ro bị đánh cắp nếu bên kia không hợp tác, nhưng rủi ro này được giảm thiểu nhờ dịch vụ ký quỹ.
- **Máy rút tiền tự động (ATM) Bitcoin**: Một số máy ATM (ki-ốt) cho phép bạn mua Bitcoin bằng tiền mặt, mặc dù phí thường cao và một số máy yêu cầu xác minh ID đối với các giao dịch lớn hơn.
- **Nền tảng giao dịch không cần KYC**: Một số nền tảng, đặc biệt là ở Thụy Sĩ, cho phép bạn mua số lượng nhỏ Bitcoin mà không cần xác minh ID.
- **Mining**: Cá nhân có thể đào Bitcoin. Các máy cũ hơn hoặc nhỏ hơn đều có thể tiếp cận, mang đến một cách thức phi truyền thống để sở hữu Bitcoin mà không cần KYC. Khi tham gia Mining pool, bạn có thể nhận được các khoản thanh toán thường xuyên, ngay cả khi chỉ có thiết bị cơ bản. Để biết thêm thông tin, [hãy xem hướng dẫn Mining của chúng tôi](https://planb.academy/tutorials/mining).
- **Thanh toán / Lương bằng Bitcoin**: Nếu bạn đang kinh doanh, bạn có thể chấp nhận Bitcoin làm phương thức thanh toán. Nếu bạn đang làm việc, tùy thuộc vào khu vực pháp lý, bạn có thể yêu cầu được trả lương bằng Bitcoin. Các phương thức này cho phép bạn nhận Bitcoin mà không cần KYC.

**Ưu điểm**:

- Tùy thuộc vào phương pháp, nó giúp bảo vệ quyền riêng tư của bạn bằng cách hạn chế các liên kết giữa danh tính của bạn và hoạt động Bitcoin của bạn;
- Giảm nguy cơ kiểm duyệt, đóng băng quỹ hoặc can thiệp của nhà nước.
- Cho phép truy cập Bitcoin ngay cả trong môi trường được quản lý chặt chẽ.

**Nhược điểm**:

- Thường phức tạp hơn các nền tảng KYC;
- Tính thanh khoản thấp hơn và ít giao dịch hơn;
- Việc mua P2P thường đi kèm với mức phí bảo hiểm;
- Rủi ro đối tác lớn hơn trong các giao dịch giữa các cá nhân (lừa đảo, gian lận, tấn công, v.v.).

### 4 loại người dùng Bitcoin

Dựa trên các tiêu chí đã thảo luận, chúng ta có thể xác định một số hồ sơ người dùng Bitcoin điển hình. Các hồ sơ này linh hoạt và có thể phát triển, nhưng chúng giúp làm rõ các chiến lược phù hợp với từng loại người dùng và hướng dẫn bạn lựa chọn công cụ phù hợp.

#### 1. Người nắm giữ

Người nắm giữ (Hodler) là nhà đầu tư mua Bitcoin với tầm nhìn dài hạn và không có kế hoạch nắm giữ nó trong nhiều năm. Không giống như người xếp chồng (Stacker), người mua thường xuyên theo thời gian, người nắm giữ thường mua số lượng lớn hơn, thỉnh thoảng mới mua và sau đó chỉ đơn giản là nắm giữ Bitcoin của mình.

Chiến lược ưu tiên:

- Mua hàng thông qua nền tảng KYC hoặc không phải KYC tùy thuộc vào nhu cầu riêng tư của họ;
- Lưu trữ trên Hardware Wallet hoặc sử dụng giải pháp đa chữ ký để tăng cường bảo mật;
- giao dịch hiếm, giảm thiểu tương tác với hệ sinh thái Bitcoin.

Người nắm giữ xem Bitcoin như một kho lưu trữ giá trị, tương tự như nhà đầu tư vàng vật chất. Thách thức chính của họ là lưu trữ Bitcoin một cách an toàn và đảm bảo họ có thể truyền lại cho người thừa kế.

#### 2. Máy xếp chồng

Stacker là người áp dụng phương pháp tiếp cận dần dần và nhất quán hơn. Họ mua Bitcoin thường xuyên, thường thông qua chiến lược Trung bình Chi phí Đô la (DCA). Mục tiêu của họ là phân bổ giá mua theo thời gian và giảm thiểu tác động của biến động giá Bitcoin.

Chiến lược ưu tiên:

- Sử dụng tính năng mua hàng tự động thông qua nền tảng KYC cung cấp DCA;
- Chuyển tiền thường xuyên vào Hardware Wallet để lưu trữ an toàn;
- Khá nhạy cảm với quyền riêng tư nhưng ưu tiên tính bảo mật và dễ tích lũy.

Stacker không tập trung vào đầu cơ ngắn hạn hay sử dụng Bitcoin thường xuyên. Họ xem Bitcoin như một kế hoạch tiết kiệm dài hạn, kéo dài nhiều năm, thậm chí nhiều thập kỷ. Điểm khác biệt giữa Stacker và Hodler nằm ở phương pháp mua lại và khối lượng giao dịch.

#### 3. Người dùng đang hoạt động

Người dùng chủ động xem Bitcoin chủ yếu là phương tiện trao đổi hơn là chỉ là một phương tiện lưu trữ giá trị. Họ sử dụng nó thường xuyên cho các giao dịch, thanh toán hoặc như một phần của nền kinh tế tuần hoàn.

Chiến lược ưu tiên:

- Sử dụng Hot Wallet trên thiết bị di động hoặc máy tính để bàn cho các giao dịch hàng ngày;
- Sử dụng Lightning Network để thanh toán nhanh chóng và rẻ;
- Duy trì phương pháp kết hợp, sử dụng Wallet di động để chi tiêu và Hardware Wallet để tiết kiệm;
- Mức độ nhạy cảm về quyền riêng tư thay đổi tùy theo nhu cầu cụ thể.

Người dùng chủ động có thể là một doanh nhân, người làm việc tự do, hoặc người sống trong môi trường sử dụng Bitcoin cho các khoản thanh toán hàng ngày. Ưu tiên hàng đầu của họ là sự tiện lợi, hướng đến sự cân bằng giữa bảo mật và khả năng truy cập.

#### 4. Người hoang tưởng

Người dùng hoang tưởng là cá nhân tiên tiến, ưu tiên quyền riêng tư và bảo mật. Họ coi Bitcoin là một công cụ cho tự do cá nhân và muốn giảm thiểu việc tiếp xúc với sự tin tưởng và quy định của bên thứ ba.

Chiến lược ưu tiên:

- Chỉ có thể lấy Bitcoin thông qua các kênh không phải KYC;
- Sử dụng ví được tăng cường bằng cụm mật khẩu BIP39 và thiết lập đa chữ ký;
- Lưu trữ Bitcoin trên nhiều thiết bị vật lý ở nhiều vị trí khác nhau;
- Tránh các dịch vụ tập trung có thể gây ảnh hưởng đến quyền riêng tư.

Hồ sơ này đặc biệt phù hợp với những người sống trong môi trường bất lợi đối với Bitcoin, nơi việc sở hữu hoặc sử dụng BTC có thể dẫn đến các lệnh trừng phạt. Đây là một cách tiếp cận phức tạp hơn, đòi hỏi trình độ chuyên môn kỹ thuật nhất định và kỷ luật nghiêm ngặt để tránh mất quyền truy cập vào tiền.

![image](assets/vi/069.webp)

### Hồ sơ nào phù hợp với bạn?

Hãy nhớ rằng các hồ sơ này không loại trừ lẫn nhau. Bạn có thể đồng nhất với nhiều hồ sơ, và cách tiếp cận của bạn có thể thay đổi theo thời gian. Ví dụ: bạn có thể bắt đầu với tư cách là một người xếp chồng (stacker) và dần dần chuyển sang người dùng tích cực hơn, hoặc thậm chí áp dụng tư duy bảo mật hoang tưởng. Bạn có thể là một người nắm giữ lâu dài trong khi vẫn sử dụng Bitcoin trong cuộc sống hàng ngày.

**Điều quan trọng nhất là phải điều chỉnh chiến lược của bạn cho phù hợp với tình hình hiện tại và tầm nhìn dài hạn.**

Trong các chương tiếp theo, chúng ta sẽ tìm hiểu chi tiết từng hồ sơ này để bạn có thể xác định hồ sơ nào phù hợp nhất với mình; và áp dụng phương pháp phù hợp nhất với mục tiêu của bạn. Chúng tôi cũng sẽ đề cập đến các công cụ phù hợp với từng hồ sơ, giúp bạn xây dựng nền tảng để xây dựng kế hoạch mua lại và bảo mật của riêng mình.

## Người nắm giữ: Đầu tư dài hạn

<chapterId>baf1adc2-3828-5265-8ee5-130be547585c</chapterId>

:::video id=3f4e5d6c-7b8a-9c0d-1e2f-3a4b5c6d7e8f:::

:::id video=3f4e5d6c-7b8a-9c0d-1e2f-3a4b5c6d7e8f:::

Trong cộng đồng Bitcoin, thuật ngữ "hodler" dùng để chỉ những người đầu tư dài hạn. Từ này bắt nguồn từ lỗi đánh máy nổi tiếng "hold" trên diễn đàn Bitcoin Talk nhiều năm trước và từ đó trở thành biểu tượng của Commitment: hãy nắm giữ Bitcoin của bạn bất kể điều gì xảy ra, mà không cần lo lắng về những biến động ngắn hạn của thị trường.

![image](assets/vi/071.webp)

Những người nắm giữ tin tưởng vào giá trị của Bitcoin. Với số lượng Supply cố định là 21 triệu coin, họ coi Bitcoin là một kho lưu trữ giá trị dài hạn (vàng kỹ thuật số).

### Tại sao nên chọn chiến lược nắm giữ?

Theo truyền thống, nắm giữ (holding) là phương pháp sinh lời nhất đối với hầu hết các nhà đầu tư không chuyên nghiệp. Nó loại bỏ nhu cầu căn thời điểm thị trường, vốn nổi tiếng là khó khăn và gây hao tổn cảm xúc. Bằng cách đơn giản là mua và nắm giữ, người nắm giữ tránh được căng thẳng khi giao dịch và những quyết định sai lầm thường đi kèm với phản ứng cảm xúc.

Bitcoin có xu hướng di chuyển theo chu kỳ; các đợt tăng giá mạnh tiếp theo là những đợt điều chỉnh mạnh, rồi đến những giai đoạn im ắng kéo dài. Từ năm 2013 đến năm 2025, Bitcoin đã trải qua nhiều chu kỳ như vậy, đôi khi mất tới 80% giá trị trong thị trường giá xuống. Những giai đoạn này có thể thử thách sự kiên nhẫn của người nắm giữ, nhưng những người kiên trì theo đuổi chiến lược này thường được đền đáp theo thời gian.

Hiểu được những chu kỳ này giúp kiểm soát cảm xúc. Những người nắm giữ cổ phiếu dày dạn kinh nghiệm biết rằng việc cố gắng bán ở đỉnh hoặc mua ở đáy đều là một canh bạc. Thay vào đó, việc nắm giữ dài hạn một cách có kỷ luật đã được chứng minh là một chiến lược chiến thắng.

### Kế hoạch đầu tư cho người nắm giữ

Những người nắm giữ áp dụng một phương pháp đơn giản và dài hạn: mua Bitcoin, bảo mật đúng cách và để nguyên. Họ không giao dịch hay tiêu xài coin của mình. Mục tiêu của họ là tích lũy và bảo vệ số coin nắm giữ với mức quản lý tối thiểu.

Sau đây là một kế hoạch chắc chắn dành cho người nắm giữ:

#### 1. Chọn và thiết lập Hardware Wallet

Trước hết, hãy chú ý đến vấn đề bảo mật. Hardware Wallet là điều bắt buộc. Nó lưu trữ khóa riêng tư của bạn ngoại tuyến, bảo vệ tiền của bạn khỏi các cuộc tấn công trực tuyến.

Sau đây là một số hướng dẫn về ví phù hợp với người nắm giữ:

https://planb.academy/tutorials/wallet/hardware/coldcard-q-73e86d1a-6fe6-4d8b-bb15-8690298020e3

https://planb.academy/tutorials/wallet/hardware/jade-plus-green-873099a4-35ec-4be8-b31a-6e7cd6a41ec0

https://planb.academy/tutorials/wallet/hardware/ledger-nano-s-plus-75043cb3-2e8e-43e8-862d-ca243b8215a4

https://planb.academy/tutorials/wallet/hardware/ledger-flex-3728773e-74d4-4177-b39f-bd923700c76a

https://planb.academy/tutorials/wallet/hardware/passport-74e53858-3fa2-43f9-b866-573297546236

https://planb.academy/tutorials/wallet/hardware/bitbox02-6af8940f-e19b-4008-8c83-81017032608c

Sau khi thiết lập Wallet, hãy đảm bảo sao lưu cụm từ khôi phục 12 hoặc 24 từ của bạn vào một phương tiện vật lý bền chắc. Để tìm hiểu thêm về phương tiện lưu trữ cụm từ của bạn, hãy làm theo hướng dẫn này:

https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

Để an tâm hơn, bạn có thể thêm các lớp bảo vệ như:

- Wallet đa chữ ký có khóa thời gian:

https://planb.academy/tutorials/wallet/desktop/liana-306ef457-700c-4fdd-b07a-8fb7a8a29f04

- Một BIP39 passphrase:

https://planb.academy/tutorials/wallet/backup/passphrase-a26a0220-806c-44b4-af14-bafdeb1adce7

![image](assets/vi/073.webp)

#### 2. Mua và rút Bitcoin

Người nắm giữ không bao giờ để tiền của họ trên các sàn giao dịch. Nền tảng có thể bị hack, phá sản hoặc bị tịch thu. Sau khi mua, hãy chuyển Bitcoin của bạn thẳng đến Hardware Wallet.

Vì người nắm giữ có xu hướng đầu tư số tiền lớn hơn, họ cần những sàn giao dịch có tính thanh khoản cao. Dưới đây là một vài sàn giao dịch đáp ứng được yêu cầu này:

https://planb.academy/tutorials/exchange/centralized/bitfinex-dc306d39-bd96-4ab9-a278-a322316716db

https://planb.academy/tutorials/exchange/centralized/kraken-1ef03e25-9b42-49bd-a47d-249e1a13cfc6

https://planb.academy/tutorials/exchange/centralized/bitstamp-5a36c896-bff5-46d7-b505-ff069c3ac47c

https://planb.academy/tutorials/exchange/centralized/bull-bitcoin-europe-0ccf713e-efcd-44ec-8205-211f49ac7d53

![image](assets/vi/074.webp)

Một số người nắm giữ cũng áp dụng phương pháp kết hợp bằng cách sử dụng DCA (Trung bình chi phí đô la) - mua số lượng nhỏ thường xuyên để làm phẳng sự biến động của thị trường.

#### 3. Thiết lập kế hoạch kế nhiệm

Điều này thường bị bỏ qua. Nhưng Bitcoin rất nghiêm ngặt; nếu khóa riêng của bạn bị mất, tiền của bạn sẽ mất mãi mãi. Việc lập kế hoạch phòng ngừa trường hợp mất khả năng lao động hoặc tử vong là rất quan trọng.

Các bước chính:

- Viết hướng dẫn chi tiết giải thích cách truy cập Bitcoin của bạn nếu bạn không thể;
- Sử dụng thiết lập đa chữ ký để cung cấp quyền truy cập đồng thời giảm thiểu rủi ro mất mát;
- Chia sẻ thông tin quan trọng với người mà bạn tin tưởng.

Chúng ta sẽ đi sâu hơn vào kế hoạch thừa kế ở phần cuối của khóa học này.

#### 4. Hãy kiên nhẫn trong suốt chu kỳ

Nắm giữ đồng nghĩa với việc chơi dài hạn. Bạn cần giữ bình tĩnh trong cả những đợt giá giảm mạnh lẫn những thị trường tăng giá hưng phấn.

Một số cách thực hành tốt nhất:

- Kiểm tra Wallet định kỳ; nhưng không quá thường xuyên. Việc theo dõi liên tục sẽ dẫn đến những quyết định mang tính cảm tính;
- Đừng hoảng loạn bán tháo khi thị trường giảm. Điều chỉnh là một phần của trò chơi;
- Hãy cập nhật thông tin về diễn biến của Bitcoin, nhưng tránh xa sự cường điệu của giới truyền thông, đặc biệt là trong thời kỳ suy thoái;
- Định kỳ kiểm tra tính toàn vẹn của cụm từ khôi phục. Kiểm tra xem phong bì có bị giả mạo không và phương tiện vẫn còn đọc được. Nếu có bất kỳ dấu hiệu bất thường nào, hãy sao lưu lại từ Wallet của bạn;

Để theo dõi tài sản của bạn mà không làm lộ khóa riêng, hãy cân nhắc sử dụng Watch-only wallet. Thiết bị này cho phép bạn theo dõi số dư và giao dịch chỉ bằng [khóa công khai](https://planb.academy/resources/glossary/public-key).

https://planb.academy/tutorials/wallet/mobile/blockstream-app-watch-only-66c3bc5a-5fa1-40ef-9998-6d6f7f2810fb

Chiến lược của người nắm giữ cổ phiếu (hodler) rất đơn giản nhưng hiệu quả: mua, bảo toàn và quên đi. Thách thức thực sự không nằm ở kỹ thuật; mà là tâm lý. Phần khó nhất là kiềm chế ham muốn bán quá sớm hoặc hoảng loạn khi giá giảm. Nhưng nếu bạn giữ kỷ luật và suy nghĩ dài hạn, nắm giữ cổ phiếu đã được chứng minh là một cách đáng tin cậy để bảo toàn giá trị và tích lũy tài sản theo thời gian.

![image](assets/vi/075.webp)

### Đánh giá hồ sơ Hodler của bạn

Nếu bạn nhận thấy mình có những đặc điểm tính cách sau, có lẽ bạn là người thích nắm giữ cổ phiếu:

- **Kiên nhẫn**: Người nắm giữ thể hiện sự kiên nhẫn phi thường. Họ sẵn sàng nắm giữ Bitcoin trong nhiều năm (đôi khi là hàng thập kỷ) mà không bị ảnh hưởng bởi biến động giá hàng ngày. Họ hiểu rằng Bitcoin biến động theo chu kỳ và không hoảng sợ khi giá giảm mạnh. Điều chỉnh chỉ là một phần của trò chơi. Đối với người nắm giữ, giá trị không được đo lường bằng tháng, mà là trong dài hạn.
- **Tầm nhìn dài hạn**: Những người nắm giữ cổ phiếu này nghĩ xa hơn lợi nhuận ngắn hạn. Họ tin tưởng vào sự tăng trưởng mang tính cấu trúc và khả năng áp dụng dần dần trên toàn cầu của Bitcoin. Bất chấp sự biến động, họ nhìn thấy một quỹ đạo đi lên; một quỹ đạo mở rộng tiếp theo là củng cố. Đối với họ, Bitcoin là một tài sản lâu dài, một công cụ phòng ngừa lạm phát và là một phần quan trọng của một cuộc cách mạng tiền tệ lớn hơn;
- **Kỷ luật bản thân**: Kỷ luật tinh thần là cốt lõi trong tư duy của những người nắm giữ cổ phiếu. Nghịch lý thay, thách thức lớn nhất của họ lại là không hành động. Họ kiềm chế cơn bốc đồng bán tháo khi giá tăng vọt và không đầu hàng nỗi sợ hãi khi giá sụp đổ. Họ hiểu rằng Bitcoin đã vượt qua nhiều thị trường giá xuống khắc nghiệt; và những người kiên trì vượt qua nỗi đau thường được hưởng lợi nhiều nhất;
- **Khả năng phục hồi cảm xúc**: Giá Bitcoin có thể giảm 50% hoặc hơn chỉ trong vài tuần; vậy mà người nắm giữ vẫn giữ được bình tĩnh. Họ đã thấm nhuần quan điểm rằng biến động không làm suy yếu các yếu tố cơ bản của Bitcoin. Sự bền bỉ về mặt cảm xúc này cho phép họ giữ được bình tĩnh trong bối cảnh bất ổn và duy trì cái nhìn lý trí khi những người khác hoảng loạn;
- **Niềm tin vào công nghệ**: Những người nắm giữ Bitcoin không chỉ xem Bitcoin là một tài sản đầu cơ. Họ tin vào công nghệ và tiềm năng đột phá của nó. Đối với nhiều người, nó không chỉ là tiền bạc; mà còn là tự do. Bitcoin được xem là một loại tiền tệ bền vững, phi chính trị và là một giải pháp thay thế thực sự cho ngân hàng truyền thống, trao quyền cho cá nhân với chủ quyền tài chính lớn hơn.

![image](assets/vi/072.webp)

Việc nắm giữ rất đơn giản, hiệu quả và dễ tiếp cận; ngay cả với người mới bắt đầu.

Trong chương tiếp theo, chúng ta sẽ khám phá một cấu hình tương tự nhưng năng động hơn: stacker.

## Stacker: Tích lũy Satoshi

<chapterId>0daf450d-1b91-5d99-9c31-b52ab52a5e21</chapterId>

:::video id=4e5f6a7b-8c9d-0e1f-2a3b-4c5d6e7f8a9b:::

:::id video=4e5f6a7b-8c9d-0e1f-2a3b-4c5d6e7f8a9b:::

Trong hệ sinh thái Bitcoin, người tích lũy (stacker) là người áp dụng phương pháp nhất quán và lâu dài để tích lũy satoshi (hay "Sats"), đơn vị nhỏ nhất của Bitcoin. Thay vì đầu tư một khoản tiền lớn cùng một lúc, người tích lũy thường xuyên mua Bitcoin với số lượng nhỏ, bất kể điều kiện thị trường.

Ý tưởng rất đơn giản: sẽ chỉ có khoảng 21 triệu bitcoin, và mỗi phần nhỏ đều có giá trị. Nhiệm vụ của người xếp chồng là tích lũy càng nhiều Sats càng tốt.

![image](assets/vi/076.webp)

### Tại sao nên chọn chiến lược xếp chồng?

Chiến lược đầu tư chồng cổ phiếu (stacking) là một chiến lược đầu tư cực kỳ hiệu quả dành cho những ai muốn giảm thiểu rủi ro biến động thị trường, đồng thời dần dần xây dựng vị thế Bitcoin. Không giống như đầu tư một lần hoặc đầu cơ thời điểm thị trường, chiến lược chồng cổ phiếu thường sử dụng một kỹ thuật gọi là Trung bình Chi phí Đô la (DCA); đầu tư một khoản tiền cố định theo định kỳ (hàng ngày, hàng tuần, hàng tháng...), bất kể giá hiện tại của Bitcoin.

Ưu điểm của việc xếp chồng:

- **Giảm biến động**: Mua thường xuyên giúp cân bằng biến động giá, vì bạn mua nhiều Sats hơn khi giá thấp và ít hơn khi giá cao; giúp trung bình hóa cơ sở chi phí của bạn và giảm căng thẳng về mặt cảm xúc.
- **Khả năng tiếp cận**: Bạn không cần vốn ban đầu lớn. Bạn có thể bắt đầu chỉ với 10€ mỗi tháng và tăng dần số tiền DCA theo thời gian.
- **Kỷ luật và tự động hóa**: Người xếp chồng tránh đưa ra quyết định dựa trên cảm xúc. Việc mua hàng định kỳ tự động đảm bảo tính nhất quán và giảm thiểu rủi ro hành động bốc đồng.
- **Chiến lược dài hạn**: Đầu tư dần dần làm tăng cơ hội hưởng lợi từ thị trường tăng giá đồng thời giảm thiểu rủi ro trong thời kỳ suy thoái.

Phương pháp này đặc biệt **phù hợp với người mới bắt đầu**; nó loại bỏ áp lực phải căn thời điểm thị trường và cho phép tích lũy ổn định, không cần động tay vào.

### Kế hoạch đầu tư của Stacker

Mục tiêu là xây dựng một chiến lược bền vững, tự động và an toàn để tích lũy Bitcoin trong thời gian dài. Dưới đây là các bước chính để triển khai kế hoạch đầu tư máy xếp chồng của bạn:

#### 1. Lựa chọn loại Wallet phù hợp

Máy xếp chồng cần có thiết lập Wallet cân bằng giữa tính dễ sử dụng và tính bảo mật lâu dài.

- **Hot Wallet**: Lý tưởng cho người mới bắt đầu, ví di động hoặc máy tính để bàn cho phép bạn dễ dàng nhận Sats và thực hiện thanh toán. Tuy nhiên, chúng phù hợp nhất với số tiền nhỏ. Ví On-Chain được ưa chuộng hơn để tích trữ so với ví Lightning do khả năng quản lý UTXO tốt hơn. Ví phần mềm được đề xuất: Green Wallet, Blue Wallet, Sparrow wallet, Bitkit, Nunchuk, Aqua, Proton

https://planb.academy/tutorials/wallet/mobile/aqua-8e6d7dd3-8c03-45cc-90dd-fe3899a7d125

https://planb.academy/tutorials/wallet/mobile/bitkit-a7224674-85c4-4045-9baf-37018d89550c

https://planb.academy/tutorials/wallet/mobile/blockstream-app-onchain-e84edaa9-fb65-48c1-a357-8a5f27996143

https://planb.academy/tutorials/wallet/desktop/blockstream-app-desktop-c1503adf-1404-4328-b814-aa97fcf0d5da

https://planb.academy/tutorials/wallet/mobile/blue-wallet-2f4093da-6d03-4f26-8378-b9351d0dbc90

https://planb.academy/tutorials/wallet/mobile/nunchuk-6cbcb406-ec84-478f-afac-bb4da366a6fa

https://planb.academy/tutorials/wallet/desktop/proton-wallet-0bee0bba-ab62-4db0-8c63-b2ad698ca178

https://planb.academy/tutorials/wallet/desktop/sparrow-c674e2ac-d46f-4c82-92a7-7d1b0e262f5d

- **Hardware Wallet**: Khi số lượng ví Bitcoin của bạn tăng lên, việc bảo mật tiền của bạn bằng ví Cold Wallet là điều cần thiết để giảm thiểu rủi ro bị đánh cắp. Một người dùng ví chuyên nghiệp thường xuyên chuyển tiền từ ví Hot Wallet sang ví Hardware Wallet. Ví dụ: Bitbox02, Trezor, Ledger, Coldcard, Jade...

https://planb.academy/tutorials/wallet/hardware/coldcard-q-73e86d1a-6fe6-4d8b-bb15-8690298020e3

https://planb.academy/tutorials/wallet/hardware/jade-plus-green-873099a4-35ec-4be8-b31a-6e7cd6a41ec0

https://planb.academy/tutorials/wallet/hardware/ledger-nano-s-plus-75043cb3-2e8e-43e8-862d-ca243b8215a4

https://planb.academy/tutorials/wallet/hardware/ledger-flex-3728773e-74d4-4177-b39f-bd923700c76a

https://planb.academy/tutorials/wallet/hardware/passport-74e53858-3fa2-43f9-b866-573297546236

https://planb.academy/tutorials/wallet/hardware/bitbox02-6af8940f-e19b-4008-8c83-81017032608c

Cách tiếp cận hiệu quả nhất là sử dụng cả hai: Hot Wallet để xếp chồng hàng ngày và Hardware Wallet để lưu trữ Cold dài hạn. Hot Wallet dễ sử dụng và tiện lợi. Nó cho phép bạn nhanh chóng sở hữu bitcoin của mình, thay vì để chúng trên nền tảng nơi chúng có thể gặp rủi ro. Tuy nhiên, so với Hardware Wallet, Hot Wallet cung cấp ít bảo mật hơn nhiều. Do đó, người xếp chồng phải thường xuyên chuyển tiền của mình sang Cold Wallet để đảm bảo khoản tiết kiệm dài hạn của mình và tránh giữ quá nhiều tiền trong Hot Wallet. Định kỳ, người xếp chồng sẽ hợp nhất các UTXO khi chuyển sang lưu trữ Cold; hợp nhất các đầu vào nhỏ để tối ưu hóa phí giao dịch trong tương lai.

![image](assets/vi/077.webp)

#### 2. Thiết lập kế hoạch mua hàng DCA

Chiến lược Trung bình Chi phí Đô la (DCA) là một chiến lược đầu tư mà bạn mua một lượng nhỏ Bitcoin theo định kỳ, bất kể giá của nó. Để thiết lập một kế hoạch DCA hiệu quả, bạn cần xác định hai yếu tố chính của Elements:

- **Tần suất mua**: Hàng tuần, hai tuần một lần hoặc hàng tháng; lựa chọn dựa trên thu nhập và mục tiêu của bạn. Tránh mua quá thường xuyên nếu điều đó dẫn đến nhiều khoản đầu tư nhỏ lẻ (UTXO) tốn kém sau này. Hãy dựa trên ngân sách và khả năng chịu rủi ro của bạn. Tính nhất quán quan trọng hơn quy mô; tốt hơn là nên tích lũy một khoản đầu tư vừa phải trong nhiều năm thay vì đầu tư quá mức và bỏ cuộc. Tất nhiên, số tiền DCA của bạn có thể được điều chỉnh khi tình hình tài chính thay đổi.

Tiếp theo, bạn có hai lựa chọn để thiết lập DCA: tự động hóa hoặc quản lý thủ công. Hầu hết các nền tảng giao dịch được quản lý hiện nay đều cung cấp tùy chọn tự động hóa DCA. Với đủ tiền pháp định trong tài khoản, nền tảng sẽ tự động thực hiện lệnh mua dựa trên các thông số bạn đã thiết lập. Ngoài ra, ngày càng có nhiều nền tảng chuyên dụng cho việc tự động hóa DCA, cung cấp các dịch vụ bổ sung như rút tiền tự động vào tài khoản tự quản lý Wallet của bạn hoặc làm tròn chi phí hàng ngày của bạn. Nếu bạn đang sử dụng một nền tảng được quản lý, tôi thực sự khuyên bạn nên tự động hóa DCA để yên tâm hơn trong việc quản lý hàng ngày.

![image](assets/vi/078.webp)

Ví dụ về nền tảng DCA:

https://planb.academy/tutorials/exchange/centralized/bitstack-29fd71be-9570-42c6-8f6f-cd355d62e746

https://planb.academy/tutorials/exchange/centralized/relai-v2-30a9671d-e407-459d-9203-4c3eae15b30e

https://planb.academy/tutorials/exchange/centralized/stackinsat-5af6a380-f3c6-4246-9f81-9957a16ab066

Các sàn giao dịch chính thống hỗ trợ DCA:

https://planb.academy/tutorials/exchange/centralized/bitfinex-dc306d39-bd96-4ab9-a278-a322316716db

https://planb.academy/tutorials/exchange/centralized/bitstamp-5a36c896-bff5-46d7-b505-ff069c3ac47c

https://planb.academy/tutorials/exchange/centralized/bull-bitcoin-europe-0ccf713e-efcd-44ec-8205-211f49ac7d53

https://planb.academy/tutorials/exchange/centralized/kraken-1ef03e25-9b42-49bd-a47d-249e1a13cfc6

https://planb.academy/tutorials/exchange/centralized/paymium-92603f76-b985-49ce-81e5-f4fa0df776e5

Người xếp chồng cũng có thể muốn tăng cường quyền riêng tư, ngay cả khi đó không phải là mục tiêu chính của họ. Trong trường hợp này, họ nên cân nhắc sử dụng phương thức mua hàng P2P, bên ngoài các nền tảng được quản lý. Không giống như giao dịch DCA tự động, các phương thức này yêu cầu bạn phải quản lý chiến lược mua hàng của mình theo cách thủ công, vì vậy tính kỷ luật là chìa khóa để duy trì đúng hướng.

https://planb.academy/tutorials/exchange/peer-to-peer/bisq-v2-c1c6a702-6c16-4101-8b90-62c424017b80

https://planb.academy/tutorials/exchange/peer-to-peer/hodlhodl-d7344cd5-6b18-40f5-8e78-2574a93a3879

https://planb.academy/tutorials/exchange/peer-to-peer/lnp2pbot-v2-e6bcb210-610b-487d-970c-7cce85273e3c

https://planb.academy/tutorials/exchange/peer-to-peer/peach-c6143241-d900-4047-9b73-1caba5e1f874

https://planb.academy/tutorials/exchange/peer-to-peer/robosats-b60e4f7c-533a-4295-9f6d-5368152e8c06

#### 3. Rút và bảo mật bitcoin

Khi chiến lược mua bán của bạn đã được thiết lập, chỉ cần rút bitcoin từ sàn giao dịch và chuyển vào ví phần cứng của bạn thường xuyên. Điều này đảm bảo quyền tự quản lý hoàn toàn và bảo vệ lâu dài.

#### 4. Chờ đợi một vài chu kỳ

Người đầu cơ tích trữ nghĩ theo năm, chứ không phải theo tháng. Giống như người nắm giữ, họ phải kiềm chế sự hoảng loạn khi thị trường sụp đổ hoặc hưng phấn khi thị trường tăng giá.

Thói quen chính:

- Đừng kiểm tra ví quá thường xuyên để tránh căng thẳng;
- Không bao giờ bán khi thị trường đang suy thoái; việc điều chỉnh là bình thường;
- Luôn cập nhật thông tin về những phát triển công nghệ của Bitcoin;
- Kiểm tra định kỳ bản sao lưu seed và trạng thái Hardware Wallet của bạn: Sau khi ví đã được thiết lập, việc thiết lập chế độ giám sát thường xuyên là rất quan trọng. Định kỳ, hãy kiểm tra xem cụm từ Mnemonic của bạn có còn ở vị trí lưu trữ không và lớp vỏ mờ đục của nó chưa bị mở. Trong quá trình kiểm tra này, bạn cũng có thể mở phong bì để kiểm tra tình trạng của thiết bị. Đảm bảo rằng nó không bị hư hỏng và câu vẫn hoàn toàn rõ ràng. Nếu có bất kỳ dấu hiệu hư hỏng nào, tốt nhất bạn nên tạo một bản sao mới từ Hardware Wallet. Bạn cũng có thể thường xuyên kiểm tra ví phần cứng của mình.

![image](assets/vi/080.webp)

Để dễ dàng theo dõi mà không ảnh hưởng đến bảo mật, hãy thiết lập Watch-only wallet để theo dõi số dư bằng khóa công khai.

https://planb.academy/tutorials/wallet/mobile/blockstream-app-watch-only-66c3bc5a-5fa1-40ef-9998-6d6f7f2810fb

### Đánh giá hồ sơ xếp chồng của bạn

Những người xếp chồng thành công thể hiện những đặc điểm sau:

- **Tổ chức**: Giữ ngân sách và bám sát kế hoạch. Người tích lũy phải đảm bảo kế hoạch tích lũy của mình khả thi trong nhiều năm.
- **Kỷ luật**: Tuân thủ lịch trình mà không phản ứng với biến động thị trường. Tính đều đặn được ưu tiên hơn những điều chỉnh bốc đồng. Tuy nhiên, vẫn có thể thực hiện những điều chỉnh hợp lý.
- **Kiên nhẫn**: Phần thưởng của việc tích lũy không đến ngay lập tức. Chiến lược này đòi hỏi thời gian dài, đôi khi mất nhiều năm, trước khi mang lại kết quả rõ rệt. Những người thành công là những người có thể kiên nhẫn, không bị ảnh hưởng bởi biến động ngắn hạn.
- **Kiến thức tài chính**: Hiểu biết vững chắc về DCA, quản lý rủi ro và các nguyên tắc cơ bản của thị trường Bitcoin giúp bạn đầu tư tự tin. Một nhà đầu tư hiểu biết sẽ được trang bị tốt hơn để chống lại những nghi ngờ và tránh đưa ra quyết định bốc đồng trong thời kỳ biến động cao.
- **Tầm nhìn dài hạn**: Một nhà đầu tư tin tưởng mạnh mẽ vào tiềm năng công nghệ của Bitcoin. Trọng tâm của họ không phải là biến động giá ngắn hạn, mà là tăng trưởng đều đặn lượng nắm giữ qua từng năm.

![image](assets/vi/079.webp)

Tóm lại, chiến lược xếp chồng này lý tưởng cho những ai muốn xây dựng vị thế Bitcoin theo thời gian, giảm rủi ro biến động và duy trì quyền kiểm soát hoàn toàn mà không cần phải dán mắt vào biểu đồ.

Tiếp theo: chúng ta sẽ khám phá một hồ sơ người dùng bitcoin hoàn toàn khác; người dùng tích cực.

## Người dùng Bitcoin năng động: Tích hợp Bitcoin vào cuộc sống hàng ngày

<chapterId>e0a022ab-207c-571f-b4ad-c432214a756c</chapterId>

:::video id=5f6a7b8c-9d0e-1f2a-3b4c-5d6e7f8a9b0c:::

Trở thành người dùng Bitcoin tích cực có nghĩa là vượt ra ngoài các chiến lược đầu tư dài hạn. Không giống như người nắm giữ (hodler) hay người tích trữ (stacker) chủ yếu tiết kiệm mà không chi tiêu, người dùng tích cực coi Bitcoin như một **phương tiện trao đổi** thực sự. Họ sử dụng nó để thanh toán hàng hóa và dịch vụ và tích cực tham gia vào nền kinh tế đối ứng.

![image](assets/vi/083.webp)

### Tại sao nên áp dụng chiến lược người dùng tích cực?

Sử dụng Bitcoin hàng ngày mang lại lợi ích cho cả cá nhân và tập thể:

- **Hỗ trợ hệ sinh thái Bitcoin thông qua việc áp dụng**:

Mỗi giao dịch đều củng cố tính hợp pháp của Bitcoin như một loại tiền tệ và khuyến khích nhiều thương nhân và dịch vụ chấp nhận nó hơn. Sử dụng Bitcoin là bỏ phiếu cho tính hữu ích của nó. Tiền có giá trị từ khả năng tạo điều kiện thuận lợi cho thương mại; không phải trao đổi hàng hóa. Nếu không có thương nhân nào chấp nhận một loại tiền tệ, nó sẽ không có giá trị khách quan. Bạn càng có thể mua nhiều hàng hóa và dịch vụ bằng nó, nó càng trở nên hữu ích. Bằng cách chọn thanh toán bằng Bitcoin, bạn đang hướng giá trị đến những thương nhân chấp nhận nó. Điều này làm tăng lợi nhuận của họ và theo thời gian, có thể làm giảm lợi nhuận của các đối thủ cạnh tranh từ chối nó. Trong toàn cộng đồng, động lực này có thể thúc đẩy sự chấp nhận rộng rãi hơn đối với Bitcoin và góp phần vào một vòng tuần hoàn áp dụng tích cực.

- **Tận dụng các tính năng độc đáo của Bitcoin**:

Bitcoin cung cấp nhiều tính năng mạnh mẽ, chẳng hạn như giao dịch nhanh chóng thông qua Lightning Network, và không bị kiểm duyệt, giám sát tài chính và thuế phát hành tiền tệ. Giá trị cốt lõi của Supply nằm ở việc loại bỏ sự kiểm soát của chính phủ đối với tiền tệ và cho phép các giao dịch không bị kiểm duyệt. Nếu những lợi thế này phù hợp với nhu cầu và giá trị cá nhân của bạn, bạn có thể thấy hồ sơ Người dùng Hoạt động rất phù hợp với mình.

- **Học bằng cách làm**:

Việc chủ động sử dụng Bitcoin giúp bạn phát triển kinh nghiệm thực tế. Bạn sẽ làm quen với các loại Wallet, quyền riêng tư thanh toán và quản lý UTXO. Việc sử dụng hàng ngày sẽ trở thành một môi trường học tập thực tế.

Những thách thức bao gồm sự phức tạp về mặt kỹ thuật, biến động giá và nhu cầu bảo mật tốt để ngăn ngừa mất tiền.

![image](assets/vi/086.webp)

### Kế hoạch đầu tư cho người dùng tích cực

Việc áp dụng Bitcoin cho các khoản thanh toán hàng ngày đòi hỏi một cách tiếp cận chu đáo và có cấu trúc. Không giống như người nắm giữ (Hodler) hay người nắm giữ (Stacker), Người dùng chủ động phải quản lý cả tính bảo mật lâu dài của tiền và sự tiện lợi của các giao dịch hàng ngày. Việc đạt được sự cân bằng phù hợp giữa việc lưu trữ Cold an toàn và khả năng truy cập dễ dàng vào tiền chi tiêu là điều cần thiết để chiến lược này hoạt động hiệu quả.

![image](assets/vi/084.webp)

#### 1. Phân tách ví rõ ràng

Người dùng tích cực nên sử dụng hai loại ví khác nhau để cân bằng giữa tính bảo mật và sự tiện lợi:

- Cold Wallet (Hardware Wallet) để đảm bảo tiết kiệm dài hạn.

https://planb.academy/tutorials/wallet/hardware/coldcard-q-73e86d1a-6fe6-4d8b-bb15-8690298020e3

https://planb.academy/tutorials/wallet/hardware/jade-plus-green-873099a4-35ec-4be8-b31a-6e7cd6a41ec0

https://planb.academy/tutorials/wallet/hardware/ledger-nano-s-plus-75043cb3-2e8e-43e8-862d-ca243b8215a4

https://planb.academy/tutorials/wallet/hardware/ledger-flex-3728773e-74d4-4177-b39f-bd923700c76a

https://planb.academy/tutorials/wallet/hardware/passport-74e53858-3fa2-43f9-b866-573297546236

https://planb.academy/tutorials/wallet/hardware/bitbox02-6af8940f-e19b-4008-8c83-81017032608c

https://planb.academy/tutorials/wallet/hardware/portal-bd6e83f0-0100-4689-aeb3-513cca944ace

https://planb.academy/tutorials/wallet/hardware/satochip-e9bc81d9-d59b-420d-9672-3360212237ba

https://planb.academy/tutorials/wallet/hardware/seedsigner-2b274bff-6fc8-407a-92d7-f6ec4d1fadfb

https://planb.academy/tutorials/wallet/hardware/tapsigner-ab2bcdf9-9509-4908-9a4a-2f2be1e7d5d2

- Một hoặc nhiều ví di động Hot cho các giao dịch hàng ngày. Thông thường, một người dùng đang hoạt động sẽ có cả ví On-Chain, Hot, Wallet và ví Lightning Wallet.

https://planb.academy/tutorials/wallet/mobile/aqua-8e6d7dd3-8c03-45cc-90dd-fe3899a7d125

https://planb.academy/tutorials/wallet/mobile/bitkit-a7224674-85c4-4045-9baf-37018d89550c

https://planb.academy/tutorials/wallet/mobile/blink-7ea5f5a4-e728-4ff9-b3f9-cf20aa6fc2bd

https://planb.academy/tutorials/wallet/mobile/blixt-04b319cf-8cbe-4027-b26f-840571f2244f

https://planb.academy/tutorials/wallet/mobile/blockstream-app-onchain-e84edaa9-fb65-48c1-a357-8a5f27996143

https://planb.academy/tutorials/wallet/mobile/blue-wallet-2f4093da-6d03-4f26-8378-b9351d0dbc90

https://planb.academy/tutorials/wallet/mobile/breez-46a6867b-c74b-45e7-869c-10a4e0263c06

https://planb.academy/tutorials/wallet/mobile/lipa-61019fbd-deb7-4b3c-8cda-0402a4b2e780

https://planb.academy/tutorials/wallet/mobile/nunchuk-6cbcb406-ec84-478f-afac-bb4da366a6fa

https://planb.academy/tutorials/wallet/mobile/phoenix-0f681345-abff-4bdc-819c-4ae800129cdf

https://planb.academy/tutorials/wallet/mobile/wallet-of-satoshi-39149d86-e42b-4e8f-ae9f-7e061e7784f7

https://planb.academy/tutorials/wallet/mobile/zeus-embedded-advanced-3e89603c-501d-439c-8691-d4a0d0de459b

Việc tách tiền theo cách này giúp giảm thiểu rủi ro trong trường hợp Wallet được sử dụng để thanh toán hàng ngày bị xâm phạm.

#### 2. Thiết lập nút của riêng bạn

Người dùng chủ động thường muốn giảm sự phụ thuộc vào các bên thứ ba đáng tin cậy, tùy thuộc vào hoàn cảnh cá nhân. Một trong những cách hiệu quả nhất để đạt được điều này là vận hành nút Bitcoin của riêng họ. Điều này cho phép họ sử dụng mạng một cách độc lập, đồng thời tăng cường tính riêng tư cho các giao dịch.

https://planb.academy/tutorials/node/bitcoin/bitcoin-core-linux-568c13a6-8746-4d63-8e95-f4a61c5ae0ed

https://planb.academy/tutorials/node/bitcoin/bitcoin-core-mac-windows-9684ab02-e0af-41c9-8102-86ac7c7727f3

https://planb.academy/tutorials/node/bitcoin/bitcoin-knots-e04b2196-4df2-4246-86ef-c02269c29098

https://planb.academy/tutorials/node/bitcoin/raspiblitz-d8cdba2e-a682-46cf-9fdc-d8602fbeac02

https://planb.academy/tutorials/node/bitcoin/mynode-a481fef3-2fd3-4df3-91c0-112cffa094eb

https://planb.academy/tutorials/node/bitcoin/start9-8c8b6827-8423-4929-bcba-89057670ed6a

Đối với những người thường xuyên sử dụng Lightning Network, việc tự quản lý node LND, LDK hoặc Core-Lightning của riêng họ có thể là một bước đi thông minh. Điều này cho phép họ kiểm soát hoàn toàn việc đóng mở kênh, tăng cường tính tự chủ và thậm chí có thể giúp giảm phí giao dịch; đặc biệt là khi so sánh với các giải pháp tự động như Phoenix, hoặc tệ hơn là ví Lightning lưu ký.

https://planb.academy/tutorials/node/lightning-network/alby-hub-62e6356c-6a6d-4134-8f22-c3b6afb9882a

#### 3. Thu thập Bitcoin thông thường

Người dùng tích cực cần cân bằng giữa khoản tiết kiệm dài hạn và số lượng Bitcoin dự trữ để sử dụng hàng ngày.

![image](assets/vi/085.webp)

Có một số cách để có được Bitcoin, tùy thuộc vào mức độ riêng tư mà bạn đang tìm kiếm:

- **Mua theo cách truyền thống thông qua các nền tảng được quản lý** như Bitfinex hoặc Bull Bitcoin (lý tưởng để nạp thêm tiền vào Cold Wallet của bạn);

https://planb.academy/tutorials/exchange/centralized/bitfinex-dc306d39-bd96-4ab9-a278-a322316716db

https://planb.academy/tutorials/exchange/centralized/bitstack-29fd71be-9570-42c6-8f6f-cd355d62e746

https://planb.academy/tutorials/exchange/centralized/bitstamp-5a36c896-bff5-46d7-b505-ff069c3ac47c

https://planb.academy/tutorials/exchange/centralized/bull-bitcoin-europe-0ccf713e-efcd-44ec-8205-211f49ac7d53

https://planb.academy/tutorials/exchange/centralized/kraken-1ef03e25-9b42-49bd-a47d-249e1a13cfc6

https://planb.academy/tutorials/exchange/centralized/paymium-92603f76-b985-49ce-81e5-f4fa0df776e5

https://planb.academy/tutorials/exchange/centralized/relai-v2-30a9671d-e407-459d-9203-4c3eae15b30e

https://planb.academy/tutorials/exchange/centralized/stackinsat-5af6a380-f3c6-4246-9f81-9957a16ab066

- **Thu thập thông tin không cần KYC** thông qua các dịch vụ ngang hàng như Peach, Robosats hoặc ATM Bitcoin; cho phép bạn nạp tiền vào Wallet mà không cần tiết lộ thông tin cá nhân.

https://planb.academy/tutorials/exchange/peer-to-peer/bisq-v2-c1c6a702-6c16-4101-8b90-62c424017b80

https://planb.academy/tutorials/exchange/peer-to-peer/hodlhodl-d7344cd5-6b18-40f5-8e78-2574a93a3879

https://planb.academy/tutorials/exchange/peer-to-peer/lnp2pbot-v2-e6bcb210-610b-487d-970c-7cce85273e3c

https://planb.academy/tutorials/exchange/peer-to-peer/peach-c6143241-d900-4047-9b73-1caba5e1f874

https://planb.academy/tutorials/exchange/peer-to-peer/robosats-b60e4f7c-533a-4295-9f6d-5368152e8c06

- **Kiếm tiền bằng Bitcoin**. Trở thành người dùng tích cực không chỉ là chi tiêu BTC; mà còn là kiếm tiền. Nếu bạn đang kinh doanh, hãy cân nhắc chấp nhận Bitcoin làm phương thức thanh toán. Nếu bạn là nhân viên, hãy xem liệu công ty có cung cấp tùy chọn nhận toàn bộ hoặc một phần lương bằng BTC hay không.

https://planb.academy/tutorials/business/point-of-sale/swiss-bitcoin-pay-2-a78b057e-ed11-47ac-860c-71019fcb451a

https://planb.academy/tutorials/business/point-of-sale/open-node-e69a0c1c-47f7-4932-8494-e6f26c3c9784

https://planb.academy/tutorials/business/point-of-sale/btcpay-server-928eb01e-824b-4b57-a3e8-8727633beddc

![image](assets/vi/081.webp)

#### 4. Quản lý chi phí chủ động

Việc lựa chọn sử dụng Bitcoin thường bị chỉ trích. Một số người dùng Bitcoin thích sử dụng tiền pháp định, loại tiền mà họ cho là kém chất lượng hơn và do đó muốn loại bỏ, hơn là sử dụng Bitcoin. Tuy nhiên, lối suy nghĩ này có hai nhược điểm chính: thứ nhất, nó bỏ qua những lợi thế của Bitcoin như một phương tiện thanh toán (xem phần trước), và thứ hai, nó đặt ra câu hỏi: tại sao lại giữ tiền pháp định, vốn được coi là ít giá trị hơn, để chi tiêu?

Một cách tiếp cận hợp lý hơn là phương pháp "Chi tiêu và Thay thế". Điều này có nghĩa là chi tiêu Bitcoin vì tất cả các lý do đã đề cập trước đó, và sau đó, thay vì để số Bitcoin tiết kiệm của bạn bị hao hụt, bạn sẽ thay thế số Sats đã chi tiêu bằng cách mua Sats mới bằng tiền pháp định.

Để việc chi tiêu Bitcoin dễ dàng hơn, có rất nhiều công cụ hỗ trợ, chẳng hạn như [BTC Map](https://btcmap.org/), giúp bạn tìm thấy các nhà cung cấp trên toàn thế giới chấp nhận Bitcoin. Một số nhà cung cấp này thậm chí còn cho phép bạn đặt hàng trực tuyến và nhận hàng.

Vì chúng ta vẫn đang trong giai đoạn chuyển đổi, khi không phải tất cả các đơn vị giao dịch hàng ngày đều chấp nhận BTC, nên có một số giải pháp đơn giản nhưng hiệu quả. Ví dụ: mua thẻ quà tặng bằng Bitcoin là một giải pháp thiết thực, và nhiều nhà cung cấp như Bitrefill, Coinsbee hoặc The Bitcoin Company đều cung cấp dịch vụ này.

https://planb.academy/tutorials/exchange/centralized/bitrefill-8c588412-1bfc-465b-9bca-e647a647fbc1

#### 5. Nhận thức và tính linh hoạt về công nghệ

Giao thức Bitcoin, cùng với các hệ thống Layer-2 như Lightning Network và toàn bộ ứng dụng Layer liên quan đến các hệ sinh thái này, đang phát triển nhanh chóng. Là người dùng tích cực, bạn nên:

- Luôn cập nhật các giải pháp mới giúp việc áp dụng hàng ngày trở nên dễ dàng hơn;
- Thường xuyên kiểm tra các công cụ và dịch vụ mới để cải thiện trải nghiệm của người dùng;
- Điều chỉnh chiến lược của bạn dựa trên những tiến bộ công nghệ và thay đổi của thị trường.

### Đánh giá hồ sơ người dùng đang hoạt động của bạn

Bạn có phù hợp để trở thành người dùng Bitcoin tích cực không? Dưới đây là những đặc điểm chính xác định hồ sơ của bạn:

- **Chủ nghĩa thực dụng và sự cam kết:** Bạn có động lực để tích hợp Bitcoin vào cuộc sống hàng ngày và đóng vai trò tích cực trong sự phát triển của hệ sinh thái này. Bạn nhận thức được cả những lợi ích lẫn thách thức đi kèm.
- **Trình độ chuyên môn:** Bạn đã nắm vững cách thức hoạt động của ví Bitcoin và Lightning Network; hoặc bạn háo hức học hỏi. Bạn có thể dễ dàng sử dụng các khái niệm nâng cao như quản lý UTXO.
- **Tính linh hoạt và khả năng thích ứng:** Bạn sẵn sàng thử các công cụ mới, chuyển đổi giữa các giải pháp và điều chỉnh các hoạt động của mình khi hệ sinh thái Bitcoin phát triển và nhu cầu của bạn thay đổi.
- **Kỷ luật tài chính:** Bạn quản lý chi tiêu và tiết kiệm Bitcoin một cách cẩn thận. Bạn hiểu rõ tầm quan trọng của việc phân tách rõ ràng các khoản tiền hàng ngày với khoản tiết kiệm dài hạn.

![image](assets/vi/082.webp)

Tóm lại, người dùng tích cực đóng vai trò trực tiếp trong việc thúc đẩy hệ sinh thái Bitcoin phát triển. Bằng cách tích hợp Bitcoin vào thói quen hàng ngày, họ không chỉ ủng hộ việc áp dụng mà còn phát triển chuyên môn của riêng mình.

Trong chương tiếp theo, chúng ta sẽ khám phá hồ sơ người dùng Bitcoin cuối cùng và có lẽ là hấp dẫn nhất: người dùng hoang tưởng.

## Người dùng Bitcoin hoang tưởng: Quyền riêng tư là trên hết

<chapterId>5c624acd-662e-5134-ab7a-fb75cde7c3f8</chapterId>

:::video id=6a7b8c9d-0e1f-2a3b-4c5d-6e7f8a9b0c1d:::

:::id video=6a7b8c9d-0e1f-2a3b-4c5d-6e7f8a9b0c1d:::

Đối với những người dùng Bitcoin hoang tưởng, quyền riêng tư không phải là tùy chọn; mà là thiết yếu. Trong một thế giới mà gần như mọi hoạt động tài chính đều bị theo dõi, họ coi việc bảo vệ danh tính và lịch sử giao dịch là một phần không thể thiếu khi sử dụng Bitcoin. Họ thường rất ý thức được những rủi ro liên quan đến việc tiết lộ dữ liệu cá nhân (đặc biệt là khi sử dụng các sàn giao dịch tuân thủ KYC yêu cầu xác minh danh tính).

Với họ, mọi hành động đều quan trọng. Mọi giao dịch, mọi thiết lập Wallet, và mọi tương tác với mạng lưới đều được cân nhắc kỹ lưỡng. Chỉ một sơ suất nhỏ cũng có thể tiết lộ thông tin nhạy cảm liên kết danh tính của họ với tài sản Bitcoin. Mục tiêu cuối cùng của họ? Sử dụng Bitcoin theo cách bảo mật hoàn toàn danh tính, vị trí và tình hình tài chính của họ.

![image](assets/vi/087.webp)

### Tại sao lại phải thận trọng như vậy?

Việc lựa chọn tư duy coi trọng quyền riêng tư mang lại một số lợi ích chính:

- **Tự chủ về tài chính:** Bằng cách tránh xa sự giám sát của các ngân hàng, chính phủ và tập đoàn, bạn sẽ bảo vệ mình khỏi sự giám sát và can thiệp không mong muốn.
- **Bảo vệ tài sản:** Nếu điều kiện kinh tế hoặc chính trị trở nên bất ổn, bên thứ ba sẽ khó có thể truy tìm hoặc thu giữ Bitcoin của bạn hơn nếu nó không liên quan đến danh tính của bạn.
- **Cải thiện an toàn cá nhân:** Những người được biết là sở hữu lượng lớn Bitcoin có thể trở thành mục tiêu. Việc giữ kín danh tính sẽ giúp giảm nguy cơ bị nhắm mục tiêu.
- **Không bị phân biệt đối xử**: Một số dịch vụ hoặc chính phủ có thể chặn giao dịch hoặc tài khoản dựa trên hoạt động trước đây hoặc danh tính của bạn. Quyền riêng tư giúp kiểm soát quyền lực đó.
- **Giữ nguyên bản chất của Bitcoin:** Bitcoin được thiết kế như một hình thức tiền tệ không kiểm duyệt và không cần xin phép. Bảo vệ quyền riêng tư của bạn là một phần trong việc thực hiện sứ mệnh ban đầu đó.

Đây là một phản ứng thông minh, được tính toán kỹ lưỡng trước những mối đe dọa thực sự đang tồn tại trong bối cảnh kỹ thuật số và tài chính ngày nay. Đúng vậy, cách tiếp cận này đòi hỏi nhiều nỗ lực hơn. Nó đòi hỏi thời gian, kiến thức chuyên môn và tính kỷ luật. Nhưng đối với những người chọn con đường này, vấn đề không chỉ là sử dụng Bitcoin; mà còn là giành lại quyền kiểm soát tài chính, danh tính và sự tự do của họ.

Họ không chỉ là nhà đầu tư hay người dùng; họ đang từ chối tham gia vào một hệ thống đòi hỏi phải biết mọi thứ về họ.

### Kế hoạch đầu tư hoang tưởng

Những người dùng Bitcoin hoang tưởng thường tuân theo một chiến lược nghiêm ngặt được xây dựng xung quanh hai mục tiêu cốt lõi: bảo vệ tính ẩn danh và tối đa hóa bảo mật. Kiểu người dùng này tìm cách tránh tiếp xúc với các nền tảng được quản lý (đặc biệt là những nền tảng yêu cầu KYC), loại bỏ khả năng truy xuất nguồn gốc giao dịch và bảo vệ tiền của họ khỏi sự giám sát, tịch thu và các mối đe dọa bên ngoài khác.

![image](assets/vi/092.webp)

#### 1. Thiết lập ví và nút

Người dùng hoang tưởng không bao giờ trộn lẫn tiền dựa trên nguồn gốc hoặc mục đích sử dụng. Thay vào đó, họ phân loại Bitcoin theo mục đích, sử dụng nhiều ví chuyên dụng:

- **Cold Wallet (ví air-gapped và/hoặc tự làm)** Các khoản nắm giữ dài hạn được lưu trữ trong các ví air-gapped hoặc tự làm có độ bảo mật cao. Đây thường là các thiết bị nguồn mở hoặc có mã xác minh, chẳng hạn như Jade, Coldcard, SeedSigner, Portal hoặc Passport... Bất cứ khi nào có thể, họ sẽ mua các thiết bị này một cách ẩn danh; bằng cách trả tiền mặt hoặc sử dụng Bitcoin tại cửa hàng thực tế, hoặc bằng cách đặt hàng trực tuyến với các tùy chọn vận chuyển kín đáo và thanh toán bằng BTC. Ví dụ, tại Pháp, [Bitcoin Bazar](https://bitcoinbazar.fr/) là một trong những cửa hàng như vậy. Bạn có thể tìm kiếm các nhà cung cấp tương tự trong khu vực của mình.

https://planb.academy/tutorials/wallet/hardware/jade-plus-sparrow-938abf16-e10a-4618-860d-cd771373a262

https://planb.academy/tutorials/wallet/hardware/coldcard-q-advanced-b8cc3f29-eea9-48fe-a953-b003d5b115e0

https://planb.academy/tutorials/wallet/hardware/passport-74e53858-3fa2-43f9-b866-573297546236

https://planb.academy/tutorials/wallet/hardware/portal-bd6e83f0-0100-4689-aeb3-513cca944ace

- **Hot Wallet dành cho các giao dịch hàng ngày**. Đối với chi tiêu thường xuyên, người dùng e ngại lựa chọn các ví tập trung vào quyền riêng tư như Ashigaru, Wasabi Wallet hoặc Sparrow wallet. Các ví này hỗ trợ các tính năng nâng cao như: giao dịch chi tiêu mini-(https://planb.academy/resources/glossary/coinjoin), payjoin, công cụ CoinJoin tiềm năng, kiểm soát Coin, gắn thẻ UTXO hoặc khả năng kết nối thông qua nút Bitcoin của riêng bạn.

https://planb.academy/tutorials/wallet/desktop/sparrow-c674e2ac-d46f-4c82-92a7-7d1b0e262f5d

https://planb.academy/tutorials/wallet/desktop/wasabi-a0b51540-32d2-4ed2-98aa-801da5d35cf6

![image](assets/vi/089.webp)

- Việc vận hành **nút Bitcoin** của riêng họ là điều cần thiết. Nó đảm bảo toàn quyền kiểm soát việc phát sóng giao dịch và tránh phụ thuộc vào cơ sở hạ tầng của bên thứ ba. Nó cũng bảo vệ quyền riêng tư, ngăn chặn kiểm duyệt và cho phép họ tự xác minh các quy tắc của giao thức Bitcoin. Dù trên Linux, Windows, macOS hay thông qua các thiết lập plug-and-play như RaspiBlitz, myNode hay Start9, người dùng hoang tưởng đều coi nút của họ như một trụ cột chủ quyền không thể thương lượng.

https://planb.academy/tutorials/node/bitcoin/bitcoin-core-linux-568c13a6-8746-4d63-8e95-f4a61c5ae0ed

https://planb.academy/tutorials/node/bitcoin/bitcoin-core-mac-windows-9684ab02-e0af-41c9-8102-86ac7c7727f3

https://planb.academy/tutorials/node/bitcoin/bitcoin-knots-e04b2196-4df2-4246-86ef-c02269c29098

https://planb.academy/tutorials/node/bitcoin/raspiblitz-d8cdba2e-a682-46cf-9fdc-d8602fbeac02

https://planb.academy/tutorials/node/bitcoin/mynode-a481fef3-2fd3-4df3-91c0-112cffa094eb

https://planb.academy/tutorials/node/bitcoin/start9-8c8b6827-8423-4929-bcba-89057670ed6a

- Để giao dịch qua **Lightning Network** mà vẫn duy trì mức độ kiểm soát và quyền tự quyết như cũ, họ cũng vận hành nút Lightning riêng. Điều này cho phép họ quản lý trực tiếp các kênh và chi tiêu theo cách giảm thiểu sự giám sát hoặc phụ thuộc vào các dịch vụ lưu ký.

https://planb.academy/tutorials/node/lightning-network/alby-hub-62e6356c-6a6d-4134-8f22-c3b6afb9882a

![image](assets/vi/090.webp)

Bảo mật không chỉ dừng lại ở đó. Để sao lưu các cụm từ seed, người dùng hoang tưởng sử dụng các bản sao lưu kim loại kết hợp với phong bì niêm phong chống giả mạo. Họ thường xuyên kiểm tra để đảm bảo các bản sao lưu không bị xâm phạm và kiểm tra các quy trình khôi phục Wallet để sẵn sàng cho mọi tình huống bất ngờ.

https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

https://planb.academy/tutorials/wallet/backup/recovery-test-5a75db51-a6a1-4338-a02a-164a8d91b895

Khi nói đến việc bảo mật tiền tiết kiệm, họ có thể triển khai các thiết lập phức tạp hơn như ví đa chữ ký (có hoặc không có khóa thời gian).

https://planb.academy/tutorials/wallet/desktop/liana-306ef457-700c-4fdd-b07a-8fb7a8a29f04

Những cấu hình này giúp giảm thiểu rủi ro. Việc thêm BIP39 passphrase vào cụm từ seed sẽ tăng thêm một lớp bảo vệ nữa cho Layer và đảm bảo quyền truy cập vào quỹ của họ không thể bị xâm phạm chỉ bằng một phát hiện duy nhất.

https://planb.academy/tutorials/wallet/backup/passphrase-a26a0220-806c-44b4-af14-bafdeb1adce7

Một người dùng Bitcoin hoang tưởng tất nhiên sẽ có một kế hoạch thừa kế được cân nhắc kỹ lưỡng cho số Bitcoin của mình. Chủ đề quan trọng này sẽ được đề cập chi tiết trong phần tiếp theo của khóa học.

Nói rộng hơn, loại người dùng này áp dụng một phương pháp tiếp cận tiên tiến, chú trọng đến quyền riêng tư cho toàn bộ hoạt động điện toán. Họ dựa vào các công cụ mã nguồn mở, an toàn, tôn trọng quyền riêng tư của người dùng trong mọi hoạt động số của họ:

- **Hệ điều hành**, cả trên điện thoại và máy tính của họ;

https://planb.academy/tutorials/computer-security/operating-system/grapheneos-08d43d7a-0b22-4638-a151-578d48d32d88

https://planb.academy/tutorials/computer-security/operating-system/linux-mint-da44852e-513f-4004-949a-8fde60c1bca5

https://planb.academy/tutorials/computer-security/operating-system/ubuntu-78a3be56-5d51-4ec3-8629-0dd27c352ab5

- **Công cụ xác thực**;

https://planb.academy/tutorials/computer-security/authentication/authy-a76ab26b-71b0-473c-aa7c-c49153705eb7

https://planb.academy/tutorials/computer-security/authentication/bitwarden-0532f569-fb00-4fad-acba-2fcb1bf05de9

https://planb.academy/tutorials/computer-security/authentication/keepass-f8073bb7-5b4a-4664-9246-228e307be246

https://planb.academy/tutorials/computer-security/authentication/security-key-61438267-74db-4f1a-87e4-97c8e673533e

- **Công cụ giao tiếp**;

https://planb.academy/tutorials/computer-security/communication/ivpn-5a0cd5df-29f1-4382-a817-975a96646e68

https://planb.academy/tutorials/computer-security/communication/mullvad-968ec5f5-b3f0-4d23-a9e0-c07a3e85aaa8

https://planb.academy/tutorials/computer-security/communication/proton-mail-c3b010ce-254d-4546-b382-19ab9261c6a2

https://planb.academy/tutorials/computer-security/communication/tor-browser-a847e83c-31ef-4439-9eac-742b255129bb

- **Công cụ quản lý dữ liệu**.

https://planb.academy/tutorials/computer-security/data/integrity-authenticity-21d0420a-be02-4663-94a3-8d487f23becc

https://planb.academy/tutorials/computer-security/data/proton-drive-03cbe49f-6ddc-491f-8786-bc20d98ebb16

https://planb.academy/tutorials/computer-security/data/veracrypt-d5ed4c83-7c1c-4181-95ea-963fdf2d83c5

Nếu bạn muốn tìm hiểu thêm về chủ đề bảo mật CNTT, chúng tôi cung cấp khóa học năm nhất hoàn chỉnh dành riêng cho chủ đề này:

https://planb.academy/courses/99c46148-7080-4915-a7e0-9df0e145cd47

#### 2. Mua bitcoin ẩn danh

Một nguyên tắc cốt lõi của các nhà đầu tư hoang tưởng là mua Bitcoin mà không liên kết nó với danh tính thực tế của họ. Để tránh các nền tảng KYC và ngăn chặn việc lấy dấu vân tay On-Chain, họ áp dụng một số chiến lược sau:

- **Nền tảng mua P2P** như Bisq, Robosat, LNP2PBot, Peach, HODL HODL, v.v.;

https://planb.academy/tutorials/exchange/peer-to-peer/bisq-v2-c1c6a702-6c16-4101-8b90-62c424017b80

https://planb.academy/tutorials/exchange/peer-to-peer/hodlhodl-d7344cd5-6b18-40f5-8e78-2574a93a3879

https://planb.academy/tutorials/exchange/peer-to-peer/lnp2pbot-v2-e6bcb210-610b-487d-970c-7cce85273e3c

https://planb.academy/tutorials/exchange/peer-to-peer/peach-c6143241-d900-4047-9b73-1caba5e1f874

https://planb.academy/tutorials/exchange/peer-to-peer/robosats-b60e4f7c-533a-4295-9f6d-5368152e8c06

- **Giao dịch tiền mặt trực tiếp**, ví dụ như tại các buổi gặp mặt Bitcoin tại địa phương;

https://planb.academy/courses/1c643dc9-a15f-4f9e-93b6-cb3c58b4ee35

- **Máy ATM Bitcoin**;
- **Kiếm sats** bằng cách cung cấp hàng hóa hoặc dịch vụ để đổi lấy bitcoin;
- **Mining bitcoin**.

https://planb.academy/tutorials/mining/hardware/attakai-0d177e6b-e167-4b25-8e38-4ec74213d1fb

https://planb.academy/tutorials/mining/hardware/braiins-mini-miner-f5aec001-fb05-4e89-b3b2-a31abec1253c

https://planb.academy/tutorials/mining/hardware/bitaxe-d317e1a7-d775-401d-85df-51f4f2217566

Các sàn giao dịch tập trung hoàn toàn bị tránh do những rủi ro mà chúng gây ra; cả về quyền riêng tư lẫn khả năng bị kiểm duyệt hoặc tịch thu.

![image](assets/vi/091.webp)

#### 3. Ngăn chặn theo dõi trên chuỗi

Ngay cả sau khi mua coin ẩn danh, bạn vẫn có thể bị rò rỉ dữ liệu cá nhân (On-Chain) nếu không cẩn thận. Người dùng hoang tưởng sẽ thực hiện thêm các bước để bảo vệ quyền riêng tư ở cấp độ giao dịch:

- **Tránh tái sử dụng Address**, bằng cách thủ công hoặc sử dụng các công cụ tiên tiến như Thanh toán im lặng hoặc BIP47;
- **Gắn nhãn UTXO của bạn**;
- **Thực hành kiểm soát Coin khi chi tiêu**;
- **Tránh việc hợp nhất UTXO**;
- **Sử dụng cấu trúc giao dịch bảo vệ quyền riêng tư**: coinjoin, payjoin, ricochets và mini-payment coinjoin...

Họ cũng quen thuộc với những kiến thức cơ bản về giám sát Blockchain và biết cách các công ty theo dõi hoạt động. Bằng cách hiểu các phương pháp và mô hình được các công ty phân tích chuỗi sử dụng, họ có thể chủ động thực hiện các bước để tránh rơi vào những cạm bẫy đã biết.

Đối với những người muốn tìm hiểu sâu hơn về quyền riêng tư và khả năng chống giám sát của On-Chain, chúng tôi cung cấp khóa đào tạo nâng cao năm thứ hai về các biện pháp bảo mật của Bitcoin:

https://planb.academy/courses/65c138b0-4161-4958-bbe3-c12916bc959c

#### 4. Luôn cảnh giác và cập nhật chiến lược của họ

Giao thức Bitcoin, Lightning Network và các công cụ liên quan đang không ngừng phát triển. Người dùng hoang tưởng vẫn luôn cảnh giác và thích nghi:

- Luôn cập nhật thông tin về các kỹ thuật bảo mật và công cụ bảo mật mới
- Thường xuyên kiểm tra các công cụ và dịch vụ mới để duy trì hoạt động ổn định
- Kiểm tra thường xuyên xem ví phần cứng và các nút có được cập nhật và hoạt động chính xác không
- Kiểm tra định kỳ rằng các bản sao lưu (ví dụ: phong bì Mnemonic đã niêm phong) không bị giả mạo
- Theo dõi diễn biến pháp lý để điều chỉnh chiến lược của họ theo thời gian thực

### Đánh giá hồ sơ hoang tưởng của bạn

Áp dụng chiến lược hoang tưởng không phải là một quyết định dễ dàng. Nó đòi hỏi tính kỷ luật nghiêm ngặt, sự siêng năng liên tục và đầu tư đáng kể thời gian để thành thạo các công cụ và phương pháp bảo mật tốt nhất.

Cách tiếp cận này có phù hợp với bạn không?

Sau đây là những đặc điểm chính của một người đầu tư bitcoin hoang tưởng:

- **Cảnh giác và ý thức bảo mật**: Bạn rất coi trọng quyền riêng tư và sẵn sàng thực hiện các biện pháp phòng ngừa bổ sung để ngăn chặn mọi rò rỉ thông tin cá nhân. Bạn coi việc giám sát tài chính là một mối đe dọa nghiêm trọng và chủ động tìm cách bảo vệ bản thân khỏi nó.
- **Độc lập**: Bạn từ chối phụ thuộc vào bất kỳ dịch vụ bên thứ ba nào để lưu trữ, quản lý hoặc chuyển giao bitcoin của mình. Bạn sẵn sàng tự lưu trữ các node Bitcoin và Lightning, sử dụng ví phức tạp và chịu hoàn toàn trách nhiệm quản lý các bản sao lưu và khóa riêng tư của mình.
- **Không tin tưởng**: Bạn chủ động tránh các nền tảng KYC và ưu tiên các phương thức thu thập P2P ẩn danh, ngay cả khi phải trả phí bảo hiểm. Bạn hiểu rằng bất kỳ dữ liệu cá nhân nào được chia sẻ một ngày nào đó đều có thể được sử dụng để chống lại bạn.
- **Có kiến thức và kỷ luật**: Bạn đam mê an ninh mạng và các công nghệ nâng cao quyền riêng tư. Bạn dành thời gian nghiên cứu các giải pháp hiện có, thử nghiệm công cụ, tự học hỏi về các phương pháp hay nhất và áp dụng các phương pháp nghiêm ngặt để giảm thiểu rủi ro.
- **Thực tế và Thực dụng**: Bạn hiểu rằng việc ẩn danh hoàn toàn gần như là không thể, nhưng bạn nỗ lực giảm thiểu tối đa khả năng bị lộ thông tin, đồng thời cân bằng giữa quyền riêng tư, bảo mật và khả năng sử dụng. Phương pháp của bạn liên tục phát triển để ứng phó với các mối đe dọa mới và sự phát triển công nghệ.

Nếu những đặc điểm này nghe quen thuộc, chiến lược này có thể phù hợp với bạn. Chiến lược này đòi hỏi nhiều nỗ lực và sự kiên trì, nhưng lại mang lại mức độ tự chủ tài chính cao và khả năng bảo vệ tiền của bạn khỏi bị giám sát, tịch thu hoặc truy xuất nguồn gốc.

![image](assets/vi/088.webp)

Cách tiếp cận này đặc biệt phù hợp nếu bạn:

- Giữ một lượng đáng kể Bitcoin,
- Sống dưới chế độ độc tài,
- Cần bảo vệ sự ẩn danh của bạn vì lý do cá nhân, nghề nghiệp hoặc ý thức hệ.

Trong phần tiếp theo của khóa học, chúng ta sẽ thảo luận về một chủ đề quan trọng thường bị bỏ qua: cách chuyển giao bitcoin của bạn trong trường hợp khẩn cấp.

# Bảo vệ người thừa kế và tài sản của bạn

<partId>0b8a9528-719b-4a2d-bd50-1e173a896570</partId>

## Tạo kế hoạch thừa kế

<chapterId>233c88d3-2e8e-5eba-ac06-efe67a209038</chapterId>

:::video id=7b8c9d0e-1f2a-3b4c-5d6e-7f8a9b0c1d2e:::

Việc đảm bảo việc chuyển giao Bitcoin của bạn là một trách nhiệm thường bị bỏ qua nhưng lại rất quan trọng. Quyền tự chủ tài chính mà Bitcoin mang lại cũng đi kèm với nhu cầu quản lý di sản cẩn thận. Nếu không, người thừa kế của bạn có thể không thể tiếp cận được số tiền bạn kiếm được từ Hard. Trong chương này, chúng ta sẽ tìm hiểu những nguyên tắc cơ bản của việc lập kế hoạch di sản áp dụng cho Bitcoin.

### Tại sao cần phải chuẩn bị kế hoạch kế nhiệm Bitcoin?

Hãy tưởng tượng một sự kiện bất ngờ (tai nạn hoặc tình huống bất ngờ) xảy ra và bạn không còn ở đây nữa. Gia đình bạn, vốn đã đau buồn, giờ lại phải đối mặt với một thách thức khác: truy cập bitcoin của bạn. Họ có thể đã nghe bạn nói về khóa riêng tư, cụm từ Mnemonic và tính không thể đảo ngược của giao dịch, nhưng những khái niệm này có thể còn mơ hồ với họ. Họ phải tự mình tìm hiểu.

Sau đó bạn có hai lựa chọn:

- Dành thời gian để thiết lập một kế hoạch rõ ràng, có cấu trúc cho phép những người thân yêu của bạn truy cập vào bitcoin của bạn một cách dễ dàng và an toàn;
- Không làm gì cả, hy vọng họ sẽ tự tìm ra cách. Nhưng nếu họ mắc lỗi, mất quyền truy cập vào Wallet hoặc vô tình gửi tiền đến nhầm Address, tài sản của bạn có thể bị mất vĩnh viễn.

Chỉ cần dành ra 15 phút đến một giờ để lập kế hoạch thừa kế có thể tạo nên sự khác biệt lớn. Đây không chỉ là vấn đề thận trọng mà còn là cách thể hiện trách nhiệm với những người có thể tin tưởng bạn.

### Mục tiêu của Kế hoạch Kế nhiệm Bitcoin

Trong cuốn sách *[Cryptoasset Inheritance Planning](https://www.amazon.com/gp/product/1947910116/)*, Pamela Morgan đã nêu ra bốn mục tiêu chính cho một kế hoạch thừa kế vững chắc:

- Đảm bảo người thừa kế của bạn có thể truy cập vào bitcoin của bạn vào đúng thời điểm (nhưng không phải trước đó);
- Giảm thiểu rủi ro bị trộm cắp hoặc bị xâm phạm trước khi họ thừa kế bitcoin;
- Hãy đảm bảo rằng những người thừa kế của bạn biết cách bảo vệ bitcoin lâu dài nếu họ muốn;
- Tránh tranh chấp gia đình và hạn chế các vấn đề pháp lý phức tạp khi chuyển giao tài sản tiền điện tử.

Một kế hoạch được cân nhắc kỹ lưỡng không chỉ là việc chuyển giao tài sản; mà còn là việc bảo vệ những người thân yêu của bạn khỏi những sai lầm phổ biến và các mối đe dọa tiềm ẩn.

Chương này được lấy cảm hứng từ công trình của [Pamela Morgan](https://x.com/pamelawjd). [Cuốn sách của bà](https://www.amazon.com/gp/product/1947910116/) và mặc dù cuốn sách của bà cung cấp một phương pháp chi tiết và được chuyên gia xác thực để lập kế hoạch thừa kế Bitcoin, **nội dung này không cấu thành tư vấn pháp lý**. Đây là một phương pháp đã được chứng minh, nhưng mỗi người nên tự nghiên cứu và điều chỉnh các khuyến nghị cho phù hợp với hoàn cảnh cá nhân và thẩm quyền pháp lý của mình.

[Pamela Morgan](https://x.com/pamelawjd) đã hào phóng cho phép sử dụng [công trình của cô ấy](https://www.amazon.com/gp/product/1947910116/) cho khóa học này, và chúng ta sẽ áp dụng phương pháp của cô ấy để xây dựng một kế hoạch thừa kế Bitcoin cụ thể. Trong chương này, chúng ta sẽ tập trung vào phần đầu tiên của cuốn sách.

![image](assets/vi/093.webp)

Để hiểu rõ hơn về cách tạo kế hoạch kế thừa Bitcoin, chúng ta sẽ xem xét ví dụ về Cédric, một người dùng Bitcoin điển hình, người cần sắp xếp cách thức chuyển giao tài sản của mình nếu có điều gì bất ngờ xảy ra.

Hồ sơ của anh ấy:

- Nhà đầu tư dài hạn không giao dịch thường xuyên;
- Sở hữu một chiếc Hardware Wallet và một chiếc Wallet di động để sử dụng thỉnh thoảng;
- Sử dụng một nền tảng giao dịch duy nhất có xác minh danh tính (KYC) để mua bitcoin;
- Được anh họ giới thiệu đến Bitcoin;
- Không có altcoin hoặc sử dụng Lightning Network.

Mục tiêu của chúng tôi là tạo ra một kế hoạch đơn giản, hiệu quả phù hợp với hồ sơ của Cédric trước khi chuyển sang các tình huống phức tạp hơn liên quan đến các loại người dùng khác.

![image](assets/vi/095.webp)

### Trước khi bạn bắt đầu

Hãy dành 30 phút. Không phải cho bản thân bạn, mà cho những người phụ thuộc vào bạn. Lập kế hoạch tài sản là một nhiệm vụ quan trọng, nhưng lại thường bị trì hoãn, bỏ bê hoặc thậm chí bị phớt lờ. Quá nhiều người, ngay cả những người thận trọng nhất, cũng trì hoãn việc này... cho đến khi quá muộn. Hàng ngàn Bitcoin đã bị mất do thiếu tầm nhìn xa. Đừng mắc phải sai lầm này! Đây là **BƯỚC CUỐI CÙNG** trong hành trình hướng đến chủ quyền tài chính của bạn: đảm bảo tài sản Bitcoin cho những người thân yêu của bạn.

#### Bạn cần gì?

Hãy đảm bảo rằng bạn có một môi trường yên tĩnh, không bị phân tâm, sau đó chuẩn bị một số công cụ sau:

- 4 đến 5 tờ giấy trắng;
- Cây bút;
- 2 phong bì;
- Một cuốn sổ điện thoại hoặc địa chỉ;
- Một máy tính (tùy chọn).

![image](assets/vi/094.webp)

#### Những quan niệm sai lầm phổ biến về kế hoạch thừa kế

Nhiều người trì hoãn bước này vì những quan niệm sai lầm khiến họ không hành động. Dưới đây là một số lầm tưởng phổ biến, được Pamela Morgan nêu ra trong cuốn _Lập kế hoạch thừa kế tài sản mã hóa_ (trang 18):

- **"*Tôi cần thuê luật sư.*"** → **Sai**. Mặc dù luật sư có thể hữu ích trong các vấn đề pháp lý, nhưng bạn vẫn có thể thiết lập kế hoạch thừa kế Bitcoin mà không cần luật sư. Điều quan trọng nhất là phải có hướng dẫn rõ ràng và dễ hiểu;
- **"*Tôi cần tin tưởng một bên thứ ba.*"** → **Sai**. Kế hoạch của bạn có thể được thiết kế theo cách giảm thiểu nhu cầu tin tưởng, chẳng hạn như phân phối thông tin cho nhiều bên hoặc sử dụng các giải pháp đa chữ ký, có hoặc không có khóa thời gian;
- **"*Việc lập kế hoạch sẽ khiến tài sản của tôi dễ bị đánh cắp.*"** → **Sai**. Một kế hoạch được cân nhắc kỹ lưỡng sẽ bảo vệ bạn khỏi các nỗ lực trộm cắp, đồng thời đảm bảo người thừa kế của bạn có thể lấy lại tiền của bạn một cách an toàn. Tuy nhiên, điều quan trọng cần lưu ý là không có giải pháp nào là hoàn hảo: kế hoạch thừa kế làm tăng nguy cơ bị đánh cắp, tùy thuộc vào thông tin được chia sẻ. Chúng ta sẽ thảo luận chi tiết hơn về vấn đề này sau;
- **"*Giá trị bitcoin của tôi quá nhỏ để lên kế hoạch.*"** → **Sai**. Lên kế hoạch trước luôn tốt hơn. Những người thân yêu của bạn có thể không biết chính xác số lượng Bitcoin mà bạn sở hữu, và đó là điều tốt. Nhưng nếu có chuyện gì xảy ra với bạn, chẳng phải họ sẽ muốn lấy lại nó sao, ngay cả khi họ không biết chính xác số lượng? Giá trị của Bitcoin có thể tăng đáng kể theo thời gian, vì vậy, hãy tạo điều kiện thuận lợi để họ tiếp cận tiền của bạn, và tránh tạo cho họ hy vọng hão huyền về giá trị hoặc khiến họ phải tìm kiếm thứ gì đó có thể không tồn tại.
- **"*Người thừa kế của tôi sẽ tự tìm hiểu.*"** → **Sai**. Bitcoin không giống như một tài khoản ngân hàng thông thường. Nếu không có hướng dẫn rõ ràng, người thân của bạn có thể không bao giờ truy cập được vào tiền của bạn, hoặc chỉ tìm thấy một phần trong số đó. Không giống như tài khoản ngân hàng, nơi các tổ chức tài chính hoặc công chứng viên liên hệ với các thành viên gia đình sau khi bạn qua đời, không có bên trung gian nào sẽ thông báo cho gia đình bạn về Bitcoin Wallet của bạn. Do đó, bạn phải tự mình đưa nó vào kế hoạch di sản của mình;
- **"*Smart contract có thể quản lý mọi thứ.*"** → **Sai**. Smart contract, chẳng hạn như multi-sig Wallet có khóa thời gian, có thể là một phần của giải pháp, nhưng nó sẽ không bao giờ thay thế được một kế hoạch được xây dựng bài bản, đặc biệt là đối với những người chưa quen với Bitcoin. Cả hai giải pháp đều bổ sung cho nhau.

Đã đến lúc hành động. Hãy dành 30 phút này để thiết lập kế hoạch thừa kế của bạn. Đây là một bước đơn giản, nhưng có thể tạo nên sự khác biệt lớn cho những người thân yêu của bạn.

### Bước 1: Chọn đúng người để hỗ trợ người thừa kế của bạn

Khi nói đến việc chuyển nhượng tài sản thừa kế trong trường hợp Bitcoin, người thân của bạn có thể sẽ không quen với việc quản lý khóa riêng hoặc khôi phục ví. Họ sẽ cần sự hỗ trợ từ những cá nhân có năng lực và đáng tin cậy. Cách tiếp cận lý tưởng là chọn ít nhất hai người khác nhau:

- **Một người thân đáng tin cậy**, người sẽ đảm bảo kế hoạch của bạn được thực hiện suôn sẻ. Họ không nhất thiết phải có kiến thức chuyên sâu về Bitcoin, nhưng phải là người mà người thừa kế của bạn có thể tin cậy.
- **Người dùng Bitcoin** có kinh nghiệm, có thể cung cấp hỗ trợ kỹ thuật để khôi phục tiền, quản lý ví và hiểu các quy trình liên quan.

Những người bạn chọn sẽ không bao giờ được phép truy cập trực tiếp vào khóa riêng tư của bạn (hoặc cụm từ Mnemonic), nhưng họ cần có khả năng:

- Hướng dẫn người thừa kế của bạn trong việc thu hồi và đảm bảo tiền;
- Giải thích những điều cơ bản về ví Bitcoin, cụm từ phục hồi và giao dịch;
- Giúp tránh những lỗi thường gặp.

Trên giấy tờ sẽ trở thành kế hoạch thừa kế Bitcoin của bạn, hãy tạo một bảng so sánh các ứng viên tiềm năng, đánh giá mức độ tin cậy, kiến thức của họ về Bitcoin và cách liên hệ với họ. Ví dụ:

| Ai               | Mức độ tin tưởng    | Kiến thức về Bitcoin   | Phương thức liên lạc         | Ghi chú                                                                                                                   |
|----------------------|-----------------|----------------------|--------------------------|------------------------------------------------------------------------------------------------------------------------|
| Anh trai tôi – Bob   | Rất cao         | Thấp                 | Điện thoại & email       | Bob không hiểu nhiều về Bitcoin, nhưng hoàn toàn đáng tin cậy. Anh ấy có thể đảm bảo mọi việc diễn ra suôn sẻ.        |
| Anh họ tôi – Nathan  | Cao             | Trung bình           | Điện thoại & Instagram   | Có một số kiến thức về Bitcoin và có thể hướng dẫn người thừa kế. Đã biết kế hoạch. Người cần liên hệ đầu tiên khi cần. |
| Ricco (bạn Bitcoiner)| Trung bình      | Rất cao              | Twitter, email & gặp trực tiếp | Rất giỏi về mặt kỹ thuật, nhưng tuyệt đối không nên có quyền truy cập vào tài sản. Chỉ liên hệ để hỗ trợ kỹ thuật.   |
| YouTuber về Bitcoin  | Thấp            | Cao                  | Kênh YouTube             | Nguồn thông tin tốt để học hỏi, nhưng không thể can thiệp trực tiếp.                                                    |

Nếu bạn không có thành viên gia đình đáng tin cậy hoặc có năng lực, bạn cũng có thể cân nhắc thuê một chuyên gia, chẳng hạn như luật sư chuyên về thừa kế Bitcoin, hoặc dịch vụ lập kế hoạch di sản chuyên biệt. Điều quan trọng là người thừa kế của bạn có thể tiếp cận được sự hỗ trợ kỹ thuật đáng tin cậy trong khi vẫn đảm bảo an toàn và bảo mật cho tài sản của bạn.

### Bước 2: Tạo một bản kiểm kê nhanh chóng và dễ dàng

Trước khi nghĩ đến việc bảo mật hoặc chuyển nhượng Bitcoin, điều quan trọng là phải lập danh mục tài sản Bitcoin rõ ràng. Danh mục này sẽ là nền tảng để bạn lập kế hoạch thừa kế và giúp người thừa kế hiểu rõ tài sản của bạn đang ở đâu và cách tiếp cận chúng.

Mục tiêu ở đây không phải là cải thiện bảo mật ngay lập tức, mà chỉ đơn giản là lập danh mục tình hình hiện tại của bạn. Đây là ảnh chụp nhanh về số bitcoin của bạn và cách thức truy cập chúng. Bạn có thể điều chỉnh và tăng cường bảo mật sau khi hoàn tất quá trình kiểm kê đầu tiên này.

Hãy xem xét tất cả những nơi bạn có bitcoin hoặc tiền pháp định liên quan đến Bitcoin. Điều này có thể bao gồm:

- **Nền tảng trao đổi**: Tài khoản có BTC hoặc tiền pháp định được liên kết với giao dịch mua Bitcoin của bạn.
- **Ví nóng (di động hoặc máy tính để bàn)**: Ứng dụng được cài đặt trên điện thoại của bạn, được sử dụng cho các giao dịch hàng ngày;
- **Ví cứng**: Thiết bị vật lý lưu trữ khóa riêng của bạn ngoại tuyến;
- **Các giải pháp khác**: Multisig, ví giấy, khóa riêng được lưu trữ đặc biệt, v.v.

Sử dụng bảng để sắp xếp danh mục này. Mục đích không phải là lưu trữ tài liệu trực tuyến mà là giữ nó ở một nơi an toàn, lý tưởng nhất là trên giấy. Ví dụ:

| Tổng quan | Loại lưu trữ | Tài sản nắm giữ | Bản địa hóa | Bản sao lưu Mnemonic | Mật khẩu (mã PIN, passphrase...) | Ghi chú |
| --------------------- | ---------------- | -------------- | --------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Bitfinex | BTC & Euro | Có thể truy cập online | Không có (nền tảng lưu ký) | | Trình quản lý mật khẩu Bitwarden & 2FA với ứng dụng Authy trên điện thoại của tôi | Tôi mua BTC tại đây. Cần rút tài sản sau khi mua |
| Ví vật lý | Jade Plus | BTC | Két sắt cá nhân tại nhà | Một bản sao tại nhà chú Bob và một bản trong két sắt ngân hàng BNP Paribas | Passphrase được lưu tại nhà mẹ tôi. Mã PIN được lưu trong trình quản lý mật khẩu Bitwarden. | Tôi sử dụng 2 ví riêng biệt: một ví thông thường chỉ dùng mnemonic phrase và một ví có passphrase. |
| Green Wallet | BTC | Trên iPhone 15 của tôi | Bản sao seed được lưu trong két sắt tại nhà | Mã PIN được lưu trong trình quản lý mật khẩu Bitwarden | Ứng dụng ở chế độ ẩn | Ứng dụng ở chế độ ẩn |

Ở giai đoạn này, bạn có thể cảm thấy thôi thúc phải ngay lập tức điều chỉnh phân bổ quỹ, cải thiện an ninh, hoặc thậm chí mua hoặc bán thêm bitcoin. Đừng hành động vội! Mục tiêu ở đây không phải là hành động mà là để thiết lập một bức tranh tổng quan về tình hình hiện tại của bạn. Bạn luôn có thể cải thiện kế hoạch của mình sau, nhưng bây giờ, hãy tập trung vào việc hoàn thành việc kiểm kê một cách kỹ lưỡng nhất có thể.

Sau khi hoàn tất việc kiểm kê, việc xác định các điểm yếu và tổ chức quy trình truyền tải an toàn và hiệu quả sẽ dễ dàng hơn nhiều.

### Bước 3: Viết thư

#### Tại sao lại là thư viết tay?

Điều quan trọng là kế hoạch thừa kế của bạn phải rõ ràng, dễ đọc và bảo mật. Để đạt được điều này, hãy viết thư bằng tay, dùng mực và giấy, và tránh các tài liệu kỹ thuật số có thể bị xâm phạm. Thư này không phải là di chúc hay tuyên bố pháp lý, mà là một hướng dẫn thực tế để giúp người thân của bạn lấy lại bitcoin một cách an toàn.

Bức thư phải đơn giản, trực tiếp và chứa những nội dung thiết yếu sau đây theo Elements:

- Phần giới thiệu giải thích bản chất của tài sản cần thu hồi;
- Liên hệ với những người đáng tin cậy có thể hỗ trợ người thừa kế của bạn trong quá trình phục hồi;
- Một bản kê khai tài sản rõ ràng, bao gồm phương pháp lưu trữ và thông tin chi tiết về quyền truy cập;
- Hướng dẫn bảo mật cụ thể để tránh lỗi hoặc lừa đảo;
- Thông điệp cuối cùng, cùng với bất kỳ hướng dẫn pháp lý nào nếu có.

#### Mẫu thư thừa kế

Dưới đây là một mô hình lấy cảm hứng từ cuốn "Kế hoạch Kế thừa Tài sản Tiền điện tử" của Pamela Morgan. Tất nhiên, bạn nên điều chỉnh nó cho phù hợp với tình huống cá nhân của mình:

---
**Ngày: `Chỉ định ngày`**

Kính gửi `Tên của những người thừa kế`,

Nếu bạn đang đọc bức thư này, tôi không còn ở đây nữa. Trước hết, xin hãy hiểu rằng tôi yêu bạn và đã dành thời gian soạn thảo tài liệu này để giúp bạn quản lý số bitcoin của tôi, vốn có thể có giá trị. Đây không phải là một nhiệm vụ phức tạp, nhưng đòi hỏi sự cẩn thận và thận trọng. Bitcoin là một hệ thống ngang hàng: sẽ không có cách khắc phục nếu xảy ra sai sót. Vui lòng dành thời gian đọc kỹ thư này trước khi thực hiện bất kỳ hành động nào.

#### 1. Liên hệ với những cá nhân đáng tin cậy

Tôi đã chỉ định một số người hỗ trợ bạn hiểu rõ điều khoản Bitcoin và thu hồi tài sản của tôi. Họ không được phép tiếp cận trực tiếp với các khoản tiền nhưng có thể hướng dẫn bạn trong suốt quá trình:

- Anh trai tôi, Bob: +33 6 00 00 00 00; bobmybrother@supermail.com. Bạn có thể tin tưởng Bob sẽ giúp bạn trong quá trình này. Mặc dù anh ấy không phải là người am hiểu kỹ thuật nhất về Bitcoin, nhưng anh ấy là người phù hợp để đặt câu hỏi về mọi thứ và đảm bảo sự thận trọng để đảm bảo thành công an toàn cho bạn.
- Anh họ tôi, Nathan: +33 6 00 00 00 00; nathandelacroix@supermail.com. Nathan đã giới thiệu tôi với Bitcoin. Anh ấy rất giỏi về máy tính và có thể trả lời hầu hết các câu hỏi của bạn. Anh ấy cũng sở hữu một chiếc Bitcoin và có thể hướng dẫn bạn về mặt kỹ thuật. Bạn đã gặp anh ấy trong các buổi họp mặt gia đình, và tôi đã đính kèm một bức ảnh của anh ấy ở đây.
- Ricco: @RiccoSuperBitcoiner trên Twitter; ricco425@supermail.com. Tôi đã làm việc chặt chẽ với Ricco nhiều năm. Bạn chưa từng gặp anh ấy, vì vậy hãy đảm bảo bạn liên hệ đúng người bằng cách hỏi anh ấy "Tên con chó của Cédric là gì?". Nếu anh ấy trả lời "12", thì đó chính là anh ấy. Ricco là một chuyên gia Bitcoin rất thân thiện và giàu kinh nghiệm. Anh ấy sẽ giải đáp mọi thắc mắc của bạn và bạn có thể tin tưởng vào đánh giá của anh ấy về bảo mật Bitcoin. Đừng ngần ngại liên hệ với anh ấy, nhưng đừng bao giờ cho anh ấy quyền truy cập trực tiếp vào tiền của bạn.

Nghe có vẻ lạ, nhưng hãy liên hệ với tất cả họ. Ngoài ra, bạn có thể tìm hiểu thêm bằng cách nghe Andreas Antonopoulos trên YouTube và mua cuốn sách *Cryptoasset Inheritance Planning* của Pamela Morgan.

Hãy liên hệ với tất cả những người này và kiểm tra lại ý kiến của họ trước khi đưa ra quyết định quan trọng. **Đừng tin tưởng bất kỳ ai một cách mù quáng.**

#### 2. Bitcoin của tôi ở đâu?

Sau đây là danh mục chi tiết về tài sản của tôi, vị trí của chúng và cách tiếp cận chúng.

- Tôi dùng điện thoại (Samsung Galaxy S8) để truy cập vào chiếc Samourai Wallet của mình. Cụm từ khôi phục cho chiếc Wallet này được lưu thành hai bản: một bản lưu trong két ngân hàng, bản còn lại do chú Bob giữ. Mã PIN cần thiết để mở khóa cả điện thoại và Wallet được tôi giữ ở nhà và bà tôi.
- Tôi truy cập nền tảng giao dịch trực tuyến Bitfinex thông qua máy tính xách tay Dell 2018 của mình. Có thể vẫn còn bitcoin hoặc đô la. Để lấy lại số tiền này, bạn cần liên hệ trực tiếp với Bitfinex hoặc thử truy cập vào tài khoản của tôi. (**Hãy cẩn thận, việc này có thể là bất hợp pháp—hãy kiểm tra luật pháp địa phương**).
- Tôi đã bảo mật quyền truy cập vào tài khoản trực tuyến của mình bằng trình quản lý mật khẩu. Bản sao lưu của trình quản lý này được lưu trữ trong két ngân hàng. Tài khoản Bitfinex của tôi cũng được bảo vệ bằng xác thực hai yếu tố, có thể truy cập qua điện thoại (Samsung Galaxy S8, ứng dụng Authy) hoặc bằng mã sao lưu tôi giữ ở nhà.
- Tôi sở hữu một ví Trezor Model One Hardware Wallet, truy cập qua máy tính bằng phần mềm Sparrow wallet. Mã khôi phục của ví Wallet này được lưu trữ thành hai bản: một bản tại két ngân hàng, bản còn lại do chú Bob giữ. Mã PIN của ví Wallet này được lưu trữ tại nhà tôi và mẹ tôi. Bản thân thiết bị có thể được cất giữ trong két sắt tại văn phòng tôi. Tôi sử dụng ví BIP39 passphrase trên ví Trezor Model One của mình. Ví passphrase này rất quan trọng để truy cập tiền trong ví. Bản sao lưu của ví passphrase này được lưu trữ ở hai nơi: tại nhà tôi và tại nhà mẹ tôi.

Hãy thực hiện tất cả các biện pháp phòng ngừa cần thiết trước khi xử lý số tiền này. Không bao giờ tiết lộ toàn bộ cụm từ khôi phục cho một người và chỉ chia sẻ những thông tin này nếu thực sự cần thiết.

#### 3. Các biện pháp phòng ngừa và hướng dẫn an toàn

- **Đừng vội vàng**. Hãy dành thời gian tìm hiểu trước khi hành động. Bitcoin an toàn khi sử dụng đúng cách.
- **Đừng bao giờ đưa câu 24 từ cho chỉ một người**. Nếu ai đó yêu cầu bạn cấp quyền truy cập đầy đủ, hãy cẩn thận.
- **Không kết nối ví của tôi với máy tính không xác định**. Hãy sử dụng môi trường an toàn, ngoại tuyến nếu có thể.
- **Hãy cẩn thận với lừa đảo**. Có rất nhiều chiêu trò lừa đảo liên quan đến Bitcoin. Chỉ tin tưởng những người được đề cập trong thư này.
- **Lưu mọi thứ bạn làm**. Ghi chú, ghi lại các bước quan trọng và bảo vệ quyền truy cập.

#### 4. Thông tin pháp lý và thông điệp cuối cùng

Bạn sẽ tìm thấy bản sao di chúc của tôi trong hồ sơ cá nhân và tại văn phòng công chứng của tôi. Thư này không thay thế di chúc chính thức, nhưng sẽ hướng dẫn bạn cách quản lý bitcoin của tôi.

Hãy chăm sóc bản thân và nhớ rằng tôi yêu bạn. Mong muốn của tôi là những tài sản này sẽ hữu ích cho bạn và giúp bạn tiến về phía trước với sự bình yên trong tâm hồn.

Chữ ký viết tay

Tên của bạn

Ngày

---
Đây là một ví dụ cụ thể về thư thừa kế. Hãy chắc chắn điều chỉnh nó cho phù hợp với hoàn cảnh cá nhân của bạn. Việc quyết định mức độ chi tiết cần đưa vào quy trình thu hồi cũng rất quan trọng. Nếu bạn lo ngại người thừa kế sẽ không thể thu hồi bitcoin của bạn với thông tin tối thiểu này, hãy cân nhắc viết một hướng dẫn từng bước giải thích chi tiết quy trình.

Hơn nữa, bạn cần cân nhắc lượng thông tin tiết lộ trong kế hoạch thừa kế này. Bạn càng chia sẻ nhiều về các phương pháp bảo mật của mình, bạn càng có nguy cơ bị xâm phạm an ninh khi còn sống nếu lá thư này rơi vào tay kẻ xấu. Ví dụ, một tên trộm đọc được lá thư này sẽ ngay lập tức biết phải tìm thông tin quan trọng về Wallet ở đâu. Mặt khác, nếu bạn chia sẻ quá ít, bạn sẽ khiến người thừa kế của mình có nguy cơ không thể truy cập vào bitcoin của bạn. Do đó, bạn phải tìm một giải pháp trung dung.

Cuối cùng, tình thế tiến thoái lưỡng nan trong kế hoạch thừa kế phản ánh các chiến lược bảo mật: nó luôn xoay quanh việc cân bằng giữa rủi ro mất mát và rủi ro bị trộm cắp. Việc không ghi chép đầy đủ thông tin sẽ làm tăng nguy cơ người thừa kế của bạn mất quyền truy cập vào bitcoin, trong khi việc chia sẻ quá nhiều thông tin có thể làm tăng nguy cơ bị trộm cắp ngay lúc này.

### Bước 4: Xem lại và lưu trữ thư của bạn

Trước khi kết thúc, hãy đảm bảo rằng bức thư của bạn bao gồm:

- Danh sách rõ ràng những cá nhân đáng tin cậy cùng thông tin liên lạc của họ
- Một bản kê khai chi tiết về tài sản của bạn và nơi tìm thấy chúng
- Hướng dẫn chính xác về quyền truy cập và bảo mật
- Một thông điệp chu đáo và đầy lòng trắc ẩn gửi đến những người thừa kế của bạn

Tùy thuộc vào hoàn cảnh cá nhân, hãy tạo nhiều bản sao viết tay của lá thư này và lưu trữ chúng ở những nơi an toàn (két sắt, nhờ người đáng tin cậy, v.v.). Hãy thông báo cho những người thừa kế của bạn về sự tồn tại của kế hoạch thừa kế này, mà không nhất thiết phải tiết lộ nội dung ngay lập tức. Để tăng cường bảo mật, bạn có thể muốn đặt lá thư trong một phong bì mờ chống giả mạo để đảm bảo rằng nó không bị truy cập trái phép.

![image](assets/vi/096.webp)

Nếu cần, hãy tham khảo ý kiến luật sư hoặc công chứng viên để chính thức đưa kế hoạch của bạn vào khuôn khổ pháp lý.

**Xin chúc mừng!** Bạn đã hoàn thành một bước quan trọng trong việc bảo vệ tài sản Bitcoin của mình. Giờ đây, bạn có thể tinh chỉnh kế hoạch thừa kế của mình bằng cách xem xét định kỳ và khám phá các giải pháp nâng cao hơn như giảm thiểu thiết lập Multisig bằng khóa thời gian:

https://planb.academy/tutorials/wallet/desktop/liana-306ef457-700c-4fdd-b07a-8fb7a8a29f04

Tài liệu tham khảo: [*Lập kế hoạch thừa kế tài sản tiền điện tử, Hướng dẫn đơn giản dành cho chủ sở hữu* - Pamela Morgan, 2018.](https://www.amazon.com/gp/product/1947910116/)

Xin chân thành cảm ơn [Pamela Morgan](https://x.com/pamelawjd) vì công sức của cô ấy và đã cho phép nội dung này được chuyển thể.

## Bây giờ bạn nằm trong số 0,1% người dùng Bitcoin có sự chuẩn bị tốt nhất

<chapterId>5f4cfab9-9af1-584b-a1fe-a0769a991f19</chapterId>

:::video id=8c9d0e1f-2a3b-4c5d-6e7f-8a9b0c1d2e3f:::

:::id video=8c9d0e1f-2a3b-4c5d-6e7f-8a9b0c1d2e3f:::

Nếu bạn đã đến được đây, giờ bạn đã là một phần của nhóm thiểu số được khai sáng! Rất ít người đạt được thành tựu này với Bitcoin, và bạn có thể tự hào về chặng đường mình đã đi. Có lẽ bạn đã làm theo tất cả các hướng dẫn được cung cấp trong khóa đào tạo này: Thiết lập Hardware Wallet đầu tiên, Đảm bảo cụm từ khôi phục, Tạo kế hoạch trung bình hóa chi phí đô la (DCA), Nhận Bitcoin có và không có KYC, Thậm chí đã lên kế hoạch cho việc thừa kế Bitcoin của bạn. Mỗi bước bạn thực hiện là một bước tiến tới chủ quyền tài chính thực sự.

Ngay cả khi bạn chưa làm tất cả, nhưng bạn quyết tâm thực hiện sớm, bạn đã đi trước một bước rồi. Quyết tâm học hỏi và hành động mới thực sự quan trọng. Giờ đây, bạn đã hiểu tại sao Bitcoin lại là một bước chuyển quan trọng cho tương lai của mỗi cá nhân và cộng đồng chúng ta.

Kể từ khi tôi bắt đầu truyền đạt kiến thức về Bitcoin cho mọi người, cả hệ sinh thái lẫn tôi đều đã phát triển đáng kể. Khóa học này đã được cập nhật toàn diện để phản ánh những gì Bitcoin mang lại ngày hôm nay. Quả là một hành trình tuyệt vời!

![image](assets/vi/097.webp)

### Bước tiếp theo của Bitcoin là gì?

Nếu nhìn vào đường cong giá logarit, có một điều trở nên rõ ràng: chúng ta không bao giờ biết chính xác mình đang ở đâu trong chu kỳ. Có lẽ bạn đang đọc khóa học này ngay khi một giai đoạn tăng trưởng mới bắt đầu, hoặc có lẽ bạn đang ở giữa một thị trường giá xuống. Dù trường hợp nào đi nữa, các chu kỳ vẫn tiếp diễn, chắc chắn sẽ được đánh dấu bằng những đỉnh cao ngoạn mục và đôi khi là những đợt điều chỉnh tàn khốc.

Nhưng ngoài giá cả, tốc độ tăng trưởng của Bitcoin còn đáng kinh ngạc hơn. Khởi đầu là một dự án ít người biết đến trong giới cypherpunk, giờ đây Bitcoin đã trở thành một phong trào toàn cầu. Từ Silk Road đến sự chấp nhận của các tổ chức, Bitcoin đã khẳng định vị thế vững chắc trên thị trường tài chính toàn cầu. Nó không còn bị coi là một thử nghiệm nhỏ lẻ nữa, mà đã trở thành một loại tài sản nghiêm túc được các tổ chức và hàng triệu cá nhân trên toàn thế giới đón nhận.

Sự thay đổi nhận thức này đi kèm với những tiến bộ kỹ thuật liên tục. Ví điện tử và cơ sở hạ tầng liên tục được cải thiện. Hàng năm, các giải pháp mới ra đời giúp Bitcoin an toàn hơn, linh hoạt hơn và thân thiện hơn với người dùng. Ngày nay, Bitcoin phi tập trung hơn, dễ tiếp cận hơn và dễ hiểu hơn bao giờ hết.

Tất nhiên, vẫn còn nhiều thách thức: quy định, quyền riêng tư, việc áp dụng rộng rãi hơn và phát triển nền kinh tế tuần hoàn. Nhưng miễn là cộng đồng vẫn năng động, ham học hỏi và gắn kết, Bitcoin sẽ vẫn là một giải pháp thay thế mạnh mẽ cho hệ thống tài chính truyền thống.

Một điều chắc chắn là: Bitcoin sẽ tiếp tục tồn tại; tạo ra một khối mới khoảng 10 phút một lần, không ngừng nghỉ, ngày này qua ngày khác, năm này qua năm khác. Chúng ta sẽ thấy nhiều người dùng, ví, sàn giao dịch hơn, cũng như nhiều quy định, rào cản kỹ thuật, khủng hoảng và đổi mới hơn.

![image](assets/vi/098.webp)

Khi khủng hoảng toàn cầu tiếp diễn và lạm phát làm xói mòn sức mua của tiền tệ fiat, Bitcoin tự nhiên sẽ thu hút thêm nhiều người tham gia, những người tìm kiếm tự do tài chính và chủ quyền cá nhân. Động lực này sẽ tiếp tục củng cố Bitcoin và thu hút thêm nhiều nhân tài mới để cải thiện giao thức, các lớp mở rộng và các ứng dụng hàng ngày của nó về bảo mật, quyền riêng tư và tiện ích.

### Bước tiếp theo của bạn là gì?

Trớ trêu thay, sự thay đổi lớn nhất không phải là Bitcoin mà là chính bạn.

Bạn sẽ phát triển cách sử dụng, hiểu và liên hệ với Bitcoin. Biết đâu bạn sẽ phát triển niềm đam mê thực sự với nó, đủ để bắt đầu dạy người khác, tham gia vào cộng đồng, hoặc đơn giản là giữ gìn tài sản của mình lâu dài. Dù bạn chọn con đường nào, Bitcoin cũng sẽ thay đổi thế giới quan của bạn; và có thể là cả mối quan hệ của bạn với tự do và trách nhiệm tài chính.

Sau đây là một số hướng dẫn bạn có thể thực hiện tiếp theo:

#### 1. Duy trì đà phát triển

Nếu chiến lược hiện tại của bạn hiệu quả, hãy tiếp tục.

Hãy thường xuyên tích trữ Sats và duy trì các biện pháp bảo mật tốt nhất. Bạn đã xây dựng được một nền tảng vững chắc, và không cần phải làm gì thêm nữa nếu mục tiêu chính của bạn chỉ đơn giản là bảo vệ khoản tiết kiệm Bitcoin của mình.

Hãy dành thời gian quan sát sự phát triển của hệ sinh thái, tinh chỉnh chiến lược và củng cố thiết lập của bạn khi cần thiết.

#### 2. Đào sâu kiến thức của bạn

Hành trình của bạn không dừng lại ở đây. Bitcoin là một thế giới không ngừng phát triển, và luôn có nhiều điều để khám phá. Dưới đây là một vài hướng dẫn bạn có thể muốn khám phá để hiểu sâu hơn về Bitcoin:

- Để nâng cao khả năng bảo mật kỹ thuật số của bạn, hãy bắt đầu với khóa học bảo mật cơ bản của chúng tôi:

https://planb.academy/courses/99c46148-7080-4915-a7e0-9df0e145cd47

- Nếu bạn quan tâm đến khía cạnh kinh tế và triết học của Bitcoin, hãy xem các khóa học chuyên sâu sau:

https://planb.academy/courses/d07b092b-fa9a-4dd7-bf94-0453e479c7df

https://planb.academy/courses/caa75343-ac90-4249-bcca-0e2e57c3a0f1

https://planb.academy/courses/9d1bde6a-33e5-45dd-b7c0-94da72e45b11

- Các chủ doanh nghiệp quan tâm đến việc tích hợp Bitcoin vào chiến lược công ty của mình không nên bỏ lỡ:

https://planb.academy/courses/a804c4b6-9ff5-4a29-a530-7d2f5d04bb7a

- Để khám phá câu chuyện hấp dẫn đằng sau sự ra đời của Bitcoin và những năm đầu của hệ thống, tôi thực sự khuyên bạn nên tham gia khóa học này:

https://planb.academy/courses/a51c7ceb-e079-4ac3-bf69-6700b985a082

- Cuối cùng, nếu bạn tò mò về các khía cạnh kỹ thuật của Bitcoin và hệ sinh thái của nó, hãy khám phá các khóa đào tạo sau:

https://planb.academy/courses/34bd43ef-6683-4a5c-b239-7cb1e40a4aeb

https://planb.academy/courses/46b0ced2-9028-4a61-8fbc-3b005ee8d70f

https://planb.academy/courses/65c138b0-4161-4958-bbe3-c12916bc959c

#### 3. Đóng góp vào Hệ sinh thái Bitcoin

Bitcoin không chỉ là một công cụ tài chính; nó là một phong trào. Tùy thuộc vào kỹ năng và sở thích của bạn, có nhiều cách bạn có thể đóng góp:

- **Giáo dục và nâng cao nhận thức**: Giúp những người thân yêu của bạn hiểu về Bitcoin, chia sẻ kiến thức và tham gia vào các cuộc thảo luận tại địa phương;
- **Phát triển các công cụ và ứng dụng**: Nếu bạn có kỹ năng kỹ thuật, đóng góp vào các công cụ nguồn mở là một cách tuyệt vời để mang lại giá trị cho Bitcoin;
- **Tham gia cộng đồng địa phương**: Tham gia các sự kiện, buổi gặp mặt và diễn đàn để giao lưu với những người dùng Bitcoin khác và trao đổi quan điểm về những diễn biến mới nhất.

Nếu bạn muốn giúp đỡ việc giáo dục người khác, Plan ₿ Academy luôn tìm kiếm cộng tác viên! Tất cả nội dung giáo dục của chúng tôi đều là mã nguồn mở và được lưu trữ trên GitHub. Chúng tôi hoan nghênh các cộng tác viên giúp hiệu đính và dịch nội dung sang nhiều ngôn ngữ khác nhau; đổi lại một vài satoshi! Cho dù bạn đang tham gia vào hành trình Bitcoin của mình bây giờ hay sau này, bạn được mời tham gia [nhóm Telegram dành cho cộng tác viên](https://t.me/PlanBNetwork_ContentBuilder) của Plan ₿ Academy.

### Lời cuối cùng

Cảm ơn bạn rất nhiều vì đã tham gia khóa học BTC102.

Tôi tên là Rogzy, người sáng lập Découvre Bitcoin và đồng sáng lập Plan ₿ Academy; một nền tảng được tạo nên bởi các đối tác mới đáng kinh ngạc của chúng tôi, Tether và Fulgur Ventures, và bởi sứ mệnh chung của chúng tôi là giáo dục càng nhiều người càng tốt về Bitcoin.

Phản hồi của bạn vô cùng quý giá. Hãy thoải mái để lại bình luận trên nền tảng và cho chúng tôi biết suy nghĩ của bạn về phiên bản cập nhật này của khóa học. Những đề xuất của bạn sẽ giúp chúng tôi tiếp tục cải thiện chất lượng nội dung.

Bây giờ bạn đã sẵn sàng để làm bài kiểm tra cuối cùng; bài kiểm tra gồm 40 câu hỏi sẽ xác nhận sự hiểu biết của bạn và cấp cho bạn chứng chỉ.

Chúc may mắn!

Tôi hy vọng sớm gặp lại bạn trong một khóa học khác về Plan ₿ Academy; dù là khóa học của tôi hay của một giáo viên tuyệt vời khác trong cộng đồng. Và biết đâu, chúng ta sẽ gặp nhau trực tiếp tại một hội nghị, sự kiện, hoặc buổi gặp mặt Bitcoin.

Một lần nữa, xin chúc mừng bạn đã đi được một chặng đường dài. Và hãy nhớ rằng; mục tiêu chính của tôi sẽ luôn như vậy:

**Giúp càng nhiều người đạt được chủ quyền tài chính thông qua Bitcoin.**

**Chúc bạn mọi điều tốt đẹp nhất trên hành trình cùng Bitcoin!**

# Phần cuối

<partId>a8425389-4a53-4b57-b9b4-36c1cab12de5</partId>

## Đánh giá & Xếp hạng

<chapterId>3f43175a-fb7a-5b1c-a887-7dcf615d7a3a</chapterId>

<isCourseReview>true</isCourseReview>
## Bài kiểm tra cuối kỳ

<chapterId>f3ff8089-8f89-56a8-be8c-b60296b4d91f</chapterId>

<isCourseExam>true</isCourseExam>
## Phần kết luận

<chapterId>b082b8eb-dabc-5d79-94cf-eb8f48fc1968</chapterId>

<isCourseConclusion>true</isCourseConclusion>
