...
Elements 是一個開放原始碼、具備 Sidechain 功能的 [Blockchain](https://planb.academy/resources/glossary/blockchain) 平台，可存取社群成員開發的強大功能，例如 Confidential Transactions 和 Issued Assets。

Elements 的核心是一個協定，它能夠圍繞[交易](https://planb.academy/resources/glossary/transaction-tx)歷史和規則形成[共識](https://planb.academy/resources/glossary/consensus)，這些交易歷史和規則管理著分佈式 Blockchain Ledger 中儲存的資產的轉移和創建。
...
Elements 於 2015 年推出，可降低內部開發及研究成本，並運用最新的 Blockchain 技術，開啟許多新的使用實例。基於 Elements 的 Blockchain 可作為獨立的 Blockchain 運作，或與另一個 Blockchain 掛接並作為 Sidechain 運行。將 Elements 作為 Sidechain 執行，可以在不同[區塊](https://planb.academy/resources/glossary/block)鏈之間可驗證地轉讓資產。
...
Elements 為 Blockchain 使用者每天面臨的問題提供技術解決方案；交易延遲、缺乏隱私以及[可替代性](https://planb.academy/resources/glossary/fungibility)風險。
...
您可以在此閱讀強大聯盟[白皮書](https://planb.academy/resources/glossary/white-paper)： https://blockstream.com/strong-federations.pdf
...
從高層次來看，當有人將 mainchain 資產傳送到由多重簽章 watchmen [Wallet](https://planb.academy/resources/glossary/wallet) 控制的 Address 時，就會發生轉入 Sidechain 的情況。這會有效凍結 mainchain 上的資產。然後 watchmen 會驗證交易，並釋放 Sidechain 內相關資產的相同金額。釋放的資產會傳送到 Sidechain Wallet，該 Wallet 可以證明對原始 mainchain 資產的索賠權。此流程有效地將資產從母鏈移至 Sidechain。
...
Elements [節點](https://planb.academy/resources/glossary/node)軟體本身稱為 `elementsd`，在使用者的機器上以 daemon 的方式執行。daemon (或 Windows 中的服務) 是一個以背景服務方式執行的程式，不需要登入使用者的直接控制。
...
- 必須符合的 `n of m` 門檻，以及可以簽署區塊的有效[公開金鑰](https://planb.academy/resources/glossary/public-key)。
- 需要滿足的[腳本](https://planb.academy/resources/glossary/script)，以便將資產轉入或轉出 Sidechain。
...
即使我們將 e2 的[私人密碼匙](https://planb.academy/resources/glossary/private-key)匯入 e1 的 Wallet，它仍然無法看到交易的資產金額和種類，因為它仍然不知道 e2 所使用的保密密碼匙。我們會將 e2 的 Wallet 所使用的私人密碼匙匯入 e1 的 Wallet 來證明這一點。首先，我們需要從 e2 匯出金鑰
...
這兩個節點位於相同的區域網路，彼此相連，因此在其交易 [Mempool](https://planb.academy/resources/glossary/mempool) 中共享相同的交易，以及相同的區塊鏈。雖然它們運行在同一台機器上，但值得注意的是，它們並不共享相同的實際 Blockchain 檔案。每個節點都管理自己的本地 Blockchain 副本，其中包含相同的交易歷史，因為它們達成共識，彼此遵守相同的協定規則。
...
我們現在需要成熟兩個鏈上的一些區塊。區塊成熟是掛鉤過程的必要條件，因為它可以防止 mainchain 上的區塊重組導致 pegged asset Supply 在 Sidechain 中[膨脹](https://planb.academy/resources/glossary/inflation)。
...
為了讓 Elements 能以 Sidechain 的方式運作，其 Blockchain 中的 [Genesis 區塊](https://planb.academy/resources/glossary/genesis-block)必須以 `fedpegscript` 的方式建立。這可在節點啟動時傳入 `fedpegscript` 參數。該腳本隨後將成為 Elements Blockchain 共識規則的一部分，並允許確認並執行 peg-in 和 peg-out 請求。
