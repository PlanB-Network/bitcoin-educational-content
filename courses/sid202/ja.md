...
*Elements Academy* の目的は、Liquidサイドチェーンが構築されているオープンソースプラットフォーム *Elements* の主要な概念を紹介し説明することです。このコースを終了する頃には、Elements Coreの動作に関与するプロセスだけでなく、機密取引や発行された資産といったElementsの主要な機能についてしっかりと理解できるようになっているはずです。各セクションには解説テキストとビデオを含むレッスンが含まれており、クイズで締めくくられます。

このトレーニングの目的は、Liquidネットワークに焦点を当てて、オープンソースプラットフォームElementsを使用し、開発する方法を学ぶことです。これらの技術がプライバシー、スケーラビリティ、およびプロジェクトの機能をどのように向上させるかを学びます。初心者でも経験豊富な開発者でも、このコースはElementsとLiquidの基本概念およびそれらの実用的な応用を習得するための強固な基盤を提供します。

**セクション 1: 導入**  
Elementsの概念に関する包括的な概要から始めます。このプラットフォームがLiquidのようなサイドチェーンを作成するためのモジュラーで柔軟な基盤を提供するように設計された方法を学びます。実際の応用に入る前に、Elementsの構造を理解することが目的です。

**セクション 2: Elements**  
このセクションでは、Elementsの動作に焦点を当てます。Elementsノードを設定し、スタンドアロンとして操作するか、サイドチェーンとして使用する方法を学びます。

**セクション 3: Elementsの利用 - 実際のユースケース**  
理論的な基礎をマスターした後、Elementsの具体的な応用について学びます。機密取引の実行、資産の発行、および資産の再発行を管理する方法を学びます。

**セクション 4: Elementsの連邦化**  
ここでは、ブロックの連邦署名、サイドチェーンとしてのElementsの利用、独立したブロックチェーンの作成といった高度なメカニズムについて学びます。このセクションは、Elementsベースのブロックチェーンのセキュリティ、完全性、および相互運用性を確保する方法を理解するのに役立ちます。

ElementsとLiquidサイドチェーンの可能性を探求する準備はできましたか？さあ、始めましょう！
...
Elementsはオープンソースのサイドチェーン対応[ブロックチェーン](https://planb.academy/resources/glossary/blockchain)プラットフォームであり、秘密取引や発行資産など、コミュニティのメンバーによって開発された強力な機能へのアクセスを提供する。

エレメンツとは、分散型ブロックチェーン台帳に格納された資産の移転と創造を支配する[取引履歴](https://planb.academy/resources/glossary/transaction-tx)とルールについて、[合意形成](https://planb.academy/resources/glossary/consensus)を可能にするプロトコルを中核とするものである。
...
エレメンツは、[ブロック](https://planb.academy/resources/glossary/block)・サイナーと呼ばれる署名者の強力な連盟を使用しており、彼らは信頼性の高いタイムリーな方法で署名し、ブロックを作成します。これにより、ランダムなポアソン分布によりブロックの時間変動が大きいPoWマイニングプロセスのトランザクション待ち時間がなくなります。Federated Block Signingプロセスは、サードパーティの信頼やアルゴリズムベースのマイニングを導入することなく、信頼性の高いブロック生成を実現します。
...
エレメンツは、ブロックチェーン利用者が日々直面する問題、すなわち取引の遅延、プライバシーの欠如、[換金性](https://planb.academy/resources/glossary/fungibility)のリスクに対する技術的な解決策を提供する。
...
ストロング・フェデレーションズの[ホワイトペーパー](https://planb.academy/resources/glossary/white-paper)はこちらからお読みいただけます： https://blockstream.com/strong-federations.pdf
...
ハイレベルでは、誰かがメインチェーンのアセットをマルチシグネチャーのウォッチメン・[ウォレット](https://planb.academy/resources/glossary/wallet)が管理するアドレスに送ると、サイドチェーンへの送金が発生する。これによってメインチェーンのアセットが効果的に凍結される。その後、ウォッチメンはトランザクションを検証し、サイドチェーン内で関連アセットと同額をリリースする。放出されたアセットは、元のメインチェーンアセットへの請求権を証明できるサイドチェーンウォレットに送られる。このプロセスにより、アセットが効果的に親チェーンからサイドチェーンに移動する。
...
Elementsデーモンはネットワーク上の他の[ノード](https://planb.academy/resources/glossary/node)に接続し、取引とブロックデータを交換し、ネットワークのブロックチェーンのローカルコピーを検証し、拡張することができる。
...
- ブロックに署名できる有効な[公開鍵](https://planb.academy/resources/glossary/public-key)。
- サイドチェーンにアセットを出し入れするために必要な[スクリプト](https://planb.academy/resources/glossary/script)。
...
仮にe2の[秘密鍵](https://planb.academy/resources/glossary/private-key)をe1のウォレットにインポートしたとしても、e2が使っている目隠し鍵のことを知らないため、e1は取引された金額や資産の種類を見ることはできない。e2のウォレットで使われている秘密鍵をe1のウォレットにインポートすることで、このことを証明しよう。まず、e2から鍵をエクスポートする必要があります。
...
2つのノードは同じローカルネットワーク上にあり、互いに接続されているため、トランザクション[メンプール](https://planb.academy/resources/glossary/mempool)内の同じトランザクションと同一のブロックチェーンを共有している。両者は同じマシン上で稼働しているが、実際のブロックチェーン・ファイルは共有していないことは注目に値する。各ノードはそれぞれブロックチェーンのローカルコピーを管理し、コンセンサスを得て互いに同じプロトコルルールを遵守しているため、同じ取引履歴を含んでいる。
...
ここで、両チェーンのブロックを成熟させる必要がある。ブロックの成熟はペッグプロセスの要件であり、メインチェーン上のブロック再編成がサイドチェーン内のペッグされた資産供給の[インフレ](https://planb.academy/resources/glossary/inflation)につながることを防ぐためである。
...
Elementsをサイドチェーンとして動作させるためには、そのブロックチェーン内の[genesisブロック](https://planb.academy/resources/glossary/genesis-block)が`fedpegscript`を配置した状態で作成されている必要があります。これはノードの起動時に `fedpegscript` パラメータを渡すことで実行される。このスクリプトはElementsブロックチェーンのコンセンサスルールの一部を形成し、ペグイン・ペグアウトのリクエストを検証して処理することを可能にする。
