...
In this configuration, if someone gets their hands on your plaintext mnemonic alone, they won't be able to steal anything without knowing the passphrase (provided, of course, that it's strong enough to withstand a brute-force attack). Conversely, if someone discovers your plaintext passphrase, it will remain unusable without the corresponding mnemonic phrase.
...
- A **passphrase strong**: it must be long, random and contain a wide variety of characters. Its complexity is not a problem for you, since you'll only have to enter it once on the keyboard during initialization, after which it will be transmitted by Seedkeeper;
...
Enter your PIN code to unlock the smartcard, then select the wallet corresponding to your passphrase.
...
- In the `SeedKeeper Functions` menu, you can :
 ...
 - Save or load your descriptors (useful function for Multisig portfolios).
