...
比特幣的前史深深植根於 cypherpunk 運動，以及人類在數位時代追求更大自由的共同努力。通過結合密碼學、去中心化和完整性的原則，比特幣已經不僅僅是一種貨幣。事實上，它是一場哲學和技術革命的產物，並將繼續重塑我們的世界。
...
事實上，任何人都可以通過比特幣節點（即交易驗證器）來查詢比特幣的供應量，只需輸入以下命令即可：`bitcoin-cli gettxoutsetinfo`.這種透明度增強了人們對比特幣系統的信任，比特幣系統不是基於中央機構或個人，而是建立在數學與密碼學的保證之上（您將在 LNP 201 中學習如何輕鬆地做到這一點）。
...
私密金鑰是最基本的，因為它構成了比特幣的所有權，因此也構成了使用或發送比特幣的權利。因此，擁有私鑰的人就擁有了對比特幣的完全支配權。
...
- **第 1 級**，您使用手機或電腦上的比特幣錢包，您是比特幣的唯一持有人，您可以輕鬆地進行交易。上述工具被稱為「熱錢包」，因為私密金鑰被儲存在可以上網的設備上。在這種情況下，備份助記詞是至關重要的，以便在丟失手機或電腦的情況下重新獲得您的資金。
...
人們對比特幣的興趣迅速增長，導致許多人測試 Bitcoin、參與辯論、解決比特幣的 Bug 以及反思比特幣的倫理、經濟和哲學方面。人們被深深吸引，中本聰為了促進這些類型的交鋒，於 2009 年 11 月 22 日創建了 BitcoinTalk 論壇。
...
對於一些人來說，比特幣被定義為像[菌絲體]一樣的真菌實體(https://brandonquittem.com/bitcoin-is-the-mycelium-of-money/)，而另一些人則把它描述為[黑洞](https://dergigi.com/)。無論你愛它還是恨它，比特幣繼續存在著，它每區塊 10 分鐘的恆定節奏，就像一個新的貨幣系統的心跳。
...
之後，Bob 打開他的比特幣錢包並按下 「發送 」來進行交易。然後，他將 Alice 的地址複製並粘貼到必填欄位中，添加他想要發送的金額，並決定交易費用，作為礦工將交易包含在下一個區塊中的獎勵。事實上，Bob 支付的費用越高，他就越有機會將該交易包含在下一個區塊中，並添加到區塊鏈中，即記錄所有比特幣交易的公共和不可變的總帳。
...
在這個階段，交易已經建立，Bob 的錢包將與比特幣網絡分享。要做到這一點，他的錢包將與比特幣網絡的節點通信，該節點將向其他節點傳播此信息。這種過程允許整個網絡看到這個新的交易並將其考慮在內。

![image](assets/en/47.webp)

此時，儘管這筆交易已為所有人所知（透過稱為 Mempool 的工具），但在被礦工插入區塊之前，它仍不能被視為已被確認，而礦工是唯一將交易加入區塊鏈以進行驗證的人。

事實上，礦工的職責是收集有效和未確認的交易，將其編譯成一個區塊。一言以蔽之，他們必須在一個稱為 「工作證明 」的過程中解決加密謎題，才能使他們的區塊成為比特幣區塊鏈中的下一個區塊。

![image](assets/en/48.webp)

### 步驟 3：交易由礦工在區塊中進行挖礦。

工作證明系統需要為有問題的區塊找到一個有效的 「哈希值」：把它想像成與區塊相關的唯一指紋，由 256 個字元組成。哈希值的有效性取決於比特幣網絡的難易度（稍後我們將進一步詳述）。現在，考慮一個礦工找到了一個有效的區塊，Bob 對 Alice 的交易包含在其中。然後，新的有效區塊被添加到區塊鏈，即所有比特幣用戶的公共帳本。
...
比特幣交易系統是分散式的，點對點運作，沒有任何可信賴的中介。

鮑勃將他的交易發送到比特幣網絡，當礦工發佈一個包含鮑勃交易的有效區塊時，愛麗絲就可以開始考慮比特幣屬於她。在比特幣所有權轉移的任何步驟中都不需要信任：僅憑協定規則和經濟誘因，在比特幣系統中進行惡意行為的成本高得令人望而卻步。

事實上，用戶通過使用自己的私人鑰匙對交易進行數字簽名來轉移資金的所有權。另一方面，礦工持有有限的權力，用戶通過使用比特幣節點來驗證新區塊和包含的交易，從而保持重要的控制權。每個節點都有一個完整或部分的帳本副本，因此比特幣節點形成的網絡使該系統真正去中心化。
...
協定內的治理超出了本基礎課程的範圍，但重要的是要注意，每個運行比特幣節點的用戶都可以決定遵循哪些規則。用戶可以選擇遵守不同的規則（即對代碼進行修改），但如果這些修改使當前的共識規則失效，則該節點將不再是比特幣網絡的一部分。因此，重大修改非常罕見，需要數以千計具有不同意識形態和利益的參與者進行大量協調，這迫使他們提供被所有比特幣用戶認為 「更好 」的更新。

### 節點是什麼樣子？

當您想安裝自己的節點時，有多種選擇，維護成本也不同。您可以簡單地在您的電腦上運行比特幣Core 軟體，但這將需要大量的儲存空間，因為區塊鏈大約 ~500GB。為了克服這個限制，您可以選擇通過創建一個 「修剪節點」，只在記憶體中保留最後的 N 個區塊。對於第二種解決方案，成本可以忽略不计，因為節點只在您需要時才會啟動。
...
比特幣 節點在硬體資源、儲存和頻寬方面的可負擔成本和可存取性是一個非常重要的特徵，因為它促進了網路的去中心化。

事實上，每個人都有運行節點的好理由！與所獲得的利益相比，所付出的成本和努力是微不足道的。您只需要開始冒險，加入成千上萬的其他 bitcoiners 一起組成比特幣網絡。

![image](assets/en/54.webp)

相反，如果區塊大100倍，我們當然可以每10分鐘進行100倍的交易，但是運行一個比特幣節點需要50TB的硬盤，超過500GB/月的帶寬，以及一個能夠在10分鐘內驗證成百上千筆交易的硬件。在這種區塊大 100 倍的假設情況下，一般人是無法運行比特幣節點的，這將會損害協定的分散性以及交易和共識規則的不可變性。
...
**Miners 保護網路安全並將交易加入區塊。他們通過 ASIC 機器使用電力來解決比特幣的工作量證明**。

![image](assets/en/55.webp)

### 工作證明說明

工作證明"（POW）是比特幣協議的安全共識機制。它是一切的基礎，在比特幣的博弈理論中扮演著重要的角色。
...
**遵循比特幣的格言之一：不要相信，自己驗證！**

### 比特幣技術特點簡介

中本聰于 2009 年 1 月發佈了比特幣協定，由於開發人員、礦工和擁有比特幣節點的用戶組成的社區不斷成長，比特幣協定自此不斷發展。比特幣區塊鏈是所有比特幣交易的公共帳簿，這些節點通過保存它們自己的副本，可以根據比特幣的共識規則確保交易的有效性。這包括確保礦工生成有效的區塊，其中包含數以千計的待定交易。
...
因此，比特幣協定是一個不可檢查和不可阻擋的貨幣系統，因為協定的每個組成部分在地理上分佈在全球各地。例如，各大洲有超過 40,000 個比特幣節點。比特幣的共識規則是這樣的：遵守規則比試圖破壞規則在經濟上更有利可圖，因此參與者之間不需要信任。比特幣沒有領導者，也無法被阻止。即使可以通過監管交易所平台來限制比特幣，這種方法對系統的影響也是微乎其微的。簡而言之，沒有法官或國家可以審查或阻止比特幣。
...
之後，比特幣的使用擴展到像 Silk Road 這樣的暗網市場，主要是因為它的不可控性和匿名性，也吸引了該平台之外的個人，包括一些從事犯罪活動的人。然而，需要強調的是，決定合法性的是工具的應用，而非工具本身。非法使用比特幣本身不會使某人成為罪犯；相反，是特定的行為可以被歸類為非法。例如，使用比特币购买某些毒品可能是合法的，也可能是非法的，这取决于交易发生地的法规。
...
今天，比特幣繼續生存和成長，越來越多地融入傳統市場，比特幣 ETFs 的推出，更清晰的法規，更完善的獲取和儲存工具，都在為這種積極的勢頭做出貢獻。比特幣再一次從投機泡沫中生存了下來，所以也許這並不是空穴來風！
...
現在我們已經有了比特幣協定的基本知識，我們將介紹一個使用比特幣協定來實現閃電式快速交易的支付網路：閃電網絡！
...
Lightning Network 於 2017 年通過驗證並實施，以解決比特幣的可擴展性問題，因為它可以實現即時、低成本的比特幣交易。

可擴展性問題指的是實施一個能夠提供每秒不斷增加的交易數量的貨幣系統，以面對越來越多的採用所帶來的挑戰。這個問題與區塊鏈的三難關係。想像一個三角形，三角形的頂點是去中心化、安全性和可擴展性。

![image](assets/en/79.webp)

根據它，基於區塊鏈的協定只能滿足這三個特性中的兩個。在比特幣協定中，開發人員做出了有利於分散性和安全性的選擇。一方面，1MB 的區塊大小和兩個區塊之間的時間（平均 10 分鐘）允許以較低的成本運行一個比特幣節點，有利於去中心化。另一方面，通過工作證明（Proof-of-Work）產生區塊使得協定內的欺詐成本極高，同時方便了網絡節點的驗證，有利於安全性。然而，這些選擇對一個區塊中的平均交易數量施加了限制，大致相等於每秒幾筆交易。與 VISA（1700/秒）等支付處理器的計算能力相比，這個數字簡直是荒謬，但為了以抗審查和無信任的方式使用比特幣進行交易，這個限制是必要的。儘管如此，比特幣開發者從一開始就在思考這個問題。

### 閃電作為頂層

經過多年的考慮和多次嘗試，閃電協定出現了。此協定使用一定數量的規格，利用比特幣交易協定的安全性和可編程性，建構出點對點的支付網路。Lightning Network 充當了支付通道網絡的角色，能以低廉的費用實現發送人的即時交易，它甚至能在沒有直接通道連接的個人之間建立交換路徑。
...
世界上有 24 億人沒有銀行賬戶，這必然造成地理上的不平等。另一方面，比特幣賦予交易平等，無視您的社會地位或政治立場。比特幣是非政治性的，不會賦予領導者或其他有影響力的人物特定的特權，確保每個人都有相同的機會推動發展，而不是讓少數人站在頂端，其他人落在後面。是否每個人都應該使用相同的貨幣，而不論其社會地位如何？我們必須考慮我們希望留給子女的世界，我們渴望創造一個開放的世界，讓他們可以自由選擇管理自己的金錢。
