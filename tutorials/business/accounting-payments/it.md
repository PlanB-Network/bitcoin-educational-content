...
In alcune aree del mondo sono nate comunità che potremmo definire _circolari_, cioè dove bitcoin è sia accettato che impiegato diffusamente dalla popolazione per pagare presso le attività commerciali. Addirittura gli stessi commercianti, che ricevono pagamenti dai loro clienti, potrebbero accordarsi con i propri fornitori per pagare in bitcoin la merce per il proprio negozio.

## Accettare Bitcoin

Il prossimo passo è accettare bitcoin direttamente nel proprio negozio; per fare ciò è necessario configurare un wallet Lightning Network (di seguito: "LN") per ricevere pagamenti instantanei, permettendo al tempo stesso al cliente di pagare ridotte commissioni di transazione.
...
Dopo aver seguito la guida sarai pronto per accettare bitcoin metodo di pagamento: davanti al cliente che vorrà pagare in bitcoin ti basterà aprire l'app da mobile e, dopo aver cliccato sul bottone `Ricevi`, inserire la quantità (generalmente nella tua valuta locale) per generare una fattura pagabile dall'utente.
...
Una configurazione leggermente più avanzata è usare la funzione Point of Sale di Wallet of Satoshi, accessibile dall'app cliccando sul menù ad hamburger in alto a destra:

![image](assets/en/01.webp) → schermata WoS principale in EN

clicca poi sul bottone `Point of Sale`

![image](assets/en/02.webp) → schermata WoS del menù sulla dx in EN

Questo è uno dei due modi per accedere alla funzione Point of Sale di Wallet of Satoshi, il secondo metodo è tramite l'app dedicata [Wallet of Satoshi PoS](https://www.walletofsatoshi.com/pos). Il vantaggio dell'app dedicata è la funzionalità di sola ricezione, quindi perfetta per lasciare i propri dipendenti collezionare i pagamenti dei clenti.

Presto sarà qui inserito il link al tutorial per Wallet of Satoshi - Point of Sale!
...
Non sarà quindi necessario investire ulteriori tempo e denaro con il tuo consulente fiscale per tenere traccia di questi introiti, dal momento che sono uguali a tutti gli altri!
...
Un altro problema che potrebbe affliggervi - ad esempio presente in alcuni paesi europei - è che la tipologia della vostra impresa potrebbe impedirvi di accettare il contante come metodo di pagamento per i beni e servizi erogati ai vostri clienti. In tal caso il metodo esposto non sarà più sufficiente.
...
**Attenzione**: compilare il campo `Note` quando si crea la fattura LN per il cliente facilita di molto l'aggiornamento della contabilità in bitcoin, stesso discorso per quando si effettua un pagamento LN verso i propri fornitori. Idealmente, ad ogni transazione è associata una nota di pagamento.
