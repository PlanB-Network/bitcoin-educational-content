---
name: SeedSigner
description: Een Hardware Wallet die doe-het-zelf, stateloos, betaalbaar en volledig air-gapped is
---
...
De SeedSigner is een open-source Hardware Wallet Bitcoin die iedereen zelf kan bouwen met goedkope elektronische componenten voor algemeen gebruik. In tegenstelling tot commerciële producten zoals de Ledger, Coldcard of Trezor, is dit geen kant-en-klaar apparaat dat door een bedrijf gemaakt wordt: het is een gemeenschapsproject waarmee iedereen zijn eigen apparaat kan maken en elke stap kan controleren.



De SeedSigner is ontworpen om 100% ***air-gapped*** te zijn: het maakt nooit verbinding met het internet, heeft geen Wi-Fi of Bluetooth (in het geval van de Raspberry Pi Zero v1.3) en is nooit verbonden met een computer om Exchange gegevens te verzamelen. De communicatie verloopt uitsluitend via een Exchange systeem met QR-code. Concreet geeft je portfoliobeheersoftware (zoals Sparrow wallet) de te ondertekenen transactie weer in de vorm van QR-codes; je scant ze met de camera van de SeedSigner, waarna het apparaat de transactie ondertekent met behulp van je privésleutels die tijdelijk in zijn RAM zijn opgeslagen. Tot slot genereert het QR-codes die de ondertekende transactie bevatten, die je scant met je software om het naar het Bitcoin netwerk te sturen.
...
- Raspberry Pi Zero
 ...
 - Vergeet niet een micro-USB voeding Supply mee te nemen.
...
- Waveshare 1,3" scherm (240×240 px)** (in het Frans)
    ...
    - Het scherm bevat drie knoppen en een mini-joystick die dienst doet als Interface van de gebruiker.
...
- Huisvesting (optioneel, maar aanbevolen)** (optioneel, maar aanbevolen)** (optioneel, maar aanbevolen)** (optioneel, maar aanbevolen)** (aanbevolen)
    - Beschermt het apparaat en maakt het gebruiksvriendelijk.
    - Het populairste model is de "*Orange Pill Case*", waarvoor [open-source STL-bestanden beschikbaar zijn voor 3D-printen] (https://github.com/SeedSigner/seedsigner/tree/dev/enclosureshttps://github.com/SeedSigner/seedsigner/tree/dev/enclosures).
...
Je kunt deze componenten apart kopen of, voor meer eenvoud, kiezen voor kant-en-klare pakketten die alle benodigde hardware bevatten. Persoonlijk bestelde ik mijn pakket [van deze Franse site](https://bitcoinbazar.fr/), maar je vindt ook een lijst van verkopers voor elke regio van de wereld op [de SeedSigner project hardware pagina](https://seedsigner.com/hardware/). Als je de componenten liever afzonderlijk koopt, zijn ze verkrijgbaar op de grote e-commerce platforms of in gespecialiseerde winkels.
...
PowerShell berekent dan de Hash SHA256 van je image-bestand. Vergelijk deze Hash met de corresponderende waarde in `seedsigner.0.8.6.sha256.txt`.
...
Als je de oriëntatie van de camera hebt gewijzigd of andere instellingen (zoals de taal Interface) op een later tijdstip wilt wijzigen, moet je de instellingen op de microSD laten behouden. Anders zullen je instellingen bij elke herstart terugvallen op de standaardinstellingen, aangezien de Raspberry Pi Zero geen persistent geheugen heeft.
...
Voor je je Bitcoin Wallet aanmaakt, moeten we de SeedSigner configureren. Omdat het draait op een Raspberry Pi Zero zonder permanent geheugen, worden de instellingen niet automatisch opgeslagen, tenzij je ze opslaat op de microSD-kaart. Zorg er dus voor dat je deze optie hebt ingeschakeld, anders gaan deze instellingen verloren bij het herstarten (zie stap 3.5).
...
De huidige firmwareversie ondersteunt verschillende softwarepakketten: Sparrow, Specter, BlueWallet, Nunchuk en Keeper. In mijn geval gebruik ik **Sparrow wallet**, die ik vooral aanbeveel vanwege zijn eenvoud en rijke functionaliteit.
...
- gW-20 (satoshis, of 1/100.000.000 BTC)



De **Sats** unit is over het algemeen het meest praktisch voor kleine hoeveelheden.
...
- gW-22 network`: alleen wijzigen als je de SeedSigner op Testnet wilt gebruiken.
...
- `Script types`: definieert de scripttypes die zijn toegestaan om je bitcoins te vergrendelen. Je hoeft deze parameter niet aan te passen, omdat het scripttype direct wordt ingesteld op Sparrow. Hier gaat het alleen om scripts die de SeedSigner mag manipuleren.
...
Tenslotte kun je in het `Taal` menu de taal van de Interface aanpassen aan je voorkeur.
...
De seed (of Mnemonic zin) vormt de basis van je Bitcoin portefeuille. Het wordt gebruikt om je private sleutels en adressen af te leiden, en geeft toegang tot je fondsen. SeedSigner biedt verschillende methoden om het te genereren, die we in deze sectie zullen verkennen.
...
- Deze zin geeft je volledige, onbeperkte toegang tot al je bitcoins.** Iedereen die in het bezit is van deze zin kan je geld stelen, zelfs zonder fysieke toegang tot je SeedSigner ;
- Gewoonlijk wordt een 12-woorden zin gebruikt om een Wallet te herstellen als een Hardware Wallet verloren of gestolen is. Maar omdat de SeedSigner een *statloos* apparaat is, wordt jouw seed nooit geregistreerd. Dus je fysieke back-ups zijn niet alleen reservekopieën, maar **de enige manier om je Wallet** te gebruiken. Als je deze back-ups verliest, zijn je bitcoins voorgoed verloren. Maak dus zorgvuldig back-ups, op verschillende media en op veilige plaatsen;
- Als u net begint, raad ik u ten zeerste aan deze tutorial te lezen voor een gedetailleerd begrip van de risico's die gepaard gaan met het beheren van een Mnemonic zin:



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### 5.1 Ga naar de seed creatietool



Ga vanuit het startscherm van SeedSigner naar het menu `Tools`.
...
Je generate is nu je seed. Een seed is gewoon een groot willekeurig getal. Hoe willekeuriger het wordt gegenereerd, hoe veiliger het is. SeedSigner biedt twee manieren om dit te doen:




- camera": seed wordt gegenereerd uit de visuele ruis van een foto. Je neemt een afbeelding van een willekeurige omgeving (object, landschap, gezicht, enz.) waarvan de pixelvariaties worden gebruikt om generate entropie te genereren. Het is een snelle methode, maar niet reproduceerbaar.
...
### 5.2 De seed maken met de foto



Als je de fotomethode kiest, klik dan op `Nieuw seed` (met het camerapictogram), maak een foto en valideer. Selecteer vervolgens de lengte van je zin (12 of 24 woorden), die op het scherm verschijnt om op te slaan. De volgende stappen zijn identiek aan deel 5.3.
...
Kies vervolgens de lengte van je Mnemonic zin. 12 woorden bieden al voldoende beveiliging, dus dit is de keuze die ik aanraad.
...
Schrijf de woorden van je Mnemonic zin zorgvuldig op een geschikte fysieke drager (papier of metaal).
...
Als je een fout maakt, zal de SeedSigner je dat laten weten en moet je opnieuw beginnen, waarbij je ervoor moet zorgen dat je je Mnemonic-zin noteert wanneer je die krijgt. Deze verificatiestap zorgt ervoor dat uw back-up correct en volledig is. Eenmaal gevalideerd, zal op het scherm `Backup Geverifieerd` verschijnen.
...
De SeedSigner is een apparaat zonder permanent geheugen. Dit betekent dat uw seed nooit in het apparaat wordt opgeslagen (in tegenstelling tot bijvoorbeeld een Ledger, Trezor of Coldcard). Zodra u de stroom uitschakelt, verdwijnt de seed volledig uit het RAM-geheugen. Wanneer de SeedSigner opnieuw opstart, keert hij terug naar een lege toestand: je moet hem opnieuw je seed geven om je transacties te ondertekenen.



Dit biedt essentiële bescherming. In tegenstelling tot andere hardware wallets, is SeedSigner gebaseerd op een Raspberry Pi Zero zonder fysieke bescherming, inclusief *secure element*. Maar omdat er geen gevoelige gegevens worden opgeslagen, zou zelfs een fysiek gecompromitteerd apparaat een aanvaller niet in staat stellen om je privésleutels te ontfutselen of je bitcoins uit te geven.
...
Ik raad je ook aan om je QR-code en je Mnemonic-zin op twee aparte locaties te bewaren, om te voorkomen dat je alles kwijtraakt bij een claim.



Een geavanceerder en veiliger alternatief is het gebruik van de SeedSigner met een **SeedKeeper**, die de seed in een secure element opslaat. Voor meer informatie, zie deze tutorial:
...
Zodra de verificatie is voltooid, toont SeedSigner de vingerafdruk van de hoofdsleutel van je Wallet. Deze vingerafdruk identificeert je Wallet en zorgt ervoor dat je in de toekomst de juiste herstelzin gebruikt. Het geeft geen informatie over je privésleutels, dus je kunt het veilig opslaan op een digitaal medium. Zorg er wel voor dat je een toegankelijke kopie bewaart en deze nooit kwijtraakt.
...
Het is ook in dit stadium dat je een **passphrase BIP39** kunt toevoegen om de veiligheid van je Wallet te versterken. Deze optie kan de moeite waard zijn, afhankelijk van je back-up strategie, maar het brengt ook risico's met zich mee: als je de passphrase verliest, ben je de toegang tot je bitcoins voorgoed kwijt.



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

Als je nog niet bekend bent met het passphrase concept, nodig ik je uit om deze uitgebreide tutorial over het onderwerp te lezen:
...
### 5.8 De seed opslaan in QR formaat (*SeedQR*)



Met de SeedSigner kun je je seed omzetten in een papieren QR code, genaamd *SeedQR*. Deze methode vereenvoudigt het herladen van je Wallet, omdat je niet elk woord handmatig hoeft over te typen.



Hiervoor heb je een blanco papieren of metalen QR code nodig die overeenkomt met de lengte van jouw Mnemonic zin. Als je een compleet pakket voor je SeedSigner hebt gekocht, zijn de sjablonen meestal inbegrepen. Zo niet, dan kun je ze hier downloaden en afdrukken (of met de hand reproduceren):
...
Selecteer `Backup seed` op uw seed scherm.
...
Reproduceer zorgvuldig elke zwarte stip op je saveblad en gebruik dan de joystick om naar de volgende BLOCK te gaan. Neem de tijd: een simpele verkeerde uitlijning kan de QR-code onbruikbaar maken.
...
Gefeliciteerd, je Bitcoin portfolio is nu klaar voor gebruik! We gaan nu de publieke componenten ervan importeren in **Sparrow wallet** om het gemakkelijk te beheren.
...
Zodra je SeedSigner is ingesteld en je seed correct is aangemaakt en opgeslagen, is de volgende stap om deze portefeuille te koppelen aan beheersoftware zoals Sparrow wallet. Uw seed zal altijd offline blijven, omdat alleen het openbare deel van uw portefeuille naar Sparrow wordt verzonden. Hierdoor kan de software je adressen en transacties weergeven en nieuwe transacties opbouwen, zonder ooit je bitcoins te kunnen uitgeven. Om je bitcoins uit te geven, zal je SeedSigner altijd de transactie moeten ondertekenen die door Sparrow is voorbereid.
...
Plaats de microSD met het besturingssysteem, zet je SeedSigner aan en laad de seed die je net hebt gemaakt van je back-up QR code. Selecteer op het beginscherm `Scan` en scan je SeedQR met de SeedSigner.
...
Controleer of de vingerafdruk op je hoofdsleutel overeenkomt met de vingerafdruk op je Wallet. Als je een passphrase gebruikt, voer deze dan in dit stadium in.
...
Er verschijnt een waarschuwing. Dit is normaal: met deze uitgebreide publieke sleutel (`xpub`) kunt u alle adressen bekijken die zijn afgeleid van uw seed (op de eerste rekening). Je kunt er geen geld mee uitgeven, maar het laat wel de structuur van je portefeuille zien. Als het ooit uitlekt, is dat een probleem voor je privacy, maar niet voor de veiligheid van je bitcoins: je kunt ze zien, maar niet uitgeven.
...
Zorg ervoor dat de Sparrow wallet software op je computer is geïnstalleerd. Als je niet weet hoe je de software moet downloaden, controleren en installeren, raadpleeg dan onze volledige handleiding over dit onderwerp:
...
Uw SeedSigner en Sparrow wallet zijn nu veilig met elkaar verbonden. Sparrow fungeert als een compleet beheer Interface, terwijl de SeedSigner het enige apparaat blijft dat uw transacties kan ondertekenen. U bent nu klaar om bitcoins te ontvangen en te verzenden in een volledig air-gapped configuratie.
...
Je SeedSigner en Sparrow wallet zijn nu geconfigureerd om samen te werken. In deze laatste sectie bekijken we hoe je bitcoins kunt ontvangen en versturen met deze configuratie.
...
#### 7.1.1 Een ontvangst genereren Address



Open Sparrow wallet op je computer en ontgrendel SeedSigner Wallet met je wachtwoord. Zorg ervoor dat de software verbonden is met een server (inkeping rechtsonder). Klik in de zijbalk op `Ontvangen`.
...
Een nieuwe Bitcoin Address wordt weergegeven. U ziet :




- De tekst Address (beginnend met `bc1p...` als je P2TR gebruikt zoals ik),
- De bijbehorende QR-code,
- Een `Label` veld voor het bijhouden van je transacties.



Ik raad u ten zeerste aan een etiket toe te voegen aan elk Bitcoin ontvangstbewijs op uw Wallet. Zo kunt u eenvoudig de herkomst van elke UTXO identificeren en uw privacybeheer verbeteren. Om dieper op dit belangrijke onderwerp in te gaan, kunt u de speciale training op Plan ₿ Academy bekijken:
...
Uw Address is nu gekoppeld aan dit label in alle Sparrow-secties.
...
Voordat u uw ontvangen Address deelt, is het erg belangrijk om te controleren of het bij uw seed hoort. Deze stap zorgt ervoor dat uw SeedSigner transacties kan ondertekenen die met deze Address geassocieerd zijn. Het beschermt ook tegen mogelijke aanvallen waarbij Sparrow een frauduleuze Address weergeeft. Vergeet niet dat Sparrow draait op een onveilige omgeving (jouw computer), die een veel groter aanvalsoppervlak heeft dan jouw SeedSigner, die volledig geïsoleerd is. Daarom moet u nooit blindelings een ontvangen Address geloven, die op Sparrow getoond wordt, totdat u het met uw Hardware Wallet geverifieerd heeft.



Klik op de QR-code van de Address op de Sparrow om deze te vergroten: de code wordt dan schermvullend weergegeven.
...
Kies `scannen` in het hoofdmenu van je SeedSigner. Scan de QR-code op je computerscherm en kies de seed die overeenkomt met je Wallet (in mijn geval de `d4149b27` vingerafdruk).
...
Als de gescande Address overeenkomt met de seed, zal het SeedSigner scherm het bericht weergeven: `Address geverifieerd`.
...
Dit bevestigt dat de Address bij jouw Wallet hoort en dat je er met een gerust hart bitcoins van kunt ontvangen.
...
Je kunt nu deze Address (in de vorm van tekst of QR-code) doorgeven aan de persoon of afdeling die je Satss moet sturen. Zodra de transactie is uitgezonden op het netwerk, verschijnt deze in het tabblad `Transacties` van Sparrow wallet.
...
In Sparrow wallet kun je klikken op de tab `Versturen` in de linker zijbalk. Ik gebruik echter liever het tabblad `UTXO's`, waarmee je "*Coin Controle*" kunt oefenen. Deze methode geeft je nauwkeurige controle over de gebruikte UTXO's, zodat je de informatie die je onthult tijdens de transactie kunt controleren.
...
- Plak in `Betalen aan` de Address van de ontvanger of klik op het camerapictogram om de QR-code te scannen;
- Voeg in `Label` een label toe om deze uitgave bij te houden;
- Voer in `Bedrag` het bedrag in dat moet worden verzonden;
- Selecteer ten slotte het vergoedingstarief op basis van de huidige marktomstandigheden (schattingen zijn beschikbaar op [Mempool.space] (https://Mempool.space/)).
...
Zet je SeedSigner aan en scan je SeedQR om toegang te krijgen tot je portfolio, zoals gewoonlijk. Selecteer op het beginscherm `Scan` en scan de QR-code die op de Sparrow staat.
...
De SeedSigner detecteert automatisch dat dit een PSBT is en geeft een samenvatting van de transactie weer:
...
Klik op `Bekijk Details` en controleer alle informatie direct op het SeedSigner scherm. De belangrijkste items om te controleren zijn het verzonden bedrag, de Address van de ontvanger en het bedrag van eventuele kosten.
...
Als alles correct is, selecteer dan `Approve PSBT` om de transactie te ondertekenen met de bijbehorende private sleutel(s).
...
Nu de transactie geldig is, moet het uitgezonden worden op het Bitcoin netwerk, zodat het een Miner bereikt die het aan een BLOCK zal toevoegen.
...
Je transactie is nu naar het Bitcoin netwerk gestuurd. U kunt de voortgang volgen op het tabblad `Transacties` van Sparrow wallet.
