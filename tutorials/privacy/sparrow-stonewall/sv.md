...
description: Förstå och använda Stonewall-transaktioner på Sparrow
---
...
Stonewall är en specifik form av Bitcoin-transaktion som är utformad för att öka användarnas sekretess när de spenderar genom att imitera en coinjoin mellan två personer, utan att faktiskt vara en. Faktum är att den här transaktionen inte är ett samarbete. En användare kan bygga den på egen hand och endast använda UTXO:er som tillhör honom som input. Så du kan skapa en Stonewall-transaktion för alla tillfällen utan att behöva synkronisera med en annan användare.
...
Låt oss ta ett exempel för att förstå denna transaktionsstruktur. Alice är hos bagaren för att köpa sin baguette, som kostar 4 000 sats. Hon vill betala i bitcoins, samtidigt som hon upprätthåller någon form av sekretess om sin betalning. Så hon bestämmer sig för att bygga en Stonewall-transaktion för betalningen.
...
Stonewall-strukturen tillför en enorm mängd entropi till transaktionen, vilket suddar ut gränserna för kedjeanalys. Sett utifrån kan en sådan transaktion tolkas som ett litet coinjoin mellan två personer. Men i själva verket är det, precis som Stonewall x2-transaktionen, en betalning. Denna metod skapar därför osäkerheter i kedjeanalysen, eller leder till och med till falska ledtrådar.
...
Denna tolkning är felaktig, eftersom, som ni vet, en UTXO skickades till bagaren, de 2 inkommande UTXO:erna kom från Alices och hon återvann 3 utbytesutgångar.
...
## Hur gör jag en Stonewall-transaktion på Sparrow?



Stonewall-transaktioner utvecklades ursprungligen av Samurai Wallet-teamet och togs över av Ashigaru-applikationen, en fork från den ursprungliga portföljen som skapades efter arresteringen av Samurai-utvecklarna, och även på Sparrow Wallet.



Du måste installera Sparrow och skapa en :



https://planb.academy/tutorials/wallet/desktop/sparrow-c674e2ac-d46f-4c82-92a7-7d1b0e262f5d

Till skillnad från transaktioner med Stowaway eller Stonewall x2 (*cahoots*) kräver Stonewall-transaktioner inte att Paynyms används. De kan utföras direkt, utan några särskilda förberedelser eller samarbete med en annan användare.



För att utföra en Stonewall-transaktion på Sparrow är proceduren mycket enkel: börja med att skapa en transaktion som vanligt, antingen via menyn `Send` eller från menyn `UTXOs` om du vill göra *Coin Control*.
...
Ange sedan transaktionsuppgifterna: mottagarens adress, en etikett, beloppet som ska skickas och beloppet eller avgiften, beroende på marknadsförhållandena.
...
Innan du bekräftar, det är här du kan välja Stonewall-strukturen. Längst ner i gränssnittet ersätter du `Efficiency` med `Privacy`. Om detta alternativ inte visas betyder det att din portfölj inte har ett tillräckligt antal UTXO för att bygga denna typ av transaktion.
...
När du har valt alternativet `Privacy` kommer du att märka att transaktionens struktur är helt förändrad: det blir en Stonewall-transaktion, som konsumerar flera av dina UTXO som input och producerar två output med identiska belopp, varav en motsvarar den faktiska betalningen av `100 000 sats`, utöver utbytesoutputen.
...
Om allt är korrekt klickar du på `Create Transaction`.



Du kan sedan kontrollera transaktionsuppgifterna en sista gång och klicka på `Finalize Transaction for Signing`.
...
Signera sedan transaktionen enligt den metod som är specifik för din portfölj och klicka på `Broadcast Transaction` för att sända den på Bitcoin-nätverket i väntan på bekräftelse.
...
Du vet nu hur en Stonewall-transaktion fungerar på Sparrow Wallet och hur man skapar en. För att fördjupa din behärskning av dessa verktyg som är utformade för att stärka din onchain-konfidentialitet, inbjuder jag dig att följa min BTC 204-utbildning på Plan ₿ Academy :
