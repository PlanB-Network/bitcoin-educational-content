...
De SeedSigner is een Hardware Wallet die je zelf in elkaar zet met standaard hardware, meestal rond een Raspberry Pi Zero. Deze Wallet wordt "*stateless*" genoemd: in tegenstelling tot de meeste andere modellen op de markt (Coldcard, Trezor, Ledger, enz.) slaat hij geen gegevens op in het permanente geheugen en werkt hij alleen live vanuit RAM. Hierdoor wordt de seed van jouw portfolio nooit opgeslagen op de SeedSigner. Elke keer dat u opnieuw opstart, moet u het invullen om het apparaat in staat te stellen uw transacties te ondertekenen. De meest gebruikelijke methode is om je seed op te slaan als een QR-code, die je dan elke keer scant als je hem gebruikt (*SeedQR*).



Deze aanpak houdt echter een aanzienlijk risico in: het seed moet toegankelijk blijven in duidelijke tekst zodat het gescand kan worden. In het geval van diefstal of inbraak kan een aanvaller het gemakkelijk bemachtigen en je bitcoins stelen.



Om deze zwakte te omzeilen, kan SeedSigner gecombineerd worden met [**Seedkeeper**](https://satochip.io/product/seedkeeper/), een smartcard ontwikkeld door Satochip. Hiermee kunnen Mnemonic zinnen (of andere geheimen) worden opgeslagen in een secure element die beschermd is met een PIN-code. De Seedkeeper applet is open-source en de secure element heeft een EAL6+ certificering. Gebruikt in combinatie met SeedSigner biedt het een zeer interessante beveiligingsfunctie: je sleutels blijven volledig offline beheerd, je ondertekent je transacties op een vertrouwd scherm en de seed is fysiek beschermd in een smartcard die bestand is tegen fysieke aanvallen.
...
- Als je nog geen Bitcoin Wallet aan je SeedSigner hebt gekoppeld, moet je de stappen **5** en **6** van de onderstaande tutorial volgen. Deze secties leggen uit hoe je een generate Mnemonic frase met de SeedSigner verbindt, opslaat via een *SeedQR*, en dan deze Wallet verbindt met Sparrow wallet om het te beheren. Ik ga hier niet in op deze procedures en **ik ga ervan uit dat je al een werkende Bitcoin Wallet hebt, geconfigureerd met Sparrow en je SeedSigner**.
...
Nu je Seedkeeper operationeel is, kun je je Bitcoin Wallet Mnemonic opslaan op de smartcard. Om te beginnen zet je zoals gewoonlijk je SeedSigner aan en scan je de *SeedQR* van je Wallet om hem in het apparaat te laden. Zodra de seed is geïmporteerd, selecteer je gewoon `Done`.
...
Kies nu de nieuwe PIN-code van je Seedkeeper, tussen 4 en 16 tekens. Kies voor extra veiligheid een lange, willekeurige code: het is de enige barrière die fysieke toegang tot je Mnemonic zin beschermt.
...
Je kunt dan een `Label` koppelen aan je Mnemonic zin. Dit label is handig als je meerdere geheimen op Seedkeeper opslaat, zodat je ze gemakkelijk kunt identificeren.
...
Je Mnemonic zin is nu opgeslagen op de smartcard.
...
Qua beveiligingsstrategie zijn er verschillende benaderingen mogelijk, afhankelijk van je behoeften en de mate van risicoblootstelling. Persoonlijk raad ik aan om minstens 2 kopieën van je seed te bewaren:




- Dit is een primeur voor smartcards, die je gemakkelijk toegankelijk kunt houden voor alledaagse handelingen, zoals het verifiëren van adressen of het ondertekenen van transacties. Deze methode is praktisch (zoals we zullen zien in deel 5) en blijft veilig dankzij de bescherming die de pincode biedt, dus je kunt hem toegankelijk houden zonder grote risico's;
- Een tweede kopie van uw onversleutelde Mnemonic zin, die dient als ultieme back-up van uw portefeuille en die alleen mag worden gebruikt in geval van verlies of diefstal van de Seedkeeper. Omdat deze versie onversleuteld is, moet deze op een aparte, veiligere locatie worden bewaard om te voorkomen dat de 2 back-ups tegelijkertijd worden gecompromitteerd.



Afhankelijk van je beschermingsstrategie en risicoprofiel, kun je de seed ook dupliceren op verschillende Seedkeepers, of meerdere fysieke kopieën van de Mnemonic maken. Bekijk de volgende tutorial voor meer informatie over deze praktijken:
...
## 5. Een seed van Seedkeeper laden
...
Je kunt nu je Seedkeeper gebruiken om je Mnemonic frase in de SeedSigner te laden bij het opstarten, en zo je Bitcoin transacties te ondertekenen. Om te beginnen, zet je je SeedSigner aan door hem in te pluggen, open dan het `Seeds` menu.
...
Uw seed is nu geladen. Controleer of dit de juiste Wallet is door de vingerafdruk die wordt weergegeven op het scherm te vergelijken met de vingerafdruk die is opgegeven in uw Sparrow wallet instellingen. Deze vingerafdruk werd ook gegeven toen de Wallet voor het eerst werd aangemaakt.



Als je een passphrase gebruikt, kun je die in dit stadium toepassen (zie deel 6 van deze tutorial). Klik anders gewoon op `Done`.
...
Ik vind deze methode bijzonder interessant, omdat het je in staat stelt te profiteren van de beveiligingsvoordelen van de passphrase, terwijl je de beperkingen die gepaard gaan met het dagelijks gebruik elimineert. Hier is een voorbeeld van een configuratie die ik zou aanraden:
...
In deze configuratie, als iemand alleen jouw klaartekst Mnemonic te pakken krijgt, kunnen ze niets stelen zonder de passphrase te kennen (mits deze natuurlijk sterk genoeg is om een brute-force aanval te weerstaan). Omgekeerd, als iemand jouw klaartekst passphrase ontdekt, blijft het onbruikbaar zonder de overeenkomstige Mnemonic zin.



Tot slot, als iemand fysieke toegang weet te krijgen tot je Seedkeeper met seed en passphrase, kunnen ze er niets uithalen zonder de PIN-code te kennen. In tegenstelling tot een passphrase kan deze code niet worden geforceerd, omdat de smartcard zichzelf automatisch vergrendelt na 5 ongeldige pogingen.
...
- Een **passphrase strong**: deze moet lang en willekeurig zijn en een grote verscheidenheid aan tekens bevatten. De complexiteit ervan is geen probleem voor jou, omdat je het maar één keer hoeft in te voeren op het toetsenbord tijdens de initialisatie, waarna het wordt verzonden door Seedkeeper;
...
Zodra je portfolio met passphrase correct is geladen op de SeedSigner, open je het `Zaad` menu en selecteer je de footprint die overeenkomt met dit portfolio. Merk op dat deze footprint verschilt van die van de portfolio zonder passphrase.
...
Klik dan op `Backup seed`, plaats de Seedkeeper in de drive en selecteer `To SeedKeeper`.
...
Voer uw PIN-code in om de smartcard te ontgrendelen en selecteer vervolgens de Wallet die overeenkomt met uw passphrase.
...
- In het `SeedKeeper Functies` menu kun je :
 ...
 - Een bestaande secret verwijderen ;
 - Je descriptoren opslaan of laden (handige functie voor Multisig portfolio's).
