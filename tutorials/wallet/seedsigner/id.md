...
description: Hardware Wallet yang dibuat sendiri, tanpa kewarganegaraan, terjangkau, dan sepenuhnya memiliki celah udara
---
...
SeedSigner adalah Hardware Wallet Bitcoin sumber terbuka yang dapat dibuat sendiri oleh siapa saja dengan menggunakan komponen elektronik yang murah dan umum. Tidak seperti produk komersial seperti Ledger, Coldcard atau Trezor, ini bukanlah perangkat siap pakai yang diproduksi oleh perusahaan: ini adalah proyek komunitas yang memungkinkan semua orang membuat perangkat mereka sendiri, mengendalikan setiap langkah.



SeedSigner dirancang untuk menjadi 100% ***air-gapped***: tidak pernah terhubung ke Internet, tidak memiliki Wi-Fi atau Bluetooth (dalam kasus Raspberry Pi Zero v1.3) dan tidak pernah terhubung ke komputer untuk data Exchange. Komunikasi secara eksklusif melalui sistem Exchange kode QR. Secara konkret, perangkat lunak manajemen portofolio Anda (seperti Sparrow wallet) menampilkan transaksi yang akan ditandatangani dalam bentuk kode QR; Anda memindainya dengan kamera SeedSigner, kemudian perangkat menandatangani transaksi menggunakan kunci pribadi Anda yang disimpan sementara di RAM. Terakhir, alat ini menghasilkan kode QR yang berisi transaksi yang telah ditandatangani, yang Anda pindai dengan perangkat lunak Anda untuk mengirimkannya ke jaringan Bitcoin.
...
Proyek SeedSigner menempati tempat yang penting dalam ekosistem Bitcoin, karena menawarkan kepada semua orang, di mana pun di dunia, kemungkinan untuk mendapatkan keuntungan dari keamanan tingkat lanjut untuk melindungi bitcoin mereka. Keuntungan utamanya terletak pada aksesibilitasnya: perangkat keras yang dibutuhkan dapat dibeli dengan harga kurang dari $50. Terlebih lagi, ini memungkinkan orang-orang yang tinggal di negara-negara yang dibatasi untuk membangun Hardware Wallet mereka sendiri dari komponen komputer standar, yang mudah ditemukan dan tidak terlalu tunduk pada batasan peraturan.



Tetapi bahkan di luar konteks khusus ini, SeedSigner dapat menjadi opsi yang menarik untuk Anda: ini adalah sumber terbuka, bekerja tanpa status dan tanpa celah udara, dan mengurangi vektor serangan yang terkait dengan rantai Supply dari Hardware Wallet Anda.
...
- Raspberry Pi Nol
 ...
 - Jangan lupa untuk menyertakan micro-USB power Supply.
...
- Layar Waveshare 1,3 inci (240×240 piksel)** (dalam bahasa Prancis)
    ...
    - Layar menggabungkan tiga tombol dan joystick mini yang berfungsi sebagai Interface pengguna.
...
- Housing (opsional, tetapi direkomendasikan)** (opsional, tetapi direkomendasikan)** (opsional, tetapi direkomendasikan)** (opsional, tetapi direkomendasikan)** (direkomendasikan)
...
Anda bisa membeli komponen-komponen ini secara terpisah atau, untuk lebih mudahnya, pilihlah paket yang sudah jadi yang sudah termasuk semua perangkat keras yang diperlukan. Secara pribadi, saya memesan paket saya [dari situs Perancis ini](https://bitcoinbazar.fr/), tetapi Anda juga akan menemukan daftar vendor untuk setiap wilayah di dunia pada [halaman perangkat keras proyek SeedSigner](https://seedsigner.com/hardware/). Jika Anda lebih suka membeli komponen secara terpisah, komponen-komponen tersebut tersedia di platform e-commerce utama atau di toko-toko spesialis.
...
Selanjutnya, buka PowerShell di folder tempat file-file yang telah diunduh berada (`Shift` + klik kanan > `Buka PowerShell di sini`). Jalankan perintah berikut untuk memeriksa tanda tangan manifes (ingatlah untuk memodifikasi perintah sesuai dengan versi Anda, di sini `0.8.6.`):
...
PowerShell kemudian menghitung Hash SHA256 dari file gambar Anda. Bandingkan Hash ini dengan nilai yang sesuai dalam `seedsigner.0.8.6.sha256.txt`.
...
Versi firmware saat ini mendukung beberapa paket perangkat lunak: Sparrow, Specter, BlueWallet, Nunchuk dan Keeper. Dalam kasus saya, saya menggunakan **Sparrow wallet**, yang secara khusus saya rekomendasikan karena kesederhanaan dan fungsionalitasnya yang kaya.
...
- gW-20 (satoshi, atau 1/100.000.000 BTC)
...
- gW-22 network`: untuk dimodifikasi hanya jika Anda ingin menggunakan SeedSigner pada Testnet.
- qR code density`: menyesuaikan jumlah informasi yang terkandung dalam setiap kode QR. Anda dapat membiarkan nilai default, kecuali jika Anda merasa kesulitan membaca saat memindai.
...
- `Jenis skrip`: mendefinisikan jenis skrip yang diperbolehkan untuk mengunci bitcoin Anda. Anda tidak perlu memodifikasi parameter ini, karena tipe skrip akan disetel langsung ke Sparrow. Di sini, hanya skrip yang diizinkan untuk dimanipulasi oleh SeedSigner yang perlu diperhatikan.
...
Terakhir, dalam menu `Language`, Anda dapat mengubah bahasa Interface sesuai dengan preferensi Anda.
...
seed (atau frasa Mnemonic) membentuk dasar portofolio Bitcoin Anda. Frasa ini digunakan untuk mendapatkan private key dan alamat Anda, dan menyediakan akses ke dana Anda. SeedSigner menawarkan beberapa metode untuk membuatnya, yang akan kita bahas di bagian ini.
...
- Frasa ini memberikan Anda akses penuh dan tidak terbatas ke semua bitcoin Anda.** Siapa pun yang memiliki frasa ini dapat mencuri dana Anda, bahkan tanpa akses fisik ke SeedSigner Anda;
- Biasanya, frasa 12 kata digunakan untuk mengembalikan Wallet jika Hardware Wallet hilang atau dicuri. Tetapi karena SeedSigner adalah perangkat *stateless*, ia tidak pernah mendaftarkan seed Anda. Jadi, cadangan fisik Anda bukan hanya salinan cadangan, tetapi **satu-satunya cara untuk menggunakan Wallet Anda**. Jika Anda kehilangan cadangan ini, bitcoin Anda akan hilang secara permanen. Jadi, buatlah cadangannya dengan hati-hati, di beberapa media dan di tempat yang aman;
- Jika Anda baru memulai, saya sangat menyarankan Anda untuk membaca tutorial ini untuk mendapatkan pemahaman yang mendetail mengenai risiko yang terlibat dalam mengelola frasa Mnemonic:
...
Sekarang Anda akan mendapatkan generate dan seed. seed hanyalah sebuah angka acak yang besar. Semakin acak angka tersebut dibuat, semakin aman angka tersebut. SeedSigner menawarkan dua cara untuk melakukan ini:




- kamera": seed dihasilkan dari noise visual sebuah foto. Anda mengambil gambar lingkungan acak (objek, lanskap, wajah, dll.) yang variasi pikselnya digunakan untuk entropi generate. Ini adalah metode yang cepat, tetapi tidak dapat direproduksi.
...
Kemudian, pilih panjang frasa Mnemonic Anda. 12 kata sudah menawarkan tingkat keamanan yang memadai, jadi ini adalah pilihan yang saya rekomendasikan.
...
Tuliskan dengan hati-hati kata-kata frasa Mnemonic Anda pada penyangga fisik yang sesuai (kertas atau logam).
...
SeedSigner adalah sebuah perangkat tanpa memori permanen. Ini berarti seed Anda tidak pernah disimpan di dalam perangkat (tidak seperti Ledger, Trezor, atau Coldcard, misalnya). Segera setelah Anda mematikan daya, seed akan menghilang sepenuhnya dari RAM. Ketika SeedSigner dinyalakan kembali, ia akan kembali ke kondisi kosong: Anda harus memberikan seed Anda lagi untuk menandatangani transaksi Anda.



Hal ini memberikan perlindungan yang penting. Tidak seperti dompet perangkat keras lainnya, SeedSigner didasarkan pada Raspberry Pi Zero tanpa perlindungan fisik, termasuk *secure element*. Tetapi karena tidak ada data sensitif yang disimpan, bahkan perangkat yang disusupi secara fisik tidak akan mengizinkan penyerang untuk mengekstrak kunci pribadi Anda atau membelanjakan bitcoin Anda.
...
Saya juga menyarankan Anda untuk menyimpan kode QR dan frasa Mnemonic Anda di dua lokasi yang terpisah, untuk menghindari kehilangan semuanya jika terjadi klaim.
...
Pada tahap ini Anda juga dapat menambahkan **passphrase BIP39** untuk memperkuat keamanan Wallet Anda. Opsi ini mungkin bermanfaat, tergantung pada strategi pencadangan Anda, tetapi juga memiliki risiko: jika Anda kehilangan passphrase, akses ke bitcoin Anda akan hilang secara permanen.
...
Untuk melakukan ini, Anda memerlukan kertas kosong atau kode QR logam yang sesuai dengan panjang frasa Mnemonic Anda. Jika Anda telah membeli paket lengkap untuk SeedSigner Anda, template biasanya disertakan. Jika tidak, Anda dapat mengunduh dan mencetaknya (atau mereproduksinya dengan tangan) di sini:
...
Dari layar seed Anda, pilih `Cadangan seed`.
...
Secara hati-hati, reproduksi setiap titik hitam pada lembar penyimpanan Anda, lalu gunakan joystick untuk melanjutkan ke BLOCK berikutnya. Luangkan waktu Anda: kesalahan peletakan yang sederhana dapat membuat kode QR tidak dapat digunakan.
...
Jika pesan `Sukses` ditampilkan, maka *SeedQR* Anda valid: Anda dapat melanjutkan ke langkah berikutnya.
...
Pastikan Anda telah menginstal perangkat lunak Sparrow wallet di komputer Anda. Jika Anda tidak tahu cara mengunduh, memeriksa, dan menginstalnya dengan benar, silakan lihat tutorial lengkap kami tentang masalah ini:
...
Tetapkan nama untuk portofolio Anda, lalu klik `Create Wallet`. Sparrow kemudian akan meminta Anda untuk mengatur kata sandi untuk mengunci akses lokal ke Wallet ini. Pilih kata sandi yang kuat: kata sandi ini akan melindungi akses ke data portofolio Anda di Sparrow (kunci publik, alamat, label, dan riwayat transaksi). Kata sandi ini tidak diperlukan untuk memulihkan portofolio di kemudian hari: hanya frasa Mnemonic Anda (dan mungkin passphrase Anda) yang diperlukan untuk tujuan ini.
...
#### 7.1.1 Menghasilkan penerimaan Address
...
Bitcoin Address yang baru ditampilkan. Anda akan melihat:




- Teks Address (dimulai dengan `bc1p...` jika Anda menggunakan P2TR seperti yang saya lakukan),
- Kode QR yang sesuai,
- Kolom `Label` untuk melacak transaksi Anda.



Saya sangat menyarankan agar Anda menambahkan label pada setiap tanda terima Bitcoin pada Wallet Anda. Hal ini akan memungkinkan Anda untuk dengan mudah mengidentifikasi asal usul setiap UTXO dan meningkatkan manajemen privasi Anda. Untuk mempelajari lebih dalam topik penting ini, Anda dapat melihat pelatihan khusus di Plan ₿ Academy:
...
Address Anda sekarang dikaitkan dengan label ini di semua bagian Sparrow.
...
Sebelum membagikan Address yang Anda terima, sangat penting untuk memeriksa apakah Address tersebut termasuk dalam seed Anda. Langkah ini memastikan bahwa SeedSigner Anda akan dapat menandatangani transaksi yang terkait dengan Address ini. Langkah ini juga melindungi dari kemungkinan serangan di mana Sparrow menampilkan Address palsu. Ingatlah bahwa Sparrow berjalan di lingkungan yang tidak aman (komputer Anda), yang memiliki permukaan serangan yang jauh lebih besar dibandingkan dengan SeedSigner Anda, yang benar-benar terisolasi. Oleh karena itu, jangan pernah percaya begitu saja dengan Address penerima yang ditampilkan di Sparrow sampai Anda memverifikasinya dengan Hardware Wallet Anda.



Pada Sparrow, klik pada kode QR Address untuk memperbesarnya: kemudian akan ditampilkan dalam layar penuh.
...
Jika Address yang dipindai cocok dengan yang berasal dari seed Anda, layar SeedSigner akan menampilkan pesan: `Address Terverifikasi`.
...
Hal ini mengonfirmasi bahwa Address adalah milik Wallet Anda dan Anda bisa menerima bitcoin darinya.
...
Anda sekarang dapat mengomunikasikan Address ini (dalam bentuk teks atau kode QR) kepada orang atau departemen yang perlu mengirimi Anda Satss. Setelah transaksi disiarkan di jaringan, transaksi tersebut akan muncul di tab `Transaksi` pada Sparrow wallet.
...
Pada Sparrow wallet, Anda dapat mengeklik tab `Send` pada bilah sisi kiri. Namun demikian, saya lebih suka menggunakan tab `UTXOs`, yang memungkinkan Anda mempraktikkan "*Coin Control*". Metode ini memberi Anda kendali yang tepat atas UTXO yang digunakan, sehingga Anda bisa mengendalikan informasi yang Anda ungkapkan selama transaksi.
...
- Di `Bayar ke`, tempelkan Address penerima atau klik ikon kamera untuk memindai kode QR;
- Di `Label`, tambahkan label untuk melacak pengeluaran ini;
- Dalam `Jumlah`, masukkan jumlah yang akan dikirim;
- Terakhir, pilih tarif biaya berdasarkan kondisi pasar saat ini (perkiraan tersedia di [Mempool.space] (https://Mempool.space/)).
...
Klik `Tinjau Detail` dan periksa dengan cermat semua informasi secara langsung pada layar SeedSigner. Hal yang paling penting untuk diperiksa adalah jumlah yang dikirim, Address penerima dan jumlah biaya yang dikenakan.
...
Setelah transaksi valid, transaksi tersebut harus disiarkan di jaringan Bitcoin, sehingga mencapai Miner yang akan menambahkannya ke BLOCK.
