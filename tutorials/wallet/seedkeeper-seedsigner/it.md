...
Per superare questa debolezza, SeedSigner può essere combinato con [**Seedkeeper**](https://satochip.io/product/seedkeeper/), una smart card sviluppata da Satochip. Ciò consente di memorizzare frasi Mnemonic (o altri segreti) in una secure element protetta da un codice PIN. L'applet Seedkeeper è open-source e il suo secure element è certificato EAL6+. Utilizzata insieme a SeedSigner, offre una funzionalità di sicurezza molto interessante: le chiavi rimangono gestite interamente off-line, si firmano le transazioni su uno schermo affidabile e il seed è fisicamente protetto in una smartcard resistente agli attacchi fisici.
...
- Se si dispone già di un portafoglio Bitcoin gestito tramite SeedSigner, è sufficiente installare il nuovo firmware. Potrete quindi continuare a utilizzare il vostro Wallet esistente, questa volta utilizzando Seedkeeper per una maggiore sicurezza.
- Se non si dispone ancora di un Bitcoin Wallet associato al SeedSigner, è necessario seguire i passaggi **5** e **6** dell'esercitazione riportata di seguito. Queste sezioni spiegano come generate una frase Mnemonic con il SeedSigner, salvarla tramite un *SeedQR* e quindi collegare questo Wallet al Sparrow wallet per gestirlo. Non mi soffermerò su queste procedure e **presumo che abbiate già un Bitcoin Wallet funzionante, configurato con Sparrow e il vostro SeedSigner**.
...
Per utilizzare SeedSigner con un Seedkeeper, è necessario installare un firmware alternativo, diverso da quello del SeedSigner originale, per supportare la lettura delle smart card. A tale scopo, [consiglio di utilizzare Fork di "*3rdIteration*"](https://github.com/3rdIteration/seedsigner). Scaricare [l'ultima versione dell'immagine](https://github.com/3rdIteration/seedsigner/releases) (`.zip`) corrispondente al modello di Raspberry Pi in uso.
...
Una volta che l'applet è stata installata correttamente, si può passare al punto 4.
...
Ora che il Seedkeeper è operativo, è possibile salvare il Bitcoin Wallet Mnemonic sulla smartcard. Per iniziare, accendere il SeedSigner come di consueto, quindi scansionare il *SeedQR* del Wallet per caricarlo nel dispositivo. Una volta importato il seed, è sufficiente selezionare "Fatto".
...
Ora scegliete il nuovo codice PIN del vostro Seedkeeper, compreso tra 4 e 16 caratteri. Per una maggiore sicurezza, scegliete un codice lungo e casuale: è l'unica barriera che protegge l'accesso fisico alla vostra frase Mnemonic.



Ricordate di salvare il PIN appena creato, in un gestore di password affidabile o su un supporto fisico separato, a seconda della vostra strategia. In quest'ultimo caso, assicuratevi di non tenere mai il supporto contenente il PIN nello stesso posto del vostro Seedkeeper, altrimenti la protezione diventerà inefficace. È importante avere una copia di backup: **Senza questo PIN, non sarà possibile accedere al seed e i bitcoin andranno persi**.
...
È quindi possibile definire un'etichetta (`Label`) associata alla frase Mnemonic. Questa etichetta è utile se si memorizzano diversi segreti su Seedkeeper, in modo da poterli identificare facilmente.
...
La frase Mnemonic è ora salvata sulla smartcard.
...
- Una seconda copia della frase Mnemonic non crittografata, che funge da backup definitivo del portafoglio, da utilizzare solo in caso di perdita o furto del Seedkeeper. Poiché questa versione non è criptata, deve essere conservata in un luogo separato e più sicuro, per evitare la compromissione simultanea dei due backup.



A seconda della vostra strategia di protezione e del vostro profilo di rischio, potete anche duplicare il seed su diversi Seedkeeper, o creare diverse copie fisiche del Mnemonic. Per saperne di più su queste pratiche, consultate il seguente tutorial:
...
Ora è possibile utilizzare il Seedkeeper per caricare la frase Mnemonic nel SeedSigner all'avvio, e quindi firmare le transazioni Bitcoin. Per iniziare, accendete il SeedSigner collegandolo, quindi aprite il menu `Seeds`.
...
Il seed è ora caricato. Verificare che si tratti del Wallet corretto confrontando l'impronta digitale visualizzata sullo schermo con quella specificata nelle impostazioni del Sparrow wallet. Questa impronta digitale è stata fornita anche al momento della creazione del Wallet.
...
State utilizzando un passphrase per proteggere il vostro portafoglio Bitcoin? Potete registrarlo nel vostro Seedkeeper, insieme al seed. Questa soluzione vi consentirà di caricare rapidamente il vostro Wallet sul SeedSigner, senza dover inserire manualmente il passphrase sulla piccola tastiera ogni volta che lo utilizzate.



Trovo questo metodo particolarmente interessante, in quanto consente di beneficiare dei vantaggi di sicurezza di passphrase, eliminando al contempo i vincoli associati al suo utilizzo quotidiano. Ecco un esempio di configurazione che consiglio:




- Conservate i vostri seed e passphrase in un Seedkeeper, protetto da un codice PIN forte (questo è importante). Questo backup vi consentirà di utilizzare facilmente il vostro Wallet su base giornaliera. Se lo desiderate, potete duplicare queste informazioni su un secondo Seedkeeper;
- Conservate anche una copia chiara dei vostri Mnemonic e passphrase, su carta o metallo. Si tratta del backup di ultima istanza in caso di perdita del Seedkeeper o del suo PIN. Assicuratevi di conservare queste copie in luoghi separati, in modo che non possano essere compromesse contemporaneamente.



In questa configurazione, se qualcuno mette le mani solo sul testo in chiaro Mnemonic, non sarà in grado di rubare nulla senza conoscere il passphrase (a condizione, ovviamente, che sia abbastanza forte da resistere a un attacco di forza bruta). Al contrario, se qualcuno scopre il testo in chiaro passphrase, questo rimarrà inutilizzabile senza la frase Mnemonic corrispondente.
...
- Un **passphrase forte**: deve essere lungo, casuale e contenere un'ampia varietà di caratteri. La sua complessità non è un problema per l'utente, poiché dovrà inserirlo una sola volta sulla tastiera durante l'inizializzazione, dopodiché verrà trasmesso da Seedkeeper;
- Un **codice PIN** per Seedkeeper: anch'esso casuale e composto da 16 caratteri.



Per impostare questa configurazione, iniziare a caricare il passphrase nel SeedSigner nel modo consueto. Potete seguire la procedura descritta in questo tutorial:



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

Una volta che il portafoglio con passphrase è stato caricato correttamente sul SeedSigner, aprire il menu `Seeds` e selezionare l'impronta corrispondente a questo portafoglio. Si noti che questa impronta differisce da quella del portafoglio senza passphrase.
...
Inserire il PIN per sbloccare Seedkeeper, quindi assegnare un'etichetta a questo segreto. È possibile lasciare l'impronta digitale come etichetta per mantenere una forma di negazione plausibile, oppure indicare esplicitamente `passphrase Wallet`, ad esempio.
...
Immettere il codice PIN per sbloccare la smartcard, quindi selezionare il Wallet corrispondente al passphrase.
...
- Nel menu `Funzioni SeedKeeper` è possibile :
 ...
 - Salvare o caricare i descrittori (funzione utile per i portafogli Multisig).
