---
name: Mostro
description: Platforma Bitcoin P2P Exchange bez KYC prostřednictvím Lightning a Nostr
---
...
**Mostro P2P** nabízí decentralizovanou alternativu kombinující Lightning Network, protokol Nostr a podržené faktury. Jeho hlavní inovace: automatizovaný systém úschovy, kdy vaše bitcoiny zůstávají pod vaší kontrolou po celou dobu trvání Exchange, což eliminuje riziko krádeže, bankrotu Exchange nebo svévolné konfiskace.
...
Mostro P2P je open source protokol Bitcoin Exchange, který vytvořil **grunch**, vývojář populárního bota Telegram **lnp2pbot** spuštěného v roce 2021. Ačkoli lnp2pbot již umožnil výměnu P2P bez KYC prostřednictvím Lightningu, představoval velkou zranitelnost: **Telegram představuje centralizovaný bod selhání** náchylný k cenzuře ze strany vlád.



Mostro představuje **decentralizovanou evoluci** tohoto konceptu: nahrazením Telegramu protokolem **Nostr** (nesčítatelná síť distribuovaných relé) Mostro toto systémové riziko eliminuje. Protokol kombinuje Lightning Network pro okamžité transakce, Nostr pro komunikaci odolnou vůči cenzuře a **podržené faktury**, čímž vytváří skutečně neúřední automatizovanou úschovu.
...
- daemon Mostrod**: koordinuje výměny mezi uživateli a Lightning Network
...
**Decentralizované rozhodčí řízení**: V případě sporu nestranný rozhodce přezkoumá důkazy a rozhodne na základě transparentních kritérií. Každý spor generuje jedinečný kód token pro dohledatelnost.



**Flexibilita platby**: Podpora mnoha fiat měn prostřednictvím služby yadio.io Exchange rate. Přijímá převody SEPA, PayPal, Revolut, hotovost a jakékoli metody dohodnuté mezi stranami.
...
**Mostro CLI** je nejvyspělejší a nejfunkčnější klient. Je napsán v jazyce Rust a nabízí celou řadu funkcí systému Mostro prostřednictvím příkazového řádku Interface. Je kompatibilní se systémy MacOS, Linux a Windows.
...
- `release`: Uvolnit úschovu a dokončit Exchange
- `getdm`: Zobrazit přímé zprávy
- `rate` : Vyhodnocení protistrany po Exchange
...
Tento návod vás provede instalací a používáním dvou hlavních klientů Mostro: CLI a mobilní aplikaci.
...
- Volitelné, ale doporučené**: Připojení VPN nebo Tor k maskování vaší IP adresy Address
...
Zkontrolujte, zda je nainstalován program Rust (vyžaduje se verze 1.64+):
...
**Důležité pro synchronizaci CLI/Mobile**: Pro přístup ke stejným objednávkám v CLI a mobilní aplikaci je nutné použít **stejný Mostro Pubkey** a **stejné Nostr relays** v obou klientech. Zkontrolujte tato nastavení v Nastavení v mobilní aplikaci.
...
Kompilace trvá 1-2 minuty. Spustitelný soubor `mostro-CLI` se nainstaluje do souboru `~/.cargo/bin/`.
...
**Krok 4**: Pokračujte v onboardingu a zjistěte :
...
- Ochrana sítě**: Aktivujte VPN nebo Tor, abyste skryli svou IP adresu Address
- Wallet secure**: Aktivujte automatické zamykání Wallet Lightning
...
Společnost Mostro vás požádá, abyste poskytli **Lightning Invoice** pro získání BTC. Bude uvedena přesná částka v satoších (zde: 4715 Sats).
...
### Krok 3: Poskytněte svůj blesk Invoice



generate a Lightning Invoice z vašeho Wallet (Phoenix, Breez atd.) na přesnou částku, pak ji pošlete :
...
Systém potvrdí zásilku a informuje vás, že máte počkat, až prodávající zaplatí částku Invoice, a teprve poté aktivovat úschovu.
...
Jakmile prodávající potvrdí přijetí fiatů a uvolní escrow příkazem `release`, okamžitě obdržíte své bitcoiny na vámi dodaný blesk Invoice.
...
- Lightning Address** (volitelně): Pro přímý příjem plateb
...
Stiskněte tlačítko Green **"+"** vpravo dole a vyberte možnost **Prodej** (červené tlačítko).
...
**Důležité pokyny**: Nyní musíte **zaplatit zobrazenou částku Invoice hold**, abyste uzamkli své bitcoiny (zde 4674 Sats za 1-5 EUR) v úschově. Máte na to maximálně **15 minut**, jinak bude Exchange zrušen.



Zkopírujte zadržení Invoice a zaplaťte ho z blesku Wallet (Phoenix, Breez atd.).
...
Kupující (zde "anonymous-gloriaZhao") vás bude kontaktovat a vyžádá si vaše platební údaje. Prosím, odpovězte a uveďte své údaje (Revtag, IBAN atd.).
...
Po **potvrzení přijetí** platby ve formátu fiat na váš účet stiskněte tlačítko Green **RELEASE** a potvrďte odeslání satss kupujícímu.
...
Bitcoiny jsou okamžitě převedeny kupujícímu prostřednictvím jeho Lightning Invoice.
...
4. Poskytněte svůj blesk Invoice (vygenerovaný z vašeho Wallet)
...
9. Přijímejte bitcoiny okamžitě na svůj Lightning Invoice
...
**Finanční suverenita**: Vaše bitcoiny nikdy neopustí vaši přímou kontrolu díky mechanismu Invoice, který eliminuje riziko bankrotu nebo pirátství Exchange.
...
- [Průvodce Lightning Network](https://lightning.network)