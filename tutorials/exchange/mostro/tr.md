---
name: Mostro
description: Lightning ve Nostr aracılığıyla KYC içermeyen Bitcoin P2P Exchange platformu
---
...
Finansal egemenliğinizden ödün vermeden nasıl bitcoin alabilir veya satabilirsiniz? Merkezi platformlar, keyfi hesap dondurma veya devlet gözetimi olasılığı ile kişisel verilerinizi açığa çıkaran invaziv KYC prosedürleri uygular.



**Mostro P2P**, Lightning Network, Nostr protokolü ve fatura tutmayı birleştiren merkezi olmayan bir alternatif sunar. En önemli yeniliği: bitcoinlerinizin Exchange boyunca kontrolünüz altında kaldığı, hırsızlık, Exchange iflası veya keyfi el koyma riskini ortadan kaldıran otomatik bir emanet sistemi.
...
**Ödeme esnekliği**: Yadio.io Exchange kur hizmeti aracılığıyla birçok fiat para birimi için destek. SEPA transferleri, PayPal, Revolut, nakit ve taraflar arasında kararlaştırılan herhangi bir yöntemi kabul eder.
...
Mostro CLI** en olgun ve işlevsel istemcidir. Rust'de yazılmış olup, Interface komut satırı aracılığıyla Mostro özelliklerinin tamamını sunar. MacOS, Linux ve Windows ile uyumludur.
...
- `serbest bırak`: Emaneti serbest bırakın ve Exchange'yi sonuçlandırın
- `getdm`: Doğrudan mesajları görüntüle
- `rate` : Bir Exchange sonrasında karşı tarafınızı değerlendirin
...
Alfa sürümündeki **mobil uygulama** doğrudan akıllı telefonunuzdan P2P ticareti yapmanızı sağlar. Sekmeli navigasyon, emir görüntüleme, gelişmiş filtreler ve karşı taraflarınızla iletişim kurmak için entegre sohbet içeren sezgisel grafik Interface.
...
- İsteğe bağlı ama önerilir**: IP Address'ünüzü maskelemek için VPN veya Tor bağlantısı
...
**1. Adım: Rust doğrulaması**



Rust'nin kurulu olup olmadığını kontrol edin (sürüm 1.64+ gereklidir):
...
**Adım 2: İndirdikten sonra, APK dosyasını indirilenler bölümünden açın. Android sizden bu kaynaktan yüklemeye izin vermenizi isteyecektir.
...
- Ağ koruması**: IP Address'inizi gizlemek için VPN veya Tor'u etkinleştirin
- Wallet güvenli**: Wallet Lightning'inizdeki otomatik kilidi etkinleştirin
...
Mostro, BTC'yi almak için sizden bir **Lightning Invoice** sağlamanızı isteyecektir. Satoshi cinsinden tam miktar belirtilecektir (burada: 4715 Sats).
...
### Adım 3: Yıldırım Invoice'inizi sağlayın



Wallet'inizden (Phoenix, Breez, vb.) tam tutar için generate bir Lightning Invoice, ardından gönderin:
...
Sistem gönderiyi onaylar ve emaneti etkinleştirmeden önce satıcının Invoice tutarı ödemesini beklemeniz gerektiğini bildirir.
...
Satıcı fiatın alındığını onaylar onaylamaz ve `release` komutuyla emaneti serbest bırakır bırakmaz, bitcoinlerinizi sağladığınız Lightning Invoice'de anında alırsınız.
...
Sağ alttaki Green **"+"** düğmesine basın, ardından **SAT** (kırmızı düğme) öğesini seçin.
...
**Önemli talimat**: Şimdi, bitcoinlerinizi (burada 1-5 EUR için 4674 Sats) emanete almak için görüntülenen Invoice hold** ödemesini yapmalısınız. En fazla **15 dakikanız** var, aksi takdirde Exchange iptal edilecektir.



Invoice bekletmesini kopyalayın ve Wallet Lightning'inizden (Phoenix, Breez, vb.) ödeyin.
...
Fiat ödemesini hesabınıza aldığınızı **onayladıktan** sonra, Green **RELEASE** düğmesine basın ve Sats'ları alıcıya göndermek için onaylayın.
...
Bitcoinler, Lightning Invoice aracılığıyla alıcıya anında aktarılır.
...
4. Lightning Invoice'nizi sağlayın (Wallet'ünüzden oluşturulur)
...
9. Lightning Invoice'ünüzde anında bitcoin alın
...
**Finansal egemenlik**: Invoice tutma mekanizması sayesinde bitcoinleriniz asla doğrudan kontrolünüzden çıkmaz ve Exchange iflas veya korsanlık riskini ortadan kaldırır.
