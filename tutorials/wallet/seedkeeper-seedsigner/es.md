...
El SeedSigner es un Hardware Wallet que montas tú mismo a partir de hardware estándar, normalmente en torno a una Raspberry Pi Zero. Esta Wallet se denomina "*stateless*": a diferencia de la mayoría de los otros modelos del mercado (Coldcard, Trezor, Ledger, etc.), no almacena ningún dato en la memoria permanente, y funciona únicamente en directo desde la RAM. Como resultado, el seed de su cartera nunca se almacena en el SeedSigner. Cada vez que reinicies, tendrás que rellenarlo para que el dispositivo pueda firmar tus transacciones. El método más común es guardar tu seed como un código QR, que luego escaneas cada vez que lo usas (*SeedQR*).
...
- Si aún no tiene una Bitcoin Wallet asociada a su SeedSigner, deberá seguir los pasos **5** y **6** del tutorial que se menciona a continuación. Estas secciones explican cómo generate una frase Mnemonic con el SeedSigner, guardarla mediante un *SeedQR*, y luego conectar este Wallet al Sparrow wallet para gestionarlo. No voy a entrar en estos procedimientos aquí y **estoy asumiendo que usted ya tiene una Bitcoin Wallet funcionando, configurada con Sparrow y su SeedSigner**.
...
Ahora que su Seedkeeper está operativo, puede guardar su Bitcoin Wallet Mnemonic en la tarjeta inteligente. Para empezar, encienda su SeedSigner como de costumbre y, a continuación, escanee el *SeedQR* de su Wallet para cargarlo en el dispositivo. Una vez importada la seed, simplemente seleccione `Hecho`.
...
Ahora elige el nuevo código PIN de tu Seedkeeper, entre 4 y 16 caracteres. Para mayor seguridad, elige un código largo y aleatorio: es la única barrera que protege el acceso físico a tu frase Mnemonic.
...
A continuación, puede definir una `Etiqueta` asociada a su frase Mnemonic. Esta etiqueta es útil si almacena varios secretos en Seedkeeper, para poder identificarlos fácilmente.
...
Su frase Mnemonic está ahora guardada en la tarjeta inteligente.
...
En términos de estrategia de seguridad, existen varios enfoques posibles, dependiendo de sus necesidades y nivel de exposición al riesgo. Personalmente, te recomiendo que guardes al menos 2 copias de tu seed:
...
- Una segunda copia de su frase Mnemonic sin cifrar, que servirá como copia de seguridad definitiva de su cartera, y que se utilizará únicamente en caso de pérdida o robo del Seedkeeper. Como esta versión no está encriptada, debe ser guardada en un lugar separado y más seguro, para prevenir el compromiso simultáneo de las 2 copias de seguridad.



En función de su estrategia de protección y su perfil de riesgo, también puede duplicar la seed en varios Seedkeepers diferentes, o crear varias copias físicas de la Mnemonic. Para saber más sobre estas prácticas, echa un vistazo al siguiente tutorial:
...
## 5. Cargar un seed desde Seedkeeper
...
Ahora puede utilizar su Seedkeeper para cargar su frase Mnemonic en el SeedSigner al iniciarse, y así firmar sus transacciones Bitcoin. Para empezar, enciende tu SeedSigner conectándolo y abre el menú `Seeds`.
...
Su seed ya está cargado. Compruebe que se trata de la Wallet correcta comparando la huella digital que aparece en pantalla con la especificada en la configuración de su Sparrow wallet. Esta huella digital también se proporcionó cuando se creó la Wallet por primera vez.



Si utilizas una passphrase, puedes aplicarla en esta fase (consulta la parte 6 de este tutorial). De lo contrario, simplemente haga clic en `Hecho`.
...
## 6. Uso de Seedkeeper con un BIP passphrase39
...
Este método me parece especialmente interesante, ya que permite beneficiarse de las ventajas de seguridad del passphrase, eliminando al mismo tiempo las limitaciones asociadas a su uso cotidiano. He aquí un ejemplo de configuración que yo recomendaría:
...
- Guarde también una copia clara de su Mnemonic y passphrase, en papel o metal. Esta es su copia de seguridad de último recurso si pierde su Seedkeeper o su PIN. Asegúrate de guardar estas copias en lugares separados, para que no puedan ser comprometidas simultáneamente.



En esta configuración, si alguien consigue tu texto plano Mnemonic solo, no podrá robar nada sin conocer el passphrase (siempre que, por supuesto, sea lo suficientemente fuerte como para resistir un ataque de fuerza bruta). Por el contrario, si alguien descubre tu texto plano passphrase, seguirá siendo inutilizable sin la correspondiente frase Mnemonic.



Por último, si alguien consigue acceder físicamente a tu Seedkeeper con la seed y la passphrase, no podrá extraer nada sin conocer el código PIN. A diferencia de la passphrase, este código no puede forzarse, ya que la tarjeta inteligente se bloquea automáticamente tras 5 intentos no válidos.
...
- Un **passphrase fuerte**: debe ser largo, aleatorio y contener una gran variedad de caracteres. Su complejidad no es un problema para ti, ya que sólo tendrás que introducirlo una vez en el teclado durante la inicialización, tras lo cual será transmitido por Seedkeeper;
- Un **código PIN** para Seedkeeper: también aleatorio y compuesto por 16 caracteres.



Para establecer esta configuración, empieza por cargar tu passphrase en el SeedSigner de la forma habitual. Puedes seguir el procedimiento detallado en este tutorial:
...
Introduzca su PIN para desbloquear Seedkeeper y asigne una etiqueta a este secreto. Puedes dejar la huella dactilar como etiqueta para mantener algún tipo de negación plausible, o indicar explícitamente `passphrase Wallet`, por ejemplo.
...
Introduzca su código PIN para desbloquear la tarjeta inteligente y, a continuación, seleccione la Wallet correspondiente a su passphrase.
...
- En el menú `Funciones de SeedKeeper`, puede :
 ...
 - Guarde o cargue sus descriptores (función útil para las carteras Multisig).
