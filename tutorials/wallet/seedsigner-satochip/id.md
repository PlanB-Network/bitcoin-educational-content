...
Satochip adalah Hardware Wallet dalam format kartu pintar, dengan elemen keamanan bersertifikasi EAL6+, salah satu standar keamanan tertinggi. Kartu ini dirancang dan diproduksi oleh perusahaan Belgia dengan nama yang sama: Satochip.
...
- Satochip berharga sekitar €25, dan karena applet ini bersifat open-source, Anda bisa memasangnya sendiri pada smartcard kosong. Kemudian Anda harus menambahkan biaya komponen SeedSigner dan ekstensi untuk membaca smartcard: tergantung di mana Anda membeli perangkat keras ini, totalnya antara €70 dan €100.
...
Strategi pencadangan yang dipilih di sini tergantung pada preferensi Anda, tetapi sangat penting untuk memiliki setidaknya satu salinan frasa Mnemonic Anda, baik di media fisik (kertas atau logam) atau, seperti di sini, di Seedkeeper. Anda juga dapat melipatgandakan jumlah cadangan sesuai kebutuhan. Untuk informasi lebih lanjut tentang strategi pencadangan portofolio, saya sarankan Anda membaca tutorial ini:
...
Pilihlah `Label` untuk dengan mudah mengidentifikasi berbagai rahasia Anda yang tersimpan di Seedkeeper. Anda bisa, misalnya, menyimpan sidik jari Wallet atau secara eksplisit menunjukkan `seed`. Pilihannya tergantung pada preferensi dan risiko Anda.
...
Kode PIN Seedkeeper harus sepanjang dan seacak mungkin, untuk mencegah upaya paksa jika terjadi gangguan fisik pada kartu. Anda juga harus menyimpan salinan cadangan dari kode PIN ini, yang disimpan di tempat yang terpisah dari Seedkeeper. Tanpa PIN ini, Anda tidak akan bisa mengakses Mnemonic yang tersimpan di Seedkeeper, dan bitcoin Anda akan hilang selamanya.
...
Sekarang portofolio Anda telah dibuat, disimpan, dan diperiksa, kita akan mentransfernya ke Satochip. Untuk melakukan ini, pastikan seed dimuat ke dalam RAM SeedSigner. Kemudian masuk ke `Tools > Smartcard Tools > Satochip Functions`.
...
Anda kemudian dapat mengatur kode PIN Satochip, dari 4 hingga 16 karakter. Untuk memperkuat keamanan Wallet Anda, pilihlah kode acak yang panjang: kode ini merupakan satu-satunya perlindungan terhadap akses fisik ke frasa Mnemonic Anda.



Ingatlah untuk menyimpan PIN ini segera setelah dibuat, baik di pengelola kata sandi yang aman atau di media fisik, tergantung strategi pribadi Anda. Dalam kasus terakhir, pastikan untuk tidak menyimpan media yang berisi PIN di tempat yang sama dengan Satochip Anda, karena jika tidak, proteksi tidak akan berguna. Penting untuk memiliki salinan cadangan: **Tanpa PIN ini, Anda tidak akan bisa lagi mengakses seed, dan bitcoin Anda akan hilang secara permanen**.
...
Setelah portofolio Anda aktif dan berjalan, kami akan mengimpor informasi publiknya ("*keystore*") ke dalam Sparrow wallet atau perangkat lunak manajemen portofolio lainnya. Perangkat lunak ini akan digunakan untuk membuat, mendistribusikan, dan melacak transaksi Anda. Akan tetapi, perangkat lunak ini tidak dapat menandatanganinya, karena hanya Satochip (dan semua cadangannya) yang memiliki kunci privat yang dibutuhkan untuk operasi ini.
...
Masukkan kartu microSD yang berisi sistem operasi, kemudian nyalakan SeedSigner Anda. Untuk saat ini, alat ini tidak dapat melakukan apa pun, karena belum mengenal seed Anda. Anda harus mulai dengan memasukkan Satochip ke dalam pembaca kartu pintar, karena kartu inilah yang menampung seed Anda.
...
Pastikan perangkat lunak Sparrow wallet sudah terinstal di komputer Anda. Jika Anda tidak tahu cara mengunduhnya, periksa keasliannya dan instal dengan benar, lihat tutorial lengkap kami tentang masalah ini:
...
SeedSigner kemudian akan meminta Anda untuk memindai Address yang diterima secara acak dari Sparrow wallet Anda untuk mengonfirmasi keabsahan impor.
...
Satochip Anda (melalui SeedSigner) dan Sparrow wallet sekarang terhubung dengan aman. Sparrow berfungsi sebagai manajemen lengkap Interface, sementara Satochip tetap menjadi satu-satunya perangkat yang mampu menandatangani transaksi Anda. Anda sekarang siap untuk menerima dan mengirim bitcoin dalam konfigurasi yang benar-benar tanpa celah.
...
Pada komputer Anda, buka Sparrow wallet dan buka `Satochip-SeedSigner` Wallet menggunakan kata sandi Anda. Pastikan perangkat lunak terhubung ke server (indikator di kanan bawah). Kemudian, pada bilah sisi, klik `Terima`.
...
Muncul Bitcoin Address yang baru. Anda akan menemukan :




- Address dalam format teks (dimulai dengan `bc1q... ` jika Anda menggunakan P2WPKH, seperti dalam contoh ini);
- Kode QR terkait;
- Kolom `Label`, berguna untuk melacak transaksi Anda.



Saya sangat menyarankan agar Anda menambahkan label pada setiap tanda terima Bitcoin di Wallet Anda. Hal ini akan membantu Anda dengan mudah mengidentifikasi asal usul setiap UTXO dan mengelola privasi Anda dengan lebih baik. Untuk mengetahui lebih lanjut tentang topik penting ini, lihat pelatihan khusus di Plan ₿ Academy :
...
Sebelum mengkomunikasikan Address yang Anda terima kepada pembayar, penting untuk memeriksa apakah Address tersebut merupakan bagian dari seed Anda. Langkah ini memastikan bahwa Satochip Anda akan dapat menandatangani transaksi yang terkait dengan Address ini. Langkah ini juga mencegah potensi serangan di mana Sparrow akan menampilkan Address yang palsu. Ingatlah bahwa Sparrow berjalan pada lingkungan yang tidak aman (komputer Anda), yang permukaan serangannya jauh lebih besar dibandingkan dengan Satochip Anda, yang benar-benar terisolasi. Inilah sebabnya mengapa anda tidak boleh mempercayai begitu saja alamat yang ditampilkan pada Sparrow sebelum memeriksanya pada Hardware Wallet anda.
...
Jika Address yang dipindai cocok dengan yang berasal dari seed Anda, SeedSigner akan menampilkan pesan: `Address Terverifikasi`.
...
Dengan demikian, Anda bisa yakin bahwa Address adalah bagian dari portofolio Anda.
...
Anda sekarang dapat mengirimkan Address ini dalam bentuk teks atau melalui kode QR kepada orang atau layanan yang perlu mengirimi Anda Satss. Setelah transaksi disiarkan di jaringan, transaksi tersebut akan muncul di tab `Transaksi` pada Sparrow wallet.
...
Pada Sparrow wallet, Anda dapat membuat transaksi dengan mengeklik tab `Kirim` pada bilah sisi kiri. Namun, saya lebih suka menggunakan tab `UTXOs`, yang memungkinkan Anda mempraktikkan *Kontrol Coin*. Metode ini menawarkan kontrol yang tepat atas UTXO yang dibelanjakan, untuk membatasi informasi yang terungkap selama transaksi Anda.
...
- Di `Bayar ke`, tempelkan Address penerima atau pindai kode QR mereka menggunakan ikon kamera;
- Di `Label`, tambahkan label untuk melacak pengeluaran ini;
- Dalam `Jumlah`, masukkan jumlah yang akan dikirim;
- Terakhir, pilih laju pengisian daya sesuai dengan kondisi jaringan saat ini (perkiraan tersedia di [Mempool.space](https://Mempool.space/)).
...
Transaksi sekarang sudah siap, namun belum ditandatangani. Untuk menampilkan [PSBT (*Partially Signed Bitcoin Transaction*)] (https://planb.academy/en/resources/glossary/PSBT) sebagai kode QR, klik `Show QR`.
...
Setelah transaksi ditandatangani dan divalidasi, yang tersisa hanyalah menyiarkannya di jaringan Bitcoin sehingga Miner dapat memasukkannya ke dalam BLOCK. Pada Sparrow, klik `Pindai QR`.
...
Tunjukkan kode QR yang ditampilkan di SeedSigner Anda (yang berisi transaksi yang ditandatangani) ke webcam. Sparrow kemudian akan menampilkan semua detail transaksi. Lakukan pemeriksaan akhir bahwa semua informasi sudah benar, lalu klik "Broadcast Transaction" untuk menyiarkannya di jaringan Bitcoin.
...
- Dengan merekam frasa Mnemonic pada media fisik;
...
Untuk mendapatkan kembali akses ke portofolio Anda dan memindahkan dana Anda, cukup muat seed Anda ke dalam SeedSigner. Tergantung pada media cadangan yang Anda gunakan, Anda memiliki beberapa opsi:





- Masukkan frasa Mnemonic Anda secara manual di menu `Seeds > Masukkan 12 kata seed`.
...
- Atau muat seed Anda dari Seedkeeper, melalui menu `Seeds > From SeedKeeper` (ini adalah metode yang saya gunakan dalam tutorial ini). Anda hanya perlu memasukkan kode PIN Seedkeeper dan memilih rahasia yang akan digunakan sebagai seed pada SeedSigner.
