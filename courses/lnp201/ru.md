...
В Bitcoin то, что мы фактически обмениваем, не прямо биткоины, а **[UTXO](https://planb.academy/resources/glossary/utxo)** (_Unspent Transaction Outputs_, непотраченные выходы транзакций).
...
В этой главе мы узнаем, как сеть Lightning позволяет осуществлять платежи через промежуточные узлы без необходимости доверять им, благодаря **[HTLC](https://planb.academy/resources/glossary/htlc)** (_Хэшированные контракты с временной блокировкой_). Эти умные контракты гарантируют, что каждый промежуточный узел получит средства из своего канала только в случае, если он перенаправит платеж конечному получателю, иначе платеж не будет подтвержден.
