---
name: SeedSigner
description: Hardware Wallet, který je kutilský, bezstavový, cenově dostupný a plně propojený se vzduchem
---
...
SeedSigner je open-source Hardware Wallet Bitcoin, který si může každý sám sestavit z levných elektronických součástek pro všeobecné použití. Na rozdíl od komerčních produktů, jako je Ledger, Coldcard nebo Trezor, se nejedná o zařízení připravené k použití, které by vyráběla nějaká společnost: je to komunitní projekt, který umožňuje každému vytvořit si vlastní zařízení a kontrolovat každý krok.



SeedSigner je navržen tak, aby byl 100% ***air-gapped***: nikdy se nepřipojuje k internetu, nemá Wi-Fi ani Bluetooth (v případě Raspberry Pi Zero v1.3) a nikdy není připojen k počítači, aby Exchange data. Komunikace probíhá výhradně prostřednictvím systému Exchange s QR kódem. Konkrétně váš software pro správu portfolia (např. Sparrow wallet) zobrazuje transakce, které mají být podepsány, ve formě QR kódů; vy je naskenujete pomocí fotoaparátu SeedSigneru a zařízení pak transakci podepíše pomocí vašich soukromých klíčů dočasně uložených v jeho paměti RAM. Nakonec vygeneruje QR kódy obsahující podepsanou transakci, které naskenujete pomocí svého softwaru a odešlete je do sítě Bitcoin.
...
Další možností, jak uložit kód seed a používat jej s aplikací SeedSigner, je použití čipové karty *SeedKeeper* ve spojení s kompatibilní čtečkou. Získáte tak velmi robustní *secure element* pro uložení seed a zároveň můžete používat obrazovku SeedSigner pro podepisování transakcí. Tato konkrétní konfigurace je však předmětem jiného specializovaného tutoriálu. Zde se zaměříme na základní použití SeedSigneru:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

Projekt SeedSigner zaujímá v ekosystému Bitcoin důležité místo, protože nabízí všem lidem na celém světě možnost využívat pokročilé zabezpečení k ochraně svých bitcoinů. Jeho hlavní výhoda spočívá v jeho dostupnosti: potřebný hardware lze pořídit za méně než 50 dolarů. Navíc umožňuje lidem žijícím v zemích s omezeným přístupem sestavit si vlastní Hardware Wallet ze standardních počítačových komponent, které jsou snadno k sehnání a méně podléhají regulačním omezením.



Ale i mimo tyto konkrétní souvislosti pro vás může být SeedSigner zajímavou volbou: je to open-source, funguje bez stavu a airgapped a omezuje vektory útoku spojené s řetězcem Supply vašeho Hardware Wallet.
...
- Raspberry Pi Zero
 ...
 - Nezapomeňte přibalit napájení micro-USB Supply.
...
- Waveshare 1,3" obrazovka (240 × 240 px)** (ve francouzštině)
    ...
    - Displej obsahuje tři tlačítka a mini joystick, který slouží jako uživatelský ovladač Interface.
...
- Bydlení (nepovinné, ale doporučené)** (nepovinné, ale doporučené)** (nepovinné, ale doporučené)** (nepovinné, ale doporučené)** (doporučené)
...
Prostředí PowerShell poté vypočítá hodnotu Hash SHA256 vašeho souboru s obrázkem. Porovnejte tuto hodnotu Hash s odpovídající hodnotou v souboru `seedsigner.0.8.6.sha256.txt`.
...
Po přípravě karty microSD a jejím flashnutí pomocí softwaru SeedSigner můžete pokračovat v konečné montáži. Nespěchejte, protože některé součásti jsou křehké (zejména ubrus, kamera a piny GPIO).
...
Nejdříve otevřete svůj kufr. Zkontrolujte, zda je čistý a zda vnitřním spojovacím prvkům nepřekáží zbytky plastu z 3D tisku. Dávejte pozor na :
...
Zavřením černého pruhu ubrus uzamknete (ucítíte lehké cvaknutí). Opatrně zkontrolujte, zda zůstává na místě a nehýbe se.
...
Poté umístěte displej Waveshare na Raspberry Pi Zero. Kolíky GPIO počítače Pi by měly dokonale lícovat se zásuvkou displeje. Pomalu přitlačte displej na kolíky a vyvíjejte rovnoměrný tlak na každou stranu, aby nedošlo k jejich ohnutí.
...
Pokud jste změnili orientaci fotoaparátu nebo chcete později změnit jiná nastavení (například jazyk Interface), musíte povolit zachování nastavení na kartě microSD. Jinak se vaše nastavení vrátí na výchozí při každém restartu, protože Raspberry Pi Zero nemá žádnou trvalou paměť.
...
Než vytvoříte Bitcoin Wallet, nakonfigurujme SeedSigner. Protože běží na Raspberry Pi Zero bez trvalé paměti, jeho nastavení se neukládá automaticky, pokud je neuložíte na kartu microSD. Ujistěte se tedy, že jste tuto možnost povolili, jinak budou tato nastavení při restartu ztracena (viz krok 3.5).
...
Aktuální verze firmwaru podporuje několik softwarových balíčků: Sparrow, Specter, BlueWallet, Nunchuk a Keeper. V mém případě používám **Sparrow wallet**, který doporučuji zejména pro jeho jednoduchost a bohatou funkčnost.
...
- `BTC`
- mBTC` (milli-Bitcoin nebo 0,001 BTC)
- gW-20 (satoši nebo 1/100 000 000 BTC)
...
- gW-22 network`: je třeba upravit pouze v případě, že chcete používat SeedSigner na Testnet.
...
V nabídce `Jazyk` můžete změnit jazyk Interface podle svých preferencí.
...
## 5. Vytvoření a uložení souboru seed



Základem portfolia Bitcoin je věta seed (nebo Mnemonic). Slouží k odvození vašich soukromých klíčů a adres a poskytuje přístup k vašim finančním prostředkům. SeedSigner nabízí několik způsobů jejího generování, kterým se budeme věnovat v této části.
...
- Tato fráze vám dává plný, neomezený přístup ke všem vašim bitcoinům.** Kdokoli, kdo má tuto frázi, může ukrást vaše prostředky, a to i bez fyzického přístupu k vašemu SeedSigneru;
- V případě ztráty nebo krádeže Hardware Wallet se k obnovení Wallet obvykle používá fráze o 12 slovech. Protože je však SeedSigner *bezstavové* zařízení, nikdy nezaregistruje váš seed. Vaše fyzické zálohy tedy nejsou jen záložními kopiemi, ale **jediným způsobem, jak používat váš Wallet**. Pokud tyto zálohy ztratíte, vaše bitcoiny budou trvale ztraceny. Zálohujte je proto pečlivě, na několika médiích a na bezpečných místech;
- Pokud teprve začínáte, doporučuji vám přečíst si tento návod, kde se podrobně seznámíte s riziky spojenými se správou fráze Mnemonic :
...
Pokud zvolíte metodu fotografie, klikněte na `New seed` (s ikonou fotoaparátu), vyfoťte snímek a potvrďte jej. Poté zvolte délku věty (12 nebo 24 slov), která se zobrazí na obrazovce pro uložení. Následující kroky jsou totožné s částí 5.3.
...
Poté zvolte délku fráze Mnemonic. dvanáct slov již poskytuje dostatečnou úroveň zabezpečení, proto doporučuji zvolit tuto možnost.
...
Pečlivě napište slova věty Mnemonic na vhodnou fyzickou podložku (papír nebo kov).
...
Pokud uděláte chybu, SeedSigner vás o tom bude informovat a vy budete muset začít znovu, přičemž si nezapomeňte poznamenat frázi Mnemonic, až vám bude předána. Tento ověřovací krok zajistí, že vaše záloha je správná a úplná. Po ověření se na obrazovce zobrazí `Záloha ověřena`.
...
SeedSigner je zařízení bez trvalé paměti. To znamená, že vaše karta seed není v zařízení nikdy uložena (na rozdíl například od karty Ledger, Trezor nebo Coldcard). Jakmile vypnete napájení, zařízení seed z paměti RAM zcela zmizí. Když se SeedSigner restartuje, vrátí se do prázdného stavu: abyste mohli podepisovat transakce, musíte mu znovu předat svůj seed.



To poskytuje základní ochranu. Na rozdíl od jiných hardwarových peněženek je SeedSigner založen na Raspberry Pi Zero bez fyzické ochrany, včetně *secure element*. Protože však nejsou uložena žádná citlivá data, ani fyzicky napadené zařízení by útočníkovi neumožnilo získat vaše soukromé klíče nebo utratit vaše bitcoiny.
...
Při každém použití SeedSigneru naskenujete tento QR kód pomocí fotoaparátu zařízení, které si dočasně načte váš kód seed do paměti, zatímco podepisujete transakce. Tuto druhou zálohu, určenou pro každodenní použití, je rovněž třeba uchovávat s maximální opatrností: kdokoli, kdo má tento QR kód, má plný přístup k vašim bitcoinům.


Doporučuji vám také, abyste si kód QR a větu Mnemonic uložili na dvě oddělená místa, abyste v případě reklamace o vše nepřišli.



Pokročilejší a bezpečnější alternativou je použití SeedSigneru s **SeedKeeper**, který ukládá seed do secure element. Chcete-li se dozvědět více, podívejte se na tento návod :
...
Po dokončení ověření SeedSigner zobrazí otisk hlavního klíče Wallet. Tento otisk identifikuje váš klíč Wallet a zajistí, že v budoucnu použijete správnou frázi pro obnovení. Neprozrazuje žádné informace o vašich soukromých klíčích, takže je můžete bezpečně uložit na digitální médium. Jen se ujistěte, že máte přístupnou kopii a nikdy ji neztratíte.
...
V této fázi můžete také přidat **passphrase BIP39**, abyste posílili zabezpečení svého Wallet. Tato možnost se může vyplatit v závislosti na vaší zálohovací strategii, ale také s sebou nese rizika: pokud ztratíte passphrase, přístup k bitcoinům bude trvale ztracen.
...
### 5.8 Uložení záznamu seed ve formátu QR (*SeedQR*)



SeedSigner umožňuje převést váš seed na papírový QR kód, který se nazývá *SeedQR*. Tato metoda zjednodušuje načítání kódu Wallet, protože se vyhnete ručnímu přepisování každého slova.



K tomu budete potřebovat prázdný papír nebo kovový QR kód odpovídající délce věty Mnemonic. Pokud jste si zakoupili kompletní balíček SeedSigner, jsou šablony obvykle součástí balení. Pokud ne, můžete si je stáhnout a vytisknout (nebo ručně reprodukovat) zde :
...
Na obrazovce seed vyberte možnost `Zálohovat seed`.
...
Pečlivě reprodukujte každou černou tečku na záznamovém archu a pak se pomocí joysticku přesuňte na další BLOCK. Nespěchejte: pouhé chybné nastavení může způsobit nepoužitelnost QR kódu.
...
Jakmile je váš SeedSigner nastaven a váš seed správně vygenerován a uložen, je dalším krokem propojení tohoto portfolia se softwarem pro správu, například Sparrow wallet. Váš seed zůstane vždy offline, protože do Sparrow bude přenášena pouze veřejná část vašeho portfolia. To umožní softwaru zobrazovat vaše adresy, transakce a vytvářet nové transakce, aniž byste kdy mohli utratit své bitcoiny. Chcete-li utratit své bitcoiny, váš SeedSigner bude muset vždy podepsat transakci připravenou Sparrow.
...
Zkontrolujte, zda se otisk prstu na hlavním klíči shoduje s otiskem prstu na klíči Wallet. Pokud používáte passphrase, zadejte jej v této fázi.
...
SeedSigner pak vygeneruje váš xpub ve formě dynamického QR kódu obsahujícího všechny údaje, které potřebujete ke správě svého portfolia v Sparrow wallet.
...
### 6.2 Import nového portfolia do systému Sparrow wallet



Ujistěte se, že máte v počítači nainstalovaný software Sparrow wallet. Pokud nevíte, jak jej správně stáhnout, zkontrolovat a nainstalovat, přečtěte si náš úplný návod na toto téma:



https://planb.academy/tutorials/wallet/desktop/sparrow-c674e2ac-d46f-4c82-92a7-7d1b0e262f5d

V počítači otevřete program Sparrow wallet a v nabídce klikněte na možnost `Soubor → Importovat Wallet`.
...
Přiřaďte svému portfoliu název a klikněte na tlačítko `Vytvořit Wallet`. Poté vás Sparrow vyzve k nastavení hesla pro uzamčení místního přístupu k tomuto Wallet. Zvolte silné heslo: chrání přístup k datům vašeho portfolia v Sparrow (veřejné klíče, adresy, štítky a historie transakcí). Toto heslo není potřeba k pozdějšímu obnovení portfolia: k tomuto účelu je vyžadována pouze vaše fráze Mnemonic (a případně passphrase).
...
Poté zkontrolujte, zda se otisk `Master fingerprint` zobrazený v Sparrow shoduje s otiskem dříve zaznamenaným v SeedSigneru.



Váš SeedSigner a Sparrow wallet jsou nyní bezpečně propojeny. Sparrow funguje jako kompletní správa Interface, zatímco SeedSigner zůstává jediným zařízením schopným podepisovat vaše transakce. Nyní jste připraveni přijímat a odesílat bitcoiny ve zcela vzduchem chráněné konfiguraci.
...
SeedSigner a Sparrow wallet jsou nyní nakonfigurovány ke spolupráci. V této závěrečné části se podíváme na to, jak pomocí této konfigurace přijímat a odesílat bitcoiny.
...
#### 7.1.1 Generování příjmu Address
...
Zobrazí se nová položka Bitcoin Address. Zobrazí se :




- Text Address (začíná `bc1p...`, pokud používáte P2TR jako já),
- Odpovídající QR kód,
- Pole `Label` pro sledování transakcí.



Důrazně doporučuji přidat štítek ke každé účtence Bitcoin na Wallet. To vám umožní snadno identifikovat původ každého UTXO a zlepší správu soukromí. Chcete-li do tohoto důležitého tématu proniknout hlouběji, můžete se podívat na specializované školení na stránkách Plan ₿ Academy :
...
Vaše zařízení Address je nyní přiřazeno k tomuto štítku ve všech sekcích Sparrow.
...
Před sdílením přijímacího zařízení Address je velmi důležité zkontrolovat, zda patří k vašemu zařízení seed. Tento krok zajistí, že váš SeedSigner bude moci podepisovat transakce spojené s tímto Address. Chrání to také před možnými útoky, při kterých by Sparrow zobrazil falešný Address. Nezapomeňte, že Sparrow běží v nezabezpečeném prostředí (váš počítač), které má mnohem větší plochu pro útoky než váš SeedSigner, který je zcela izolovaný. Proto byste nikdy neměli slepě věřit žádnému příjemci Address zobrazenému na Sparrow, dokud si jej neověříte pomocí svého Hardware Wallet.



Na obrazovce Sparrow klikněte na QR kód Address a zvětšete jej: zobrazí se na celou obrazovku.
...
Pokud se naskenovaný kód Address shoduje s kódem seed, zobrazí se na obrazovce SeedSigner zpráva: `Address ověřeno`.
...
Tím potvrdíte, že Address patří k vašemu Wallet a že z něj můžete bez obav přijímat bitcoiny.
...
Nyní můžete tuto zprávu Address (ve formě textu nebo QR kódu) sdělit osobě nebo oddělení, které vám má zaslat Satss. Jakmile bude transakce odvysílána v síti, objeví se na kartě `Transakce` v okně Sparrow wallet.
...
- Vytvoření transakce v Sparrow ;
- Podpis transakce na SeedSigner ;
- Konečné rozdělení transakce prostřednictvím Sparrow.
...
V aplikaci Sparrow wallet můžete kliknout na kartu `Odeslat` v levém postranním panelu. Já však raději používám záložku `UTXOs`, která vám umožňuje praktikovat "*Ovládání Coin*". Tato metoda vám dává přesnou kontrolu nad použitými UTXO, takže můžete kontrolovat informace, které během transakce odhalíte.
...
- Do pole `Platba příjemci` vložte číslo Address příjemce nebo klikněte na ikonu fotoaparátu a naskenujte kód QR;
- V poli `Label` přidejte štítek pro sledování tohoto výdaje;
- Do pole `Suma` zadejte částku, která má být odeslána;
- Nakonec zvolte sazbu poplatku na základě aktuálních tržních podmínek (odhady jsou k dispozici na [Mempool.space](https://Mempool.space/)).
...
Klikněte na `Review Details` a pečlivě zkontrolujte všechny informace přímo na obrazovce SeedSigner. Nejdůležitějšími položkami, které je třeba zkontrolovat, jsou odeslaná částka, číslo Address příjemce a výše případných poplatků.
...
Nyní, když je transakce platná, musí být vysílána v síti Bitcoin, aby se dostala k Miner, který ji přidá do BLOCK.
