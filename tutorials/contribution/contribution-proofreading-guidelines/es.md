...
Bienvenido a este tutorial sobre las pautas que hay que seguir al corregir contenidos en Plan ₿ Network. Nos alegramos de que comparta nuestra misión de traducir los materiales de Bitcoin al mayor número de idiomas posible, con el fin de ayudar a la gente a conocer mejor su funcionamiento y cómo puede utilizarse en su vida cotidiana.
...
El presente tutorial está dedicado a los colaboradores que desean corregir contenidos. La mayoría de ellos no saben mucho acerca de [Github](https://planb.network/en/tutorials/contribution/others/create-github-account-a75fc39d-f0d0-44dc-9cd5-cd94aee0c07c) o el [lenguaje Markdown](https://www.markdownguide.org/basic-syntax/) que utilizamos en el repositorio, por lo que es importante compartir algunas ideas sobre las cosas que deben prestar atención al ejecutar esta tarea.


Lo he creado tras recopilar los problemas más comunes que encuentran los correctores al completar sus tareas. Siéntase libre de sugerir más, ya que puede ayudar a otros a mejorar.


Antes de entrar en detalles, lo primero que hay que hacer es leer este tutorial sobre las acciones prácticas que hay que aplicar en Github, bifurcando el repositorio Plan ₿ Network, confirmando cambios y enviando PR:
...
Este sistema permite al coordinador realizar un seguimiento del progreso dentro del repositorio, y permite que el contenido sea "reclamado" por el corrector, lo que evita la duplicación de esfuerzos por parte de otra persona.

En la propia edición, encontrarás los enlaces que te llevan directamente al contenido a comprobar. Puedes hacer clic directamente en ellos, o incluso mejor, puedes volver a tu propio repositorio y trabajar directamente desde allí.


En primer lugar, recuerda SIEMPRE SINCRONIZAR tu repo, en la rama "dev". De esta forma, el contenido siempre estará actualizado antes de que empieces cualquier tipo de tarea, y no crearás ningún conflicto entre el material antiguo y el nuevo. Asegúrate de hacer click en "Sync Fork" y "Update branch".
...
Dentro de esta nueva página, encontrarás todas las sucursales que ya has abierto bajo el título "Tus sucursales". Esta sección es muy útil porque te permite encontrar fácilmente el lugar donde modificaste algún contenido. Si quieres abrir una nueva rama, puedes hacer clic en "Nueva rama" en la esquina superior derecha de la página.
...
Después de crear esta nueva rama, asegúrese de hacer clic en ella desde "Sus ramas" en la página anterior y empezar a trabajar en el archivo *.md* relacionado con el contenido específico (en mi caso, haré clic en la carpeta "cursos", y en la subcarpeta BTC101, para buscar fr.md). Todos los commits relacionados con el archivo específico tendrán que ser confirmados (guardados) dentro de la misma rama.


## ¿Lengua original o traducción?


Al revisar un contenido, es importante comprobar siempre la versión original en inglés (o francés). Tenga en cuenta que traducimos con herramientas de traducción automática, por lo que la traducción al idioma de destino puede no ser fluida o comprensible para el lector final.
...
Las herramientas LLM pueden traducir literalmente algunas palabras relacionadas con Bitcoin, como Lightning Network, que vendría a ser "Rete Fulmine" en italiano, por ejemplo.


Ocurre especialmente cuando se trata de palabras muy técnicas. En casos como éste, es aconsejable mantener la palabra original en inglés en el idioma de destino para mayor claridad, a menos que las normas de su idioma le impongan traducir cada una de las palabras. En este segundo caso, investigue siempre para ver si alguien más en su comunidad Bitcoin ya ha traducido esa palabra y ahora se utiliza ampliamente.
...
- En cualquier caso, mi consejo sería insertar la palabra EN, no obstante, y a continuación el significado correspondiente en la lengua de llegada dentro de un paréntesis redondo, siguiendo el esquema EN (LANG), o viceversa. Ej. Address (indirizzo) o indirizzo (Address).



- Otra buena solución consiste en mantener la palabra/frase ES original y, a continuación, crear un hipervínculo que redirija al [glosario](https://planb.network/en/resources/glossary) en planb.network. Para ello, debe insertar la palabra/frase entre corchetes y el enlace entre paréntesis redondos, como puede ver en el ejemplo siguiente:
...
El contenido del repositorio está lleno de hipervínculos como los de arriba. Ahora que sabes lo que significan, asegúrate de no borrar ningún enlace insertado por el autor.


Otra cosa relacionada con la traducción de palabras es la siguiente. Si encuentra "Plan ₿ Network" en el texto, déjelo en esta forma original. No traduzca la palabra "plan" ni la palabra "red". Además, NO utilice el artículo "The" al presentar Plan ₿ Network, y **considérelo como una marca**.


Lo mismo ocurre con "₿-CERT", "BIZ SCHOOL", "TECH SCHOOL", que también deben conservarse en su forma original.


Un último apunte a este párrafo: como decíamos más arriba, utilizamos herramientas de IA para traducir contenidos, y luego pedimos la intervención "humana" para asegurarnos de que todo es fluido y está bien revisado.


Si corrige utilizando IA para la mayor parte del texto, seguro que nos daremos cuenta, ya que estamos familiarizados con las estructuras de frases típicas que genera la IA. Si descubrimos que ha confiado únicamente en la IA para la corrección, sin realizar cambios significativos, ¡la recompensa final en Sats puede reducirse a la mitad!
...
En los cursos, las partes más importantes se introducen mediante un único signo Hash, mientras que las subpartes pueden tener de dos a cuatro signos Hash. En los tutoriales, normalmente sólo utilizamos cabeceras con dos signos Hash.
...
Además, en la versión EN de los cursos, los títulos que tienen uno o dos ``#`` tienen todas las palabras empezando en mayúsculas, mientras que los títulos que empiezan con tres o cuatro, no suelen seguir esta regla. Si es posible, asegúrese de que los títulos en su idioma de destino siguen esta estructura.
...
Siguiendo con el tema de los enlaces, si el autor inserta un enlace que hace referencia a un curso o tutorial de Plan ₿ Network, el sitio web creará automáticamente una "tarjeta" que mostrará la miniatura relacionada. En consecuencia, asegúrese siempre de que haya un espacio entre el texto y el propio enlace, ya que de lo contrario podría aparecer el siguiente error en el sitio web.
...
Por último, otra buena práctica que puedes aplicar cuando termines tu tarea de corrección y envíes el RP es volver a la incidencia original abierta por el coordinador y comentar "Corrección realizada". Asegúrate de insertar también allí el enlace a tu RP.


## Conclusión


En resumen, conocer los errores más comunes de los correctores puede ayudarte a mejorar tus habilidades a la hora de revisar contenidos. Es fácil pasar por alto cosas como el contexto o la coherencia, por lo que detectar estos errores puede marcar una gran diferencia. Ten siempre en cuenta que un principiante puede leer estos cursos y tutoriales, por lo que es nuestra responsabilidad asegurarnos de que lo entienden perfectamente.
