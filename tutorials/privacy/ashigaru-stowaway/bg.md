...
Payjoin е структура на транзакциите Bitcoin, предназначена да повиши поверителността на потребителите, като включва пряко сътрудничество с получателя на плащането. Съществуват няколко имплементации, които улесняват прилагането ѝ и автоматизират процеса. Най-известната от тях несъмнено е Stowaway, първоначално разработена от екипа на Samоurai Wallet, а сега интегрирана в неговата fork Ashigaru.
...
Отдалеч този метод става твърде сложен. За да поправи това, Samourai Wallet е разработил криптиран протокол за комуникация, базиран на Tor, наречен "*Soroban*". Благодарение на Soroban обменът, необходим за Payjoin, е автоматизиран и се извършва във фонов режим.
...
Накратко, Paynym е уникален идентификатор, свързан с вашия портфейл, който ви позволява да активирате различни функции, включително криптиран обмен. Той е под формата на идентификатор, придружен от илюстрация. Ето например този, който използвам за Testnet:
...
- `Stowaway` = имплементация на Payjoin, налична в Ashigaru;
...
- paynym" = Уникален идентификатор в портфейла, използван за установяване на връзка с друг потребител в "Soroban", за да се извърши транзакция в "Chahoots".
...
За да извършите транзакция в Stowaway, кликнете върху изображението на вашия Paynym в горния ляв ъгъл на екрана, след което отворете менюто `Collaborate`. Лицето, което участва в транзакцията заедно с вас, трябва да направи същото, освен ако не разменяте QR кодове лично.
...
Имате две възможности: изберете `Initiate`, ако сте изпращач на плащането, или `Participate`, ако сте получател на плащането.
...
Ако вие сте получателят, процедурата е много проста. За отдалечено сътрудничество чрез мрежата на Soroban кликнете върху `Participate`, изберете акаунта, който искате да използвате, след което натиснете `LISTEN FOR CAHOOTS REQUESTS`, за да изчакате искането, изпратено от платеца.
...
Ако сте избрали опцията `Online`, изберете получателя от Paynyms, които следвате.
...
След това кликнете върху `Review transaction setup`.
...
Ако желаете да извършите обмяната лично, изберете типа транзакция `STONEWALL X2`, след което изберете опцията `In Person / Manual`.
