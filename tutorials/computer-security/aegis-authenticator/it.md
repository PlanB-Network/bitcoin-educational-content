...
Una volta entrati nell'applicazione (l'interfaccia principale è inizialmente vuota), familiarizza con le opzioni di configurazione disponibili. Accedi alle impostazioni tramite il menu a discesa in alto a destra dello schermo (tre punti verticali), quindi seleziona "Impostazioni".

![Interface principale et paramètres](assets/fr/02.webp)

*Interfaccia principale Aegis vuota all'avvio, accesso al menu dei parametri e panoramica delle opzioni disponibili*
...
- **Comportamento**: configura il comportamento dell'applicazione quando interagisce con l'elenco di voci
...
- **Backup**: configura i backup automatici in una posizione a scelta
...
Dopo aver configurato Aegis, passiamo all'elemento fondamentale: aggiungere gli account a due fattori. Il processo è semplice e Aegis offre diversi metodi per integrare i codici di autenticazione.
...
Se preferisci o non puoi scansionare il codice QR, utilizza l'opzione "Inserisci manualmente". Il modulo consente di inserire:
...
**Autorizzazioni minime**: Aegis funziona offline per impostazione predefinita, richiedendo solo l'accesso alla fotocamera (scansione QR), alla biometria e alla vibrazione. Nessun dato viene raccolto o condiviso.
...
*Interfaccia completa di aggiunta account, avviso di backup, impostazioni di backup automatico e strategie di backup*
...
Aegis Authenticator è una soluzione eccellente per chi cerca un'applicazione 2FA trasparente, sicura e rispettosa della privacy. Il suo approccio open-source, unito a una robusta crittografia e a un'ordinata interfaccia, lo rende una scelta di prim'ordine per proteggere i tuoi account online.
...
La sicurezza dei tuoi account online non deve dipendere dalla buona volontà di aziende commerciali. Con Aegis, mantieni il controllo dei tuoi codici di autenticazione, in una cassaforte digitale di cui solo tu possiedi la chiave.

## Risorse

### Siti web ufficiali

- **Sito web ufficiale**: [getaegis.app](https://getaegis.app/) - Presentazione dell'app e download
