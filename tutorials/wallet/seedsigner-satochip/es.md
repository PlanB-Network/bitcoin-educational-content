...
El Satochip es un Hardware Wallet en formato de tarjeta inteligente, con un elemento de seguridad certificado EAL6+, uno de los estándares de seguridad más elevados. Está diseñada y fabricada por la empresa belga del mismo nombre: Satochip.



Con un precio de unos 25 euros, el Satochip destaca entre la competencia por su excelente relación calidad-precio. Gracias a su chip seguro, ofrece resistencia a los ataques físicos. Además, el código fuente de su applet es totalmente de código abierto, con licencia *AGPLv3*.
...
- El Satochip cuesta unos 25 euros, y como el applet es de código abierto, puedes instalarlo tú mismo en una tarjeta inteligente vacía. Luego hay que añadir el coste de los componentes de SeedSigner y la extensión para leer tarjetas inteligentes: dependiendo de dónde compres este hardware, el total debería oscilar entre 70 y 100 euros.
...
- Por último, haz una copia de seguridad del seed en un soporte físico y compruébala cuidadosamente.
...
Una vez generado el seed, es la única vez que reside en la memoria RAM del SeedSigner. En mi caso, quiero guardarlo en un [Seedkeeper](https://satochip.io/product/seedkeeper/), otro producto Satochip diseñado para almacenar secretos. Utilizaré este dispositivo como último recurso, en caso de pérdida de mi Satochip.



La estrategia de copia de seguridad elegida aquí depende de tus preferencias, pero es imprescindible tener al menos una copia de tu frase Mnemonic, ya sea en soporte físico (papel o metal) o, como aquí, en un Seedkeeper. También puede multiplicar el número de copias de seguridad según sus necesidades. Para obtener más información sobre las estrategias de copia de seguridad de la cartera, le sugiero que lea este tutorial :
...
Elige una `Etiqueta` para identificar fácilmente tus diferentes secretos almacenados en Seedkeeper. Puede, por ejemplo, mantener simplemente la huella digital del Wallet o indicar explícitamente `seed`. La elección depende de sus preferencias y riesgos.
...
El código PIN de Seedkeeper debe ser lo más largo y aleatorio posible, para evitar intentos de fuerza bruta en caso de compromiso físico de la tarjeta. También debe guardar una copia de seguridad de este código PIN, almacenada en una ubicación separada de Seedkeeper. Sin este PIN, no podrás acceder al Mnemonic almacenado en Seedkeeper, y tus bitcoins se perderán para siempre.



### 5.3. Guardar el seed en el Satochip



Ahora que tu cartera ha sido generada, guardada y comprobada, vamos a transferirla al Satochip. Para ello, asegúrate de que la seed está cargada en la RAM del SeedSigner. Luego ve a `Herramientas > Herramientas de Tarjeta Inteligente > Funciones de Satochip`.
...
Inserte su Satochip en el lector de tarjetas inteligentes y, a continuación, seleccione `Inicializar con seed`.
...
El dispositivo le pide que introduzca el código PIN de Satochip; como la tarjeta es nueva y no está inicializada, aún no existe ningún PIN. Introduce cualquier código para saltarte este paso (no es de bloqueo).
...
A continuación, puede configurar el código PIN Satochip, de 4 a 16 caracteres. Para reforzar la seguridad de su Wallet, elija un código largo y aleatorio: es la única protección contra el acceso físico a su frase Mnemonic.



No olvide guardar este PIN en cuanto lo cree, ya sea en un gestor de contraseñas seguro o en un soporte físico, según su estrategia personal. En este último caso, asegúrese de no guardar nunca el soporte que contiene el PIN en el mismo lugar que su Satochip, de lo contrario la protección será inútil. Es importante tener una copia de seguridad: **Sin este PIN, ya no podrás acceder a tu seed y tus bitcoins se perderán definitivamente**.
...
Si desea utilizar una passphrase BIP39 para mejorar la seguridad de su Wallet, consulte la parte 6 de este tutorial:
...
A continuación, el SeedSigner genera tu xpub en forma de código QR dinámico, que contiene todos los datos que necesitas para gestionar tu cartera en Sparrow wallet. Puedes ajustar el brillo de la pantalla mediante el joystick para facilitar el escaneo del código QR.
...
Asegúrese de que el software Sparrow wallet está instalado en su ordenador. Si no sabes cómo descargarlo, comprobar su autenticidad e instalarlo correctamente, consulta nuestro completo tutorial al respecto..:



https://planb.academy/tutorials/wallet/desktop/sparrow-c674e2ac-d46f-4c82-92a7-7d1b0e262f5d

En su ordenador, abra Sparrow wallet y, en la barra de menús, haga clic en `Archivo → Importar Wallet`.
...
Asigne un nombre a su cartera y haga clic en `Crear Wallet`. A continuación, Sparrow le pedirá que establezca una contraseña para bloquear el acceso local a esta Wallet. Elija una contraseña fuerte: protege sus datos en Sparrow (claves públicas, direcciones, etiquetas e historial de transacciones). Sin embargo, esta contraseña no es necesaria para restaurar la Wallet en el futuro: sólo lo será su frase Mnemonic (y posiblemente su passphrase).
...
Ahora compruebe que la `Huella maestra` que aparece en Sparrow wallet coincide con la que se encontró previamente en su SeedSigner.



A continuación, el SeedSigner le pedirá que escanee una Address de recepción aleatoria de su Sparrow wallet para confirmar la validez de la importación.
...
Su Satochip (a través de SeedSigner) y Sparrow wallet están ahora conectados de forma segura. Sparrow sirve como una gestión completa Interface, mientras que el Satochip sigue siendo el único dispositivo capaz de firmar sus transacciones. Ahora estás listo para recibir y enviar bitcoins en una configuración totalmente air-gapped.
...
Tu Satochip y Sparrow wallet están ahora configurados para trabajar juntos. En esta sección, explicaremos paso a paso cómo recibir y enviar bitcoins en este modo.
...
#### 7.1.1 Generar una recepción Address
...
Aparece un nuevo Bitcoin Address. Encontrará :




- La Address en formato de texto (empezando por `bc1q...` si está utilizando P2WPKH, como en este ejemplo) ;
- El código QR asociado ;
- Un campo `Label`, útil para rastrear sus transacciones.



Le recomiendo encarecidamente que añada una etiqueta a cada recibo de Bitcoin en su Wallet. Esto le ayudará a identificar fácilmente la procedencia de cada UTXO y a gestionar mejor su privacidad. Para saber más sobre este importante tema, consulta la formación dedicada en Plan ₿ Academy :
...
Su Address está ahora asociada a esta etiqueta en todas las secciones Sparrow.
...
Antes de comunicar su Address de recepción al pagador, es importante comprobar que pertenece a su seed. Este paso garantiza que su Satochip podrá firmar las transacciones asociadas a esta Address. También evita posibles ataques en los que Sparrow mostraría una Address fraudulenta. Tenga en cuenta que Sparrow se ejecuta en un entorno inseguro (su ordenador), cuya superficie de ataque es mucho mayor que la de su Satochip, que está totalmente aislada. Por eso, nunca debes fiarte ciegamente de las direcciones mostradas en Sparrow antes de comprobarlas en tu Hardware Wallet.



En Sparrow, haga clic en el código QR de Address para ampliarlo: aparecerá a pantalla completa.
...
Si el Address escaneado coincide con el derivado de su seed, el SeedSigner mostrará el mensaje `Address Verificado`.
...
Así podrá estar seguro de que la Address pertenece a su cartera.
...
Ahora puede transmitir esta Address en forma de texto o a través de su código QR a la persona o servicio que necesite enviarle Satss. Una vez emitida la transacción en la red, aparecerá en la pestaña `Transacciones` de Sparrow wallet.
...
En Sparrow wallet, puedes crear una transacción haciendo clic en la pestaña `Enviar` de la barra lateral izquierda. Sin embargo, yo prefiero utilizar la pestaña "UTXOs", que te permite practicar el "Control Coin". Este método ofrece un control preciso sobre los UTXOs gastados, para limitar la información revelada durante tus transacciones.
...
- En "Pagar a", pega el Address del destinatario o escanea su código QR con el icono de la cámara;
- En `Etiqueta`, añada una etiqueta para rastrear este gasto;
- En "Importe", introduzca el importe que desea enviar;
- Por último, seleccione la tasa de carga en función de las condiciones actuales de la red (las estimaciones están disponibles en [Mempool.space](https://Mempool.space/)).
...
El SeedSigner detecta que se trata de una PSBT y muestra un resumen de la transacción:
...
Una vez completada la firma, el SeedSigner genera un nuevo código QR que contiene la transacción firmada, listo para ser escaneado por Sparrow.
...
Ahora que la transacción está firmada y validada, sólo queda difundirla en la red Bitcoin para que un Miner pueda incluirla en un BLOCK. En Sparrow, haz clic en `Escanear QR`.
...
- Grabando la frase Mnemonic en un soporte físico;
...
La mejor opción es recomendar los componentes necesarios y reconstruir un nuevo SeedSigner desde cero. Al tratarse de un dispositivo "sin estado", no importa si utilizas el mismo u otro SeedSigner: mientras puedas insertar tu Satochip, todo funcionará con normalidad.



Si no quieres reconstruir uno, también puedes utilizar tu Satochip de la forma clásica, es decir, directamente desde tu ordenador, sin pasar por el SeedSigner. Este método funciona perfectamente, pero reduce considerablemente la seguridad de tu Bitcoin Wallet: pierdes el aislamiento "*air-gapped*" y ahora tienes que firmar a ciegas, ya que el SeedSigner actuaba como pantalla de confianza. Sin embargo, esta puede ser una solución temporal en caso de emergencia, o si no puede reconstruir un SeedSigner.



Para ello, necesitarás una tarjeta inteligente USB o un lector NFC. Abre la Wallet que deseas restaurar en Sparrow, luego ve a la pestaña `Configuración` y haz clic en `Reemplazar`.
...
Por último, introduce el PIN de tu tarjeta inteligente para desbloquearla. A continuación, podrá acceder a su Wallet, crear transacciones y firmarlas directamente utilizando el Satochip conectado.
...
- Introduzca su frase Mnemonic manualmente en el menú `Semillas > Introducir seed de 12 palabras`.
...
- O carga tu seed desde un Seedkeeper, a través del menú `Seeds > From SeedKeeper` (este es el método que estoy utilizando en este tutorial). Simplemente tendrás que introducir el código PIN del Seedkeeper y seleccionar el secreto que se utilizará como seed en el SeedSigner.
...
Una vez cargada la seed en el SeedSigner, sea cual sea el método que utilices, podrás firmar una o varias transacciones de escaneo para mover tus bitcoins a una nueva Wallet no comprometida. Para saber cómo hacerlo, consulta la parte 7.2 del siguiente tutorial:
