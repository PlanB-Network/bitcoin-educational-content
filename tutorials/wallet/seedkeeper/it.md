...
Seedkeeper è una smartcard sviluppata da Satochip, un'azienda belga specializzata in soluzioni hardware per la gestione e la protezione dei segreti digitali. Rinomata per la sua gamma di smartcard per l'ecosistema Bitcoin, Satochip ha progettato Seedkeeper come alternativa ai metodi tradizionali di memorizzazione delle frasi Mnemonic.



In concreto, Seedkeeper si presenta come una smart card multifunzionale, certificata EAL6, dotata di un processore sicuro e di una memoria a prova di manomissione (ovvero un "secure element*"). Come suggerisce il nome, il suo ruolo è quello di memorizzare frasi e password Bitcoin Mnemonic in modo criptato e protetto. Con Seedkeeper, è possibile generate, importare, organizzare e salvare i propri segreti direttamente nel componente sicuro della scheda.
...
- Memorizzazione di frasi Bitcoin** Mnemonic: invece di scrivere le vostre 12 o 24 parole su carta, potete importarle nella smartcard e proteggerle con un codice PIN.
- Gestione delle password**: è possibile utilizzare generate password forti tramite l'applicazione Seedkeeper e memorizzarle direttamente nella smartcard, ottenendo così un gestore di password offline comodo e facile da usare.
...
Seedkeeper è molto interessante anche se combinato con un Hardware Wallet stateless come SeedSigner o Specter DIY. In questo caso, non è necessario utilizzare il computer o il client mobile di Satochip. Seedkeeper mantiene il seed nel suo secure element e può essere utilizzato direttamente con il dispositivo di firma, eliminando la necessità di un codice QR cartaceo. Non svilupperò questo particolare caso d'uso in questa guida, poiché è oggetto di un'altra guida dedicata:
...
Rispetto a un semplice backup cartaceo della frase Mnemonic, l'utilizzo di un Seedkeeper presenta diversi vantaggi:
...
- Suddivisione del rischio su due fattori:** è possibile suddividere la sicurezza tra un fattore digitale e uno fisico. Ad esempio, se si memorizza il PIN di Seedkeeper nel gestore di password, per ottenere il seed è necessario sia l'accesso a questo gestore che il possesso fisico della smartcard (una probabilità di attacco notevolmente ridotta).
...
Tuttavia, ci sono diversi svantaggi rispetto a un semplice backup cartaceo del vostro seed:
...
- Dipendenza da un dispositivo informatico generico:** L'accesso e la gestione del seed richiedono un computer o uno smartphone, il che significa che il Mnemonic passa attraverso una macchina con una superficie di attacco molto più ampia rispetto al Hardware Wallet. Questo può rappresentare un rischio se la stazione di lavoro viene compromessa. Questo può rappresentare un rischio se la workstation viene compromessa. Per questo motivo non consiglio di utilizzare Seedkeeper per memorizzare il seed di un Hardware Wallet (tranne che per l'uso stateless senza computer, come con SeedSigner). Il ruolo del Hardware Wallet è proprio quello di memorizzare il seed in un ambiente minimalista e altamente sicuro. Inserendo manualmente il seed sul vostro computer abituale, esso non è più confinato nel Hardware Wallet: finisce anche su una macchina generica, esposta a molteplici vettori di attacco. Quindi è meglio usare Seedkeeper per un Hot Wallet piuttosto che per un Cold (tranne SeedSigner / Hardware Wallet stateless).





- Il rischio di perdita legato al PIN:** l'inaccessibilità diretta del seed, a differenza di un backup cartaceo, fornisce effettivamente una protezione contro il furto fisico. Ma come sempre, la sicurezza è un gioco di equilibri tra il rischio di furto e il rischio di perdita. Se il backup richiede un PIN, la perdita di questo codice renderà impossibile recuperare la frase del Mnemonic e quindi accedere ai bitcoin.



Alla luce di questi vantaggi e svantaggi, ritengo che i migliori utilizzi di Seedkeeper (a parte la sua funzione di password manager) siano, da un lato, l'archiviazione dei semi dei vostri **portafogli software**, dato che risiedono già sul vostro telefono o computer, o dei vostri Portafogli Hardware senza schermo come il Satochip, e dall'altro, l'utilizzo in combinazione con un Hardware Wallet senza stato come il SeedSigner, dove si rivela davvero efficace.
...
Selezionare "Importa segreto*". L'opzione "*Segreto generate*" consente di creare una nuova frase Mnemonic direttamente dall'applicazione.
...
Immettere quindi la frase di recupero nell'apposito campo. Se lo desiderate, potete anche aggiungere un passphrase BIP39 o i vostri *Descrittori*. Quindi fare clic su "Importa*".
...
*Il Mnemonic visualizzato in questa immagine è fittizio e non appartiene a nessuno. È solo un esempio. Non rivelate mai il vostro Mnemonic a nessuno, o i vostri bitcoin saranno rubati
...
## 6. Accedere al proprio seed su Seedkeeper



Se volete controllare la vostra frase Mnemonic, prendete il vostro Seedkeeper e fate clic sul pulsante "*Click & Scan*".
...
In questo modo si accede a un elenco di tutti i segreti registrati. In questo esempio, voglio visualizzare il seed nel mio portfolio "*BLOCKSTREAM App*", quindi faccio clic su di esso.
...
La frase Mnemonic precedentemente registrata viene ora visualizzata sullo schermo.
...
Si accede così alla pagina iniziale con l'elenco dei segreti. Fare clic sui tre puntini in alto a destra di Interface.
...
Questo è tutto! Ora sapete come usare Seedkeeper per memorizzare la frase Mnemonic di un Bitcoin Wallet. In un prossimo tutorial vedremo come utilizzare Seedkeeper per memorizzare le password. Vi invito anche a scoprire il suo uso combinato con SeedSigner :
...
In questo tutorial abbiamo menzionato più volte i ***Descrittori*** del vostro portafoglio Bitcoin. Non sapete cosa sono? In questo caso, vi consiglio di seguire il nostro corso di formazione gratuito CYP 201, che approfondisce tutti i meccanismi di funzionamento dei portafogli HD!
