...
Der SeedSigner ist ein Hardware Wallet, den Sie selbst aus Standardhardware zusammenbauen, in der Regel aus einem Raspberry Pi Zero. Dieser Wallet wird als "*zustandslos*" bezeichnet: Im Gegensatz zu den meisten anderen Modellen auf dem Markt (Coldcard, Trezor, Ledger usw.) speichert er keine Daten im permanenten Speicher, sondern arbeitet nur live aus dem RAM. Folglich wird der seed Ihres Portfolios niemals auf dem SeedSigner gespeichert. Bei jedem Neustart müssen Sie es ausfüllen, damit das Gerät Ihre Transaktionen signieren kann. Die gebräuchlichste Methode ist, Ihren seed als QR-Code zu speichern, den Sie dann jedes Mal scannen, wenn Sie ihn benutzen (*SeedQR*).
...
- Wenn Sie noch keinen Bitcoin Wallet mit Ihrem SeedSigner verbunden haben, müssen Sie die Schritte **5** und **6** des unten genannten Tutorials befolgen. In diesen Abschnitten wird erklärt, wie man einen generate Mnemonic Satz mit dem SeedSigner verknüpft, ihn über eine *SeedQR* speichert und dann diesen Wallet mit Sparrow wallet verbindet, um ihn zu verwalten. Ich werde hier nicht auf diese Verfahren eingehen, und **ich gehe davon aus, dass Sie bereits einen funktionierenden Bitcoin Wallet haben, der mit Sparrow und Ihrem SeedSigner konfiguriert ist**.
...
Um zu beginnen, öffnen Sie das Menü "Tools > Smartcard Tools" auf Ihrem SeedSigner.
...
Jetzt, wo Ihr Seedkeeper betriebsbereit ist, können Sie Ihren Bitcoin Wallet Mnemonic auf der Smartcard speichern. Um zu beginnen, schalten Sie Ihren SeedSigner wie gewohnt ein und scannen dann den *SeedQR* Ihres Wallet, um ihn in das Gerät zu laden. Sobald der seed importiert wurde, wählen Sie einfach "Fertig".
...
Wenn das seed geladen ist, rufen Sie das Menü "seed sichern" auf.
...
Wählen Sie nun den neuen PIN-Code Ihres Seedkeepers, der zwischen 4 und 16 Zeichen lang ist. Für zusätzliche Sicherheit wählen Sie einen langen, zufälligen Code: Er ist die einzige Barriere, die den physischen Zugang zu Ihrem Mnemonic-Satz schützt.
...
Sie können dann ein "Label" definieren, das mit Ihrer Mnemonic-Phrase verbunden ist. Dieses Label ist nützlich, wenn Sie mehrere Geheimnisse auf Seedkeeper speichern, damit Sie sie leicht identifizieren können.
...
Ihr Mnemonic-Satz ist nun auf der Smartcard gespeichert.
...
- Eine zweite Kopie Ihres unverschlüsselten Mnemonic-Satzes, die als ultimative Sicherung Ihres Portfolios dient und nur bei Verlust oder Diebstahl des Seedkeepers zu verwenden ist. Da diese Version unverschlüsselt ist, muss sie an einem separaten, sichereren Ort aufbewahrt werden, um eine gleichzeitige Kompromittierung der beiden Backups zu verhindern.



Je nach Ihrer Schutzstrategie und Ihrem Risikoprofil können Sie den seed auch auf mehreren verschiedenen Seedkeepern duplizieren oder mehrere physische Kopien des Mnemonic erstellen. Um mehr über diese Praktiken zu erfahren, sehen Sie sich das folgende Tutorial an:
...
Sie können nun Ihren Seedkeeper verwenden, um Ihre Mnemonic Phrase beim Start in den SeedSigner zu laden und so Ihre Bitcoin Transaktionen zu signieren. Um zu beginnen, schalten Sie Ihren SeedSigner ein, indem Sie ihn einstecken und dann das Menü "Seeds" öffnen.
...
Sie können Ihren Wallet dann wie gewohnt nutzen: Überprüfen Sie Ihre Lieferadressen und unterschreiben Sie Transaktionen, genau wie mit einem klassischen SeedSigner. Um mehr über die Verwendung zu erfahren, sehen Sie sich das entsprechende Tutorial an:
...
Verwenden Sie einen passphrase, um Ihr Bitcoin-Portfolio zu schützen? Sie können ihn auch in Ihrem Seedkeeper registrieren, zusammen mit Ihrem seed. Diese Lösung ermöglicht es Ihnen, Ihr Wallet schnell auf den SeedSigner zu laden, ohne das passphrase jedes Mal manuell auf der kleinen Tastatur eingeben zu müssen, wenn Sie es benutzen.
...
- Bewahren Sie Ihr seed und passphrase in einem Seedkeeper auf, der durch einen starken PIN-Code geschützt ist (dies ist wichtig). Mit dieser Sicherung können Sie Ihr Wallet problemlos täglich benutzen. Wenn Sie möchten, können Sie diese Informationen auf einem zweiten Seedkeeper duplizieren;
- Bewahren Sie außerdem eine eindeutige Kopie Ihres Mnemonic und passphrase auf, auf Papier oder Metall. Dies ist der letzte Ausweg, falls Sie Ihren Seedkeeper oder seine PIN verlieren. Bewahren Sie diese Kopien an verschiedenen Orten auf, damit sie nicht gleichzeitig gefährdet werden können.



Wenn in dieser Konfiguration jemand nur Ihren Klartext Mnemonic in die Hände bekommt, kann er nichts stehlen, ohne den passphrase zu kennen (vorausgesetzt natürlich, er ist stark genug, um einem Brute-Force-Angriff standzuhalten). Umgekehrt bleibt Ihr Klartext passphrase ohne die entsprechende Mnemonic-Phrase unbrauchbar, wenn jemand ihn entdeckt.



Sollte es jemandem gelingen, sich physischen Zugang zu Ihrem Seedkeeper zu verschaffen, in dem sich der seed und der passphrase befinden, kann er ohne Kenntnis des PIN-Codes nichts entnehmen. Anders als beim passphrase kann dieser Code nicht geknackt werden, da sich die Smartcard nach fünf ungültigen Versuchen automatisch sperrt.
...
- Ein **passphrase stark**: Er muss lang und zufällig sein und eine Vielzahl von Zeichen enthalten. Seine Komplexität stellt für Sie kein Problem dar, da Sie ihn nur einmal während der Initialisierung auf der Tastatur eingeben müssen und er dann von Seedkeeper übertragen wird;
- Ein **starker PIN-Code** für Seedkeeper: ebenfalls zufällig und aus 16 Zeichen bestehend.



Um dieses Setup einzurichten, laden Sie zunächst Ihren passphrase auf die übliche Weise in den SeedSigner. Sie können dem in diesem Tutorial beschriebenen Verfahren folgen:
...
Klicken Sie dann auf "seed sichern", legen Sie den Seedkeeper in das Laufwerk ein und wählen Sie "An SeedKeeper".
...
Geben Sie Ihre PIN ein, um Seedkeeper zu entsperren, und weisen Sie diesem Geheimnis eine Bezeichnung zu. Sie können den Fingerabdruck als Kennzeichnung belassen, um eine Form der plausiblen Bestreitbarkeit aufrechtzuerhalten, oder Sie können z. B. ausdrücklich "passphrase Wallet" angeben.
...
Geben Sie Ihren PIN-Code ein, um die Smartcard zu entsperren, und wählen Sie dann das Wallet aus, das Ihrem passphrase entspricht.
...
Sie können jetzt mit passphrase auf Ihr Portfolio zugreifen und Ihre Transaktionen wie bei einem SeedSigner unterschreiben.
...
- Im Menü "SeedKeeper-Funktionen" können Sie :
 ...
 - Speichern oder laden Sie Ihre Deskriptoren (nützliche Funktion für Multisig-Portfolios).
