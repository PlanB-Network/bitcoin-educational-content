...
Например, хеш-функция SHA256 производит хеш фиксированной длины 256 бит. Таким образом, если мы используем вход "_Plan ₿_", сообщение произвольной длины, сгенерированный хеш будет следующим 256-битным отпечатком:
...
В данном примере, получение хеша `24f1b9…` зная вход "_Plan ₿_" просто и быстро. Однако, найти сообщение "_Plan ₿_" зная только `24f1b9…` невозможно.
...
Если вернуться к нашему примеру с входным значением "_Plan ₿_" и функцией SHA256, мы видели, что сгенерированный хеш выглядит следующим образом:
...
Битовая последовательность $\text{ENT} \Vert \text{CS}$ затем делится на сегменты по 11 бит. Каждый 11-битный сегмент, после преобразования в десятичное число, соответствует числу между 0 и 2047, которое обозначает позицию слова [в списке из 2048 слов, стандартизированном BIP39](https://github.com/Plan ₿-Network/bitcoin-educational-content/blob/dev/resources/bet/bip39-wordlist/assets/BIP39-WORDLIST.pdf).
...
| Префикс Base 58 | Префикс Base 16 | Сеть     | Назначение            | Ассоциированные Скрипты | Производная               | Тип Ключа   |
|-----------------|-----------------|----------|-----------------------|--------------------------|---------------------------|-------------|
| `xpub`          | `0488b21e`      | Mainnet  | Legacy и SegWit V1    | P2PK / P2PKH / P2TR      | `m/44'/0'`, `m/86'/0'`    | публичный   |
| `xprv`          | `0488ade4`      | Mainnet  | Legacy и SegWit V1    | P2PK / P2PKH / P2TR      | `m/44'/0'`, `m/86'/0'`    | приватный   |
| `tpub`          | `043587cf`      | Testnet  | Legacy и SegWit V1    | P2PK / P2PKH / P2TR      | `m/44'/1'`, `m/86'/1'`    | публичный   |
| `tprv`          | `04358394`      | Testnet  | Legacy и SegWit V1    | P2PK / P2PKH / P2TR      | `m/44'/1'`, `m/86'/1'`    | приватный   |
| `ypub`          | `049d7cb2`      | Mainnet  | Nested SegWit         | P2WPKH in P2SH           | `m/49'/0'`                | публичный   |
| `yprv`         | `049d7878`         | Mainnet  | Вложенный SegWit        | P2WPKH в P2SH           | `m/49'/0'`                 | private     |
| `upub`         | `049d7cb2`         | Testnet  | Вложенный SegWit        | P2WPKH в P2SH           | `m/49'/1'`                 | public      |
| `uprv`         | `044a4e28`         | Testnet  | Вложенный SegWit        | P2WPKH в P2SH           | `m/49'/1'`                 | private     |
| `zpub`         | `04b24746`         | Mainnet  | SegWit V0            | P2WPKH                   | `m/84'/0'`                 | public      |

Эта таблица предоставляет полный обзор префиксов, используемых в расширенных ключах, детализируя их префиксы в base 58 и base 16, сеть, с которой они ассоциированы (Mainnet или Testnet), их назначение, ассоциированные с ними скрипты, их путь производного ключа и являются ли они публичными или приватными ключами.
...
\begin{array}{|c|c|}
\hline
\text{5-битные группы} & \text{Десятичное значение} \\
\hline
10011 & 19 \\
11110 & 30 \\
00000 & 0 \\
...
Хеш ветви \(\text{h}_{\text{branch}}\) вычисляется как функция хеширования с тегом `TapBranch`, применяемая к конкатенации хешей листьев \(\text{h}_{\text{leaf1}} \Vert \text{h}\_{\text{leaf2}}\):
...
После вычисления корня Меркла \(h*{\text{root}}\) мы можем вычислить модификацию. Для этого мы конкатенируем внутренний публичный ключ кошелька \(P\) с корнем \(h*{\text{root}}\), а затем пропускаем все через функцию хеширования с тегом `TapTweak`:

\[
t = \text{H}_{\text{TapTweak}}(P \Vert h_{\text{root}})
\]

Наконец, как и ранее, публичный ключ Taproot \(Q\) получается путем добавления внутреннего публичного ключа \(P\) к произведению модификации \(t\) на точку генератора \(G\):
