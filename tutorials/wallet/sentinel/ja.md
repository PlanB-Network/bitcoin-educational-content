...
現在、最も包括的なwatch-onlyアプリケーションは[Sentinel](https://github.com/wanderingking072/sentinel-android)であり、当初はSamourai Walletのチームによって開発され、現在はコミュニティによって維持されています。優れたwatch-onlyウォレットに不可欠なすべての機能を備えています。
...
### インストールとセットアップ
まずSentinelアプリケーションをインストールしてください。[プロジェクトのgithubリポジトリでダウンロード可能なAPK](https://github.com/wanderingking072/sentinel-android/releases)を使用して行うことができます。





次に、Samourai Walletのサーバーが利用できなくなったため、必ず自分自身のDojoに接続する必要があります。まだ自分のDojoをお持ちでない場合は、サイト[The Dojo Bay](https://dojobay.pw/)でコミュニティが提供しているDojoを使用するか、独自のDojoをインストールするためのチュートリアルに従ってください :
