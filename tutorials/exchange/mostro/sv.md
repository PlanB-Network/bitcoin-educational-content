---
name: Mostro
description: KYC-fri Bitcoin P2P Exchange-plattform via Lightning och Nostr
---
...
Hur förvärvar eller säljer du bitcoins utan att äventyra din ekonomiska suveränitet? Centraliserade plattformar inför invasiva KYC-förfaranden som exponerar dina personuppgifter, med möjlighet till godtycklig frysning av konton eller statlig övervakning.



**Mostro P2P** erbjuder ett decentraliserat alternativ som kombinerar Lightning Network, Nostr-protokollet och hold-fakturor. Dess viktigaste innovation: ett automatiserat spärrsystem där dina bitcoins förblir under din kontroll under hela Exchange, vilket eliminerar risken för stöld, Exchange-konkurs eller godtycklig konfiskering.
...
** Betalningsflexibilitet**: Stöd för många fiat-valutor via yadio.io Exchange-räntetjänsten. Accepterar SEPA-överföringar, PayPal, Revolut, kontanter och alla metoder som överenskommits mellan parterna.
...
Mostro erbjuder två huvudsakliga operativa klienter för dina P2P-utbyten.
...
**Mostro CLI** är den mest mogna och funktionella klienten. Den är skriven i Rust och erbjuder hela utbudet av Mostro-funktioner via en Interface-kommandorad. Kompatibel med macOS, Linux och Windows.
...
- `release`: Frigör spärren och slutför Exchange
- `getdm`: Visa direktmeddelanden
- `rate` : Utvärdera din motpart efter en Exchange
...
Den **mobila appen** i alpha-version möjliggör handel med P2P direkt från din smartphone. Intuitiv grafisk Interface med navigering i flikar, ordervisning, avancerade filter och integrerad chatt för att kommunicera med dina motparter.
...
- Valfritt men rekommenderat**: VPN- eller Tor-anslutning för att maskera din IP Address
...
**Viktigt för synkronisering mellan CLI/Mobile**: För att få tillgång till samma order på CLI och i mobilappen måste du använda **samma Mostro Pubkey** och **samma Nostr reläer** i båda klienterna. Kontrollera dessa inställningar i Inställningar i mobilappen.
...
- Nätverksskydd**: Aktivera VPN eller Tor för att dölja din IP Address
- Wallet säker**: Aktivera det automatiska låset på din Wallet Lightning
...
Mostro kommer att be dig att tillhandahålla en **Lightning Invoice** för att ta emot BTC. Det exakta beloppet i satoshis kommer att anges (här: 4715 Sats).
...
### Steg 3: Ange din Lightning Invoice



generate en Lightning Invoice från din Wallet (Phoenix, Breez, etc.) för det exakta beloppet och skicka det sedan :
...
Systemet bekräftar försändelsen och informerar dig om att du ska vänta på att säljaren betalar Invoice-avgiften innan du aktiverar spärren.
...
Så snart säljaren bekräftar mottagandet av fiat och frigör spärren med kommandot `release` får du omedelbart dina bitcoins på den Lightning Invoice du levererade.
...
Tryck på Green **"+"**-knappen längst ned till höger och välj sedan **SELL** (röd knapp).
...
**Viktig instruktion**: Du måste nu **betala Invoice hold** som visas för att låsa dina bitcoins (här 4674 Sats för 1-5 EUR) i spärren. Du har **15 minuter max** annars kommer Exchange att annulleras.



Kopiera Invoice-spärren och betala den från din Wallet Lightning (Phoenix, Breez, etc.).
...
När du har ** bekräftat mottagandet ** av fiat-betalningen på ditt konto, tryck på Green **RELEASE ** -knappen och bekräfta att skicka Satss till köparen.
...
Bitcoins överförs omedelbart till köparen via hans Lightning Invoice.
...
4. Tillhandahåll din Lightning Invoice (genererad från din Wallet)
...
9. Ta emot bitcoins direkt på din Lightning Invoice
...
**Finansiell suveränitet**: Dina bitcoins lämnar aldrig din direkta kontroll tack vare Invoice-hållmekanismen, vilket eliminerar risken för Exchange-konkurs eller piratkopiering.
