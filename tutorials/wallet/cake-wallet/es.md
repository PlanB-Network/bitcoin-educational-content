...
- [**Pagos Silenciosos (BIP-352)**](https://BIPs.dev/352/) mejorando los anteriores [BIP 47 códigos de pago](https://silentpayments.xyz/docs/comparing-proposals/bip47/) también llamados "PayNyms" con direcciones ocultas reutilizables. Cuando un remitente utiliza su Address de pago silencioso, su Wallet deriva un Address único de una sola vez utilizando diferentes claves que se combinarán en un Taproot Address único de una sola vez. Los registros Blockchain muestran transacciones no relacionadas, impidiendo la vinculación de los pagos entrantes. Los pagos silenciosos ofrecen una serie de ventajas, entre las que se incluyen:
    - Direcciones reutilizables: No es necesario generate una nueva Address para cada transacción, lo que proporciona una mejor experiencia de usuario y una mayor privacidad
    - Aumento cero de los costes: Los pagos silenciosos no aumentan el tamaño ni el coste de las transacciones.
    - Mayor anonimato: Los observadores externos no pueden vincular las transacciones a un Address de Pago Silencioso.
    - No se requiere interacción entre emisor y receptor: Las transacciones pueden realizarse sin comunicación entre las partes.
    - Direcciones únicas para cada pago: Eliminación del riesgo de reutilización accidental de Address.
...
- RBF** (Sustituir por tasa) le permite ajustar la tasa después de enviar una transacción.


## 1️⃣ Configuración de su Wallet


Cake Wallet ofrece una amplia gama de plataformas compatibles. Puede elegir entre `Android`, `iOS / macOS` , `Linux` y `Windows`.  Para empezar, visite https://docs.cakewallet.com/get-started/ y seleccione su sistema operativo.
...
En la siguiente pantalla podrás elegir entre una amplia gama de criptomonedas. Selecciona `Bitcoin` y pulsa en `Next` e introduce un `Wallet name` para identificar el Wallet. Al pulsar sobre "Configuración avanzada" aparecerá una serie de "Ajustes de privacidad". Realice estos cambios:



- Fiat API:** seleccione `Sólo Tor` (enruta las solicitudes de precios a través de Tor)
- Swap:** seleccione `Tor Only` (anonimiza el tráfico Exchange)
...
Para conocer las mejores prácticas para guardar su frase Mnemonic, consulte este tutorial:
...
- Saldo disponible
- Tarjeta de Pagos Silenciosos para empezar a escanear las transacciones enviadas a su Address de Pagos Silenciosos
- Tarjeta PayJoin para `Habilitar` PayJoin como función de preservación de la intimidad y ahorro de tasas
...
Aquí podemos reconectar la Wallet, gestionar nodos y conectarnos a nuestro propio nodo (recomendado). El `Silent Payments Scanning` nos permite personalizar el escaneo especificando `Scan from BLOCK height` o `Scan from date`.
...
Podemos activar la tarjeta de Pagos Silenciosos en la pantalla de inicio para mostrar esta función. Si activamos "Escanear siempre", Wallet monitorizará continuamente Blockchain en busca de Pagos Silenciosos entrantes. Podemos especificar parámetros de escaneo para personalizar el proceso de escaneo a nuestras necesidades como se ha descrito anteriormente.
...
Para proteger nuestra Wallet, podemos crear una copia de seguridad siguiendo las instrucciones de la aplicación. Así nos aseguraremos de tener una copia segura de nuestras claves privadas, lo que nos permitirá recuperar nuestra Wallet si la perdemos o nos la roban. Además, podemos ver nuestra frase seed y nuestras claves privadas, cambiar nuestro PIN, activar la autenticación biométrica, Firmar / Verificar y configurar 2FA para un Layer extra de protección.
...
Hay varias opciones y tipos de Address disponibles para recibir Bitcoin. `SegWit (P2WPKH)` *(empezando por bc1q....)* es la opción por defecto.  Seleccionemos `Pagos silenciosos` en este ejemplo.


Para recibir un pago silencioso, pulse primero el icono "Recibir" en Cake Wallet. A continuación, introduzca el importe que espera recibir. A continuación, introduzca la cantidad que espera recibir. Para especificar el tipo de Address, vuelva a pulsar "Recibir" en la parte superior de la pantalla y seleccione "Pagos silenciosos" entre las opciones.


En la pantalla principal, aparecerá su código QR de Pago Silencioso reutilizable y la Address. Como era de esperar, la Address es bastante larga:
...
Ahora, utilice una Wallet compatible con BIP-352 (como Blue Wallet) para escanear este código QR y enviar el pago. Verás que la Wallet deriva una Address de destino única de tu Address silenciosa.
...
Como Blue Wallet sólo puede `Enviar` Pagos Silenciosos, utilizaremos otra BIP 352 compatible como parte receptora. Este proceso es idéntico al de una transacción Bitcoin normal.



- Pulse `Enviar` en la pantalla de inicio
- pegando nuestro código reutilizable `sp1qq...` Address o escaneando el código QR directamente en la aplicación.
...
Una vez que hemos introducido la `sp1qq...` Address, la Wallet deriva automáticamente una correspondiente `bc1p...` Taproot Address (P2TR) en segundo plano, que se utilizará para el Pago Silencioso.
...
Una vez enviada la transacción, se le preguntará si desea añadir este contacto a su libreta Address.
...
Para utilizar PayJoin ambas partes necesitan un Wallet compatible con PayJoin, y el destinatario necesita tener al menos un Coin o salida en su Wallet. Para empezar, siga estos pasos:
...
3.  Pulse "Recibir" en la pantalla de inicio y aparecerá un código QR PayJoin y un botón de copia (si selecciona SegWit)


![image](assets/en/16.webp)


## 7️⃣ Otras características
...
Este es nuestro análisis de Cake Wallet, que ofrece la práctica privacidad de Bitcoin gracias a funciones como los pagos silenciosos (BIP-352) y PayJoin v2.


Los Pagos Silenciosos sustituyen las direcciones desechables por direcciones ocultas reutilizables para impedir la On-Chain vinculación de las transacciones entrantes. Aunque los problemas de sincronización de versiones anteriores han mejorado notablemente, se requieren algunos requisitos computacionales mayores para escanear y detectar los Pagos Silenciosos, lo que exige más recursos y ancho de banda.


PayJoin v2 altera el análisis de las cadenas al fusionar las entradas del remitente y el destinatario en transacciones únicas sin tasas adicionales ni coordinación central. Esto rompe la heurística de entrada común-Ownership, lo que supone una ventaja significativa, ya que significa que no se puede asumir que todas las entradas pertenecen al remitente.


Para los usuarios que dan prioridad al anonimato financiero, Cake Wallet es una opción viable. Incorpora protocolos de privacidad directamente en su funcionalidad principal, haciéndolos accesibles sin ninguna complejidad técnica. A medida que aumenta la vigilancia sobre las cadenas de bloques públicas, herramientas como ésta ayudan a mantener la privacidad de las transacciones donde más importa. Una implantación más amplia de estas normas en el panorama de Wallet sería un avance bienvenido.
