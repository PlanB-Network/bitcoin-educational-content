...
objectives:
  ...
  - Понимать методы ончейн анализа и разрабатывать стратегии защиты
---
...
В то время как предыдущие разделы сосредоточены на решениях для конфиденциальности на уровне приложений, этот шестой раздел будет погружаться в вопросы конфиденциальности на уровне Биткойн Core для конфиденциальности пользователей. Мы обсудим конфиденциальность на уровне сети узлов и вещание транзакций. Также мы обсудим различные протоколы, которые были предложены на протяжении многих лет для улучшения конфиденциальности пользователей в Биткойн, включая протоколы с фиксированным адресом. В заключение мы рассмотрим влияние на конфиденциальность, как положительное, так и отрицательное, последнего крупного мягкого форка Биткойн, Taproot.
...
**> Напоминание:** Сатоши, часто сокращенно sat, является самой маленькой единицей Биткойн, сравнимой с центом в фиатных валютах.
...
Таким образом, каждая транзакция Биткойн состоит из одного или нескольких входов и одного или нескольких выходов. Входы - это UTXO, потребляемые транзакцией для генерации выходов. Выходы - это новые UTXO, которые будут использоваться в качестве входов для будущих транзакций.
...
Coinbase - это специфический тип транзакции Биткойн, уникальный для каждого блока и всегда первый в них. Он позволяет майнеру, который нашел действительное доказательство работы, получить свою награду за блок. Эта награда состоит из двух элементов: **субсидии за блок** и **комиссий за транзакции**, о которых мы говорили в предыдущей части.
...
### Предотвращение двойного расходования на Биткойн
...
Одна из целей анализа цепочек - это группировка различных активностей на Биткойн с целью определения уникальности пользователя, который их осуществил. Впоследствии становится возможным попытаться связать этот набор активностей с реальной личностью.
...
Действительно, для лучшего противодействия анализу блокчейна, что может быть лучше, чем ознакомление с методами, используемыми в анализе блокчейна? Если вы хотите знать, как улучшить свою конфиденциальность на Биткойн, вы должны понимать эти методы. Это позволит вам лучше освоить техники, такие как [coinjoin](https://planb.network/fr/tutorials/privacy/coinjoin-samourai-wallet) или [payjoin](https://planb.network/fr/tutorials/privacy/payjoin) (техники, которые мы изучим в последних частях обучения), и сократить количество ошибок, которые вы могли бы совершить.
...
Следует отметить, что первые две эвристики для анализа цепочек были обнаружены самим Сатоши Накамото. Он обсуждает их в части 10 Белой книги Биткойн. Это:
- эвристика общего владения входами (CIOH);
- и повторное использование адресов.

![BTC204](assets/notext/31/6.webp)

Источник: S. Nakamoto, "Биткойн: A Peer-to-Peer Electronic Cash System", https://Биткойн.org/Биткойн.pdf, 2009.
...
Например, вот транзакция Биткойн, которая следует модели sweeping:
...
Это одна из самых известных эвристик среди энтузиастов Биткойн. Повторное использование адресов позволяет устанавливать связь между различными транзакциями и различными UTXO. Это наблюдается, когда адрес для получения Биткойн используется несколько раз.
...
Источник: С. Накамото, "Биткойн: Электронная пиринговая система денежных переводов", https://Биткойн.org/Биткойн.pdf, 2009.
...
- *Серия из четырех статей под названием: [Understanding Биткойн Privacy with OXT](https://medium.com/oxt-research/understanding-Биткойн-privacy-with-oxt-part-1-4-8177a40a5923), подготовленная кошельком Samourai в 2021 году;*
...
Изучив техники, которые могут нарушить вашу конфиденциальность в Биткойн, в этой третьей части мы теперь рассмотрим лучшие практики, которые следует принять для вашей защиты. Эта часть не направлена на изучение методов улучшения конфиденциальности, тема, которая будет рассмотрена позже, а скорее на понимание того, как правильно взаимодействовать с Биткойн, чтобы поддерживать конфиденциальность, которую он естественно предлагает, без использования дополнительных техник.
...
Источник: С. Накамото, "Биткойн: A Peer-to-Peer Electronic Cash System", https://Биткойн.org/Биткойн.pdf, 2009.
...
Повторное использование адресов является настоящим бичом для Биткойн. Согласно сайту OXT.me (в настоящее время недоступен), общий уровень повторного использования адресов в Биткойн составлял около 52% в 2022 году:
...
В этой главе мы изучим эти функции и увидим, как вы могли бы использовать их разумно, не добавляя слишком много работы, чтобы значительно оптимизировать вашу конфиденциальность в Биткойн.
...
В предыдущих разделах мы обсуждали ручной выбор UTXO для транзакции. Но что происходит, когда программное обеспечение кошелька делает этот выбор автоматически? Существует несколько методов для определения, какие монеты использовать, и выбор UTXO является настоящим полем исследований в Биткойн. Основная цель этого автоматического процесса часто заключается в минимизации комиссий за транзакцию для пользователя.
...
Если вы хотите узнать, как маркировать свои UTXO, мы подготовили полный учебник по основному существующему программному обеспечению Биткойн-кошелька. Найдите его [нажав здесь](https://planb.network/tutorials/privacy/utxo-labelling).

## KYC и ключевая идентификация
<chapterId>cec6b9d9-0eed-4f85-bc4e-1e9aa59ca605</chapterId>
KYC означает "Узнай своего клиента" (Know Your Customer), что является регуляторной процедурой, реализуемой некоторыми компаниями, работающими в секторе Биткойн. Эта процедура направлена на проверку и запись личности их клиентов с заявленной целью борьбы с отмыванием денег и финансированием терроризма.
...
Эта процедура не лишена рисков для конфиденциальности и безопасности пользователей. В этой главе мы подробно рассмотрим эти риски и проанализируем конкретные последствия KYC и процессов идентификации для конфиденциальности пользователей Биткойн.
...
Действительно, мы уже доверяем много наших личных данных частным компаниям. Однако здесь риск двойной, поскольку эти данные не только позволяют вас идентифицировать, но и связаны с деятельностью на Биткойн. Действительно, когда хакеру удается получить доступ к данным клиентов платформы обмена, они могут с уверенностью предположить, что эти клиенты владеют биткоинами. Таким образом, этот риск усиливается тем фактом, что биткоин, как и любой другой ценный актив, привлекает внимание воров.
...
С точки зрения личной безопасности, этот метод покупки значительно безопаснее, чем обмен наличными. Однако, как было сказано ранее, онлайн P2P обмены оставляют больше следов, чем физический обмен, что может негативно сказаться на конфиденциальности в Биткойн. Используя онлайн метод платежа в фиате, например, через банк, вы раскрываете больше информации, которая может облегчить идентификацию ключей.
...
Как мы видели в предыдущей главе, такой тип платформ избавляет вас от рисков, связанных с процедурами KYC, но они представляют более высокий уровень риска для ключевой идентификации. С точки зрения конфиденциальности в Биткойн, эти платформы, следовательно, предлагают лучшую защиту, чем методы покупки с KYC, но они менее интересны, чем P2P обмены.
...
Принцип консолидации заключается именно в использовании периодов, когда комиссии в Биткойн низкие, для объединения своих мелких UTXO в один большой. Таким образом, когда комиссии в Биткойн увеличиваются, можно совершать транзакции с минимальным количеством входов и, следовательно, тратить меньше в абсолютном выражении комиссий. Цель состоит в том, чтобы планировать обязательные транзакции на периоды высоких комиссий.
...
### Полноценный узел
Владение вашими биткоинами в личном хранении - это хорошо, но использование собственного полноценного узла - еще лучше! Вот почему наличие собственного узла критически важно для полностью суверенного использования Биткойн:
...
- **Техническое образование**: Управление полноценным узлом - отличный способ углубить ваши технические знания о Биткойн.
...
Первый совет, который я мог бы дать, - это смешиваться с самой плотной толпой. На Биткойн это означает использование наиболее распространенных сценариев скриптов. Например, скрипты P2WSH, часто используемые для конфигураций мультисигнатуры SegWit V0, очень редки. Они не позволяют вам скрыться в большом анонимном наборе. То же самое касается старых моделей, таких как P2PKH или P2SH. Хотя они широко представлены в наборе UTXO, они используются все реже для новых транзакций.
...
Coinjoin часто считается наиболее эффективным методом защиты конфиденциальности пользователей Биткойн. Но что же такое транзакция coinjoin? Давайте узнаем вместе.
...
Таким образом, конфиденциальность в Биткойн нарушается, когда внешние наблюдатели умудряются ассоциировать конкретные UTXO с идентифицированными пользователями. Как только эта ассоциация установлена, становится возможным проследить их транзакции и анализировать историю их биткойнов. Coinjoin - это именно техника, разработанная для нарушения прослеживаемости UTXO, чтобы предложить определенный уровень конфиденциальности пользователям Биткойн на уровне транзакций.
...
Именно поэтому на Биткойн развились координаторы coinjoin. Их роль заключается в соединении различных пользователей и передаче необходимой информации для успешного завершения совместной транзакции.
...
Исходной точкой любого coinjoin Whirlpool является **депозитный аккаунт**. Этот аккаунт автоматически используется, когда вы создаете новый Биткойн кошелек. На этот аккаунт должны быть зачислены биткоины, которые вы хотите смешать.
...
Теперь, когда мы подробно обсудили coinjoins, мы исследуем другие доступные на Биткойн техники обеспечения конфиденциальности в последнем разделе нашего обучения. Мы рассмотрим payjoins, специфические типы транзакций псевдо-coinjoins, протоколы с фиксированным адресом, а также меры, направленные на повышение конфиденциальности не на уровне транзакций, а на уровне сети узлов.
...
Payjoin - это специфическая структура транзакции Биткойн, которая повышает конфиденциальность пользователя во время расходования путем сотрудничества с получателем платежа.
...
* Алиса транслирует эту подписанную транзакцию $m_B$ в блокчейн Биткойн, чтобы восстановить обещанный Бобом BTC. Когда Боб видит эту транзакцию в блокчейне, он может извлечь подпись $s_B = s_B' + t$. С этой информацией Боб затем может выделить знаменитый секрет $t$, который ему был нужен:
$$t = (s_B' + t) - s_B' = s_B - s_B'$$
...
Стоит отметить, что обмен монет был впервые предложен [Грегори Максвеллом в октябре 2013 года на БиткойнTalk](https://Биткойнtalk.org/index.php?topic=321228.0).
...
Транспорт P2P V2 был включен как опция (по умолчанию отключена) в версии 26.0 Биткойн Core, развернутой в декабре 2023 года. Затем он был включен по умолчанию в версии 27.0 в апреле 2024 года. Его можно изменить с помощью опции `v2transport=` в файле конфигурации.
...
Однако, работа Биткойн отличается: необходимо генерировать новый адрес для получения для каждой входящей транзакции. Этот компромисс между удобством использования и конфиденциальностью восходит к самому началу Белой книги Биткойн. С публикации первой версии его документа в конце 2008 года, Сатоши Накамото уже предупреждал нас об этом риске:
...
Изначально BIP47 был предложением, сформулированным для интеграции в Биткойн Core, но оно так и не было принято. Некоторые программные продукты все же решили самостоятельно реализовать его на уровне приложения. Так, команды Samourai Wallet разработали собственную реализацию BIP47 под названием "PayNym".
...
- Алиса использует этот общий секрет $s$ для вычисления адреса приема платежей в Биткойн. Сначала она проверяет, содержится ли $s$ в пределах порядка кривой secp256k1. Если нет, она увеличивает индекс публичного ключа Боба для вычисления другого общего секрета;
...
Давайте начнем с простого примера, который поможет вам понять основное функционирование SP (Silent Payments, Тихих Платежей). Возьмем Алису и Боба, двух пользователей Биткойн. Алиса хочет отправить биткоины Бобу на новый адрес получения. В этом процессе должны быть достигнуты три цели:
...
Эти пары получают следуя этим путям (*Биткойн Mainnet*):
...
Поздравляю с завершением этого обучения по конфиденциальности в Биткойн!
