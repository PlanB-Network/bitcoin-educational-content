---
name: SeedSigner
description: Hardware Wallet, joka on itse tehty, valtioton, edullinen ja täysin ilmakapseloitu
---
...
SeedSigner on avoimen lähdekoodin Hardware Wallet Bitcoin, jonka kuka tahansa voi rakentaa itse käyttämällä edullisia, yleiskäyttöisiä elektronisia komponentteja. Toisin kuin kaupalliset tuotteet, kuten Ledger, Coldcard tai Trezor, tämä ei ole yrityksen valmistama valmis laite: kyseessä on yhteisöprojekti, jonka avulla jokainen voi luoda oman laitteensa ja valvoa jokaista vaihetta.



SeedSigner on suunniteltu olemaan 100-prosenttisesti ***air-gapped***: se ei koskaan yhdisty Internetiin, siinä ei ole Wi-Fi- tai Bluetooth-yhteyttä (Raspberry Pi Zero v1.3:n tapauksessa) eikä sitä koskaan yhdistetä tietokoneeseen Exchange-tietojen saamiseksi. Viestintä tapahtuu yksinomaan QR-koodin Exchange-järjestelmän kautta. Konkreettisesti sanottuna salkunhallintaohjelmistosi (kuten Sparrow wallet) näyttää allekirjoitettavan tapahtuman QR-koodien muodossa; skannaat ne SeedSignerin kameralla, minkä jälkeen laite allekirjoittaa tapahtuman käyttämällä RAM-muistiinsa väliaikaisesti tallennettuja yksityisiä avaimiasi. Lopuksi laite luo QR-koodit, jotka sisältävät allekirjoitetun tapahtuman, jonka skannaat ohjelmallasi lähettääksesi sen Bitcoin-verkkoon.
...
Toinen vaihtoehto seed:n tallentamiseen ja käyttämiseen SeedSignerin kanssa on käyttää *SeedKeeper*-älykorttia yhdessä yhteensopivan lukulaitteen kanssa. Näin saat erittäin vankan *secure element*:n seed:n tallentamiseen ja voit käyttää SeedSignerin näyttöä tapahtumien allekirjoittamiseen. Mutta tämä erityinen kokoonpano on aiheena toisessa erillisessä opetusohjelmassa. Tässä keskitymme SeedSignerin peruskäyttöön:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

SeedSigner-hankkeella on tärkeä asema Bitcoin-ekosysteemissä, sillä se tarjoaa kaikille kaikkialla maailmassa mahdollisuuden hyötyä kehittyneestä turvallisuudesta bitcoiniensa suojaamiseksi. Sen tärkein etu on sen helppokäyttöisyys: tarvittavan laitteiston voi ostaa alle 50 dollarilla. Lisäksi rajoitetuissa maissa asuvat ihmiset voivat rakentaa oman Hardware Wallet:nsä tavallisista tietokonekomponenteista, joita on helppo löytää ja joihin ei kohdistu niin paljon sääntelyrajoitteita.



SeedSigner voi kuitenkin olla mielenkiintoinen vaihtoehto myös näiden erityisten yhteyksien ulkopuolella: se on avoimen lähdekoodin, toimii tilattomasti ja ilman ilmatilaa ja vähentää Hardware Wallet:n Supply-ketjuun liittyviä hyökkäysvektoreita.
...
- Raspberry Pi Zero
 ...
 - W- ja v2-versiot ovat myös yhteensopivia, mutta niissä on Wi-Fi/Bluetooth-siru. Siksi on suositeltavaa poistaa se fyysisesti käytöstä poistamalla radiomoduuli kortista. Toimenpide on suhteellisen yksinkertainen, mutta vaatii tarkkuutta (Zero W -mallissa riittää hienot pihdit, kun taas v2-mallissa tarvitaan Hot-kynää moduulin piilottavan metallilevyn irrottamiseen). En mene yksityiskohtiin tässä ohjeessa, mutta löydät kaikki ohjeet tästä asiakirjasta: *[WiFi-/Bluetooth-yhteyksien poistaminen käytöstä laitteiston avulla](https://github.com/DesobedienteTecnologico/rpi_disable_wifi_and_bt_by_hardware)*.
 ...
 - Älä unohda sisällyttää mikro-USB-virta Supply:tä.
...
- Waveshare 1,3" näyttö (240×240 px)** (ranskaksi)
    ...
    - Näytössä on kolme painiketta ja minijoystick, joka toimii käyttäjän Interface:na.
...
- Asuminen (valinnainen, mutta suositeltavaa)** (valinnainen, mutta suositeltavaa)** (valinnainen, mutta suositeltavaa)** (valinnainen, mutta suositeltavaa)** (suositeltavaa)
...
PowerShell laskee sitten kuvatiedoston Hash SHA256:n. Vertaa tätä Hash-arvoa vastaavaan arvoon tiedostossa `seedsigner.0.8.6.sha256.txt`.
...
Aseta sitten Waveshare-näyttö Raspberry Pi Zeron päälle. Piin GPIO-nastojen pitäisi olla täydellisesti linjassa näytön naarasliittimen kanssa. Paina näyttöä hitaasti nastoihin ja paina tasaisesti molemmilta puolilta, jotta ne eivät taivu.
...
Jos olet muuttanut kameran suuntausta tai haluat muuttaa muita asetuksia (kuten Interface-kielen) myöhemmin, sinun on otettava käyttöön asetusten pysyvyys microSD-kortilla. Muuten asetuksesi palautuvat oletusasetuksiin aina uudelleenkäynnistyksen yhteydessä, sillä Raspberry Pi Zerossa ei ole pysyvää muistia.
...
Nykyinen laiteohjelmistoversio tukee useita ohjelmistopaketteja: Sparrow, Specter, BlueWallet, Nunchuk ja Keeper. Omassa tapauksessani käytän **Sparrow wallet**, jota suosittelen erityisesti sen yksinkertaisuuden ja runsaan toiminnallisuuden vuoksi.
...
- `BTC`
- mBTC` (milli-Bitcoin tai 0,001 BTC)
- gW-20 (satoshi, tai 1/100,000,000 BTC)
...
- gW-22 network`: muutetaan vain, jos haluat käyttää SeedSigneria Testnet:ssä.
...
Lopuksi voit vaihtaa Interface:n kielen haluamallasi tavalla Kieli-valikossa.
...
seed (tai Mnemonic-lause) muodostaa Bitcoin-salkun perustan. Sitä käytetään yksityisten avainten ja osoitteiden johtamiseen, ja sen avulla pääset käsiksi varoihisi. SeedSigner tarjoaa useita menetelmiä sen luomiseen, joita tarkastelemme tässä jaksossa.
...
- Tämä lause antaa sinulle täyden, rajoittamattoman pääsyn kaikkiin bitcoineihisi.** Kuka tahansa, jolla on tämä lause hallussaan, voi varastaa varasi, vaikka hänellä ei olisi fyysistä pääsyä SeedSigneriin;
- Yleensä Wallet:n palauttamiseen käytetään 12-sanaista lausetta, jos Hardware Wallet on kadonnut tai varastettu. Mutta koska SeedSigner on *tilaton* laite, se ei koskaan rekisteröi seed:ääsi. Fyysiset varmuuskopiot eivät siis ole pelkkiä varmuuskopioita, vaan **ainoa tapa käyttää Wallet:a**. Jos menetät nämä varmuuskopiot, bitcoinisi menetetään pysyvästi. Varmuuskopioi ne siis huolellisesti, useille eri välineille ja turvallisiin paikkoihin;
- Jos olet vasta aloittamassa, suosittelen sinua lukemaan tämän ohjeen, jotta saat yksityiskohtaisen käsityksen Mnemonic-lauseen hallintaan liittyvistä riskeistä:



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### 5.1 Käynnistä seed:n luontityökalu
...
Nyt generate on seed. seed on yksinkertaisesti suuri satunnaisluku. Mitä satunnaisemmin se on luotu, sitä turvallisempi se on. SeedSigner tarjoaa kaksi tapaa tehdä tämä:




- kamera": seed syntyy valokuvan visuaalisesta kohinasta. Otetaan kuva satunnaisesta ympäristöstä (esine, maisema, kasvot jne.), jonka pikselivaihteluita käytetään generate entropian määrittämiseen. Se on nopea menetelmä, mutta ei toistettavissa.
...
Valitse sitten Mnemonic-lauseen pituus. 12 sanaa tarjoaa jo riittävän turvallisuustason, joten suosittelen tätä vaihtoehtoa.
...
Kirjoita Mnemonic-lauseesi sanat huolellisesti sopivalle fyysiselle alustalle (paperille tai metallille).
...
Jos teet virheen, SeedSigner ilmoittaa siitä sinulle, ja sinun on aloitettava alusta ja muistettava Mnemonic-lauseesi, kun se annetaan sinulle. Tämä varmennusvaihe varmistaa, että varmuuskopiosi on oikea ja täydellinen. Kun varmennus on vahvistettu, näytöllä näkyy `Backup Verified`.
...
SeedSigner on laite, jolla ei ole pysyvää muistia. Tämä tarkoittaa, että seed:tä ei koskaan tallenneta laitteen sisälle (toisin kuin esimerkiksi Ledger:tä, Trezoria tai Coldcardia). Heti kun katkaiset laitteen virran, seed katoaa kokonaan sen RAM-muistista. Kun SeedSigner käynnistyy uudelleen, se palaa tyhjään tilaan: sinun on annettava sille seed uudelleen, jotta voit allekirjoittaa tapahtumia.



Tämä tarjoaa välttämättömän suojan. Toisin kuin muut laitteistolompakot, SeedSigner perustuu Raspberry Pi Zeroon, jossa ei ole fyysistä suojausta, mukaan lukien *secure element*. Koska arkaluonteisia tietoja ei kuitenkaan tallenneta, hyökkääjä ei voisi edes fyysisesti vahingoittuneen laitteen avulla poimia yksityisiä avaimiasi tai käyttää bitcoinejasi.
...
Aina kun käytät SeedSigneria, skannaat tämän QR-koodin laitteen kameralla, jolloin laite lataa seed:n väliaikaisesti muistiinsa sillä aikaa, kun allekirjoitat tapahtumia. Tätä toista, jokapäiväiseen käyttöön tarkoitettua varmuuskopiota on myös säilytettävä erittäin huolellisesti: kuka tahansa, jolla on hallussaan tämä QR-koodi, pääsee täysin käsiksi bitcoineihisi.


Suosittelen myös säilyttämään QR-koodin ja Mnemonic-lausekkeen kahdessa eri paikassa, jotta et menetä kaikkea korvausvaatimuksen sattuessa.



Kehittyneempi ja turvallisempi vaihtoehto on käyttää SeedSigneria yhdessä **SeedKeeper**:n kanssa, joka tallentaa seed:n secure element:een. Lisätietoja on tässä opetusohjelmassa :
...
Tässä vaiheessa voit myös lisätä **passphrase BIP39**:n vahvistamaan Wallet:n turvallisuutta. Tämä vaihtoehto voi olla kannattava varmuuskopiointistrategiastasi riippuen, mutta siihen liittyy myös riskejä: jos menetät passphrase:n, bitcoinien käyttöoikeus menetetään pysyvästi.



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

Jos passphrase-konsepti ei ole vielä tuttu, pyydän sinua lukemaan tämän kattavan oppaan aiheesta:
...
SeedSignerin avulla voit muuntaa seed:n paperiseksi QR-koodiksi, jota kutsutaan *SeedQR*:ksi. Tämä menetelmä yksinkertaistaa Wallet:n lataamista uudelleen, koska näin vältetään jokaisen sanan kirjoittaminen uudelleen manuaalisesti.



Tätä varten tarvitset tyhjän paperisen tai metallisen QR-koodin, joka vastaa Mnemonic-lauseesi pituutta. Jos olet ostanut täydellisen paketin SeedSigneriäsi varten, mallit ovat yleensä mukana. Jos näin ei ole, voit ladata ja tulostaa ne (tai jäljentää ne käsin) täältä :
...
Valitse seed:n näytöltä `Backup seed`.
...
Jäljennä huolellisesti jokainen musta piste tallennuslomakkeellesi ja siirry sitten joystickillä seuraavaan BLOCK:een. Käytä aikaa: yksinkertainen virheellinen kohdistus voi tehdä QR-koodista käyttökelvottoman.
...
Onnittelut, Bitcoin-salkkusi on nyt toiminnassa! Tuomme nyt sen julkiset komponentit **Sparrow wallet**:een, jotta voimme hallita sitä helposti.



## 6. Tuo Wallet Sparrow:aan



Kun SeedSigner on perustettu ja seed on luotu ja tallennettu oikein, seuraava vaihe on yhdistää salkku hallintaohjelmistoon, kuten Sparrow wallet:ään. seed pysyy aina offline-tilassa, sillä vain salkun julkinen osa siirretään Sparrow:een. Näin ohjelmisto voi näyttää osoitteesi, transaktiosi ja rakentaa uusia transaktioita, mutta ei voi koskaan käyttää bitcoinejasi. Käyttääksesi bitcoinejasi SeedSignerisi on aina allekirjoitettava Sparrow:n valmistelemat transaktiot.
...
Tarkista, että yleisavaimessa oleva sormenjälki vastaa Wallet:ssä olevaa sormenjälkeä. Jos käytät passphrase:ää, syötä se tässä vaiheessa.
...
Sen jälkeen SeedSigner luo xpubin dynaamisen QR-koodin muodossa, joka sisältää kaikki tiedot, joita tarvitset salkkusi hallintaan Sparrow wallet:ssa.
...
Varmista, että Sparrow wallet-ohjelmisto on asennettu tietokoneeseen. Jos et tiedä, miten se ladataan, tarkistetaan ja asennetaan oikein, tutustu aiheeseen liittyvään täydelliseen opetusohjelmaan:
...
Anna salkulle nimi ja napsauta sitten "Luo Wallet". Sparrow pyytää sinua asettamaan salasanan, jolla lukitset paikallisen pääsyn tähän Wallet:een. Valitse vahva salasana: se suojaa pääsyn salkun tietoihin Sparrow:ssä (julkiset avaimet, osoitteet, tarrat ja tapahtumahistoria). Tätä salasanaa ei tarvita salkun palauttamiseen myöhemmin: tähän tarkoitukseen tarvitaan vain Mnemonic-lauseesi (ja mahdollisesti passphrase).
...
Tarkista sitten, että Sparrow:ssä näkyvä `Master fingerprint` vastaa SeedSignerissä aiemmin merkittyä sormenjälkeä.



SeedSigner ja Sparrow wallet ovat nyt turvallisesti yhteydessä toisiinsa. Sparrow toimii Interface:n täydellisenä hallintalaitteena, kun taas SeedSigner on edelleen ainoa laite, joka pystyy allekirjoittamaan tapahtumasi. Olet nyt valmis vastaanottamaan ja lähettämään bitcoineja täysin ilmaverkossa.
...
Näyttöön tulee uusi Bitcoin Address. Näet :




- Teksti Address (joka alkaa kirjaimella `bc1p...`, jos käytät P2TR:a kuten minä),
- Vastaava QR-koodi,
- `Label`-kenttä tapahtumien seurantaa varten.



Suosittelen lämpimästi, että lisäät tarran jokaiseen Bitcoin-kuittiin Wallet:ssa. Näin voit helposti tunnistaa jokaisen UTXO:n alkuperän ja parantaa yksityisyyden hallintaa. Jos haluat syventyä tähän tärkeään aiheeseen tarkemmin, voit tutustua Plan ₿ Academyn erityiskoulutukseen:
...
Address on nyt yhdistetty tähän merkintään kaikissa Sparrow-osiossa.
...
#### 7.1.2 Address-tarkastus SeedSigner-järjestelmässä



Ennen kuin jaat vastaanotetun Address:n, on erittäin tärkeää tarkistaa, että se kuuluu seed:aan. Tämä vaihe varmistaa, että SeedSigner voi allekirjoittaa tähän Address:een liittyviä tapahtumia. Se suojaa myös mahdollisilta hyökkäyksiltä, joissa Sparrow näyttää väärennetyn Address:n. Muista, että Sparrow toimii turvattomassa ympäristössä (tietokoneellasi), jossa on paljon suurempi hyökkäyspinta kuin täysin eristetyssä SeedSignerissäsi. Siksi sinun ei koskaan pidä sokeasti uskoa Sparrow:n näyttämää vastaanottajan Address:ta, ennen kuin olet tarkistanut sen Hardware Wallet:lläsi.



Suurenna Sparrow:n QR-koodia klikkaamalla Address:n QR-koodia: se näkyy sitten koko näytön kokoisena.
...
Jos skannattu Address vastaa seed:stä peräisin olevaa Address:a, SeedSigner-näytöllä näkyy viesti: "Address vahvistettu".
...
Tämä vahvistaa, että Address kuuluu Wallet:een ja että voit vastaanottaa siitä bitcoineja.
...
Voit nyt välittää tämän Address:n (tekstinä tai QR-koodina) henkilölle tai osastolle, jonka on lähetettävä sinulle Satss. Kun maksutapahtuma on lähetetty verkossa, se näkyy Sparrow wallet:n "maksutapahtumat"-välilehdellä.
...
- Liitä vastaanottajan Address tai skannaa QR-koodi napsauttamalla kamerakuvaketta kohdassa "Maksu vastaanottajalle";
- Lisää kohtaan `Label` merkintä, jolla tätä kulua seurataan;
- Kirjoita lähetettävä summa kohtaan `Määrä`;
- Valitse lopuksi maksuosuus nykyisten markkinaolosuhteiden perusteella (arvioita on saatavilla [Mempool.space](https://Mempool.space/)).
...
Kytke SeedSigner päälle ja skannaa SeedQR:si päästäksesi salkkuusi käsiksi tavalliseen tapaan. Valitse aloitusnäytöltä `Scan` ja skannaa sitten Sparrow:ssa näkyvä QR-koodi.
...
Klikkaa `Review Details` ja tarkista kaikki tiedot huolellisesti suoraan SeedSigner-näytöltä. Tärkeimmät tarkistettavat tiedot ovat lähetetty summa, vastaanottajan Address-tunnus ja mahdollisesti perittyjen maksujen määrä.
...
Jos kaikki on oikein, valitse `Hyväksy PSBT` allekirjoittaaksesi tapahtuman käyttämällä vastaavaa yksityistä avainta tai vastaavia yksityisiä avaimia.
...
Allekirjoituksen jälkeen SeedSigner luo uuden QR-koodin, joka sisältää allekirjoitetun tapahtuman ja on valmis skannattavaksi Sparrow:lla.
...
Nyt kun tapahtuma on voimassa, se on lähetettävä Bitcoin-verkossa, jotta se saavuttaa Miner:n, joka lisää sen BLOCK:een.
