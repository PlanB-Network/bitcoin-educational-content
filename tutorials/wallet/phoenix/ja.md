...
Phoenix は、スマートフォンに組み込まれた本格的な Lightning ノードとして機能し、自動的に ACINQ の Lightning ノードとチャネルを開きます。アプリケーションは Lightning-KMP に基づいており、Kotlin によるクロスプラットフォーム実装の Lightning Network で、モバイルウォレット向けに最適化されています。他の Lightning ノードソリューションとは異なり、Phoenix は管理を大幅に簡素化します。ユーザーはチャネルの開閉、Bitcoin ノードの運用、Lightning ネットワーク上の流動性管理を行う必要はありません。Phoenix はこれらすべての技術的な操作をバックグラウンドで処理します。
