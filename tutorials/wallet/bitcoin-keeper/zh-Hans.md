---
name: Bitcoin Keeper - 多钥匙安全系统
description: Bitcoin Keeper 应用程序--一款移动应用程序，可创建 Hot 钱包和 Cold 存储库，用于长期储蓄
---

![cover](assets/cover.webp)


## 导言


在本视频教程中，您将了解如何设置和使用 Bitcoin Keeper 应用程序--这是一款移动应用程序，允许您创建 Hot 钱包和 Cold 存储库，以实现长期储蓄。Keeper 广泛支持硬件钱包，包括来自 Coinkite 的 Coldcard 和 Tapsigner。


![video](https://youtu.be/st9Oe4Dxxl4)


要下载 Keeper，请参阅 [此处]（bitcoinkeeper.app）


**注：** 本教程仅为英文版草稿，我们仍需要有人就此主题撰写详尽的指南。如果你是这样的人，请通过 [Telegram](https://t.me/PlanBNetwork_ContentBuilder/325) 或 [GitHub](https://github.com/PlanB-Network/Bitcoin-educational-content) 联系我们。