...
objectives: 
  ...
  - Comprendere il proof-of-work e il suo ruolo nel Bitcoin.
...
Capire il proof of work vuol dire capire come funziona il Bitcoin. Senza questa invenzione e il suo ingegnoso utilizzo, il Bitcoin non sarebbe potuto esistere. Questo corso vi fornisce tutta la teoria del mining di cui avete bisogno per il vostro viaggio in bitcoin.
...
Alla fine di questo corso, sarete in grado di spiegare il funzionamento del proof-of-work in modo semplice e rigoroso. MIN 101 è anche un'introduzione ideale a tutti gli altri corsi più avanzati dedicati al Bitcoin mining e al Plan ₿ Academy, sia teorici che pratici.
...
Benvenuti al corso MIN 101, nel quale scoprirete i concetti teorici fondamentali del mining e del Proof-of-Work all'interno del sistema Bitcoin. Questo corso è il primo passo del tuo viaggio da bitcoiner per capire come funziona il mining. Una volta completato, potrai passare a corsi teorici più avanzati, oppure passare all'azione e diventare tu stesso un miner di bitcoin!



In questo corso MIN 101 non riprenderemo i concetti di base del Bitcoin, ma andremo dritti al nocciolo della questione: IL mining. Se non avete mai sentito parlare del Bitcoin o se i suoi fondamenti non vi sono ancora chiari, vi consiglio vivamente di iniziare con il nostro corso introduttivo BTC 101. Una volta acquisite queste nozioni di base, potrete scegliere il corso di MIN. Una volta acquisiti questi fondamenti, sarete in grado di affrontare il MIN 101 con fiducia:
...
### Parte 2 - Come funziona il proof of work



Dopo l'introduzione, la Parte 2 è la base tecnica del programma di formazione. Il suo scopo è spiegare, passo dopo passo, come il Bitcoin produce blocchi validi. Inizieremo a scoprire la struttura di un blocco, prima di approfondire il meccanismo del proof-of-work: il ruolo del target, del nonce e della funzione di hash. Infine, vedremo come Bitcoin riesca a mantenere un tasso di produzione di blocchi stabile nonostante le variazioni della potenza di hash, grazie al meccanismo di regolazione della difficoltà. Alla fine di questa sezione, avrete una comprensione completa dei principi fondamentali del Bitcoin e del proof-of-work.
...
La quarta parte riporta il mining nella sua realtà operativa. Si ripercorre l'evoluzione delle macchine mining, dagli inizi del Bitcoin al moderno ASIC, per comprendere gli attuali vincoli hardware. Si analizzano anche le basi dei pool mining, per capire come i minatori riescano a ridurre la varianza delle loro entrate.
...
## Bitcoin mining facile da usare
...
Il Mining è il lavoro di convalida di un blocco consumando energia. Gli attori chiamati minatori utilizzano computer specializzati. Queste macchine consumano elettricità per eseguire un numero molto elevato di prove, in un ciclo, finché non trovano una prova che la rete accetta. Quando un minatore trova questa prova, il suo blocco è considerato valido.
...
Se i minatori consumano elettricità, non è perché sono volontari. Lo fanno perché c'è una ricompensa. Quando un minatore convalida un blocco, riceve due tipi di reddito. Da un lato, riceve i bitcoin appena creati. Dall'altro, raccoglie le commissioni pagate dagli utenti per le transazioni incluse nel blocco. In altre parole, il minatore viene compensato sia attraverso l'emissione monetaria programmata, sia attraverso le commissioni di transazione determinate da un mercato.



In questa fase, viene data deliberatamente una visione molto semplice del mining. Non spiega ancora come il blocco sia costruito in dettaglio, né come funziona esattamente la prova che i minatori stanno cercando, né come Bitcoin mantenga un ritmo costante, né come la ricompensa sia calcolata con precisione, né come venga richiesta. Non importa, questo è tutto ciò che vedremo nel resto di questo corso MIN 101!
...
Per capire che cos'è il Bitcoin mining, dobbiamo innanzitutto seguire il percorso di una tipica transazione Bitcoin. Questo mostrerà esattamente dove entra in gioco il blocco e perché è il cuore del sistema. Questo vi mostrerà esattamente dove entra in gioco il blocco e perché è il cuore del sistema. Questo è ciò che vorrei che scopriste in questo primo capitolo.
...
La firma digitale nel Bitcoin svolge due ruoli importanti:
...
Una volta che la transazione è stata costruita e firmata correttamente dal Bitcoin dell'utente, essa deve essere trasmessa sulla rete Bitcoin.
...
In questa fase, la rete è a conoscenza della transazione, l'ha verificata e la tiene in memoria fino alla sua conferma. Ma la conferma di questa transazione avverrà solo quando un minatore la inserirà in un blocco e questo blocco verrà accettato dalla rete.
...
Essendo una moneta immateriale, il bitcoin deve risolvere un problema: impedire la doppia spesa senza un'autorità centrale. Se due transazioni tentano di spendere lo stesso UTXO, tutti devono essere in grado di convergere su un unico stato coerente. Il Satoshi Nakamoto riassume questo problema con questa famosa frase:
...
I Miner devono costruire da soli i loro blocchi prima di cercare di estrarli. Ogni minatore, a sua volta, costruisce il cosiddetto blocco candidato a partire dalle transazioni in sospeso nella sua mempool. La costruzione di un blocco candidato consiste quindi in:




- scegliere quali transazioni includere;
- organizzare queste transazioni in modo compatibile con le regole del Bitcoin;
- produrre i metadati del blocco, memorizzati nella sua intestazione.



La scelta delle transazioni segue una semplice logica economica: un blocco ha una capacità limitata dal protocollo Bitcoin, quindi il minatore cerca di massimizzare il guadagno per questo spazio. Seleziona in via prioritaria le transazioni che offrono le commissioni più alte rispetto allo spazio che occupano nel blocco (questo è noto come "fee rate", espresso in sats/vB). I dettagli sulle commissioni saranno trattati in seguito; è sufficiente ricordare l'idea di ordinare in base all'efficienza dello spazio.
...
L'intestazione è essenziale, in quanto viene utilizzata come base per il proof-of-work: nel Bitcoin, non si estrae direttamente un intero blocco, ma solo l'intestazione di un blocco, che riassume le informazioni necessarie per collegare il blocco alla catena e impegnarne il contenuto. Per far sì che l'intestazione rappresenti tutte le transazioni, Bitcoin utilizza uno strumento crittografico: l'albero di Merkle.
...
L'intestazione del blocco è lunga 80 byte e contiene esattamente 6 campi. Sono questi sei elementi a essere sottoposti a hashing durante la ricerca di un proof of work (si veda il capitolo successivo):
...
- Radice di Merkle (`merkleroot`): È l'impronta digitale di tutte le transazioni del blocco (tramite l'albero di Merkle). Collega l'intestazione al contenuto: se il minatore modifica la selezione o l'ordine delle transazioni, la radice cambia.
...
- Nonce (`nonce`): È un valore che il minatore può modificare liberamente. Serve come variabile regolabile durante la proof-of-work. Spiegherò il suo ruolo in modo più dettagliato nel prossimo capitolo, ma è importante capire che il nonce fa parte dell'intestazione del blocco ed è progettato per consentire tentativi successivi.
...
Questa intestazione del blocco candidato, costruita dal miner, costituisce la base del suo lavoro. Quando si cerca un proof-of-work valido, non è l'intero elenco delle transazioni a essere sottoposto a hash direttamente in un ciclo, ma piuttosto questo blocco di 80 byte, che contiene tutto ciò che serve per collegare il blocco al passato e impegnarne il contenuto, includendo anche i parametri necessari per il meccanismo mining, che analizzeremo nel prossimo capitolo.



## L'hash, l'obiettivo e il nonce
...
Nei capitoli precedenti abbiamo seguito il percorso di una transazione Bitcoin: creata e firmata da un wallet, trasmessa dai nodi, memorizzata nei mempool, quindi confermata quando un minatore la include in un blocco accettato dalla rete. Ma non abbiamo ancora visto come un minatore possa aggiungere il suo blocco alla blockchain. Qual è il processo alla base del mining?
...
La funzione hash utilizzata nel Bitcoin per il mining è `SHA256`, applicata due volte di seguito. Questa funzione è nota come doppio SHA256, o `SHA256d`. È questo doppio hashing che produce l'impronta digitale del blocco.
...
### Prova del lavoro: trovare un hash più piccolo dell'obiettivo



Proof-of-Work viene spesso descritto come la soluzione di un problema complesso. In realtà, non si tratta tanto di un problema quanto di una ricerca per tentativi: il minatore deve trovare una versione dell'intestazione il cui hash (dopo essere passato attraverso la funzione di hash `SHA256d`) rispetti una semplice condizione: deve essere più piccolo di un certo obiettivo.
...
Per ritentare la fortuna, il minatore non ha bisogno di modificare l'intera intestazione del suo blocco candidato: deve solo cambiarne una piccola parte, perché anche un solo bit diverso darà luogo a un hash completamente nuovo, potenzialmente valido se è più piccolo dell'obiettivo.
...
### Qual è lo scopo del proof of work?
...
Nel Libro Bianco del Bitcoin, il Satoshi Nakamoto presenta due vantaggi nell'utilizzo di un sistema proof-of-work nel Bitcoin:





- Sealing la storia economica:**



Una volta esaurito il carico computazionale, il blocco è bloccato: modificarlo richiederebbe di rifare il proof of work di quel blocco. E poiché i blocchi sono concatenati, modificare un vecchio blocco significherebbe anche ricalcolare tutti i blocchi successivi, e quindi raggiungere e superare il lavoro in corso della catena onesta.



In altre parole, il proof-of-work funge da spina dorsale del sistema di marcatura temporale, rendendo sempre più costosa la falsificazione del passato man mano che i blocchi si accumulano. Quando viene estratto un nuovo blocco, la sicurezza fornita dal proof of work viene applicata simultaneamente e uniformemente a tutti i UTXO esistenti. Ad ogni blocco aggiunto, ogni UTXO accumula quindi una quantità supplementare di sicurezza dal Proof-of-Work.





- Definire la regola della maggioranza (consenso) e neutralizzare la Sibilla:**



Il Proof-of-Work permette anche al Bitcoin di raggiungere il consenso senza affidarsi alla regola di voto "un ID = un voto", che potrebbe essere facilmente falsata dalla creazione massiccia di identità (IP, nodi, chiavi...).



Nel Bitcoin, la "*maggioranza*" non è il maggior numero di partecipanti, ma la **catena che accumula più lavoro**. Come dice il Satoshi, si tratta del principio "una CPU = un voto", ossia un voto ponderato in base all'effettiva potenza di calcolo spesa per produrre blocchi validi. Quindi la distribuzione di migliaia di nodi non porta alcun vantaggio in sé rispetto al Bitcoin. Senza potenza di calcolo aggiuntiva, non si accumulano più prove di lavoro e l'attacco Sybil diventa inutile, mentre la regola decisionale rimane oggettiva e non richiede l'identificazione dei partecipanti.
...
I principi relativi all'utilità e ai poteri dei minori sono un argomento molto complesso, che non approfondirò in questo corso. Tuttavia, torneremo su questo argomento in modo approfondito nei prossimi corsi di formazione MIN 201.



Per il momento, è possibile riassumere il funzionamento di mining in questo modo: i minatori costruiscono un blocco candidato con le transazioni in sospeso nei mempool, quindi cercano un hash della sua intestazione (tramite `SHA256d`) che sia inferiore o uguale a un obiettivo. Questo obiettivo viene raggiunto testando i nonces attraverso tentativi ed errori.



Nel prossimo capitolo, faremo una breve digressione storica sul principio del proof-of-work per comprenderne il background, per poi esaminare come il sistema determina l'obiettivo di difficoltà.



## La storia del proof of work
...
Il Proof-of-work non è stato inventato per il Bitcoin. Il Satoshi Nakamoto ha ripreso e assemblato diverse idee precedenti, già esplorate in contesti diversi.
...
Tra la fine degli anni '90 e l'inizio del 2000, Nick Szabo ha esplorato l'idea della scarsità digitale basata sul proof of work. Il suo progetto concettuale, chiamato Bit Gold, prevede la creazione di unità di valore risolvendo un costoso proof of work e registrando poi queste prove in un registro per stabilire una forma di proprietà.



Bit Gold non ha portato a un sistema distribuito come il Bitcoin, ma contiene diverse intuizioni importanti: l'idea che la computazione possa produrre scarsità e l'idea di datare gli elementi nel tempo per creare una storia difficile da riscrivere.
...
Quando un minatore trova un blocco valido, lo distribuisce immediatamente ai suoi pari. I nodi che lo ricevono ne verificano la validità (transazioni, proof of work, regole di consenso, ecc.), quindi lo trasmettono a loro volta. Questa propagazione richiede un certo tempo, limitato dalla latenza di Internet, dalla larghezza di banda e dalla capacità di ciascun nodo di verificare il blocco.
...
Se, durante questo ritardo di diffusione, un altro minatore scopre un blocco valido alla stessa altezza, la rete può essere temporaneamente divisa: una parte dei nodi e dei minatori si affida al blocco A, mentre l'altra si affida al blocco B. Questa è una divisione temporanea della rete.
...
*"Hashrate*" si riferisce alla quantità di calcoli hash prodotti al secondo, sia da un singolo minatore, sia da un gruppo di minatori, sia da tutti i minatori del Bitcoin. È espresso in `H/s` (hash al secondo), con multipli come `TH/s` (terahashes al secondo) o `EH/s` (exahashes al secondo). Rappresenta il numero di tentativi che i minatori possono fare ogni secondo per cercare di ottenere un hash inferiore all'obiettivo.
...
- se gli ultimi blocchi del 2016 sono stati trovati troppo velocemente, significa che il hashrate è aumentato durante questo periodo; il Bitcoin rende quindi la condizione più difficile abbassando l'obiettivo per il periodo successivo;
...
Nei capitoli precedenti abbiamo visto come i minatori riescono a trovare un blocco valido. Una volta che un minatore ha trovato un'intestazione il cui hash è più piccolo dell'obiettivo, il suo blocco candidato è considerato valido. Può quindi distribuirlo a tutta la rete Bitcoin. Il blocco viene aggiunto al resto della blockchain, confermando le transazioni che contiene.



È proprio questo evento (l'aggiunta effettiva del blocco alla blockchain) che determina l'assegnazione di una ricompensa al minatore vincitore. Questa ricompensa è composta da due elementi distinti che vengono sommati:
...
Il sussidio di blocco corrisponde alla parte di creazione monetaria della ricompensa. Quando un minatore produce un blocco valido, il protocollo lo autorizza a creare un certo numero di nuovi bitcoin e ad assegnarseli come ricompensa. Questi bitcoin sono creati ex nihilo. Non esistevano prima.



Tuttavia, la quantità di nuovi bitcoin creati non è affatto arbitraria. È strettamente definita dalle regole del protocollo Bitcoin ed è identica per tutti i minatori. Analizzeremo più da vicino questo meccanismo nel prossimo capitolo, poiché la sovvenzione non è un valore fisso all'infinito: viene suddivisa periodicamente secondo un preciso programma. Per ora è sufficiente ricordare che:




- il sussidio di blocco è una delle due componenti del premio di blocco;
- è limitato e determinato dal protocollo, non dal minatore (anche se il minatore può tecnicamente richiedere meno dell'importo massimo);
- crea bitcoin dal nulla.



Questa sovvenzione svolge due ruoli principali all'interno del protocollo Bitcoin. La prima è quella di incoraggiare i giocatori a partecipare al mining. Nei primi anni del Bitcoin (e talvolta ancora oggi), le commissioni di transazione erano molto basse. La sovvenzione ha quindi garantito un compenso sufficiente per attirare i minatori e mantenere un livello di sicurezza per il sistema.
...
La seconda componente della ricompensa dei blocchi è legata all'utilizzo del sistema: quando un utente inserisce una transazione, vuole che questa venga confermata. Tuttavia, lo spazio per i blocchi è limitato e i blocchi appaiono in media solo ogni 10 minuti circa. Lo spazio per i blocchi è quindi una risorsa scarsa. Quando la domanda supera l'offerta, il prezzo aumenta: questo è il mercato delle commissioni di transazione. Ogni minatore che riesce a produrre un blocco valido ottiene il diritto di riscuotere, per proprio conto, l'intera commissione di transazione associata a tutte le transazioni che ha incluso nel suo blocco.



Si può pensare a un sistema di aste: ogni transazione propone un importo, e i minatori danno la priorità a quelle che massimizzano le loro entrate, nel rispetto dei vincoli di spazio. Questo meccanismo allinea naturalmente gli interessi:




- gli utenti che hanno fretta pagano di più per essere inclusi rapidamente;
- i minatori sono incoraggiati a includere le transazioni che pagano le tariffe più alte per lo spazio dei blocchi.
...
La somma degli input è quindi di `250.000 sats`, mentre la somma degli output è di `247.000 sats`. Ciò significa che `3.000 sats` sono stati consumati negli input senza essere ricreati negli output: questo importo corrisponde alle tariffe proposte da questa transazione.
...
Se un miner include questa transazione in un blocco valido, avrà diritto a recuperare questi `3.000 sats`, oltre alle commissioni di tutte le altre transazioni incluse nel blocco. Tuttavia, non esiste un legame diretto on-chain tra la transazione che paga la tassa e il sats effettivamente riscosso dal minatore. Tecnicamente, i `3.000 sats` di commissioni vengono distrutti e, in cambio, il minatore ottiene il diritto di ricreare la stessa quantità per sé.
...
Due transazioni possono quindi pagare lo stesso importo di tasse in termini assoluti, ma non essere economicamente equivalenti dal punto di vista del minatore. Se una di esse è due volte più grande, costa il doppio dello spazio nel blocco. Lo spazio è scarso, quindi il minatore cerca di massimizzare le sue entrate per unità di spazio.
...
Questo rapporto spiega le scelte economiche dei minatori: a capacità fissa, l'inclusione di transazioni ad alto tasso massimizza le tariffe totali del blocco e quindi il compenso del minatore. Spiega anche perché le transazioni a basso costo rimangono in coda nei mempool per lunghi periodi: sono in concorrenza con altre transazioni che pagano di più per unità di spazio.
...
In pratica, i nodi applicano politiche di relay locali (regole di mempool) e spesso fissano una soglia minima di tariffe al di sotto della quale non relayano una transazione (per impostazione predefinita, `0,1 sat/vB` in Bitcoin Core tramite `minRelayTxFee`). Una transazione può essere valida nel senso stretto delle regole di consenso, ma non trasmessa dalla maggior parte dei nodi se le sue tariffe sono troppo basse. Di conseguenza, non circola, non raggiunge i minatori e ha poche possibilità di essere confermata.



A questo punto, avete capito il senso della ricompensa di blocco: corrisponde al compenso per il miner vincente ed è composta da due elementi distinti. Da un lato, una sovvenzione di blocco, definita dalle regole del protocollo, che crea nuovi bitcoin ex nihilo. Dall'altro, le tariffe delle transazioni incluse nel blocco minato.
...
Il Halving è un evento programmato nel protocollo Bitcoin che dimezza la sovvenzione del blocco, ossia la quantità massima di nuovi bitcoin che il minatore vincitore può creare con ogni blocco. Non influisce sulle commissioni di transazione: le commissioni esistono indipendentemente e rimangono determinate dall'attività degli utenti e dalla competizione per lo spazio dei blocchi.



Quando il Bitcoin è stato lanciato nel 2009, la sovvenzione per i blocchi è stata fissata a 50 BTC per ogni blocco estratto. Da allora, questa sovvenzione è stata dimezzata più volte per ogni dimezzamento.
...
- bitcoin bloccati in scritture irrisolvibili;
...
- una sovvenzione di blocco (bitcoin creati ex nihilo, il cui importo massimo è fissato dal protocollo e ridotto gradualmente tramite dime);
- tutte le commissioni di transazione pagate dagli utenti le cui transazioni sono state incluse nel blocco.



Rimane tuttavia una domanda: con quale meccanismo il minatore raccoglie questa ricompensa in Bitcoin? È proprio questo il ruolo di una particolare transazione chiamata "coinbase".
...
Come abbiamo visto nella prima parte del corso, ogni blocco Bitcoin contiene un elenco di transazioni in attesa di conferma. La prima di queste è sempre la transazione coinbase. È quella che permette al minatore vincitore di ricevere la sua ricompensa.
...
- Il primo è quello appena descritto: assegna al minatore la ricompensa che gli spetta per aver prodotto un blocco valido.
...
Poiché lo `scriptSig` permette di inserire liberamente informazioni arbitrarie scelte dal minatore, alcuni hanno sfruttato l'opportunità di aggiungere messaggi di natura più personale, piuttosto che tecnica. Il caso più famoso è ovviamente quello di Satoshi Nakamoto, con il suo ormai iconico messaggio:
...
Una volta che il blocco è stato estratto e distribuito, la transazione di coinbase appare sulla blockchain come qualsiasi altra transazione. Crea UTXO per il minatore vincitore, consentendogli di riscuotere la sua ricompensa. Tuttavia, questi UTXO non sono immediatamente spendibili: sono soggetti a un periodo di maturità. Questa scadenza è fissata a 100 blocchi dopo il blocco contenente il coinbase. In concreto, la transazione coinbase deve quindi totalizzare 101 conferme perché i suoi output diventino spendibili dal minatore vincitore.


![Image](assets/fr/040.webp)


Lo scopo di questa regola è limitare l'impatto delle riorganizzazioni della catena sull'economia. Come abbiamo visto nei capitoli precedenti, può accadere che alla stessa altezza due blocchi validi distinti vengano trovati quasi contemporaneamente da minatori diversi. Per un breve momento, la rete può dividersi: alcuni nodi ricevono per primi il blocco A, mentre altri vedono per primi il blocco B. Poi, nel corso dei blocchi successivi, la catena si riorganizza. Poi, nel corso dei blocchi successivi, uno dei due rami accumula più lavoro e diventa il ramo di riferimento. L'altro ramo viene abbandonato e i suoi blocchi diventano obsoleti. Le transazioni che conteneva possono quindi, in teoria, tornare ai mempool in attesa di conferma.
...
- un minatore spende bitcoin da un coinbase,
...
Agli inizi del Bitcoin, il mining non era un'attività industriale. Faceva parte del software del Bitcoin, lanciato su un personal computer, spesso per curiosità, a volte per supportare la rete e, in secondo luogo, per ottenere bitcoin che all'epoca non avevano praticamente alcun valore di mercato.
...
Nel 2009 e nei primi anni, il mining veniva eseguito principalmente utilizzando la CPU di un computer convenzionale. Il Bitcoin era allora un semplice software che fungeva da wallet, da nodo e da miner. Il semplice lancio del software del Satoshi Nakamoto sul proprio computer era sufficiente per partecipare al processo mining e, in molti casi, per trovare blocchi.
...
Il Mining è ora guidato da operatori industriali, alcuni dei quali quotati in borsa, che stanno costruendo e gestendo allevamenti su larga scala. Tra questi, MARA Holdings (Nasdaq: `MARA`) e Riot Platforms (Nasdaq: `RIOT`).
...
Naturalmente, ci sono ancora situazioni in cui il mining domestico può essere interessante, ad esempio se si beneficia di un'elettricità molto economica o se si utilizza il calore generated del miner per riscaldare la casa in inverno. Ma in ogni caso, dovrete comunque acquistare una macchina dotata di chip ASIC. Inoltre, poiché la vostra potenza mining rimarrà estremamente ridotta rispetto alla rete Bitcoin, dovrete trovare un modo per ridurre la varianza delle vostre entrate: è proprio questo il ruolo dei pool mining, di cui parleremo nel prossimo capitolo.
...
Mining Il Bitcoin comporta costi continui e inevitabili, primo fra tutti il consumo di energia della macchina. Queste spese sono sostenute indipendentemente dai risultati, anche se i ricavi del mining sono, per loro stessa natura, rari e casuali. La scoperta di un blocco dipende esclusivamente dalla quota di hashrate del minatore, il che rende i guadagni tanto più imprevedibili quanto più piccola è tale quota. È proprio questo problema pratico che ha portato rapidamente alla diffusione dei pool mining. In questo capitolo finale del corso MIN 101, offro un'introduzione ai principi e al funzionamento dei pool mining in Bitcoin.



### Cos'è una piscina mining?



Un pool mining è un'organizzazione (spesso un servizio online) che aggrega la potenza di calcolo di molti minatori indipendenti, al fine di aumentare la frequenza con cui il gruppo trova i blocchi. Quando il pool trova un blocco, la ricompensa del blocco viene ridistribuita tra i partecipanti in base alle regole interne del pool (che saranno trattate nel corso MIN 201, poiché sono troppo complesse per MIN 101).



I partecipanti a un pool mining sono spesso chiamati "hasher", piuttosto che "minatori", poiché non svolgono più tutto il lavoro mining, ma si limitano a eseguire l'hash dei dati trasmessi loro dal pool.



Attenzione a non confondere il pool mining con la farm mining. Si tratta di due concetti diversi. Come abbiamo visto nel capitolo precedente, una farm è un sito fisico in cui un'unica entità gestisce numerose macchine mining. Un pool, invece, è soprattutto un raggruppamento virtuale: migliaia di macchine, spesso geograficamente disperse, lavorano sotto un coordinamento comune. Tuttavia, una farm può, e spesso lo fa, partecipare a un pool.
...
Dal punto di vista tecnico, i pool sono stati strutturati attorno a protocolli di comunicazione specializzati tra il pool e i minatori (ad esempio Stratum, poi Stratum V2), al fine di orchestrare in modo efficiente il lavoro distribuito. Approfondiremo questi concetti nel nostro corso di formazione MIN 201.
...
Al momento in cui scriviamo (inizio 2026), il Bitcoin globale è a un ordine di grandezza di zetta-hash al secondo (= 1.000 EH/s = 1.000.000.000.000.000 hash al secondo), e quasi tutti i blocchi trovati provengono da pool mining .



Ecco una classifica, ad oggi, dei principali pool mining e delle rispettive quote di hashrate. È probabile che questa classifica cambi nel momento in cui leggerete questo corso. Per dati aggiornati, visitare [mempool.space](https://mempool.space/graphs/mining/pools).
...
*Fonte [mempool.space](https://mempool.space/graphs/mining/pools), dati di un mese, dal 16 dicembre 2025 al 16 gennaio 2025.*



In un corso più avanzato, approfondiremo il funzionamento interno dei pool (azioni, protocolli di rete, metodi di pagamento...), perché è qui che entrano in gioco i dettagli che determinano sia la redditività del miner che le potenziali implicazioni per il Bitcoin.
