---
name: Mostro
description: Piattaforma Bitcoin P2P Exchange esente da KYC tramite Lightning e Nostr
---
...
**Mostro P2P** offre un'alternativa decentralizzata che combina il Lightning Network, il protocollo Nostr e le fatture di deposito. La sua principale innovazione: un sistema di deposito a garanzia automatizzato in cui i bitcoin rimangono sotto il vostro controllo per tutta la durata del Exchange, eliminando il rischio di furto, bancarotta del Exchange o confisca arbitraria.
...
- daemon Mostrod**: coordina gli scambi tra gli utenti e Lightning Network
...
**Flessibilità di pagamento**: Supporto per molte valute fiat tramite il servizio di tassi Exchange di yadio.io. Accetta bonifici SEPA, PayPal, Revolut, contanti e qualsiasi metodo concordato tra le parti.
...
Mostro offre due principali client operativi per gli scambi di P2P.
...
Il **Mostro CLI** è il client più maturo e funzionale. Scritto in Rust, offre l'intera gamma di funzioni di Mostro tramite la linea di comando Interface. È compatibile con macOS, Linux e Windows.
...
- `rilascio`: Rilascia l'escrow e finalizza il Exchange
- `getdm`: Visualizza i messaggi diretti
- `rate` : valutare la controparte dopo un Exchange
...
L'applicazione **mobile** in versione alpha permette di fare trading P2P direttamente dal proprio smartphone. Interface grafica intuitiva con navigazione a schede, visualizzazione degli ordini, filtri avanzati e chat integrata per comunicare con le controparti.
...
- Un Lightning Network** Wallet funzionante con liquidità sufficiente (o compatibile con Lightning)
 - Consigliato: Phoenix, Breez, Wallet, Satoshi...
...
- Opzionale ma consigliato**: Connessione VPN o Tor per mascherare il proprio IP Address
...
**Importante per la sincronizzazione CLI/Mobile**: Per accedere agli stessi ordini sul CLI e sull'app mobile, è necessario utilizzare la **stessa Mostro Pubkey** e gli **stessi relè Nostr** in entrambi i client. Controllare queste impostazioni in Impostazioni dell'app mobile.
...
- Protezione della rete**: Attivare VPN o Tor per nascondere il proprio IP Address
- Wallet secure**: Attiva il blocco automatico del tuo Wallet Lightning
...
Mostro vi chiederà di fornire un **Lightning Invoice** per ricevere il BTC. L'importo esatto in satoshi sarà indicato (qui: 4715 Sats).
...
### Fase 3: Fornire il proprio Lightning Invoice



generate un Lightning Invoice dal vostro Wallet (Phoenix, Breez, ecc.) per l'importo esatto, quindi inviatelo :
...
Il sistema conferma la spedizione e informa di attendere che il venditore paghi la trattenuta Invoice prima di attivare l'escrow.
...
Non appena il venditore conferma la ricezione del fiat e rilascia l'escrow con il comando `release`, si ricevono immediatamente i bitcoin sul Lightning Invoice fornito.
...
- Fulmine Address** (opzionale): Per ricevere direttamente i pagamenti
...
Premere il pulsante **"+"** del Green in basso a destra, quindi selezionare **VENDITA** (pulsante rosso).
...
**Istruzione importante**: Ora devi **pagare il Invoice hold** visualizzato per bloccare i tuoi bitcoin (qui 4674 Sats per 1-5 EUR) nell'escrow. Hai **15 minuti al massimo** altrimenti il Exchange verrà annullato.



Copiare la presa Invoice e pagarla dal proprio Wallet Lightning (Phoenix, Breez, ecc.).
...
Una volta **confermata la ricezione** del pagamento in fiat sul proprio conto, premere il pulsante Green **RELEASE** e confermare per inviare il Satss all'acquirente.
...
I Bitcoin vengono trasferiti istantaneamente all'acquirente tramite la sua Lightning Invoice.
...
4. Fornire il proprio Lightning Invoice (generato dal proprio Wallet)
...
9. Ricevi bitcoin istantaneamente sul tuo Lightning Invoice
...
**Sovranità finanziaria**: I vostri bitcoin non lasceranno mai il vostro controllo diretto grazie al meccanismo di blocco Invoice, eliminando il rischio di bancarotta o pirateria Exchange.
...
Mostro P2P rappresenta una promettente alternativa alle borse centralizzate, combinando Lightning Network, Nostr e l'escrow automatizzato per un trading veramente decentralizzato. Nonostante lo sviluppo iniziale e la liquidità limitata, la piattaforma offre già sovranità finanziaria, resistenza alla censura e anonimato di default.
