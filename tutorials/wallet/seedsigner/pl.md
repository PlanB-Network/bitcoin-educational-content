---
name: SeedSigner
description: Hardware Wallet, który jest DIY, bezpaństwowy, przystępny cenowo i w pełni hermetyczny
---
...
SeedSigner to Hardware Wallet Bitcoin o otwartym kodzie źródłowym, który każdy może zbudować samodzielnie przy użyciu niedrogich komponentów elektronicznych ogólnego przeznaczenia. W przeciwieństwie do produktów komercyjnych, takich jak Ledger, Coldcard lub Trezor, nie jest to gotowe do użycia urządzenie wyprodukowane przez firmę: jest to projekt społecznościowy, który pozwala każdemu stworzyć własne urządzenie, kontrolując każdy krok.



SeedSigner został zaprojektowany tak, aby był w 100% ***air-gapped***: nigdy nie łączy się z Internetem, nie ma Wi-Fi ani Bluetooth (w przypadku Raspberry Pi Zero v1.3) i nigdy nie jest podłączony do komputera w celu Exchange danych. Komunikacja odbywa się wyłącznie za pośrednictwem systemu Exchange z kodem QR. Mówiąc konkretnie, oprogramowanie do zarządzania portfelem (takie jak Sparrow wallet) wyświetla transakcję do podpisania w postaci kodów QR; użytkownik skanuje je za pomocą kamery SeedSigner, a następnie urządzenie podpisuje transakcję przy użyciu kluczy prywatnych tymczasowo przechowywanych w pamięci RAM. Na koniec generuje kody QR zawierające podpisaną transakcję, które użytkownik skanuje za pomocą swojego oprogramowania, aby wysłać je do sieci Bitcoin.
...
Inną opcją przechowywania seed i używania go z SeedSigner jest użycie karty inteligentnej *SeedKeeper* w połączeniu z kompatybilnym czytnikiem. Daje to bardzo solidny *secure element* do przechowywania seed, podczas korzystania z ekranu SeedSigner do podpisywania transakcji. Ale ta konkretna konfiguracja jest tematem innego dedykowanego samouczka. Tutaj skoncentrujemy się na podstawowym użyciu SeedSigner:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

Projekt SeedSigner zajmuje ważne miejsce w ekosystemie Bitcoin, ponieważ oferuje każdemu, na całym świecie, możliwość korzystania z zaawansowanych zabezpieczeń w celu ochrony swoich bitcoinów. Jego główną zaletą jest dostępność: wymagany sprzęt można kupić za mniej niż 50 USD. Co więcej, umożliwia to osobom mieszkającym w krajach o ograniczonym dostępie zbudowanie własnego Hardware Wallet ze standardowych komponentów komputerowych, które są łatwe do znalezienia i mniej podlegają ograniczeniom regulacyjnym.



Ale nawet poza tymi konkretnymi kontekstami, SeedSigner może być interesującą opcją dla ciebie: jest open-source, działa bezstanowo i jest airgapped, i redukuje wektory ataku związane z łańcuchem Supply twojego Hardware Wallet.
...
- Raspberry Pi Zero
 ...
 - Wersje W i v2 są również kompatybilne, ale zawierają układ Wi-Fi/Bluetooth. Dlatego też zaleca się jego fizyczną dezaktywację poprzez usunięcie modułu radiowego z karty. Operacja jest stosunkowo prosta, ale wymaga precyzji (w przypadku Zero W wystarczą cienkie szczypce, natomiast w przypadku v2 do usunięcia metalowej płytki skrywającej moduł potrzebny jest długopis Hot). Nie będę zagłębiał się w szczegóły w tym poradniku, ale wszystkie instrukcje znajdziesz w tym dokumencie: *[Sprzętowe wyłączanie WiFi/Bluetooth](https://github.com/DesobedienteTecnologico/rpi_disable_wifi_and_bt_by_hardware)*.
 ...
 - Nie zapomnij dołączyć zasilacza micro-USB Supply.
...
- Ekran Waveshare 1,3" (240×240 px)** (w języku francuskim)
    ...
    - Wyświetlacz zawiera trzy przyciski i mini-joystick służący jako Interface użytkownika.
...
- Obudowa (opcjonalna, ale zalecana)** (opcjonalna, ale zalecana)** (opcjonalna, ale zalecana)** (opcjonalna, ale zalecana)** (zalecana)
...
Następnie PowerShell oblicza Hash SHA256 pliku obrazu. Porównaj ten Hash z odpowiednią wartością w pliku `seedsigner.0.8.6.sha256.txt`.
...
Jeśli zmieniłeś orientację kamery lub chcesz zmienić inne ustawienia (takie jak język Interface) w późniejszym terminie, musisz włączyć trwałość ustawień na karcie microSD. W przeciwnym razie ustawienia zostaną przywrócone do wartości domyślnych przy każdym ponownym uruchomieniu, ponieważ Raspberry Pi Zero nie ma pamięci trwałej.
...
Aktualna wersja firmware'u obsługuje kilka pakietów oprogramowania: Sparrow, Specter, BlueWallet, Nunchuk oraz Keeper. W moim przypadku korzystam z **Sparrow wallet**, który szczególnie polecam ze względu na prostotę i bogatą funkcjonalność.
...
- `BTC`
- mBTC` (milli-Bitcoin lub 0,001 BTC)
- gW-20 (satoshi lub 1/100,000,000 BTC)
...
- gW-22 network`: należy zmodyfikować tylko, jeśli chcesz używać SeedSigner na Testnet.
...
Wreszcie, w menu `Language` można zmienić język Interface zgodnie z własnymi preferencjami.
...
Zwrot seed (lub Mnemonic) stanowi podstawę portfela Bitcoin. Służy do uzyskiwania kluczy prywatnych i adresów oraz zapewnia dostęp do funduszy. SeedSigner oferuje kilka metod jej generowania, które omówimy w tej sekcji.
...
- Ta fraza daje ci pełny, nieograniczony dostęp do wszystkich twoich bitcoinów.** Każdy, kto jest w posiadaniu tej frazy, może ukraść twoje fundusze, nawet bez fizycznego dostępu do twojego SeedSigner ;
- Zazwyczaj 12-wyrazowa fraza jest używana do przywrócenia Wallet w przypadku zgubienia lub kradzieży Hardware Wallet. Ale ponieważ SeedSigner jest urządzeniem *bezstanowym*, nigdy nie zarejestruje seed. Tak więc fizyczne kopie zapasowe nie są tylko kopiami zapasowymi, ale **jedynym sposobem korzystania z Wallet**. Jeśli utracisz te kopie zapasowe, twoje bitcoiny zostaną trwale utracone. Twórz więc kopie zapasowe ostrożnie, na kilku nośnikach i w bezpiecznych miejscach;
- Jeśli dopiero zaczynasz, zdecydowanie radzę przeczytać ten samouczek, aby uzyskać szczegółowe informacje na temat ryzyka związanego z zarządzaniem frazą Mnemonic:
...
- aparat": seed jest generowany z wizualnego szumu zdjęcia. Wykonujesz zdjęcie losowego środowiska (obiektu, krajobrazu, twarzy itp.), którego zmiany pikseli są wykorzystywane do entropii generate. Jest to szybka metoda, ale nie powtarzalna.
- rzut kostką": rzucasz kostką, aby stworzyć niezbędną entropię. Jest to bardziej czasochłonne, ale powtarzalne, a zatem weryfikowalne. Jeśli zdecydujesz się na tę metodę, postępuj zgodnie ze wskazówkami zawartymi w tym samouczku (nie ma potrzeby obliczania sumy kontrolnej, SeedSigner się tym zajmuje):
...
Następnie wybierz długość frazy Mnemonic. 12 słów zapewnia już wystarczający poziom bezpieczeństwa, więc jest to wybór, który polecam.
...
Ostrożnie zapisz słowa swojej frazy Mnemonic na odpowiednim fizycznym nośniku (papier lub metal).
...
Jeśli popełnisz błąd, SeedSigner poinformuje Cię o tym i będziesz musiał zacząć od nowa, pamiętając o zanotowaniu frazy Mnemonic, gdy zostanie Ci ona podana. Ten krok weryfikacji zapewnia, że kopia zapasowa jest poprawna i kompletna. Po zweryfikowaniu na ekranie pojawi się komunikat `Backup Verified`.
...
Radzę również przechowywać kod QR i frazę Mnemonic w dwóch oddzielnych miejscach, aby uniknąć utraty wszystkiego w przypadku reklamacji.



Wreszcie, bardziej zaawansowaną i bezpieczną alternatywą jest użycie SeedSigner z **SeedKeeper**, który przechowuje seed w secure element. Aby dowiedzieć się więcej, zobacz ten samouczek :
...
Na tym etapie można również dodać **passphrase BIP39**, aby wzmocnić bezpieczeństwo Wallet. Ta opcja może być opłacalna, w zależności od strategii tworzenia kopii zapasowych, ale wiąże się również z ryzykiem: jeśli stracisz passphrase, dostęp do twoich bitcoinów zostanie trwale utracony.
...
Aby to zrobić, potrzebny będzie pusty papierowy lub metalowy kod QR odpowiadający długości frazy Mnemonic. Jeśli zakupiłeś kompletny pakiet SeedSigner, szablony są zazwyczaj dołączone. Jeśli nie, możesz je pobrać i wydrukować (lub odtworzyć ręcznie) tutaj :




- [format 12 słów](https://github.com/SeedSigner/seedsigner/blob/dev/docs/seed_qr/printable_templates/grid_25x25.pdf)
- [format 24 słów](https://github.com/SeedSigner/seedsigner/blob/dev/docs/seed_qr/printable_templates/grid_29x29.pdf)
...
Na ekranie seed wybierz `Backup seed`.
...
Starannie odtwórz każdą czarną kropkę na arkuszu zapisu, a następnie użyj joysticka, aby przejść do następnego BLOCK. Nie spiesz się: zwykłe niedopasowanie może sprawić, że kod QR będzie bezużyteczny.
...
Gratulacje, portfolio Bitcoin jest już gotowe do pracy! Teraz zaimportujemy jego publiczne komponenty do **Sparrow wallet**, aby łatwo nim zarządzać.
...
Pojawi się komunikat ostrzegawczy. Jest to normalne: ten rozszerzony klucz publiczny (`xpub`) umożliwia przeglądanie wszystkich adresów pochodzących z seed (na pierwszym koncie). Nie pozwala na wydawanie środków, ale ujawnia strukturę portfela. Jeśli kiedykolwiek wycieknie, będzie to problem dla prywatności, ale nie dla bezpieczeństwa bitcoinów: pozwala je zobaczyć, ale nie wydać.
...
Upewnij się, że masz zainstalowane oprogramowanie Sparrow wallet na swoim komputerze. Jeśli nie wiesz, jak pobrać, sprawdzić i zainstalować je poprawnie, zapoznaj się z naszym pełnym samouczkiem na ten temat:
...
Przypisz nazwę do portfela, a następnie kliknij `Create Wallet`. Sparrow poprosi o ustawienie hasła, aby zablokować lokalny dostęp do tego Wallet. Wybierz silne hasło: chroni ono dostęp do danych portfela w Sparrow (klucze publiczne, adresy, etykiety i historia transakcji). To hasło nie jest potrzebne do przywrócenia portfela w późniejszym terminie: do tego celu wymagana jest tylko fraza Mnemonic (i ewentualnie passphrase).
...
SeedSigner i Sparrow wallet są teraz bezpiecznie połączone. Sparrow działa jako kompletne zarządzanie Interface, podczas gdy SeedSigner pozostaje jedynym urządzeniem zdolnym do podpisywania transakcji. Jesteś teraz gotowy do odbierania i wysyłania bitcoinów w całkowicie hermetycznej konfiguracji.
...
#### 7.1.1 Generowanie odbioru Address



Na komputerze otwórz Sparrow wallet i odblokuj SeedSigner Wallet za pomocą hasła. Upewnij się, że oprogramowanie jest połączone z serwerem (wycięcie w prawym dolnym rogu). Na pasku bocznym kliknij `Receive`.
...
Wyświetlony zostanie nowy Bitcoin Address. Zobaczysz :




- Tekst Address (zaczynający się od `bc1p...`, jeśli używasz P2TR tak jak ja),
- Odpowiedni kod QR,
- Pole `Label` do śledzenia transakcji.



Zdecydowanie zalecam dodanie etykiety do każdego paragonu Bitcoin na Wallet. Pozwoli to łatwo zidentyfikować pochodzenie każdego UTXO i usprawni zarządzanie prywatnością. Aby zagłębić się w ten ważny temat, możesz zapoznać się z dedykowanym szkoleniem w Plan ₿ Academy :
...
Twój Address jest teraz powiązany z tą etykietą we wszystkich sekcjach Sparrow.
...
Przed udostępnieniem otrzymanego Address bardzo ważne jest sprawdzenie, czy należy on do seed. Ten krok gwarantuje, że SeedSigner będzie mógł podpisywać transakcje powiązane z tym Address. Chroni to również przed możliwymi atakami, w których Sparrow wyświetla fałszywy Address. Należy pamiętać, że Sparrow działa w niezabezpieczonym środowisku (na komputerze użytkownika), które ma znacznie większą powierzchnię ataku niż SeedSigner, który jest całkowicie odizolowany. Dlatego nigdy nie powinieneś ślepo wierzyć odbiorcy Address wyświetlanemu na Sparrow, dopóki nie zweryfikujesz go za pomocą Hardware Wallet.



W Sparrow kliknij kod QR Address, aby go powiększyć: zostanie on wyświetlony na pełnym ekranie.
...
Jeśli zeskanowany Address jest zgodny z seed, na ekranie SeedSigner zostanie wyświetlony komunikat: `Address Verified`.
...
Potwierdza to, że Address należy do twojego Wallet i że możesz bez obaw odbierać z niego bitcoiny.
...
Możesz teraz przekazać ten Address (w formie tekstu lub kodu QR) osobie lub działowi, który musi wysłać Ci Satss. Gdy transakcja zostanie rozesłana w sieci, pojawi się w zakładce "Transakcje" w Sparrow wallet.
...
- W `Pay to` wklej Address odbiorcy lub kliknij ikonę aparatu, aby zeskanować kod QR;
...
Włącz SeedSigner i zeskanuj SeedQR, aby uzyskać dostęp do swojego portfela, jak zwykle. Na ekranie głównym wybierz `Scan`, a następnie zeskanuj kod QR wyświetlony na Sparrow.
...
Kliknij `Review Details` i dokładnie sprawdź wszystkie informacje bezpośrednio na ekranie SeedSigner. Najważniejszymi elementami do sprawdzenia są wysłana kwota, Address odbiorcy i kwota wszelkich zastosowanych opłat.
...
Teraz, gdy transakcja jest ważna, należy ją rozgłosić w sieci Bitcoin, aby dotarła do Miner, który doda ją do BLOCK.



W Sparrow kliknij `QR Scan`.
