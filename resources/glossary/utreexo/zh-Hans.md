...
在这个例子中，为了便于理解，我有意将 UTXO 集减少到 4 个 UTXO。实际上，在撰写这几行文字时，比特币上有近 1.4 亿个 UTXO，这一点很重要。在本图中，Utreexo 节点只需要将 Merkle Root 保存在 RAM 中。如果它收到一笔花费第 3 号 UTXO（黑色）的交易，证明将由以下要素组成：
...
- 它计算出 UTXO 3 的印记，从而得到 HASH 3；
- 它将 HASH 3 与 HASH 4 连接起来；
- 它会计算它们的印记，从而得到 HASH 3-4；
- 它将 HASH 3-4 与 HASH 1-2 连接起来；
- 它计算出它们的印记，从而得到哈希树根。

如果通过该过程获得的 Merkle 根与存储在 RAM 中的 Merkle 根相同，那么它就确信 第 3 号 UTXO 确实是 UTXO 集的一部分。
