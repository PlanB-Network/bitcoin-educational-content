...
現在我們可以建立 RGB 資產。指令將取決於您想要建立的資產類型及其參數。在這裡，我要建立一個名稱為 "Plan ₿ Academy" 的 NIA (*Non Inflatable Asset*) 符記，Supply 為 1000 個單位。精確度」允許您定義單位的可分割性。
...
接下來，我們將透過指定相關資產 (`Plan ₿ Academy`)來開啟通道。`/openchannel` 指令可讓您以 satoshis 為單位定義通道的大小，並選擇是否包含 RGB 資產。這取決於您想要建立什麼，但在我的情況下，命令是 ：
...
Lightning channel（閃電通道）現在打開，節點 n°1 一側也包含 500 `Plan ₿ Academy` 代幣。如果節點 n°2 希望接收 `Plan ₿ Academy` 代幣，它必須 generate 和 Invoice。以下是操作方法：
