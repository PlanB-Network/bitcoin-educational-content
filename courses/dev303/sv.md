...
LDK:s arkitektoniska flexibilitet öppnar upp för många användningsfall som sträcker sig långt bortom traditionella Lightning-noddistributioner. Mobil wallet-utveckling representerar en av de mest övertygande applikationerna, där LDK möjliggör skapandet av icke-frihetsberövande Lightning-plånböcker som liknar Phoenix wallet. Dessa mobila implementeringar kan behålla användarkontrollen över [privata nycklar](https://planb.academy/resources/glossary/private-key) samtidigt som de synkroniseras med Lightning Service Providers (LSP) när de kommer online, vilket möjliggör sömlös betalningsmottagning och kanalhantering även med intermittent anslutning.
...
För resursbegränsade miljöer tillhandahåller LDK en filtreringsbaserad metod som minskar kraven på bandbredd och lagring. I det här läget kommunicerar LDK sina övervakningsintressen genom abstrakta gränssnitt och begär övervakning av specifika transaktions-ID, [UTXO](https://planb.academy/resources/glossary/utxo) eller skriptmönster. Applikationslagret kan sedan implementera denna övervakning med hjälp av Electrum-servrar, blockutforskare eller andra lätta blockkedjedatakällor. Detta tillvägagångssätt gör det möjligt för mobila plånböcker och webbapplikationer att upprätthålla Lightning-funktionalitet utan att kräva fullständig blockkedjesynkronisering.
