...
Bisq est une plateforme d'échange décentralisée pour les actifs numériques, principalement Bitcoin. Elle permet des transactions directes, sécurisées et privées entre les utilisateurs du monde entier, sans avoir besoin d'un intermédiaire.

Site web : https://bisq.network/
info : https://bisq.network/getting-started/

_Faites attention, Bisq est une solution avancée : une fois installée, elle offre de grandes possibilités. Cependant, si vous êtes totalement débutant, elle pourrait ne pas être adaptée à votre niveau._

## vidéo tutoriel

Tutoriel complet par BTC Sessions :
...
_Si vous connaissez un guide plus récent, illustré et adapté aux débutants, n’hésitez pas à le partager avec nous !_
...
Si vous souhaitez construire Bisq à partir à partir du code source, voici les instructions à suivre (https://github.com/bisq-network/bisq/blob/master/docs/build.md). Bisq est un logiciel libre et open source auquel vous pouvez contribuer. (https://bisq.wiki/Contributor_checklist)

![image](assets/1.webp)

2. **Sauvegarder vos clés, noter vos mots de récupération**
...
Avec Bisq, vous avez un contrôle total sur vos fonds et vos données. Cela signifie que vous conservez une souveraineté inégalée, mais cela signifie également que personne ne peut vous aider si vous perdez quelque chose d'important, il est donc essentiel de faire des sauvegardes appropriées avant d'utiliser Bisq pour échanger.
...
Afin d'échanger du bitcoin sur Bisq, vous devez mettre en place un moyen d'envoyer ou de recevoir d'autres fonds. Bisq ne gère que le côté bitcoin d'une transaction - l'autre côté est géré par des services de paiement en monnaie fiduciaire (banques, mandats, espèces) ou des portefeuilles d'altcoins.
Vous ne savez pas quel type de compte de paiement mettre en place ? Vous trouverez une liste complète des méthodes de paiement sur le wiki (https://bisq.wiki/Payment_methods). 
Si vous prévoyez d'échanger de la monnaie fiduciaire, assurez-vous de regarder la vidéo sur les limites de compte ou de consulter l’article correspondant sur le wiki. (https://bisq.wiki/Account_limits).

Il y a de nombreuses altcoins disponibles pour l'échange sur Bisq. Voici quelques conseils pour les principaux marchés :
...
## Guide étape par étape pour échanger

Guide étape par étape par Bitcoiner.guide, alias bitcoinQ&A https://bitcoiner.guide/bisq/

Maintenant que votre méthode de paiement est configurée, vous pouvez aller dans l'onglet "Acheter BTC" et voir les vendeurs proposant du bitcoin en échange de votre méthode de paiement choisie. Ici, vous pouvez voir le prix du BTC, la différence en pourcentage par rapport au prix "spot" et la quantité de bitcoin en vente de chaque vendeur. Une fois que vous avez trouvé une offre qui vous convient, cliquez sur "Accepter l'offre pour acheter du BTC".

![image](assets/3.webp)

Vous serez redirigé vers un écran où vous pourrez vérifier et confirmer les détails avant de lancer la transaction. Une fois tout vérifié, cliquez sur "étape suivante".

![image](assets/4.webp)

Vous devez maintenant approvisionner votre portefeuille d'échange avec une petite quantité de bitcoins qui servira de dépôt de garantie. Cela représente généralement environ 15% de la taille de la transaction. Le vendeur doit également verser un dépôt de garantie de son côté.

![image](assets/5.webp)

Vous pouvez maintenant confirmer l'offre et démarrer la transaction.

![image](assets/6.webp)

Vous devrez maintenant patienter quelques instants pendant que le réseau Bitcoin confirme la transaction, ce qui place les dépôts de garantie des deux parties dans l'escrow multi-sig.

![image](assets/7.webp)

Après une confirmation sur la chaîne, vous verrez ensuite une fenêtre avec les coordonnées du vendeur pour effectuer le paiement.

![image](assets/8.webp)

Une fois que vous avez effectué le paiement, appuyez sur le bouton pour confirmer que vous l'avez fait, puis attendez que le vendeur confirme qu'il a bien reçu vos fonds. À ce stade, vous pouvez également discuter avec votre partenaire commercial via une messagerie chiffrée de bout en bout en appuyant sur "Ouvrir le chat du trader".
...
Voilà, votre première transaction est maintenant terminée ! Vous pouvez maintenant soit retirer vos bitcoins vers un portefeuille externe, soit les laisser dans votre compte Bisq.
...
Les échanges décentralisés peuvent parfois souffrir de pénuries de liquidités par rapport aux entités centralisées plus importantes. Si vous cherchez à acheter des montants plus importants plus fréquemment, vous pourriez rencontrer des difficultés.

Bien que cela ne me soit jamais arrivé, il arrive parfois que les transactions de pair à pair ne se déroulent pas comme prévu et que les utilisateurs doivent passer par le processus de résolution des litiges. Heureusement, grâce à la structure d'incitation de Bisq, ces cas sont rares.

Comme pour la plupart des choses liées à Bitcoin, le débat KYC/Non-KYC est nuancé et plein de compromis. Je comprends que les rampes d'accès fiat les plus courantes comme Coinbase et CashApp facilitent grandement l'achat, en particulier pour les débutants, mais prenez s'il vous plaît un instant pour considérer les compromis et vous renseignez sur les alternatives avant de donner si facilement vos informations personnelles.
