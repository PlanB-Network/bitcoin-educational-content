---
name: Mostro
description: KYC-fri Bitcoin P2P Exchange-plattform via Lightning og Nostr
---
...
**Mostro P2P** tilbyr et desentralisert alternativ som kombinerer Lightning Network, Nostr-protokollen og hold-fakturaer. Den viktigste innovasjonen: et automatisert escrow-system der bitcoinsene dine forblir under din kontroll gjennom hele Exchange, noe som eliminerer risikoen for tyveri, Exchange-konkurs eller vilkårlig konfiskering.
...
Mostro P2P er en åpen kildekode Bitcoin Exchange-protokoll opprettet av **grunch **, utvikler av den populære Telegram-bot **lnp2pbot ** lansert i 2021. Mens lnp2pbot allerede muliggjorde KYC-fri P2P-utveksling via Lightning, presenterte den en stor sårbarhet: **Telegram utgjør et sentralisert feilpunkt** som er utsatt for sensur fra myndigheter.
...
- daemon Mostrod**: koordinerer utveksling mellom brukere og Lightning Network
...
**Desentralisert voldgift**: Ved en eventuell tvist undersøker en upartisk voldgiftsdommer bevisene og tar en avgjørelse basert på transparente kriterier. Hver tvist genererer en unik token for sporbarhet.



**Fleksibilitet i betaling**: Støtte for mange fiat-valutaer via yadio.io Exchange-kurstjenesten. Aksepterer SEPA-overføringer, PayPal, Revolut, kontanter og andre metoder som avtales mellom partene.
...
**Mostro CLI** er den mest modne og funksjonelle klienten. Den er skrevet i Rust og tilbyr hele spekteret av Mostro-funksjoner via en Interface-kommandolinje. Kompatibel med macOS, Linux og Windows.
...
- frigjør: Frigjør sperringen og fullfør Exchange
- `getdm`: Vis direkte meldinger
- `rate` : Evaluer motparten din etter en Exchange
...
Den **mobile appen** i alfaversjon gjør det mulig å handle P2P direkte fra smarttelefonen din. Intuitiv grafisk Interface med navigering i faner, ordrevisning, avanserte filtre og integrert chat for å kommunisere med motpartene dine.
...
- Valgfritt, men anbefalt**: VPN- eller Tor-tilkobling for å maskere din IP Address
...
- Nettverksbeskyttelse**: Aktiver VPN eller Tor for å skjule IP-en din Address
- Wallet sikker**: Aktiver den automatiske låsen på din Wallet Lightning
...
Denne delen demonstrerer kjøp av bitcoins via Mostro CLI etter et virkelig brukstilfelle.
...
Mostro vil be deg om å oppgi en **Lightning Invoice** for å motta BTC. Det nøyaktige beløpet i satoshis vil bli angitt (her: 4715 Sats).
...
### Trinn 3: Oppgi din Lightning Invoice



generate en Lightning Invoice fra din Wallet (Phoenix, Breez osv.) for det nøyaktige beløpet, og send det deretter :
...
Systemet bekrefter forsendelsen og informerer deg om at du må vente på at selgeren betaler Invoice-beløpet før du aktiverer spærringen.
...
Så snart selgeren bekrefter mottak av fiat og frigjør sperringen med kommandoen `release`, mottar du øyeblikkelig bitcoinsene dine på Lightning Invoice du leverte.
...
- Lightning Address** (valgfritt): For å motta betalinger direkte
...
Trykk på Green **"+"**-knappen nederst til høyre, og velg deretter **SELL** (rød knapp).
...
**Viktig instruksjon**: Du må nå **betale Invoice hold** som vises for å låse bitcoinsene dine (her 4674 Sats for 1-5 EUR) i escrow. Du har **15 minutter maksimalt** ellers vil Exchange bli kansellert.



Kopier Invoice-reservasjonen og betal den fra Wallet Lightning (Phoenix, Breez osv.).
...
Når du har **bekreftet mottak** av fiat-betalingen på kontoen din, trykker du på Green **RELEASE**-knappen og bekrefter å sende Satss til kjøperen.
...
Bitcoins overføres umiddelbart til kjøperen via hans Lightning Invoice.
...
4. Oppgi din Lightning Invoice (generert fra din Wallet)
...
9. Motta bitcoins umiddelbart på din Lightning Invoice
...
**Økonomisk suverenitet**: Bitcoinsene dine forlater aldri din direkte kontroll takket være Invoice-hold-mekanismen, noe som eliminerer risikoen for Exchange-konkurs eller piratkopiering.
...
For bitcoinere som foretrekker autonomi og konfidensialitet, er Mostro et levedyktig alternativ som fortjener progressiv utforskning. Den desentraliserte arkitekturen garanterer fellesskap snarere enn kommersiell utvikling, noe som baner vei for en fremtid med virkelig frie Bitcoin-børser.
