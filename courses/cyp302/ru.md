---
name: Основы современной криптографии
...
---
# Глубокое погружение в современную криптографию

В этом курсе мы рассмотрим основы современной криптографии ясным и доступным способом, без необходимости серьёзной математической подготовки. В главах вы изучите ключевые концепции, такие как симметричное и асимметричное шифрование, хеш-функции, цифровые подписи, обмен ключами и реальные протоколы. По ходу дела мы свяжем точки с практическими приложениями, такими как безопасный обмен сообщениями, TLS, хранение паролей и аутентификация.

Материал разработан для учащихся всех уровней и сочетает интуицию с достаточной технической глубиной для удовлетворения любопытства. Ожидайте сфокусированное, увлекательное путешествие. К концу вы поймёте, как и почему работает современная криптография и как использовать её ответственно.
+++
# Введение
...
Добро пожаловать на курс CYP302!
...
Этот образовательный контент адаптирован из книги и repo [JWBurgers](https://github.com/JWBurgers/An_Introduction_to_Cryptography). Хотя автор любезно разрешил его использование в образовательных целях, все права интеллектуальной собственности остаются за первоначальным создателем.
...
Давайте начнем наше исследование области [криптографии](https://planb.academy/resources/glossary/cryptography) с одного из самых очаровательных и забавных эпизодов в ее истории - шифров Била. [1]
...
Важно отметить, что современная криптография - это не только изучение криптографических схем с симметричными и ассиметричными ключами (хотя это и охватывает большую часть области). Например, криптография также занимается [хэш-функциями](https://planb.academy/resources/glossary/hash-function) и генераторами псевдослучайных чисел, и на этих примитивах можно строить приложения, не связанные с криптографией симметричных или ассиметричных ключей.
...
Предположим, что вы равномерно выбрали ключ со значением $17$. Буква «O» соответствует $14$. Без операции модуля сложение этого числа открытого текста с ключом дало бы число шифртекста $31$. Однако это число нельзя преобразовать в букву шифра, так как в английском алфавите всего $26$ букв. Операция модуля обеспечивает, что число шифртекста на самом деле равно $5$ (результат $31 \mod 26$), что соответствует букве шифра «F».

Полный шифр слова "DOG" со значением ключа 17 выглядит следующим образом:


**Сообщение = DOG = D,O,G = 3,14,6**
...
2. Для любых $x$, $y$ и $z$, являющихся элементами $\mathbb{Z}$, $(x + y) + z = x + (y + z)$. Таким образом, $\langle \mathbb{Z}, + \rangle$ удовлетворяет условию ассоциативности.

3. В $\langle \mathbb{Z}, + \rangle$ существует элемент тождества, а именно 0. Для любого $x$ в $\mathbb{Z}$ имеет место то, что: $0 + x = x + 0 = x$. Таким образом, $\langle \mathbb{Z}, + \rangle$ удовлетворяет условию тождества.

4. Наконец, для каждого элемента $x$ в $\mathbb{Z}$ найдется такой $y$, что $x + y = y + x = 0$. Например, если $x$ равно 10, то $y$ будет равно $-10$ (в случае, когда $x$ равно 0, $y$ также равно 0). Значит, $\langle \mathbb{Z}, + \rangle$ удовлетворяет обратному условию.
...
(которое является наибольшим простым числом, меньшим чем $2^{256}$).
...
К **бесконечному полю** относится поле, в котором множество **S** бесконечно велико. Примером бесконечного поля является множество вещественных чисел $\mathbb{R}$, оснащенное сложением и умножением. **Конечное поле**, также известное как поле **Галуа**, - это поле, в котором множество **S** конечное. В нашем примере $\langle \mathbb{Z} \mod 7, +, \cdot \rangle$ является конечным полем.
...
Первым решением, которое они предложили, был удаленный протокол *обмена ключами*, то есть набор правил для обмена одним или несколькими симметричными ключами по незащищенному каналу связи. Этот протокол сегодня известен как *обмен ключами [Диффи-Хельмана](https://planb.academy/resources/glossary/diffiehellman)* или *обмен ключами Диффи-Хельмана-Меркле*. [2]
...
Схемы на основе дискретного логарифма опираются на два основных вида циклических групп: мультипликативные группы целых чисел и группы, включающие точки на [эллиптических кривых](https://planb.academy/resources/glossary/elliptic-curve). Оригинальный обмен ключами Диффи-Хельмана, представленный в книге "Новые направления в криптографии", работает с циклической мультипликативной группой целых чисел. Алгоритм [цифровой подписи](https://planb.academy/resources/glossary/digital-signature) Bitcoin и недавно представленная схема [подписи Шнорра](https://planb.academy/resources/glossary/schnorr-protocol) (2021) основаны на задаче дискретного логарифма для определенной циклической группы эллиптических кривых.
...
Примером популярной хэш-функции является **[SHA-256](https://planb.academy/resources/glossary/sha256)** (безопасный хэш-алгоритм 256). Независимо от размера входных данных (например, 15 бит, 100 бит или 10 000 бит), эта функция выдает 256-битное хэш-значение. Ниже приведено несколько примеров вывода функции SHA-256.
