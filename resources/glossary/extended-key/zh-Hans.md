...
definition: 包含密钥、链码（chain code）及元数据的字符串，用于 HD 钱包的密钥衍生。
---
...
在实践中，扩展公钥用于生成接收地址和观察账户的交易情况，而不会暴露相关的私钥。例如，这样就可以创建一个所谓的“观察钱包”。不过，需要注意的是，扩展公开密钥是用户隐私的敏感信息，因为它的泄露会让第三方追踪交易并查看相关账户的余额。
