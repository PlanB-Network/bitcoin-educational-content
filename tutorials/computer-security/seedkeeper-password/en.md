...
In concrete terms, the Seedkeeper takes the form of a multifunctional, EAL6-certified smart card with a secure processor and tamper-proof memory (i.e. a "*secure element*"). As its name suggests, its role is to store Bitcoin portfolio mnemonics and passwords in an encrypted and protected way. With Seedkeeper, you can generate, import, organize and save your secrets directly in the card's secure component.
...
Assign a "*Label*" to this secret so that it can be easily identified if you store several pieces of information in your Seedkeeper. You can also add a username associated with the password and the URL of the site.
...
This takes you to a list of all your registered secrets. In this example, I want to display the password for my Plan ₿ Network account, so I click on it.
...
That's all there is to it! Now you know how to use Seedkeeper to store your passwords. In a future tutorial, we'll look at how to use Seedkeeper to back up a Bitcoin portfolio. I also invite you to discover its combined use with SeedSigner :
