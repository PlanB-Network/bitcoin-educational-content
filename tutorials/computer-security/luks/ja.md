...
ここで、"`luksFormat`"サブコマンドはデバイスを初期化し、LUKS暗号化を使用するようにフォーマットする。大文字で`YES`と入力してこの操作を確認するようプロンプトが表示されるので、次に*passphrase*を定義する。 **passphrase*は、紛失した場合に攻撃者がブルートフォース攻撃で発見できないように、堅牢なものを選 択してください。
