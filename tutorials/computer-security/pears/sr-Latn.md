...
U ovom vodiču ćemo naučiti kako pokrenuti aplikacije na **Pears**, peer-to-peer (P2P) tehnologiji koju je razvila kompanija **Holepunch** uz podršku **Tether**. Cilj je jednostavan: omogućiti distribuciju i korišćenje web aplikacija bez oslanjanja na bilo kakvu centralizovanu infrastrukturu (bez servera, domaćina, posrednika). Drugim rečima, čak i ako provajder oblaka prestane sa radom ili neka zemlja blokira domen, aplikacija nastavlja da živi među mrežnim vršnjacima.
...
Pears je okruženje za izvršavanje, alat za razvoj i platforma za implementaciju aplikacija od korisnika do korisnika. Ovaj alat otvorenog koda omogućava izradu, deljenje i pokretanje softvera bez servera ili infrastrukture, direktno između korisnika. U konkretnim terminima, to znači da umesto hostovanja aplikacije na centralnom serveru, svaki korisnik postaje mrežni čvor, deleći deo aplikacije i podataka sa drugim korisnicima. Ceo sistem formira distribuiranu mrežu, pri čemu svaka instanca sarađuje kako bi usluga ostala dostupna.
...
- Hypercore**: distribuirani log koji garantuje doslednost podataka i sigurnost bez centralne baze podataka.
- Hyperbee**: indeksator na vrhu Hypercore-a, za efikasnu organizaciju i pregledavanje podataka.
- Hyperdrive**: distribuirani datotečni sistem koji se koristi za skladištenje i sinhronizaciju aplikacionih datoteka između vršnjaka.
...
- Secretstream**: E2E enkripcioni protokol za osiguranje razmene između dva korisnika.
...
*Ovaj vodič za instalaciju Pears-a podeljen je na nekoliko delova u zavisnosti od vašeg operativnog sistema. Idite direktno na deo koji odgovara vašem okruženju kako biste pratili odgovarajuća uputstva :*
...
Instalacija Pears na Debian sistemu je relativno jednostavna, ali zahteva nekoliko preduslova, koje ćemo detaljno objasniti u ovom odeljku.
...
Pears se distribuira putem *npm*, menadžera paketa za *Node.js*. Iako Pears ne zavisi direktno od *Node.js* za funkcionisanje, neophodan je za instalaciju. Preporučeni metod za instaliranje *Node.js* na Linuxu je *NVM* (*Node Version Manager*), koji vam omogućava upravljanje sa nekoliko verzija Node-a paralelno.
...
### 3.6. Testiranje krušaka sa Keetom
...
Instaliranje Pears-a na Windows-u je jednako lako kao i na Linux-u, ali zahteva nekoliko posebnih alata.
...
Pears se instalira putem *npm*, menadžera paketa za *Node.js*. Na Windows-u, metoda koju preporučuje Holepunch je korišćenje *NVS* (*Node Version Switcher*), koji je stabilniji od *NVM* na ovom sistemu.
...
Trebalo bi da vidite listu dostupnih verzija *Node.js*. Izaberite prvu tako što ćete pritisnuti taster `a` na tastaturi.
...
Kada su *Node.js* i *npm* dostupni, instalirajte **Pears CLI** globalno na vašem sistemu:
...
Ovo će instalirati `pear` binarnu datoteku u vaš globalni *npm* direktorijum.
...
Ako je sve prošlo dobro, trebali biste videti CLI Pears ekran za pomoć sa listom dostupnih pod-komandi (run, seed, info...).



### 4.7. Testiranje krušaka sa Keet
...
Instaliranje Pears na macOS-u je slično instaliranju na Linuxu, ali zahteva nekoliko prilagođavanja specifičnih za Apple okruženje. Hajde da zajedno otkrijemo ove korake.
...
Pears se distribuira putem *npm*, *Node.js* menadžera paketa. Iako Pears ne zavisi direktno od *Node.js* za funkcionisanje, neophodan je za instalaciju. Preporučeni metod za instaliranje *Node.js* na macOS je *NVM* (*Node Version Manager*), koji vam omogućava da upravljate sa nekoliko verzija Node-a paralelno.
...
Kada je *npm* dostupan, možete instalirati Pears CLI globalno na vašem sistemu. Ovo će vam omogućiti da pokrenete komandu `pear` iz bilo kog direktorijuma.
...
Na prvom pokretanju, Pears će se povezati na peer-to-peer mrežu kako bi preuzeo potrebne komponente. Ovaj proces ne zahteva centralni server: fajlovi se dobijaju direktno od drugih korisnika.
...
Ako je sve ispravno instalirano, biće prikazana Pears Help sa listom dostupnih komandi.
...
Da biste naučili kako pokrenuti našu platformu Plan ₿ Academy na Pears, pogledajte ovaj sveobuhvatni vodič :
