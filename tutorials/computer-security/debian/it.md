...
Debian è una distribuzione GNU/Linux gratuita, rinomata per la sua robustezza e affidabilità. Il suo kernel Linux e tutti i suoi pacchetti sono rigorosamente testati per garantire una stabilità solida come una roccia e un alto livello di sicurezza. Adatta sia per i server che per le postazioni di lavoro, Debian offre un'esperienza facile da usare e un vasto catalogo di software. Se state cercando un sistema sicuro per l'uso quotidiano o per un ambiente di produzione, Debian è la scelta giusta.
...
- **Cinnamon**: interfaccia semplice ed elegante, ispirata a Windows.
...
Prima di iniziare l'installazione, accertati di disporre delle seguenti apparecchiature:
...
- **Altre architetture**: trova l'ISO corrispondente alla tua architettura [qui](https://www.debian.org/CD/http-ftp/#stable).
...
- Scarica **Balena Etcher** dal [sito web ufficiale](https://etcher.balena.io/), quindi ottieni il binario per il tuo sistema e installalo.
...
- **Riavvia** quindi accedi al BIOS/UEFI premendo immediatamente `ESC`, `F2`, `F11` (o il tasto dedicato a seconda della marca).
- Nel menu di avvio, **seleziona la chiave USB** come dispositivo di avvio.
- **Conferma** con il tasto "Invio" per avviare l'immagine Debian: questo ti porterà alla schermata di benvenuto dell'installer.
...
- **Live System**: lancia Debian senza installarlo, ideale per testare l'ambiente.
...
Seleziona la lingua principale del sistema Debian dall'elenco, quindi fai clic su OK.

![language](assets/fr/06.webp)

- **Fuso orario** (GMT)

Scegli la zona geografica per impostare automaticamente la data e l'ora.
...
Viene visualizzato un riepilogo delle scelte effettuate: controlla ogni voce e torna indietro per modificarla, se necessario.

![settings](assets/fr/11.webp)

- Avvio dell'installazione

Fai clic su **Install** per avviare la copia dei file e la configurazione del sistema, quindi attendi il completamento del processo.

![install](assets/fr/12.webp)

- **Riavvio**

Una volta completata l'installazione, riavvia il computer per applicare tutte le configurazioni e accedere al nuovo sistema Debian.
...
### Opzione 1: Aggiornamento tramite interfaccia grafica (GNOME)

Se hai installato Debian con l'ambiente desktop GNOME, è possibile eseguire gli aggiornamenti graficamente. Per farlo, aprire l'applicazione **Software**, quindi andare alla scheda **Aggiornamenti**. Cliccare quindi su **Riavvia e aggiorna** per avviare il processo.
...
- Apri il terminale utilizzando la scorciatoia `Ctrl + Alt + T`.
...
- Per scrivere documenti, utilizza **LibreOffice Writer**, l'equivalente di Microsoft Word.
...
È possibile utilizzare il **software manager** (accessibile tramite l'interfaccia grafica) per cercare e installare facilmente le applicazioni.

### Metodo a riga di comando:

Se l'applicazione che stai cercando non appare nell'interfaccia grafica o se preferisci il terminale, usa il seguente comando:
...
Grazie all'ambiente desktop **GNOME**, è possibile accedere a un'ampia gamma di applicazioni tramite un'interfaccia grafica di facile utilizzo, ideale sia per i principianti che per gli utenti avanzati.
