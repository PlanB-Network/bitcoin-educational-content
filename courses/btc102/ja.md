...
- Bitcoinのエコシステムは、強力な分散化、長期的な回復力、プライバシー、そして個人の主権を基盤にした健全な貨幣という考え方を中心にしています。2009年の開始以来、Bitcoinはグローバルで献身的な開発者コミュニティによって支えられ、確実かつ安全に運用されてきました。Bitcoinは一過性のトレンドではなく、長期にわたって価値を維持するように設計された、安定し確立されたプロトコルなのです。
...
- トレーディング**と投資**の違いを知る；
- 投資の**税金**への影響に注意すること；
- [回復フレーズ](https://planb.academy/resources/glossary/recovery-phrase)**を大切に守ってください**；
...
- ルールno.1：決して損失が出る以上の投資はしないこと。**Bitcoin**は不安定な資産です。利益を追い求め、経済的安定を危険にさらさないでください。あなたの本質的なニーズと心の平穏を常に優先してください。
...
簡単に言うと、Bitcoinネットワークの参加者（「[ノード](https://planb.academy/resources/glossary/node)」と呼ばれる）はすべて、Blockchainとして知られるトランザクションLedgerの完全なコピーを持っている。新しい取引が発生すると、ネットワークにブロードキャストされる。その後、マイナーはこれらのトランザクションをブロックにまとめ、チェーンの末尾に追加することで確認する（これが「Blockchain」という名前の由来である）。
...
- アナーキストは、Bitcoinを国家統制からの解放のための道具と見なしている；
...
- スイスやカナダなど、Bitcoinを脅威ではなくチャンスと捉え、より歓迎すべき環境を提供する国もある。
...
現地の規制を遵守する取引所は通常、厳格な**"Know Your Customer "**（[KYC](https://planb.academy/resources/glossary/kyc-know-your-customer)）プロトコルに従わなければならず、ユーザーはサービスにアクセスする前に本人確認書類を提出する必要がある。このようなプロセスは違法行為を防ぐために設計されていますが、Bitcoinが本来提供するプライバシーを損なう可能性があります。
...
- ハードウェア・メーカー：ビットメイン社のような企業は、ASIC（特定用途向け集積回路）、つまり**Mining Bitcoin**のためだけに作られた超特殊なチップを設計・製造している。
...
- 採掘者：Miningのハードウェアとソフトウェアを運用する個人や組織である。一方には、**Antminer S9**のようなマシンを使用する小規模な採掘者がおり、もう一方には、Mining専用の巨大な施設を管理する**Galaxy Digital**のような産業事業者がいる。
...
- 独立系開発者は、Mempool.space（取引アクティビティを追跡するための視覚的なInterface）やAlby（ブラウザやアプリでLightning決済を利用するためのツール）など、ユーザーエクスペリエンスを向上させるツールやアプリケーションの開発に注力している。
...
RGBの主な特徴はDouble-spendingに対する保護であり、Single-use Sealsと呼ばれる暗号技術を用いて実現されている。このメカニズムは、Bitcoin の [UTXO](https://planb.academy/resources/glossary/utxo)（Unspent Transaction Outputs）が一度しか使用できないという事実に依存している。トークンの真正性は、Contract の全履歴（作成から現在の状態まで）のユーザー側の検証によって保証される。
...
- サイバー攻撃：サイバーセキュリティに精通していなかったり、脆弱なデバイスを使用している場合、オンライン攻撃の標的になる可能性があります。その場合、ビットコインをオフラインで保管（**Cold**ストレージ）するのが安全かもしれません。
...
- 信頼できる親族で、あなたの計画の円滑な実行を保証してくれる人。必ずしもBitcoinに関する深い知識は必要ないが、相続人が信頼できる人物でなければならない。
- 経験豊富なBitcoinユーザーで、資金の回収、ウォレットの管理、関連プロセスの理解などの技術サポートを提供できる方。
...
- **[ハードウェアウォレット](https://planb.academy/resources/glossary/hardware-wallet)**：オフラインで秘密鍵を保管する物理的なデバイス；
...
- ローカルコミュニティに参加しましょう：イベントやミートアップ、フォーラムに参加して、他のビットコイナーと出会い、最新動向についてExchangeの見解を得る。
