...
È il caso di  **Leendert de Borst** e del suo `Alias Vault`, un rivoluzionario strumento (il primo nel suo genere) che consente di gestire e memorizare le password, di usare i record delle stesse per autenticarsi ai servizi web,  amministrare l'autenticazione a due fattori, ma soprattutto generare veri e propri _alias_, tutto in un'uninca interfaccia.

**Ma Alias Vault non si ferma qui**.

## Caratteristiche principali
Alias Vault lavora in cloud sui server dello sviluppatore o self-hosted sul proprio server, opzione per la quale sono disponibili file e immagine Docker da installare con uno scipt. Oltre all'interfaccia web, sono disponibili le estensioni per tutti i browser più utilizzati, nonché le app mobili per iOS e Android; quest'ultima si può scaricare anche da F-Droid, bypassando lo store ufficiale di Google.

In un'unica interfaccia Alias Vault è:
- **Free e open source**
- **Password Manager**, per memorizzare tutte le password complesse. Grazie all'estensione del browser, il password manager completa i login ai siti web in `autofill`
- **2FA**, per supportare l'autenticazione a due fattori
- **Alias manager con server email incorporato**: Alias Vault non crea alias che inoltrano email ad una casella dell'utente, bensì crea dei veri e propri alter-ego, con tanto di nome, cognome, gender, username, password e data di compleanno (qualora questa faccia parte di una specifica richiesta).

Fanno parte del pacchetto un'ampia e accurata documentazione, che accompagneranno i nuovi arrivati alla scoperta di questo potente tool.

## Si parte: niente dati personali!
Si inizia, come sempre, dal sito web [aliasvault.net](aliasvault.net). Come detto, Alias Vault si può utilizzare sul proprio server, oppure dal cloud dello sviluppatore per usarlo e conoscerlo prima di passare alla soluzione di self-hosting.
...
Con enorme sorpresa scoprirai che Alias Vault non  chiede informazioni personali: basta un qualunque nickname, o parola a te familiare, per creare l'account. Purché sia disponibile. Accetta i Termini di Servizio, scegli il nome e prosegui.

![img](assets/en/02.webp)

Imposta adesso la **`master password`** che è il dato più importante di tutto il tuo nuovo sistema. Con quest'unica password, infatti, sarai l'unico a poter accedere e recuperare l'account, in quanto terrà il tuo `vault` criptato sul server che ospiterà le tue informazioni.
...
Se hai già un password manager, puoi importare il file da quello in uso, per valutare le differenze (e magari eliminare l'alias manager così da gestire tutto in un'unica app).
![img](assets/en/05.webp)
Alias Vault è estremamente semplice: hai una pagina principale, che è la `Home`, con due menu:
- `Credential`: che  ti permette di creare e poi gestire le identità e gli alias
- `Email`: una inbox dove potrai controllare le email in arrivo per gli alias che hai generato.
...
La parte superiore della prima schermata, puoi considerarla come il tuo spazio dove importare le credenziali con password che già usi per i servizi con cui hai un abbonamento, o che usi più spesso online.

Se hai attivato l'autentizazione a due tattori su qualcuno dei suddetti servizi (o tutti), con Alias Vault puoi gestire anche questo ulteriore livello di sicurezza, importando la `secret key`. Alias Vault creerà la necessaria `TOTP` - one time password a tempo, necessaria per l'accesso.

![img](assets/en/09.webp)

**N.B.**: nello spazio riservato all'indirizzo email, Alias Vault presenta di default il proprio dominio; per poter utilizzare l'indirizzo corretto, con cui hai precedentemente creato gli account, clicca su _Enter custom domain_, così da poter impostare il corretto dominio dopo `@`.

![img](assets/en/14.webp)

La parte inferiore di questa schermata è quella più divertente. Clicca su _Generate Random Alias_ e Alias Vault creerà per te identità separate per diverse necessità, ognuna delle quali con la propria email, password di livello molto robusto, completo di altri dettagli come il gender, la data di nascita e anche un nickname.
...
Naturalmente, un alias di questo tipo serve per iscriverti a servizi online che non richiedano la tua vera identità o un kyc completo (banca ecc). Puoi usare l'email dell'alias suggerito da Alias Vault, o cambiare dominio se clicchi sul menu a tendina corrispondente.
...
---
**⚠️ N.B.**: La ricezione delle email è possibile grazie al server `SMPT` incorporato da Alias Vault, ma questo permette esclusivamente di ricevere email e non di rispondere, o usare la casella email con le funzioni "convenzionali" di un servizio `alias`. 

In questo si differenzia molto da [Simple Login](https://planb.network/tutorials/computer-security/communication/simple-login-c17a10d6-8f84-4f97-8d50-7a83428d0f41), Addy ed altre piattaforme che si dedicano esclusivamente a questo tipo di servizio.

---
Per cancellare un alias che hai creato a titolo temporaneo, non dovrai far altro che accedere alla tua `Home`, poi `Credentials` e cliccare sull'identità da cancellare. In alto a destra ti compare il comando _Delete_ per procedere.
...
Si installa come già hai fatto con tutte le altre estensioni, non mi soffermerò troppo su questo particolare.

Come per altri password manager affermati, l'estensione del browser è lì per facilitare le operazioni di login ai servizi online, o per creare nuovi alias all'occorrenza: se il servizio è memorizzato tra i tuoi record di Alias Vault, l'auto-fill fa quel che serve.

![img](assets/en/17.webp)

L'unica attenzione è verificare che Alias Vault sia attivo. L'applicazione ha infatti un'impostazione di default, per cui si mette in pausa dopo un periodo di inattività. È una funzione molto utile, **quando ti devi ad esempio allontanare dal computer ed evitare che qualcun altro possa accedere ai tuoi account**. Una procedura snella ti permetterà di accedere nuovamente, soltanto immettendo la `master password`, se la sessione precedente è ancora nella cache. Il tempo per la disconnessione è uno dei parametri che puoi personalizzare, accorciandolo o allungandolo, a seconda delle tue esigenze.
...
Al momento della stesura di questa guida (fine agosto 2025), l'app mobile considera l' `auto-fill` una feature sperimentale. Io la sto usando da tempo, seppure sporadicamente e con login fittizie, ma fino ad ora anche l'app mobile si è dimostrata efficiente.

Ti basterà fare login con lo user e la `master password` creati sulla web app.
...
Se decidi impostare il controllo biometrico, procedi. Potrai eventualmente configurarlo anche in seguito, dal menu dei _Settings_.
...
Alias Vault non è il primo `password manager` che permette di implementare il servizio sulla tua infrastruttura. L'opportunità è incredibile: **anziché dipendere da fornitori di servizi o cloud esterni, puoi usare il tuo server locale per custodire e gestire le password, gli alias e le informazioni estremamente sensibili associate a tutto ciò**. Con Alias Vault puoi anche far puntare il servizio email verso il tuo server di posta elettronica, per una maggiore confidenzialità.
...
Il tuo server deve girare su una macchina 64 bit, con distribuzione Linux, 1 GB di RAM e almeno 16 GB di memoria; la versione di Docker (CE) dev'essere almeno la 20.10 o superiore, mentre per Docker Compose è necessaria una release dalla 2.0 in su.
...
Le informazioni restano nascoste anche al gestore del cloud/hosting. Infatti, dal pannello di amministrazione non si accede ai dettagli degli utenti, si può solo sapere se hanno creato alias, ricevuto email e poco altro.

Tutti i contenuti archiviati, vengono cifrati e decifrati da chiavi crittografiche, derivate dalla `master password`. **Sul server vengono conservati solo i dati cifrati, niente compare in chiaro**. Se un utente dimentica o perde la sua `master password`, l'account ad essa collegato è irreversibilmente perso, in quanto il server non riesce ad avere accesso ai contenuti in chiaro.
...
Essendo Alias Vault in fase _Beta_ potresti avere difficoltà di accesso in caso di cambio/aggiornamento della master password. Ti consiglio di sceglierla robusta e complessa fin dall'inizio, in modo da poter sperimentare il servizio e, infine, decidere se utilizzarlo. In caso di difficoltà di accesso al cloud, a seguito dell'aggiornamento password, contatta il supporto di Alias Vault.

Per una completa comprensione dell'architettura e sicurezza adottate da Alias Vault, ti consiglio vivamente di consultare [la pagina](https://docs.aliasvault.net/architecture/), che contiene i dettagli della crittografia alla base del suo funzionamento.
...
**Uno strumento così è ideale a tutti coloro che mettono la privacy online al centro dell'attenzione**.
