...
- Vá para [o repositório GitHub da PlanB dedicado a dados](https://github.com/PlanB-Network/bitcoin-educational-content/tree/dev/resources/builders) na seção `resources/project/`:
...
- Nesta pasta, você criará um primeiro arquivo YAML chamado `project.yml`:
...
- Volte para a pasta `resources/projects/` e selecione a pasta do seu negócio que você acabou de criar no commit anterior:
...
- Deixe o título do commit como padrão, e certifique-se de que a caixa `Commit directly to the patch-1 branch` esteja marcada, então clique em `Commit changes`: ![project](assets/23.webp)
...
![project](assets/30.webp)- Uma vez no arquivo, clique nos três pequenos pontos no canto superior direito e então em `Delete file` (Apagar arquivo):
