...
Bei dieser Art von Aufgabe ist es wichtig, dass Sie sich an die Bedeutung der Originalsprache (EN oder FR) halten, aber sicherstellen, dass der Text in der endgültigen Sprache so flüssig wie möglich für einen Muttersprachler ist.


Denken Sie immer daran, dass Übersetzung/Korrekturlesen BILDUNG ist!
...
Nachdem Sie diesen neuen Zweig erstellt haben, klicken Sie ihn auf der vorherigen Seite unter "Ihre Zweige" an und beginnen Sie mit der Arbeit an der *.md*-Datei, die sich auf den spezifischen Inhalt bezieht (in meinem Fall werde ich auf "Kurse" -> "BTC101" -> "fr.md" klicken). Alle Übertragungen, die sich auf die spezifische Datei beziehen, müssen innerhalb desselben Zweigs übertragen (gespeichert) werden.
...
Beim Korrekturlesen von Inhalten ist es wichtig, **immer die englische (oder französische)** Originalversion zu überprüfen. Seien Sie sich bewusst, dass wir mit KI-Sprachwerkzeugen übersetzen, so dass die Wiedergabe in der Zielsprache für den Endleser möglicherweise nicht flüssig oder gut verständlich ist.
...
Behalten Sie in diesem ersten Abschnitt das ursprüngliche Format bei. Fügen Sie am Anfang des Textes nichts hinzu. Fügen Sie z. B. kein "tt" vor den Bindestrichen ein, wie in der Abbildung unten!
...
Überprüfen Sie daher immer den englischen Originaltext, um festzustellen, ob ein fettgedruckter Text vorhanden sein sollte. In diesem Fall fügen Sie einfach zwei Sternchen am Anfang des Wortes ein, damit es auf der Website korrekt angezeigt wird. In der Markdown-Sprache **müssen Sie zwei Sternchen vor und nach dem Wort/Satz einfügen, um fett zu sein** (siehe Beispiel unten).
