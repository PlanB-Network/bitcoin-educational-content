...
在「*名稱*」欄位中，為您的憑證輸入一個描述性的名稱。這將使您更容易搜尋和組織您的密碼，尤其是當您有大量密碼時。例如，如果您要儲存 Plan ₿ Academy 網站的憑證，您可以為此項目命名，使其在您未來搜尋時可立即辨認。
...
您可以在建立識別碼時為其指定資料夾，就像我們之前所做的一樣，或是修改現有的識別碼。例如，按一下 Plan ₿ Academy 的識別碼，我就可以選擇將它分類到 "*Bitcoin*" 資料夾。
...
例如，如果我回到 Plan ₿ Academy 網站並嘗試登入我的帳戶，您可以看到整合到瀏覽器的 Bitwarden 延伸程式會識別登入欄位，並自動提供我選擇之前儲存的識別碼。
...
該擴充套件還可偵測新帳戶的建立。例如，在 Plan ₿ Academy 上建立新帳號時，Bitwarden 會自動建議儲存新的識別碼。
