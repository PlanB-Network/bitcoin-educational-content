...
[FOSS](https://planb.academy/resources/glossary/foss) je zkratka pro Free & Open-Source Software. První z nich označuje podmínky, které umožňují komukoli kopírovat, upravovat a dokonce šířit verze softwaru (i za účelem zisku). Druhý termín označuje otevřené sdílení zdrojového kódu a podporuje veřejnost v jeho přispívání a vylepšování.
...
Cvičení začíná po zhlédnutí úvodního filmu "Jak funguje Bitcoin a [Blockchain](https://planb.academy/resources/glossary/blockchain)" od BBC.


:::video id=c20b6df7-0c3a-4785-94b9-42ef59093acc:::


Toto cvičení vyžaduje minimálně devět účastníků. Cílem tohoto cvičení je fyzicky pochopit, jak funguje Bitcoin. Tím, že budete hrát jednotlivé role v síti, získáte interaktivní a hravý způsob učení. Toto cvičení se netýká sítě [Lightning Network](https://planb.academy/resources/glossary/lightning-network).
...
- Pravidelně aktualizujte/přidávejte záznamy s novými transakcemi přijatými z "náhodného" počítače, pokud jsou všechny obsahy platné (poznámka: pro zjednodušení prozatím ignorujeme komponentu [Proof of Work](https://planb.academy/resources/glossary/proof-of-work)), jinak je odmítněte a pokračujte jako dříve, dokud další "náhodný" počítač nepošle aktualizaci
...
- Jeden počítač je "náhodně" vybrán, aby navrhl dokončení navrhované transakce odvysíláním "[bloku](https://planb.academy/resources/glossary/block)", který ji obsahuje; pokud se potvrdí, obdrží odměnu BTC.
...
- [Obtížnost](https://planb.academy/resources/glossary/difficulty) Cíl
- Blok
...
Blockchain, Transakce, Dvojité výdaje, Byzantský generálský problém, Mining, Proof of Work (PoW), Hash Funkce, Block reward, Blockchain, Nejdelší řetězec, 51% útok, Výstup, Výstupní zámek, Změna, [Satoshis](https://planb.academy/resources/glossary/satoshi-sat), Veřejný/soukromý klíč, Address, [Kryptografie](https://planb.academy/resources/glossary/cryptography) veřejného klíče, [Digitální podpis](https://planb.academy/resources/glossary/digital-signature), Wallet
...
- Adresy začínající `bc1...` se pomalu přesouvají mezi velmi běžné adresy. Tyto adresy se nazývají (nativní) adresy [SegWit](https://planb.academy/resources/glossary/segwit). Ty nabízejí lepší strukturu poplatků než ostatní zmíněné Adresy. Nativní adresy SegWit používají kódování Bech32 a umožňují psát pouze malá písmena.
...
- Peněženky HD však mají jinou strukturu. `xpub...`, `ypub...` nebo `zpub...` se nazývají rozšířené veřejné klíče neboli [xpub](https://planb.academy/resources/glossary/xpub). Tyto klíče se používají k odvození mnoha veřejných klíčů jako součást HD Wallet. Protože váš xpub obsahuje záznamy o celé vaší historii, tedy o minulých i budoucích transakcích, nikdy je nesdílejte s nedůvěryhodnými stranami.
...
Existují různé způsoby ukládání klíčů Bitcoin do úložiště Cold, od papírových peněženek přes mozkové peněženky, hardwarové peněženky nebo od počátku soubor Wallet. Většina peněženek používá k zápisu [BIP](https://planb.academy/resources/glossary/bip) 39 do generate frázi seed. V rámci softwaru Bitcoin core však zatím nebylo dosaženo konsenzu o jeho používání. Software Bitcoin core bude stále generate soubor Wallet.dat, který je třeba uložit na bezpečné offline místo.
...
| [Payjoin](https://planb.academy/resources/glossary/payjoin)        | Nezaplaceno, časovač faktury dosud nevypršel         |
| Payjoin-odhalen| [UTXO](https://planb.academy/resources/glossary/utxo) bylo odhaleno prostřednictvím návrhu payjoin ve faktuře |
...
Server BTCPay vás vždy vyzve, abyste generate následující dostupné Address, aby se zabránilo opakovanému použití Address. Po kliknutí na "generate další dostupný BTC Address" vygeneruje BTCPay Server nový Address a QR. Umožňuje vám také přímo nastavit [štítek](https://planb.academy/resources/glossary/label) k Address pro lepší správu vašich adres.
...
Správci serverů mohou kliknutím na informace o interním uzlu zobrazit stav serveru (Online/Offline) a řetězec připojení pro Clearnet nebo [Tor](https://planb.academy/resources/glossary/tor).
...
| Pole | Popis |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| .fields.constant | Pokud je true, musí být .value nastaveno v definici formuláře a uživatel nebude moci hodnotu pole změnit. (příklad: verze definice formuláře) |
| .fields.type | Typ vstupu HTML: text, radio, checkbox, password, hidden, button, color, date, datetime-local, month, week, time, email, number, range, search, url, select, tel |
| .fields.options | Pokud je .fields.type select, seznam volitelných hodnot |
| .fields.options.text | Text zobrazený pro tuto možnost |
| .fields.options.value | Hodnota pole, pokud je vybrána tato možnost |
| .fields.type=fieldset | Vytvoří HTML fieldset kolem dceřiných prvků .fields.fields (viz níže) |
| .fields.name | Název vlastnosti JSON pole, jak se objeví v metadatech faktury |
| .fields.value | Výchozí hodnota pole |
| .fields.required | pokud je true, pole bude povinné |
| .fields.label | Popisek pole |
| .fields.helpText | Dodatečný text poskytující vysvětlení k poli. |
| .fields.fields | Pole můžete organizovat v hierarchii, což umožňuje vnoření dceřiných polí do metadat faktury. Tato struktura vám může pomoci lépe organizovat a spravovat shromážděné informace, což usnadňuje jejich přístup a interpretaci. Pokud máte například formulář, který shromažďuje informace o zákazníkovi, můžete pole seskupit pod nadřazené pole s názvem customer. V tomto nadřazeném poli můžete mít dceřiná pole jako name, Email a address. |
...
| Protokoly pro | Název kontejneru                   |
| ------------ | --------------------------------- |
