...
Sparrow Wallet är en självförvaltande Bitcoin Wallet-hanteringsprogramvara utvecklad av Craig Raw. Denna programvara med öppen källkod uppskattas av bitcoiners för sina många funktioner och intuitiva Interface.
...
- Som en Cold Wallet-manager, där privata nycklar hålls på en Hardware Wallet. I det här läget manipulerar Sparrow endast offentlig Wallet-information, spårar medel, genererar adresser och bygger transaktioner, men Hardware Wallet-signaturen krävs för att göra dessa transaktioner giltiga. Det kan därför ersätta applikationer som Ledger Live eller Trezor Suite.
...
När du är ansluten kommer Sparrow Wallet att visa en gul bock i det nedre högra hörnet av Interface för att indikera att du är ansluten till en offentlig nod.


![Image](assets/fr/06.webp)


### Anslutning till en Bitcoin Core 🟢


Den andra metoden för att ansluta till en Bitcoin-nod är att länka Sparrow till en Bitcoin Core. Om Bitcoin Core är installerat på samma maskin kommer autentiseringen att ske via cookie-filen. Om Bitcoin Core finns på en fjärrmaskin måste du använda det lösenord som definieras i filen `Bitcoin.conf`.


Observera att om du använder en beskuren Bitcoin Core-nod kommer du inte att kunna återställa en Wallet som innehåller transaktioner som föregår de lokalt lagrade blocken. För en ny Wallet som skapats på Sparrow kommer detta dock inte att vara ett problem: dina nya transaktioner kommer att vara synliga, även med en beskuren nod.


Om du vill konfigurera en Bitcoin Core-nod kan du läsa någon av följande handledningar, beroende på operativsystem:
...
Om Bitcoin Core är installerat på en annan maskin som är ansluten till samma nätverk, leta först upp filen `Bitcoin.conf` bland programvarufilerna. Om filen ännu inte finns kan du skapa den. Öppna filen med en textredigerare och lägg till följande rad:
...
I filen `Bitcoin.conf` lägger du till följande rader och anger `rpcbind=192.168.1.18` så att den matchar IP Address för din nod.
...
Det sista alternativet för att ansluta är att använda en fjärrserver för Electrum. Med den här metoden kan du ansluta till din nod via Tor från en annan enhet och dra nytta av en indexerare för att bläddra igenom dina plånböcker på Sparrow snabbare. Det är särskilt lämpligt om du har en nod-i-en-låda-lösning som Umbrel eller Start9, som gör att du kan installera Electrum med ett enda klick.
...
För att skapa en Hot Wallet, från Sparrow Wallet startskärm, klicka på fliken "*File*" och sedan på "*New Wallet*".
...
Längst upp på Interface kan du välja om du vill skapa en "*Single Signature*" eller "*Multi Signature*" Wallet. Precis nedanför väljer du vilken typ av skript som ska låsa dina UTXO:er. Jag rekommenderar att du använder den senaste standarden: "*Taproot (P2TR)*".
...
Välj BIP39-standarden, eftersom den stöds av praktiskt taget all Bitcoin Wallet-programvara. Därefter väljer du längden på din återställningsfras. För närvarande räcker det med en 12-ordsfras, eftersom båda erbjuder liknande säkerhet, men 12-ordsfrasen är enklare att spara.


![Image](assets/fr/25.webp)


Klicka på knappen "*generate New*" för att generate din Wallet:s Mnemonic-fras. Den här frasen ger fullständig, obegränsad tillgång till alla dina bitcoins. Vem som helst som har tillgång till den här frasen kan stjäla dina pengar, även utan fysisk tillgång till din dator.
...
Du kan skriva den på papper eller, för ökad säkerhet, gravera den på rostfritt stål för att skydda den mot brand, översvämning eller kollaps. Valet av medium för din Mnemonic beror på din säkerhetsstrategi, men om du använder Sparrow som en varm spenderande Wallet som innehåller måttliga mängder bör papper vara tillräckligt.
...
**Du får naturligtvis aldrig dela med dig av dessa ord på Internet, vilket jag gör i denna handledning. Detta exempel Wallet kommer endast att användas på Testnet och kommer att raderas i slutet av handledningen.**
...
## Hantera en Cold Wallet


Det andra sättet att använda Sparrow Wallet är att konfigurera den som en Wallet-hanterare med en Hardware Wallet. I den här konfigurationen finns de privata nycklarna till din Bitcoin Wallet uteslutande på Hardware Wallet, medan Sparrow endast har tillgång till den offentliga informationen. Detta tillvägagångssätt erbjuder en högre säkerhetsnivå än Hot-plånböckerna som diskuteras ovan, eftersom de privata nycklarna förvaras på en specialiserad enhet, ofta med ett säkert chip, som inte är ansluten till Internet och därför utgör en mycket mindre attackyta än en traditionell dator.
...
Överst på Interface väljer du mellan "*Single Signature*" eller "*Multi Signature*" Wallet. I vårt exempel kommer vi att konfigurera en Wallet med en enda signatur.
...
Klicka på "*Scan*" för att Sparrow ska upptäcka din enhet. Kontrollera att den är inkopplad och upplåst. För vissa modeller, t.ex. Ledger, måste du öppna applikationen "*Bitcoin*" för att möjliggöra detektering.
...
Ange ett starkt lösenord för att säkra åtkomsten till din Sparrow Wallet. Detta lösenord skyddar dina publika nycklar, adresser och transaktionshistorik. Vi rekommenderar att du sparar det i en lösenordshanterare. Observera att det här lösenordet inte har någon betydelse för hur dina nycklar härleds. Även utan det kan du återfå åtkomst till dina bitcoins med din Mnemonic via valfri BIP39-kompatibel programvara.
...
När betalaren har sänt transaktionen visas den på fliken "*Transaktioner*". Du kan klicka på den för att få mer information, t.ex. txid.


![Image](assets/fr/45.webp)


På fliken "*Addresser*" finns en lista över alla dina inkorgsadresser. Du kan se om de redan har använts och om en etikett har lagts till. *Adresserna "*Mottag*" är de som Sparrow visar när du klickar på "*Mottag*" och är avsedda för inkommande betalningar. Adresserna "*Change*" används för Exchange i dina transaktioner, dvs. för att återfå den oanvända delen av dina UTXO:er i inputs.
...
Sparrow visar ett sammanfattande diagram som visar dina inmatningar, utmatningar och transaktionsavgifter (observera att avgifter faktiskt inte är en utmatning, tvärtemot vad diagrammet antyder). Om du är nöjd med allt klickar du på "*Create Transaction*".
...
- För en Hardware Wallet Air-Gap, klicka på "*Show QR*" för att visa en PSBT som du kan signera med din enhet, ladda sedan den signerade PSBT i Sparrow med "*Scan QR*". Alternativet "*Save Transaction*" fungerar på ett liknande sätt, men med utbyten på ett microSD-kort;
- För en Hot Wallet klickar du bara på "*Signera*" och anger lösenordet för Wallet för att signera;
- För en trådbunden Hardware Wallet klickar du även på "*Sign*" för att skicka den osignerade transaktionen till din enhet.


![Image](assets/fr/57.webp)


På din Hardware Wallet kontrollerar du mottagarens Address, det skickade beloppet och avgifterna. Om allt är korrekt, fortsätt med underskriften.


När transaktionen har signerats kommer den att visas i Sparrow igen, redo att sändas ut på Bitcoin-nätverket för att sedan inkluderas i ett block. Om allt är korrekt klickar du på "*Broadcast Transaction*".
...
Knappen "*Avancerat*" ger tillgång till avancerade inställningar, t.ex. anpassning av Sparrows Address-sökning och ändring av Wallet-lösenordet.
...
- "*Send To Many*" erbjuder en förenklad Interface för att utföra transaktioner till flera mottagaradresser samtidigt, vilket är bekvämt för batchutgifter.
...
- "*Restart In*" låter dig byta till dina plånböcker på Testnet eller Signet-nätverk. Detta kan vara användbart om du vill komma åt testnätverk med mynt som inte har något verkligt värde.


![Image](assets/fr/67.webp)


Du vet nu allt om Sparrow Wallet-programvaran, ett utmärkt verktyg för att hantera dina Bitcoin-plånböcker på daglig basis.
...
Jag rekommenderar också denna andra handledning där jag förklarar hur man konfigurerar Hardware Wallet COLDCARD Q med Sparrow Wallet :
