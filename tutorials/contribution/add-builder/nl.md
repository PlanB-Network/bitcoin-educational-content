...
De missie van PlanB is om eersteklas educatieve bronnen aan te bieden op Bitcoin, in zoveel mogelijk talen. Alle inhoud die op de site wordt gepubliceerd is open-source en wordt gehost op GitHub, waardoor iedereen kan deelnemen aan het verrijken van het platform.


Wil je een nieuw Bitcoin "project" toevoegen aan de PlanB Network site en zichtbaarheid geven aan je bedrijf of software, maar weet je niet hoe? Deze tutorial is voor jou!
...
- Maak een map voor je bedrijf. Om dit te doen, schrijf je in het vak `Naam je bestand...` de naam van je bedrijf in kleine letters met streepjes in plaats van spaties. Als je bedrijf bijvoorbeeld "Bitcoin Baguette" heet, schrijf je `Bitcoin-baguette`:
...
- `naam`: De naam van je bedrijf (verplicht);
- `Address` : De locatie van uw bedrijf (optioneel);
- `taal`: de landen waarin je bedrijf actief is of de talen die worden ondersteund (optioneel);
- `links` : De verschillende officiële links van je bedrijf (optioneel);
- `tags` : 2 termen die je bedrijf kwalificeren (verplicht);
- `categorie` : De categorie die het best het gebied beschrijft waarin uw bedrijf actief is uit de volgende keuzes:
 - gW-7`,
 - `infrastructuur`,
 - gW-8`,
 - `onderwijs`,
...
- Ga terug naar de map `resources/projects/` en selecteer de map van je bedrijf die je zojuist in de vorige vastlegging hebt gemaakt:

![project](assets/18.webp)


- Klik in de map van je bedrijf op de knop `Bestand toevoegen` en vervolgens op `Nieuw bestand maken`:
...
- Laat de commit titel als standaard staan, en zorg ervoor dat het `Commit direct to the patch-1 branch` vakje aangevinkt is, klik dan op `Commit changes`: ![project](assets/23.webp)
...
- Er wordt een nieuwe pagina geopend. Sleep een afbeelding van uw bedrijf of uw software naar het gebied. Deze afbeelding wordt weergegeven op de PlanB Network-site:
...
- Maak een nieuw YAML-bestand aan door het de naam te geven van de indicator van je moedertaal. Dit bestand wordt gebruikt voor de beschrijving van het project. Bijvoorbeeld, als ik mijn beschrijving in het Engels wil schrijven, dan noem ik dit bestand `en.yml`:
...
- Voor de `contributors` sleutel kun je je contributor identifier aan PlanB Network toevoegen als je die hebt. Als je die niet hebt, laat dit veld dan leeg.
- Voor de `description` sleutel moet je gewoon een korte paragraaf toevoegen die je bedrijf of je software beschrijft. De beschrijving moet in dezelfde taal zijn als de bestandsnaam. U hoeft deze beschrijving niet te vertalen in alle talen die worden ondersteund op de site, omdat de PlanB-teams dit zullen doen met behulp van hun model. Zo zou uw bestand er bijvoorbeeld uit kunnen zien:
...
- Zorg ervoor dat het vakje `Direct vastleggen op de patch-1 tak` aangevinkt is, voeg een titel toe en klik dan op `Wijzigingen vastleggen`:
