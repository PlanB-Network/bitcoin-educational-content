...
Bitcoin adresleri, ham anahtarların kendileri değil, [açık anahtarların](https://planb.academy/resources/glossary/public-key) hash'leridir. Bir adresi generate yapmak için, açık anahtarı SEC biçiminde serileştirin, hash160 hesaplayın ([SHA-256](https://planb.academy/resources/glossary/sha256) ve ardından RIPEMD-160), ağ önekini ekleyin (mainnet için 0x00, testnet için 0x6F), çift SHA-256 kullanarak bir sağlama toplamı hesaplayın, ilk dört sağlama toplamı baytını ekleyin ve sonucu Base58 kullanarak kodlayın. Bu kodlama belirsiz karakterleri ortadan kaldırır ve transkripsiyon hatalarını önlemek için sağlama toplamını içerir. Çok adımlı işlem hattı, bir harcama gerçekleşene kadar açık anahtarı gizler, ağ tanımlaması ekler ve insan tarafından okunabilir, hataya dayanıklı adresler sağlar.
...
Bitcoin hesap bakiyeleri yerine harcanmamış çıktıları izler. UTXO'lar tamamen harcanmalıdır - kısmi harcama mümkün değildir. 100 BTC'lik bir UTXO'den 1[ BTC](https://planb.academy/resources/glossary/utxo) harcamak için, bir işlem bir değişim çıktısı içermelidir. Değişim çıktısının unutulması, kalanı madenci ücretine dönüştürür.
...
Bitcoin [blokları](https://planb.academy/resources/glossary/block) işlemleri gruplandırır ve [proof of work](https://planb.academy/resources/glossary/proof-of-work) kullanarak bunları güvence altına alır. Her blok 80 baytlık bir [başlık](https://planb.academy/resources/glossary/block-header) ve işlemlerin bir listesini içerir. Geçerli bir blok üretmenin yüksek enerji maliyetine rağmen, bir bloğu doğrulamak ucuzdur: tüm ~900k başlıkları depolamak sadece ~72 MB gerektirir, bu da küçük cihazların bile zincirin proof of work'sini verimli bir şekilde doğrulamasına izin verir.
...
Bitcoin'nin [eşler arası](https://planb.academy/resources/glossary/peertopeer-p2p) ağı, düğümlerin birbirlerine güvenmeden işlemleri ve blokları aktardığı merkezi olmayan bir dedikodu sistemi olarak çalışır. Yeni düğümler, çekirdek geliştiriciler tarafından tutulan sabit kodlu DNS tohumlarıyla iletişim kurarak önyükleme yapar. Bu DNS tohumları aktif eşlerin IP'lerini döndürerek düğümlerin ağı keşfetmesini ve ardından getaddr aracılığıyla ek eşler talep etmesini sağlar. Ağ oluşturma kasıtlı olarak [mutabakat](https://planb.academy/resources/glossary/consensus) açısından kritik değildir, bu nedenle mutabakat kuralları değişmediği sürece uygulamalar farklılık gösterebilir.
