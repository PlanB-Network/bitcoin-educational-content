...
Интерес к Биткойн растет стремительно, и многие люди начинают тестировать, обсуждать, исправлять ошибки и размышлять о этических, экономических и философских аспектах Биткойн. Чтобы облегчить эти обмены мнениями, 22 ноября 2009 года Сатоши создает форум BitcoinTalk. Этот форум быстро становится предпочтительным местом обсуждения для пользователей Биткойн. Именно там появляются многие мемы и символы, ассоциируемые с Биткойн, такие как [логотип Биткойн](https://bitcointalk.org/index.php?topic=64.0), знаменитый [Hodl](https://bitcointalk.org/index.php?topic=375643.0), или даже [День пиццы](https://bitcointalk.org/index.php?topic=137.msg1195).

> **Знаете ли вы?** Действительно, 22 мая 2010 года, Ласло Ханец совершил историческую покупку в Биткойн, предложив купить 2 пиццы за 10 000 BTC. Это был первый случай использования Биткойн для покупки физических товаров.
...
Биткойн воспринимается каждым индивидуумом по-разному. Для некоторых это грибковая сущность, подобная [мицелию](https://brandonquittem.com/bitcoin-is-the-mycelium-of-money/), для других это [черная дыра](https://dergigi.com/2019/05/01/bitcoins-gravity/i). Любишь ты его или ненавидишь, Биткойн продолжает существовать, с его постоянным ритмом в 10 минут на блок, как сердцебиение новой денежной системы.
...
Теперь, когда у нас есть некоторое представление, давайте рассмотрим, как в общем работает транзакция Биткойн.
...
Чтобы Боб передал биткойны Алисе, Алиса должна предоставить Бобу один из своих Биткойн адресов. Этот адрес, полученный из публичного ключа Алисы, уникален для ее Биткойн кошелька.
...
На этом этапе транзакция создана, и кошелек Боба будет делиться ей с сетью Биткойн. Для этого его кошелек будет общаться с узлом сети Биткойн, и этот узел будет распространять эту информацию среди других узлов. Этот шаг распространения позволяет всей сети увидеть эту новую транзакцию и учесть ее.
...
Майнеры имеют задачу брать валидные и неподтвержденные транзакции, а затем компилировать их в блок. Чтобы их блок стал следующим в блокчейне Биткойн, они должны решить криптографическую загадку в процессе, называемом "доказательство работы".
...
Этот новый действительный блок добавляется в блокчейн Биткойн, который является публичным и неизменяемым регистром всех транзакций Биткойн. Можно думать о нем как о общем регистре для всех пользователей Биткойн. Согласно правилам протокола, блок добавляется примерно каждые десять минут благодаря корректировке сложности. Мы увидим в разделе о майнерах, какой механизм предотвращает изменение регистра транзакций Биткойн.
...
Пользователи передают право собственности на свои деньги, цифрово подписывая транзакции своими приватными ключами. Майнеры обладают небольшой властью, поскольку пользователи также имеют значительный контроль через узлы Биткойн, которые обрабатывают проверку новых блоков и включенных в них транзакций. Именно через эту сеть узлов Биткойн сеть действительно децентрализована.

Фактически, для полного уничтожения сети Биткойн необходимо было бы уничтожить все копии блокчейна на всех узлах Биткойн - задача практически невозможная из-за географического распределения этих узлов и сложности физического их захвата.
...
Таким образом, любое устройство, на котором работает программное обеспечение Биткойн, называемое узлами Биткойн (часто через [Bitcoin Core](https://bitcoin.org/en/bitcoin-core/)), способствует децентрализации сети.

![image](assets/en/chapter11/1.webp)

### Таким образом, узлы являются центральным ядром Биткойн.

Каждый узел хранит копию блокчейна, что позволяет проверять транзакции и предотвращать любые попытки мошенничества. Децентрализованный аспект сети придает Биткойн исключительную устойчивость и надежность - чтобы остановить протокол Биткойн, необходимо было бы выключить все узлы по всему миру. Для вашего сведения, на данный момент (сентябрь 2023 года) в мире распределено примерно [45 000 узлов](https://bitnodes.io/nodes/all/).
Узлы способны проверять валидность блоков и транзакций, поскольку следуют правилам консенсуса Биткойн. Эти правила регулируют, среди прочего, монетарную политику Биткойн, такую как размер вознаграждения майнеров (о чем мы подробнее поговорим в следующем разделе) и количество биткойнов в обращении. Таким образом, узлы действуют как юридическая система сети. Благодаря им, все участники сети следуют одним и тем же правилам. Они обеспечивают нейтралитет протокола Биткойн. Правила консенсуса меняются очень редко, если вообще меняются, поскольку для внесения изменений требуется одобрение всех узлов.
![image](assets/en/chapter11/2.webp)

Управление в рамках протокола выходит за рамки этого обучения, но знайте, что каждый пользователь, запускающий узел Биткойн, решает, каким правилам следовать. Таким образом, пользователь может решить следовать другим правилам (то есть внести изменения в код), но если эти изменения делают недействительными текущие правила консенсуса, то этот узел больше не будет частью сети Биткойн. Крупные изменения, следовательно, редки и требуют значительной координации среди тысяч участников с различными идеологиями и интересами, что заставляет протокол производить только те обновления, которые делают его "лучше" в понимании всех пользователей Биткойн.
...
Действительно, у каждого есть веская причина запустить узел! Цена и усилия минимальны по сравнению с получаемой выгодой. Вам просто нужно начать приключение и присоединиться к тысячам других биткойнеров, потому что вместе мы формируем сеть Биткойн.

![image](assets/en/chapter11/11.webp)

Например, если бы блоки были в 100 раз тяжелее, мы могли бы совершать в 100 раз больше транзакций каждые 10 минут, но для запуска узла Биткойн потребовался бы жесткий диск на 50ТБ, пропускная способность свыше 500ГБ/месяц и оборудование, способное проверять сотни тысяч транзакций менее чем за 10 минут. В этой гипотетической ситуации с блоками, увеличенными в 100 раз, запуск узла Биткойн не был бы доступен для обычного человека, что поставило бы под угрозу децентрализацию протокола и неизменность транзакций и правил консенсуса. Таким образом, ограничения протокола также были выбраны для того, чтобы как можно больше людей могли запускать свой собственный узел Биткойн.
...
Proof of Work, также известный как "Доказательство работы" (POW), является консенсусом безопасности протокола Биткойн. Это правило позволяет Биткойн функционировать и обеспечивать его надежность. POW является основой всего и играет ключевую роль в теории игр Биткойн.
...
Действительно, для первых блоков субсидия составляла 50 биткойнов, затем каждые 210 000 блоков (примерно каждые 4 года) она уменьшается вдвое. Сегодня (в 2023 году) мы находимся в 4-й Эпохе; следовательно, субсидия составляет 6.25 биткойнов. Эта субсидия является механизмом создания новых биткойнов в Биткойн. Поскольку субсидия со временем уменьшается, это означает, что существует лимит в 21 миллион биткойнов - на данный момент в обороте уже более 19,4 миллиона биткойнов, что составляет более 92%.
