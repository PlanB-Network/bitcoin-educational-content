...
Phoenix działa jako prawdziwy węzeł Lightning osadzony w telefonie, automatycznie otwierając kanał z węzłem Lightning ACINQ. Aplikacja oparta jest na Lightning-KMP, wieloplatformowej implementacji [Lightning Network](https://planb.academy/resources/glossary/lightning-network) w Kotlinie, zoptymalizowanej pod kątem portfeli mobilnych. W przeciwieństwie do innych rozwiązań węzła Lightning, Phoenix znacznie upraszcza zarządzanie. Użytkownik nie musi obsługiwać otwierania i zamykania kanałów, uruchamiać węzła Bitcoin ani zarządzać płynnością na Lightning Network. Phoenix zajmuje się wszystkimi tymi operacjami technicznymi w tle.
