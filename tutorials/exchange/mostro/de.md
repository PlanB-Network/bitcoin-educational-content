---
name: Mostro
description: KYC-freie Bitcoin P2P Exchange Plattform über Lightning und Nostr
---
...
- daemon Mostrod**: koordiniert den Austausch zwischen den Nutzern und Lightning Network
...
**Escrow nicht verpfändbar**: Lightning Hold Rechnungen sperren Ihre Bitcoins, ohne sie dauerhaft zu übertragen. Ihr Geld bleibt unter Ihrer Kontrolle und wird im Falle eines Problems automatisch an Sie zurückgegeben (~24h).
...
**Flexibilität bei der Bezahlung**: Unterstützung für viele Fiat-Währungen über den yadio.io Exchange Kurs-Service. Akzeptiert SEPA-Überweisungen, PayPal, Revolut, Bargeld und jede zwischen den Parteien vereinbarte Methode.
...
Der **Mostro CLI** ist der ausgereifteste und funktionellste Client. Er ist in Rust geschrieben und bietet die volle Bandbreite der Mostro-Funktionen über eine Interface-Befehlszeile. Kompatibel mit macOS, Linux und Windows.
...
- freigeben": Freigabe des Treuhandkontos und Abschluss des Exchange
- getdm": Direkte Nachrichten anzeigen
- rate": Bewerten Sie Ihre Gegenpartei nach einer Exchange
...
Erhältlich für **Android** (via APK auf GitHub), ist es die optimale Wahl für Nutzer, die Einfachheit und gelegentlichen mobilen Handel bevorzugen.
...
Interface fortgeschrittene JavaScript-Webanwendung in aktiver Entwicklung. Entwickelt, um eine vollständige Benutzererfahrung mit umfangreichen Handels- und Portfoliomanagement-Funktionen zu bieten. Der Zugang erfolgt über einen Browser und erfordert keine Installation.
...
- Ein funktionierendes Lightning Network** Wallet mit ausreichender Liquidität (oder Lightning-kompatibel)
...
- Optional, aber empfohlen**: VPN- oder Tor-Verbindung zur Maskierung Ihrer IP Address
...
Kompilieren und installieren Sie das CLI :
...
- Kleine Beträge**: Beginnen Sie mit weniger als Sats 10.000 für den Einstieg
- Mehrere Relais**: Konfigurieren Sie 3-5 geografisch verteilte Relais
- Netzwerkschutz**: Aktivieren Sie VPN oder Tor, um Ihre IP Address zu verbergen
- Wallet sicher**: Aktivieren Sie die automatische Sperre Ihres Wallet Lightning
...
Mostro wird Sie bitten, einen **Lightning Invoice** anzugeben, um die BTC zu erhalten. Der genaue Betrag in Satoshis wird angezeigt (hier: 4715 Sats).
...
### Schritt 3: Stellen Sie Ihren Lightning Invoice bereit



generate ein Lightning Invoice von Ihrem Wallet (Phoenix, Breez, etc.) für den genauen Betrag, dann senden Sie es :
...
Das System bestätigt die Sendung und teilt Ihnen mit, dass Sie auf die Zahlung der Invoice-Haltegebühr durch den Verkäufer warten sollen, bevor Sie das Treuhandkonto aktivieren.
...
Prüfen Sie direkte Nachrichten, um die Antwort des Verkäufers zu sehen:
...
Sobald der Verkäufer den Erhalt des Fiat bestätigt und das Treuhandkonto mit dem Befehl `release` freigibt, erhalten Sie Ihre Bitcoins sofort auf dem von Ihnen bereitgestellten Lightning Invoice.
...
**Wichtig für CLI/Mobile-Synchronisierung**: Wenn Sie sowohl den CLI als auch die mobile App verwenden, konfigurieren Sie **exakt dieselben Nostr-Relais und Mostro Pubkey** in beiden Clients. Ohne diese identische Konfiguration werden Sie nicht die gleichen Aufträge auf beiden Schnittstellen sehen. Die in den Einstellungen (Screenshot oben) sichtbaren Relais und der Mostro-Pubkey müssen mit denen in der CLI-"env"-Datei übereinstimmen.
...
Drücken Sie die Taste **"+"** des Green unten rechts, und wählen Sie dann **Verkaufen** (rote Taste).
...
**Wichtige Anweisung**: Sie müssen jetzt **den angezeigten Invoice-Hold** bezahlen, um Ihre Bitcoins (hier 4674 Sats für 1-5 EUR) im Treuhandkonto zu sperren. Sie haben **maximal 15 Minuten** Zeit, sonst wird der Exchange storniert.



Kopieren Sie den Invoice-Halt und bezahlen Sie ihn von Ihrem Wallet Lightning (Phoenix, Breez usw.).
...
Sobald Sie **den Eingang** der Fiat-Zahlung auf Ihrem Konto bestätigt haben, drücken Sie die Green-Taste **Freigeben** und bestätigen Sie, um die Satss an den Käufer zu senden.
...
Die Bitcoins werden dem Käufer sofort über seinen Lightning Invoice überwiesen.
...
4. Geben Sie Ihren Lightning Invoice an (generiert aus Ihrem Wallet)
...
9. Erhalten Sie Bitcoins sofort auf Ihrem Lightning Invoice
...
**Finanzielle Souveränität**: Ihre Bitcoins verlassen dank des Invoice-Hold-Mechanismus niemals Ihre direkte Kontrolle, wodurch das Risiko eines Exchange-Bankrotts oder von Piraterie ausgeschlossen wird.
