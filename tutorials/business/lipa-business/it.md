...
Affinché un'azienda possa adottare facilmente il Bitcoin, la soluzione di pagamento deve rimanere semplice con un intuitivo registratore di cassa Interface, offrire funzioni multi-utente e, idealmente, offrire la conversione automatica in valuta locale per mitigare la volatilità.
...
Lipa for Business è un'applicazione mobile che funziona come un registratore di cassa Bitcoin e Lightning. Offre un Interface semplificato per raccogliere i pagamenti in Sats e integra funzioni professionali: accesso ai dipendenti, esportazioni contabili, dashboard web, il tutto senza mai entrare in possesso dei vostri fondi.
...
**Pagamenti istantanei Lightning**: Le fatture Bitcoin Lightning vengono generate in pochi secondi, garantendo transazioni praticamente istantanee senza intermediari bancari. Le commissioni sono basse e trasparenti rispetto alle soluzioni tradizionali.



**Interface POS intuitivo**: L'applicazione fornisce un POS Interface chiaro, progettato per un facile utilizzo in negozio. Inserendo l'importo in valuta locale, l'app visualizza immediatamente l'importo in BTC e un codice QR per il pagamento. Compatibile con tutti i portafogli Lightning presenti sul mercato.
...
**Cruscotto web**: Pannello di amministrazione del Interface accessibile tramite dashboard.lipa.swiss, che consente di visualizzare tutte le transazioni, filtrare per periodo o dipendente ed esportare i dati contabili in formato CSV. Il cruscotto può essere utilizzato anche per emettere fatture web generate con codici QR direttamente da Interface.
...
- Andate su lipa.swiss/for-business e scaricate l'applicazione per la vostra piattaforma (Android o iOS)
- Installare "lipa Wallet for business" da Google Play o dall'App Store
- Al primo avvio, inserire i dati dell'azienda: nome dell'azienda, e-mail di contatto, telefono e Address
- L'e-mail è l'identificativo principale per accedere al cruscotto web



Una volta inviato il modulo, Lipa crea il vostro spazio commerciale. Prima dell'attivazione definitiva può essere effettuato un breve controllo manuale (processo KYC semplificato). L'attivazione avviene solitamente entro 24 ore, ma i tempi possono variare.



**Importante**: Non è necessario un collegamento al conto bancario per iniziare a incassare Bitcoin. Le coordinate bancarie sono necessarie solo se si sceglie la conversione automatica in CHF.



## Installazione e Interface



**Applicazione mobile**: Disponibile per smartphone e tablet Android/iOS. Interface è stato progettato per essere utilizzato nel punto vendita, con Elements di facile lettura e interazioni limitate allo stretto necessario. Un pulsante "Incassa un pagamento" consente di accedere alla schermata di inserimento dell'importo.
...
**Cruscotto web**: Cruscotto gratuito accessibile tramite dashboard.lipa.swiss. Connessione e-mail sicura (link magico senza password). Interface mostra tutte le transazioni con tutti i dettagli: data, importo BTC/fiat, tasso Exchange, dipendente, ecc. Esportazione CSV per l'integrazione contabile.
...
Il cruscotto può essere utilizzato anche per emettere fatture web generate con codici QR direttamente dal Interface :
...
**Multi-terminale**: Supporto nativo per più terminali all'interno di un'azienda. Aggiungete nuovi dispositivi creando dipendenti tramite invito con codice QR. Ogni terminale è collegato allo stesso esercente Wallet, mantenendo la tracciabilità per cassiere.
...
**Storia e statistiche**: Ogni pagamento viene registrato con tutti i dettagli. Il cruscotto offre una panoramica con filtri per periodo o dipendente, paragonabile a un classico sistema di cassa.
...
**Backup**: Conto commerciale non custodiale con frase di recupero di 24 parole da conservare. Solo il proprietario del Wallet principale deve gestire questo backup; i dipendenti non possono accedervi.
...
**Commissioni**: 0.commissione del 98% sugli importi convertiti (rispetto allo 0% per i pagamenti in BTC). Nessun'altra commissione nascosta o abbonamento.



**Come funziona** : L'importo ricevuto viene immediatamente venduto al tasso di mercato e trasferito sul vostro conto bancario. Il trasferimento avviene secondo le scadenze bancarie standard della vostra banca.
...
**Non detentivo**: L'utente mantiene il controllo permanente sui fondi ricevuti. Durante la configurazione viene generata una coppia di chiavi private/pubbliche (da qui la frase di 24 parole). Lipa non memorizza le chiavi private.



**Sicurezza dei dipendenti**: I dipendenti possono solo creare fatture, non spendere fondi. In caso di problemi al terminale, i fondi restano al sicuro e l'utente può revocare l'accesso.
...
- Interface semplice, senza necessità di competenze tecniche
- Soluzione non detentiva con controllo totale dei fondi
- Gestione di più dipendenti con tracciabilità
- Esportazione contabile e cruscotto web inclusi
...
- Solo supporto per fulmini (no Bitcoin On-Chain)
...
Lipa for Business si propone come soluzione completa per l'accettazione di Bitcoin in negozio. Non sono necessarie infrastrutture costose (basta un semplice smartphone), i costi sono bassi e fissi e l'integrazione nei processi esistenti è semplice.



La sua natura non custodiale e rispettosa della privacy, unita a strumenti di gestione professionali, lo rende una scelta interessante per gli esercenti che desiderano adottare il Bitcoin mantenendo semplicità e sicurezza.
