---
name: SeedSigner
description: A Hardware Wallet that's DIY, stateless, affordable and fully air-gapped
---

![cover](assets/cover.webp)


The SeedSigner is an open-source Hardware Wallet Bitcoin that anyone can build themselves using inexpensive, general-purpose electronic components. Unlike commercial products such as the Ledger, Coldcard or Trezor, this is not a ready-to-use device manufactured by a company: it's a community project that lets everyone create their own device, controlling every step.
...
- Waveshare 1.3" screen (240×240 px)** (in French)
    ...
    - The display incorporates three buttons and a mini-joystick serving as the user's Interface.
...
- Housing (optional, but recommended)** (optional, but recommended)** (optional, but recommended)** (optional, but recommended)** (recommended)
...
You can buy these components separately or, for greater simplicity, opt for ready-made packs that include all the necessary hardware. Personally, I ordered my pack [from this French site](https://bitcoinbazar.fr/), but you'll also find a list of vendors for each region of the world on [the SeedSigner project hardware page](https://seedsigner.com/hardware/). If you prefer to buy the components individually, they are available on the major e-commerce platforms or in specialist stores.
...
PowerShell then calculates the Hash SHA256 of your image file. Compare this Hash with the corresponding value in `seedsigner.0.8.6.sha256.txt`.
...
The current firmware version supports several software packages: Sparrow, Specter, BlueWallet, Nunchuk and Keeper. In my case, I use **Sparrow wallet**, which I particularly recommend for its simplicity and rich functionality.
...
- gW-20 (satoshis, or 1/100,000,000 BTC)
...
- gW-22 network`: to be modified only if you wish to use the SeedSigner on Testnet.
...
Finally, in the `Language` menu, you can change the language of the Interface to suit your preference.


![Image](assets/fr/028.webp)


## 5. Creating and saving the seed


The seed (or mnemonic phrase) forms the basis of your Bitcoin portfolio. It is used to derive your private keys and addresses, and provides access to your funds. SeedSigner offers several methods for generating it, which we will explore in this section.


Before we begin, a few essential reminders:


- This phrase gives you full, unrestricted access to all your bitcoins.** Anyone in possession of this phrase can steal your funds, even without physical access to your SeedSigner ;
- Usually, a 12-word phrase is used to restore a wallet if a Hardware Wallet is lost or stolen. But since the SeedSigner is a *stateless* device, it never registers your seed. So your physical backups are not just backup copies, but **the only way to use your wallet**. If you lose these backups, your bitcoins will be permanently lost. So back them up carefully, on several media and in safe places;
...
### 5.1 Access the seed creation tool
...
Finally, a more advanced and secure alternative is to use the SeedSigner with a **SeedKeeper**, which stores the seed in a secure element. To find out more, see this tutorial :
...
It's also at this stage that you can add a **passphrase BIP39** to reinforce the security of your wallet. This option may be worthwhile, depending on your backup strategy, but it also entails risks: if you lose the passphrase, access to your bitcoins will be permanently lost.
...
The SeedSigner lets you convert your seed into a paper QR code, called *SeedQR*. This method simplifies reloading your wallet, as it avoids retyping each word manually.
...
Congratulations, your Bitcoin portfolio is now up and running! We're now going to import its public components into **Sparrow wallet** to manage it easily.
...
Make sure you have the Sparrow wallet software installed on your computer. If you don't know how to download, check and install it correctly, please see our full tutorial on the subject:
...
Before sharing your receiving address, it is very important to check that it belongs to your seed. This step ensures that your SeedSigner will be able to sign transactions associated with this address. It also protects against possible attacks in which Sparrow displays a fraudulent address. Remember that Sparrow runs on an insecure environment (your computer), which has a much larger attack surface than your SeedSigner, which is totally isolated. That's why you should never blindly believe any recipient address displayed on Sparrow until you've verified it with your Hardware Wallet.
...
- Finally, select the fee rate based on current market conditions (estimates are available on [Mempool.space](https://Mempool.space/)).
...
Click on `Review Details` and carefully check all the information directly on the SeedSigner screen. The most important items to check are the amount sent, the recipient's address and the amount of any charges applied.
