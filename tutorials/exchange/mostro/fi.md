---
name: Mostro
description: KYC-vapaa Bitcoin P2P Exchange -alusta Lightningin ja Nostrin kautta
---
...
**Mostro P2P** tarjoaa hajautetun vaihtoehdon, jossa yhdistyvät Lightning Network, Nostr-protokolla ja holding-laskut. Sen tärkein innovaatio: automatisoitu escrow-järjestelmä, jossa bitcoinisi pysyvät hallinnassasi koko Exchange:n ajan, mikä poistaa varkauden, Exchange:n konkurssin tai mielivaltaisen takavarikon riskin.
...
**Depentralisoitu välimiesmenettely**: Riitatilanteessa puolueeton välimies tutkii todisteet ja tekee päätöksen avoimiin kriteereihin perustuen. Jokaisesta riidasta syntyy yksilöllinen token jäljitettävyyden varmistamiseksi.



**Maksamisen joustavuus**: Yadio.io Exchange-kurssipalvelun kautta. Hyväksyy SEPA-siirrot, PayPalin, Revolutin, käteisen ja minkä tahansa osapuolten välillä sovitun menetelmän.
...
**Mostro CLI** on kehittynein ja toimivin asiakasohjelma. Se on kirjoitettu Rust-kielellä ja tarjoaa kaikki Mostro-ominaisuudet Interface-komentorivin kautta. Yhteensopiva macOS:n, Linuxin ja Windowsin kanssa.
...
- `release`: Vapauta escrow ja viimeistele Exchange
- `getdm`: Näytä suorat viestit
- `rate` : Arvioi vastapuolesi Exchange:n jälkeen
...
Alpha-version **mobiilisovellus** mahdollistaa P2P-kaupankäynnin suoraan älypuhelimesta. Intuitiivinen graafinen Interface, jossa on välilehtinavigointi, toimeksiantojen katselu, kehittyneet suodattimet ja integroitu chat, jonka avulla voit kommunikoida vastapuoliesi kanssa.
...
Interface kehittynyt JavaScript-verkkosovellus, jota kehitetään parhaillaan. Suunniteltu tarjoamaan kattava käyttäjäkokemus ja laajat kaupankäynti- ja salkunhallintatoiminnot. Käyttö selaimen kautta ilman asennusta.
...
- Toimiva Lightning Network** Wallet, jossa on riittävästi likviditeettiä (tai Lightning-yhteensopiva)
 - Suositellaan: Phoenix, Breez, Wallet ja Satoshi...
...
- Valinnainen mutta suositeltava**: VPN- tai Tor-yhteys IP Address:n peittämiseksi
...
**Vaihe 1: Rust-tarkastus** *Vaihe 1: Rust-tarkastus**
...
**Tärkeää CLI/Mobile**-synkronoinnin kannalta: Jos haluat käyttää samoja tilauksia CLI:ssä ja mobiilisovelluksessa, sinun on käytettävä **samaa Mostro Pubkey -avainta** ja **samaa Nostr-releetä** molemmissa asiakkaissa. Tarkista nämä asetukset mobiilisovelluksen Asetuksista.
...
Kokoaminen kestää 1-2 minuuttia. Suoritettava `mostro-CLI`-tiedosto asennetaan tiedostoon `~/.cargo/bin/`.
...
- Kauppa Bitcoin vapaasti - ei KYC** : Kaupankäynti ilman henkilöllisyyden tarkistusta Nostrin ansiosta
...
- Verkon suojaus**: Aktivoi VPN tai Tor piilottaaksesi IP-osoitteesi Address
...
Mostro pyytää sinua antamaan **Lightning Invoice** saadaksesi BTC:n. Tarkka määrä satosheina ilmoitetaan (tässä: 4715 Sats).
...
### Vaihe 3: Anna Lightning Invoice -laitteesi



generate Lightning Invoice:n Wallet:stä (Phoenix, Breez jne.) tarkalleen vastaavan summan ja lähetä se sitten :
...
Järjestelmä vahvistaa lähetyksen ja ilmoittaa, että sinun on odotettava, että myyjä maksaa Invoice-varausmaksun, ennen kuin voit aktivoida escrow-talletuksen.
...
Heti kun myyjä vahvistaa fiatin vastaanottamisen ja vapauttaa escrow-komennon `release`-komennolla, saat bitcoinisi välittömästi toimittamallasi Lightning Invoice -laitteella.
...
- Lightning Address** (valinnainen): Maksujen vastaanottaminen suoraan
...
Paina Green:n **"+"**-painiketta oikeassa alakulmassa ja valitse sitten **MYYNTI** (punainen painike).
...
**Tärkeä ohje**: Sinun täytyy nyt **maksaa näytetty Invoice hold** lukitaksesi bitcoinisi (tässä 4674 Sats 1-5 EUR:lle) escrowiin. Sinulla on **15 minuuttia aikaa**, muuten Exchange peruutetaan.



Kopioi Invoice:n pidätys ja maksa se Wallet Lightningista (Phoenix, Breez jne.).
...
Kun olet **vahvistanut** fiat-maksun saapumisen tilillesi, paina Green **VAPAUTTAA** -painiketta ja vahvista lähettääksesi Satssin ostajalle.
...
Bitcoinit siirretään välittömästi ostajalle hänen Lightning Invoice:nsä kautta.
...
4. Toimita Lightning Invoice (joka on luotu Wallet:sta)
...
9. Vastaanota bitcoineja välittömästi Lightning Invoice -laitteellasi
...
**Rahoituksellinen suvereniteetti**: Invoice hold-mekanismin ansiosta bitcoinisi eivät koskaan poistu suorasta hallinnastasi, mikä eliminoi Exchange konkurssin tai piratismin riskin.
...
Mostro P2P on lupaava vaihtoehto keskitetyille pörsseille, sillä se yhdistää Lightning Network:n, Nostrin ja automaattisen escrow:n aidosti hajautetun kaupankäynnin mahdollistamiseksi. Huolimatta varhaisesta kehitysvaiheestaan ja vähäisestä likviditeetistään alusta tarjoaa jo nyt taloudellista suvereniteettia, sensuurinsietokykyä ja oletusarvoista anonymiteettiä.
