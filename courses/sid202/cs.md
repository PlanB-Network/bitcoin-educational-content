...
Elements je [blockchainová](https://planb.academy/resources/glossary/blockchain) platforma s otevřeným zdrojovým kódem, která umožňuje přístup k výkonným funkcím vyvinutým členy komunity, jako jsou důvěrné transakce a emitovaná aktiva.

Elements je ve své podstatě protokol, který umožňuje dosáhnout [konsenzu](https://planb.academy/resources/glossary/consensus) kolem historie [transakcí](https://planb.academy/resources/glossary/transaction-tx) a pravidel, jimiž se řídí převod a vytváření aktiv uložených v distribuované blockchainové účetní knize.
...
Elements poskytuje technické řešení problémů, se kterými se uživatelé blockchainu denně potýkají: zpoždění transakcí, nedostatek soukromí a riziko [zaměnitelnosti](https://planb.academy/resources/glossary/fungibility).
...
[Dokument](https://planb.academy/resources/glossary/white-paper) Strong Federations si můžete přečíst zde: https://blockstream.com/strong-federations.pdf
...
Na vysoké úrovni dochází k převodům do sidechainu, když někdo pošle aktiva z mainchainu na adresu kontrolovanou [peněženkou](https://planb.academy/resources/glossary/wallet) Watchmen s více podpisy. Tím se aktiva v mainchainu účinně zmrazí. Watchmen pak transakci ověří a uvolní stejné množství přidružených aktiv v rámci sidechainu. Uvolněná aktiva jsou odeslána do sidechainové peněženky, která může prokázat nárok na původní aktiva v mainchainu. Tento proces účinně přesouvá aktiva z hlavního řetězce do sidechainu.
...
Samotný software [uzlu](https://planb.academy/resources/glossary/node) Elements se nazývá `elementsd` a běží jako démon na počítači uživatele. Démon (nebo služba v systému Windows) je program, který běží jako služba na pozadí, aniž by vyžadoval přímé řízení přihlášeného uživatele.
...
- Prahová hodnota `n z m`, kterou je třeba splnit, a platné [veřejné klíče](https://planb.academy/resources/glossary/public-key), které mohou podepisovat bloky.
- [Skript](https://planb.academy/resources/glossary/script), který je třeba splnit, aby bylo možné převádět aktiva do sidechainu a z něj.
...
I kdybychom importovali [soukromý klíč](https://planb.academy/resources/glossary/private-key) e2 do peněženky e1, stále by nebyla schopna zjistit množství a typ transakcí, protože stále nezná zaslepující klíč používaný e2. Dokážeme to importem soukromého klíče používaného peněženkou e2 do peněženky e1. Nejprve musíme vyexportovat klíč z e2
...
Nyní je třeba vyzkoušet některé bloky na obou řetězcích. Zralost bloků je požadavkem procesu peg, protože chrání před reorganizací bloků v hlavním řetězci, která by vedla k [inflaci](https://planb.academy/resources/glossary/inflation) nabídky pegovaných aktiv v rámci sidechainu.
