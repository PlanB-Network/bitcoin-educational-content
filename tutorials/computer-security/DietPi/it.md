---
name: DietPi
description: Sistema operativo leggero e ottimizzato per macchine poco prestanti. Con un occhio al self-hosting
---
...
Durante l'installazione ti verranno mostrati i progressi.
...
Cosa puoi fare?
Automatizzare i processi da avviare all'accensione del server, configurare il `Locale` e tutte le impostazioni correlate alla Time Zone, impostare le schede di rete, le password e le periferiche audio/video, ad esempio.
...
Per l'amministrazione del server in generale, invece, troverai più comodo l'uso del terminale. **Ricorda che per effettuare le onfigurazioni devi sempre avere i permessi di `root`**. 
...
- **Server DNS**
Con DietPi avrai a disposizione una manciata di server DNS. Potresti iniziare da **Pi Hole**, un sistema che blocca gli annunci pubblicitari su qualunque dispositivo collegato in rete con il tuo server. 
