...
> "Ele é muito grande", todos diziam, e o galo da Índia, que nasceu com esporas e se achava o imperador, inflou-se como um navio com todas as velas içadas e avançou furiosamente em sua direção, vermelho de raiva até os olhos. O pobre patinho não sabia se devia parar ou continuar andando: ele ficou muito triste por ser ridicularizado por todos os patos da corte.
...
Essa questão é tão antiga quanto o White Paper. Satoshi já nos alertava sobre esse risco em seu trabalho publicado no final de 2008:
...
> A única diferença importante está na transação de notificação. A versão 1 usa um endereço simples com OP_RETURN para notificação, a versão 2 usa um script multisig (bloom-multisig) com OP_RETURN e as versões 3 e 4 simplesmente usam um script multisig (cfilter-multisig). Os mecanismos discutidos neste artigo, incluindo os métodos criptográficos estudados, são aplicáveis às quatro versões. Até o momento, a implementação do PayNym no Samourai Wallet e Sparrow usa a primeira versão do BIP47.
...
Além disso, você provavelmente já ouviu de um entusiasta do bitcoin que os endereços de recebimento são de uso único e que é necessário gerar um novo para cada novo pagamento recebido em sua carteira. Certo, mas por quê?
...
Como mencionado na introdução, a transparência e a distribuição da rede Bitcoin permitem que qualquer usuário, desde que tenha acesso a um nó, possa observar as transações do sistema de pagamento. Portanto, ele pode ver os diferentes saldos dos endereços. Satoshi Nakamoto mencionou a possibilidade de gerar novos pares de chaves e, assim, novos endereços, para cada novo pagamento recebido em uma carteira. O objetivo seria ter uma camada adicional de proteção caso haja associação entre a identidade do usuário e um de seus pares de chaves.
...
A busca pela privacidade não é um conforto ou fantasia de um maximalista do bitcoin. É um parâmetro específico que afeta diretamente sua segurança pessoal e a segurança de seus fundos. Para ilustrar isso, aqui está um exemplo muito concreto:
...
Esse problema de reutilização de endereço não é insignificante no Bitcoin. Como você pode ver no gráfico abaixo, extraído do site [oxt.me](http://oxt.me/), a taxa global de reutilização de endereços pelos usuários do Bitcoin é atualmente de 52%:
...
Um usuário pode então comunicar livremente seu código de pagamento (nas redes sociais, em seu site...) sem o risco de perda de privacidade, ao contrário de um endereço de recebimento comum ou de uma chave pública.
...
Para o usuário, o processo de pagamento BIP47 com a implementação do PayNym é extremamente simples. Vamos supor que Alice queira enviar pagamentos para Bob:
...
Agora que vimos a teoria, vamos estudar a prática juntos. A ideia dos tutoriais abaixo é conectar meu PayNym na minha carteira Sparrow com meu PayNym na minha carteira Samourai. O primeiro tutorial mostra como fazer uma transação usando o código de pagamento reutilizável do Samourai para o Sparrow, e o segundo tutorial descreve o mesmo mecanismo do Sparrow para o Samourai.
...
O primeiro passo para fazer um pagamento BIP47 é obter o código de pagamento reutilizável do nosso destinatário. Em seguida, poderemos nos conectar a ele e, posteriormente, fazer a transação:
...
Da mesma forma que com a Samourai, você obviamente precisa ter o software Sparrow. Ele está disponível para computador. Você pode baixá-lo em seu [site oficial](https://sparrowwallet.com/).
...
Agora que pudemos estudar a parte prática da implementação do PayNym do BIP47, vamos ver como todos esses mecanismos funcionam e quais métodos criptográficos são usados.
...
* Byte 1: O campo de bits. Este espaço é reservado para fornecer informações adicionais em caso de uso específico. Se estivermos usando apenas o PayNym, este byte será igual a 0x00.
...
* Do byte 3 ao byte 34: O valor de x. Esses bytes indicam a abscissa de nossa chave pública. A concatenação de x e a paridade de y nos dá nossa chave pública comprimida.
...
Este segredo compartilhado (a chave vermelha) pode então ser usado para realizar outras tarefas. Tipicamente, este segredo compartilhado pode ser usado para criptografar e descriptografar uma comunicação em uma rede não segura:

![image](assets/12.png)

Para realizar esta troca com sucesso, Diffie-Hellman utiliza a aritmética modular para calcular o segredo comum. Aqui está uma explicação simplificada de como funciona:

* Alice e Bob escolhem uma cor comum, neste caso, amarelo. Esta cor é conhecida por todos. É um dado público.

* Alice escolhe uma cor secreta, neste caso, vermelho. Ela mistura as duas cores, o que resulta em laranja.

* Bob escolhe uma cor secreta, neste caso, azul-pato. Ele mistura as duas cores, o que resulta em azul-celeste.

* Alice e Bob podem trocar as cores obtidas: laranja e azul-celeste. Esta troca pode ser feita em uma rede não segura e observada por atacantes.

* Alice mistura a cor azul-celeste recebida de Bob com sua cor secreta (vermelho). Ela obtém marrom.
...
> A é igual a g elevado a potência a módulo p:
A = g^a % p

> B é igual a g elevado a potência b módulo p:
B = g^b % p
...
> z é igual a B elevado a potência a módulo p:
z = B^a % p
...
![Diagrama do funcionamento técnico do Diffie-Hellman](assets/14.png)
...
Assim, desde que escolhamos a, b e p suficientemente grandes, o Diffie-Hellman é seguro. Tipicamente, com parâmetros de 2.048 bits (um número de 600 dígitos em decimal), testar todas as possibilidades para a e b seria quimérico. Até o momento, com números desse tamanho, o algoritmo é considerado seguro.

É justamente nesse ponto que reside a principal desvantagem do protocolo Diffie-Hellman. Para ser seguro, o algoritmo deve usar números grandes. Como resultado, hoje em dia preferimos usar o algoritmo ECDH, uma variante do Diffie-Hellman que usa uma curva algébrica, mais especificamente uma curva elíptica. Isso nos permite trabalhar com números muito menores, mantendo uma segurança equivalente e, portanto, reduzindo os recursos necessários para o cálculo e o armazenamento.

O princípio geral do algoritmo permanece o mesmo. No entanto, em vez de usar um número aleatório a e um número A calculado a partir de a com exponenciação modular, usaremos um par de chaves estabelecidas em uma curva elíptica. Em vez de depender da distributividade do operador módulo, usaremos aqui a lei de grupo em curvas elípticas, mais precisamente a associatividade dessa lei.
Se você não tem noção de como funcionam as chaves privadas e públicas em uma curva elíptica, vou explicar os conceitos básicos dessa metodologia nas seis primeiras partes deste artigo.
...
Portanto, é essencial manter uma separação estrita entre os códigos de pagamento dos usuários. Com esse objetivo, a etapa de comunicação inicial do código é um momento crítico para a privacidade do pagamento, e ainda assim é obrigatória para o bom funcionamento do protocolo. Se um dos códigos de pagamento puder ser recuperado publicamente (por exemplo, em um site), o segundo código, ou seja, o do remetente, não deve ser associado ao primeiro.
...
Como posso transmitir isso para eles? Se eu enviar por meio de uma forma de comunicação convencional, a informação pode vazar e eu posso ser marcado como uma pessoa que apoia movimentos pacíficos.
...
Agora, vamos ver como funciona essa transação de notificação. Vamos imaginar que Alice queira enviar fundos para Bob com o BIP47. No meu exemplo, Alice age como remetente e Bob como destinatário. Este último publicou seu código de pagamento em seu site. Alice já está ciente do código de pagamento de Bob.
...
* Alice calcula um ponto secreto "S" (maiúsculo) na curva elíptica adicionando e duplicando pontos aplicando sua chave privada "a" à chave pública de Bob "B".
...
* Alice calcula o cifrado (x') da abscissa da chave pública (x) de seu código de pagamento e o cifrado (c') de seu código de cadeia (c) separadamente. "f1" e "f2" atuam como chaves de criptografia. A operação usada é o XOR (ou exclusivo).
...
Essa simetria é permitida pelas propriedades comutativa e associativa do operador XOR:
...
OP_RETURN é um Opcode, ou seja, um script, que marca uma saída de transação Bitcoin como inválida. Hoje, ele é usado para transmitir ou ancorar informações na blockchain do Bitcoin. Até 80 bytes de dados podem ser armazenados nele, que são registrados na cadeia e, portanto, visíveis para todos os outros usuários.
...
![Diagrama Alice envia seu código de pagamento mascarado para o endereço de notificação de Bob](assets/16.png)
...
Se relacionarmos esse diagrama com o que descrevi anteriormente:
...
* Ela usa esse fator de cegamento para criptografar a carga útil de seu próprio código de pagamento.
...
> 010002
>
> A abscissa criptografada da chave pública do meu código de pagamento:
>
...
Entre os opcodes, podemos reconhecer 0x6a que indica o OP_RETURN e 0x4c que indica o OP_PUSHDATA1. O byte seguinte a este último opcode indica o tamanho da carga útil que segue. Ele indica 0x50, ou seja, 80 bytes.
...
* Bob determina o fator cego "f" que permitirá descriptografar a carga útil do código de pagamento de Alice. Da mesma forma que Alice calculou anteriormente, Bob encontrará "f" aplicando HMAC-SHA512 em (x) o valor da abscissa do ponto secreto "S" e em (o) o UTXO consumido como entrada dessa transação de notificação:

> f = HMAC-SHA512(o, x)


4. Bob interpreta os dados do OP_RETURN na transação de notificação como um código de pagamento. Ele simplesmente descriptografa a carga útil desse potencial código de pagamento usando o fator cego "f":
...
Por que isso funciona? Como Bob consegue determinar o mesmo fator cegante que Alice e, portanto, descriptografar seu código de pagamento? Vamos examinar mais detalhadamente a ação do ECDH no que acabamos de descrever.

Primeiro, estamos lidando com uma criptografia simétrica. Isso significa que a chave de criptografia e a chave de descriptografia são o mesmo valor. Essa chave na transação de notificação é o fator cegante (f = f1 || f2). Portanto, Alice e Bob devem obter o mesmo valor para f, sem transmiti-lo diretamente, pois um atacante poderia roubá-lo e descriptografar a informação secreta.

Esse fator cegante é obtido aplicando HMAC-SHA512 a dois valores: a abscissa de um ponto secreto e o UTXO consumido como entrada da transação. Portanto, Bob precisa ter essas duas informações para descriptografar a carga útil do código de pagamento de Alice.
...
Vou resumir as etapas que acabamos de ver juntos para receber e interpretar uma transação de notificação:
...
* Ele usa esse fator de cegamento para descriptografar a carga útil do código de pagamento de Alice contida no OP_RETURN.

### A transação de pagamento BIP47.

Agora vamos estudar juntos o processo de pagamento com o BIP47. Para lembrar o estado atual da situação:

* Alice conhece o código de pagamento de Bob que ela simplesmente pegou em seu site.
...
Cada vez que Alice deseja enviar um pagamento para Bob, ela gera um novo endereço único em branco, mais uma vez usando o protocolo ECDH:
...
![Alice envia bitcoins com BIP47 para Bob](assets/21.png)

Crédito: Reusable Payment Codes for Hierarchical Deterministic Wallets, Justus Ranvier. https://github.com/bitcoin/bips/blob/master/bip-0047.mediawiki
Se correspondermos esse esquema com o que eu descrevi anteriormente para vocês:
...
* Alice pode enviar uma transação comum para o Bob com o endereço de recebimento efêmero derivado.
...
![Alice deriva três endereços de recebimento BIP47 para o Bob](assets/22.png)
...
* Da mesma forma que Alice, Bob calcula o ponto "s·G" na curva elíptica. Em seguida, ele adiciona esse ponto secreto à sua chave pública "B". Ele obtém um novo ponto na curva elíptica que interpreta como uma chave pública "K0":
...
Se associarmos esse esquema ao que eu descrevi anteriormente:
...
É possível que no futuro sejam encontradas outras soluções menos dispendiosas para comunicar o código de pagamento do remetente ao destinatário e permitir que este o armazene de forma segura. Mas, por enquanto, a transação de notificação continua sendo a solução com menos compromissos.

Essa desvantagem é insignificante quando se observam todos os benefícios do BIP47. Entre todas as propostas existentes para resolver esse problema de reutilização de endereço, parece-me ser a melhor solução.
...
Qualquer solução verdadeiramente positiva para a privacidade do usuário deve ser discutida, impulsionada e defendida, para que o Bitcoin não se torne um playground para CA e uma ferramenta de vigilância governamental.

Ele estava pensando na maneira como havia sido perseguido e insultado em todos os lugares, e agora ele os ouvia todos dizerem que ele era o mais bonito de todos aqueles belos pássaros! E até mesmo o sabugueiro inclinava seus galhos para ele, e o sol espalhava uma luz tão quente e benéfica! Então suas penas se encheram, seu pescoço elegante se ergueu, e ele exclamou de todo o coração: "Como eu poderia sonhar com tanta felicidade quando eu era apenas um patinho feio."
...
Agradeço a LaurentMT e Théo Pantamis pelos muitos conceitos que eles me explicaram e que usei neste artigo. Espero ter conseguido transmiti-los com precisão.
