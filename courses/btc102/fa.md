...
همانطور که می‌دانید، پلتفرم Plan ₿ Academy به طور انحصاری به Bitcoin اختصاص دارد. با این حال، درک تفاوت با سایر ارزهای دیجیتال به شما کمک می‌کند تا از مشکلات مرتبط با پروژه‌های بی‌فایده و گاهی حتی کلاهبرداری جلوگیری کنید.
...
قبل از کلیک کردن روی یک لینک یا انجام هر اقدامی، همیشه هویت فرستنده را دوباره بررسی کنید. در صورت شک، به جای استفاده از لینک ارائه شده، به صورت دستی به وب‌سایت مراجعه کنید. مهم‌تر از همه، هرگز [کلیدهای خصوصی](https://planb.academy/resources/glossary/private-key) یا رمزهای عبور خود را با کسی به اشتراک نگذارید.


#### Bitcoin هاردفورک‌ها


در طول سال‌ها، Bitcoin چندین *انشعاب Hard* را تجربه کرده است که منجر به ایجاد نسخه‌های جایگزین از ارز دیجیتال اصلی شده‌اند. به زبان ساده، یک *Hard Fork* شکافی در شبکه است که به دو [بلاکچین](https://planb.academy/resources/glossary/blockchain) جداگانه منجر می‌شود و هر دو تا لحظه انشعاب تاریخچه یکسانی دارند. این انشعابات معمولاً زمانی رخ می‌دهند که بخشی از جامعه توسعه‌دهندگان یا اکوسیستم گسترده‌تر Bitcoin می‌خواهد تغییرات عمده‌ای را در پروتکل اصلی معرفی کند اما نمی‌تواند به توافق گسترده‌ای دست یابد. به جای رها کردن ایده‌های خود، تصمیم می‌گیرند نسخه جدیدی از Bitcoin (با قوانین تغییر یافته) را راه‌اندازی کنند به امید اینکه کاربران و ماینرها تصمیم بگیرند به جای آن از Fork پیروی کنند.
...
اگر می‌خواهید عمیق‌تر بروید، دوره SEC101 در Plan ₿ Academy یک تجزیه و تحلیل جامع از استراتژی‌های امنیت سایبری ارائه می‌دهد که به‌طور خاص برای کاربران Bitcoin طراحی شده است.
...
ما چندین آموزش در دسترس برای Plan ₿ Academy داریم که می‌تواند شما را در راه‌اندازی آسان یک VPN راهنمایی کند. من به‌ویژه IVPN یا Mullvad را توصیه می‌کنم:
...
همچنین حتماً [آموزش‌های امنیت کامپیوتر ما](https://planb.academy/tutorials/computer-security) را در Plan ₿ Academy بررسی کنید.
...
- از **[Shitcoins](https://planb.academy/resources/glossary/shitcoin)** و سایر ارزهای دیجیتال بی‌فایده بر حذر باشید؛
...
چند ساعتی را به کاوش در Bitcoin بگذرانید، منابع رایگان در Plan ₿ Academy را بررسی کنید، چند کتاب بخوانید، پنج یورو برای امتحان کردن آن بپردازید و محتوای با کیفیتی را به صورت آنلاین تماشا کنید. کنجکاو بمانید. هرچه راحت‌تر شوید، بازبینی استراتژی، اصلاح رویکرد و پیشرفت با اطمینان برایتان آسان‌تر خواهد شد.
...
Bitcoin یک شبه به وجود نیامد. این نتیجه دهه‌ها تحقیق و آزمایش در [رمزنگاری](https://planb.academy/resources/glossary/cryptography)، علوم کامپیوتر و نظریه پولی است. پیش از راه‌اندازی آن در سال 2009، چندین پروژه (مانند eCash، b-money، Bit Gold و RPOW) تلاش کردند تا ارزهای دیجیتال ایجاد کنند. همه با همان مشکل اصلی مواجه شدند: چگونه می‌توان در یک سیستم غیرمتمرکز بدون اتکا به یک مرجع مرکزی از دوبار خرج کردن جلوگیری کرد.


این چالش تا اواخر سال ۲۰۰۸ حل نشده باقی ماند، زمانی که فردی با نام Satoshi ناکاموتو [مقاله سفید](https://planb.academy/resources/glossary/white-paper) Bitcoin را منتشر کرد. چند ماه بعد، اولین نسخه از نرم‌افزار [متن‌باز](https://planb.academy/resources/glossary/foss) Bitcoin به صورت زنده اجرا شد و سیستمی را راه‌اندازی کرد که می‌توانست به‌طور مستقل از بانک‌ها یا دولت‌ها عمل کند.


این پروژه به شدت تحت تأثیر اصول Cypherpunk قرار گرفت؛ جامعه‌ای از توسعه‌دهندگان و متفکران که به استفاده از رمزنگاری برای حفاظت از آزادی‌های فردی در فضای آنلاین اعتقاد داشتند. برای آن‌ها، حریم خصوصی و تمرکززدایی ترجیحات فنی نبودند، بلکه ضرورت‌های ایدئولوژیک بودند. Bitcoin به عنوان موفق‌ترین تجسم این ایده‌ها ظهور کرد: یک شبکه پولی [همتا به همتا](https://planb.academy/resources/glossary/peertopeer-p2p) که هر کسی می‌توانست از آن استفاده کند، هیچ‌کس نمی‌توانست آن را کنترل کند و همه می‌توانستند آن را تأیید کنند.


![image](assets/fa/030.webp)


[سایفرپانک‌ها](https://planb.academy/resources/glossary/cypherpunks) یک جامعه غیررسمی و بین‌المللی از افرادی هستند که از استفاده از رمزنگاری برای دفاع از آزادی‌های شخصی آنلاین حمایت می‌کنند. آن‌ها به شدت به حق حریم خصوصی فرد اعتقاد دارند؛ به‌ویژه در دنیایی که به طور فزاینده‌ای تحت تأثیر نظارت دولتی و بهره‌برداری از داده‌های شرکتی قرار دارد.
...
در این محیط ترس و عدم اطمینان بود که Bitcoin متولد شد. خالق آن، که به نام Satoshi Nakamoto شناخته می‌شود، یک پیام بسیار نمادین را در اولین بلوک Bitcoin Blockchain، که به عنوان [بلوک Genesis](https://planb.academy/resources/glossary/genesis-block) شناخته می‌شود، قرار داد. پیام این بود:
...
به زبان ساده، هر شرکت‌کننده در شبکه Bitcoin (که به آن "[گره](https://planb.academy/resources/glossary/node)" گفته می‌شود) یک نسخه کامل از تراکنش Ledger دارد که به عنوان Blockchain شناخته می‌شود. هنگامی که یک تراکنش جدید اتفاق می‌افتد، به شبکه پخش می‌شود. سپس [ماینرها](https://planb.academy/resources/glossary/miner) این تراکنش‌ها را با گروه‌بندی آن‌ها در بلوک‌هایی تأیید می‌کنند که سپس به انتهای زنجیره اضافه می‌شوند (به همین دلیل به آن "Blockchain" گفته می‌شود).
...
این سطح از شفافیت معرفی درب‌های پشتی یا ایجاد تغییرات مخفی را به شدت دشوار می‌کند. هر کسی با مهارت‌های فنی می‌تواند یک نود را اجرا کند، به توسعه کمک کند، یا ابزارهای سازگار بسازد. در Bitcoin، اعتماد از طریق کد و [اجماع](https://planb.academy/resources/glossary/consensus) به دست می‌آید، نه از طریق کنترل متمرکز.
...
اگر مایل به گسترش دانش خود هستید، می‌توانید دوره BTC101 را در Plan ₿ Academy بگذرانید که به بررسی عمیق‌تری از اصول فنی و اقتصادی Bitcoin می‌پردازد.
...
- **طلا**، اگرچه برای هزاران سال به عنوان ذخیره ارزش خدمت کرده است، برای تقسیم *Hard*، حمل و نقل نامناسب و برای استفاده در شرایط بحرانی فوری غیرعملی است.
...
- [اقتصاددانان اتریشی](https://planb.academy/resources/glossary/austrian-school)، مدافع پول سالم و آزادی از دستکاری دولت؛
...
Mining یک عملکرد اصلی از شبکه Bitcoin است. این عملکرد امنیت سیستم را تضمین می‌کند و Blockchain را عملیاتی نگه می‌دارد. ماینرها با انجام محاسبات انرژی‌بر که به عنوان [Proof of Work](https://planb.academy/resources/glossary/proof-of-work) شناخته می‌شوند، تراکنش‌ها را تأیید و شبکه را ایمن می‌کنند. هر بلوک جدید استخراج شده، یک دسته از تراکنش‌ها را به Blockchain اضافه می‌کند و بیت‌کوین‌های جدید را طبق برنامه انتشار پروتکل آزاد می‌کند.
...
نام من روگزی است، بنیان‌گذار Découvre Bitcoin و هم‌بنیان‌گذار Plan ₿ Academy؛ پلتفرمی که با همکاری شرکای جدید و فوق‌العاده ما، Tether و Fulgur Ventures، و با مأموریت مشترک ما برای آموزش هر چه بیشتر مردم درباره Bitcoin ممکن شده است.
...
امیدوارم به زودی در دوره دیگری در Plan ₿ Academy شما را دوباره ببینم؛ چه یکی از دوره‌های من باشد یا یکی از یک معلم شگفت‌انگیز دیگر در جامعه. و چه بسا، شاید حتی در یک کنفرانس، رویداد، یا ملاقات Bitcoin به صورت حضوری ملاقات کنیم.
