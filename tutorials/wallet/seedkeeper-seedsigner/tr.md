...
- SeedSigner'ınız aracılığıyla yönetilen bir Bitcoin portföyünüz zaten varsa, yeni aygıt yazılımını yüklemeniz yeterlidir. Daha sonra mevcut Wallet'unuzu kullanmaya devam edebilir, bu kez daha fazla güvenlik için Seedkeeper'ı kullanabilirsiniz.
- Henüz SeedSigner'ınızla ilişkilendirilmiş bir Bitcoin Wallet'nız yoksa, aşağıda belirtilen eğitimin **5** ve **6** adımlarını izlemeniz gerekecektir. Bu bölümler, SeedSigner ile bir Mnemonic ifadesinin nasıl generate yapılacağını, bir *SeedQR* aracılığıyla nasıl kaydedileceğini ve ardından bu Wallet'yı yönetmek için Sparrow wallet'e nasıl bağlanacağını açıklamaktadır. Bu prosedürlere burada girmeyeceğim ve **Sparrow ve SeedSigner'ınızla yapılandırılmış, çalışan bir Bitcoin Wallet'nız olduğunu varsayıyorum**.
...
SeedSigner'ınızı bir Seedkeeper ile kullanmak için, akıllı kart okumayı desteklemek amacıyla orijinal SeedSigner'ınkinden farklı alternatif bir ürün yazılımı yüklemeniz gerekir. Bunun için "*3rdIteration*"dan Fork kullanmanızı öneririm](https://github.com/3rdIteration/seedsigner). Kullandığınız Raspberry Pi modeline karşılık gelen [görüntünün en son sürümünü](https://github.com/3rdIteration/seedsigner/releases) (`.zip`) indirin.
...
Artık Seedkeeper'ınız çalışır durumda olduğuna göre, Bitcoin Wallet Mnemonic'inizi akıllı karta kaydedebilirsiniz. Başlamak için, SeedSigner'ınızı her zamanki gibi açın, ardından Wallet'nizin *SeedQR*'sini cihaza yüklemek için tarayın. seed içe aktarıldıktan sonra, sadece `Bitti` seçeneğini seçin.
...
seed yüklendiğinde, `seed`yi Yedekle` menüsüne erişin.
...
Şimdi Seedkeeper'ınızın yeni PIN kodunu 4 ila 16 karakter arasında seçin. Daha fazla güvenlik için uzun, rastgele bir kod seçin: Mnemonic ifadenize fiziksel erişimi koruyan tek bariyer budur.



Bu PIN'i oluşturulur oluşturulmaz güvenilir bir parola yöneticisine veya stratejinize bağlı olarak ayrı bir fiziksel ortama kaydetmeyi unutmayın. İkinci durumda, PIN'i içeren ortamı asla Seedkeeper'ınızla aynı yerde tutmadığınızdan emin olun, aksi takdirde koruma etkisiz hale gelecektir. Yedek bir kopyaya sahip olmak önemlidir: **Bu PIN olmadan seed'ünüze erişemezsiniz ve bitcoinleriniz kaybolur**.
...
Daha sonra Mnemonic ifadenizle ilişkili bir `Etiket` tanımlayabilirsiniz. Bu etiket, Seedkeeper'da birden fazla sır saklıyorsanız, bunları kolayca tanımlayabilmeniz için kullanışlıdır.
...
Mnemonic ifadeniz artık akıllı karta kaydedilmiştir.
...
Güvenlik stratejisi açısından, ihtiyaçlarınıza ve riske maruz kalma düzeyinize bağlı olarak çeşitli yaklaşımlar mümkündür. Kişisel olarak, seed'nizin en az 2 kopyasını saklamanızı öneririm:
...
- Şifrelenmemiş Mnemonic ifadenizin ikinci bir kopyası, yalnızca Seedkeeper'ın kaybolması veya çalınması durumunda kullanılmak üzere portföyünüzün nihai yedeği olarak hizmet eder. Bu sürüm şifrelenmemiş olduğundan, 2 yedeğin aynı anda tehlikeye girmesini önlemek için ayrı ve daha güvenli bir yerde tutulmalıdır.



Koruma stratejinize ve risk profilinize bağlı olarak, seed'u birkaç farklı Seedkeeper'da çoğaltabilir veya Mnemonic'un birkaç fiziksel kopyasını oluşturabilirsiniz. Bu uygulamalar hakkında daha fazla bilgi edinmek için aşağıdaki eğitime göz atın:
...
Artık Seedkeeper'ınızı kullanarak Mnemonic ifadenizi başlangıçta SeedSigner'a yükleyebilir ve böylece Bitcoin işlemlerinizi imzalayabilirsiniz. Başlamak için SeedSigner'ınızı fişe takarak açın, ardından `Seeds' menüsünü açın.
...
seed'niz şimdi yüklenmiştir. Ekranda görüntülenen parmak izini Sparrow wallet ayarlarınızda belirtilenle karşılaştırarak bunun doğru Wallet olup olmadığını kontrol edin. Bu parmak izi, Wallet ilk oluşturulduğunda da sağlanmıştır.
...
Daha sonra Wallet'unuzu her zamanki gibi kullanabilirsiniz: teslimat adreslerinizi kontrol edin ve klasik bir SeedSigner'da olduğu gibi işlemleri imzalayın. Nasıl kullanılacağı hakkında daha fazla bilgi edinmek için özel eğitime bakın :
...
Bitcoin portföyünüzü korumak için bir passphrase mi kullanıyorsunuz? seed ile birlikte Seedkeeper'ınıza da kaydedebilirsiniz. Bu çözüm, her kullandığınızda passphrase'i küçük tuş takımına manuel olarak girmek zorunda kalmadan Wallet'ünüzü SeedSigner'a hızlı bir şekilde yüklemenizi sağlayacaktır.



Bu yöntemi özellikle ilginç buluyorum, çünkü passphrase'in güvenlik avantajlarından faydalanmanızı sağlarken, günlük kullanımıyla ilgili kısıtlamaları ortadan kaldırıyor. İşte size tavsiye edebileceğim bir yapılandırma örneği:




- seed ve passphrase'nızı güçlü bir PIN koduyla korunan bir Seedkeeper'da saklayın (bu önemlidir). Bu yedekleme Wallet'nizi günlük olarak kolayca kullanmanızı sağlayacaktır. Dilerseniz bu bilgileri ikinci bir Seedkeeper'da çoğaltabilirsiniz;
- Ayrıca Mnemonic ve passphrase'unuzun net bir kopyasını kağıt veya metal üzerinde saklayın. Bu, Seedkeeper'ınızı veya PIN'ini kaybetmeniz durumunda son çare olarak kullanacağınız yedeğinizdir. Bu kopyaları ayrı yerlerde sakladığınızdan emin olun, böylece aynı anda tehlikeye atılamazlar.



Bu yapılandırmada, birisi sadece Mnemonic düz metninizi ele geçirirse, passphrase'i bilmeden hiçbir şey çalamaz (tabii ki kaba kuvvet saldırısına dayanacak kadar güçlü olması şartıyla). Tersine, birisi düz metniniz passphrase'i keşfederse, ilgili Mnemonic ifadesi olmadan kullanılamaz durumda kalacaktır.



Son olarak, eğer birisi seed ve passphrase içeren Seedkeeper'ınıza fiziksel erişim sağlamayı başarırsa, PIN kodunu bilmeden hiçbir şey çıkaramaz. passphrase'ün aksine, akıllı kart 5 geçersiz denemeden sonra kendini otomatik olarak kilitlediğinden, bu kod kaba kuvvetle zorlanamaz.
...
- Bir **passphrase strong**: uzun, rastgele olmalı ve çok çeşitli karakterler içermelidir. Karmaşıklığı sizin için bir sorun değildir, çünkü başlatma sırasında klavyede yalnızca bir kez girmeniz gerekecek ve ardından Seedkeeper tarafından iletilecektir;
- Seedkeeper için bir **güçlü PIN kodu**: ayrıca rastgele ve 16 karakterden oluşur.



Bu kurulumu ayarlamak için, passphrase'nızı her zamanki gibi SeedSigner'a yükleyerek başlayın. Bu eğitimde ayrıntılı olarak açıklanan prosedürü takip edebilirsiniz:



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

passphrase'li portföyünüz SeedSigner'a doğru şekilde yüklendikten sonra, `Seeds' menüsünü açın ve bu portföye karşılık gelen ayak izini seçin. Bu ayak izinin passphrase içermeyen portföyün ayak izinden farklı olduğunu unutmayın.
...
Ardından `seed`i Yedekle`ye tıklayın, Seedkeeper`ı sürücüye yerleştirin ve `To SeedKeeper`ı seçin.
...
Akıllı kartın kilidini açmak için PIN kodunuzu girin, ardından passphrase'nize karşılık gelen Wallet'ü seçin.
...
- SeedKeeper İşlevleri` menüsünde şunları yapabilirsiniz:
 ...
 - Tanımlayıcılarınızı kaydedin veya yükleyin (Multisig portföyleri için yararlı işlev).
