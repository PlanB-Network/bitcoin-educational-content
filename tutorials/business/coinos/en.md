---
name: Coinos
description: A simple web application for sending, receiving and accepting bitcoin payments.
---
...
You will then be redirected to the main Interface portfolio. Whether on your computer or smartphone, Interface remains the same.
...
The bottom left and right corners show the current price of a Bitcoin in the chosen local currency, and the number of satoshis equivalent to a unit of account denominated in the chosen local currency.


![screen|220](assets/fr/13.webp)


Coinos, as a web wallet, is not available on the Play Store or App Store. However, there is a way to install it. From your browser, once on Coinos, first click on the **three dots** located in the top Coin of your screen.
...
Click on the **Interface button in the drop-down menu** and then on **Preferences**.
...
To receive bitcoins on Coinos, click either on **Receive**, or on the **Receive Interface** icon. Once on the receiving Interface, ask your sender **by which network he wishes to send the bitcoins**, in order to determine **which address to communicate** to him.
...
Coinos reminds you that only Liquid **bitcoins** (**L-BTC**) should be sent to this address or via this QR code.
...
By default, the Interface reception opens on the Lightning option. This gives you your QR code and Lightning address, which you can personalize with a specific amount and a memo.
...
To receive payment via **Lightning**, you can use the small drop-down menu on the main Interface wallet. This displays your Lightning address: you can then copy it or switch to its QR code for easy sharing.
...
- Enter a Bitcoin address (On-Chain, Liquid or Bolt LN), a Invoice Lightning, or your recipient's Coinos username in the field provided.
...
You can also try sending me some satoshis via my Lightning address: Raimi@coinos.io.
...
For each transaction, the following information will be displayed: the amount, the network used, and the date and time. While the balance displayed on the main Interface wallet is a unified balance.
...
Coinos wallet includes a map listing the merchants who use the platform. All the establishments listed accept Bitcoin payments, notably via their Coinos wallet. To view this map, click on the **Down menu** icon, then select **Map**. You'll then see a list of businesses and their locations around the world.
...
2. or send an e-mail directly to support@coinos.io.
