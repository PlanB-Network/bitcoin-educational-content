...
Il problema è concreto: ricevi un pagamento Lightning ma vuoi conservarlo in Cold storage, oppure hai bitcoin On-Chain ma hai bisogno di liquidità Lightning. Le soluzioni tradizionali prevedono l'apertura/chiusura manuale dei canali Lightning (costosa e tecnica) o piattaforme centralizzate che richiedono KYC (Know Your Customer).

Zeus Swap risolve questo problema con un servizio di scambio automatico e non-custodial. Sviluppato da Zeus LSP, consente di convertire bitcoin On-Chain in satoshi Lightning e viceversa, senza affidare i fondi a un intermediario. Il processo utilizza contratti atomici (HTLC) che garantiscono che lo scambio si completi o venga annullato.
...
Zeus Swap è un servizio di scambio di liquidità sviluppato da Zeus LSP che consente swap atomici tra la rete Bitcoin principale e Lightning Network. È un'infrastruttura tecnica che utilizza submarine swap e reverse swap per facilitare la conversione bidirezionale tra BTC On-Chain e satoshi Lightning, mantenendo la natura non-custodial dell'operazione.
...
Per un submarine swap (On-Chain → Lightning), l'utente invia bitcoin a un indirizzo che incorpora l'hash di una fattura Lightning. Zeus LSP sblocca questi fondi solo pagando la fattura corrispondente, rivelando la pre-image che sblocca automaticamente i bitcoin. Questo meccanismo garantisce l'atomicità.

Per un reverse swap (Lightning → On-Chain), l'utente paga una fattura Lightning di Zeus LSP, rivelando una pre-image che permette il rilascio di una transazione Bitcoin già pronta verso l'indirizzo di destinazione.
...
**Submarine swaps (On-Chain → Lightning)**: inietta liquidità Lightning dai tuoi bitcoin, utile per alimentare un wallet mobile o un nodo Lightning senza aprire canali manualmente.

**Reverse swaps (Lightning → On-Chain)**: converte satoshi Lightning in bitcoin On-Chain per conservazione a lungo termine, evitando costose chiusure dei canali.
...
**Integrazione Zeus Wallet**: swap diretti dall'applicazione, gestione automatica di fatture e indirizzi, eliminando errori di gestione.
...
**Zeus Swaps Rescue Key**: durante uno swap On-Chain → Lightning, Zeus genera automaticamente una chiave di recupero universale che sostituisce i vecchi file di rimborso individuali. Questa chiave funziona su qualsiasi dispositivo e per tutti gli swap creati con essa. Scaricala e conservala in un luogo sicuro per poter recuperare i fondi in caso di errore nello swap.

### Ottimizzazione della rete

Zeus Swap regola automaticamente i tempi di scadenza e le commissioni di mining in base alle condizioni della rete. Gli utenti Zeus possono scegliere LSP, ritardi personalizzati e compatibilità con altri servizi (Boltz).
...
Consulta il nostro tutorial Zeus per conoscere meglio questo wallet completo:
...
*Interfaccia Zeus Swap: seleziona importo (Sats 50.000 → Sats 49.648 dopo commissioni) con visualizzazione trasparente delle fee di rete (Sats 302) e del servizio Zeus (Sats 50).*

Durante il processo, Zeus ti propone di scaricare la chiave di recupero universale:

![Téléchargement de la Zeus Swaps Rescue Key](assets/fr/02.webp)

*Finestra di download della Zeus Swaps Rescue Key - chiave universale che sostituisce i vecchi file di rimborso.*
...
*Conferma di successo: 49.648 Sats ricevuti su Lightning dopo detrazione delle commissioni di rete e servizio.*

### Uso dell'app Zeus

**Lightning → On-Chain**: l'app Zeus offre un'esperienza integrata per reverse swap (Lightning → Bitcoin).
...
*Schermate di completamento: pagamento fattura Lightning con "PAY THIS Invoice" ("PAGA QUESTA Fattura"), conferma pagamento Lightning in 9,96 secondi e bilancio con 49.162 Sats in attesa di conferma.*
...
- **Non-custodial**: niente KYC (Know Your Customer), niente account, fondi sempre sotto il tuo controllo
- **Trasparenza**: commissioni visualizzate prima della conferma (0,1%-0,5% + mining, verificare le fee attuali)
...
- **Conferma necessaria**: minimo 10 minuti per validazione Bitcoin
...
- Preferisci weekend e ore non di punta per ridurre le fee di mining
- Calcola la convenienza: importi piccoli vs apertura diretta di canali

### Sicurezza

- Controlla attentamente gli indirizzi Bitcoin (consigliato copia-incolla)
- Salva la Zeus Swaps Rescue Key in un luogo sicuro
- Documenta: ID contratto, indirizzo di rimborso, data di scadenza
- Usa fee di mining adeguate per conferma tempestiva
...
- **Interfaccia unificata**: integrazione nativa in Zeus Wallet vs web tecnico Boltz
- **WebSocket API**: aggiornamenti in tempo reale vs polling manuale
- **Gestione automatica**: fatturazione e indirizzi automatici
- **Supporto mobile**: smartphone vs ottimizzazione desktop
- **Documentazione Swagger**: REST API completa per sviluppatori

**Boltz** resta vantaggioso per completa indipendenza e uso con qualsiasi setup Bitcoin/Lightning.

Zeus Swap trasforma la tecnologia provata di Boltz in un'esperienza utente mainstream, simile alla differenza tra protocollo grezzo e applicazione user-friendly.

### Zeus Swap vs Phoenix/Breez (swap integrati)

Phoenix e Breez integrano swap trasparenti che nascondono la complessità tecnica. Phoenix usa un sistema automatico swap-in/swap-out dove l'utente "manda a un indirizzo Bitcoin" e l'app gestisce lo swap in background.
...
⚠️ **Attenzione**: le fee possono variare nel tempo secondo condizioni di mercato e aggiornamenti del servizio. Controlla sempre le fee mostrate nell'interfaccia prima di confermare uno swap.

| Servizio | Submarine Swap (BTC→LN) | Reverse Swap (LN→BTC) | Importo minimo |
|---------|-------------------------|----------------------|-----------------|
| **Zeus Swap** | ~0,1% + fee minage | 0,5% + fee minage | 25.000 sats |
| **Boltz** | 0,2% + fee minage | 0,5% + fee minage | 50.000 sats |
| **Phoenix** | solo fee minage | 0,4% fissa | 10.000 sats |
| **Breez** | 0,25% + fee rete | 0,5% + fee minage | 50.000 sats |
...
L'architettura non-custodial di Zeus Swap, ereditata dalla tecnologia comprovata di Boltz, garantisce che i fondi restino sotto il tuo esclusivo controllo durante tutto il processo. Trasparenza dei prezzi e assenza di KYC (Know Your Customer) rafforzano questa proposta di valore unica.
