---
name: SeedSigner
description: Ein Hardware Wallet zum Selbermachen, zustandslos, erschwinglich und mit vollständiger Luftabdeckung
---
...
Der SeedSigner ist ein quelloffener Hardware Wallet Bitcoin, den jeder selbst mit preiswerten, universell einsetzbaren elektronischen Bauteilen bauen kann. Im Gegensatz zu kommerziellen Produkten wie dem Ledger, der Coldcard oder dem Trezor handelt es sich nicht um ein fertiges Gerät, das von einem Unternehmen hergestellt wird: Es ist ein Gemeinschaftsprojekt, bei dem jeder sein eigenes Gerät erstellen und jeden Schritt kontrollieren kann.



Der SeedSigner ist so konzipiert, dass er zu 100% ***air-gapped*** ist: er verbindet sich nie mit dem Internet, hat kein Wi-Fi oder Bluetooth (im Falle des Raspberry Pi Zero v1.3) und ist nie mit einem Computer verbunden, um Exchange Daten zu übertragen. Die Kommunikation erfolgt ausschließlich über ein Exchange-System mit QR-Code. Konkret zeigt Ihre Portfolioverwaltungssoftware (z. B. Sparrow wallet) die zu signierende Transaktion in Form von QR-Codes an; Sie scannen sie mit der Kamera des SeedSigners, dann signiert das Gerät die Transaktion mit Ihren privaten Schlüsseln, die vorübergehend in seinem RAM gespeichert sind. Schließlich erzeugt es QR-Codes mit der signierten Transaktion, die Sie mit Ihrer Software scannen, um sie an das Bitcoin-Netzwerk zu senden.
...
Eine weitere Möglichkeit, Ihr seed zu speichern und es mit dem SeedSigner zu verwenden, ist die Verwendung einer *SeedKeeper* Smartcard in Verbindung mit einem kompatiblen Lesegerät. Damit erhalten Sie einen sehr robusten *secure element* zum Speichern Ihres seed, während Sie den SeedSigner-Bildschirm zum Signieren von Transaktionen verwenden. Aber diese spezielle Konfiguration ist Gegenstand eines anderen speziellen Tutorials. Hier konzentrieren wir uns auf die grundlegende Verwendung des SeedSigners:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

Das SeedSigner-Projekt nimmt einen wichtigen Platz im Bitcoin-Ökosystem ein, da es jedem, überall auf der Welt, die Möglichkeit bietet, von fortschrittlicher Sicherheit zum Schutz seiner Bitcoins zu profitieren. Sein Hauptvorteil liegt in seiner Zugänglichkeit: die benötigte Hardware kann für weniger als 50 Dollar erworben werden. Darüber hinaus können Menschen, die in Ländern mit eingeschränktem Zugang leben, ihren eigenen Hardware Wallet aus Standard-Computerkomponenten bauen, die leicht zu finden sind und weniger behördlichen Beschränkungen unterliegen.



Aber auch außerhalb dieser speziellen Kontexte kann SeedSigner eine interessante Option für Sie sein: Es ist Open-Source, arbeitet zustandslos und airgapped und reduziert die Angriffsvektoren, die mit der Supply-Kette Ihres Hardware Wallet verbunden sind.
...
- Raspberry Pi Zero
 ...
 - Die Versionen W und v2 sind ebenfalls kompatibel, enthalten aber einen Wi-Fi/Bluetooth-Chip. Es ist daher ratsam, ihn physisch zu deaktivieren, indem man das Funkmodul aus der Karte entfernt. Der Vorgang ist relativ einfach, erfordert aber Präzision (für die Zero W genügt eine feine Zange, während für die v2 ein Hot-Stift erforderlich ist, um die Metallplatte zu entfernen, die das Modul verdeckt). Ich werde in dieser Anleitung nicht ins Detail gehen, aber Sie finden alle Anweisungen in diesem Dokument: *[Deaktivieren von WiFi/Bluetooth per Hardware](https://github.com/DesobedienteTecnologico/rpi_disable_wifi_and_bt_by_hardware)*.
 ...
 - Vergessen Sie nicht, einen Micro-USB-Strom Supply mitzuliefern.
...
- Waveshare 1,3" Bildschirm (240×240 px)** (auf Französisch)
    ...
    - Das Display enthält drei Tasten und einen Mini-Joystick, der als Interface des Benutzers dient.
...
- Gehäuse (optional, aber empfohlen)** (optional, aber empfohlen)** (optional, aber empfohlen)** (optional, aber empfohlen)** (empfohlen)
...
Sie können diese Komponenten einzeln kaufen oder sich der Einfachheit halber für fertige Pakete entscheiden, die die gesamte erforderliche Hardware enthalten. Ich persönlich habe mein Paket [von dieser französischen Website](https://bitcoinbazar.fr/) bestellt, aber Sie finden auch eine Liste von Anbietern für jede Region der Welt auf [der Hardware-Seite des SeedSigner-Projekts](https://seedsigner.com/hardware/). Wenn Sie die Komponenten lieber einzeln kaufen möchten, sind sie auf den großen E-Commerce-Plattformen oder in Fachgeschäften erhältlich.
...
PowerShell berechnet dann den Hash SHA256-Wert Ihrer Bilddatei. Vergleichen Sie diesen Hash mit dem entsprechenden Wert in `seedsigner.0.8.6.sha256.txt`.
...
Die aktuelle Firmware-Version unterstützt mehrere Softwarepakete: Sparrow, Specter, BlueWallet, Nunchuk und Keeper. In meinem Fall verwende ich **Sparrow wallet**, das ich besonders wegen seiner Einfachheit und reichen Funktionalität empfehle.
...
- gW-20 (Satoshis, oder 1/100.000.000 BTC)



Das Gerät **Sats** ist im Allgemeinen das praktischste für kleine Mengen.
...
- gW-22 network": muss nur geändert werden, wenn Sie den SeedSigner auf Testnet verwenden möchten.
...
- skripttypen": legt die Skripttypen fest, die zum Sperren Ihrer Bitcoins zugelassen sind. Sie brauchen diesen Parameter nicht zu ändern, da der Skripttyp direkt auf Sparrow gesetzt wird. Hier sind nur Skripte betroffen, die der SeedSigner manipulieren darf.
...
Schließlich können Sie im Menü "Sprache" die Sprache des Interface nach Ihren Wünschen ändern.
...
## 5. Erstellen und Speichern der seed



Der seed (oder Mnemonic-Satz) bildet die Grundlage für Ihr Bitcoin-Portfolio. Sie wird verwendet, um Ihre privaten Schlüssel und Adressen abzuleiten, und ermöglicht den Zugriff auf Ihr Kapital. SeedSigner bietet mehrere Methoden zur Generierung an, die wir in diesem Abschnitt untersuchen werden.
...
- Normalerweise wird eine 12-Wort-Phrase verwendet, um einen Wallet wiederherzustellen, wenn ein Hardware Wallet verloren geht oder gestohlen wird. Da der SeedSigner aber ein *zustandsloses* Gerät ist, registriert er niemals Ihren seed. Ihre physischen Backups sind also nicht nur Sicherungskopien, sondern **die einzige Möglichkeit, Ihren Wallet zu benutzen**. Wenn Sie diese Sicherungskopien verlieren, sind Ihre Bitcoins für immer verloren. Sichern Sie sie also sorgfältig, auf mehreren Medien und an sicheren Orten;
- Wenn Sie gerade erst anfangen, empfehle ich Ihnen dringend, dieses Tutorial zu lesen, um ein detailliertes Verständnis der Risiken zu erhalten, die mit der Verwaltung eines Mnemonic-Satzes verbunden sind:
...
Wenn Sie die Fotomethode wählen, klicken Sie auf "Neue seed" (mit dem Kamerasymbol), machen Sie ein Foto und bestätigen Sie es. Wählen Sie dann die Länge Ihres Satzes (12 oder 24 Wörter), der auf dem Bildschirm zum Speichern erscheint. Die folgenden Schritte sind identisch mit Teil 5.3.
...
In diesem Tutorial verwenden wir die Methode **Würfelwürfe**. Klicken Sie auf "Neuer seed" (mit dem Würfelsymbol).
...
Wählen Sie dann die Länge Ihres Mnemonic-Satzes. 12 Wörter bieten bereits ein ausreichendes Maß an Sicherheit, so dass ich diese Wahl empfehle.
...
Würfeln Sie und geben Sie die resultierenden Zahlen mit dem Cursor ein. Drücken Sie die mittlere Taste, um jede Eingabe zu bestätigen. Wenn Sie einen Fehler machen, können Sie zurückgehen. Verwenden Sie mehrere verschiedene Würfel, um den Einfluss unausgewogener Würfel zu verringern. Vergewissern Sie sich, dass Sie während dieses Vorgangs nicht beobachtet werden.
...
Schreiben Sie die Wörter Ihres Mnemonic-Satzes sorgfältig auf einen geeigneten physischen Träger (Papier oder Metall).
...
Wenn Ihnen ein Fehler unterläuft, informiert Sie der SeedSigner darüber, und Sie müssen von vorne beginnen. Achten Sie darauf, dass Sie Ihren Mnemonic-Satz notieren, wenn er Ihnen mitgeteilt wird. Dieser Überprüfungsschritt stellt sicher, dass Ihre Sicherung korrekt und vollständig ist. Nach der Überprüfung wird auf dem Bildschirm "Backup Verified" angezeigt.
...
Der SeedSigner ist ein Gerät ohne permanenten Speicher. Das bedeutet, dass Ihr seed niemals im Gerät gespeichert wird (anders als z. B. ein Ledger, Trezor oder eine Coldcard). Sobald Sie das Gerät ausschalten, verschwindet der seed vollständig aus seinem RAM. Wenn der SeedSigner neu startet, kehrt er in einen leeren Zustand zurück: Sie müssen ihm Ihren seed erneut geben, um Ihre Transaktionen zu signieren.



Dies bietet einen wesentlichen Schutz. Im Gegensatz zu anderen Hardware-Wallets basiert SeedSigner auf einem Raspberry Pi Zero ohne physischen Schutz, einschließlich *secure element*. Da jedoch keine sensiblen Daten gespeichert werden, würde selbst ein physisch kompromittiertes Gerät einem Angreifer nicht erlauben, Ihre privaten Schlüssel zu extrahieren oder Ihre Bitcoins auszugeben.



Andererseits bringt diese Architektur eine zusätzliche Verantwortung mit sich: Ohne ein Backup sind Ihre Gelder definitiv verloren. Deshalb empfehle ich ein **doppeltes Backup**. Sie haben bereits Ihre Wiederherstellungsphrase: Das ist Ihre langfristige Hauptsicherung, die Sie an einem sicheren Ort aufbewahren. Jetzt werden wir eine Kopie dieses Satzes in Form eines **QR-Codes** erstellen.



Jedes Mal, wenn Sie den SeedSigner verwenden, scannen Sie diesen QR-Code mit der Kamera des Geräts, damit es Ihren seed vorübergehend in seinen Speicher lädt, während Sie Ihre Transaktionen unterschreiben. Diese zweite Sicherung, die für den täglichen Gebrauch bestimmt ist, muss ebenfalls mit größter Sorgfalt aufbewahrt werden: Jeder, der im Besitz dieses QR-Codes ist, hat vollen Zugriff auf Ihre Bitcoins.


Ich empfehle Ihnen auch, Ihren QR-Code und Ihren Mnemonic-Satz an zwei verschiedenen Orten aufzubewahren, damit Sie im Schadensfall nicht alles verlieren.



Eine fortschrittlichere und sicherere Alternative ist die Verwendung des SeedSigners mit einem **SeedKeeper**, der das seed in einem secure element speichert. Mehr dazu erfahren Sie in diesem Tutorial:
...
In diesem Stadium können Sie auch ein **passphrase BIP39** hinzufügen, um die Sicherheit Ihres Wallet zu verstärken. Diese Option kann sich je nach Ihrer Backup-Strategie lohnen, birgt aber auch Risiken: Wenn Sie den passphrase verlieren, ist der Zugang zu Ihren Bitcoins dauerhaft verloren.



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

Wenn Sie mit dem passphrase-Konzept noch nicht vertraut sind, lade ich Sie ein, diesen umfassenden Leitfaden zu lesen:
...
Mit dem SeedSigner können Sie Ihren seed in einen Papier-QR-Code umwandeln, der *SeedQR* genannt wird. Diese Methode vereinfacht das Nachladen Ihres Wallet, da Sie nicht jedes Wort manuell neu eingeben müssen.



Dazu benötigen Sie einen leeren QR-Code aus Papier oder Metall, der der Länge Ihres Mnemonic-Satzes entspricht. Wenn Sie ein Komplettpaket für Ihren SeedSigner gekauft haben, sind die Vorlagen normalerweise enthalten. Wenn nicht, können Sie sie hier herunterladen und ausdrucken (oder von Hand reproduzieren):
...
Wählen Sie auf Ihrem seed-Bildschirm "seed sichern".
...
- Beginnen Sie mit einem Bleistift, damit Sie eventuelle Fehler korrigieren können, und verwenden Sie anschließend wieder einen feinen schwarzen Stift, wenn Sie fertig sind;
...
Herzlichen Glückwunsch, Ihr Bitcoin-Portfolio ist jetzt einsatzbereit! Wir werden nun die öffentlichen Komponenten in **Sparrow wallet** importieren, um sie einfach zu verwalten.
...
Legen Sie die microSD-Karte mit dem Betriebssystem ein, schalten Sie Ihren SeedSigner ein und laden Sie dann den seed, den Sie gerade mit Ihrem Backup-QR-Code erstellt haben. Wählen Sie auf dem Startbildschirm "Scannen" und scannen Sie dann Ihren SeedQR mit dem SeedSigner.
...
Überprüfen Sie, ob der Fingerabdruck auf Ihrem Hauptschlüssel mit dem Fingerabdruck auf Ihrem Wallet übereinstimmt. Wenn Sie einen passphrase verwenden, geben Sie ihn in diesem Stadium ein.
...
Vergewissern Sie sich, dass Sie die Sparrow wallet Software auf Ihrem Computer installiert haben. Wenn Sie nicht wissen, wie man sie herunterlädt, prüft und richtig installiert, lesen Sie bitte unsere vollständige Anleitung zu diesem Thema:
...
Vergeben Sie einen Namen für Ihr Portfolio und klicken Sie auf "Wallet erstellen". Der Sparrow fordert Sie dann auf, ein Kennwort festzulegen, um den lokalen Zugriff auf diesen Wallet zu sperren. Wählen Sie ein starkes Passwort: Es schützt den Zugang zu Ihren Portfoliodaten in Sparrow (öffentliche Schlüssel, Adressen, Labels und Transaktionshistorie). Dieses Kennwort wird nicht benötigt, um das Portfolio zu einem späteren Zeitpunkt wiederherzustellen: Zu diesem Zweck wird nur Ihr Mnemonic-Satz (und möglicherweise Ihr passphrase) benötigt.
...
Ihr SeedSigner und das Sparrow wallet sind jetzt sicher miteinander verbunden. Der Sparrow fungiert als kompletter Verwaltungs-Interface, während der SeedSigner das einzige Gerät bleibt, das Ihre Transaktionen signieren kann. Sie sind jetzt bereit, Bitcoins in einer vollständig abgekapselten Konfiguration zu empfangen und zu senden.
...
Ihr SeedSigner und Sparrow wallet sind nun so konfiguriert, dass sie zusammenarbeiten. In diesem letzten Abschnitt sehen wir uns an, wie Sie mit dieser Konfiguration Bitcoins empfangen und senden können.
...
#### 7.1.1 Generierung eines Empfangs Address
...
Ein neues Bitcoin Address wird angezeigt. Sie werden sehen:




- Der Text Address (beginnend mit `bc1p...`, wenn Sie wie ich P2TR verwenden),
- Der entsprechende QR-Code,
- Ein Feld "Bezeichnung" zur Verfolgung Ihrer Transaktionen.



Ich empfehle Ihnen dringend, jeden Bitcoin-Beleg auf Ihrem Wallet mit einem Etikett zu versehen. Auf diese Weise können Sie die Herkunft jedes UTXO leicht identifizieren und Ihr Datenschutzmanagement verbessern. Wenn Sie sich eingehender mit diesem wichtigen Thema befassen möchten, können Sie die entsprechende Schulung auf der Plan ₿ Academy besuchen:
...
Ihr Address ist nun in allen Sparrow-Abschnitten mit diesem Etikett verbunden.
...
Bevor Sie Ihr empfangenes Address freigeben, ist es sehr wichtig zu überprüfen, ob es zu Ihrem seed gehört. Dieser Schritt stellt sicher, dass Ihr SeedSigner in der Lage ist, mit diesem Address verbundene Transaktionen zu unterzeichnen. Er schützt auch vor möglichen Angriffen, bei denen Sparrow ein gefälschtes Address anzeigt. Denken Sie daran, dass Sparrow in einer unsicheren Umgebung (Ihrem Computer) läuft, die eine viel größere Angriffsfläche bietet als Ihr SeedSigner, der völlig isoliert ist. Aus diesem Grund sollten Sie niemals einem auf Sparrow angezeigten Empfänger-Address blindlings glauben, bevor Sie ihn nicht mit Ihrem Hardware Wallet überprüft haben.



Klicken Sie auf dem Sparrow auf den QR-Code des Address, um ihn zu vergrößern: Er wird dann als Vollbild angezeigt.
...
Wählen Sie auf Ihrem SeedSigner im Hauptmenü "Scannen". Scannen Sie den QR-Code, der auf Ihrem Computerbildschirm angezeigt wird, und wählen Sie dann den seed, der Ihrem Wallet entspricht (in meinem Fall den Fingerabdruck "d4149b27").
...
Wenn der gescannte Address mit dem von Ihrem seed abgeleiteten Address übereinstimmt, wird auf dem SeedSigner-Bildschirm die Meldung angezeigt: gW-113 verifiziert".
...
Dies bestätigt, dass das Address zu Ihrem Wallet gehört und dass Sie getrost Bitcoins von ihm empfangen können.
...
Sie können nun diese Address (in Form eines Textes oder QR-Codes) an die Person oder Abteilung übermitteln, die Ihnen Satss senden muss. Sobald die Transaktion im Netz übertragen wurde, erscheint sie auf der Registerkarte "Transaktionen" in Sparrow wallet.
...
- Fügen Sie unter "Bezahlen an" den Address des Empfängers ein oder klicken Sie auf das Kamerasymbol, um den QR-Code zu scannen;
...
- Wählen Sie schließlich den Gebührensatz auf der Grundlage der aktuellen Marktbedingungen (Schätzungen sind auf [Mempool.space] (https://Mempool.space/) verfügbar).
...
Klicken Sie auf "Details überprüfen" und überprüfen Sie sorgfältig alle Informationen direkt auf dem Bildschirm des SeedSigners. Die wichtigsten zu prüfenden Punkte sind der gesendete Betrag, der Address des Empfängers und die Höhe der angefallenen Gebühren.
...
Nach dem Signieren erzeugt der SeedSigner einen neuen QR-Code mit der signierten Transaktion, der vom Sparrow gescannt werden kann.
...
Da die Transaktion nun gültig ist, muss sie im Bitcoin-Netz verbreitet werden, damit sie einen Miner erreicht, der sie einem BLOCK hinzufügt.
...
Ihre Transaktion wurde nun an das Bitcoin-Netz gesendet. Sie können den Fortschritt der Transaktion auf der Registerkarte "Transaktionen" des Sparrow wallet verfolgen.
