---
name: Breez - POS
description: Breez ułatwia zbieranie płatności Bitcoin dla Twojej firmy.
---
...
W menu "*Punkt sprzedaży*" znajduje się Interface do pobierania płatności. Jest on podzielony na dwie części:
...
Pierwsza część to klawiatura polecenia zapłaty. Ten Interface jest przydatny do pobierania pojedynczej płatności, gdy znasz całkowite zakupy klienta lub gdy nie potrzebujesz stałego katalogu produktów w swojej firmie (np. usługi freelancerskie).
...
Aby skorzystać z Breez POS po raz pierwszy, należy uiścić opłatę w wysokości ponad 2500 satoshi (około 3 euro po dzisiejszym kursie Exchange). Kwota ta, płacona tylko przy pierwszej wypłacie gotówki, stanowi koszt utworzenia kanału płatności, dzięki któremu można komunikować się z innymi węzłami Lightning Network oraz wysyłać i odbierać satoshi.
...
Druga część to katalog produktów. Ten Interface jest idealny, gdy masz katalog produktów ze wstępnie zdefiniowanymi cenami. W tym miejscu można wstępnie skonfigurować produkty, a następnie użyć ich do faktur generate, aby poprawić identyfikowalność wpływów gotówkowych.
...
Możesz ręcznie skonfigurować każdą pozycję z tego Interface, klikając przycisk "**Plus**", a następnie definiując nazwę, cenę i identyfikator dla tej pozycji.
...
W tej samej sekcji można zdefiniować okres ważności faktur Lightning. Od tej chwili klienci mają `N` sekund na dokonanie płatności za wszystkie faktury, w przeciwnym razie konieczne będzie ponowne utworzenie nowej faktury Lightning Invoice.
