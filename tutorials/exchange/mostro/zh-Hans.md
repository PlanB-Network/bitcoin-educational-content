---
name: 莫斯特罗
description: 通过 Lightning 和 Nostr 实现免 KYC 的 Bitcoin P2P Exchange 平台
---
...
**Mostro P2P**提供了一种结合Lightning Network、Nostr协议和持有发票的去中心化替代方案。它的主要创新点是：自动托管系统，在整个 Exchange 期间，您的比特币始终处于您的控制之下，消除了盗窃、Exchange 破产或任意没收的风险。
...
Mostro P2P 是一种开源的 Bitcoin Exchange 协议，由 2021 年推出的流行 Telegram 机器人**lnp2pbot**的开发者**grunch**创建。虽然lnp2pbot已经通过Lightning实现了无KYC的P2P交换，但它存在一个重大漏洞： **Telegram是一个集中式故障点**，容易受到政府的审查。



Mostro代表了这一概念的**去中心化演进**：通过用**Nostr**协议（一个不可感知的分布式中继网络）取代Telegram，Mostro消除了这种系统性风险。该协议结合了用于即时交易的 Lightning Network、用于抗审查通信的 Nostr 和**持有发票**，从而创建了一个真正的非托管自动托管系统。
...
**分散仲裁**：发生争议时，公正的仲裁员会审查证据，并根据透明的标准做出裁决。每个争议都会生成一个唯一的 token，以便追溯。



**支付灵活**：通过 yadio.io Exchange 汇率服务支持多种法定货币。接受 SEPA 转账、PayPal、Revolut、现金以及双方商定的任何方式。
...
**Mostro CLI** 是最成熟、功能最强大的客户端。它以 Rust 编写，通过 Interface 命令行提供 Mostro 的全部功能。兼容 macOS、Linux 和 Windows。
...
- 可选但建议**：VPN 或 Tor 连接，以掩盖您的 IP Address
...
**对 CLI/Mobile** 同步很重要：要在 CLI 和移动应用程序上访问相同的订单，您必须在两个客户端中使用**相同的 Mostro Pubkey** 和**相同的 Nostr 中继器**。请在移动应用程序的 "设置 "中检查这些设置。
...
**步骤 2：下载完成后，从下载区打开 APK 文件。安卓系统会要求您授权从该来源进行安装。
...
- 小额**：起步资金少于 10,000 古瓦-44
- 多个中继**：配置 3-5 个不同地理位置的中继器
- 网络保护**：激活 VPN 或 Tor，隐藏您的 IP Address
- Wallet 安全**：激活 Wallet Lightning 上的自动锁
...
Mostro 会要求您提供**闪电 Invoice** 以接收 BTC。您将看到以卫星为单位的确切金额（此处：4715 Sats）。
...
generate 从您的 Wallet（Phoenix、Breez 等）中获取一个 Lightning Invoice 的准确金额，然后将其 ：
...
系统确认发货后，会通知您等待卖家支付 Invoice 扣留款，然后再启动托管。
...
一旦卖方确认收到法币并使用 "释放 "命令解除托管，您就会立即通过您提供的闪电 Invoice 收到比特币。
...
**对 CLI/Mobile 同步**重要：如果您同时使用 CLI 和移动应用程序，请在两个客户端配置**完全相同的 Nostr 中继和 Mostro Pubkey**。如果没有完全相同的配置，您将无法在两个界面上看到相同的订单。在 "设置"（上面的截图）中可以看到的中继器和 Mostro Pubkey 必须与 CLI `.env'文件中的一致。
...
您的订单现已发布！该订单的有效期为 24 小时。您可以随时执行 `cancel` 命令，在买家接受之前取消订单。
...
**重要提示**：您现在必须**支付显示的 Invoice hold**，将您的比特币（此处 4674 Sats 为 1-5 欧元）锁定在托管中。您最多有**15分钟的时间**，否则 Exchange 将被取消。



复制 Invoice 扣留，并从 Wallet Lightning（Phoenix、Breez 等）中支付。
...
在您的账户上**确认收到**法币付款后，按 Green **RELEASE**按钮并确认将 Satss 发送给买家。
...
4.提供您的 Lightning Invoice（从您的 Wallet 生成）。
...
9.通过 Lightning Invoice 即时接收比特币
...
**金融主权**：由于 Invoice 的持有机制，您的比特币永远不会离开您的直接控制，消除了 Exchange 破产或盗版的风险。
...
**付款灵活**：可采用任何双方都接受的付款方式（转账、移动应用程序、现金、易货）。
...
Mostro P2P 将 Lightning Network、Nostr 和自动托管结合起来，实现了真正的去中心化交易，是中心化交易所的一个有前途的替代选择。尽管开发尚早，流动性有限，但该平台已经提供了金融主权、抗审查和默认匿名性。
