...
description: Manage your multi-signature Bitcoin portfolios in total sovereignty with your own node
---
...
- Creating a simple wallet with a Hardware Wallet (Coldcard)
...
All your transactions will be validated locally via your own infrastructure, without transmitting any information to external servers, guaranteeing your confidentiality and financial sovereignty. Always check transactions on your Hardware Wallet screen before signing.
...
In the left-hand menu, click on "Add device" to add your Hardware Wallet.
...
Select your Hardware Wallet. In this example, we're using a Coldcard MK4.


Please find below our tutorial for this Hardware Wallet :


https://planb.network/tutorials/wallet/hardware/coldcard-5d44dd94-423d-4e37-9a8c-3fc38b45ce59

For a Coldcard, you need to export the public keys from the Hardware Wallet either via a USB connection or a microSD card.


![Import des clés du Coldcard](assets/fr/07.webp)


Follow the instructions displayed to export the keys from your Coldcard. Give your Hardware Wallet a name (here "MK4 Tuto"). Once the keys have been imported, you can create a wallet with a single key, or add other hardware wallets for a multi-signature wallet.
...
After adding your Hardware Wallet, click on "Create single key Wallet" to create a single-signature wallet.


Give your portfolio a name (e.g. "Wallet for tuto") and select the address type. Select "SegWit" to use native BECH32 addresses, which optimize transaction costs.


![Configuration du portefeuille](assets/fr/09.webp)


Once your portfolio has been created, Specter offers to save a backup PDF file containing all the public information needed to restore your portfolio (descriptors, extended public keys). This file does not contain your private keys.
...
You can copy the address or scan the QR code. Always check the address on your Hardware Wallet screen before passing it on to anyone.
...
The "Addresses" tab lets you view all the addresses generated by your portfolio, with their usage status and associated amounts.
...
Click on "Create Unsigned Transaction" to build the transaction. Specter will then ask you to sign the transaction with your Hardware Wallet.


![Signature de la transaction](assets/fr/15.webp)


If you're using a Coldcard, you'll have the choice of signing via USB or using the microSD card (air-gapped). Confirm the transaction on your Hardware Wallet screen, carefully checking the destination address and the amount.
...
## Creating and using a multi-signature portfolio


One of Specter Desktop's major assets is its ability to simplify the management of multi-signature portfolios. A Multisig wallet requires multiple signatures to authorize a transaction, eliminating the single point of failure. A 2-on-3 configuration, for example, requires two signatures from three separate hardware wallets to validate any expenditure.


To create a Multisig wallet, start by adding all signatory hardware wallets via "Add device". In this example, we'll be using three different hardware wallets: a Coldcard MK4 (already added earlier), a Passport and a Ledger. This diversification of manufacturers strengthens security by avoiding dependence on a single supply chain or firmware.
...
Add the Passport by naming the Hardware Wallet (e.g. "Passport multi") and importing its keys via microSD card or QR code. Then click on "Continue" to continue.


![Ajout du Passport](assets/fr/23.webp)


Then add the Ledger by connecting it via USB and opening the Bitcoin application on the Hardware Wallet. Name it (e.g. "Ledger multi") and click on "Get via USB" then "Continue" to import its public keys.
...
Choose your multi-signature configuration. Select "SegWit" as the address type to benefit from optimized charges. The "Required Signatures to Authorize Transactions (m of 3)" parameter lets you define the threshold: for a 2-on-3 configuration, 2 signatures are required. Each Hardware Wallet displays its corresponding Multisig key. Click on "Create Wallet" to finalize creation.


![Configuration 2-sur-3 Segwit](assets/fr/27.webp)


Your "Multi tuto" multisignature portfolio is now created. Specter immediately recommends that you save the backup PDF file containing the portfolio descriptor. Click on "Save Backup PDF" to download this critical file.


![Wallet multisig créé](assets/fr/28.webp)


Specter also lets you export Wallet information to each of your hardware wallets via QR code or file. This enables certain hardware wallets (such as Coldcard or Passport) to store the Multisig configuration directly in their memory.


For the Passport, unlock your device then go to "Manage Account" > "Connect Wallet" > "Specter" > "Multisig" > "QR Code", then scan the QR code generated by Specter. Your Passport will then ask you to scan a receiving address from your wallet to validate the Multisig configuration.


For the MK4, plug it into your PC and unlock it. Then click on "Save MK4 Tuto file" and save the file to your MK4. The next time you sign your Hardware Wallet, the MK4 will use this file to finish configuring the Multisig.


![Export vers les hardware wallets](assets/fr/29.webp)


For your information, you can access backups at any time from the "Settings" tab of your portfolio, then "Export":
...
Specter builds a PSBT (Partially Signed Bitcoin Transaction) and displays "Acquired 0 of 2 signatures". You must now sign with at least two of your three hardware wallets. Click on the first Hardware Wallet (e.g. "MK4 Tuto") to sign with your Coldcard, then on the second (e.g. "Passport multi") to obtain the second signature required.
