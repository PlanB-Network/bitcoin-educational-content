...
SeedSigner är en Hardware Wallet som du själv monterar ihop av standardhårdvara, vanligtvis runt en Raspberry Pi Zero. Denna Wallet kallas "*stateless*": till skillnad från de flesta andra modeller på marknaden (Coldcard, Trezor, Ledger, etc.) lagrar den inte någon data i permanent minne och fungerar endast live från RAM. Som ett resultat lagras din portföljs seed aldrig på SeedSigner. Varje gång du startar om måste du fylla i den för att göra det möjligt för enheten att signera dina transaktioner. Den vanligaste metoden är att spara din seed som en QR-kod, som du sedan skannar varje gång du använder den (*SeedQR*).
...
- Om du ännu inte har en Bitcoin Wallet kopplad till din SeedSigner måste du följa steg **5** och **6** i handledningen som nämns nedan. Dessa avsnitt förklarar hur man generate en Mnemonic-fras med SeedSigner, sparar den via en *SeedQR* och sedan ansluter denna Wallet till Sparrow wallet för att hantera den. Jag kommer inte att gå in på dessa procedurer här och ** Jag antar att du redan har en fungerande Bitcoin Wallet, konfigurerad med Sparrow och din SeedSigner**.
...
Nu när din Seedkeeper är i drift kan du spara din Bitcoin Wallet Mnemonic på smartkortet. Börja med att slå på din SeedSigner som vanligt och skanna sedan din Wallet:s *SeedQR* för att ladda den i enheten. När seed har importerats väljer du helt enkelt `Done`.
...
Välj nu din Seedkeepers nya PIN-kod, mellan 4 och 16 tecken. För extra säkerhet, välj en lång, slumpmässig kod: det är den enda barriären som skyddar fysisk åtkomst till din Mnemonic-fras.
...
Du kan sedan definiera en "etikett" som är associerad med din Mnemonic-fras. Denna etikett är användbar om du lagrar flera hemligheter på Seedkeeper, så att du enkelt kan identifiera dem.
...
Din Mnemonic-fras är nu sparad på smartkortet.
...
- En andra kopia av din okrypterade Mnemonic-fras, som fungerar som den ultimata säkerhetskopian av din portfölj, som endast ska användas i händelse av förlust eller stöld av Seedkeeper. Eftersom den här versionen är okrypterad måste den förvaras på en separat, säkrare plats för att förhindra att de två säkerhetskopiorna äventyras samtidigt.



Beroende på din skyddsstrategi och riskprofil kan du också duplicera seed på flera olika Seedkeepers eller skapa flera fysiska kopior av Mnemonic. För att lära dig mer om dessa metoder, ta en titt på följande handledning:
...
Du kan nu använda din Seedkeeper för att ladda din Mnemonic-fras i SeedSigner vid uppstart och därmed signera dina Bitcoin-transaktioner. För att börja, slå på din SeedSigner genom att koppla in den och öppna sedan menyn `Seeds`.
...
Använder du en passphrase för att skydda din Bitcoin-portfölj? Du kan också registrera den i din Seedkeeper, tillsammans med din seed. Denna lösning gör att du snabbt kan ladda din Wallet på SeedSigner, utan att behöva ange passphrase manuellt på den lilla knappsatsen varje gång du använder den.
...
- Förvara din seed och passphrase i en Seedkeeper, skyddad med en stark PIN-kod (detta är viktigt). Denna säkerhetskopia gör att du enkelt kan använda din Wallet dagligen. Om du vill kan du duplicera denna information på en andra Seedkeeper;
- Förvara också en tydlig kopia av din Mnemonic och passphrase, på papper eller metall. Detta är din sista utväg om du förlorar din Seedkeeper eller dess PIN-kod. Se till att förvara dessa kopior på separata platser, så att de inte kan äventyras samtidigt.



I den här konfigurationen, om någon får tag på din klartext Mnemonic ensam, kommer de inte att kunna stjäla någonting utan att känna till passphrase (förutsatt, naturligtvis, att den är tillräckligt stark för att motstå en brute-force-attack). Omvänt, om någon upptäcker din klartext passphrase, kommer den att förbli oanvändbar utan motsvarande Mnemonic-fras.
...
- En **passphrase strong**: den måste vara lång, slumpmässig och innehålla en mängd olika tecken. Dess komplexitet är inget problem för dig, eftersom du bara behöver ange det en gång på tangentbordet under initialiseringen, varefter det kommer att överföras av Seedkeeper;
...
När din portfölj med passphrase har laddats korrekt på SeedSigner öppnar du menyn `Seeds` och väljer det fotavtryck som motsvarar denna portfölj. Observera att detta fotavtryck skiljer sig från det för portföljen utan passphrase.
...
Klicka sedan på `Backup seed`, sätt in Seedkeeper i enheten och välj `To SeedKeeper`.
...
Ange din PIN-kod för att låsa upp Seedkeeper och tilldela sedan en etikett till denna hemlighet. Du kan låta fingeravtrycket vara etiketten för att upprätthålla någon form av trovärdig förnekelse, eller till exempel uttryckligen ange `passphrase Wallet`.
...
Kontrollera avtrycket från passphrase och din Wallet och bekräfta sedan.
...
- I menyn `SeedKeeper-funktioner` kan du :
 ...
 - Spara eller ladda dina deskriptorer (användbar funktion för Multisig-portföljer).
