...
- Přejděte na [GitHub repozitář PlanB věnovaný datům](https://github.com/PlanB-Network/bitcoin-educational-content/tree/dev/resources/builders) v sekci `resources/project/`:
...
- V této složce vytvoříte první YAML soubor pojmenovaný `project.yml`:
...
- Vraťte se do složky `resources/projects/` a vyberte složku vašeho podnikání, kterou jste právě vytvořili v předchozím commitu:
...
- Nechte název commitu jako výchozí a ujistěte se, že je zaškrtnuto políčko `Commit directly to the patch-1 branch`, poté klikněte na `Commit changes`: ![project](assets/23.webp)
...
![project](assets/30.webp)- Jakmile budete na souboru, klikněte na tři malé tečky v pravém horním rohu a poté na `Delete file` (Smazat soubor):
...
- Vytvořte nový YAML soubor pojmenováním ho indikátorem vašeho mateřského jazyka. Tento soubor bude použit pro popis projectu. Například, pokud chci napsat můj popis v angličtině, pojmenuji tento soubor `en.yml`:
