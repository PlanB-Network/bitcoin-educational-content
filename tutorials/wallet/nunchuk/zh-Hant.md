...
在這些步驟的最後，您會被詢問是否要在您的 Mnemonic 通訊短語中加入 [passphrase](https://planb.academy/en/resources/glossary/passphrase-bip39)。如果您對如何使用 passphrase、安排它的備份或它的運作方式沒有必要的認知，我建議您選擇 _我不需要密碼_。
...
- 右上角的鏡頭圖示可讓您進行交易搜尋；
- `View Wallet config` 允許存取設定功能表，您可以在此編輯 Wallet 的名稱，並啟用右上方的進階選項（您無法取得其螢幕截圖）。在這裡您可以匯出 Wallet 設定、標籤、取代按鍵、變更 [gap limit](https://planb.academy/en/resources/glossary/gap-limit) 等。
