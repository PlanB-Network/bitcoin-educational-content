...
SeedSigner adalah Hardware Wallet yang Anda rakit sendiri dari perangkat keras standar, biasanya menggunakan Raspberry Pi Zero. Wallet ini disebut "*stateless*": tidak seperti kebanyakan model lain yang ada di pasaran (Coldcard, Trezor, Ledger, dll.), Wallet ini tidak menyimpan data apa pun di memori permanen, dan hanya beroperasi langsung dari RAM. Akibatnya, seed portofolio Anda tidak pernah disimpan di SeedSigner. Setiap kali Anda memulai ulang, Anda harus mengisinya agar perangkat dapat menandatangani transaksi Anda. Metode yang paling umum adalah dengan menyimpan seed Anda sebagai kode QR, yang kemudian Anda pindai setiap kali Anda menggunakannya (*SeedQR*).
...
Untuk mengatasi kelemahan ini, SeedSigner dapat digabungkan dengan [**Seedkeeper**] (https://satochip.io/product/seedkeeper/), sebuah kartu pintar yang dikembangkan oleh Satochip. Hal ini memungkinkan frasa Mnemonic (atau rahasia lainnya) untuk disimpan di dalam secure element yang dilindungi oleh kode PIN. Applet Seedkeeper merupakan sumber terbuka, dan secure element-nya memiliki sertifikasi EAL6+. Digunakan bersama dengan SeedSigner, aplikasi ini menawarkan fitur keamanan yang sangat menarik: kunci Anda tetap dikelola sepenuhnya secara off-line, Anda menandatangani transaksi Anda di layar yang terpercaya, dan seed secara fisik terlindungi dalam smartcard yang tahan terhadap serangan fisik.
...
- Jika Anda belum memiliki Bitcoin Wallet yang terhubung dengan SeedSigner Anda, Anda harus mengikuti langkah **5** dan **6** pada tutorial di bawah ini. Bagian ini menjelaskan bagaimana cara membuat generate frasa Mnemonic dengan SeedSigner, menyimpannya melalui *SeedQR*, dan kemudian menghubungkan Wallet ini ke Sparrow wallet untuk mengelolanya. Saya tidak akan membahas prosedur-prosedur ini di sini dan **Saya mengasumsikan bahwa Anda telah memiliki Bitcoin Wallet yang berfungsi, yang dikonfigurasi dengan Sparrow dan SeedSigner Anda**.
...
Sekarang setelah Seedkeeper Anda beroperasi, Anda bisa menyimpan Bitcoin Wallet Mnemonic Anda di smartcard. Untuk memulai, nyalakan SeedSigner Anda seperti biasa, lalu pindai *SeedQR* Wallet Anda untuk memuatnya ke dalam perangkat. Setelah seed diimpor, cukup pilih `Selesai`.
...
Sekarang pilih kode PIN baru Seedkeeper Anda, antara 4 hingga 16 karakter. Untuk keamanan tambahan, pilihlah kode yang panjang dan acak: kode ini adalah satu-satunya penghalang yang melindungi akses fisik ke frasa Mnemonic Anda.



Ingatlah untuk menyimpan PIN ini segera setelah dibuat, baik di dalam sebuah pengelola kata sandi yang handal, atau di sebuah media fisik yang terpisah, tergantung pada strategi Anda. Pada kasus terakhir, pastikan untuk tidak menyimpan media yang berisi PIN di tempat yang sama dengan Seedkeeper Anda, jika tidak, proteksi tidak akan efektif. Penting untuk memiliki salinan cadangan: **Tanpa PIN ini, Anda tidak akan bisa mengakses seed Anda, dan bitcoin Anda akan hilang**.
...
Anda kemudian dapat mendefinisikan sebuah `Label` yang terkait dengan frase Mnemonic Anda. Label ini berguna jika Anda menyimpan beberapa rahasia di Seedkeeper, sehingga Anda dapat dengan mudah mengidentifikasinya.
...
Frasa Mnemonic Anda sekarang tersimpan pada kartu pintar.
...
- Salinan kedua dari frasa Mnemonic yang tidak terenkripsi, yang berfungsi sebagai cadangan utama portofolio Anda, untuk digunakan hanya jika terjadi kehilangan atau pencurian terhadap Penyimpanan Benih. Karena versi ini tidak terenkripsi, versi ini harus disimpan di lokasi yang terpisah dan lebih aman, untuk mencegah kompromi simultan dari 2 cadangan.



Tergantung pada strategi perlindungan dan profil risiko Anda, Anda juga bisa menduplikasi seed di beberapa Seedkeeper yang berbeda, atau membuat beberapa salinan fisik Mnemonic. Untuk mempelajari lebih lanjut tentang praktik-praktik ini, lihat tutorial berikut:
...
Anda sekarang dapat menggunakan Seedkeeper Anda untuk memuat frasa Mnemonic Anda ke dalam SeedSigner pada saat startup, dan dengan demikian menandatangani transaksi Bitcoin Anda. Untuk memulai, nyalakan SeedSigner Anda dengan mencolokkannya, kemudian buka menu `Seeds`.
...
seed Anda sekarang telah dimuat. Periksa apakah ini adalah Wallet yang benar dengan membandingkan sidik jari yang ditampilkan di layar dengan sidik jari yang ditentukan dalam pengaturan Sparrow wallet Anda. Sidik jari ini juga disediakan ketika Wallet pertama kali dibuat.
...
- Simpan seed dan passphrase Anda di Seedkeeper, yang dilindungi oleh kode PIN yang kuat (ini penting). Cadangan ini akan memungkinkan Anda untuk dengan mudah menggunakan Wallet Anda setiap hari. Jika Anda mau, Anda bisa menduplikasi informasi ini pada Seedkeeper kedua;
- Simpan juga salinan jelas Mnemonic dan passphrase Anda, di atas kertas atau logam. Ini adalah cadangan terakhir Anda jika Anda kehilangan Seedkeeper atau PIN-nya. Pastikan untuk menyimpan salinan ini di lokasi yang terpisah, sehingga tidak dapat dikompromikan secara bersamaan.



Dalam konfigurasi ini, jika seseorang mendapatkan plaintext Mnemonic Anda saja, mereka tidak akan dapat mencuri apapun tanpa mengetahui passphrase (dengan catatan, tentu saja, cukup kuat untuk menahan serangan brute-force). Sebaliknya, jika seseorang menemukan plaintext passphrase Anda, ia tidak akan dapat digunakan tanpa frasa Mnemonic yang sesuai.
...
- Sebuah **passphrase yang kuat**: harus panjang, acak dan berisi berbagai macam karakter. Kerumitannya tidak menjadi masalah bagi Anda, karena Anda hanya perlu memasukkannya sekali pada keyboard selama inisialisasi, setelah itu akan dikirimkan oleh Seedkeeper;
...
Masukkan PIN Anda untuk membuka kunci Seedkeeper, lalu tetapkan label pada rahasia ini. Anda dapat membiarkan sidik jari sebagai label untuk mempertahankan suatu bentuk penyangkalan yang masuk akal, atau secara eksplisit menyatakan `passphrase Wallet`, misalnya.
...
Masukkan kode PIN Anda untuk membuka kunci smartcard, lalu pilih Wallet yang sesuai dengan passphrase Anda.
...
- Dalam menu `SeedKeeper Functions`, Anda dapat :
 ...
 - Simpan atau muat deskriptor Anda (fungsi yang berguna untuk portofolio Multisig).
