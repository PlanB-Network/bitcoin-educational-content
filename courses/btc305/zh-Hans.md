...
BTCPay服务器配备了一套标准的插件（应用程序），可以将BTCPay服务器转变为电子商务支付网关。通过添加销售点、众筹平台和渐简便的支付按钮，BTCPay服务器已成为一种易于部署的解决方案。
...
默认情况下，BTCPay Server的支付按钮显示为“用BTCPay支付”。店主可以根据自己的意愿设置这段文本，并将BTCPay Server的logo更换为自己的。通过使用“支付按钮文本”设置文本，并在“支付按钮图片网址”下粘贴图片网址。
...
服务器IPN（即时支付通知）用于webhooks，可以通过网址填写以发布购买数据。
