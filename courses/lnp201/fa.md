...
به دوره LNP201 خوش آمدید!

هدف این آموزش، ارائه درک فنی عمیق از شبکه لایتنینگ (Lightning Network) است؛ شبکه‌ای در لایه دوم که برای انجام تراکنش‌های بیت‌کوین با تسویه سریع‌تر و معمولاً با هزینه کمتر طراحی شده است. شما به تدریج با مفاهیم بنیادی این سیستم آشنا خواهید شد — از باز کردن کانال‌های پرداخت گرفته تا تکنیک‌های مسیریابی و مدیریت نقدینگی.

**بخش ۱: مبانی**
در این بخش با معرفی کلی شبکه لایتنینگ آغاز می‌کنیم و مبانی ضروری بیت‌کوین، آدرس‌ها، UTXOها و نحوه عملکرد تراکنش‌ها را مرور خواهیم کرد. این مرور برای درک نحوه تکیه لایتنینگ بر سازوکارهای بلاک‌چین اصلی جهت حفظ امنیت، ضروری است.

**بخش ۲: باز و بسته کردن کانال‌ها**
در این بخش فرآیند باز کردن کانال‌ها را بررسی خواهیم کرد — سنگ‌بنای شبکه لایتنینگ. خواهید آموخت که تراکنش‌های تعهد چگونه ایجاد می‌شوند، نقش کلیدهای ابطال (Revocation Keys) در امنیت چیست و چگونه می‌توان کانال‌ها را به‌صورت تعاملی یا یک‌طرفه بست. هر مرحله با جزئیات فنی توضیح داده می‌شود تا درک کاملی از آن‌ها به دست آورید.

**بخش ۳: شبکه‌ای از نقدینگی**
شبکه لایتنینگ تنها مجموعه‌ای از کانال‌ها نیست، بلکه یک شبکه پرداخت واقعی است. در این بخش یاد می‌گیرید چگونه تراکنش‌ها از طریق گره‌های واسطه و با استفاده از HTLCها مسیریابی می‌شوند. همچنین با مفاهیم نقدینگی ورودی و خروجی آشنا خواهید شد.

**بخش ۴: ابزارهای شبکه لایتنینگ**
در این بخش ابزارهای کاربردی شبکه لایتنینگ معرفی می‌شوند؛ از جمله *Invoice*، *LNURL* و *Keysend*. همچنین می‌آموزید چگونه نقدینگی کانال‌های خود را مدیریت کنید — عاملی کلیدی برای تضمین روان بودن پرداخت‌ها و افزایش کارایی تراکنش‌های شما در لایتنینگ.

**بخش ۵: گام بعدی**
در پایان، مروری بر مفاهیم آموخته‌شده خواهیم داشت و مسیر یادگیری را برای افرادی که مایل به مطالعه عمیق‌تر شبکه لایتنینگ هستند، هموار می‌کنیم.

آماده‌اید تا با جزئیات فنی شبکه لایتنینگ آشنا شوید؟ پس شروع کنیم!

---

*در اینجا چند اصطلاح وجود دارد که در نمودارهای دوره به زبان انگلیسی با آنها روبرو خواهید شد، همراه با ترجمه آن‌ها تا به شما کمک کند آن‌ها را بهتر به زبان خود درک کنید:*

| انگلیسی            | ترجمه - توضیح                 |
| ------------------ | ----------------------------- |
| *timelock*         | قفل زمانی                     |
| *Revocation Key*   | کلید لغو                      |
| *invoice*          | فاکتور / درخواست پرداخت       |
| *sig* (signature)  | امضا                          |
| *secret*           | راز                           |
| *amount*           | مبلغ                          |
| *scan QR code*     | اسکن کد QR                    |
| *Show QR code*     | نمایش کد QR                   |
| *Asks the invoice* | درخواست فاکتور                |
| *Give the invoice* | ارائه فاکتور                  |
| *Payment*          | پرداخت                        |
| *Preimage*         | پیش‌تصویر                     |
...
به دوره LNP201 خوش آمدید، که هدف آن توضیح عملکرد فنی [شبکه Lightning](https://planb.academy/resources/glossary/lightning-network) است.

شبکه Lightning یک شبکه از [کانال‌های پرداخت](https://planb.academy/resources/glossary/payment-channel) است که بر روی پروتکل بیت کوین، با هدف فراهم کردن تراکنش‌های سریع و کم هزینه ساخته شده است. این شبکه این امکان را می‌دهد تا کانال‌های پرداخت بین شرکت‌کنندگان ایجاد شود،به صورتی که در آن تراکنش‌ها تقریباً به طور فوری و با هزینه‌های کمی، بدون نیاز به ثبت هر تراکنش به طور جداگانه بر روی [بلاکچین](https://planb.academy/resources/glossary/blockchain) انجام می‌شود. بنابراین، شبکه Lightning سعی دارد تا قابلیت [مقیاس‌پذیری](https://planb.academy/resources/glossary/scalability) بیت کوین را بهبود بخشد و آن را برای پرداخت‌هایی با ارزش پایین قابل استفاده کند.
...
در مثال ما، آلیس 100،000 [ساتوشی](https://planb.academy/resources/glossary/satoshi-sat) در سمت خود از کانال دارد و باب 30،000 ساتوشی، در مجموع 130،000 ساتوشی، که **[ظرفیت کانال](https://planb.academy/resources/glossary/lightning-channel-capacity)** را تشکیل می‌دهد.
...
راه دیگری برای تصور تخصیص وجوه این است که یک **نشانگر** را تصور کنید که نشان می‌دهد وجوه در کانال کجا قرار دارند. در ابتدا، با ۱۰۰٬۰۰۰ ساتوشی برای آلیس و ۳۰٬۰۰۰ برای باب، نشانگر بیشتر در سمت باب است، زیرا آلیس سرمایه بسیار بیشتری دارد. پس از تراکنش ۴۰٬۰۰۰ ساتوشی، نشانگر کمی به سمت آلیس حرکت می‌کند که اکنون ۶۰٬۰۰۰ ساتوشی دارد.
...
آدرس بیت کوین مجموعه ای از کاراکترها است که از یک **[کلید عمومی](https://planb.academy/resources/glossary/public-key)** مشتق شده است، که خود از یک **[کلید خصوصی](https://planb.academy/resources/glossary/private-key)** محاسبه می شود. همانطور که شما قطعاً می دانید، کلید خصوصی برای قفل کردن بیت کوین ها استفاده می شود، که معادل دریافت آنها در کیف پول ما است.
...
- **ارتباطات [نقطه به نقطه](https://planb.academy/resources/glossary/peertopeer-p2p) (Peer-to-peer) (پروتکل Lightning)**: این پیام‌هایی هستند که [گره‌های Lightning](https://planb.academy/resources/glossary/lightning-node) برای ارتباط با یکدیگر ارسال می‌کنند. ما این پیام‌ها را با خطوط مشکی خط خطی در نمودارهایمان نشان خواهیم داد.
...
- **معاملات بیت کوین (پروتکل بیت کوین)**: این معاملاتی هستند که [در زنجیره](https://planb.academy/resources/glossary/onchain) انجام می‌شوند، که ما آنها را با خطوط نارنجی نشان خواهیم داد.
...
کانال زمانی باز به حساب می‌آید که معامله سپرده در یک [بلوک](https://planb.academy/resources/glossary/block) بیت کوین قرار گرفته و به عمق معینی از [تاییدیه‌ها](https://planb.academy/resources/glossary/confirmation) (تعداد بلوک‌های بعدی) رسیده باشد.
...
وقتی آلیس یک معامله در کانال برای ارسال وجوه به باب ایجاد می کند، یک معامله بیت کوین جدید برای بازتاب این تغییر در توزیع وجوه ایجاد می شود. این معامله، که **[معامله تعهد](https://planb.academy/resources/glossary/commitment-transaction)** نامیده می شود، در بلاکچین منتشر نمی شود اما نمایانگر وضعیت جدید کانال پس از معامله Lightning است.
...
به لطف این دو مکانیزم ترکیبی، باب فرصت می‌یابد تلاش آلیس برای تقلب را تشخیص دهد و با بازیابی [خروجی](https://planb.academy/resources/glossary/output) خود با کلید ابطال، که برای باب به معنای بازیابی تمام وجوه کانال است، او را مجازات کند. معامله تعهد جدید ما حالا به این شکل خواهد بود:
...
3. آلیس و باب [هزینه های](https://planb.academy/resources/glossary/transaction-fees) **معامله بسته شدن** را با هم مذاکره می کنند. این هزینه ها معمولا بر اساس بازار هزینه بیت کوین در زمان بسته شدن محاسبه می شوند. مهم است توجه داشته باشید که **همیشه فردی که کانال را باز کرده است** (در مثال ما آلیس) هزینه های بسته شدن را می پردازد.
...
در این فصل، ما بررسی خواهیم کرد که چگونه پرداخت ها در شبکه Lightning می توانند به گیرنده برسند حتی اگر آنها مستقیماً توسط یک کانال پرداخت متصل نشوند. Lightning در واقع یک **شبکه از کانال های پرداخت** است، که اجازه می دهد تا وجوه از طریق کانال های سایر شرکت کنندگان به یک گره دور دست ارسال شود. ما کشف خواهیم کرد که چگونه پرداخت ها در سراسر شبکه مسیریابی می شوند، چگونه [سیالیت](https://planb.academy/resources/glossary/liquidity-lightning) بین کانال ها حرکت می کند و چگونه هزینه های معامله محاسبه می شوند.

### شبکه کانال‌های پرداخت

در شبکه Lightning، یک تراکنش متناظر با انتقال وجوه بین دو نود است. همانطور که در فصل های قبلی دیده شد، برای انجام تراکنش های Lightning، لازم است که یک کانال با شخصی باز کنید. این کانال امکان انجام تعداد تقریبا بی نهایتی از تراکنش های [off-chain](https://planb.academy/resources/glossary/offchain) را قبل از بستن آن برای بازیابی موجودی on-chain فراهم می کند. با این حال، این روش این معایب را دارد که نیاز به یک کانال مستقیم با شخص دیگری برای دریافت یا ارسال وجوه دارد، که این بدان معناست که یک تراکنش افتتاحیه و یک تراکنش بسته شدن برای هر کانال لازم است. اگر من قصد داشته باشم تعداد زیادی پرداخت با این شخص انجام دهم، باز کردن و بستن یک کانال مقرون به صرفه می شود. در مقابل، اگر من فقط نیاز به انجام چند تراکنش Lightning داشته باشم، باز کردن یک کانال مستقیم مزیتی ندارد، زیرا این به من هزینه 2 تراکنش on-chain را برای تعداد محدودی از تراکنش های off-chain می آورد. این مورد ممکن است به عنوان مثال زمانی رخ دهد که بخواهم بدون برنامه ریزی برای بازگشت، با Lightning در یک فروشگاه پرداخت کنم.
...
$$ f_{\text{Carol-Suzie}} = 0 + \frac{200 \times 40001.04}{10^6} = 0 + 8.0002 \approx 8 \text{ sats} $$
...
**ایجاد رمز**: باب یک رمز تصادفی با نام _s_ (پیش تصویر) تولید می کند و با استفاده از [تابع هش](https://planb.academy/resources/glossary/hash-function) با نام _h_، هش آن را که با _r_ نشان داده شده است، محاسبه می کند. ما داریم:
...
در این فصل، ما به عملکرد **صورتحساب های** Lightning نگاه عمیق تری خواهیم انداخت، یعنی درخواست های پرداختی که توسط گره گیرنده به گره فرستنده ارسال می شوند. هدف درک چگونگی پرداخت و دریافت پرداختی ها در Lightning است. ما همچنین در مورد 2 جایگزین برای صورتحساب های کلاسیک یعنی [LNURL](https://planb.academy/resources/glossary/lnurl) و Keysend بحث خواهیم کرد.
...
فاکتورها سپس به فرمت **[bech32](https://planb.academy/resources/glossary/bech32-and-bech32m)** کدگذاری می‌شوند، همان فرمتی که برای آدرس‌های بیتکوین [SegWit](https://planb.academy/resources/glossary/segwit) استفاده می‌شود (فرمتی که با `bc1` شروع می‌شود).
...
1. **پرداخت کننده**: این فردی است که پرداخت ها را انجام می دهد. آن ها برای انتقال وجوه به کاربران دیگر به [نقدینگی خروجی](https://planb.academy/resources/glossary/outbound-capacity) نیاز دارند. به عنوان مثال، این می تواند یک مصرف کننده باشد.

2. **فروشنده (یا دریافت کننده پرداخت)**: این فردی است که پرداخت ها را دریافت می کند. او نیاز به [نقدینگی ورودی](https://planb.academy/resources/glossary/inbound-capacity) دارد تا بتواند پرداخت ها را به نود خود بپذیرد. به عنوان مثال، این می تواند یک کسب و کار یا یک فروشگاه آنلاین باشد.
...
![LNP201](assets/en/076.webp)۲. **تراکنش‌ها در کانال**: در این کانال، امکان انجام تعداد زیادی تراکنش بدون نیاز به انتشار آن‌ها در بلاکچین وجود دارد. هر تراکنش در شبکه لایتنینگ یک وضعیت جدید از کانال ایجاد می‌کند که در یک تراکنش تعهد ([commitment transaction](https://planb.academy/resources/glossary/commitment-transaction)) منعکس می‌شود.
...
اکنون که با عملکرد نظری شبکه Lightning آشنا شده‌اید، می‌توانید وارد مرحله عملی شوید و نخستین نود Lightning خود را راه‌اندازی کنید تا در استفاده خود به استقلال بیشتری دست یابید. برای این منظور، دوره LNP 202 را دنبال کنید:
