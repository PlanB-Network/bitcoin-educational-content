...
Kako nabaviti ili prodati bitkoine bez ugrožavanja vaše finansijske suverenosti? Centralizovane platforme nameću invazivne KYC procedure koje izlažu vaše lične podatke, uz mogućnost proizvoljnog zamrzavanja računa ili državnog nadzora.



**Mostro P2P** nudi decentralizovanu alternativu koja kombinuje Lightning Network, Nostr protokol i hold fakture. Njegova glavna inovacija: automatizovani escrow sistem gde vaši bitkoini ostaju pod vašom kontrolom tokom celog Exchange, eliminišući rizik od krađe, bankrota Exchange ili proizvoljne konfiskacije.
...
Mostro P2P je open source Bitcoin Exchange protokol koji je kreirao **grunch**, developer popularnog Telegram bota **lnp2pbot** lansiranog 2021. godine. Iako je lnp2pbot već omogućavao KYC-free P2P razmene putem Lightning-a, predstavljao je veliku ranjivost: **Telegram predstavlja centralizovanu tačku kvara** podložnu cenzuri od strane vlada.



Mostro predstavlja **decentralizovanu evoluciju** ovog koncepta: zamenom Telegrama sa **Nostr** protokolom (necenzurisana mreža distribuiranih releja), Mostro eliminiše ovaj sistemski rizik. Protokol kombinuje Lightning Network za trenutne transakcije, Nostr za komunikaciju otpornu na cenzuru, i **hold invoices** kako bi stvorio zaista nekustodijalni automatizovani eskrou.
...
- Mostro** kupci: korisnički interfejsi (CLI, Mobilni, Veb)



Nalozi se objavljuju na Nostr kao javni događaji (tip 38383), dok se privatne trgovine prenose putem end-to-end enkriptovanih poruka (NIP-59, NIP-44). Svaka Mostro instanca definiše svoje naknade (uglavnom između 0.3% i 1%) i limite transakcija (u rasponu od nekoliko hiljada do nekoliko stotina hiljada Sats, u zavisnosti od instance).
...
**Otporan na cenzuru**: Nijedna vlada ne može ugasiti Mostro sve dok Nostr releji postoje negde u svetu. Za razliku od ranjivog lnp2pbot-a putem Telegrama, Mostro omogućava razmene koje su **otporne na cenzuru**.



**Escrow bez staratelja**: Lightning hold fakture zaključavaju vaše bitkoine bez trajnog prenosa. Vaša sredstva ostaju pod vašom kontrolom i automatski vam se vraćaju u slučaju problema (~24h).



**Poverljivost po dizajnu**: Dva režima dostupna da odgovaraju vašim potrebama. Režim reputacije** povezuje vašu reputaciju sa vašim Nostr ključem kako bi se izgradilo trajno poverenje. Potpuno privatni režim** favorizuje apsolutnu anonimnost sa jednokratnim efemernim ključevima.
...
**Fleksibilnost plaćanja**: Podrška za mnoge fiat valute putem yadio.io Exchange usluge kursa. Prihvata SEPA transfere, PayPal, Revolut, gotovinu i bilo koji metod dogovoren između strana.
...
**Mostro CLI** je najzreliji i najfunkcionalniji klijent. Napisan u Rust, nudi čitav spektar Mostro funkcija putem Interface komandne linije. Kompatibilan sa macOS, Linux i Windows.
...
- `fiatsent`: Potvrdite slanje fiat uplate
- `release`: Oslobodi escrow i finaliziraj Exchange
- `getdm`: Pregledaj direktne poruke
- `rate` : Proceni svog protivnika nakon Exchange
...
**Mobilna aplikacija** u alfa verziji omogućava trgovanje P2P direktno sa vašeg pametnog telefona. Intuitivni grafički Interface sa navigacijom u karticama, pregledom narudžbi, naprednim filtrima i integrisanim četom za komunikaciju sa vašim suprotnim stranama.
...
- Opcionalno, ali preporučeno**: VPN ili Tor konekcija za maskiranje vašeg IP Address
...
**Važno za CLI/Mobilnu** sinhronizaciju: Da biste pristupili istim nalozima na CLI i mobilnoj aplikaciji, morate koristiti **isti Mostro Pubkey** i iste **Nostr releje** u oba klijenta. Proverite ova podešavanja u Podešavanjima na mobilnoj aplikaciji.
...
**Korak 1**: Idite na [GitHub stranicu sa izdanjima](https://github.com/MostroP2P/mobile/releases) i preuzmite **app-release.apk** datoteku (približno 65 MB).
...
**Korak 2: Kada se preuzme, otvorite APK datoteku iz svojih preuzimanja. Android će vas pitati da autorizujete instalaciju sa ovog izvora.
...
- Privatnost po defaultu**: Izaberite između režima reputacije i režima potpune privatnosti
...
**Korak 4**: Nastavite sa uvodnim procesom da biste otkrili :




- Potpuno šifrovani čet**: Komunikacija šifrovana od kraja do kraja
- Prihvatite ponudu**: Pregledajte knjigu narudžbi i izaberite ponudu
...
- Višestruki releji**: Konfigurišite 3-5 geografski raznovrsnih releja
- Zaštita mreže**: Aktivirajte VPN ili Tor da sakrijete svoj IP Address
- Wallet secure**: Aktivirajte automatsko zaključavanje na vašem Wallet Lightning
...
Ovaj odeljak prikazuje kupovinu bitkoina putem Mostro CLI prateći stvarni slučaj upotrebe.
...
Mostro će vas zamoliti da obezbedite **Lightning Invoice** da biste primili BTC. Tačan iznos u satoshijima će biti naznačen (ovde: 4715 Sats).
...
### Korak 3: Pružite vaš Lightning Invoice



generate a Lightning Invoice from your Wallet (Phoenix, Breez, etc.) za tačan iznos, zatim ga pošaljite :
...
Sistem potvrđuje pošiljku i obaveštava vas da sačekate da prodavac plati Invoice zadržavanje pre nego što aktivirate eskrou.
...
### Korak 6: Izvršenje fiat uplate



Pošaljite uplatu putem dogovorenog metoda (Revolut u ovom primeru) na dostavljene kontakt podatke. **Sačuvajte svu prateću dokumentaciju** (screenshot-ove, reference transakcija).
...
Kada uplata bude izvršena, obavesti Mostro :
...
Čim prodavac potvrdi prijem fiat-a i oslobodi escrow komandom `release`, odmah dobijaš svoje bitkoine na Lightning Invoice koji si naveo.
...
Ocenite prodavca da doprinesete decentralizovanoj reputaciji:
...
**Otkaži narudžbu** (pre nego što je preuzeta) :
...
- Ćaskanje**: Komunicirajte sa svojim suparnicima koristeći brojke
...
- Releji**: Dodajte nekoliko Nostr releja za otpornost (npr. `wss://nos.lol`, `wss://relay.damus.io`)
...
Pritisnite Green **"+"** dugme u donjem desnom uglu, zatim odaberite **PRODAJ** (crveno dugme).
...
5. **Premium**: Podesite premiju (klizač od -10% do +10%, preporučeno: 0% za brzu prodaju)
...
**Važno uputstvo**: Morate sada **platiti Invoice hold** prikazan da zaključate vaše bitkoine (ovde 4674 Sats za 1-5 EUR) u escrow. Imate **maksimalno 15 minuta** inače će Exchange biti otkazan.



Kopiraj Invoice hold i plati ga sa svog Wallet Lightning (Phoenix, Breez, itd.).
...
Kada je eskrou aktiviran, pritisnite **KONTAKT** da otvorite šifrovani čet sa kupcem.
...
Jednom kada **potvrdite prijem** fiat uplate na vašem računu, pritisnite dugme Green **OSLOBODI** i potvrdite slanje Satss kupcu.
...
Bitcoini se trenutno prenose kupcu putem njegovog Lightning Invoice.
...
2. Kliknite na zanimljivu narudžbu da biste videli detalje


3. Pritisnite **Take Order**


4. Obezbedite vaš Lightning Invoice (generisan iz vašeg Wallet)
...
7. Pošalji fiat uplatu (sačuvaj dokaz)


8. Prodavac oslobađa escrow nakon verifikacije


9. Primite bitkoine trenutno na vaš Lightning Invoice
...
**Otkaži narudžbinu**: U **Moje trgovine**, pritisnite vašu narudžbinu, a zatim dugme **OTKAŽI** (dostupno samo pre nego što bude prihvaćena).
...
**Finansijski suverenitet**: Vaši bitkoini nikada ne napuštaju vašu direktnu kontrolu zahvaljujući Invoice mehanizmu zadržavanja, eliminišući rizik od Exchange bankrota ili piraterije.



**Otporno na cenzuru**: Nijedna vlast ne može ugasiti mrežu ili cenzurisati vaše naloge. Sistem funkcioniše sve dok su Nostr releji operativni.
...
**Rani razvoj**: Rudimentarni interfejsi i tehnička kriva učenja potrebni.
...
Za bitkoinere koji preferiraju autonomiju i poverljivost, Mostro je održiva opcija vredna progresivnog istraživanja. Njegova decentralizovana arhitektura garantuje evoluciju zajednice umesto komercijalne evolucije, otvarajući put ka budućnosti zaista slobodnih Bitcoin razmena.
...
- [Glavni GitHub repozitorijum](https://github.com/MostroP2P/mostro)
