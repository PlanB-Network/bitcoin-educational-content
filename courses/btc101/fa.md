...
در حقیقت، ماینر ها نقش جمع آوری تراکنش های معتبر و تایید نشده را برای تهیه آنها در یک بلوک دارند. در یک کلام، آنها باید یک معمای رمزنگاری را در فرآیندی به نام "اثبات کار" ([proof of work](https://planb.academy/resources/glossary/proof-of-work)) حل کنند تا بلوک آنها بعدی در زنجیره بلوک بیت کوین باشد.
...
**ماینرها شبکه را امن می کنند و معاملات را به بلوک ها اضافه می کنند. آنها از طریق دستگاه های [ASIC](https://planb.academy/resources/glossary/asic) برق مصرف می کنند تا مسئله اثبات کار بیت کوین را حل کنند.**
...
لایتنینگ بر کانال‌های پرداخت دوطرفه متکی است. یک کانال، یک رابطهٔ فنی میان دو شرکت‌کننده است که به آن‌ها اجازه می‌دهد سات‌ها را به‌صورت offchain مبادله کنند، یعنی بدون ثبت هر پرداخت روی بلاکچین.
...
لایتنینگ به‌گونه‌ای طراحی شده است که امکان انجام پرداخت از طریق واسطه‌ها را بدون نیاز به اعتماد به آن‌ها فراهم کند. برای این منظور، پروتکل از قراردادهای هوشمندی به نام HTLC (*Hashed Time-Locked Contracts*) استفاده می‌کند. بدون ورود به تمام جزئیات، سازوکار کلی به این شکل است:
...
در این فصل، چهار پروتکل مهم را معرفی می‌کنیم که هر یک امکانات جدیدی را بر بستر بیت‌کوین فراهم می‌کنند: سایدچین‌ها (به‌ویژه Liquid)، Ark، RGB و Taproot Assets.
...
این ایدهٔ پروتکل لایهٔ دوم توسط Burak در ماه مه 2023 معرفی شد. مانند شبکهٔ لایتنینگ، Ark سیستمی است که بر روی زنجیرهٔ اصلی بیت‌کوین مستقر می‌شود. این سیستم امکان انجام پرداخت‌های بیت‌کوینی خارج از زنجیره، به‌صورت سریع، ناشناس و با کارمزد پایین را فراهم می‌کند. در مقایسه با لایتنینگ، Ark برای دریافت پرداخت‌ها نیازی به نقدینگی ورودی ندارد، که تجربهٔ کاربری را به‌طور قابل توجهی بهبود می‌بخشد. علاوه بر این، سطحی از حریم خصوصی نزدیک به تراکنش‌های coinjoin ارائه می‌دهد. Ark همچنین می‌تواند در صورت افزودن covenantها به بیت‌کوین، غیرتعاملی باشد.
