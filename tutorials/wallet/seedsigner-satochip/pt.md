...
O Satochip é um Hardware Wallet em formato de cartão inteligente, com um elemento de segurança certificado EAL6+, um dos mais elevados padrões de segurança. Foi concebido e produzido pela empresa belga com o mesmo nome: Satochip.



Com um preço de cerca de 25 euros, o Satochip destaca-se da concorrência pela sua excelente relação qualidade/preço. Graças ao seu chip seguro, oferece resistência a ataques físicos. Além disso, o código-fonte da sua aplicação é inteiramente open-source, licenciado sob *AGPLv3*.
...
Para ultrapassar esta fraqueza, uma configuração particularmente interessante consiste em utilizá-lo em conjunto com um SeedSigner. Nesta configuração, a comunicação já não se faz diretamente entre o computador e o Satochip, mas através de trocas de códigos QR entre o computador e o SeedSigner. O SeedSigner actua então como um ecrã de confiança: apresenta a informação a ser assinada, enquanto a assinatura propriamente dita é realizada pelo Satochip. Ao contrário do uso convencional do SeedSigner (ou mesmo do uso em combinação com o Seedkeeper), o seed nunca é carregado no SeedSigner. O SeedSigner torna-se assim o ecrã do Satochip, eliminando os riscos associados à assinatura cega.
...
- O Satochip custa cerca de 25 euros e, uma vez que o applet é de código aberto, pode instalá-lo num smartcard vazio. Depois, é preciso acrescentar o custo dos componentes do SeedSigner e da extensão para leitura de cartões inteligentes: dependendo de onde se compra este hardware, o total deve situar-se entre 70 e 100 euros.
...
Agora que tem todo o seu hardware e software a funcionar corretamente, pode proceder à criação da sua carteira Bitcoin. Para o fazer, ligue o seu SeedSigner e, em seguida, o generate e o seu seed como num SeedSigner convencional, lançando os dados ou tirando uma fotografia:
...
- Por último, faça uma cópia de segurança do seed num suporte físico e verifique-a cuidadosamente.
...
A estratégia de cópia de segurança escolhida aqui depende das suas preferências, mas é imperativo ter pelo menos uma cópia da sua frase Mnemonic, quer em suporte físico (papel ou metal) ou, como aqui, num Seedkeeper. Também pode multiplicar o número de cópias de segurança conforme necessário. Para mais informações sobre as estratégias de backup do portefólio, sugiro que leia este tutorial :
...
Escolha um `Rótulo` para identificar facilmente seus diferentes segredos armazenados no Seedkeeper. Pode, por exemplo, simplesmente manter a impressão digital do Wallet ou indicar explicitamente `seed`. A escolha depende da sua preferência e risco.
...
O código PIN do Seedkeeper deve ser tão longo e aleatório quanto possível, para evitar tentativas de força bruta em caso de comprometimento físico do cartão. Deve também manter uma cópia de segurança deste código PIN, guardada num local separado do Seedkeeper. Sem este PIN, não será possível aceder ao Mnemonic armazenado no Seedkeeper, e os seus bitcoins ficarão perdidos para sempre.
...
Agora que o seu portefólio foi gerado, guardado e verificado, vamos transferi-lo para o Satochip. Para fazer isso, certifique-se de que o seed está carregado na memória RAM do SeedSigner. Depois vá a `Ferramentas > Ferramentas Smartcard > Funções Satochip`.
...
Pode então definir o código PIN Satochip, de 4 a 16 caracteres. Para reforçar a segurança do seu Wallet, escolha um código longo e aleatório: é a única proteção contra o acesso físico à sua frase Mnemonic.



Lembre-se de guardar este PIN assim que for criado, seja num gestor de senhas seguro ou num suporte físico, dependendo da sua estratégia pessoal. Neste último caso, certifique-se de que nunca guarda o suporte que contém o PIN no mesmo local que o seu Satochip, caso contrário a proteção tornar-se-á inútil. É importante ter uma cópia de segurança: **sem este PIN, deixará de poder aceder ao seu seed e os seus bitcoins ficarão permanentemente perdidos**.
...
O seu seed está agora importado para o Satochip.
...
Se pretender utilizar um passphrase BIP39 para aumentar a segurança do seu Wallet, consulte a parte 6 deste tutorial:
...
Em seguida, introduza o código PIN do seu Satochip para o desbloquear. Este é o código que definiu e guardou na etapa 5.
...
O SeedSigner gera então o seu xpub sob a forma de um código QR dinâmico, contendo todos os dados necessários para gerir a sua carteira no Sparrow wallet. É possível regular a luminosidade do ecrã com o joystick para facilitar a leitura do código QR.
...
No seu computador, abra o Sparrow wallet e, na barra de menu, clique em `Arquivo → Importar Wallet`.
...
Atribua um nome ao seu portfólio e, em seguida, clique em `Create Wallet`. O Sparrow irá então pedir-lhe para definir uma palavra-passe para bloquear o acesso local a este Wallet. Escolha uma palavra-passe forte: ela protege os seus dados no Sparrow (chaves públicas, endereços, etiquetas e histórico de transacções). No entanto, esta palavra-passe não é necessária para restaurar o Wallet no futuro: apenas a sua frase Mnemonic (e possivelmente o seu passphrase) o será.
...
Agora verifique se a `Master fingerprint` exibida no Sparrow wallet corresponde à encontrada anteriormente no seu SeedSigner.



O SeedSigner pedir-lhe-á então que digitalize um Address de receção aleatória a partir do seu Sparrow wallet para confirmar a validade da importação.
...
Aparece um novo Bitcoin Address. Encontrará :




- O Address em formato de texto (começando com `bc1q...` se estiver a utilizar o P2WPKH, como neste exemplo) ;
- O código QR associado ;
- Um campo `Label`, útil para rastrear as suas transacções.



Recomendo vivamente que acrescente uma etiqueta a cada recibo Bitcoin no seu Wallet. Isto ajudá-lo-á a identificar facilmente a proveniência de cada UTXO e a gerir melhor a sua privacidade. Para saber mais sobre este importante tópico, consulte a formação específica na Plan ₿ Academy :
...
Antes de comunicar o Address recebido ao pagador, é importante verificar se este pertence ao seu seed. Este passo assegura que o seu Satochip poderá então assinar transacções associadas a este Address. Também evita potenciais ataques em que o Sparrow apresentaria um Address fraudulento. Tenha em mente que o Sparrow é executado num ambiente inseguro (o seu computador), cuja superfície de ataque é muito maior do que a do seu Satochip, que está totalmente isolado. É por isso que nunca deve confiar cegamente nos endereços apresentados no Sparrow antes de os verificar no seu Hardware Wallet.
...
Em seguida, confirme o tipo de script utilizado (neste caso, `Native SegWit`), introduza o código PIN Satochip para o desbloquear e valide a informação `xpub`.
...
Se o Address digitalizado corresponder ao derivado do seu seed, o SeedSigner apresentará a mensagem: `Address Verificado`.
...
Assim, pode ter a certeza de que o Address pertence à sua carteira.
...
Pode agora transmitir este Address sob a forma de texto ou através do seu código QR à pessoa ou serviço que precisa de lhe enviar Satss. Uma vez transmitida a transação na rede, esta aparecerá no separador `Transacções` do Sparrow wallet.
...
- Em `Pay to`, cole o Address do destinatário ou digitalize o seu código QR utilizando o ícone da câmara ;
- Em `Label`, adicione uma etiqueta para rastrear esta despesa;
- Em `Montante`, introduza o montante a enviar;
- Por fim, selecionar a taxa de carregamento de acordo com as condições actuais da rede (as estimativas estão disponíveis em [Mempool.space](https://Mempool.space/)).
...
Ligue o seu SeedSigner e insira o seu Satochip como de costume. A partir do ecrã inicial, selecione `Scan` e depois leia o código QR apresentado no Sparrow.
...
Agora que a transação está assinada e validada, só falta difundi-la na rede Bitcoin para que um Miner a possa incluir num BLOCK. No Sparrow, clique em `Scan QR`.
...
Apresente o código QR exibido no seu SeedSigner (o que contém a transação assinada) à webcam. O Sparrow apresentará então todos os detalhes da transação. Faça uma verificação final para verificar se todas as informações estão corretas e clique em "Broadcast Transaction" para transmiti-la na rede Bitcoin.
...
- Gravando a frase Mnemonic num suporte físico;
...
Se não quiser reconstruir um, pode também utilizar o seu Satochip da forma clássica, ou seja, diretamente a partir do seu computador, sem passar pelo SeedSigner. Este método funciona perfeitamente, mas reduz consideravelmente a segurança do seu Bitcoin Wallet: perde o isolamento "*air-gapped*" e agora tem de assinar às cegas, uma vez que o SeedSigner funcionava como um ecrã de confiança. No entanto, esta pode ser uma solução temporária numa emergência, ou se não for possível reconstruir um SeedSigner.



Para o fazer, necessita de um cartão inteligente USB ou de um leitor NFC. Abra o Wallet que pretende restaurar no Sparrow, aceda ao separador `Configurações` e clique em `Substituir`.
...
Para recuperar o acesso à sua carteira e movimentar os seus fundos, basta carregar o seu seed no SeedSigner. Dependendo do suporte de cópia de segurança que utilizou, tem várias opções:





- Introduzir manualmente a sua frase Mnemonic no menu `Sementes > Introduzir 12 palavras seed`.
...
- Ou carregue seu seed de um Seedkeeper, através do menu `Seeds > From SeedKeeper` (este é o método que estou usando neste tutorial). Basta introduzir o código PIN do Seedkeeper e selecionar o segredo a ser utilizado como seed no SeedSigner.
...
Uma vez que o seed tenha sido carregado no SeedSigner, qualquer que seja o método utilizado, poderá assinar uma ou mais transacções de scan para mover os seus bitcoins para um novo Wallet não comprometido. Para saber como fazer isso, consulte a parte 7.2 do seguinte tutorial:
