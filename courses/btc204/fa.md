...
پروتکل Bitcoin بر اساس مدل [UTXO](https://planb.academy/resources/glossary/utxo) است که مخفف "خروجی تراکنش خرج‌نشده" می‌باشد.
...
**یادآوری:** [Satoshi](https://planb.academy/resources/glossary/satoshi-sat)، که اغلب به sat خلاصه می‌شود، کوچک‌ترین واحد Bitcoin است و با سنتیم در ارزهای [فیات](https://planb.academy/resources/glossary/fiat) قابل مقایسه است.
...
به‌طور نظری، یک UTXO می‌تواند هر مقداری در بیت‌کوین‌ها را نمایندگی کند، از یک سات تا حداکثر نظری حدود ۲۱ میلیون BTC. با این حال، به‌طور منطقی غیرممکن است که همه ۲۱ میلیون بیت‌کوین را مالک باشید و یک آستانه اقتصادی پایین‌تر به نام "[Dust](https://planb.academy/resources/glossary/dust)" وجود دارد که زیر آن، یک UTXO از نظر اقتصادی غیرسودآور برای خرج کردن در نظر گرفته می‌شود.


**آیا می‌دانستید؟** بزرگترین UTXO که تاکنون بر روی Bitcoin ایجاد شده بود، ارزشی برابر با `500,000 BTC` داشت. این توسط پلتفرم MtGox در طی یک عملیات [تجمیع](https://planb.academy/resources/glossary/consolidation) در نوامبر 2011 ایجاد شد: [29a3efd3ef04f9153d47a990bd7b048a4b2d213daaa5fb8ed670fb85f13bdbcf](https://Mempool.space/fr/tx/29a3efd3ef04f9153d47a990bd7b048a4b2d213daaa5fb8ed670fb85f13bdbcf)
...
![BTC204](assets/fa/007.webp)


همه UTXOها توسط [اسکریپت‌هایی](https://planb.academy/resources/glossary/script) محافظت می‌شوند که شرایطی را تعریف می‌کنند که تحت آن‌ها می‌توانند خرج شوند. برای مصرف یک UTXO، کاربر باید به شبکه نشان دهد که شرایط تعیین‌شده توسط اسکریپت محافظت‌کننده آن UTXO را برآورده می‌کند. معمولاً، UTXOها توسط یک [کلید عمومی](https://planb.academy/resources/glossary/public-key) (یا یک [Address دریافت‌کننده](https://planb.academy/resources/glossary/receiving-address) که نماینده این کلید عمومی است) محافظت می‌شوند. برای خرج کردن یک UTXO مرتبط با این کلید عمومی، کاربر باید ثابت کند که [کلید خصوصی](https://planb.academy/resources/glossary/private-key) مربوطه را در اختیار دارد، با ارائه یک [امضای دیجیتال](https://planb.academy/resources/glossary/digital-signature) که با این کلید ساخته شده است. به همین دلیل است که می‌گوییم [Bitcoin Wallet](https://planb.academy/resources/glossary/wallet) شما در واقع حاوی بیت‌کوین نیست، بلکه کلیدهای خصوصی شما را ذخیره می‌کند که به نوبه خود به شما دسترسی به UTXOهای شما و به‌تبع آن، به بیت‌کوین‌هایی که نمایندگی می‌کنند، می‌دهد.


![BTC204](assets/fa/008.webp)
...
تراکنش Bitcoin عملیاتی است که در [Blockchain](https://planb.academy/resources/glossary/blockchain) ثبت شده و Ownership بیت‌کوین را از یک شخص به شخص دیگر منتقل می‌کند. به طور دقیق‌تر، از آنجا که ما در مدل UTXO هستیم و حسابی وجود ندارد، تراکنش شرایط خرج کردن که یک یا چند UTXO را ایمن کرده بود، برآورده می‌کند، آنها را مصرف کرده و معادل آن UTXOهای جدیدی با شرایط خرج کردن جدید ایجاد می‌کند. به طور خلاصه، یک تراکنش بیت‌کوین‌ها را از یک اسکریپت برآورده شده به یک اسکریپت جدید طراحی شده برای ایمن‌سازی آنها منتقل می‌کند.


![BTC204](assets/fa/010.webp)


بنابراین هر تراکنش Bitcoin شامل یک یا چند [ورودی](https://planb.academy/resources/glossary/input) و یک یا چند [خروجی](https://planb.academy/resources/glossary/output) است. ورودی‌ها UTXOهایی هستند که توسط تراکنش برای خروجی‌های generate مصرف می‌شوند. خروجی‌ها UTXOهای جدیدی هستند که می‌توانند به عنوان ورودی برای تراکنش‌های آینده استفاده شوند.


![BTC204](assets/fa/011.webp)


**آیا می‌دانستید؟** از نظر تئوری، یک تراکنش Bitcoin می‌تواند تعداد بی‌نهایتی ورودی و خروجی داشته باشد. تنها محدودیت، حداکثر اندازه [بلوک](https://planb.academy/resources/glossary/block) است.
...
برای اینکه یک تراکنش بر اساس [قوانین اجماع](https://planb.academy/resources/glossary/consensus-rules) معتبر تلقی شود، مجموع خروجی‌ها باید کمتر یا مساوی مجموع ورودی‌ها باشد. به عبارت دیگر، مجموع UTXOهای جدیدی که توسط تراکنش ایجاد می‌شوند نباید از مجموع UTXOهایی که به عنوان ورودی مصرف می‌شوند بیشتر باشد. این اصل منطقی است: اگر شما فقط `500,000 Sats` دارید، نمی‌توانید خریدی به مبلغ `700,000 Sats` انجام دهید.
...
وقتی یک Miner در اعتبارسنجی یک بلاک موفق می‌شود، او مجاز است که این کارمزدها را برای تمام تراکنش‌های موجود در بلاک خود، از طریق تراکنش موسوم به "[coinbase](https://planb.academy/resources/glossary/coinbase-transaction)" جمع‌آوری کند.
...
کوین‌بیس نوع خاصی از تراکنش Bitcoin است که برای هر بلوک منحصربه‌فرد بوده و همیشه اولین مورد از این‌هاست. این امکان را به Miner می‌دهد که با یافتن یک [Proof of Work](https://planb.academy/resources/glossary/proof-of-work) معتبر، Block reward خود را دریافت کند. این پاداش از دو Elements تشکیل شده است: **[کمک‌هزینه بلوک](https://planb.academy/resources/glossary/block-subsidy)** و **کارمزد تراکنش** که در بخش قبلی مورد بحث قرار گرفت.
...
![BTC204](assets/fa/016.webp)


بیت‌کوین‌های یارانه‌ای بلاک، BTC جدیدی هستند که از ابتدا طبق یک [برنامه انتشار](https://planb.academy/resources/glossary/limite-demission) از پیش تعیین‌شده در قوانین اجماع ایجاد می‌شوند. یارانه بلاک هر ۲۱۰,۰۰۰ بلاک، یعنی تقریباً هر چهار سال، در فرآیندی که به عنوان "[Halving](https://planb.academy/resources/glossary/halving)" شناخته می‌شود، نصف می‌شود. در ابتدا، با هر یارانه ۵۰ بیت‌کوین ایجاد می‌شد، اما این مقدار به تدریج کاهش یافته است؛ در حال حاضر، این مقدار ۳.۱۲۵ بیت‌کوین به ازای هر بلاک است.
...
**آیا می‌دانستید؟** بیت‌کوین‌های تولید شده توسط Coinbase Transaction تحت یک دوره بلوغ 100 بلوکی قرار دارند که در طی آن نمی‌توانند توسط Miner خرج شوند. این قانون به منظور جلوگیری از مشکلات مرتبط با استفاده از بیت‌کوین‌های تازه ایجاد شده در زنجیره‌ای که ممکن است بعداً منسوخ شود، طراحی شده است.
...
از سوی دیگر، به دلیل ماهیتشان، [دوبار خرج کردن](https://planb.academy/resources/glossary/double-spending-attack) برای ارزهای فیزیکی مشکلی ایجاد نمی‌کند. اگر من یک اسکناس ۱۰ یورویی به شما بدهم، به طور غیرقابل برگشتی از مالکیت من خارج شده و وارد مالکیت شما می‌شود، که به طور طبیعی هرگونه امکان خرج کردن چندباره واحدهای پولی که در بر دارد را منتفی می‌کند. به طور خلاصه، من قادر نخواهم بود این اسکناس ۱۰ یورویی را دوباره خرج کنم.


![BTC204](assets/fa/019.webp)
...
![BTC204](assets/fa/021.webp)


در Bitcoin، Double-spending به همان روش جلوگیری می‌شود. ما به دنبال تأیید عدم وجود تراکنشی هستیم که قبلاً سکه‌های مورد نظر را خرج کرده باشد. اگر سکه‌ها هرگز استفاده نشده باشند، می‌توانیم مطمئن باشیم که هیچ خرج مضاعفی رخ نخواهد داد. این اصل توسط [Satoshi ناکاموتو](https://planb.academy/resources/glossary/nakamoto-satoshi) در [وایت پیپر](https://planb.academy/resources/glossary/white-paper) با عبارت معروف توصیف شده است:


**تنها راه تأیید عدم وجود یک تراکنش، آگاهی از تمام تراکنش‌ها است**


اما برخلاف مدل بانکی، ما نمی‌خواهیم به یک نهاد مرکزی در Bitcoin اعتماد کنیم. بنابراین همه کاربران باید بتوانند عدم وجود دوبار خرج کردن را تأیید کنند، بدون اینکه به یک شخص ثالث متکی باشند. بنابراین همه باید از تمام تراکنش‌های Bitcoin آگاه باشند. به همین دلیل است که تراکنش‌های Bitcoin به‌صورت عمومی در تمام [نودهای](https://planb.academy/resources/glossary/node) شبکه پخش می‌شوند و به‌صورت متن واضح در Blockchain ثبت می‌شوند.
...
تجزیه و تحلیل Blockchain به معنای ردیابی جریان بیت‌کوین‌ها در Blockchain است. به طور کلی، تجزیه و تحلیل [زنجیره‌ای](https://planb.academy/resources/glossary/onchain) بر اساس مشاهده ویژگی‌ها در نمونه‌هایی از تراکنش‌های قبلی استوار است. سپس شامل شناسایی همین ویژگی‌ها در تراکنشی است که قصد تحلیل آن را داریم و استنتاج تفسیرهای محتمل از آن‌ها. این روش حل مسئله که بر اساس یک رویکرد عملی برای یافتن یک راه‌حل به اندازه کافی خوب است، به عنوان "[هیوریستیک](https://planb.academy/resources/glossary/analysis-heuristic)" شناخته می‌شود.
...
به‌طور نظری، این بیانیه صحیح است. در بخش اول این دوره، دیدیم که جفت کلیدهای [رمزنگاری](https://planb.academy/resources/glossary/cryptography) برای ایجاد شرایط بر روی UTXO استفاده می‌شوند. در اصل، این جفت کلیدها هیچ اطلاعاتی درباره هویت دارندگان خود فاش نمی‌کنند. بنابراین، حتی اگر بتوانیم فعالیت‌های مرتبط با جفت کلیدهای مختلف را گروه‌بندی کنیم، این به ما چیزی درباره موجودیت پشت این فعالیت‌ها نمی‌گوید.


![BTC204](assets/fa/028.webp)
...
- [cIOH](https://planb.academy/resources/glossary/cioh) (*ورودی مشترک Ownership اکتشافی*);
- و [استفاده مجدد از Address](https://planb.academy/resources/glossary/address-reuse).


![BTC204](assets/fa/031.webp)
...
به عنوان مثال، در اینجا یک تراکنش Bitcoin وجود دارد که الگوی [جارو](https://planb.academy/resources/glossary/sweep-transaction) را اتخاذ می‌کند:
...
![BTC204](assets/fa/040.webp)


تفسیر این مدل این است که ما در حضور [هزینه‌های گروه‌بندی شده](https://planb.academy/resources/glossary/batched-spending) هستیم. این یک عملی است که احتمالاً فعالیت اقتصادی بسیار بزرگی را نشان می‌دهد، مانند یک پلتفرم Exchange. هزینه‌های گروه‌بندی شده به این نهادها امکان می‌دهد تا با ترکیب هزینه‌های خود در یک تراکنش، هزینه‌ها را کاهش دهند.
...
![BTC204](assets/fa/048.webp)


منبع: [Mempool.space](https://Mempool.space/tx/db07516288771ce5d0a06b275962ec4af1b74500739f168e5800cbcb0e9dd578)


در مورد دومی، می‌توانیم ببینیم که ورودی شماره 0 و خروجی شماره 1 از اسکریپت‌های P2WPKH (SegWit V0) استفاده می‌کنند، در حالی که خروجی شماره 0 از یک اسکریپت [P2PKH](https://planb.academy/resources/glossary/p2pkh) متفاوت (Legacy) استفاده می‌کند.
...
اولاً، یک تحلیل‌گر می‌تواند به دنبال شباهت‌ها در استفاده از اسکریپت باشد. برای مثال، برخی اسکریپت‌های اقلیتی مانند [Multisig](https://planb.academy/resources/glossary/multisig) ممکن است راحت‌تر از اسکریپت‌های SegWit V0 قابل تشخیص باشند. هرچه گروهی که در آن پنهان می‌شویم بزرگ‌تر باشد، تشخیص ما سخت‌تر است. این یکی از دلایلی است که در پروتکل‌های خوب CoinJoin، همه شرکت‌کنندگان دقیقاً از همان نوع اسکریپت استفاده می‌کنند.
...
عوامل متعددی این فرضیه را تقویت می‌کنند. اولاً، نوع اسکریپتی که برای ایمن‌سازی ورودی UTXO استفاده شده است، یک اسکریپت [P2SH](https://planb.academy/resources/glossary/p2sh) 2/3 Multisig است که نشان‌دهنده سطح پیشرفته‌ای از امنیت است که معمولاً در پلتفرم‌های Exchange دیده می‌شود:
...
SegWit V0 (`P2WPKH` و `P2WSH`) و [Taproot](https://planb.academy/resources/glossary/taproot) / SegWit V1 (`P2TR`) آدرس‌ها نمایانگر فرمت‌های مدرن هستند. آدرس‌های SegWit با `bc1q` شروع می‌شوند و آدرس‌های Taproot که در سال 2021 معرفی شدند، با `bc1p` شروع می‌شوند.
...
یک روش پیشرفته‌تر *Knapsack Solver* است. این روش تا نسخه 0.17 بر روی [Bitcoin Core](https://planb.academy/resources/glossary/bitcoin-core) Wallet استفاده می‌شد. این روش شامل انتخاب تصادفی و تکراری UTXOها از Wallet، جمع‌آوری آن‌ها در زیرمجموعه‌ها و نگه‌داشتن راه‌حلی است که وزن تراکنش را تا حد ممکن کاهش می‌دهد، به منظور کاهش هزینه برای کاربر.
...
اولین توصیه‌ای که می‌توانم بدهم این است که با متراکم‌ترین جمعیت ترکیب شوید. در Bitcoin، این به معنای استفاده از قالب‌های اسکریپت به‌طور گسترده پذیرفته شده است. به عنوان مثال، اسکریپت‌های [P2WSH](https://planb.academy/resources/glossary/p2wsh) که اغلب برای پیکربندی‌های SegWit V0 Multisig استفاده می‌شوند، بسیار نادر هستند. آن‌ها به شما اجازه نمی‌دهند که در یک مجموعه ناشناس بزرگ پنهان شوید. همین امر برای مدل‌های قدیمی‌تر مانند P2PKH یا P2SH نیز صدق می‌کند. اگرچه آن‌ها به‌طور گسترده در مجموعه UTXO حضور دارند، اما برای تراکنش‌های جدید کمتر و کمتر استفاده می‌شوند.
...
![BTC204](assets/fa/118.webp)


در مثال اول دیدیم که coinjoin چگونه می‌تواند حریم خصوصی یک کوین را نسبت به گذشتهٔ آن محافظت کند، و در مثال دوم، چگونه می‌تواند تاریخچهٔ یک کوین را نسبت به آیندهٔ آن نیز ایمن سازد. به همین دلیل اشاره کردم که coinjoin باید به‌عنوان یک رویداد نقطه‌ای در نظر گرفته شود که تاریخچهٔ کوین را در هر دو جهت بخش‌بندی می‌کند:


![BTC204](assets/fa/119.webp)
...
## Zerolink و chaumian [coinjoin](https://planb.academy/resources/glossary/coinjoin)s
...
علاوه بر این سه پیاده‌سازی تاریخی، اخیراً یک مورد جدید پدیدار شده است: Joinstr. در این فصل به آن نمی‌پردازم، اما می‌توانید تمام اطلاعات مربوط به آن را در این آموزش اختصاصی بیابید:
...
![BTC204](assets/fa/139.webp)


با Wabisabi، نسخهٔ 2.0 رویکرد خود به coinjoinها را تطبیق داده است تا به مدل Whirlpool نزدیک‌تر شود. اگرچه تراکنش‌های coinjoin همچنان بسیار بزرگ هستند، اکنون امکان زنجیره‌کردن چندین چرخهٔ متوالی وجود دارد و بدین ترتیب از مدل Whirlpool پیروی می‌شود. همچنین تلاش ویژه‌ای در زمینهٔ مدیریت چِینج انجام شده است: برخلاف Wasabi 1.0 که در آن چِینج مستقیماً به ورودی‌های کاربران وابسته بود، Wabisabi تلاش می‌کند چِینج را به چندین مبلغ کوچک‌تر تقسیم کند که در قالب مقادیر مساوی میان همهٔ شرکت‌کنندگان توزیع می‌شوند.
...
![BTC204](assets/fa/153.webp)


### ریمیکس‌ها


پس از انجام میکس اولیه، UTXOها به حساب **postmix** منتقل می‌شوند. این حساب شامل UTXOهایی است که قبلاً میکس شده‌اند و همچنین آن‌هایی که در انتظار میکس مجدد هستند. زمانی که کلاینت Whirlpool فعال باشد، UTXOهای موجود در حساب **postmix** به‌طور خودکار برای میکس مجدد در دسترس خواهند بود و به‌صورت تصادفی برای شرکت در این چرخه‌های جدید انتخاب می‌شوند.
...
ساده‌ترین و امن‌ترین راه این است که UTXOهای مخلوط‌شده خود را در حساب **postmix** رها کنید تا در حالت استراحت باقی بمانند، به بازترکیب خود ادامه دهند و تنها هنگام خرج کردن آن‌ها را لمس کنید. کیف‌پول‌های Samourai (و اکنون Ashigaru) و Sparrow دارای محافظت‌های اضافی در برابر تمام این خطرات مرتبط با تحلیل زنجیره هستند. این لایه‌های حفاظتی به شما کمک می‌کنند از بروز اشتباه جلوگیری کنید.
...
- **تجمیع از طریق مونرو:** کیف‌پول Samourai پیش‌تر خدمات سواپ اتمی بین BTC و XMR ارائه می‌داد، اما این سرویس پس از بازداشت سازندگان در دسترس نیست. این رویکرد امکان می‌داد UTXOهای آلوده با تجمیع در مونرو بدون آسیب به محرمانگی شما از طریق CIOH پردازش شوند و سپس دوباره به بیت‌کوین بازگردند. اگر خدمات مشابهی دوباره ظاهر شود، می‌تواند گزینه‌ای مناسب باشد. با این حال، باید توجه داشت که این روش ممکن است پرهزینه شود؛ چه از نظر کارمزد استخراج و چه از نظر پرمیوم ناشی از محدودیت نقدینگی هنگام تبدیل معکوس به بیت‌کوین.
...
هستهٔ عملکرد Whirlpool بدون تغییر باقی می‌ماند، اما با این حال چند تفاوت نسبت به استفادهٔ آن در Samourai وجود دارد: در سال ۲۰۲۵، روی Ashigaru، تنها دو استخر در دسترس است به مبالغ `0.25 BTC` و `0.025 BTC`، با کارمزدهای ورودی متناظر `0.0125 BTC` و `0.00125 BTC`.
...
![BTC204](assets/fa/165.webp)


در مقابل، یک coinjoin که بر اساس مدل Whirlpool 5x5 ساختاربندی شده باشد، $1\,496$ ترکیب ممکن را ارائه می‌دهد:


![BTC204](assets/fa/166.webp)


یک چرخه طغیان Whirlpool 8x8 CoinJoin دارای $9\,934\,563$ تفسیر ممکن است:


![BTC204](assets/fa/167.webp)
...
![BTC204](assets/fa/177.webp)




پیشرفته‌ترین پیاده‌سازی payjoin بدون تردید Stowaway است که توسط توسعه‌دهندگان Samourai Wallet اختراع شده است. پس از بازداشت بنیان‌گذاران نرم‌افزار، این ابزار تنها به‌صورت ناقص روی Samourai کار می‌کند. اما در برنامه Ashigaru دوباره راه‌اندازی شده است.

مزیت Stowaway این است که یک پروتکل کامل و بسیار آسان برای استفاده است که هم دریافت و هم ارسال payjoin را پشتیبانی می‌کند. تراکنش‌های امضاشدهٔ جزئی می‌توانند به‌صورت دستی از طریق اسکن چندین کد QR یا به‌صورت خودکار از طریق Tor و با استفاده از Soroban مبادله شوند.

![BTC204](assets/fa/178.webp)
...
![BTC204](assets/fa/187.webp)


از دیدگاه خارجی، الگوی تراکنش دقیقاً یکسان است.


![BTC204](assets/fa/188.webp)
...
![BTC204](assets/fa/190.webp)


آنچه ما برای جلوگیری از این نیاز داریم، ابزاری است که قادر به محو کردن ردپای گذشته یک سکه Bitcoin باشد تا به نوعی قابلیت [تعویض‌پذیری](https://planb.academy/resources/glossary/fungibility) را بازگرداند. این دقیقاً هدف ریکوشه است.


![BTC204](assets/fa/191.webp)
...
ایده این روش «ریکوشه» در ابتدا از تیم‌های Samourai Wallet نشأت گرفت که آن را برای خودکارسازی فرآیند در برنامه خود ادغام کرده بودند. این خدمت در Samourai پولی بود، زیرا هر ریکوشه هزینه ثابت 100٬000 سات برای کارمزد خدمات به همراه داشت که به آن هزینه‌های استخراج نیز افزوده می‌شد. امروز نیز همین وضعیت در Ashigaru برقرار است. بنابراین استفاده از آن بیشتر برای انتقال مبالغ بالا توصیه می‌شود.
...
- ریکوشه تقویت‌شده، یا «تحویل مرحله‌ای»، این مزیت را دارد که کارمزدهای خدمت Ashigaru را میان پنج تراکنش پیاپی توزیع می‌کند. این گزینه همچنین تضمین می‌کند که هر تراکنش در زمانی متفاوت پخش شده و در بلاکی جداگانه ثبت شود؛ امری که امکان شبیه‌سازی هرچه دقیق‌تر رفتار تغییر مالکیت را فراهم می‌کند. هرچند کندتر است، این روش برای افرادی که عجله‌ای ندارند مناسب‌تر است، زیرا کارایی ریکوشه را با افزایش مقاومت آن در برابر تحلیل زنجیره به حداکثر می‌رساند؛


![BTC204](assets/fa/197.webp)
...
![BTC204](assets/fa/201.webp)


علاوه بر این، هیچ تضمینی وجود ندارد که آلیس، پس از دریافت کلید خصوصی $B$ از باب، کلید خصوصی $A$ خود را در مقابل ارسال کند. بنابراین، این تبادل بر پایه اعتماد گسترده بین طرفین استوار است و برای تضمین انتقال محرمانه مالکیت به شیوه‌ای امن، ناکارآمد است.


![BTC204](assets/fa/202.webp)
...
یک راه برای جلوگیری از حملات مختلف رفع ناشناس‌سازی استفاده از پیشنهاد [Dandelion](https://planb.academy/resources/glossary/dandelion) است. این پروتکل پخش در BIP156 رسمی شد، اما هرگز بر روی Bitcoin پیاده‌سازی نشده است.
...
برای انجام یک تراکنش، هر دو طرف باید یک کیف‌پول بیت‌کوین با پیاده‌سازی BIP47 داشته باشند، مانند PayNym در Ashigaru یا Sparrow Wallet. استفاده مشترک از کدهای پرداخت آن‌ها یک کانال محرمانه میانشان ایجاد می‌کند. برای برقراری مؤثر این کانال، فرستنده باید یک تراکنش خاص روی بلاک‌چین بیت‌کوین انجام دهد که به نام «تراکنش اعلان» شناخته می‌شود (جزئیات آن را بعداً ارائه خواهم کرد).
...
در مورد ربات‌های PayNym، همان ربات‌هایی که گاهی در X (توییتر) دیده می‌شوند، باید گفت آن‌ها نمایش‌های بصری کد پرداخت هستند که توسط Samourai Wallet ایجاد شده‌اند. اکنون با Ashigaru کمی متفاوت شده‌اند، اما اصل موضوع تغییر نکرده است. آن‌ها از طریق یک تابع هش تولید می‌شوند که به آن‌ها نوعی یکتایی شبه‌کامل می‌بخشد. این ربات‌ها به صورت یک رشته کوچک از کاراکترها که با `+` آغاز می‌شود، ظاهر می‌گردند:
...
![BTC204](assets/fa/224.webp)


### کلید دیفی-هلمن Exchange بر روی منحنی‌های بیضوی (ECDH)


پروتکل رمزنگاری که در قلب BIP47 قرار دارد با نام اختصاری [ECDH](https://planb.academy/resources/glossary/ecdh) شناخته می‌شود، که مخفف *Elliptic-Curve Diffie-Hellman* است. این روش یک نوع از کلید اصلی Diffie-Hellman به نام Exchange است.
