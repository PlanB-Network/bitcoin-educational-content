...
This minimalist approach, combined with high stability, makes Slackware a distribution favored by experienced users or Unix purists, especially in contexts where predictability and control are important (servers, embedded systems, mission-critical environments, etc.).
...
- Fedora → GNOME (main edition), but offers several official Spins:
 ...
 - Fedora SoaS, i3, etc. → specific environments
...
- Manjaro → KDE Plasma, GNOME or Xfce depending on edition; community editions available with Cinnamon, MATE, LXQt, i3, etc.
...
- openSUSE → choose at installation between KDE Plasma (most integrated edition), GNOME, Xfce, MATE, etc.
...
- Kali Linux → Xfce (default since 2019), also offers KDE, GNOME, i3, MATE, etc.
...
Under macOS, there are 2 main methods: via *Launchpad* or via *Finder*. If the application has been downloaded from the App Store, open *Launchpad* (from the Dock or the `/Applications` directory), locate the application, then hold down the Option key or perform a long click until the icons start to shake. Then click on the cross next to the application, and confirm deletion.


![Image](assets/fr/210.webp)


If the delete button doesn't appear, the application hasn't been installed from the App Store. In this case, use *Finder*. Close the application first, if it's running. Open *Finder*, go to the `/Applications` directory, select the application you wish to delete, then move it to the recycle garbage can. Finally, empty the recycle garbage can to complete the uninstall.
...
You can now access your key via the `/mnt/usb` directory, and read and write files just like on any other volume.
...
The volume will be automatically mounted and accessible from your file manager, usually in the `/media/username/usb` directory.
...
[Chrome](https://www.google.com/chrome/) benefits from rapid and continuous development, 	primarily led by Google. It is recognized for its high performance, speed, broad compatibility with modern web standards and a good integration of Google services.
...
[Vivaldi](https://vivaldi.com/), founded by the former CEO of Opera, is targeting advanced users looking for a deeper personalization and advanced features such as side panels, an advanced tab manager and integrated note-taking.
...
[Opera](https://www.opera.com/) is a Long standing browser, having been around since 1995. It now uses Blink after abandoning its proprietary engine (*Presto*). It has a modern interface and some innovative features.
...
An interesting alternative to Safari is [Orion](https://kagi.com/orion/), a modern browser designed exclusively for the Apple ecosystem, and also based on WebKit. Its aim is to offer a fast, lightweight and deeply privacy-respecting alternative, without compromising on advanced features.
...
Finally, I'd like to end this overview with a much less known and very marginal alternative: text-mode browsers, the best-known of which is [Lynx Browser](https://lynx.invisible-island.net/).
...
Conversely, if the site is still using HTTP (without the "S"), the connection is unencrypted. Any information entered can then be intercepted by a malicious actor located between you and the site: network operator, access provider, booby-trapped Wi-Fi hotspot, local malware, etc.
...
[LineageOS](https://lineageos.org/), successor to CyanogenMod, is the most widely compatible alternative ROM. It supports several hundred models, thanks to a large community of contributors. Based on AOSP, it emphasizes customization, simplicity and freedom of use.
...
As for your home Wi-Fi network, we'll talk about that in the end of this SCU-202 course.
...
- A free version, offering all functions except audio and video calls (receiving only) and multi-device synchronization
- A paid version at €4.99 per month unlocks all features
...
Finally, some modern routers feature two management interfaces: A local interface web (accessible via an address such as `192.168.1.1`) and a cloud interface (enabling remote management via the Internet). If the latter is enabled by default, I advise you to disable it if you're not using it, or reinforce its security with a strong password and possibly two-factor authentication (2FA) if available.
