...
正是在這樣充滿創新與思想碰撞的環境中，神秘的中本聰（Satoshi Nakamoto）才在 2008 年發表了《比特幣白皮書》。在這份白皮書中，他結合了幾個來自 cypherpunk 運動的想法，例如工作量證明（[proof of work](https://planb.academy/resources/glossary/proof-of-work)）和加密時間戳（Cryptographic Timestamps），創造一種去中心化且可抗審查的數位貨幣。
...
事實上，礦工的職責是收集有效和未確認的交易，將其編譯成一個區塊。簡而言之，他們必須在一個稱為「工作量證明」（[proof of work](https://planb.academy/resources/glossary/proof-of-work), PoW）的過程中解決加密謎題，才能使他們的區塊成為比特幣區塊鏈中的下一個區塊。
...
**Miners 保護網路安全並將交易加入區塊。他們透過 [ASIC](https://planb.academy/resources/glossary/asic) 機器消耗電力，來解決比特幣的工作量證明（[proof of work](https://planb.academy/resources/glossary/proof-of-work), PoW）**。

![image](assets/zh-Hant/054.webp)

### 工作量證明說明

「工作量證明」（[proof of work](https://planb.academy/resources/glossary/proof-of-work), PoW）是比特幣協議的安全共識機制。它是一切的基礎，在比特幣的博弈理論中扮演著重要的角色。
...
簡而言之，比特幣的創新之處在於透過基於工作量證明 ([proof of work](https://planb.academy/resources/glossary/proof-of-work)) 且具浮動難度的區塊鏈，解決了數位世界中的「雙重支付（Double Spending）」問題。在數位世界中，所有權的概念與實體世界不同。事實上，在數位世界中，所有東西都可以被複製和貼上，這就產生了使用數位資產價值更多次的風險。傳統上，為了解決這個問題，銀行等受信任的中介機構 被創建來確保當某項資產被轉移時，它不再屬於原持有者。
...
比特幣區塊鏈的設計宗旨，是讓最多數的人能夠在無需授權、無需信任的前提下進行驗證。這一要求帶來了結構性的限制：區塊鏈無法處理無限數量的交易，因為區塊必須保持在合理的大小範圍內，才能讓獨立節點在不投資過於昂貴硬體的情況下完成下載、儲存與驗證。這種取捨通常被概括為「三難困境」：去中心化、安全性與可擴展性。任何基於區塊鏈的系統，都無法同時最大化這三種屬性。比特幣優先保障去中心化與安全性，這也自然限制了其鏈上交易的吞吐量。
...
閃電網路基於雙向支付通道運作。一個通道，是兩個參與者之間的一種技術關係，使他們能夠在鏈下（offchain）交換 sats，也就是說，無需將每一筆支付都寫入區塊鏈。
...
閃電網路被設計為允許透過中介節點進行支付，而無需信任這些中介。為此，該協議使用了一種稱為 HTLC（*Hashed Time-Locked Contracts*，雜湊時間鎖合約）的智慧合約。無需深入所有細節，只需記住其整體機制：
* 支付以揭示一個秘密（預映像）作為條件；
...
在本章中，我們將介紹四種為比特幣帶來新可能性的關鍵協議：側鏈（特別是 Liquid）、Ark、RGB 以及 Taproot Assets。
...
這一第二層協議的構想由 Burak 於 2023 年 5 月提出。與閃電網路類似，Ark 是部署在比特幣主鏈之上的系統，能夠在鏈下實現快速、匿名且低成本的比特幣支付。與閃電網路相比，Ark 不需要具備入帳流動性即可接收支付，從而顯著改善使用者體驗。此外，它還能提供接近 CoinJoin 交易的隱私性。如果比特幣引入 covenant，Ark 甚至可以實現非互動式運作。
