...
|----------------|-------------|
| **auth** | Contiene gli script relativi all'autenticazione sui servizi, inclusi l'accesso anonimo o l'enumerazione degli utenti. Esempi: `oracle-enum-users`, `ftp-anon`. |
| **broadcast** | Contiene gli script relativi alle operazioni di broadcast sulla rete, in particolare al fine di sfruttare e scoprire determinati servizi, host o protocolli basati sul broadcast (IPv6, wake on lan, IGMP, ecc.). Esempi: `broadcast-dhcp6-discover`, `broadcast-ospf2-discover`. |
| **brute** | Contiene gli script relativi alle operazioni di brute force dell'autenticazione sui servizi (brute force [SSH](https://www.it-connect.fr/cours/comprendre-et-maitriser-ssh/), MSSQL, ecc.). Esempi: `ssh-brute`, `vnc-brute`. |
| **default** | Contiene gli script utilizzati nel caso predefinito (uso di `-sC`). Vengono utilizzati diversi criteri per convalidare l'inserimento di uno script in questa categoria, tra cui la velocità di esecuzione, la struttura dell'output, l'affidabilità del test, il carattere "intrusivo" o "rischioso", ecc. |
| **discovery** | Contiene gli script relativi alla scoperta avanzata della rete e dei servizi. Vi si trovano ad esempio l'enumerazione del contenuto di una condivisione SMB, di una versione di un servizio VNC, delle query SNMP, ecc. Esempi: `mysql-info`, `http-security-headers`. |
| **dos** | Contiene gli script che possono causare un denial of service. Può trattarsi di script creati per sfruttare una vulnerabilità di tipo denial of service o di script che hanno come effetto collaterale un denial of service. Prudenza quindi (sono esclusi dalla categoria `default`). Esempi: `http-slowloris`, `ipv6-ra-flood`. |
| **exploit** | Contiene gli script creati per sfruttare direttamente una vulnerabilità. Esempi: `http-shellsock`, `smb-vuln-ms08-067`. |
| **external** | Contiene gli script che richiedono l'uso di una risorsa di terze parti, come un database di informazioni online. Ciò indica in particolare un tentativo di connessione verso l'esterno (attenzione alla riservatezza). Esempi: `whois-ip`, `dns-blacklist`, `ip-geolocation-geoplugin`. |
| **fuzzer** | Contiene gli script progettati per inviare frame, pacchetti o parametri imprevisti da un servizio. Ciò consente in particolare di causare errori o malfunzionamenti al fine di ottenere indizi di vulnerabilità o informazioni tecniche. Esempi: `dns-fuzz`, `http-form-fuzzer`. |
| **intrusive** | Contiene gli script classificati come "rischiosi" dal punto di vista della disponibilità o del rilevamento. Possono causare un crash del sistema o essere rilevati come dannosi da una soluzione di sicurezza. Si tratta della categoria opposta a `safe`. Esempi: `smtp-brute`, `smb-vuln-ms08-067`, `smb-psexec`. |
| **malware** | Contiene gli script progettati per rilevare la presenza di elementi caratteristici di un malware, come una porta in ascolto comunemente usata da una backdoor nota. Esempi: `ftp-proftpd-backdoor`, `smtp-strangeport`. |
| **safe** | Contiene gli script considerati sicuri dal punto di vista del rilevamento o della stabilità. Si tratta della categoria opposta a `intrusive` e contiene nella stragrande maggioranza script avanzati di identificazione della versione o di rilievo di elementi di configurazione. Esempi: `html-title`, `smb2-security-mode`, `ms-sql-info`. |
| **version** | Contiene gli script che consentono un rilevamento avanzato della versione. Possono essere utilizzati in aggiunta ai Probe e Match studiati in precedenza quando il rilevamento di una versione richiede operazioni un po' più complesse. Esempi: `http-php-version`, `vmware-version`. |
| **vuln** | Contiene gli script progettati per rilevare la presenza di vulnerabilità note (CVE) senza necessariamente sfruttarle (al contrario della categoria `exploit`). Si limitano generalmente a segnalare lo stato "vulnerabile" o meno di un servizio. Esempi: `smb-vuln-ms17-010` (eternal blue), `http-phpmyadmin-dir-traversal`. |
