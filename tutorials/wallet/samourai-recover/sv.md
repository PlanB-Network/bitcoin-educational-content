...
Efter att ha hjälpt flera användare att återställa sina bitcoins de senaste dagarna tror jag att jag har stött på de flesta av de problem som kan uppstå under restaureringen av en Samourai Wallet. Därför kommer denna handledning att börja med en situationsrapport för att identifiera de funktioner som förblir operativa och de som inte längre är tillgängliga inom Samourai Wallet-ekosystemet och den programvara som påverkas av denna incident. Därefter fortsätter vi steg för steg för att återställa en Samourai Wallet med hjälp av Sparrow Wallet-programvaran. Vi kommer att undersöka alla potentiella hinder som uppstått under denna process och se lösningar för att lösa dem. Slutligen, i den sista delen, kommer du att upptäcka de potentiella riskerna för din integritet efter serverbeslaget.
...
Om du inte hade en ansluten Dojo tidigare men har en nu kan du ställa in den så att den använder din Samourai-app igen. Detta innebär att du kontrollerar dina säkerhetskopior, raderar Wallet (Wallet, inte applikationen) och återställer Wallet genom att ansluta din Dojo till applikationen. För mer information om dessa steg kan du läsa [denna handledning, i avsnittet "_Förbereda din Samourai-plånbok_" : CoinJoin - DOJO](https://planb.network/tutorials/privacy/On-Chain/CoinJoin-dojo-c4b20263-5b30-4c74-ae59-dc8d0f8715c2).
...
JoinBot-verktyget är inte längre i drift, eftersom det var helt beroende av att ett Wallet som administrerades av Samourai deltog.
...
När det gäller RoninDojo fortsätter denna node-in-box-mjukvara att fungera korrekt trots att vissa specifika verktyg som Whirlpool CLI och WST inte är tillgängliga. Den kan fortfarande användas för annan Wallet-programvara tack vare Fulcrum eller Electrs. Om du vill få mer information om RoninDojo eller om du har specifika frågor, uppmuntrar jag dig att gå med i [deras Telegram-grupp] (https://t.me/RoninDojoNode).
...
För att återställa din Wallet måste du ha din passphrase, även om du väljer att återställa via backupfilen. Börja med att verifiera giltigheten för denna passphrase. Öppna din Samourai Wallet-app, klicka på din Paynym-ikon längst upp till vänster och välj sedan `Inställningar`.
...
Ange din passphrase och klicka på `Ok`. Om det är korrekt kommer Samurai att bekräfta det. Du har också möjlighet att verifiera backupfilen om du planerar att använda den senare.


![samourai](assets/3.webp)


Detta steg är valfritt men rekommenderas. Det bekräftar att passphrase är korrekt, vilket eliminerar en potentiell källa till problem senare. Om Samourai indikerar att passphrase är felaktig i detta skede är det inte möjligt att återhämta sig. Kontrollera att du har angett passphrase korrekt och kontrollera det igen.


### Alternativ 1: Återställ Wallet på Sparrow med säkerhetskopian
...
Välj sedan `Exportera till urklipp`. Efter det måste du överföra den här filen till din dator på ett säkert sätt. Eftersom filen är krypterad, men passphrase ensam är tillräcklig för att dekryptera den, är det viktigt att vidta försiktighetsåtgärder under överföringen. Om du väljer att överföra filen direkt i klartext måste du skapa filen "samourai.txt" på din dator och klistra in innehållet i urklippet i den. Ett alternativ skulle vara att direkt hämta filen `samourai.txt` från filerna som lagras på din telefon.

När du har tillgång till filen på din dator öppnar du Sparrow Wallet, klickar på fliken `Fil` och väljer `Import Wallet` för att börja importera din Wallet.
...
Sparrow kommer sedan att be dig om ett lösenord för att dekryptera filen. Detta lösenord är faktiskt din passphrase. Ange det i motsvarande fält och klicka på `Import`.


![samourai](assets/7.webp)


Om din Wallet inte visas i det här skedet är det möjligt att du gjorde ett misstag när du kopierade filen `samourai.txt` eller när du skrev in passphrase. Du kan konsultera felsökningsavsnittet för mer hjälp.
...
Sparrow kommer sedan att be dig välja ett lösenord. Detta lösenord skyddar endast åtkomsten till din Wallet på den här datorn och gäller inte härledningen av nycklarna till din Wallet. Se till att välja ett starkt lösenord, anteckna det så att du kommer ihåg det och klicka på "Set Password".
...
Du kommer då att se dina olika Whirlpool-konton visas, och Sparrow kommer att härleda de nödvändiga nycklarna för att använda de tillhörande bitcoins.
...
Jag rekommenderar också att du manuellt importerar de etiketter som du hade associerade med dina UTXO:er på Samourai. Detta kommer att göra det möjligt för dig att utföra effektiv myntkontroll på Sparrow därefter.


### Alternativ 2: Återställ Wallet på Sparrow med Mnemonic-frasen


Om du inte vill utföra återställningen med backupfilen kan du välja en mer traditionell metod genom att helt enkelt använda din 12-ords återställningsfras och din passphrase. Detta andra alternativ är ofta enklare.


För att börja, se till att du har din återställningsfras och din passphrase till hands. Öppna sedan Sparrow Wallet-programvaran, klicka på fliken `File` och välj `Import Wallet` för att börja importera din Wallet.
...
Om Sparrow visar meddelandet "Invalid Checksum" innebär det att checksumman för återställningsfrasen inte är giltig, vilket förmodligen innebär att du gjorde ett misstag när du skrev in orden.
...
Sparrow kommer sedan att be dig att välja ett lösenord. Detta lösenord skyddar endast åtkomsten till din Wallet på den här datorn och har inget att göra med hur du får fram nycklarna till din Wallet. Se till att välja ett starkt lösenord, skriv ner det så att du kommer ihåg det och klicka på "Set Password".
...
Efter att ha hjälpt flera personer under de senaste dagarna tror jag att jag har stött på de flesta av de problem som kan förhindra återställningen av din Wallet. Om du fortfarande inte kan komma åt din Wallet trots de tidigare handledningarna, här är några ytterligare rekommendationer.
...
Därefter kommer ett skrivfel i din passphrase under återställningen att resultera i felaktiga härledda nycklar, vilket förhindrar återställningen av din Wallet på Sparrow. **passphrase måste vara helt korrekt!**


För att lösa detta råder jag dig först att kontrollera giltigheten för din passphrase i Samourai-applikationen enligt beskrivningen i avsnittet "_Verifiera lösenfrasen_" i den här artikeln:


1. **Validering i Samourai:** Om Samourai bekräftar att passphrase är korrekt, försök återhämta dig igen från början och se till att ange passphrase korrekt i Sparrow utan fel;

2. **passphrase Error:** Om Samourai indikerar att passphrase är felaktig, är det meningslöst att fortsätta försöken på Sparrow. Så länge som den korrekta passphrase inte hittas är det omöjligt att återfå din Wallet. Om du har förlorat din passphrase permanent, förvara din Samourai-applikation säkert. Allt du kan göra är att hoppas att servrarna startas om så att du kan göra utgifter direkt från applikationen utan att behöva återskapa den. **Försök inte att ansluta en Dojo i detta fall**, eftersom det skulle innebära att du återställer din Wallet på Samourai, vilket kräver tillgång till passphrase.


Bland andra fel som uppstått är många relaterade till nätverkskonfigurationen på Sparrow.


Först måste du se till att Sparrow är korrekt konfigurerad i läget `Mainnet` och inte i läget `Testnet`. Om Sparrow söker efter dina transaktioner på Testnet kommer den inte att hitta något, eftersom din Wallet finns på Mainnet. Testnet är en alternativ version av Bitcoin, som endast används för testning och utveckling, och fungerar på ett separat nätverk från huvudnätverket (Mainnet), med sina egna block och transaktioner. För att kontrollera vilket nätverk du befinner dig på, klicka på fliken `Tools` och sedan på `Restart In`. Om alternativet `Mainnet` visas är du inte på huvudnätverket. Välj det för att starta om Sparrow på Mainnet och börja sedan din återställningsprocess igen.


![samourai](assets/27.webp)

Vissa har också stött på svårigheter med att ansluta Sparrow till sin nod. Längst ner till höger i Sparrow finns en färgad kontakt som indikerar om din programvara är korrekt ansluten till en Bitcoin-nod. För att hämta dina Samourai-transaktioner är det viktigt att programvaran är väl ansluten. Kontrollera att omkopplaren är aktiverad, som i min bild nedan (gul för en publik nod, Green för Bitcoin Core och blå för en Electrum-server).
...
- Om du försöker ansluta till din egen Electrum-server (blå) eller din Bitcoin Core (Green) och Sparrow inte kan ansluta, kontrollera anslutningsinformationen under `File > Preferences... > Server`;
...
- Om anslutningsproblemet kvarstår kan det bero på en ofullständig synkronisering av din nod. Se till att din nod och din indexerare är 100% synkroniserade. Om det behövs som en sista utväg, koppla bort din nod från Sparrow och anslut till en offentlig nod;
...
För att härleda grenar för andra skripttyper måste du upprepa återställningsprocessen för varje skripttyp som används. För detta går du till `File > New Wallet` på Sparrow, väljer en annan skripttyp i rullgardinsmenyn, klickar på `New or Imported Software Wallet` och följer samma steg som i den inledande handledningen.


![samourai](assets/32.webp)


Ett annat problem jag stött på när det gäller avledning är värdet för Gap Limit. Detta värde talar om för Sparrow efter hur många tomma adresser den ska sluta härleda nya adresser. Om du efter återställning märker att vissa transaktioner saknas kan detta bero på en för låg Gap Limit. För att lösa detta går du till det konto som orsakar problemet, t.ex. postmix-kontot (om flera konton berörs, upprepa denna operation för varje konto).
...
Det täcker de olika återhämtningsproblem som jag har stött på de senaste dagarna. Om du fortfarande har problem efter att ha provat alla dessa lösningar, inbjuder jag dig att gå med i [Discover Bitcoin Discord] (https://discord.gg/xKKm29XGBb) för att be om hjälp. Jag besöker regelbundet denna Discord och skulle vara glad att hjälpa till om jag har lösningen. Andra bitcoiners kommer också att kunna dela med sig av sina erfarenheter och erbjuda sin hjälp. **I vilket fall som helst är det viktigt att hålla din återställningsfras, din backup-fil och din passphrase konfidentiella**. Dela dem inte med någon, eftersom det skulle kunna göra det möjligt för dem att stjäla dina bitcoins.
...
Det är därför viktigt att överväga det värsta tänkbara scenariot och att dela med sig av denna Wallet, potentiellt komprometterad när det gäller integritet. För att göra detta, skapa en ny Wallet från grunden med en annan programvara, som Sparrow Wallet. Efter att ha verifierat giltigheten för dina säkerhetskopior, överför alla dina medel genom att göra transaktioner. Även om denna operation inte bryter spårbarhetslänken för dina mynt, kommer det att förhindra myndigheterna från att med säkerhet veta adresserna till din nya Wallet.
...
Om vi emellertid anser att xpubarna har läckt ut och att du stötte på en användare utan en dojo under den första CoinJoin, och sedan 2 under den första nedströms CoinJoin, skulle din potentiella anonset bara vara 10 istället för 13 ur myndighetens synvinkel.
...
Watch-only wallet-applikationen Sentinel fungerar på samma sätt i nätverket som Samourai. För att komma åt din Wallet-information måste applikationen överföra de xpubar, publika nycklar och adresser som du har angett till en Dojo. Om du alltid har använt din egen Dojo på Sentinel är det inget problem, och du kan fortsätta använda applikationen utan bekymmer. Men om du var beroende av Samourais servrar för din Sentinel är det möjligt att dina xpubar har exponerats. I det här fallet är det tillrådligt att följa samma Wallet-ändringsprocess som rekommenderas för Samourai Wallet när du är ansluten till Samourais servrar.
