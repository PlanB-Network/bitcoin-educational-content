...
Seedkeeper er et smartkort utviklet av Satochip, et belgisk selskap som spesialiserer seg på maskinvareløsninger for håndtering og beskyttelse av digitale hemmeligheter. Satochip er kjent for sitt utvalg av smartkort for Bitcoin-økosystemet, og har utviklet Seedkeeper som et alternativ til tradisjonelle metoder for lagring av Mnemonic-fraser.



Seedkeeper har form av et multifunksjonelt, EAL6-sertifisert smartkort med en sikker prosessor og sabotasjesikkert minne (dvs. en "secure element*"). Som navnet antyder, er kortets rolle å lagre Bitcoin Mnemonic-fraser og passord på en kryptert og beskyttet måte. Med Seedkeeper kan du generate, importere, organisere og lagre hemmelighetene dine direkte i kortets sikre komponent.
...
- Bitcoin** Mnemonic fraselagring: I stedet for å skrive ned de 12 eller 24 ordene dine på papir, kan du importere dem til smartkortet og beskytte dem med en PIN-kode.
...
Seedkeeper er også svært interessant når den kombineres med en statsløs Hardware Wallet som SeedSigner eller Spectre DIY. I dette tilfellet er det ikke nødvendig å bruke Satochips datamaskin eller mobilklient. Seedkeeper beholder seed i sin secure element og kan brukes direkte med signeringsenheten, noe som eliminerer behovet for en QR-kode på papir. Jeg vil ikke gå nærmere inn på dette bruksområdet i denne veiledningen, da det er gjenstand for en annen dedikert veiledning:
...
Sammenlignet med en enkel papirbackup av Mnemonic-frasen har en Seedkeeper flere fordeler:
...
- Avhengighet av en generell databehandlingsenhet:** For å gå inn i og administrere seed kreves det en datamaskin eller smarttelefon, noe som betyr at Mnemonic passerer gjennom en maskin med en mye større angrepsflate enn en Hardware Wallet. Dette kan utgjøre en risiko hvis arbeidsstasjonen blir kompromittert. Derfor anbefaler jeg ikke å bruke Seedkeeper til å lagre seed fra en Hardware Wallet (bortsett fra for tilstandsløs bruk uten datamaskin, som med SeedSigner). Hardware Wallets rolle er nettopp å lagre seed i et minimalistisk, svært sikkert miljø. Ved å legge inn seed manuelt på din vanlige datamaskin, er den ikke lenger begrenset til Hardware Wallet: den havner også på en generell maskin, som er utsatt for flere angrepsvektorer. Så det er bedre å bruke Seedkeeper for en Hot Wallet enn en Cold (unntatt SeedSigner / stateless Hardware Wallet).





- Tapsrisikoen knyttet til PIN-koden:** Den direkte utilgjengeligheten til seed, i motsetning til en papirbackup, gir faktisk beskyttelse mot fysisk tyveri. Men som alltid er sikkerhet en balansegang mellom risikoen for tyveri og risikoen for tap. Hvis sikkerhetskopien din krever en PIN-kode, vil tap av denne koden gjøre det umulig å gjenopprette Mnemonic-frasen din, og dermed få tilgang til bitcoinsene dine.



Med tanke på disse fordelene og ulempene mener jeg at de beste bruksområdene for Seedkeeper (bortsett fra passordadministratorfunksjonen) er på den ene siden å lagre frø fra **programvareporteføljene**, siden de allerede ligger på telefonen eller datamaskinen din, eller fra skjermløse maskinvareporteføljer som Satochip, og på den andre siden å bruke den i kombinasjon med en statsløs Hardware Wallet som SeedSigner, der den virkelig kommer til sin rett.
...
Du vil bli bedt om å oppgi en PIN-kode for din Seedkeeper. Siden dette er et nytt kort, er det ennå ikke definert noen PIN-kode. Skriv inn en hvilken som helst kode for å hoppe over dette trinnet, og klikk deretter på "*Neste*".
...
Velg "Importer hemmelighet*". Med alternativet "*generate-hemmelighet*" kan du opprette en ny Mnemonic-frase direkte fra programmet.
...
*Mnemonic som vises på dette bildet er fiktiv og tilhører ikke noen. Den er kun et eksempel. Vis aldri din egen Mnemonic til noen, ellers vil bitcoinsene dine bli stjålet
...
## 6. Få tilgang til dine seed på Seedkeeper



Hvis du vil sjekke Mnemonic-frasen din, tar du opp Seedkeeper og klikker på "*Click & Scan*"-knappen.
...
Dette tar deg til en liste over alle dine registrerte hemmeligheter. I dette eksempelet ønsker jeg å vise seed i porteføljen min "*BLOCKSTREAM App*", så jeg klikker på den.
...
Den tidligere innspilte Mnemonic-frasen vises nå på skjermen.
...
Dette tar deg til startsiden med en liste over hemmelighetene dine. Klikk på de tre små prikkene øverst til høyre på Interface.
...
Det er alt som trengs! Nå vet du hvordan du bruker Seedkeeper til å lagre Mnemonic-frasen til en Bitcoin Wallet. I en fremtidig veiledning vil vi se på hvordan du kan bruke Seedkeeper til å lagre passordene dine. Jeg inviterer deg også til å oppdage den kombinerte bruken med SeedSigner :
