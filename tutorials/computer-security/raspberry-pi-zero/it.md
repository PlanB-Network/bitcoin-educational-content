...
**6** Una scheda micro SD, per avere un minimo di memoria di massa almeno da 32 GB; se di qualità/livello industriale è meglio.
...
_(1) Se Raspi OS non ti ha fatto creare la password di `root` durante le fasi precedenti, ti consiglio di farlo adesso, con il comando `passwd`. Avere l'utente `root` che si muove in maniera indipendente dal tuo utente personale, può rivelarsi molto comodo nelle situazioni di recovery._
...
E anche vedere `cat proc/meminfo`.
