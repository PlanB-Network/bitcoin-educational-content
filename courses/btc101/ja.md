...
プルーフ・オブ・ワーク（[proof of work](https://planb.academy/resources/glossary/proof-of-work)）システムでは、問題のブロックに対して有効な「ハッシュ」を見つける必要があります。これは、256 文字で構成されるブロックに関連付けられた一意の指紋と考えてください。このハッシュの有効性はビットコインネットワークの難易度に依存する（詳細は後述する）。今のところ、マイナー（採掘者）が有効なブロックを見つけ、その中にボブからアリスへの取引が含まれていると考える。そして、新しい有効なブロックは、すべてのビットコインユーザーの共通台帳であるブロックチェーンに追加される。
...
Bitcoinプロトコルの基礎を理解したところで、次にBitcoinの上に構築された主要な決済ネットワークであるLightning Network（一般に「LN」と略されます）を紹介します。その目的は非常にシンプルです。ブロックチェーンを混雑させることなく、ほぼ即時に確定するBTC決済を、一般的に非常に低い手数料で実現することです。

### ブロックチェーンはすべてを担えない

Bitcoinのブロックチェーンは、できる限り多くの人が、許可不要かつ信頼に依存せずに検証できるよう設計されています。この要件には構造的な制約が伴います。すなわち、ブロックは独立したノードが高価なハードウェアに投資せずともダウンロード・保存・検証できる範囲に収める必要があるため、ブロックチェーンは無制限のトランザクションを処理できません。これはしばしば「トリレンマ」として要約されます。分散性、セキュリティ、スケーラビリティです。ブロックチェーンを基盤とするシステムは、これら3つの性質を同時に最大化することはできません。Bitcoinは分散性とセキュリティを優先しており、その結果、オンチェーンで処理可能なトランザクション数は自然と制限されます。

![image](assets/ja/081.webp)

Bitcoinでは、開発者たちがこれらを重視するために明確な選択を行ってきました。一方では、ブロックサイズを1MBに制限し、ブロック生成間隔を平均10分とすることで、低コストでBitcoinノードを運用でき、ネットワークの分散性が促進されます。他方では、プルーフ・オブ・ワークによるブロック生成によって不正行為は極めて高コストとなり、ノードによる検証が容易になると同時に、プロトコル全体のセキュリティが強化されます。

しかし、これらの選択には重要な制約も伴います。各ブロックに含められるトランザクション数が限られているのです。これは1秒あたり数件のトランザクションに過ぎません。この数値は、VISAのような中央集権的決済システム（理論上の最大処理能力は約65,000件/秒）と比べると非常に小さいものです。しかし、この制限こそが、検閲されず、信頼できる第三者を必要としないトランザクションを可能にするための代償です。

具体的には、これはBitcoinの日常利用において非常に重要な2つの意味を持ちます。
* ブロックスペースの需要が高まると、オンチェーン手数料が非常に高騰する可能性がある；
* オンチェーン決済には承認が必要であり、日常的な買い物には必ずしも適していない。

Lightning Networkは、まさにこれらの課題に対する回答です。Lightningの考え方はレイヤー構造のアプローチに基づいています。Bitcoinは引き続き基盤レイヤー（堅牢で非常に安全な決済レイヤー）として機能し、Lightningはその上に高速な決済レイヤーとして位置づけられます。

![image](assets/ja/080.webp)

### Bitcoinにアンカーされたペイメントチャネル

Lightningは双方向のペイメントチャネルに基づいています。チャネルとは、2人の参加者間の技術的な関係であり、各支払いをブロックチェーンに記録することなく、オフチェーンでサトシをやり取りできる仕組みです。

Bitcoin（オンチェーン）の視点では、チャネルを開くことは特別なトランザクションで資金をロックすることを意味します。これはエスクローのようなものと捉えることができます。資金は、有効なチャネルクローズによってのみ再分配できる形でロックされます。

Lightningの視点では、同じ仕組みが、両当事者がサトシの分配状態を何度でも、ほぼ即時に更新できるチャネルとして機能します。しかも、そのたびにメインのブロックチェーンへ記録する必要はありません。
...
このように、Lightningチャネルを共有する2人は、毎回オンチェーントランザクションを行うことなく、非常に多くの支払いを実行できます。ここにもスケーラビリティの考え方があります。ブロックチェーンは稀で重要な操作（チャネルの開設と閉鎖、すなわち最終決済）のみに用い、その他の小額で中間的な支払いは、より効率的なレイヤーへ移します。

### 相互接続されたチャネルのネットワーク

Lightningは単なる孤立したチャネルの集合ではありません。これはネットワークです。数千のノードがチャネルによって相互に接続され、接続グラフを形成しています。

![image](assets/ja/082.webp)

このネットワークのおかげで、直接のチャネルを持っていない相手に対しても、支払いをルーティングできる経路が存在すれば支払いが可能です。その場合、支払いは複数の中継ノードを経由し、ホップごとに転送されます。

ここでLightningの重要な概念である「流動性」が登場します。チャネルの容量とは、そのチャネルにロックされている資金の総額を指します。一方、流動性とは、その資金がチャネルのどちら側に配置されているか、つまりサトシがどの方向に流れるかを意味します。言い換えると、チャネルは大きな容量を持っていても、流動性が反対側に偏っていれば、特定の方向では利用できない場合があります。支払いの成功は、経路の存在だけでなく、その経路全体にわたる流動性にも依存します。

### 仲介者を信頼せずに支払いをルーティングする

Lightningは、仲介者を信頼することなく支払いを経由できるよう設計されています。そのために、HTLC（Hashed Time-Locked Contracts）と呼ばれるスマートコントラクトが用いられます。詳細には踏み込みませんが、基本的な仕組みは次のとおりです。
...
Lightningは、従来はBitcoinオンチェーンだけでは不便、あるいは不可能だった多様なユースケースを解き放ちます。
...
前章までで見てきたように、Bitcoinは極めて堅牢なベースレイヤーとして設計されています。すなわち、公開され、シンプルで安全な台帳ですが、速度、プログラマビリティ、決済スループットには本質的な制限があります。Ethereumのように、このレイヤーにすべてを担わせるのではなく、Bitcoinエコシステムは段階的にレイヤー構造のアプローチを採用してきました。ブロックチェーンが基盤（最終決済）として機能し、その上にあるレイヤーが高速決済、より高いプライバシー、資産発行（ステーブルコイン、トークン化証券など）といった新しい性質を付加します。
...
Burakは、Lightningがメインチェーンへの依存によってスケールしにくい点をしばしば批判し、Arkは理論上、世界中の人々をセルフカストディに統合できる可能性があると述べています。ArkはLightning Networkの競合と見なされることもありますが、実際には両者は共存でき、むしろ補完的な関係になり得ます。

Arkは依然として非常に活発で若い分野です。目標は魅力的（ユーザーあたりのオンチェーン負荷を大幅に削減）ですが、BitcoinやLightningとは異なる仮定とリスクを持つ、より複雑なアーキテクチャである点を理解しておく必要があります。
...
RGBは長年にわたって開発が続けられているプロトコルです。進展は段階的ですが、すでにRGBを活用する具体的なアプリケーションが存在します。さらに理解を深めたい方には、Plan ₿ Academyでこのプロトコルを詳しく解説するエキスパート向けコースを提供しています。
...
現在のBitcoinエコシステムを最も正確に表すイメージは、固定されたプロトコルでも、Ethereumのように何でもこなすスーパー・ブロックチェーンでもありません。むしろ、それは意図的に保守的な基盤であり、その周囲を、最小限のリスクで実験と革新を可能にするレイヤーやプロトコルが取り囲んでいる姿です。
