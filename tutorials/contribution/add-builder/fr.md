...
- Rendez-vous sur [le dépôt GitHub de PlanB dédié à la data](https://github.com/PlanB-Network/bitcoin-educational-content/tree/dev/resources/builders) dans la section `resources/project/` :
...
- Dans ce dossier, vous allez créer un premier fichier YAML nommé `project.yml` :
...
- Retournez dans le dossier `resources/projects/` et sélectionnez le dossier de votre entreprise que vous venez de créer dans le commit précédent :
...
- Créez un nouveau fichier YAML en le nommant avec l'indicateur de votre langue natale. Ce fichier va être utilisé pour la description du project. Par exemple, si je veux rédiger ma description en anglais, je vais nommer ce fichier `en.yml` :
