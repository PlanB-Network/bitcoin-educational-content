---
name: Oikolukuohjeet
description: Mitkä ovat tärkeitä tekijöitä, jotka on pidettävä mielessä Plan ₿ Academy -ohjelman oikolukemisessa?
---

![github](assets/cover.webp)


Tervetuloa tähän ohjeeseen, jossa käsitellään **ohjeita, joita on noudatettava, kun oikolukee sisältöä Plan ₿ Academyssa**. Olemme iloisia, että jaat tavoitteemme kääntää Bitcoin-materiaalia mahdollisimman monelle kielelle, jotta ihmiset saisivat tietoa siitä, miten se toimii ja miten sitä voidaan käyttää heidän jokapäiväisessä elämässään.


Ensinnäkin, osallistumalla Plan ₿ Academy [julkinen arkisto](https://github.com/PlanB-Network/bitcoin-educational-content) -hankkeeseen voit kirjoittaa opetusohjelmia, oikolukea olemassa olevaa sisältöä tai jopa ehdottaa uuden kielen lisäämistä alustalle. Jos haluat lisätietoja, liity ensin [Telegram-ryhmään](https://t.me/PlanBNetwork_ContentBuilder) ja kirjoita lyhyt esittely itsestäsi ja kielistä, joita osaat.
...
Ennen kuin sukellat yksityiskohtiin, kannattaa ensin lukea tämä ohje käytännön toimista Githubissa haarukoimalla Plan ₿ Academy -arkisto, tekemällä muutoksia ja lähettämällä PR-kirjoituksia:
...
Itse asiassa yhteinen tavoitteemme on kouluttaa mahdollisimman monia ihmisiä Bitcoin:stä, joten on tärkeää, että heidän lukemansa materiaali on sujuvaa ja selkeää.

Tässä mielessä kaikki Plan ₿ Academyn työntekijät ovat kasvattajia!
...
Tämän jälkeen saat ponnahdusikkunan, johon sinun on lisättävä uuden haaran nimi. Alla olevassa tapauksessa valitsin nimeksi "BTC101-FR". Näin muistan aina, että tätä haaraa on käytettävä ranskankielisen kurssin BTC101 oikolukemiseen, enkä **kä käytä sitä mihinkään muuhun tehtävään**.
...
LLM-työkalut saattavat kääntää joitakin Bitcoin:ään liittyviä sanoja kirjaimellisesti, kuten Lightning Network. Näin on erityisesti silloin, kun kyse on hyvin teknisistä sanoista. Tällaisissa tapauksissa on suositeltavaa säilyttää alkuperäinen englanninkielinen sana kohdekielellä paremman selkeyden vuoksi, elleivät kielisäännöt edellytä jokaisen sanan kääntämistä.
...
- Joka tapauksessa neuvoisin teitä **sisällyttämään EN-sanan kuitenkin** ja lisäämään kohdekielen vastaavan merkityksen pyöreiden sulkujen sisälle kaavion EN (LANG) mukaisesti, tai päinvastoin. Esim. Address (indirizzo) tai indirizzo (osoite).
...
- Toinen asia, joka liittyy sanojen esittämiseen, on seuraava. Jos tekstissä lukee "Plan ₿ Academy", **jättäkää se tässä alkuperäisessä muodossa**. Älä käännä sanaa "plan" tai sanaa "network". Älä myöskään käytä artikkelia "The", kun esittelet Plan ₿ Academy: **Harkitse sitä tuotemerkkinä**.
...
Jos käytät tekoälyä oikolukemaan suurimman osan tekstistä, huomaamme sen varmasti, koska tunnemme tekoälyn tuottamat tyypilliset lauserakenteet. Jos huomaamme, että luotat oikolukemisessa pelkästään tekoälyyn tekemättä merkittäviä muutoksia, sats:n lopullinen palkkio voi pienentyä puoleen!
...
Markdown-kielessä otsikot (ja kappaleiden otsikot) alkavat kaikki hash-merkillä ``#``. Rasterimerkkien määrä vastaa otsikon tasoa. Esimerkiksi kolmannen tason otsikossa on kolme numeromerkkiä ennen tekstiä (esim. `### Otsikkoni`).
...
Varmista, ettet **KOSKAAN poista hash-merkkejä** ennen otsikkoa, sillä muuten aiheutat ongelmia tekstin rakenteeseen.
...
## Miten käsitellä kurssikuvia


Verkkosivuillamme on nyt käännettyjä kuvia lähes jokaisesta kurssista!


Kun oikoluketat, tarkista aina, että kaikki kuvat ovat mukana ja näkyvät oikein. Jos koodinäkymässä on tällainen rivi `![KUVA](assets/en/001.webp)`, se tarkoittaa, että kuva näytetään siellä.


Varmista, että lisäät aina uuden rivin kuvakoodin ja tekstin väliin. Esimerkki alla:
...
Muista myös lukea jokaisen kuvan sisältö. Jos huomaat, että kuvien sisällä olevan tekstin kääntämisessä on ongelmia, ilmoita asiasta koordinaattorillesi, niin saat mahdollisuuden oikolukea myös kuvat!


Voit tarkastella kuvaa Githubin `Preview`-osiossa (tai verkkosivuillamme, avaa toisessa välilehdessä). Palaa sitten oikolukemista varten viereiseen `koodi`-osioon.
...
## Tutoriaalin oikoluku


Jos päätät oikolukea opetusohjelmia, koordinaattori avaa erityisen kysymyksen **kokonaista opetusohjelmaosiota** varten. Kun olet saanut tehtäväsi valmiiksi, voit dokumentoida edistymisesi kommentoimalla asiaa ja laatimalla luettelon tarkistetuista opetusohjelmista: näin luot selkeän seurantajärjestelmän tulevaa käyttöä varten, mikä on tärkeää, koska uutta sisältöä lisätään joka kuukausi. Voit nähdä esimerkin tästä lähestymistavasta [täällä](https://github.com/PlanB-Network/bitcoin-educational-content/issues/3023#issuecomment-3364923190).


![REVIEW](assets/en/13.webp)


Koska uusia opetusohjelmia lisätään kuukausittain, haarasi voi vanhentua oikolukuprosessin aikana. Jotkut oikolukijat ovat lähestyneet tätä ongelmaa synkronoimalla täsmälleen sen haaran, jossa he työskentelevät: **ÄLÄ KOSKAAN tee niin! Jos teet niin, vaarana on, että menetät kaiken siihen mennessä saavuttamasi edistyksen! **


Sen sijaan sinun pitäisi ensin lukea loppuun nykyisen fork:n opetusohjelmat. Sitten **synkronoi `dev`** ja luo uusi haara, jossa keskityt juuri lisättyjen opetusohjelmien oikolukemiseen (vain ne, jotka puuttuvat edellisestä haarasta).


Ohjeissa on mahdollista, että **kuvia ei ehkä käännetä**. Koska suurin osa opetusohjelmista on **alkuperäisesti kirjoitettu ranskaksi tai englanniksi**, löydät todennäköisesti kuvia, jotka sisältävät komentoja tai ohjeita niiden alkuperäiskielellä. Otetaan esimerkki hollanninkielisestä Sparrow-oppaasta raportoimalla sekä teksti että siihen liittyvä kuva.
...
Kuten näet, kuva osoittaa selvästi `Public Server`, englanniksi, kun taas tekstissä mainitaan ilmaisu `_Publieke server_`. Tässä tapauksessa on kyse johdonmukaisuusongelmasta, koska lukija löytää ristiriitaista tietoa, kun hän asettaa kuvan ja tekstin vastakkain.


Voit ratkaista tämän ongelman lisäämällä komennon sellaisena kuin se näkyy kuvassa (englanniksi tai ranskaksi), ja sen jälkeen suluissa on käännös omalla kielelläsi, kuten alla näkyy:
...
Tärkeä asia, joka kannattaa pitää mielessä, kun oikolukee tämäntyyppistä _yml_-tiedostoa, on välttää kaksoispisteiden ``:`` tai lainausmerkkien lisäämistä tekstin sisälle. Itse asiassa kaksoispistettä käytetään **vain** erottamaan avain-arvoparit, kuten "wrong_answers", muista. Näet esimerkin alla olevasta kuvasta:


![REVIEW](assets/en/15.webp)


Kun olet tarkistanut kysymyksen, varmista, että muutat "tarkistettu"-tilan "väärästä" "oikeaksi", kuten alla olevassa kuvassa näkyy. Varmista, että **pitää nämä tilasanat englanninkielisinä**, riippumatta siitä, millä kielellä työskentelet!
...
Jos otsikko on englanninkielinen, voit mukauttaa lauseen omaan kieleesi: "Swahiliksi tämä ilmaisu..."
...
- Jos kirjoittaja lisää linkin, joka viittaa Plan ₿ Academyn kurssiin tai opetusohjelmaan (**ei** suluissa), sivusto luo automaattisesti "kortin", joka näyttää siihen liittyvän pikkukuvan. Varmista siis aina, että **lisäät uuden rivin tekstin ja itse linkin väliin**, muuten saatat nähdä seuraavan virheen verkkosivustolla.
...
Nyt olet valmis aloittamaan kurssien, opetusohjelmien, tietokilpailujen ja sanaston sanojen oikolukemisen. Pysy kuulolla ja aloita myös videoiden transkriptien tarkistaminen!
