...
SeedSigner 是由标准硬件（通常是 Raspberry Pi Zero）自行组装而成的 Hardware Wallet。这种 Wallet 被称为 "*无状态*"：与市场上的大多数其他型号（Coldcard、Trezor、Ledger 等）不同，它不在永久内存中存储任何数据，仅通过 RAM 实时运行。因此，您的 seed 投资组合永远不会存储在 SeedSigner 上。每次重新启动时，您都需要填写它，以使设备能够签署您的交易。最常用的方法是将 seed 保存为 QR 码，每次使用时扫描一下 (*SeedQR*)。
...
为了克服这一弱点，SeedSigner 可以与 Satochip 开发的智能卡 [**Seedkeeper**](https://satochip.io/product/seedkeeper/) 结合使用。这样，Mnemonic 短语（或其他秘密）就可以存储在由 PIN 码保护的 secure element 中。Seedkeeper 小程序是开源的，其 secure element 已通过 EAL6+ 认证。它与 SeedSigner 结合使用，可以提供非常有趣的安全功能：你的密钥完全离线管理，你可以在一个可信的屏幕上签署交易，seed 在智能卡中受到物理保护，可以抵御物理攻击。
...
- 如果您还没有与 SeedSigner 关联的 Bitcoin Wallet，则需要按照下面提到的教程中的**5**和**6**步骤进行操作。这两节介绍了如何将 generate 与 SeedSigner 相连的 Mnemonic 短语通过 *SeedQR* 保存，然后将该 Wallet 与 Sparrow wallet 相连以对其进行管理。我不会在这里详细介绍这些步骤，**我假设您已经有了一个可用的 Bitcoin Wallet，并配置了 Sparrow 和您的 SeedSigner**。
...
要将 SeedSigner 与 Seedkeeper 结合使用，需要安装一个不同于原始 SeedSigner 的固件，以支持智能卡读取。为此，[我建议使用 "*3rdIteration*"的 Fork](https://github.com/3rdIteration/seedsigner)。下载与你使用的 Raspberry Pi 型号相对应的 [最新版本的映像](https://github.com/3rdIteration/seedsigner/releases) (`.zip`)。
...
现在您的 Seedkeeper 已经可以运行，您可以将 Bitcoin Wallet Mnemonic 保存在智能卡上。首先，像往常一样打开 SeedSigner，然后扫描 Wallet 的 *SeedQR* 将其加载到设备中。导入 seed 后，只需选择 "完成"。
...
现在选择 Seedkeeper 的新 PIN 码，长度在 4 到 16 个字符之间。为了提高安全性，请选择一个随机的长密码：这是保护物理访问 Mnemonic 短语的唯一屏障。
...
然后，您可以定义一个与 Mnemonic 短语相关的 "标签"。如果您在 Seedkeeper 上存储了多个秘密，这个标签就非常有用，这样您就可以很容易地识别它们。
...
在安全策略方面，根据您的需求和面临风险的程度，有几种可行的方法。我个人建议，至少保留两份 seed 的副本：




- 这是智能卡的首创，你可以方便地使用智能卡进行日常操作，如验证地址或签署交易。这种方法很实用（我们将在第 5 部分中看到），而且由于有 PIN 码的保护，它仍然是安全的，所以你可以随时取用它，而不会有太大的风险；
- 第二份未加密的 Mnemonic 短语，作为您投资组合的最终备份，仅在 Seedkeeper 丢失或被盗时使用。由于该版本未加密，因此必须保存在一个单独、更安全的位置，以防止两个备份同时受到破坏。



根据您的保护策略和风险状况，您还可以在多个不同的 Seedkeepers 上复制 seed，或创建多个 Mnemonic 的物理副本。要了解有关这些做法的更多信息，请参阅以下教程：
...
现在，您可以使用 Seedkeeper 在启动时将 Mnemonic 短语加载到 SeedSigner 中，从而签署 Bitcoin 交易。首先，插入 SeedSigner，打开 "种子 "菜单。
...
Seedkeeper 可以包含多个秘密，因此 SeedSigner 会提示您选择要加载的秘密。显示的标签与您在步骤 4 中定义的名称相对应。如果像我这样只注册了一个 seed，则只有一个选项可用。
...
您的 seed 现在已加载。将屏幕上显示的指纹与 Sparrow wallet 设置中指定的指纹进行比较，检查这是否是正确的 Wallet。该指纹也是在首次创建 Wallet 时提供的。



如果您使用的是 passphrase，可以在此阶段应用它（请参阅本教程第 6 部分）。否则，只需点击 "完成"。
...
- 将您的 seed 和 passphrase 保存在 Seedkeeper 中，并使用强大的 PIN 码进行保护（这一点很重要）。有了这个备份，您就可以方便地每天使用 Wallet。如果您愿意，可以在第二个 Seedkeeper 上复制这些信息；
- 此外，请用纸张或金属保存一份清晰的 Mnemonic 和 passphrase 副本。这是您丢失 Seedkeeper 或其 PIN 码时的最后备份。确保将这些副本分别存放在不同的地方，以防它们同时被盗。



在这种配置下，如果有人单独获取了您的明文 Mnemonic，那么在不知道 passphrase 的情况下，他们将无法窃取任何东西（当然，前提是 passphrase 足够强大，能够抵御暴力破解攻击）。相反，如果有人发现了你的明文 passphrase，如果没有相应的 Mnemonic 短语，它仍然无法使用。
...
- 一个**passphrase 强**：它必须很长、随机，并包含多种字符。它的复杂性对您来说不是问题，因为您只需在初始化时在键盘上输入一次，之后就会由 Seedkeeper 发送；
...
将带有 passphrase 的投资组合正确载入 SeedSigner 后，打开 "种子 "菜单，选择与该投资组合相对应的足迹。请注意，该足迹与不含 passphrase 的投资组合的足迹不同。
...
然后点击 "备份 seed"，将 Seedkeeper 插入驱动器，并选择 "到 SeedKeeper"。
...
输入 PIN 码解锁 Seedkeeper，然后为这个秘密指定一个标签。你可以将指纹作为标签，以保持某种形式的可信度，或者明确说明 "passphrase Wallet"。
...
输入 PIN 码解锁智能卡，然后选择与 passphrase 对应的 Wallet。
