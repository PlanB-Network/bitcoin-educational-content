...
Đối với những ai đã từng tham gia xây dựng nội dung giáo dục trên Plan ₿ Academy, có thể bạn đã có hồ sơ giảng viên. Bạn có thể tìm thông tin trong thư mục `/professors` [trên kho lưu trữ (repository) GitHub của chúng tôi](https://github.com/PlanB-Network/Bitcoin-educational-content/tree/dev/professors). Nếu hồ sơ của bạn đã tồn tại, hãy tìm tên định danh của bạn trong tệp `professor.yml`.
...
Bạn phải có bản fork của riêng mình từ [kho lưu trữ Plan ₿ Academy trên GitHub](https://github.com/PlanB-Network/Bitcoin-educational-content).

Đồng bộ hóa nhánh chính (`dev`) của bản fork với kho lưu trữ gốc.
...
Đi đến thư mục `scripts/tutorial-related/data-creator/` trong bản clone tại máy tính của bạn. Hãy đảm bảo rằng bạn đã cài đặt Python và tất cả các thư viện phụ thuộc (dependencies) cần thiết cho phần mềm:
...
- Nhấn vào "*Select Image*" để chọn ảnh đại diện từ máy tính (phần mềm sẽ tự động xử lý mọi định dạng và tên tệp. Bạn chỉ cần đảm bảo ảnh có tỷ lệ khung hình vuông);
- Viết mô tả ngắn cho hồ sơ của bạn.

Hoàn tất việc tạo bằng cách nhấn vào "*Create Professo*". Thao tác này sẽ tự động tạo tất cả các tệp cần thiết cho hồ sơ của bạn.
...
Để thay đổi metadata của hồ sơ như địa chỉ Lightning, tên hoặc các liên kết, hãy chọn tệp "*professor.yml*". Để thay đổi phần mô tả, hãy nhấn vào tệp YAML tương ứng với ngôn ngữ của bạn (ví dụ: "*en.yml*" hoặc "*vi.yml*").
...
Khi đã mở tệp bạn muốn sửa đổi, hãy nhấp vào biểu tượng bút chì.
...
Thực hiện các thay đổi mong muốn trong tệp này. Sau khi hoàn tất, nhấn chọn "*Commit changes*".
...
Pull Request là một yêu cầu được gửi đi để tích hợp các thay đổi từ nhánh của bạn vào nhánh chính của kho lưu trữ Plan ₿ Academy, cho phép xem xét và thảo luận về các thay đổi trước khi chúng được merge chính thức.

![Image](assets/fr/10.webp)

Hãy đảm bảo rằng ở phía trên cùng của giao diện, nhánh làm việc của bạn đang được merge vào nhánh `dev` (đây là nhánh chính) của kho lưu trữ Plan ₿ Academy.

Nhập tiêu đề tóm tắt ngắn gọn các thay đổi bạn muốn merge vào kho lưu trữ gốc. Thêm một bình luận ngắn mô tả những thay đổi này, sau đó nhấn vào nút "*Create pull request*" để xác nhận:

![Image](assets/fr/11.webp)

PR của bạn sẽ hiển thị trong tab "*Pull Request*" của kho lưu trữ gốc Plan ₿ Academy. Việc cuối cùng bạn cần làm là chờ quản trị viên merge những thay đổi của mình.
