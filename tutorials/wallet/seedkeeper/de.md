...
Der Seedkeeper ist eine Smartcard, die von Satochip entwickelt wurde, einem belgischen Unternehmen, das auf Hardwarelösungen für die Verwaltung und den Schutz digitaler Geheimnisse spezialisiert ist. Satochip, das für seine Smartcards für das Bitcoin-Ökosystem bekannt ist, hat den Seedkeeper als Alternative zu den herkömmlichen Methoden zur Speicherung von Mnemonic-Phrasen entwickelt.



Konkret handelt es sich bei Seedkeeper um eine multifunktionale, EAL6-zertifizierte Smartcard mit sicherem Prozessor und manipulationssicherem Speicher (d. h. ein "secure element*"). Wie der Name schon sagt, besteht seine Aufgabe darin, Bitcoin Mnemonic Phrasen und Passwörter verschlüsselt und geschützt zu speichern. Mit Seedkeeper können Sie Ihre Geheimnisse generate importieren, organisieren und direkt in der sicheren Komponente der Karte speichern.
...
- Bitcoin** Mnemonic Phrasenspeicher: Anstatt Ihre 12 oder 24 Wörter auf Papier zu notieren, können Sie sie auf die Smartcard importieren und mit einem PIN-Code schützen.
...
In diesem Tutorium werde ich nur auf Anwendungsfälle im Zusammenhang mit Bitcoin eingehen, da dies das Thema dieses Tutoriums ist. Wir werden nicht auf die Passwortverwaltungsfunktionen eingehen, die Gegenstand eines anderen Tutorials sein werden.



Im Vergleich zu einem einfachen Papier-Backup des Mnemonic-Satzes hat die Verwendung eines Seedkeepers mehrere Vorteile:
...
- Abhängigkeit von einem Allzweck-Computergerät:** Für die Eingabe und Verwaltung des seed ist ein Computer oder Smartphone erforderlich, was bedeutet, dass Ihr Mnemonic ein Gerät mit einer viel größeren Angriffsfläche als ein Hardware Wallet durchläuft. Dies kann ein Risiko darstellen, wenn die Workstation kompromittiert wird. Aus diesem Grund empfehle ich nicht, Seedkeeper zu verwenden, um den seed eines Hardware Wallet zu speichern (außer für die zustandslose Verwendung ohne Computer, wie bei SeedSigner). Die Rolle des Hardware Wallet besteht genau darin, den seed in einer minimalistischen, hochsicheren Umgebung zu speichern. Wenn Sie Ihren seed manuell auf Ihrem üblichen Computer eingeben, ist er nicht mehr auf den Hardware Wallet beschränkt: Er landet auch auf einem Allzweckrechner, der mehreren Angriffsvektoren ausgesetzt ist. Es ist also besser, Seedkeeper für einen Hot Wallet zu verwenden als für einen Cold (außer SeedSigner / zustandsloser Hardware Wallet).





- Das mit der PIN verbundene Verlustrisiko:** Die direkte Unzugänglichkeit der seed bietet im Gegensatz zu einem Papier-Backup tatsächlich Schutz vor physischem Diebstahl. Aber wie immer ist Sicherheit eine Abwägung zwischen dem Diebstahl- und dem Verlustrisiko. Wenn Ihr Backup eine PIN erfordert, macht es der Verlust dieses Codes unmöglich, Ihren Mnemonic-Satz wiederherzustellen und damit auf Ihre Bitcoins zuzugreifen.



In Anbetracht dieser Vor- und Nachteile bin ich der Meinung, dass Seedkeeper (abgesehen von seiner Passwortverwaltungsfunktion) am besten für die Speicherung von Seeds aus Ihren **Softwareportfolios** geeignet ist, da sie sich bereits auf Ihrem Telefon oder Computer befinden, oder aus Ihren bildschirmlosen Hardware-Brieftaschen wie dem Satochip, und dass er in Kombination mit einem zustandslosen Hardware Wallet wie dem SeedSigner verwendet werden kann, wo er seine Stärken voll ausspielt.
...
Legen Sie die Karte dann auf die Rückseite Ihres Smartphones. Die Anwendung erkennt, dass Seedkeeper noch nicht initialisiert wurde, und fordert Sie auf, den PIN-Code für Ihre Smartcard festzulegen, der zwischen 4 und 16 Zeichen lang sein kann. Um optimale Sicherheit zu gewährleisten, wählen Sie ein möglichst langes, zufälliges und aus einer Vielzahl von Zeichen bestehendes Passwort. Dieser PIN-Code ist die einzige Barriere gegen den physischen Zugriff auf Ihre Wiederherstellungsphrase.
...
## 5. Einen seed bei Seedkeeper registrieren
...
Wählen Sie "Geheimnis importieren*". Mit der Option "*generate-Geheimnis*" können Sie direkt in der Anwendung einen neuen Mnemonic-Satz erstellen.
...
*Der Mnemonic auf diesem Bild ist fiktiv und gehört niemandem. Er ist nur ein Beispiel. Zeigen Sie niemals jemandem Ihren eigenen Mnemonic, sonst werden Ihre Bitcoins gestohlen
...
Wenn Sie Ihren Mnemonic-Satz überprüfen möchten, nehmen Sie Ihren Seedkeeper zur Hand und klicken Sie auf die Schaltfläche "*Klick & Scan*".
...
Dies führt Sie zu einer Liste aller Ihrer registrierten Geheimnisse. In diesem Beispiel möchte ich den seed in meinem Portfolio "*BLOCKSTREAM App*" anzeigen, also klicke ich ihn an.
...
Ihre zuvor aufgenommene Mnemonic-Phrase wird nun auf dem Bildschirm angezeigt.
...
Wir werden nun ein Backup meines Seedkeepers auf einem zweiten Seedkeeper erstellen, um zwei Kopien zu haben. Diese Redundanz kann Teil einer Strategie zur Sicherung Ihrer Bitcoins sein: zum Beispiel die Aufbewahrung Ihrer Phrase an zwei verschiedenen Orten, um physische Risiken zu begrenzen, oder das Anvertrauen einer Kopie an einen vertrauenswürdigen Verwandten als Teil eines Erbschaftsplans.
...
Dadurch gelangen Sie auf die Startseite mit einer Liste Ihrer Geheimnisse. Klicken Sie auf die drei kleinen Punkte oben rechts auf Interface.
...
Das war's schon! Jetzt wissen Sie, wie Sie Seedkeeper verwenden, um die Mnemonic-Phrase eines Bitcoin Wallet zu speichern. In einem zukünftigen Tutorial werden wir uns ansehen, wie Sie Seedkeeper zum Speichern Ihrer Passwörter verwenden können. Ich lade Sie auch ein, seine kombinierte Verwendung mit SeedSigner zu entdecken:
...
In diesem Lehrgang haben wir mehrmals die ***Deskriptoren*** in Ihrem Bitcoin-Portfolio erwähnt. Sie wissen nicht, was sie sind? Dann empfehle ich Ihnen, unseren kostenlosen CYP 201 Kurs zu besuchen, in dem alle Mechanismen, die mit dem Betrieb von HD-Portfolios zusammenhängen, detailliert behandelt werden!
