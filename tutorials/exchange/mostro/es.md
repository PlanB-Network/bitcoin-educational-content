---
name: Mostro
description: Plataforma Bitcoin P2P Exchange sin KYC a través de Lightning y Nostr
---
...
¿Cómo adquirir o vender bitcoins sin comprometer su soberanía financiera? Las plataformas centralizadas imponen procedimientos KYC invasivos que exponen tus datos personales, con la posibilidad de congelación arbitraria de la cuenta o vigilancia estatal.



**Mostro P2P** ofrece una alternativa descentralizada que combina Lightning Network, el protocolo Nostr y las facturas retenidas. Su mayor innovación: un sistema de custodia automatizado en el que sus bitcoins permanecen bajo su control durante toda la Exchange, eliminando el riesgo de robo, quiebra de Exchange o confiscación arbitraria.
...
- daemon Mostrod**: coordina los intercambios entre los usuarios y Lightning Network
...
**Escrow sin custodia**: Las facturas de custodia Lightning bloquean tus bitcoins sin transferirlos permanentemente. Tus fondos permanecen bajo tu control y se te devuelven automáticamente en caso de problema (~24h).
...
**Flexibilidad de pago**: Compatible con muchas monedas fiduciarias a través del servicio de tarifas Exchange de yadio.io. Acepta transferencias SEPA, PayPal, Revolut, efectivo y cualquier método acordado entre las partes.
...
El **Mostro CLI** es el cliente más maduro y funcional. Escrito en Rust, ofrece toda la gama de funciones de Mostro a través de una línea de comandos Interface. Compatible con macOS, Linux y Windows.
...
- `fiatsent`: Confirmar envío de pago fiat
- liberar: Liberar la plica y finalizar el Exchange
- `getdm`: Ver mensajes directos
- `rate` : Evalúe a su contraparte después de un Exchange
...
La **aplicación móvil** en versión alfa permite negociar P2P directamente desde su smartphone. Interface gráfica intuitiva con navegación por pestañas, visualización de órdenes, filtros avanzados y chat integrado para comunicarse con sus contrapartes.
...
Interface aplicación web JavaScript avanzada en desarrollo activo. Diseñada para ofrecer una experiencia de usuario completa con amplias funcionalidades de negociación y gestión de carteras. Acceso a través del navegador sin necesidad de instalación.
...
Este tutorial le guiará a través de la instalación y el uso de los dos principales clientes Mostro: CLI y la aplicación móvil.
...
- Un Lightning Network** Wallet en funcionamiento con suficiente liquidez (o compatible con Lightning)
...
- Una clave privada Nostr** (formato `nsec1...`)
 ...
 - Importante**: Nunca utilices tu clave Nostr personal principal
...
- Opcional pero recomendado**: VPN o conexión Tor para enmascarar tu IP Address
...
Abra `.env` y configure sus ajustes:
...
Compile e instale la aplicación CLI :
...
Debería ver la lista completa de pedidos.
...
En primer lugar, instale Rust a través de [rustup.rs](https://rustup.rs/) y, a continuación, utilice PowerShell :
...
Configuración idéntica: copie `.env-sample` en `.env` y rellene sus parámetros.
...
**Paso 1**: Vaya a la [página de versiones de GitHub](https://github.com/MostroP2P/mobile/releases) y descargue el archivo **app-release.apk** (aprox. 65 MB).
...
- Opere Bitcoin libremente - sin KYC** : Comercio sin verificación de identidad gracias a Nostr
...
- Acepte una oferta**: Navegue por la cartera de pedidos y elija una oferta
- ¿No encuentra lo que necesita?** : Cree su propio pedido personalizado
...
**Importante**: Anote su frase de recuperación en un lugar seguro (gestor de contraseñas, caja fuerte de papel).
...
- Protección de red**: Activa VPN o Tor para ocultar tu IP Address
- Wallet seguro**: Activa el bloqueo automático de tu Wallet Lightning
...
Esta sección demuestra la compra de bitcoins a través de Mostro CLI siguiendo un caso de uso real.
...
Verá una tabla con los detalles de cada pedido: ID, tipo (compra/venta), importe, divisa, forma de pago.
...
Para comprar estos bitcoins, toma la orden con `takesell` :
...
Mostro le pedirá que proporcione un **Lightning Invoice** para recibir el BTC. Se indicará la cantidad exacta en satoshis (aquí: 4715 Sats).
...
### Paso 3: Proporcione su Invoice Lightning



generate un Invoice Lightning de su Wallet (Phoenix, Breez, etc.) por el importe exacto, luego envíelo :
...
El sistema confirma el envío y le informa de que debe esperar a que el vendedor pague la retención Invoice antes de activar la plica.
...
Una vez activada la custodia, utilice `dmtouser` para solicitar los datos de pago al vendedor:
...
Envíe el pago a través del método acordado (Revolut en este ejemplo) a los datos de contacto facilitados. **Conserve todos los documentos justificativos** (capturas de pantalla, referencias de transacciones).
...
Una vez efectuado el pago, notifíquelo a Mostro :
...
### Paso 8: Recepción de bitcoins



En cuanto el vendedor confirme la recepción del fiat y libere la plica con el comando `release`, recibirás al instante tus bitcoins en el Lightning Invoice que le hayas proporcionado.
...
Esta sección demuestra la venta de bitcoins a través de Mostro Mobile siguiendo un caso de uso real.
...
- Mis Operaciones**: Vea sus órdenes activas e históricas
- Chat**: Comuníquese con sus contrapartes utilizando cifras
...
- Lightning Address** (opcional): Para recibir pagos directamente
...
**Importante para la sincronización CLI/Móvil**: Si utilizas tanto CLI como la app móvil, configura **exactamente los mismos relés Nostr y Mostro Pubkey** en ambos clientes. Sin esta configuración idéntica, no verá las mismas órdenes disponibles en ambas interfaces. Los relés y Mostro Pubkey visibles en Ajustes (captura de pantalla anterior) deben coincidir con los de su archivo `.env' de CLI.
...
Pulse el botón **"+"** de Green en la parte inferior derecha y seleccione **VENDER** (botón rojo).
...
1. **Divisa**: Seleccione la moneda que desea recibir (EUR, USD, etc.)


2. **Importe** : Introduzca el importe en fiat (por ejemplo, de 1 a 3 EUR)


3. **Métodos de pago** : Elija el método (por ejemplo, "Revolut")


4. **Tipo de precio**: Seleccione "Precio de mercado"
...
Pulse **Submitir** para publicar su pedido.
...
Su pedido ya está publicado Estará disponible durante 24 horas. Puede cancelarlo en cualquier momento antes de que un comprador se lo lleve ejecutando el comando `cancel`.
...
### Paso 3: Un comprador recibe su pedido



Cuando un comprador acepta su pedido, usted recibe una notificación. Ver detalles en **Mis operaciones**.
...
**Instrucción importante**: Ahora debe **pagar la retención Invoice** que se muestra para bloquear sus bitcoins (aquí 4674 Sats por 1-5 EUR) en la custodia. Tienes **15 minutos máximo** de lo contrario la Exchange será cancelada.



Copia la retención Invoice y págala desde tu Wallet Lightning (Phoenix, Breez, etc.).
...
Una vez activada la custodia, pulse **CONTACT** para abrir el chat encriptado con el comprador.



El comprador (aquí "anonymous-gloriaZhao") se pondrá en contacto con usted para solicitarle sus datos de pago. Por favor, responda con sus datos (Revtag, IBAN, etc.).
...
Espere a recibir el pago en fiat en su cuenta Revolut (u otro método acordado). **Compruébelo cuidadosamente**:
...
Una vez que haya **confirmado la recepción** del pago en fiat en su cuenta, pulse el botón Green **RELEASE** y confirme para enviar el Satss al comprador.
...
Los Bitcoins se transfieren instantáneamente al comprador a través de su Invoice Lightning.



### Paso 7: Evaluar la consideración



Después de la liberación, pulse **Calificar** para calificar al comprador. Seleccione de 1 a 5 estrellas (aquí 5/5) y pulse **Submitir valoración**.
...
¡Se acabó la Exchange!



### Comprar bitcoins con la aplicación móvil



El proceso para **comprar** bitcoins es similar pero a la inversa:



1. Navegue por el **Libro de Órdenes** > pestaña **COMPRAR BTC** para ver las órdenes de venta


2. Haga clic en un pedido interesante para ver los detalles


3. Pulse **Tomar pedido**


4. Proporcione su Invoice Lightning (generado a partir de su Wallet)


5. Esperar a que el vendedor active la plica


6. Póngase en contacto con el vendedor a través de **CONTACT** para obtener información sobre el pago
...
9. Recibe bitcoins al instante en tu Lightning Invoice


10. Valore al vendedor (1-5 estrellas)
...
**Cancelar una orden**: En **Mis Operaciones**, pulse su orden y después el botón **CANCELAR** (disponible sólo antes de que se tome).
...
**Soberanía financiera**: Tus bitcoins nunca abandonan tu control directo gracias al mecanismo de retención Invoice, eliminando el riesgo de quiebra o piratería Exchange.
...
Mostro P2P representa una alternativa prometedora a las bolsas centralizadas, combinando Lightning Network, Nostr y escrow automatizado para un comercio verdaderamente descentralizado. A pesar de su incipiente desarrollo y su limitada liquidez, la plataforma ya ofrece soberanía financiera, resistencia a la censura y anonimato por defecto.



Para los bitcoiners que prefieren autonomía y confidencialidad, Mostro es una opción viable digna de exploración progresiva. Su arquitectura descentralizada garantiza la evolución comunitaria en lugar de la comercial, allanando el camino para un futuro de intercambios Bitcoin verdaderamente libres.
