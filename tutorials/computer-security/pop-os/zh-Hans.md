...
| 工具 | 描述 | 安装命令 |
| ---------------------------- | ------------------------------------------ | ----------------------------------------------- |
| Firefox | 自由且流行的网页浏览器 | `sudo apt install firefox` |
| Brave | 注重隐私的网页浏览器 | 通过 Pop!_Shop 或官方网站安装 |
| Visual Studio Code (VS Code) | 功能强大的开发者代码编辑器 | `flatpak install flathub com.visualstudio.code` |
| Git | 版本管理器 | `sudo apt install git` |
| Flatpak | 备选软件包管理器 | `sudo apt install flatpak` |
| VLC | 多功能多媒体播放器 | `sudo apt install vlc` |
| GNOME Terminal | 默认终端 | Pop!OS 已预装 |
| Curl | 在线数据传输工具 | `sudo apt install curl` |
| Wget | 通过 HTTP/FTP 下载文件 | `sudo apt install wget` |
| Docker | 应用容器化 | 通过官方脚本或 `apt` 安装 |
| Node.js | 服务端 JavaScript 运行环境 | 通过 `apt` 或 NodeSource 安装 |
| Python3 | 编程语言 | `sudo apt install python3 python3-pip` |
| GIMP | 高级图像编辑器 | `sudo apt install gimp` |
| Thunderbird | 邮件客户端 | `sudo apt install thunderbird` |
| Transmission | 轻量级 BitTorrent 客户端 | `sudo apt install transmission-gtk` |
| Htop | 交互式系统监视器 | `sudo apt install htop` |
