...
- [**Silent Payments (BIP-352)**](https://BIPs.dev/352/) forbedrer den tidligere [BIP 47 betalingskoder](https://silentpayments.xyz/docs/comparing-proposals/bip47/), også kalt "PayNyms", med gjenbrukbare stealth-adresser. Når en avsender bruker din Silent Payment Address, utleder deres Wallet en unik engangs-Address ved hjelp av forskjellige nøkler som kombineres til en unik engangs-Taproot Address. Blockchain-registrene viser ikke-relaterte transaksjoner, noe som forhindrer kobling av innkommende betalinger. Silent Payments tilbyr en rekke fordeler, blant annet
    - Gjenbrukbare adresser: Du trenger ikke å generate en ny Address for hver transaksjon, noe som gir en bedre brukeropplevelse og økt personvern
    - Null kostnadsøkning: Silent Payments øker ikke størrelsen på eller kostnadene ved transaksjoner.
    - Forbedret anonymitet: Eksterne observatører kan ikke koble transaksjoner til en Silent Payment Address.
    ...
    - Unike adresser for hver betaling: Eliminerer risikoen for utilsiktet gjenbruk av Address.
    - Ingen server er nødvendig: Stille betalinger kan utføres uten behov for en dedikert server.
- PayJoin v2** reduserer transaksjonsgrafanalysen ved å slå sammen inndataene fra avsendere og mottakere til én enkelt transaksjon. Cake Wallet implementerer to viktige fremskritt:
...
2. gjenopprett Wallet (for eksisterende lommebøker)
...
- Swap:** velg `Tor Only` (anonymiserer Exchange-trafikk)
...
Hvis du ønsker å legge til en ekstra sikkerhet Layer, kan du sette opp en `passphrase`.  Hovedformålet med en passphrase er å gi ekstra beskyttelse mot fysiske angrep. Selv om en angriper finner seed-frasen, får han eller hun ikke tilgang til Wallet uten riktig passphrase. Med andre ord representerer seed-frasen alene én Wallet, mens seed-frasen pluss passphrase skaper en helt annen Wallet uten noen forbindelse til den opprinnelige. Denne funksjonen muliggjør også "hemmelige lommebøker" som er beskyttet av passphrase, og gir deg plausibel benektelse. I en tvangssituasjon kan du avsløre seed-frasen mens du holder større verdier trygge i den passphrase-beskyttede Wallet.
...
Hvis du vil lære hvordan du lagrer Mnemonic-frasen din, kan du lese denne veiledningen:
...
- Tilgjengelig saldo
- Silent Payments Card for å begynne å skanne etter transaksjoner sendt til Silent Payment Address
- PayJoin-kort for å "aktivere" PayJoin som en personvernbevarende og avgiftsbesparende funksjon
- nederst er det snarveier til `Wallet Overview`, `Receive`, `Swap` mellom Bitcoin og andre valutaer, `Send` og `Buy`
...
Vi kan slå på Silent Payments-kortet på startskjermen for å vise denne funksjonen. Ved å aktivere "Alltid skanning" kan Wallet kontinuerlig overvåke Blockchain for innkommende Stille betalinger. Vi kan spesifisere skanneparametere for å tilpasse skanneprosessen til våre behov som beskrevet ovenfor.
...
Det finnes flere alternativer og Address-typer for mottak av Bitcoin. `SegWit (P2WPKH)` *(starter med bc1q....)* er standardalternativet.  La oss velge `Silent Payments` i dette eksempelet.


For å motta en Silent Payment trykker du først på `Mottak`-ikonet i Cake Wallet. Deretter skriver du inn beløpet du forventer å motta. For å spesifisere Address-typen trykker du på `Mottak` øverst på skjermen igjen, og velger deretter `Silent Payments` blant alternativene.


På hovedskjermen vises din gjenbrukbare Silent Payment QR-kode og Address. Som forventet er Address ganske lang:
...
Bruk nå en BIP-352-kompatibel Wallet (for eksempel Blue Wallet) til å skanne denne QR-koden og sende betalingen. Du vil se at Wallet utleder en unik destinasjon Address fra din stille Address.
...
- enten ved å lime inn vår gjenbrukbare `sp1qq...` Address eller ved å skanne QR-koden direkte i appen.
...
Når vi har angitt `sp1qq...` Address, utleder Wallet automatisk en tilsvarende `bc1p...` Taproot Address (P2TR) i bakgrunnen, som vil bli brukt til Silent Payment.
...
Når du har sendt transaksjonen, blir du spurt om du ønsker å legge til denne kontakten i Address-boken din.
...
For å bruke PayJoin trenger begge parter en PayJoin-kompatibel Wallet, og mottakeren må ha minst én Coin eller utgang i sin Wallet. Følg disse trinnene for å starte:


1. Trykk på `Hamburger Menu` og trykk på `Privacy`-knappen

2. Slå på og av alternativet `Bruk PayJoin`
...
Dette er vår gjennomgang av Cake Wallet, som tilbyr praktisk Bitcoin-personvern takket være funksjoner som Silent Payments (BIP-352) og PayJoin v2.
...
PayJoin v2 forstyrrer kjedeanalysen ved å slå sammen avsender- og mottakerinput til én transaksjon uten ekstra avgifter eller sentral koordinering. Dette bryter med den felles input-Ownership-heuristikken, noe som er en betydelig fordel ettersom det betyr at du ikke kan anta at alle input tilhører avsenderen.


For brukere som prioriterer økonomisk anonymitet, er Cake Wallet et levedyktig alternativ. Den inkorporerer personvernprotokoller direkte i kjernefunksjonaliteten, noe som gjør dem tilgjengelige uten teknisk kompleksitet. Etter hvert som overvåkningen av offentlige blokkjeder øker, bidrar verktøy som dette til å opprettholde transaksjoners personvern der det betyr mest. En bredere implementering av disse standardene i Wallet-landskapet ville være en velkommen utvikling.
