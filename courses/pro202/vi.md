...
Nhân bất kỳ phần tử k nào khác 0 với tất cả các phần tử của một trường số nguyên tố sẽ tạo ra một hoán vị của trường đó. Thuộc tính này đảm bảo tính đồng nhất và ngăn ngừa các điểm yếu về cấu trúc, khiến các trường số nguyên tố trở nên lý tưởng cho việc tạo khóa an toàn và [chữ ký số](https://planb.academy/resources/glossary/digital-signature).
...
Vì vậy, nghịch đảo là n^(p–2). Python hỗ trợ trực tiếp điều này với `pow(n, -1, p)`. Các phép toán này xuất hiện liên tục trong các thuật toán mã hóa cơ bản của [ECDSA](https://planb.academy/resources/glossary/ecdsa) và Bitcoin.
...
Chương này giới thiệu các đường cong elip được định nghĩa trên trường hữu hạn và giải thích lý do tại sao chúng tạo thành xương sống toán học của [mật mã](https://planb.academy/resources/glossary/cryptography) Bitcoin. Trong khi các đường cong elip trên tập số thực trông mượt mà và liên tục, việc áp dụng cùng các phương trình đó trên trường hữu hạn tạo ra một tập hợp các điểm rời rạc, phân tán. Mặc dù có sự khác biệt về mặt hình ảnh, tất cả các công thức cộng điểm, độ dốc và quy tắc đại số đều hoạt động chính xác như nhau—chỉ có phép toán số học thay đổi thành phép toán số học modulo. Bitcoin sử dụng đường cong y² = x³ + 7 (secp256k1), bảo toàn tính đối xứng và hành vi phi tuyến tính cần thiết cho bảo mật mật mã.
...
Điều này làm tăng chi phí xử lý, mở rộng chữ ký 64 byte lên khoảng 71–72 byte. [Taproot](https://planb.academy/resources/glossary/taproot) loại bỏ sự thiếu hiệu quả này bằng cách áp dụng chữ ký [Schnorr](https://planb.academy/resources/glossary/schnorr-protocol) có kích thước cố định.
...
Các tham chiếu đầu vào đề cập đến các [UTXO](https://planb.academy/resources/glossary/utxo) trước đó bằng mã băm giao dịch và chỉ mục đầu ra của chúng, và bao gồm một [kịch bản](https://planb.academy/resources/glossary/script) mở khóa (scriptSig) và một số thứ tự được sử dụng cho khóa thời gian hoặc RBF. Các tham chiếu đầu ra chỉ định số tiền (8 byte) và kịch bản khóa (scriptPubKey), xác định các điều kiện chi tiêu. Địa chỉ Bitcoin là biểu diễn của các kịch bản này.


#### Mô hình UTXO

Bitcoin theo dõi các UTXO chưa được sử dụng thay vì số dư tài khoản. UTXO phải được sử dụng toàn bộ — việc sử dụng một phần là không thể. Để sử dụng 1 BTC từ 100 BTC trong UTXO, giao dịch phải bao gồm một UTXO trả lại tiền thừa. Nếu quên UTXO trả lại tiền thừa, số tiền còn lại sẽ trở thành phí [khai thác](https://planb.academy/resources/glossary/mining).
...
Bất kỳ giá trị nào chưa được gán sẽ trở thành phí, do đó việc xây dựng đầu ra tiền thừa chính xác là rất cần thiết. Trước [SegWit](https://planb.academy/resources/glossary/segwit), chữ ký cho phép sự thay đổi linh hoạt—việc sửa đổi S thành N-S tạo ra một giao dịch hợp lệ mới với ID khác. Bitcoin hiện thực thi quy tắc S thấp, và SegWit tách biệt chữ ký khỏi phép tính txid, làm cho ID ổn định và cho phép các giao thức lớp thứ hai như [Lightning](https://planb.academy/resources/glossary/lightning-network).
...
Các [khối](https://planb.academy/resources/glossary/block) Bitcoin nhóm các giao dịch lại với nhau và bảo mật chúng bằng [proof of work](https://planb.academy/resources/glossary/proof-of-work). Mỗi khối bao gồm một [tiêu đề](https://planb.academy/resources/glossary/block-header) 80 byte cộng với một danh sách các giao dịch. Mặc dù việc tạo ra một khối hợp lệ tốn nhiều năng lượng, nhưng việc xác minh một khối lại rất rẻ: lưu trữ tất cả khoảng 900.000 tiêu đề chỉ cần khoảng 72 MB, cho phép ngay cả các thiết bị nhỏ cũng có thể xác minh proof of work của chuỗi một cách hiệu quả.


### Giao dịch Coinbase và phần thưởng khối


Mỗi khối bắt đầu bằng chính xác một [giao dịch Coinbase](https://planb.academy/resources/glossary/coinbase-transaction)—đây là cách duy nhất để bitcoin mới được đưa vào lưu thông. Nó có mã băm prev-tx bằng 0 và chỉ mục là 0xffffffff, xác định duy nhất nó. Phần thưởng bắt đầu ở mức 50 BTC và giảm một nửa sau mỗi 210.000 khối (50, 25, 12,5, 6,25, 3,125, …). Thợ đào cũng bao gồm phí giao dịch. Vì nonce 4 byte quá nhỏ đối với các ASIC hiện đại, thợ đào sửa đổi dữ liệu trong giao dịch Coinbase để thay đổi gốc [Merkle](https://planb.academy/resources/glossary/merkle-tree) và tạo thêm không gian tìm kiếm. [BIP34](https://planb.academy/resources/glossary/bip) yêu cầu nhúng chiều cao khối vào scriptSig của Coinbase để đảm bảo mỗi Coinbase txid là duy nhất.
...
- bit (mục tiêu [độ khó](https://planb.academy/resources/glossary/difficulty), 4 byte)
- nonce (4 byte)


Số phiên bản đã phát triển thành một hệ thống tín hiệu trường bit thông qua BIP9, cho phép các thợ đào phối hợp sự sẵn sàng cho [soft-fork](https://planb.academy/resources/glossary/soft-fork). Dấu thời gian là giá trị thời gian Unix 32 bit và sẽ cần được cập nhật vào khoảng năm 2106.
...
Bitcoin lưu trữ một gốc Merkle 32 byte duy nhất trong mỗi tiêu đề khối như một cam kết cho tất cả các giao dịch trong khối. Các giao dịch được [băm](https://planb.academy/resources/glossary/hash-function) (hash256), ghép cặp, nối lại và băm lặp đi lặp lại cho đến khi chỉ còn một giá trị băm duy nhất. Khi một cấp có số lượng lẻ, giá trị băm cuối cùng sẽ được sao chép. Về mặt nội bộ, các giá trị băm là big-endian, trong khi dữ liệu khối được tuần tự hóa sử dụng little-endian, yêu cầu đảo ngược trước và sau khi xây dựng cây.


#### Merkle Proofs và SPV

Bằng chứng Merkle cho phép xác minh rằng một giao dịch được bao gồm trong một khối mà không cần tải xuống toàn bộ khối. Bằng chứng bao gồm các hàm băm anh em dọc theo đường dẫn đến gốc. Các máy khách SPV nhẹ chỉ lưu trữ tiêu đề khối và yêu cầu các bằng chứng này từ các [nút đầy đủ](https://planb.academy/resources/glossary/full-node). Vì cây Merkle phát triển theo hàm logarit, việc chứng minh sự có mặt trong một khối với hàng nghìn giao dịch chỉ cần vài trăm byte.
