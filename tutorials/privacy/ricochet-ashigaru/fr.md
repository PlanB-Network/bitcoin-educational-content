...
Le Ricochet est une technique consistant à réaliser plusieurs transactions fictives vers soi-même pour simuler un transfert de propriété des bitcoins. Cet outil est différent des autres transactions d'Ashigaru (héritées de Samourai Wallet), car il ne permet pas de gagner de l'anonymat prospectif, mais plutôt une forme d'anonymat rétrospectif. En effet, Ricochet permet d'estomper les spécificités pouvant compromettre la fongibilité d'une pièce Bitcoin.
...
## Comment fonctionne Ricochet sur Ashigaru ?

Le Ricochet est simplement une méthode où l'on s'envoie des bitcoins à soi-même, initialement inventé par les développeurs de Samourai Wallet. Il est donc tout à fait possible de simuler un Ricochet manuellement sans faire appel à un outil spécialisé. Cependant, pour ceux qui souhaitent automatiser le processus tout en bénéficiant d'un résultat plus soigné, l'outil Ricochet disponible via l'application Ashigaru (qui est un fork de Samourai) représente une bonne solution.
