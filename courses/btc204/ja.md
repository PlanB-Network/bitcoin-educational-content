...
その後、トランザクションは "capitulation "フェーズでネットワーク全体に配信される。

![BTC204](assets/fr/206.webp)

茎と花の頭は、ネットワークを通じてトランザクションが伝播する挙動を表しており、タンポポの形に似ている。

したがって、スパイノードは降伏フェーズ（マスブロードキャスト）を開始したノードまでトランザクションを追跡できる可能性があるが、このノードはステム上の最後のノードからトランザクションを受信したため、最初にブロードキャストしたノードではない。スパイノードがステムを追跡できなければ、ソースノードを特定することもできない。
