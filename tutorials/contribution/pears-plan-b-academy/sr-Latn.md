...
U ovom vodiču ćemo naučiti kako pokrenuti platformu **Plan ₿ Academy** na zaista otporan način na cenzuru koristeći **Pears**, peer-to-peer (P2P) tehnologiju koju je razvio **Holepunch** i podržava **Tether**.


Pears je softver koji nam omogućava da pokrećemo platformu Plan ₿ Academy bez oslanjanja na centralizovani vebsajt. U ovom uputstvu ćemo instalirati Pears na vašem računaru kako bismo pristupili Plan ₿ Academy preko Pears-a.


Cilj Pears-a je jednostavan: omogućiti distribuciju i korišćenje veb aplikacija bez oslanjanja na bilo kakvu centralizovanu infrastrukturu (bez servera, bez hostova, bez posrednika). Drugim rečima, čak i ako provajder u oblaku prestane sa radom ili neka zemlja blokira domen, aplikacija nastavlja da živi među mrežnim čvorovima. Ovaj pristup omogućava našoj obrazovnoj platformi, Plan ₿ Academy, da ostane dostupna širom sveta, bez jedne tačke otkaza.
...
- Pokrenite sledeću komandu da biste pokrenuli aplikaciju Plan ₿ Academy:
...
- Hyperbee**: indeks izgrađen na vrhu Hypercore-a koji omogućava da se podaci organizuju i pretražuju efikasno.
...
*Ovaj vodič za instalaciju Pears-a je podeljen na više sekcija u zavisnosti od vašeg operativnog sistema. Idite direktno na onu koja odgovara vašem okruženju kako biste pratili odgovarajuća uputstva:*



- Linux (Debian)** → Sekcija **1.2**
- Windows** → Section **1.3**
- macOS** → Sekcija **1.4**


### 1.2. Kako instalirati Pears na Linux (Debian)?
...
#### 1.2.3. Instalirajte Node.js i npm putem NVM


Pears se distribuira putem *npm*, menadžera paketa za *Node.js*. Iako Pears ne zavisi direktno od *Node.js* za rad, on je potreban za instalaciju. Preporučeni način za instalaciju *Node.js* na Linuxu je putem *NVM* (*Node Version Manager*), koji vam omogućava upravljanje višestrukim verzijama Node-a istovremeno.
...
Proverite da je *NVM* pravilno instaliran:
...
Ako je sve ispravno instalirano, meni za pomoć Pears će se pojaviti sa spiskom dostupnih komandi.


#### 1.2.6. Test Kruške sa Keet
...
### 1.3. Kako instalirati Pears na Windows


Instaliranje Pears na Windows je jednako jednostavno kao i na Linuxu, ali zahteva nekoliko specifičnih alata.


*Ako koristite Linux i već ste instalirali Pears, možete preskočiti direktno na **Korak 2**.*


#### 1.3.1. Otvorite PowerShell kao Administrator
...
#### 1.3.5. Instalirajte Pears pomoću npm-a
...
#### 1.3.6. Verifikuj i inicijalizuj kruške
...
### 1.4. Kako instalirati Pears na macOS
...
*Ako koristite Linux ili Windows i već ste instalirali Pears, možete odmah preći na **Korak 2**.*


#### 1.4.1. Proveri sistemske preduslove
...
#### 1.4.3. Instalirajte Node.js i npm
...
#### 1.4.4. Instalirajte Pears pomoću npm-a


Kada je *npm* dostupan, možete globalno instalirati Pears CLI na vaš sistem. Ovo će vam omogućiti da izvršavate komandu `pear` iz bilo kog direktorijuma.
...
Prilikom prvog pokretanja, Pears se povezuje na peer-to-peer mrežu kako bi preuzeo potrebne komponente. Ovaj proces ne zahteva centralni server — fajlovi se preuzimaju direktno od drugih korisnika.
...
#### 1.4.6. Testirajte kruške sa Keet
...
## 2. Kako koristiti Plan ₿ Akademiju na Pears


Jednom kada je Pears instaliran i pokrenut, možete direktno pokrenuti platformu **Plan ₿ Academy** putem P2P mreže. Jednostavno pokrenite sledeću komandu u vašem terminalu (ista komanda radi na Linux, Windows i macOS):
...
## 3. Kako planirati seed ₿ Akademiju na Pears


U Pears mreži, *seed* aplikaciju znači redistribuirati je drugim peer-ovima sa sopstvene mašine. U praksi, kada seed Plan ₿ Akademiju, vaš računar postaje izvor podataka koji omogućava drugim korisnicima da preuzmu aplikaciju bez oslanjanja na centralni server.
...
Hvala vam što doprinosite decentralizovanoj distribuciji Plan ₿ Akademije na Pears i pomažete da Bitcoin obrazovanje postane zaista otporno na cenzuru!