...
在比特币上，我们实际交换的不是直接的比特币，而是**[UTXO](https://planb.academy/resources/glossary/utxo)**（_Unspent Transaction Outputs_，未花费的交易输出）。
...
在本章中，我们将发现Lightning是如何允许支付通过中间节点进行传输，而无需信任它们，这要归功于**[HTLC](https://planb.academy/resources/glossary/htlc)**（_哈希时间锁定合约_）。这些智能合约确保每个中间节点只有在将支付转发给最终接收者后才能从其通道中获得资金，否则，支付将不会被验证。
