...
Tento průvodce se zabývá [**Cake Wallet**](https://cakewallet.com/): open-source, necustodialní, na soukromí zaměřený multiměnový Wallet dostupný pro Android, iOS, macOS, Linux a Windows. Ponoříme se do jeho funkcí specifických pro ochranu soukromí Bitcoin, projdeme si odesílání/přijímání Bitcoin prostřednictvím **Tichých plateb** (vylepšený protokol pro ochranu soukromí On-Chain) a podíváme se na implementaci PayJoin v2 pro asynchronní transakce.
...
- [**Tiché platby (BIP-352)**](https://BIPs.dev/352/) vylepšující předchozí [BIP 47 platebních kódů](https://silentpayments.xyz/docs/comparing-proposals/bip47/) nazývané také "PayNyms" o opakovaně použitelné skryté adresy. Když odesílatel použije váš Tichý platební kód Address, jeho kód Wallet získá jedinečný jednorázový kód Address pomocí různých klíčů, které se spojí do jedinečného jednorázového kódu Taproot Address. Záznamy Blockchain zobrazují nesouvisející transakce, což zabraňuje propojení příchozích plateb. Tiché platby nabízejí řadu výhod, mezi něž patří např:
    ...
    - Zvýšená anonymita: Vnější pozorovatelé nemohou spojit transakce s tichou platbou Address.
    - Není nutná interakce mezi odesílatelem a příjemcem: Transakce lze provádět bez jakékoliv komunikace mezi stranami.
    - Jedinečné adresy pro každou platbu: Eliminuje se riziko náhodného opakovaného použití Address.
    - Není vyžadován žádný server: Tiché platby lze provádět bez potřeby vyhrazeného serveru.
- PayJoin v2** zmírňuje analýzu transakčního grafu sloučením vstupů odesílatelů a příjemců do jedné transakce. Dort Wallet zavádí dva zásadní pokroky:
...
1. `Vytvořit nový Wallet` (pro nové uživatele)
...
- Swap:** vyberte `Tor Only` (anonymizuje provoz Exchange)
...
Chcete-li se seznámit s osvědčenými postupy pro ukládání fráze Mnemonic, přečtěte si tento návod:


https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

Ťukněte na položku `Rozumím. Ukažte mi můj seed` a uložte tato slova na bezpečné místo! Poté klepněte na `Ověřit seed` a po ověření na `Otevřít Wallet`.
...
- Dostupný zůstatek
- Karta Tiché platby pro zahájení skenování transakcí odeslaných na vaši kartu Tiché platby Address
...
Zde můžeme znovu připojit Wallet, spravovat uzly a připojit se k vlastnímu uzlu (doporučeno). Funkce `Tiché skenování plateb` nám umožňuje přizpůsobit skenování zadáním buď možnosti `Skenovat z výšky BLOCK`, nebo `Skenovat z data`.
...
Tuto funkci můžeme zobrazit přepnutím karty Tiché platby na domovské obrazovce. Povolení možnosti `Vždy skenovat` umožňuje zařízení Wallet nepřetržitě monitorovat zařízení Blockchain pro příchozí Tiché platby. Můžeme zadat parametry skenování a přizpůsobit tak proces skenování našim potřebám, jak je popsáno výše.
...
Abychom zabezpečili naši službu Wallet, můžeme vytvořit zálohu podle pokynů v aplikaci. Tím zajistíme, že budeme mít bezpečnou kopii svých soukromých klíčů, což nám umožní obnovit naši jednotku Wallet v případě její ztráty nebo krádeže. Kromě toho si můžeme zobrazit naši frázi seed a soukromé klíče, změnit kód PIN, povolit biometrické ověřování, Podepsat/Ověřit a nastavit 2FA pro dodatečnou ochranu Layer.
...
Pro příjem Bitcoin je k dispozici několik možností a typů Address. `SegWit (P2WPKH)` *(počínaje bc1q....)* je výchozí volbou.  V tomto příkladu vybereme `Tiché platby`.


Chcete-li přijmout tichou platbu, klepněte nejprve na ikonu `Přijmout` v aplikaci Cake Wallet. Poté zadejte částku, kterou očekáváte obdržet. Chcete-li zadat typ Address, znovu klepněte na ikonu `Přijmout` v horní části obrazovky a z možností vyberte možnost `Tiché platby`.


Na hlavní obrazovce se zobrazí váš opakovaně použitelný kód QR pro tichou platbu a kód Address. Jak se dalo očekávat, Address je poměrně dlouhý:
...
Nyní pomocí kompatibilního zařízení BIP-352 (například modrého Wallet) naskenujte tento QR kód a odešlete platbu. Uvidíte, že zařízení Wallet získá z vašeho tichého zařízení Address jedinečné cílové zařízení Address.
...
Protože modrý Wallet umí pouze "odesílat" tiché platby, použijeme jako přijímající stranu jiný kompatibilní BIP 352. Tento proces je totožný s běžnou transakcí Bitcoin.



- Klepněte na možnost `Odeslat` na domovské obrazovce
- buď vložením našeho opakovaně použitelného kódu `sp1qq...` Address, nebo naskenováním QR kódu přímo v aplikaci.
- Zvolte, kolik chcete utratit ze svého dostupného zůstatku
- Klepnutím na `Odeslat` v dolní části obrazovky potvrďte transakci


Jakmile zadáme `sp1qq...` Address, Wallet na pozadí automaticky odvodí odpovídající `bc1p...` Taproot Address (P2TR), který bude použit pro Tichou platbu.
...
Po odeslání transakce budete dotázáni, zda si přejete přidat tento kontakt do své knihy Address.
...
Zopakujme si, co je PayJoin [o](https://docs.cakewallet.com/cryptos/Bitcoin/#PayJoin):


_Payjoin v2 je funkce Bitcoin, která zachovává soukromí a šetří poplatky a umožňuje odesílateli a příjemci transakce spolupracovat při vytváření jediné transakce. Tato transakce má vstupy od *obou* odesílatele i příjemce, což prolamuje nejběžnější techniky sledování proti Bitcoin a umožňuje lepší škálování a za určitých okolností také úsporu poplatků._
...
Pro použití PayJoin potřebují obě strany PayJoin kompatibilní Wallet a příjemce musí mít alespoň jeden Coin nebo výstup ve svém Wallet. Chcete-li začít, postupujte podle následujících kroků:
...
3.  Klepněte na `Přijmout` na domovské obrazovce a zobrazí se kód QR PayJoin a tlačítko pro kopírování (při výběru SegWit)
...
Přinášíme recenzi produktu Cake Wallet, který díky funkcím jako Tiché platby (BIP-352) a PayJoin v2 nabízí praktické soukromí Bitcoin.
...
PayJoin v2 narušuje analýzu řetězců tím, že spojuje vstupy odesílatele a příjemce do jedné transakce bez dalších poplatků nebo centrální koordinace. To narušuje společnou heuristiku vstupů Ownership, což je významná výhoda, protože to znamená, že nelze předpokládat, že všechny vstupy patří odesílateli.


Pro uživatele, kteří upřednostňují finanční anonymitu, je Cake Wallet vhodnou volbou. Zahrnuje protokoly o ochraně soukromí přímo do svých základních funkcí, takže jsou přístupné bez jakýchkoli technických složitostí. Vzhledem k tomu, že se zvyšuje dohled nad veřejnými blockchainy, pomáhají nástroje, jako je tento, zachovat soukromí transakcí tam, kde je to nejdůležitější. Širší implementace těchto standardů v rámci prostředí Wallet by byla vítaným vývojem.
