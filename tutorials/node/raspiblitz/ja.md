...
[GitHubページのガイド](https://github.com/rootzoll/raspiblitz)は優れていますが、中程度の経験を持つユーザーには詳細すぎる可能性があります。私の指示はより簡潔で、できればよりわかりやすくなるでしょう。

基本的に、このプロセスはRaspberry Pi 4で[MyNodeノード](https://armantheparman.com/mynode-bitcoin-node-easy-setup-guide-raspberry-pi/)をセットアップするプロセスと非常によく似ています。Raspiblitzガイドではモニターを購入するように勧めていますが、実際には必要ありませんし、私はお勧めしません。追加のキーボードやマウスも必要ありません。同じ家庭内ネットワーク上のコンピュータからデバイスのターミナルメニューにアクセスし、ターミナルを使用してsshコマンドを使うだけです。これはLinux/Mac（簡単）で可能で、Windowsでは少し難しいです。
...
ダウンロードしたファイルのsha-256ハッシュはウェブサイトに記載されています。これはアップデートのたびに変わります。これが何のことか理解できない場合、理解すべきですので、私は[ここで読めるガイドを書きました。](https://armantheparman.com/gpg/)
...
Windowsの場合、Piにssh接続するために [putty](http://putty.org/) をインストールする必要があります。上記と同じコマンドを入力してください。
