...
Ce guide vous explique l'installation, la configuration et l'utilisation de Bull Bitcoin Wallet. Vous apprendrez à envoyer et à recevoir des fonds sur les réseaux Bitcoin on-chain, Liquid et Lightning, ainsi qu'à déplacer des bitcoins entre eux. Les nombreuses fonctionnalités de ce portefeuille en font un outil puissant et tout-en-un pour la gestion de vos bitcoins. Commençons.
...
Bull Bitcoin Wallet vous permet également d'envoyer et de recevoir des paiements via le Lightning Network. Une caractéristique clé est que les fonds reçus via Lightning sont automatiquement échangés et stockés sur `Liquid` au sein de votre portefeuille `Paiements instantanés`. Ce service est alimenté par `Boltz`. 
...
Vous pouvez accéder à l'écran d'envoi directement depuis la page d'accueil ou depuis n'importe lequel de vos portefeuilles. Bull Bitcoin Wallet simplifie le processus en détectant automatiquement le réseau de destination - `Bitcoin`, `Liquid` ou `Lightning` - en fonction de l'adresse ou de la facture que vous saisissez, qu'elle soit collée ou scannée via un QR code.
...
Bull Bitcoin Wallet envoie des paiements Lightning en retirant des fonds de votre portefeuille `Paiements instantanés` (sur Liquid) et en les échangeant via `Boltz`. Cette approche hybride est entièrement autonome et évite les frais élevés de gestion d'un canal Lightning dédié, mais elle nécessite de payer des "frais d'échange". Pour le coût le plus bas, envoyez directement à l'adresse Liquid d'un destinataire s'il utilise également un Bull Bitcoin wallet.
...
En connectant un Hardware Wallet à l'application Bull Bitcoin, vous obtenez le meilleur des deux mondes : la sécurité sans compromis du stockage à froid de vos clés privées, combinée aux puissantes fonctions et à l'interface conviviale de Bull Bitcoin Wallet pour la consultation des soldes et la gestion des transactions. Dans ce dernier chapitre, nous vous montrerons comment connecter un Hardware Wallet, tel qu'un [Coldcard Q](https://coldcard.com/q), à votre Bull Bitcoin Wallet. Ce tutoriel n'aborde pas en profondeur la configuration d'un Coldcard Q ; vous pouvez en savoir plus à ce sujet ici :
...
Ouvrez Bull Bitcoin Wallet et naviguez vers `Paramètres` > `Paramètres Bitcoin` > `Importer un portefeuille` et sélectionnez `Coldcard Q` sur votre téléphone et appuyez sur `Ouvrir la caméra` pour scanner le QR code afin d'importer les clés publiques de votre Hardware Wallet.
...
Pour recevoir du bitcoin en utilisant votre Coldcard Q, vous n'avez pas besoin que l'appareil soit physiquement connecté à votre téléphone. Bull Bitcoin Wallet a déjà importé les clés publiques nécessaires, ce qui lui permet de gérer les adresses de réception par lui-même.
...
L'envoi de bitcoin avec votre Coldcard Q nécessite votre confirmation physique pour autoriser toute transaction. Bien que l'application Bull Bitcoin Wallet soit utilisée pour créer la transaction, la signature finale ne peut être créée que sur le Hardware Wallet lui-même.


Pour commencer, ouvrez votre wallet `Coldcard Q` et tapez sur `Envoyer`. Ensuite, ouvrez la caméra pour scanner le QR code de l'adresse de réception. Après avoir scanné, entrez le montant que vous voulez envoyer et ajustez la priorité des frais si nécessaire.
