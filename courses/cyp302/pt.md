---
name: Bases da criptografia moderna
...
---
# Mergulho profundo na criptografia moderna

Neste curso, abordaremos os fundamentos da criptografia moderna de forma clara e acessível, sem exigir uma formação matemática avançada. Ao longo dos capítulos, você aprenderá conceitos centrais como criptografia simétrica e de chave pública, funções hash, assinaturas digitais, troca de chaves e protocolos do mundo real. Pelo caminho, conectaremos os pontos a aplicações práticas como mensagens seguras, TLS, armazenamento de senhas e autenticação.

O material é projetado para alunos de todos os níveis e equilibra intuição com profundidade técnica suficiente para satisfazer a curiosidade. Espere uma jornada focada e envolvente. Ao final, você entenderá como e por que a criptografia moderna funciona e como usá-la de forma responsável.
+++
# Introdução
...
Este conteúdo educacional é adaptado do livro e repo [JWBurgers](https://github.com/JWBurgers/An_Introduction_to_Cryptography). Embora o autor tenha gentilmente permitido seu uso educacional, todos os direitos de propriedade intelectual permanecem com o criador original.
...
Comecemos a nossa investigação no domínio da [criptografia](https://planb.academy/resources/glossary/cryptography) com um dos episódios mais encantadores e divertidos da sua história: o das cifras de Beale. [1]
...
Em segundo lugar, os esquemas de encriptação clássicos, como as cifras de Beale, envolvem sempre uma chave que foi partilhada entre todas as partes relevantes. No entanto, muitos esquemas criptográficos modernos envolvem não apenas uma, mas duas chaves: uma **privada** e uma **[chave pública](https://planb.academy/resources/glossary/public-key)**. Enquanto a primeira deve permanecer privada em todas as aplicações, a segunda é normalmente do conhecimento público (daí os seus respectivos nomes). No domínio da encriptação, a chave pública pode ser utilizada para encriptar a mensagem, enquanto a [chave privada](https://planb.academy/resources/glossary/private-key) pode ser utilizada para a desencriptação.
...
Os esquemas baseados em logaritmos discretos baseiam-se em dois tipos principais de grupos cíclicos: grupos multiplicativos de inteiros e grupos que incluem os pontos em [curvas elípticas](https://planb.academy/resources/glossary/elliptic-curve). A troca de chaves Diffie Helmann original, apresentada em "New Diretions in Cryptography", funciona com um grupo cíclico multiplicativo de inteiros. O algoritmo de [assinatura digital](https://planb.academy/resources/glossary/digital-signature) da Bitcoin e o esquema de [assinatura Schnorr](https://planb.academy/resources/glossary/schnorr-protocol) recentemente introduzido (2021) baseiam-se ambos no problema do logaritmo discreto para um determinado grupo cíclico de curvas elípticas.
...
Um exemplo de uma função de hash popular é o **[SHA-256](https://planb.academy/resources/glossary/sha256)** (algoritmo de hash seguro 256). Independentemente do tamanho da entrada (por exemplo, 15 bits, 100 bits ou 10.000 bits), esta função produzirá um valor de hash de 256 bits. Abaixo pode ver alguns exemplos de resultados da função SHA-256.
