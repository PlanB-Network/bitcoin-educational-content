...
- Este tutorial explica como utilizar a aplicação móvel **Blockstream App** para gerir um Bitcoin **onchain** Wallet, ou seja, transacções registadas diretamente no Blockchain Bitcoin principal.
- Abrange a instalação, a configuração inicial, a criação de um Software Wallet e as operações de receção e envio de bitcoins.
- Nota: Outros tutoriais nos apêndices abrangem Liquid, Watch-Only e a versão para computador.
...
### 1.3. Lembretes sobre as carteiras Hot
...
- (2) **"Restore from Backup "**: Importar um portefólio existente utilizando uma frase Mnemonic (12 ou 24 palavras). Atenção: Não importar uma frase Cold Wallet, pois esta ficará exposta num dispositivo ligado, invalidando a sua segurança.
- (3) **"Watch-Only "**: Importar uma carteira só de leitura existente, para ver o saldo (por exemplo, do seu Cold Wallet) sem expor a frase Mnemonic. Ver o tutorial "Watch Only" no apêndice.
...
O seu Wallet é criado automaticamente e a página inicial do Wallet, aqui denominada "My Wallet 5", é apresentada:
...
**Importante**: A Blockstream App simplificou a criação de um Wallet ao não exibir automaticamente a frase de 12 palavras seed. *Embora a carteira seja agora criada com um clique, corre o risco de perder o acesso aos seus fundos se não guardar a sua frase seed*.
...
- No ecrã inicial do Wallet, clique no separador "Security" (Segurança) e, em seguida, na mensagem "Back Up" (Cópia de segurança) ou no menu "Recovery Phrase" (Frase de recuperação):
...
A frase de 12 palavras do seed será apresentada para ser guardada.
...
Para o fazer, vamos escrever uma referência, apagar o Wallet, restaurá-lo com a cópia de segurança e verificar se a referência se mantém inalterada.





- No ecrã inicial do Wallet, clique no separador "Settings" (Definições), depois em "Wallet Details" (Detalhes do Wallet) e copie o zPub ([extended public key](https://planb.network/courses/46b0ced2-9028-4a61-8fbc-3b005ee8d70f/8dcffce1-31bd-5e0b-965b-735f5f9e4602):
...
- Apague a aplicação e, em seguida, restaure o Wallet através de **"Restore from Backup "** introduzindo a frase Mnemonic e verifique se o zpub se mantém inalterado. Se sim, então a sua cópia de segurança está correta e pode enviar fundos para o Wallet.
...
### 4.5. Garantir o acesso à aplicação
...
- No ecrã inicial do Wallet, vá para **"Segurança "** e clique em **"PIN "**
...
- A aplicação apresenta um Address** de receção **em branco (formato SegWit v0, começando por `bc1q...`). A utilização de um novo Address de cada vez que recebe um Bitcoin melhora a sua confidencialidade.
...
- Opções** :
    ...
    - (4) Para solicitar um montante específico, clique nos três pontos no canto superior direito, selecione "Request amount" e introduza o montante pretendido. O QR será atualizado e o Address será substituído por um URI de pagamento Bitcoin.
...
O montante mínimo (limite de descarga) em 03/08/2025 é de 546 Sats.




    - (4) Selecionar **taxas de transação** :
        - Escolha entre as opções sugeridas (por exemplo, rápido, médio, lento), consoante a urgência, e será apresentado um tempo de transferência aproximado.
        - Para taxas personalizadas, ajuste manualmente o número de Satoshi por vbytes (consulte [Mempool.space](https://Mempool.space/) para obter as taxas de mercado).
...
- Desde que a transação não tenha sido confirmada, a função "Replace by fee" (ver Apêndice) permite-lhe acelerar o seu tratamento aumentando as taxas de transação:
...
### A2. Explicação da Replace-by-fee (RBF)
...
- A transação inicial deve ser marcada como compatível com o RBF quando é criada, o que a Blockstream App faz automaticamente.
