---
name: 莫斯特羅
description: 透過 Lightning 和 Nostr 的免 KYC Bitcoin P2P Exchange 平台
---
...
如何在不損害您的金融主權的前提下獲取或出售比特幣？集中式平台會強加侵入性的 KYC 程序，暴露您的個人資料，並有可能任意凍結帳戶或進行國家監控。



**Mostro P2P** 提供了結合 Lightning Network、Nostr 協定和持有發票的分散式替代方案。它的主要創新：一個自動化的託管系統，您的比特幣在整個 Exchange 期間都在您的控制之下，消除了盜竊、Exchange 破產或任意沒收的風險。
...
Mostro P2P 是一個開放原始碼的 Bitcoin Exchange 協定，由 2021 年推出的熱門 Telegram 機器人 **lnp2pbot** 的開發者 **grunch** 所創造。雖然 lnp2pbot 已經透過 Lightning 啟用免 KYC 的 P2P 交換，但它卻提出了一個重大的漏洞： **Telegram 構成集中式故障點**，容易受到政府的審查。
...
訂單在 Nostr 上公佈為公開事件 (類型 38383)，而私人交易則透過端對端加密訊息 (NIP-59、NIP-44) 傳送。每個 Mostro 實例都定義了自己的費用（一般在 0.3% 到 1% 之間）和交易限額（從幾千到幾十萬 Sats 不等，視實例而定）。
...
**Reputation system**：透過迭代計算的 5 星評級永久儲存於 Nostr。讓您逐漸建立可靠交易者的聲譽。



**分散仲裁**：發生爭議時，公正的仲裁員會審查證據，並根據透明的標準做出決定。每項爭議都會產生唯一的 token，以便追蹤。



**付款彈性**：透過 yadio.io Exchange 匯率服務支援多種法定貨幣。接受 SEPA 轉帳、PayPal、Revolut、現金以及雙方同意的任何方式。
...
**Mostro CLI** 是最成熟、功能最齊全的用戶端。以 Rust 寫成，透過 Interface 指令行提供完整的 Mostro 功能。相容於 macOS、Linux 和 Windows。
...
Alpha 版本的**手機應用程式**可直接從您的智慧型手機進行 P2P 交易。直觀的圖形 Interface 具有標籤式導覽、訂單檢視、進階篩選器和整合式聊天功能，可與您的交易對手溝通。
...
Interface 進階 JavaScript Web 應用程式正在積極開發中。專為提供完整的使用者體驗而設計，具有廣泛的交易和投資組合管理功能。可透過瀏覽器存取，無需安裝。
...
- 具備足夠流動性的 Lightning Network** Wallet (或與 Lightning 相容)
...
- 可選擇但建議**：VPN 或 Tor 連線以遮蔽您的 IP Address
...
- 網路保護**：啟動 VPN 或 Tor 隱藏您的 IP Address
...
Mostro 會要求您提供 **Lightning Invoice** 來接收 BTC。以 Satoshis 為單位的確實金額將會顯示（在此：4715 Sats）。
...
### 步驟 3：提供您的 Lightning Invoice



generate a Lightning Invoice 從您的 Wallet (Phoenix、Breez 等) 獲取準確金額，然後將其 ：
...
系統會確認貨件，並通知您等待賣家支付 Invoice 扣款後再啟動託管。
...
只要賣方確認收到法幣，並使用 `release` 指令解除託管，您就會立即在您提供的 Lightning Invoice 上收到您的比特幣。
...
**對於 CLI/Mobile 同步化**很重要：如果您同時使用 CLI 和行動應用程式，請在兩個用戶端設定 ** 完全相同的 Nostr 中繼器和 Mostro Pubkey**。如果沒有相同的設定，您將無法在兩個介面上看到相同的訂單。Settings (上述截圖) 中可見的中繼器和 Mostro Pubkey 必須與 CLI `.env' 檔案中的相符。
...
按下右下方的 Green **"+"**按鈕，然後選擇 **SELL**（紅色按鈕）。
...
**重要指示**：您現在必須**支付所顯示的 Invoice 持有**，以將您的 bitcoins（在此 4674 Sats 為 1-5 歐元）鎖定在託管中。您最多有**15分鐘的時間**，否則 Exchange 將被取消。



複製 Invoice 保留，並從您的 Wallet Lightning（Phoenix、Breez 等）支付。
...
一旦您確認收到您帳戶上的法幣付款，請按下 Green **RELEASE**按鈕，並確認將 Satss 寄給買家。
...
比特幣會透過買方的 Lightning Invoice 即時轉移給買方。
...
4.提供您的 Lightning Invoice (由您的 Wallet 產生)
...
9.在您的 Lightning Invoice 上即時接收比特幣
...
**金融主權**：由於 Invoice 的持有機制，您的比特幣永遠不會離開您的直接控制，消除了 Exchange 破產或盜版的風險。
...
Mostro P2P 結合了 Lightning Network、Nostr 和自動化託管，是真正去中心化交易的選擇。儘管其發展尚早且流動性有限，但該平台已經提供了金融主權、抗審查性和默認匿名性。



對於喜歡自主性和保密性的比特幣使用者來說，Mostro 是一個值得逐步探索的可行選擇。其分散式架構保證了社群而非商業演進，為未來真正自由的 Bitcoin 交易所鋪路。
