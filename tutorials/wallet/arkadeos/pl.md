...
| Kryterium | Arkade (Ark) | Lightning Network | Cashu (E-cash) |
| :--- | :--- | :--- | :--- |
| **Model** | Współdzielone UTXO koordynowane przez serwer (ASP) | Sieć P2P kanałów płatności | Ślepe tokeny emitowane przez bank (Mint) |
| **Powiernictwo** | **Non-custodial** (masz klucze) | **Non-custodial** (masz klucze) | **Custodial** (Mint posiada środki) |
| **Prywatność** | **Wysoka** (natywny CoinJoin, ślepy dla publiczności) | **Średnia** (routing cebulowy, ale kanały widoczne) | **Bardzo wysoka** (ślepy nawet dla Mintu) |
| **Skalowalność** | Doskonała (masowy batching on-chain) | Doskonała (nieskończone transakcje off-chain) | Doskonała (proste podpisy serwera) |
| **Doświadczenie** | Proste (zbliżone do portfela on-chain) | Złożone (zarządzanie kanałami, płynność) | Bardzo proste (jak cyfrowa gotówka) |
| **Główne ryzyko** | Dostępność ASP i wygaśnięcie | Zarządzanie kanałami i kopie zapasowe | Zaufanie do Mintu (ryzyko kradzieży) |
