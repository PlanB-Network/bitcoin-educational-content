---
name: SeedSigner
description: Um Hardware Wallet que é DIY, stateless, acessível e totalmente air-gapped
---
...
Outra opção para armazenar seu seed e usá-lo com o SeedSigner é usar um cartão inteligente *SeedKeeper* em conjunto com um leitor compatível. Isto dá-lhe um *secure element* muito robusto para armazenar o seu seed, enquanto utiliza o ecrã do SeedSigner para assinar transacções. Mas esta configuração em particular é o assunto de outro tutorial dedicado. Aqui, concentrar-nos-emos na utilização básica do SeedSigner:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

O projeto SeedSigner ocupa um lugar importante no ecossistema Bitcoin, pois oferece a todos, em qualquer parte do mundo, a possibilidade de beneficiar de uma segurança avançada para proteger os seus bitcoins. A sua principal vantagem reside na sua acessibilidade: o hardware necessário pode ser adquirido por menos de 50 dólares. Além disso, permite que as pessoas que vivem em países restritos construam o seu próprio Hardware Wallet a partir de componentes informáticos padrão, que são fáceis de encontrar e menos sujeitos a restrições regulamentares.



Mas mesmo fora destes contextos particulares, o SeedSigner pode ser uma opção interessante para si: é de código aberto, funciona sem estado e com airgap e reduz os vectores de ataque ligados à cadeia Supply do seu Hardware Wallet.
...
- Raspberry Pi Zero
 ...
 - Não se esqueça de incluir uma fonte de alimentação micro-USB Supply.
...
- Ecrã Waveshare de 1,3" (240×240 px)** (em francês)
    ...
    - O ecrã incorpora três botões e um mini-joystick que serve de Interface do utilizador.
...
- Habitação (opcional, mas recomendada)** (opcional, mas recomendada)** (opcional, mas recomendada)** (opcional, mas recomendada)** (recomendada)
...
O PowerShell calcula então o Hash SHA256 do seu ficheiro de imagem. Compare esse Hash com o valor correspondente em `seedsigner.0.8.6.sha256.txt`.
...
Se tiver alterado a orientação da câmara ou pretender alterar outras definições (como o idioma Interface) mais tarde, terá de ativar a persistência das definições no microSD. Caso contrário, as suas definições serão repostas por defeito sempre que reiniciar, uma vez que o Raspberry Pi Zero não tem memória persistente.
...
A versão atual do firmware suporta vários pacotes de software: Sparrow, Specter, BlueWallet, Nunchuk e Keeper. No meu caso, utilizo o **Sparrow wallet**, que recomendo particularmente pela sua simplicidade e riqueza de funcionalidades.
...
- `BTC`
- mBTC` (milli-Bitcoin, ou 0,001 BTC)
- gW-20 (satoshis, ou 1/100.000.000 BTC)
...
- gW-22 network`: só deve ser modificado se pretender utilizar o SeedSigner no Testnet.
- qR code density`: ajusta a quantidade de informação contida em cada código QR. Pode deixar o valor predefinido, a não ser que ache difícil ler durante a leitura.
...
Por fim, no menu `Língua`, pode alterar a língua do Interface de acordo com as suas preferências.
...
O seed (ou frase Mnemonic) constitui a base da sua carteira Bitcoin. É usada para derivar as suas chaves privadas e endereços, e fornece acesso aos seus fundos. O SeedSigner oferece vários métodos para a sua geração, que iremos explorar nesta secção.
...
- Esta frase dá-lhe acesso total e irrestrito a todos os seus bitcoins.** Qualquer pessoa na posse desta frase pode roubar os seus fundos, mesmo sem acesso físico ao seu SeedSigner ;
- Normalmente, uma frase de 12 palavras é usada para restaurar um Wallet se um Hardware Wallet for perdido ou roubado. Mas como o SeedSigner é um dispositivo *sem estado*, ele nunca regista o seu seed. Portanto, os seus backups físicos não são apenas cópias de segurança, mas **a única maneira de usar o seu Wallet**. Se perderes estas cópias de segurança, os teus bitcoins ficarão permanentemente perdidos. Por isso, faça cópias de segurança com cuidado, em vários suportes e em locais seguros;
- Se está a começar, aconselho-o vivamente a ler este tutorial para obter uma compreensão detalhada dos riscos envolvidos na gestão de uma frase Mnemonic:



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### 5.1 Aceder à ferramenta de criação do seed
...
Agora vai fazer o generate, o seu seed. Um seed é simplesmente um grande número aleatório. Quanto mais aleatório ele for gerado, mais seguro ele é. O SeedSigner oferece duas maneiras de fazer isso:




- câmara": O seed é gerado a partir do ruído visual de uma fotografia. Tira-se uma imagem de um ambiente aleatório (objeto, paisagem, rosto, etc.) cujas variações de pixel são utilizadas para o generate entropia. É um método rápido, mas não reproduzível.
...
Neste tutorial, usamos o método **Dice Rolls**. Clique em `Novo seed` (com o ícone do dado).
...
De seguida, escolha o comprimento da sua frase Mnemonic. 12 palavras já oferecem um nível de segurança suficiente, pelo que esta é a escolha que recomendo.
...
Escreva cuidadosamente as palavras da sua frase Mnemonic num suporte físico adequado (papel ou metal).
...
Se cometer um erro, o SeedSigner informá-lo-á e terá de começar de novo, certificando-se de que anota a sua frase Mnemonic quando esta lhe for dada. Este passo de verificação garante que a sua cópia de segurança está correta e completa. Uma vez validado, o ecrã mostrará `Backup Verified`.
...
O SeedSigner é um dispositivo sem memória permanente. Isto significa que o teu seed nunca é guardado dentro do dispositivo (ao contrário de um Ledger, Trezor ou Coldcard, por exemplo). Assim que se desliga a alimentação, o seed desaparece completamente da sua memória RAM. Quando o SeedSigner é reiniciado, volta a um estado vazio: terá de lhe dar novamente o seu seed para assinar as suas transacções.



Isto proporciona uma proteção essencial. Ao contrário de outras carteiras de hardware, o SeedSigner é baseado num Raspberry Pi Zero sem proteção física, incluindo *secure element*. Mas como não são armazenados dados sensíveis, mesmo um dispositivo fisicamente comprometido não permitiria a um atacante extrair as suas chaves privadas ou gastar os seus bitcoins.
...
Aconselho-o também a guardar o seu código QR e a sua frase Mnemonic em dois locais separados, para evitar perder tudo em caso de sinistro.
...
É também nesta fase que pode adicionar um **passphrase BIP39** para reforçar a segurança do seu Wallet. Esta opção pode valer a pena, dependendo da sua estratégia de backup, mas também acarreta riscos: se perder o passphrase, o acesso aos seus bitcoins ficará permanentemente perdido.
...
O SeedSigner permite-lhe converter o seu seed num código QR em papel, denominado *SeedQR*. Este método simplifica o recarregamento do seu Wallet, uma vez que evita redigitar cada palavra manualmente.



Para o fazer, necessitará de um papel em branco ou de um código QR metálico correspondente ao comprimento da sua frase Mnemonic. Se tiver adquirido um pacote completo para o seu SeedSigner, os modelos estão normalmente incluídos. Caso contrário, pode descarregá-los e imprimi-los (ou reproduzi-los à mão) aqui :
...
Reproduza cuidadosamente cada ponto preto na sua folha de registo e, em seguida, utilize o joystick para passar ao BLOCK seguinte. Não tenha pressa: um simples desalinhamento pode tornar o código QR inutilizável.
...
Parabéns, a sua carteira Bitcoin já está a funcionar! Vamos agora importar os seus componentes públicos para o **Sparrow wallet** para o gerir facilmente.
...
Insira o microSD que contém o sistema operativo, ligue o seu SeedSigner e carregue o seed que acabou de criar a partir do seu código QR de backup. No ecrã inicial, selecione `Scan`, depois digitalize o seu SeedQR com o SeedSigner.
...
No seu computador, abra o Sparrow wallet, depois na barra de menu, clique em `File → Import Wallet`.
...
Atribuir um nome ao seu portfólio, em seguida, clique em `Create Wallet`. O Sparrow pedir-lhe-á então para definir uma palavra-passe para bloquear o acesso local a este Wallet. Escolha uma senha forte: ela protege o acesso aos dados da sua carteira no Sparrow (chaves públicas, endereços, etiquetas e histórico de transacções). Esta palavra-passe não é necessária para restaurar a carteira numa data posterior: apenas a sua frase Mnemonic (e possivelmente o seu passphrase) é necessária para este fim.
...
#### 7.1.1 Geração de uma receção Address



No teu computador, abre o Sparrow wallet e desbloqueia o teu SeedSigner Wallet com a tua palavra-passe. Certifique-se de que o software está conectado a um servidor (entalhe no canto inferior direito). Na barra lateral, clique em `Receber`.
...
É visualizado um novo Bitcoin Address. É apresentado :




- O texto Address (começando por `bc1p...` se utilizar P2TR como eu),
- O código QR correspondente,
- Um campo `Label` para acompanhar as suas transacções.



Recomendo vivamente que adicione uma etiqueta a cada recibo Bitcoin no seu Wallet. Isto permitir-lhe-á identificar facilmente a proveniência de cada UTXO e melhorar a sua gestão da privacidade. Para aprofundar este importante tópico, pode consultar a formação específica na Plan ₿ Academy :
...
O seu Address está agora associado a esta etiqueta em todas as secções do Sparrow.
...
Antes de partilhar o Address recebido, é muito importante verificar se este pertence ao seu seed. Este passo garante que o seu SeedSigner poderá assinar transacções associadas a este Address. Também protege contra possíveis ataques em que o Sparrow exibe um Address fraudulento. Lembre-se que o Sparrow é executado num ambiente inseguro (o seu computador), que tem uma superfície de ataque muito maior do que o seu SeedSigner, que está totalmente isolado. É por isso que você nunca deve acreditar cegamente em qualquer destinatário Address exibido no Sparrow até que você o tenha verificado com o seu Hardware Wallet.



No Sparrow, clique no código QR do Address para o ampliar: será então apresentado em ecrã inteiro.
...
Se o Address digitalizado corresponder ao derivado do seu seed, o ecrã do SeedSigner apresentará a mensagem: `Address Verificado`.
...
Isto confirma que o Address pertence ao seu Wallet e que pode receber bitcoins dele com confiança.
...
Pode agora comunicar este Address (sob a forma de texto ou de código QR) à pessoa ou ao serviço que tem de lhe enviar o Satss. Quando a transação tiver sido transmitida na rede, aparecerá no separador `Transacções` do Sparrow wallet.
...
No Sparrow wallet, pode clicar no separador "Enviar" na barra lateral esquerda. No entanto, prefiro utilizar o separador `UTXOs`, que permite praticar o "*Coin Control*". Este método permite-lhe controlar com precisão os UTXOs utilizados, de modo a poder controlar as informações reveladas durante a transação.
...
- Em "Pagar a", cole o Address do destinatário ou clique no ícone da câmara para digitalizar o código QR;
- Em `Label`, adicione uma etiqueta para registar esta despesa;
- Em `Montante`, introduza o montante a enviar;
- Finalmente, selecionar a taxa de honorários com base nas condições actuais do mercado (as estimativas estão disponíveis em [Mempool.space](https://Mempool.space/)).
...
Clique em "Rever detalhes" e verifique cuidadosamente todas as informações diretamente no ecrã do SeedSigner. Os itens mais importantes a verificar são o montante enviado, o Address do destinatário e o montante de quaisquer encargos aplicados.
...
Uma vez assinado, o SeedSigner gera um novo código QR contendo a transação assinada, pronto a ser digitalizado pelo Sparrow.
...
Agora que a transação é válida, precisa de ser difundida na rede Bitcoin, para que chegue a um Miner que a adicionará a um BLOCK.
...
A sua transação foi agora enviada para a rede Bitcoin. Pode seguir o seu progresso no separador `Transacções` do Sparrow wallet.
