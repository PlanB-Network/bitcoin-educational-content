---
name: Mostro
description: KYC-vaba Bitcoin P2P Exchange platvorm Lightning ja Nostr kaudu
---
...
Kuidas omandada või müüa bitcoin'e ilma oma finantssuveräänsust ohustamata? Tsentraliseeritud platvormid kehtestavad invasiivseid KYC-protseduure, mis paljastavad teie isikuandmed, kusjuures konto võidakse omavoliliselt külmutada või riiklikku järelevalvet teostada.



**Mostro P2P** pakub detsentraliseeritud alternatiivi, mis ühendab Lightning Network, Nostr-protokolli ja ootearveid. Selle peamine uuendus: automatiseeritud deposiidisüsteem, kus teie bitcoinid jäävad kogu Exchange jooksul teie kontrolli alla, kõrvaldades varguse, Exchange pankroti või suvalise konfiskeerimise ohu.
...
- Mostro** kliendid: kasutajaliidesed (CLI, Mobile, Web)



Tellimused avaldatakse Nostris avalike sündmustena (tüüp 38383), samas kui privaatseid tehinguid edastatakse otsast lõpuni krüpteeritud sõnumite (NIP-59, NIP-44) kaudu. Iga Mostro instants määrab ise oma tasud (tavaliselt vahemikus 0,3% kuni 1%) ja tehingulimiidid (sõltuvalt instantsist vahemikus paarist tuhandest kuni mitusada tuhat Sats).
...
**Maksmise paindlikkus**: Yadio.io Exchange kursimisteenuse kaudu paljude fiatvaluutade toetus. Aktsepteerib SEPA ülekandeid, PayPali, Revoluti, sularaha ja kõiki osapoolte vahel kokkulepitud meetodeid.
...
Mostro pakub oma P2P vahetuste jaoks kahte peamist operatiivklienti.
...
**Mostro CLI** on kõige küpsem ja funktsionaalsem klient. See on kirjutatud Rust keeles ja pakub kõiki Mostro funktsioone Interface käsurea kaudu. Ühildub macOSi, Linuxi ja Windowsiga.
...
- "vabastamine": Vabastage escrow ja lõpetage Exchange
- `getdm`: Otsesõnumite vaatamine
- "rate" : hinda oma vastaspoolt pärast Exchange-i
...
### Mostro-web - Interface web (väljatöötamisel)



Interface täiustatud JavaScript veebirakendus, mis on aktiivses arenduses. Mõeldud pakkuma täielikku kasutajakogemust koos ulatusliku kauplemis- ja portfellihaldusfunktsiooniga. Juurdepääs veebilehitseja kaudu ilma installimiseta.
...
See õpetus juhendab teid kahe peamise Mostro kliendi paigaldamise ja kasutamise kaudu: CLI ja mobiilirakendust.
...
- Vabatahtlik, kuid soovitatav**: VPN- või Tor-ühendus IP Address maskeerimiseks
...
- Täielikult krüpteeritud vestlus**: End-to-end krüpteeritud suhtlus
...
**Samm 5: Kui sisseelamine on lõpetatud, genereerib rakendus automaatselt Nostr-võtmepaari. Minge menüüsse (☰) ja seejärel **Konto**, et salvestada oma **Saladussõnad** (taastumisfraas). Sellel ekraanil on teil ka võimalus muuta eelnevalt mainitud privaatsusrežiimi.
...
- Võrgukaitse**: Aktiveerige VPN või Tor, et varjata oma IP Address
- Wallet turvaline**: Aktiveerige oma Wallet välklambi automaatne lukustus
...
Mostro palub teil esitada **Lightning Invoice**, et saada BTC. Täpne summa satoshides näidatakse (siin: 4715 Sats).
...
### 3. samm: esitage oma Lightning Invoice



generate Lightning Invoice oma Wallet (Phoenix, Breez jne.) täpse summa eest, siis saatke see :
...
Süsteem kinnitab saadetise ja teatab teile, et peate ootama, kuni müüja maksab Invoice kinnipidamiskohustuse, enne kui aktiveerite eskroo.
...
Niipea kui müüja kinnitab fiatraha kättesaamist ja vabastab käsuga "release" eskrovi, saate oma bitcoinid koheselt kätte teie esitatud Lightning Invoice-l.
...
Vajutage Green **"+"** nuppu all paremal, seejärel valige **MÜÜDA** (punane nupp).
...
**Tähtis juhis**: Te peate nüüd **maksama Invoice ootel**, et lukustada oma bitcoinid (siin 4674 Sats 1-5 EUR eest) deposiiti. Teil on aega **15 minutit maksimaalselt**, vastasel juhul tühistatakse Exchange.



Kopeerige Invoice ooteaeg ja makske see oma Wallet Lightningilt (Phoenix, Breez jne).
...
Kui olete **kinnitanud** fiat-makse laekumist oma kontole, vajutage Green **VÄLJENDAMINE** nuppu ja kinnitage, et saata Satss ostjale.
...
Bitcoins kantakse ostjale koheselt üle tema Lightning Invoice kaudu.
...
4. Esitage oma Lightning Invoice (genereeritud teie Wallet-st)
...
9. Saate bitcoin'e koheselt oma Lightning Invoice-ga
...
**Finantssuveräänsus**: Tänu Invoice hoidmismehhanismile ei kao teie bitcoinid kunagi teie otsese kontrolli alt, välistades Exchange pankroti või piraatluse riski.
...
Mostro P2P on paljulubav alternatiiv tsentraliseeritud börsidele, ühendades Lightning Network, Nostr ja automatiseeritud eskroo tõeliselt detsentraliseeritud kauplemiseks. Hoolimata oma varajasest arengust ja piiratud likviidsusest pakub platvorm juba praegu finantssuveräänsust, tsensuurikindlus ja vaikimisi anonüümsust.
