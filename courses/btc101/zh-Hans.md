...
**矿工确保网络安全，并将交易添加到区块中。他们通过 [ASIC](https://planb.academy/resources/glossary/asic) 机器使用电力来解决比特币工作证明**。
...
简而言之，比特币的创新之处在于通过使用基于工作证明（[proof of work](https://planb.academy/resources/glossary/proof-of-work)）、难度浮动的区块链，提出了解决重复消费问题的方案。在数字世界，所有权的概念不同于物理世界。事实上，在数字世界中，一切都可以复制和粘贴，这就产生了多次使用有价值的数字资产的风险。银行等可信赖的中介机构应运而生，以解决这一技术问题，确保资产转移后不再属于发送者。
...
比特币区块链的设计目标，是让尽可能多的人在无需许可、无需信任的前提下进行验证。这一要求决定了其结构性限制：区块链无法处理无限数量的交易，因为区块必须保持在一个合理的规模内，以便独立节点能够下载、存储并验证，而无需投入过于昂贵的硬件。这种取舍通常被总结为“三难困境”：去中心化、安全性与可扩展性。基于区块链的系统无法同时最大化这三种属性。比特币优先考虑去中心化与安全性，这自然限制了链上交易的吞吐量。
...
闪电网络基于双向支付通道运行。一个通道，是两个参与者之间的一种技术关系，使他们能够在链下（offchain）交换 sats，也就是说，无需将每一笔支付都写入区块链。
...
闪电网络被设计为允许通过中介节点进行支付，而无需信任这些中介。为此，该协议使用了一种称为 HTLC（*Hashed Time-Locked Contracts*，哈希时间锁合约）的智能合约。无需深入所有细节，只需记住其基本机制：
* 支付以揭示一个秘密（预映像）作为条件；
...
在本章中，我们将介绍四种为比特币带来新可能性的关键协议：侧链（尤其是 Liquid）、Ark、RGB 以及 Taproot Assets。
...
这一二层协议的理念由 Burak 于 2023 年 5 月提出。与闪电网络类似，Ark 是构建在比特币主链之上的系统，它可以在链下实现快速、匿名且低成本的比特币支付。与闪电网络相比，Ark 不需要具备入账流动性即可接收支付，从而显著改善用户体验。此外，它还能提供接近 CoinJoin 交易的隐私性。如果比特币中引入 covenant，Ark 甚至可以实现非交互式运作。
