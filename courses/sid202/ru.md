---
name: Создание приложений с Elements и Liquid Network
...
Elements - это [блокчейн](https://planb.academy/resources/glossary/blockchain)-платформа с открытым исходным кодом и поддержкой сайдчейна, предоставляющая доступ к мощным функциям, разработанным членами сообщества, таким как конфиденциальные транзакции и выпущенные активы.

По своей сути Elements - это протокол, который позволяет достичь [консенсуса](https://planb.academy/resources/glossary/consensus) в отношении истории [транзакций](https://planb.academy/resources/glossary/transaction-tx) и правил, регулирующих передачу и создание активов, хранящихся в распределенной блокчейн-бухгалтерии.
...
Elements предлагает техническое решение проблем, с которыми пользователи блокчейна сталкиваются ежедневно: задержка транзакций, отсутствие конфиденциальности и риск [взаимозаменяемости](https://planb.academy/resources/glossary/fungibility).
...
Ознакомиться с [информационным бюллетенем](https://planb.academy/resources/glossary/white-paper) "Сильные федерации" можно здесь: https://blockstream.com/strong-federations.pdf
...
Само программное обеспечение [узла](https://planb.academy/resources/glossary/node) Elements называется `elementsd` и запускается как демон на машине пользователя. Демон (или служба в Windows) - это программа, которая работает в качестве фоновой службы, не требуя прямого контроля со стороны вошедшего в систему пользователя.
...
- Порог `n из m`, который должен быть выполнен, и действительные [открытые ключи](https://planb.academy/resources/glossary/public-key), которыми можно подписывать блоки.
- [Скрипт](https://planb.academy/resources/glossary/script), который необходимо удовлетворить для передачи активов в сайдчейн и из него.
...
Даже если мы импортируем [закрытый ключ](https://planb.academy/resources/glossary/private-key) e2 в кошелек e1, он все равно не сможет увидеть суммы и тип активов, с которыми были совершены операции, поскольку не будет знать о слепом ключе, используемом e2. Мы докажем это, импортировав закрытый ключ, используемый кошельком e2, в кошелек e1. Сначала нам нужно экспортировать ключ из e2
...
Эти два узла находятся в одной локальной сети и связаны друг с другом, а значит, имеют одинаковые транзакции в своем [пуле транзакций](https://planb.academy/resources/glossary/mempool) и идентичные блокчейны. Хотя они работают на одной машине, стоит отметить, что у них нет общих фактических файлов блокчейна. Каждый узел управляет своей локальной копией блокчейна, которая содержит одну и ту же историю транзакций, поскольку они находятся в консенсусе и придерживаются тех же правил протокола, что и друг друга.
...
Для того чтобы Elements мог работать как сайдчейн, [блок genesis](https://planb.academy/resources/glossary/genesis-block) в его блокчейне должен быть создан с `fedpegscript`. Это делается путем передачи параметра `fedpegscript` при запуске узла. Этот скрипт станет частью правил консенсуса блокчейна Elements и позволит проверять и выполнять запросы на привязку и отвязку.
...
Прежде чем привязка будет принята сайдчейном Elements, она должна получить достаточное количество подтверждений на мейнчейне. Это необходимо для того, чтобы избежать [инфляции](https://planb.academy/resources/glossary/inflation) предложения привязанного актива на сайдчейне Elements, которая может быть вызвана реорганизацией мейнчейна.
