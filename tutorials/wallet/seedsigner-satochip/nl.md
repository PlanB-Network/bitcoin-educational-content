...
De Satochip is een Hardware Wallet in smartcardformaat, met een EAL6+ gecertificeerd beveiligingselement, een van de hoogste beveiligingsstandaarden. Hij is ontworpen en geproduceerd door het gelijknamige Belgische bedrijf: Satochip.
...
Zodra de seed is gegenereerd, is dit de enige keer dat het in het RAM van de SeedSigner zit. In mijn geval wil ik het opslaan op een [Seedkeeper](https://satochip.io/product/seedkeeper/), een ander Satochip product ontworpen om geheimen op te slaan. Ik gebruik dit apparaat als laatste redmiddel, in geval van verlies van mijn Satochip.



De gekozen back-upstrategie hangt af van je voorkeuren, maar het is noodzakelijk om minstens één kopie van je Mnemonic zin te hebben, op fysieke media (papier of metaal) of, zoals hier, in een Seedkeeper. U kunt het aantal back-ups naar behoefte ook vermenigvuldigen. Voor meer informatie over portfolio back-up strategieën, raad ik je aan deze tutorial te lezen:
...
Kies een `label` om gemakkelijk je verschillende geheimen die op Seedkeeper zijn opgeslagen te identificeren. Je kunt bijvoorbeeld gewoon de vingerafdruk van de Wallet behouden of expliciet `seed` aangeven. De keuze hangt af van je voorkeur en risico.
...
De PIN-code van Seedkeeper moet zo lang en willekeurig mogelijk zijn, om brute force pogingen te voorkomen in het geval van fysieke compromittering van de kaart. Je moet ook een reservekopie van deze PIN-code bewaren, op een andere locatie dan Seedkeeper. Zonder deze PIN-code heb je geen toegang tot de Mnemonic die is opgeslagen in Seedkeeper, en zijn je bitcoins voor altijd verloren.



### 5.3. De seed opslaan op de Satochip



Nu je portfolio gegenereerd, opgeslagen en gecontroleerd is, gaan we het overbrengen naar de Satochip. Om dit te doen, zorg je ervoor dat de seed in het RAM van de SeedSigner geladen is. Ga dan naar `Tools > Smartcard Tools > Satochip Functions`.
...
Steek je Satochip in de smartcardlezer en selecteer vervolgens `Initialiseren met seed`.
...
De SeedSigner vraagt je dan welke seed je wilt importeren in de Satochip. Selecteer degene waarvan de vingerafdruk overeenkomt met de portefeuille die je zojuist hebt aangemaakt.
...
Je seed is nu geïmporteerd in de Satochip.
...
Als je een passphrase BIP39 wilt gebruiken om de beveiliging van je Wallet te verbeteren, zie dan deel 6 van deze tutorial:
...
Nu je portefeuille operationeel is, importeren we de openbare informatie (de "*keystore*") in Sparrow wallet of een andere software voor portefeuillebeheer. Deze software zal gebruikt worden om je transacties aan te maken, te verdelen en bij te houden. Het zal ze echter niet kunnen ondertekenen, omdat alleen de Satochip (en eventuele back-ups) de privésleutels bevat die hiervoor nodig zijn.
...
Plaats de microSD-kaart met het besturingssysteem en zet dan je SeedSigner aan. Op dit moment kan hij nog niets doen, omdat hij jouw seed nog niet kent. Je moet beginnen met de Satochip in de smartcardlezer te steken, omdat deze jouw seed bevat.
...
De SeedSigner genereert dan je xpub in de vorm van een dynamische QR-code, die alle gegevens bevat die je nodig hebt om je portefeuille in Sparrow wallet te beheren. Je kunt de helderheid van het scherm aanpassen met de joystick om het scannen van de QR-code te vergemakkelijken.
...
Zorg ervoor dat de Sparrow wallet software op je computer is geïnstalleerd. Als je niet weet hoe je de software moet downloaden, controleren op echtheid en correct installeren, raadpleeg dan onze complete handleiding over dit onderwerp:



https://planb.academy/tutorials/wallet/desktop/sparrow-c674e2ac-d46f-4c82-92a7-7d1b0e262f5d

Open Sparrow wallet op je computer en klik in de menubalk op `Bestand → Wallet importeren`.
...
Scroll naar beneden naar `SeedSigner` en selecteer vervolgens `Scan...`. Uw webcam wordt geactiveerd: scan de dynamische QR-code die wordt weergegeven op uw SeedSigner-scherm.
...
Wijs een naam toe aan je portfolio en klik dan op `Creëer Wallet`. Sparrow zal je dan vragen een wachtwoord in te stellen om de lokale toegang tot deze Wallet te vergrendelen. Kies een sterk wachtwoord: het beschermt uw gegevens in Sparrow (publieke sleutels, adressen, labels en transactiegeschiedenis). Dit wachtwoord is echter niet nodig om de Wallet in de toekomst te herstellen: alleen je Mnemonic-zin (en mogelijk je passphrase) is dat nodig.
...
Controleer nu of de `Master fingerprint` die wordt weergegeven in Sparrow wallet overeenkomt met degene die eerder op jouw SeedSigner is gevonden.



De SeedSigner zal je dan vragen om een willekeurige ontvangende Address van je Sparrow wallet te scannen om de geldigheid van de import te bevestigen.
...
#### 7.1.1 Een ontvangst Address genereren



Open Sparrow wallet op je computer en ontgrendel je `Satochip-SeedSigner` Wallet met je wachtwoord. Controleer of de software verbonden is met een server (indicator rechtsonder). Klik dan in de zijbalk op `Ontvangen`.
...
Een nieuwe Bitcoin Address verschijnt. Je vindt :




- De Address in tekstformaat (beginnend met `bc1q...` als je P2WPKH gebruikt, zoals in dit voorbeeld) ;
- De bijbehorende QR-code ;
- Een `Label` veld, handig voor het traceren van je transacties.



Ik raad je sterk aan om een label toe te voegen aan elk Bitcoin ontvangstbewijs in je Wallet. Zo kunt u de herkomst van elke UTXO gemakkelijk identificeren en uw privacy beter beheren. Bekijk de speciale training op Plan ₿ Academy voor meer informatie over dit belangrijke onderwerp:
...
Je Address wordt nu geassocieerd met dit label in alle Sparrow secties.
...
Voordat je de ontvangen Address doorgeeft aan de betaler, is het belangrijk om te controleren of deze bij je seed hoort. Deze stap zorgt ervoor dat je Satochip dan transacties kan ondertekenen die bij deze Address horen. Het voorkomt ook mogelijke aanvallen waarbij Sparrow een frauduleuze Address zou weergeven. Houd er rekening mee dat Sparrow draait op een onveilige omgeving (jouw computer), waarvan het aanvalsoppervlak veel groter is dan dat van jouw Satochip, die volledig geïsoleerd is. Daarom moet je nooit blindelings vertrouwen op de adressen die in Sparrow worden weergegeven, voordat je ze op je Hardware Wallet hebt gecontroleerd.



Klik in Sparrow op de QR-code van de Address om deze te vergroten: de code wordt dan schermvullend weergegeven.
...
Bevestig dan het type script dat gebruikt is (in dit geval `Native SegWit`), voer de Satochip PIN code in om het te ontgrendelen en valideer de `xpub` informatie.
...
Als de gescande Address overeenkomt met degene die is afgeleid van je seed, zal de SeedSigner het bericht weergeven: `Address geverifieerd`.
...
U kunt er dan zeker van zijn dat de Address deel uitmaakt van uw portfolio.
...
Je kunt nu deze Address in tekstvorm of via de QR-code doorsturen naar de persoon of dienst die je Satss moet sturen. Zodra de transactie is uitgezonden op het netwerk, verschijnt deze in het tabblad `Transacties` van Sparrow wallet.
...
- Transactie aanmaken in Sparrow ;
- Het ondertekenen van deze transactie op de Satochip, via de SeedSigner ;
- Tenslotte wordt de transactie vanaf Sparrow over het netwerk uitgezonden.
...
In Sparrow wallet kun je een transactie maken door op de `Verstuur` tab in de linker zijbalk te klikken. Ik gebruik echter liever het tabblad `UTXO's`, waarmee je *Coin Controle* kunt uitoefenen. Deze methode biedt precieze controle over de bestede UTXO's om de informatie die tijdens je transacties wordt onthuld te beperken.
...
- Plak in `Betaal aan` de Address van de ontvanger of scan hun QR-code met het camerapictogram ;
- Voeg in `Label` een label toe om deze uitgave te traceren;
- Voer in `Bedrag` het bedrag in dat moet worden verzonden;
- Selecteer ten slotte de oplaadsnelheid volgens de huidige netwerkomstandigheden (schattingen zijn beschikbaar op [Mempool.space](https://Mempool.space/)).
...
Zet uw SeedSigner aan en plaats uw Satochip zoals gewoonlijk. Selecteer op het beginscherm `Scan` en scan de QR-code die op Sparrow wordt weergegeven.
...
Als alles in orde is, selecteer dan `Approve PSBT` om de transactie te ondertekenen met de Satochip.
...
Nu de transactie getekend en gevalideerd is, hoeft deze alleen nog maar uitgezonden te worden op het Bitcoin netwerk, zodat een Miner het kan opnemen in een BLOCK. Klik in Sparrow op `Scan QR`.
...
Presenteer de QR-code die op je SeedSigner staat (degene die de ondertekende transactie bevat) aan de webcam. Sparrow toont dan alle details van de transactie. Controleer nog een laatste keer of alle informatie correct is en klik dan op "Transactie uitzenden" om het uit te zenden op het Bitcoin netwerk.
...
Uw transactie wordt nu naar het netwerk verzonden. U kunt de bevestiging volgen in het tabblad `Transacties` van Sparrow wallet.
...
- Door de Mnemonic frase op een fysiek medium op te nemen;
...
Als je nog steeds je Satochip hebt, maar je SeedSigner is kapot of verloren, is de situatie vrij eenvoudig te beheersen, omdat je Wallet nog steeds in de Satochip zit.
...
Als je er geen wilt herbouwen, kun je je Satochip ook op de klassieke manier gebruiken, d.w.z. rechtstreeks vanaf je computer, zonder via de SeedSigner te gaan. Deze methode werkt perfect, maar vermindert de veiligheid van je Bitcoin Wallet aanzienlijk: je verliest de "*air-gapped*" isolatie en moet nu blind tekenen, aangezien de SeedSigner als vertrouwd scherm fungeerde. Dit kan echter een tijdelijke oplossing zijn in een noodgeval, of als je niet in staat bent om een SeedSigner te herbouwen.
...
Voer tot slot de PIN-code van je smartcard in om hem te ontgrendelen. Daarna heb je toegang tot je Wallet, kun je transacties aanmaken en direct ondertekenen met de aangesloten Satochip.
...
Het andere, meer delicate scenario is wanneer u de toegang verliest tot uw Satochip die de seed bevat: of hij nu kapot, verloren of gestolen is, of u bent de PIN-code vergeten. Als je Satochip is gestolen of kwijtgeraakt, raden we je sterk aan om, zodra de toegang tot je fondsen is hersteld, je bitcoins onmiddellijk over te boeken naar een gloednieuwe Wallet, gegenereerd met een andere seed. Dit zorgt ervoor dat een potentiële aanvaller nooit toegang kan krijgen tot uw Satss.



Om weer toegang te krijgen tot uw portefeuille en uw fondsen te verplaatsen, laadt u gewoon uw seed in SeedSigner. Afhankelijk van het back-upmedium dat u hebt gebruikt, hebt u verschillende opties:





- Voer uw Mnemonic-zin handmatig in in het menu `Seeds > Voer 12-woords seed in`.
...
- Of laad je seed van een Seedkeeper, via het `Seeds > From SeedKeeper` menu (dit is de methode die ik gebruik in deze tutorial). Je hoeft alleen maar de PIN-code van de Seedkeeper in te voeren en het geheim te selecteren dat gebruikt moet worden als seed op de SeedSigner.
...
Zodra de seed in de SeedSigner is geladen, welke methode je ook gebruikt, kun je een of meer scantransacties ondertekenen om je bitcoins naar een nieuwe, ongecompromitteerde Wallet te verplaatsen. Om uit te vinden hoe je dit doet, zie deel 7.2 van de volgende tutorial:
