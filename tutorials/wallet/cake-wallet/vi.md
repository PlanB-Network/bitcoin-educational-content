...
- [**Thanh toán Im lặng (BIP-352)**](https://BIPs.dev/352/) cải thiện [mã thanh toán BIP 47](https://silentpayments.xyz/docs/comparing-proposals/bip47/) trước đây, còn được gọi là "PayNyms" với các địa chỉ ẩn có thể tái sử dụng. Khi người gửi sử dụng Thanh toán Im lặng Address của bạn, Wallet của họ sẽ tạo ra một mã Address dùng một lần duy nhất bằng các khóa khác nhau, sau đó được kết hợp thành một mã Taproot Address dùng một lần duy nhất. Hồ sơ Blockchain hiển thị các giao dịch không liên quan, ngăn chặn việc liên kết các khoản thanh toán đến. Thanh toán Im lặng mang lại nhiều lợi ích, bao gồm:
    - Địa chỉ có thể tái sử dụng: Không cần generate một Address mới cho mỗi giao dịch, mang lại trải nghiệm người dùng tốt hơn và tăng cường quyền riêng tư
    - Không tăng chi phí: Thanh toán im lặng không làm tăng quy mô hoặc chi phí của giao dịch.
    - Nâng cao tính ẩn danh: Người quan sát bên ngoài không thể liên kết các giao dịch với Thanh toán im lặng Address.
    ...
    - Địa chỉ duy nhất cho mỗi khoản thanh toán: Loại bỏ nguy cơ tái sử dụng Address một cách vô tình.
...
- PayJoin v2** giảm thiểu việc phân tích đồ thị giao dịch bằng cách hợp nhất dữ liệu đầu vào của bên gửi và bên nhận thành một giao dịch duy nhất. Cake Wallet triển khai hai cải tiến quan trọng:
    ...
    - Truyền thông không cần máy chủ**: Không bên nào cần phải chạy máy chủ PayJoin, loại bỏ rào cản kỹ thuật lớn.
...
- Hoán đổi:** chọn `Chỉ Tor` (ẩn danh lưu lượng Exchange)
...
Để tìm hiểu các phương pháp tốt nhất để lưu cụm từ Mnemonic của bạn, vui lòng tham khảo hướng dẫn này:
...
- Số dư khả dụng
- Thẻ Thanh toán Im lặng để Bắt đầu quét các giao dịch được gửi đến Thẻ Thanh toán Im lặng Address của bạn
- Thẻ PayJoin để `Kích hoạt` PayJoin như một tính năng bảo vệ quyền riêng tư và tiết kiệm phí
...
Tại đây, chúng ta có thể kết nối lại Wallet, quản lý các nút và kết nối với nút của riêng mình (khuyến nghị). `Silent Payments Scanning` cho phép chúng ta tùy chỉnh chế độ quét bằng cách chỉ định `Quét từ độ cao BLOCK` hoặc `Quét từ ngày`.
...
Chúng ta có thể bật thẻ Thanh toán Im lặng trên màn hình chính để hiển thị tính năng này. Việc bật tùy chọn `Luôn quét` cho phép Wallet liên tục theo dõi Blockchain để phát hiện các khoản Thanh toán Im lặng đến. Chúng ta có thể chỉ định các thông số quét để tùy chỉnh quy trình quét theo nhu cầu của mình như đã mô tả ở trên.
...
Có một số tùy chọn và loại Address có sẵn để nhận Bitcoin. `SegWit (P2WPKH)` *(bắt đầu bằng bc1q....)* là tùy chọn mặc định. Trong ví dụ này, hãy chọn `Thanh toán Im lặng`.


Để nhận Thanh toán Im lặng, trước tiên hãy chạm vào biểu tượng `Nhận` trong Cake Wallet. Tiếp theo, nhập số tiền bạn mong muốn nhận. Để chỉ định loại Address, hãy chạm lại vào `Nhận` ở đầu màn hình, sau đó chọn `Thanh toán Im lặng` từ các tùy chọn.


Trên màn hình chính, mã QR Thanh toán Im lặng có thể tái sử dụng và mã Address của bạn sẽ được hiển thị. Đúng như dự đoán, mã Address khá dài:
...
Bây giờ, hãy sử dụng Wallet tương thích với BIP-352 (chẳng hạn như Blue Wallet) để quét mã QR này và gửi thanh toán. Bạn sẽ thấy Wallet tạo ra một điểm đến duy nhất là Address từ Address không hoạt động của bạn.
...
- hoặc dán mã `sp1qq...` Address có thể tái sử dụng hoặc quét mã QR trực tiếp trong ứng dụng.
- Chọn số tiền bạn muốn chi tiêu từ số dư khả dụng của mình
- Chạm vào `Gửi` ở cuối màn hình để xác nhận giao dịch


Sau khi chúng ta nhập `sp1qq...` Address, Wallet sẽ tự động suy ra `bc1p...` tương ứng Taproot Address (P2TR) ở chế độ nền, sẽ được sử dụng cho Thanh toán im lặng.
...
Sau khi bạn đã gửi giao dịch, bạn sẽ được hỏi xem bạn có muốn thêm địa chỉ liên hệ này vào sổ Address của mình hay không.
...
Để sử dụng PayJoin, cả hai bên đều cần có Wallet tương thích với PayJoin, và bên nhận cần có ít nhất một Coin hoặc đầu ra trong Wallet của họ. Để bắt đầu, vui lòng làm theo các bước sau:
...
PayJoin v2 phá vỡ quy trình phân tích chuỗi bằng cách hợp nhất dữ liệu đầu vào của bên gửi và bên nhận thành một giao dịch duy nhất mà không cần thêm phí hay điều phối tập trung. Điều này phá vỡ phương pháp đầu vào thông thường của Ownership, một lợi thế đáng kể vì nó cho phép bạn không thể giả định rằng tất cả dữ liệu đầu vào đều thuộc về bên gửi.


Đối với những người dùng ưu tiên tính ẩn danh tài chính, Cake Wallet là một lựa chọn khả thi. Nó tích hợp các giao thức bảo mật trực tiếp vào chức năng cốt lõi, giúp chúng dễ dàng truy cập mà không gặp bất kỳ sự phức tạp kỹ thuật nào. Khi hoạt động giám sát trên các blockchain công khai ngày càng tăng, các công cụ như thế này giúp duy trì quyền riêng tư trong giao dịch ở những nơi quan trọng nhất. Việc triển khai rộng rãi các tiêu chuẩn này trong bối cảnh Wallet sẽ là một bước tiến đáng hoan nghênh.
