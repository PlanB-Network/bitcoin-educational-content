...
- Apri Balena Etcher, quindi seleziona l'immagine ISO di Pop!OS.
...
- Spegni il computer.
- Inserisci la chiave USB (contenente Pop!OS).
...
Seleziona una lingua per l'input da tastiera.
...
Scegli l'opzione `Installazione pulita` per un'installazione standard. Questa è l'opzione migliore per i nuovi utenti di Linux, ma bisogna tenere presente che cancellerà tutto il contenuto dell'unità di destinazione. In alternativa, è possibile selezionare l'opzione `Prova modalità demo` per continuare a testare Pop!OS nell'ambiente live.

![Capture](assets/fr/14.webp)

Seleziona `Personalizzato (avanzato)` per accedere a GParted. Questo strumento consente di configurare funzioni avanzate come il doppio avvio, la creazione di una partizione `/home` separata o il posizionamento della partizione `/tmp` su un'altra unità.

Fai clic su `Cancella e installa` per installare Pop!OS sull'unità selezionata.

![Capture](assets/fr/15.webp)

### Configurazione dell'account utente

La sezione successiva del programma di installazione ti guiderà nella creazione di un account utente per poter accedere al nuovo sistema operativo.

Fornisci un nome completo (che può includere qualsiasi testo a tua scelta, maiuscolo o minuscolo) e un nome utente (che deve essere in minuscolo):
...
L'unità può essere crittografata utilizzando la password di accesso selezionando `La password di crittografia è uguale alla password dell'account utente`. È anche possibile deselezionare questa casella e selezionare `Imposta password` in basso. Seleziona `Non crittografare` per ignorare il processo di crittografia del disco.
...
Una volta completata l'installazione, riavvia il computer e accedi per completare il processo di configurazione dell'account utente.

Se è stato modificato l'ordine di avvio per dare priorità alla chiave USB Live all'avvio, spegni completamente il computer e rimuovi la chiave USB di installazione. Se si è in modalità dual-boot, premi i tasti appropriati per accedere alla configurazione e selezionare l'unità contenente l'installazione di Pop!OS.
...
| ---------------------------- | --------------------------------------------- | ----------------------------------------------- |
| Firefox                      | Browser web gratuito e popolare               | `sudo apt install firefox`                      |
| Brave                        | Browser web basato sulla privacy              | Installazione via Pop!_Shop dal sito ufficiale  |
| Visual Studio Code (VS Code) | Editor di codici potente per gli sviluppatori | `flatpak install flathub com.visualstudio.code` |
| Git                          | Gestione delle versioni                       | `sudo apt install git`                          |
| Flatpak                      | Gestione di pacchetti alternativi             | `sudo apt install flatpak`                      |
| VLC                          | Player multimediale multiuso                  | `sudo apt install vlc`                          |
...
| Wget                         | Scaricamento dei file tramite HTTP/FTP        | `sudo apt install wget`                         |
| Docker                       | Container delle applicazioni                  | Installazione via script ufficiale in `apt`     |
| Node.js                      | Ambiente JavaScript lato server               | Installazione via `apt` in NodeSource           |
...
| Transmission                 | Client BitTorrent                             | `sudo apt install transmission-gtk`             |
| Htop                         | Monitor di sistema interattivo                | `sudo apt install htop`                         |

### Installazione tramite Pop! Shop (interfaccia grafica)

- Apri **Pop!_Shop** dal menu principale.
- Utilizza la barra di ricerca per trovare le applicazioni desiderate (ad esempio, "Brave").
- Fai clic su **Installa** per ogni applicazione.
...
```bash
sudo adduser tuonome && sudo usermod -aG sudo tuonome
```

Esci, quindi accedi nuovamente con questo nuovo utente.

### Gestione dei driver grafici

- Per le schede Nvidia, verifica che siano installati i driver proprietari:
...
Pop!OS non blocca il traffico di rete per impostazione predefinita. Attiva UFW per migliorare la sicurezza:
...
- Apri **Impostazioni di sistema** → **Aspetto** per scegliere un tema chiaro o scuro.
- Configura gli angoli attivi, le animazioni e le estensioni tramite il manager COSMIC.
- Regola il layout del desktop per ottimizzare il flusso di lavoro.

### Configurare il backup automatico

Pop!OS integra strumenti come Deja Dup per il backup:

- Avvia **Backup** dal menu.
- Scegli un'unità esterna o una posizione di rete.
...
Quindi attivali nelle impostazioni.
...
Se necessario, aumenta la dimensione dello swap o configurare un file di swap:
...
Aggiungilo al file `/etc/fstab` per il montaggio automatico.
...
Sentiti libero di esplorare ulteriormente Pop!OS e di personalizzare il tuo ambiente per ottenere il massimo da esso.
