---
name: SeedSigner
description: En Hardware Wallet som är självbyggd, statslös, prisvärd och helt luftburen
---
...
SeedSigner är en Hardware Wallet Bitcoin med öppen källkod som vem som helst kan bygga själv med hjälp av billiga elektroniska komponenter för allmänt bruk. Till skillnad från kommersiella produkter som Ledger, Coldcard eller Trezor är det här inte en färdig enhet som tillverkas av ett företag: det är ett community-projekt som låter alla skapa sin egen enhet och kontrollera varje steg.



SeedSigner är utformad för att vara 100% ***air-gapped***: den ansluter aldrig till Internet, har ingen Wi-Fi eller Bluetooth (i fallet med Raspberry Pi Zero v1.3) och är aldrig ansluten till en dator för Exchange-data. Kommunikationen sker uteslutande via ett Exchange-system med QR-kod. Rent konkret visar din programvara för portföljhantering (t.ex. Sparrow wallet) den transaktion som ska signeras i form av QR-koder; du skannar dem med SeedSigners kamera, sedan signerar enheten transaktionen med dina privata nycklar som tillfälligt lagras i dess RAM-minne. Slutligen genererar den QR-koder som innehåller den signerade transaktionen, som du skannar med din programvara för att skicka den till Bitcoin-nätverket.
...
Ett annat alternativ för att lagra din seed och använda den med SeedSigner är att använda ett *SeedKeeper* smartkort tillsammans med en kompatibel läsare. Detta ger dig en mycket robust * secure element* för att lagra din seed, samtidigt som du använder SeedSigner-skärmen för att signera transaktioner. Men denna speciella konfiguration är föremål för en annan dedikerad handledning. Här kommer vi att koncentrera oss på den grundläggande användningen av SeedSigner:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

SeedSigner-projektet intar en viktig plats i Bitcoin-ekosystemet, eftersom det erbjuder alla, överallt i världen, möjligheten att dra nytta av avancerad säkerhet för att skydda sina bitcoins. Dess främsta fördel ligger i dess tillgänglighet: den hårdvara som krävs kan köpas för mindre än $ 50. Dessutom gör den det möjligt för människor som bor i begränsade länder att bygga sin egen Hardware Wallet från standarddatorkomponenter, som är lätta att hitta och mindre föremål för regleringsbegränsningar.



Men även utanför dessa särskilda sammanhang kan SeedSigner vara ett intressant alternativ för dig: det är öppen källkod, fungerar statslöst och airgapped och minskar attackvektorerna kopplade till Supply-kedjan i din Hardware Wallet.
...
- Raspberry Pi Zero
 ...
 - Glöm inte att inkludera en mikro-USB-ström Supply.
...
- Waveshare 1,3"-skärm (240×240 px)** (på franska)
    ...
    - Displayen har tre knappar och en mini-joystick som fungerar som användarens Interface.
...
- Bostäder (frivilligt, men rekommenderas)** (frivilligt, men rekommenderas)** (frivilligt, men rekommenderas)** (frivilligt, men rekommenderas)** (rekommenderas)
...
Du kan köpa dessa komponenter separat eller, för större enkelhet, välja färdiga paket som innehåller all nödvändig hårdvara. Själv beställde jag mitt paket [från den här franska webbplatsen](https://bitcoinbazar.fr/), men du hittar också en lista över leverantörer för varje region i världen på [SeedSigner-projektets hårdvarusida](https://seedsigner.com/hardware/). Om du föredrar att köpa komponenterna individuellt finns de tillgängliga på de stora e-handelsplattformarna eller i specialbutiker.
...
PowerShell beräknar sedan Hash SHA256 för din bildfil. Jämför denna Hash med motsvarande värde i `seedsigner.0.8.6.sha256.txt`.
...
Om du har ändrat kamerans orientering eller vill ändra andra inställningar (t.ex. Interface-språket) vid ett senare tillfälle måste du aktivera persistens av inställningar på microSD-kortet. Annars kommer dina inställningar att återgå till standardinställningarna varje gång du startar om, eftersom Raspberry Pi Zero inte har något permanent minne.
...
Den aktuella firmwareversionen stöder flera mjukvarupaket: Sparrow, Spectre, BlueWallet, Nunchuk och Keeper. I mitt fall använder jag **Sparrow wallet**, som jag särskilt rekommenderar för dess enkelhet och rika funktionalitet.
...
- gW-20 (satoshis, eller 1/100 000 000 BTC)
...
- gW-22 network`: ska endast ändras om du vill använda SeedSigner på Testnet.
...
- `Script types`: definierar de skripttyper som tillåts låsa dina bitcoins. Du behöver inte ändra den här parametern, eftersom skripttypen kommer att ställas in direkt till Sparrow. Här berörs endast skript som SeedSigner är behörig att manipulera.
...
Slutligen kan du i menyn `Language` ändra språket i Interface så att det passar dina önskemål.
...
seed (eller Mnemonic-fras) utgör grunden för din Bitcoin-portfölj. Den används för att härleda dina privata nycklar och adresser, och ger tillgång till dina medel. SeedSigner erbjuder flera metoder för att generera den, som vi kommer att utforska i det här avsnittet.
...
- Denna fras ger dig full, obegränsad tillgång till alla dina bitcoins.** Alla som har tillgång till denna fras kan stjäla dina pengar, även utan fysisk tillgång till din SeedSigner ;
- Vanligtvis används en fras på 12 ord för att återställa en Wallet om en Hardware Wallet tappas bort eller stjäls. Men eftersom SeedSigner är en *statlös* enhet registrerar den aldrig din seed. Så dina fysiska säkerhetskopior är inte bara säkerhetskopior, utan **det enda sättet att använda din Wallet**. Om du förlorar dessa säkerhetskopior kommer dina bitcoins att gå förlorade permanent. Så säkerhetskopiera dem noggrant, på flera medier och på säkra platser;
- Om du precis har börjat rekommenderar jag starkt att du läser den här handledningen för att få en detaljerad förståelse för riskerna med att hantera en Mnemonic-fras :



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### 5.1 Öppna verktyget för skapande av seed
...
Du kommer nu att generate din seed. En seed är helt enkelt ett stort slumpmässigt nummer. Ju mer slumpmässigt det genereras, desto säkrare är det. SeedSigner erbjuder två sätt att göra detta:
...
### 5.2 Skapa en seed med hjälp av ett foto
...
I denna handledning använder vi metoden **Dice Rolls**. Klicka på `Ny seed` (med tärningsikonen).
...
Välj sedan längden på din Mnemonic-fras. 12 ord erbjuder redan en tillräcklig säkerhetsnivå, så det är det val jag rekommenderar.
...
Skriv försiktigt ner orden i din Mnemonic-fras på ett lämpligt fysiskt underlag (papper eller metall).
...
Om du gör ett misstag kommer SeedSigner att informera dig, och du måste börja om igen och se till att notera din Mnemonic-fras när den ges till dig. Detta verifieringssteg säkerställer att din säkerhetskopia är korrekt och fullständig. När den har validerats kommer skärmen att visa `Backup Verified`.
...
SeedSigner är en enhet utan permanent minne. Det innebär att din seed aldrig lagras inuti enheten (till skillnad från t.ex. en Ledger, Trezor eller Coldcard). Så snart du stänger av strömmen försvinner seed helt från dess RAM-minne. När SeedSigner startas om återgår den till ett tomt tillstånd: du måste ge den din seed igen för att signera dina transaktioner.



Detta ger ett viktigt skydd. Till skillnad från andra hårdvaruplånböcker är SeedSigner baserad på en Raspberry Pi Zero utan fysiskt skydd, inklusive *secure element*. Men eftersom inga känsliga data lagras, skulle inte ens en fysiskt komprometterad enhet tillåta en angripare att extrahera dina privata nycklar eller spendera dina bitcoins.
...
Varje gång du använder SeedSigner skannar du denna QR-kod med enhetens kamera så att den tillfälligt laddar dina seed i sitt minne medan du signerar dina transaktioner. Denna andra säkerhetskopia, avsedd för daglig användning, måste också förvaras med största försiktighet: alla som har denna QR-kod har full tillgång till dina bitcoins.


Jag råder dig också att förvara din QR-kod och din Mnemonic-fras på två separata platser för att undvika att förlora allt i händelse av ett anspråk.



Ett mer avancerat och säkert alternativ är att använda SeedSigner med en **SeedKeeper**, som lagrar seed i en secure element. För mer information, se denna handledning :
...
Det är också i det här skedet som du kan lägga till en **passphrase BIP39** för att förstärka säkerheten för din Wallet. Det här alternativet kan vara värt det, beroende på din backup-strategi, men det medför också risker: om du förlorar passphrase kommer tillgången till dina bitcoins att förloras permanent.
...
För att göra detta behöver du ett tomt papper eller en QR-kod i metall som motsvarar längden på din Mnemonic-fras. Om du har köpt ett komplett paket för din SeedSigner ingår mallarna vanligtvis. Om inte, kan du ladda ner och skriva ut dem (eller reproducera dem för hand) här :
...
Välj `Backup seed` på skärmen seed.
...
Återge noggrant varje svart punkt på ditt sparark och använd sedan joysticken för att gå vidare till nästa BLOCK. Ta god tid på dig: en enkel felinriktning kan göra QR-koden oanvändbar.
...
Grattis, din Bitcoin-portfölj är nu igång och fungerar! Vi kommer nu att importera dess offentliga komponenter till **Sparrow wallet** för att hantera den enkelt.
...
Sätt i microSD-minnet som innehåller operativsystemet, slå på din SeedSigner och ladda sedan den seed som du just har skapat från din backup-QR-kod. Välj `Scan` på startskärmen och skanna sedan din SeedQR med SeedSigner.
...
Ett varningsmeddelande kommer att visas. Detta är normalt: med denna utökade publika nyckel (`xpub`) kan du se alla adresser som härrör från din seed (på det första kontot). Den tillåter dig inte att spendera dina pengar, men den avslöjar strukturen i din portfölj. Om det någonsin läcker ut är det ett problem för din integritet, men inte för säkerheten för dina bitcoins: det låter dig se dem, men inte spendera dem.
...
Se till att du har Sparrow wallet-programvaran installerad på din dator. Om du inte vet hur du laddar ner, kontrollerar och installerar den korrekt, se vår fullständiga handledning i ämnet:
...
Tilldela ett namn till din portfölj och klicka sedan på `Create Wallet`. Sparrow kommer sedan att be dig att ange ett lösenord för att låsa lokal åtkomst till denna Wallet. Välj ett starkt lösenord: det skyddar åtkomsten till dina portföljdata i Sparrow (publika nycklar, adresser, etiketter och transaktionshistorik). Det här lösenordet behövs inte för att återställa portföljen vid ett senare tillfälle: endast din Mnemonic-fras (och eventuellt din passphrase) behövs för detta ändamål.
...
Din SeedSigner och Sparrow wallet är nu säkert länkade. Sparrow fungerar som en komplett hantering av Interface, medan SeedSigner förblir den enda enheten som kan signera dina transaktioner. Du är nu redo att ta emot och skicka bitcoins i en helt luftgapad konfiguration.
...
Din SeedSigner och Sparrow wallet är nu konfigurerade för att fungera tillsammans. I det här sista avsnittet tittar vi på hur man tar emot och skickar bitcoins med hjälp av den här konfigurationen.
...
#### 7.1.1 Generera en mottagning Address



Öppna Sparrow wallet på din dator och lås upp din SeedSigner Wallet med hjälp av ditt lösenord. Kontrollera att programvaran är ansluten till en server (skåran längst ner till höger). Klicka på `Receive` i sidofältet.
...
En ny Bitcoin Address visas. Du kommer att se :




- Texten Address (börjar med `bc1p...` om du använder P2TR som jag gör),
- Den motsvarande QR-koden,
- Ett "Label"-fält för att spåra dina transaktioner.



Jag rekommenderar starkt att du lägger till en etikett på varje Bitcoin-kvitto på din Wallet. Detta gör att du enkelt kan identifiera varifrån varje UTXO kommer och förbättra din integritetshantering. För att fördjupa dig i detta viktiga ämne kan du kolla in den dedikerade utbildningen på Plan ₿ Academy :
...
Din Address är nu associerad med den här etiketten i alla Sparrow-avsnitt.
...
Innan du delar din mottagna Address är det mycket viktigt att kontrollera att den tillhör din seed. Detta steg säkerställer att din SeedSigner kommer att kunna signera transaktioner som är associerade med denna Address. Det skyddar också mot möjliga attacker där Sparrow visar en bedräglig Address. Kom ihåg att Sparrow körs i en osäker miljö (din dator), som har en mycket större attackyta än din SeedSigner, som är helt isolerad. Därför bör du aldrig blint tro på en mottagares Address som visas på Sparrow förrän du har verifierat den med din Hardware Wallet.



På Sparrow, klicka på QR-koden för Address för att förstora den: den kommer då att visas i helskärm.
...
Om den skannade Address matchar den som härrör från din seed, kommer SeedSigner-skärmen att visa meddelandet: `Address verifierad`.
...
Detta bekräftar att Address tillhör din Wallet och att du tryggt kan ta emot bitcoins från den.
...
Du kan nu kommunicera denna Address (i form av text eller QR-kod) till den person eller avdelning som behöver skicka dig Satss. När transaktionen har sänts ut i nätverket kommer den att visas på fliken Transaktioner i Sparrow wallet.
...
- Transaktionsskapande i Sparrow ;
...
I Sparrow wallet kan du klicka på fliken `Sänd` i den vänstra sidofältet. Jag föredrar dock att använda fliken `UTXOs`, som låter dig öva "* Coin Control*". Denna metod ger dig exakt kontroll över de UTXO som används, så att du kan kontrollera den information du avslöjar under transaktionen.
...
- I `Pay to` klistrar du in mottagarens Address eller klickar på kameraikonen för att skanna QR-koden;
- I `Label`, lägg till en etikett för att spåra denna kostnad;
- I `Amount` anger du det belopp som ska skickas;
- Välj slutligen avgiftssats baserat på aktuella marknadsförhållanden (uppskattningar finns tillgängliga på [Mempool.space](https://Mempool.space/)).
...
Välj sedan den seed som passar din portfölj.
...
Klicka på `Review Details` och kontrollera noggrant all information direkt på SeedSigner-skärmen. Det viktigaste att kontrollera är det skickade beloppet, mottagarens Address och beloppet för eventuella avgifter.
...
Om allt är korrekt väljer du `Approve PSBT` för att signera transaktionen med motsvarande privata nyckel(n).
...
När SeedSigner har signerat genererar den en ny QR-kod som innehåller den signerade transaktionen, redo att skannas av Sparrow.
...
Nu när transaktionen är giltig måste den sändas ut i Bitcoin-nätverket så att den når en Miner som lägger till den i en BLOCK.
