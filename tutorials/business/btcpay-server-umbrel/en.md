...
This tutorial takes you through the complete installation of BTCPAY SERVER on Umbrel, configuring your Bitcoin wallet and Lightning node, creating and paying invoices, and managing accounting reporting. You'll find out how to use BTCPAY SERVER effectively on your local network, and then we'll talk about solutions for public display if you want to integrate it with an e-commerce site.
...
Your Bitcoin core node must be fully synchronized with Blockchain (100% in Umbrel's Bitcoin application). This initial synchronization usually takes between 3 days and 2 weeks, depending on your hardware and Internet connection.
...
From Umbrel Interface (`umbrel.local`), go to the App Store and search for "BTCPAY SERVER" in the Bitcoin category.
...
Once your account has been created, BTCPAY SERVER will immediately prompt you to set up your first store. Choose a professional name and select a reference currency (EUR, USD or BTC).
...
To accept payments, you need to configure a Bitcoin wallet. BTCPAY SERVER displays the configuration options in the dashboard.
...
In Interface BTCPAY SERVER, navigate to Invoices > Create Invoice. Enter the amount, add an optional description, and click Create.
...
BTCPAY SERVER lets you manage multiple stores with distinct parameters. Each store represents a separate business entity: e-commerce store, physical point of sale, or service billing.
...
- General Settings**: Store name, reference currency (BTC, EUR, USD), invoice expiry time (default 15 minutes), number of Blockchain confirmations required
...
- Access Tokens**: API token management for e-commerce integrations (WooCommerce, Shopify, etc.)
...
**Benefits of BTCPAY SERVER on Umbrel** :
...
- Local network only**: BTCPAY SERVER on Umbrel is only accessible from your home network. Perfect for face-to-face billing, freelance services or small physical businesses, but unsuitable for online stores that are publicly accessible on the Internet.
...
This LAN limitation is the main obstacle to integrating BTCPAY SERVER into an e-commerce store, where customers need to be able to access payment pages from anywhere on the Internet.


## Best practices and safety


Activate automatic Umbrel backups and store a copy on external media (USB stick, hard disk, encrypted cloud). Keep your Bitcoin seeds (recovery phrases) in a safe, physically separate place. Save the LND channel.backup file for Lightning recovery.
...
## Show BTCPAY SERVER publicly for an online store


To integrate BTCPAY SERVER into a web-based e-commerce store (WooCommerce, Shopify, etc.), your customers need to be able to access the payment pages from anywhere, not just from your local network.
