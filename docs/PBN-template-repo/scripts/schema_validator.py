...
The validator auto-detects the repository root and schema locations.
Works with both absolute paths and relative paths from anywhere in the repo.

Usage:
    python schema_validator.py <folder_path>
    python schema_validator.py ../../courses/btc101
    python schema_validator.py /path/to/repo/tutorials/wallet/sparrow-wallet
    python schema_validator.py professors/pbn-author
"""
...
CONTENT_TYPES: dict[str, ContentTypeConfig] = {
    ...
    "resources/projects": ContentTypeConfig(
        ...
        schema_file="non-community-builder-scheme.json",  # Default, may need community detection
        content_schema_file="non-community-builder-content-scheme.json",
        content_uses_yml=True,
    ),
}
...
class SchemaValidator:
    ...
    def __init__(self, base_path: str, schema_path: Optional[str] = None):
        self.base_path = Path(base_path).resolve()
        # Schema path defaults to template repo location
        if schema_path:
            self.schema_path = Path(schema_path).resolve()
        else:
            # Auto-detect schema path: look for docs/PBN-template-repo relative to base_path
            template_repo = self.base_path / "docs" / "PBN-template-repo"
            if template_repo.exists():
                self.schema_path = template_repo
            else:
                # Try going up to find it
                self.schema_path = self._find_template_repo(self.base_path)
        self.results: list[ValidationResult] = []

    def _find_template_repo(self, start_path: Path) -> Path:
        """Find the template repo by traversing up the directory tree."""
        current = start_path
        for _ in range(10):  # Max 10 levels up
            template_repo = current / "docs" / "PBN-template-repo"
            if template_repo.exists():
                return template_repo
            if current.parent == current:
                break
            current = current.parent
        # Fallback to base_path if not found
        return start_path
    ...
    def find_schema_file(self, config: ContentTypeConfig, folder_path: Path) -> Optional[Path]:
        """Find the schema file for the content type."""
        # Find the content type path prefix
        repo_root = self._find_repo_root(folder_path)
        try:
            rel_path = folder_path.relative_to(repo_root)
        except ValueError:
            rel_path = folder_path

        # Determine the content type directory (e.g., "tutorials", "courses", "resources/bet")
        for content_path in CONTENT_TYPES.keys():
            content_parts = content_path.split("/")
            if list(rel_path.parts)[:len(content_parts)] == content_parts:
                # Found the content type - look for schema in template repo
                schema_in_template = self.schema_path / content_path / config.schema_file
                if schema_in_template.exists():
                    return schema_in_template
                break

        # Fallback: search in parent directories relative to folder
        search_paths = [
            folder_path / config.schema_file,
            folder_path.parent / config.schema_file,
            folder_path.parent.parent / config.schema_file,
            folder_path.parent.parent.parent / config.schema_file,
        ]

        # Also check in repo root content type directories
        for content_path in CONTENT_TYPES.keys():
            search_paths.append(repo_root / content_path / config.schema_file)
    ...
    def find_content_schema_file(self, config: ContentTypeConfig, folder_path: Path) -> Optional[Path]:
        """Find the content schema file for the content type."""
        ...
        # Determine the content type directory and look in template repo
        for content_path in CONTENT_TYPES.keys():
            content_parts = content_path.split("/")
            if list(rel_path.parts)[:len(content_parts)] == content_parts:
                schema_in_template = self.schema_path / content_path / config.content_schema_file
                if schema_in_template.exists():
                    return schema_in_template
                break
        ...
        search_paths = [
            folder_path / config.content_schema_file,
            folder_path.parent / config.content_schema_file,
            folder_path.parent.parent / config.content_schema_file,
            folder_path.parent.parent.parent / config.content_schema_file,
        ]

        # Also check in repo root content type directories
        for content_path in CONTENT_TYPES.keys():
            search_paths.append(repo_root / content_path / config.content_schema_file)
    ...
    def validate_folder(self, folder_path: str) -> list[ValidationResult]:
        ...
        if not yaml_path.exists():
            ...
            schema_path = self.find_schema_file(config, folder)
            if schema_path:
        ...
        if config.has_markdown_content:
            content_schema_path = self.find_content_schema_file(config, folder)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python schema_validator.py tutorials/this-folder-is-a-tutorial-category/awesome-tutorial
  python schema_validator.py professors/pbn-author
  python schema_validator.py resources/bet/a-game-on-bitcoin
  python schema_validator.py events/awesome-workshop-YYYY
...
Supported content types:
  ...
  - resources/projects/<project-name>
    ...
    parser.add_argument(
        "--schema-path",
        default=None,
        help="Path to schema files (defaults to docs/PBN-template-repo in repo root)"
    )
    ...
    if not schema_path:
        # Try to find template repo
        template_repo = base_path / "docs" / "PBN-template-repo"
        if template_repo.exists():
            schema_path = str(template_repo)

    validator = SchemaValidator(str(base_path), schema_path)
