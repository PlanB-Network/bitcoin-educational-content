---
name: Elementary OS
description: Uingizwaji bora wa Windows na MacOS
---

![cover](assets/cover.webp)



Elementary OS ni mfumo wa uendeshaji unaotegemea Ubuntu, iliyoundwa kuwa rahisi, haraka na thabiti kwa matumizi mengi ya kila siku. Inawakilisha mbadala ya Linux yenye usawa kwa MacOS na Windows. Interface yake ya kiowevu, angavu na isiyo na vitu vingi hurahisisha kujifunza, hata kwa wanaoanza. Pia inazingatia ergonomics, usalama na utendaji.



https://planb.network/tutorials/computer-security/operating-system/ubuntu-78a3be56-5d51-4ec3-8629-0dd27c352ab5

## Kwa nini uchague Mfumo wa Uendeshaji wa Msingi





- **Urahisi na urahisi wa kutumia**: Mchoro wa Interface wa Elementary OS uko katikati kati ya MacO na Windows. Uzoefu huu hurahisisha kutumia, hata kwa watumiaji wasio na uzoefu.





- **Usalama**: Kama ugawaji mwingi wa Linux, Mfumo wa Uendeshaji wa Msingi hufaidika na kiwango cha juu cha usalama. Sasisho za mara kwa mara, usimamizi wa haki na kutokuwepo kwa virusi vya kawaida hufanya mfumo wa kuaminika.





- **Kasi**: Mfumo wa Uendeshaji wa Msingi ni usambazaji mwepesi. Inahitaji rasilimali chache, na kuifanya iwe ya haraka na inayofaa kwa kompyuta zilizo na usanidi wa kawaida.





- **Bure**: Mfumo ni bure kabisa. Hata hivyo, unapoipakua, unaweza kutoa mchango ili kusaidia wasanidi programu.





- **Jumuiya inayotumika**: Jumuiya inayozunguka Mfumo wa Uendeshaji wa Msingi ni tofauti na inaitikia. Ikiwa unapata shida, unaweza kupata msaada kwa urahisi kwenye vikao au mitandao ya kijamii.



## Ufungaji na usanidi


### Mahitaji ya vifaa



Kabla ya kuanza ufungaji, hakikisha kuwa una vifaa vifuatavyo:





- ** Kitufe cha USB** cha angalau GB 12
- Kumbukumbu ya **RAM** ya angalau GB 4
- **Diski ya Hard ya GB 20** au zaidi kwa matumizi ya starehe



## Pakua picha ya ISO



Nenda kwenye tovuti rasmi ya mfumo wa uendeshaji [msingi](https://elementary.io/) na uchague kiasi cha fedha ili kusaidia mradi. Hatua hii ni ya hiari.


Iwapo ungependa kupakua picha ya ISO bila malipo, ingiza 0 katika **"uga Nyingine "** na uanze kupakua picha ya mfumo wa ISO.



![0_01](assets/fr/01.webp)



## Inaunda ufunguo wa USB unaoweza kuwashwa



Mara tu unapopakua picha ya ISO, utahitaji kuifanya iweze kuwashwa kwenye kitufe cha USB ili kuendelea na usakinishaji.



Pakua programu kama vile [Balena Etcher](https://etcher.balena.io/) au zana kama hiyo, kisha uzindue programu.


Chagua picha ya **Elementary OS** ISO iliyopakuliwa na uweke ufunguo wako wa USB kama lengwa.



Bofya kwenye kitufe cha ** flash ** ili kuanza mchakato, na kusubiri hadi mchakato ukamilike kabla ya kuondoa ufunguo wa USB.



![0_02](assets/fr/02.webp)



## Kuanzisha ufunguo na ufikiaji wa BIOS



Zima kompyuta ambayo ungependa kusakinisha Elementary OS, kisha chomeka kitufe cha USB.


Kompyuta yako inapowashwa, fikia BIOS (`ESC`, `F9` au `F11` kulingana na chapa) na uchague kitufe cha USB kama kifaa cha kuwasha, kisha ubonyeze `Enter` ili kuanza mchakato wa kuwasha.



## Inasakinisha Mfumo wa Uendeshaji wa Msingi



Usakinishaji huanza kiotomatiki ikiwa ufunguo wa USB umesanidiwa kwa usahihi.



### Uchaguzi wa lugha



Chagua lugha ambayo ungependa kusakinisha mfumo. Unaweza pia kuchagua lahaja ya kikanda.



![0_03](assets/fr/03.webp)



![0_04](assets/fr/04.webp)



### Mpangilio wa kibodi



Chagua mpangilio unaolingana na kibodi yako. Sehemu imetolewa ili kuangalia ikiwa funguo hutoa herufi sahihi.



![0_05](assets/fr/05.webp)



![0_06](assets/fr/06.webp)



### Mtihani wa hali



Mfumo wa Uendeshaji wa Msingi hukuwezesha kujaribu mfumo kabla ya kuusakinisha. Hali hii hukuruhusu kuchunguza Interface bila kurekebisha chochote kwenye diski yako ya Hard.



### Kuzindua ufungaji





- Bonyeza **"Futa diski na usakinishe "** ili kusakinisha moja kwa moja kwenye diski nzima.
- Bonyeza **"Sakinisha maalum "** ikiwa ungependa kudhibiti kizigeu wewe mwenyewe.



![0_07](assets/fr/07.webp)



### Uchaguzi wa diski



Chagua diski ambayo Elementary OS itasakinishwa, kisha bofya kitufe cha Endelea.



![0_08](assets/fr/08.webp)



### Usimbaji fiche wa diski



Chaguo hukuruhusu kusimba diski ili **kulinda data yako**. Utahitaji kuweka nenosiri thabiti ili kuwezesha ulinzi huu. Hata hivyo, ni hiari.



![0_09](assets/fr/09.webp)



![0_10](assets/fr/10.webp)



### Kuzindua ufungaji



Kabla ya kuzindua usakinishaji, unaweza kuidhinisha usakinishaji wa kiotomatiki wa viendeshi vya ziada vya vifaa, kulingana na utangamano wa mashine yako.





- Bonyeza "Futa na usakinishe" ili kuanza usakinishaji.
- Subiri hadi mchakato ukamilike.



![0_11](assets/fr/11.webp)



![0_12](assets/fr/12.webp)



### Anzisha upya



Mara baada ya kumaliza, bofya kitufe cha **ingiza ** ili kuanzisha upya, kisha uondoe kitufe cha USB kwa wakati unaofaa ili kuepuka kuanzisha upya usakinishaji.



![0_13](assets/fr/13.webp)



## Usanidi baada ya ufungaji



Baada ya kuwasha upya, hatua chache za ziada zinahitajika.



![0_14](assets/fr/14.webp)



### Uchaguzi wa lugha



Thibitisha au ubadilishe lugha ya mfumo wakati wa kuingia.



![0_15](assets/fr/15.webp)



### Mpangilio wa kibodi



Hakikisha mpangilio wa kibodi ndio unaotaka.



![0_16](assets/fr/05.webp)



### Kuunda akaunti ya mtumiaji



Husisha akaunti ya mtumiaji na mfumo wako wa uendeshaji kwa kufafanua jina la mtumiaji na kisha kupata ufikiaji wa data yako kwa nenosiri la alphanumeric la angalau vibambo na alama 20.



![0_17](assets/fr/17.webp)



### Uunganisho wa kwanza



Unapoingia kwa mara ya kwanza, Mfumo wa Uendeshaji wa Msingi hukuwezesha kubinafsisha mazingira yako kwa mipangilio michache ya msingi.



![0_18](assets/fr/18.webp)



![0_19](assets/fr/19.webp)



## Sasisho la mfumo



Kabla ya matumizi ya muda mrefu, ni muhimu kusasisha mfumo na viraka vya hivi karibuni.


### Chaguo 1: Sasisha kupitia michoro ya Interface



Ingiza **AppCenter** na ubofye kwenye ikoni ya sasisho kwenye kona ya juu kulia. Subiri masasisho yaorodheshwe, kisha ubofye **"Sasisha Zote"** ili kuanza usakinishaji.



![0_20](assets/fr/20.webp)



![0_21](assets/fr/21.webp)



### Chaguo 2: Sasisha kupitia terminal



Unaweza pia kufanya sasisho kutoka kwa mstari wa amri kupitia terminal yako: chaguo lililopendekezwa kwa usahihi wake.



```shell
sudo apt update
```



```shell
sudo apt full-upgrade
```



Kwa masasisho yako ya kwanza, mfumo wa uendeshaji unahitaji nenosiri lako la mtumiaji na uthibitisho ili kusasisha vifurushi vya programu, hata kwenye Kiini cha Msingi.



## Mpangilio wa mazingira ya kazi



Mfumo wa Uendeshaji wa Msingi ni pamoja na zana muhimu tu. Ili kurekebisha mazingira yako kulingana na mahitaji yako, haswa ikiwa wewe ni msanidi programu, tunapendekeza usakinishe zana za ziada.





- Unaweza kuongeza utegemezi muhimu na amri ifuatayo (ili kubadilishwa kwa mahitaji yako):



```shell
sudo apt update && sudo apt install -y git python3 python3-pip build-essential wget curl zsh make snapd && sudo snap install code --classic
```



Amri hii husakinisha **Git**, **Python 3**, **pip**, **zana za mkusanyaji**, **wget**, **curl**, **zsh**, **make**, **snapd** na **vscode** ili kuandaa mazingira ya msingi ya maendeleo.



Mfumo wa Uendeshaji wa Msingi sasa umeanza kutumika kwenye mashine yako. Falsafa yake ya unyenyekevu, wepesi na uzuri hufanya kuwa chaguo bora kwa matumizi ya kibinafsi na ya kitaaluma. Unapata mfumo thabiti, wa majimaji na usio na vitu vingi, tayari kubinafsishwa kulingana na matakwa yako. Iwe ni kwa ajili ya maendeleo, matumizi ya ofisi au kuvinjari kila siku, kila kitu kiko tayari kujenga mazingira bora, angavu na ya kufurahisha ya kufanyia kazi. Pia angalia mafunzo yetu juu ya Fedora, usambazaji rahisi, thabiti na wa kawaida wa Linux.



https://planb.network/tutorials/computer-security/operating-system/fedora-8c17b6ca-5acb-4825-a069-4474375534b0