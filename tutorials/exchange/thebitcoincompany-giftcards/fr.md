...
**The Bitcoin Company** est comme sont concurrent Bitrefill ([Plan ₿ Network - Bitrefill](https://planb.network/fr/tutorials/exchange/centralized/bitrefill-8c588412-1bfc-465b-9bca-e647a647fbc1)) un moyen d'échanger vos bitcoins contre des cartes cadeaux d'enseignes grand public. Vous pouvez ainsi pratiquement vivre sous un "bitcoin standard" grâce à ce type de solutions, en créant un simple compte.
...
Pour cela rendez-vous sur le site internet www.thebitcoincompany.com .
...
Sur la page suivante vous est présenté un QR code qu'il vous faudra scanner avec votre wallet Lightning favori.
Votre wallet Lightning devient ainsi votre moyen d'identification unique pour tous les sites supportant ce protocole, appelé LNURL-AUTH (pour authentification).
Plus besoin de mail ni de mot de passe pour chaque site votre wallet vous identifie, et cerise sur le gateau, une nouvelle identité est générée pour chaque site visité. Pratique du point de vue de la préservation de la vie privée.

![TheBitcoinCompany](assets/fr/08.webp)

Ouvrons donc notre wallet Phoenix ([Plan ₿ Network - Phoenix](https://planb.network/fr/tutorials/wallet/mobile/phoenix-0f681345-abff-4bdc-819c-4ae800129cdf)), et scannons le QR code présenté.
Une fois scanné cliquez sur "Sign-in", puis "OK".
...
Et ainsi observer votre *Username* qui est un nombre aléatoire et qui sera différent pour tous les sites visités supportant ce mode d'authentification. En bas de l'écran on voit effectivement que la méthode d'authentification est bien spécifié comme étant LNURL Auth.
...
Vous voici sur la page de présentation de la carte cadeau choisie.
N'hésitez pas à aller jeter un œil aux détails qui pourraient avoir leur importance au moment de d'utiliser la carte sur le site du marchand. Renseignez le montant que vous souhaitez attribuer à la carte cadeau, nous choisirons pour l'exemple 10 USD mais cela peut aller si vous le souhaitez jusqu'à 500 USD par carte.
...
Payez l'invoice affichée ensuite soit on-chain soit via Lightning.

![TheBitcoinCompany](assets/fr/16.webp)

Une fois que l'invoice est payée, on vous indique de la carte cadeau vient d'être expédié par mail au destinataire choisi à l'étape précédente. Allons vérifier comment ça se passe.
...
Pour finir, notons qu'il est possible de consulter à tout moment l'ensemble des cartes cadeau achetées via ce compte grâce à l'onglet "My Cards" en haut de l'écran. Une fois qu'une carte est utilisée n'oubliez pas de l'archiver afin que vous y retrouver. Vous pourrez ainsi naviguer plus clairement entre vos cartes encore valides, et les autres déjà vides.

![TheBitcoinCompany](assets/fr/21.webp)

Enfin pour terminer, si vous souhaitez utiliser **sur mobile** la méthode d'authentification LNURL Auth que nous avons vu lors de ce tuto, il vous faudra accéder au site de The Bitcoin Company via votre navigateur mobile puis ajouter un raccourci sur votre écran d'accueil. En effet l'application officielle permet uniquement l'authentification par email/mot de passe.
