...
Tiankii - termine nahuatl che significa "mercato all'aperto accessibile a tutti" - è la prima start-up al 100% di Bitcoin di El Salvador. Fondata da Darvin Otero in seguito all'adozione del Bitcoin come moneta legale del Paese, l'azienda mira a trasformare il Bitcoin da riserva di valore a moneta di scambio per gli acquisti quotidiani. A differenza delle piattaforme di custodia, Tiankii adotta un approccio non custodiale in cui gli utenti mantengono il controllo dei propri fondi e l'infrastruttura funge solo da intermediario tecnico.
...
Tiankii si posiziona come fornitore di un'infrastruttura Bitcoin-as-a-Service basata sul Lightning Network. Questa tecnologia di secondo livello consente transazioni quasi istantanee con costi trascurabili, rendendo possibili micropagamenti e acquisti quotidiani.
...
**Standard aperti**: Uso di LNURL per automatizzare le richieste di pagamento, Lightning Address per ID e-mail leggibili e Bolt Card per pagamenti NFC interoperabili.



**Modularità wallet-agnostica**: Possibilità di collegare diversi portafogli Lightning (LNbits, Blink, BlueWallet) o il proprio nodo, offrendo la massima flessibilità in base al livello di competenza e autonomia richiesto.
...
### Carta senza contatto Bitcoin (carta Bolt)



La carta NFC Bolt rappresenta la principale innovazione di Tiankii nella democratizzazione del Bitcoin per il pubblico. Funzionando come una carta bancaria contactless, consente di pagare gli acquisti Bitcoin Lightning semplicemente toccando un terminale compatibile.
...
### App IBI - Interface singolo Bitcoin



L'applicazione IBI (Individual Bitcoin Interface) è progettata per le persone che desiderano utilizzare il Bitcoin Lightning su base quotidiana. Il suo principale vantaggio consiste nella generazione di un Address Lightning personalizzato, un identificativo di pagamento leggibile in formato e-mail (esempio: alice@ibi.me).



Questo identificatore semplifica drasticamente la ricezione dei pagamenti: non c'è bisogno di generate una nuova fattura per ogni transazione, il mittente può semplicemente inserire l'indirizzo Lightning. L'interfaccia consente inoltre di gestire la carta Bolt (attivazione, disattivazione, limiti di spesa), di collegare vari portafogli Lightning e di effettuare pagamenti tramite la scansione di codici QR.
...
**Per gli utenti privati (App IBI)**: Non è necessario un wallet Lightning precedente. Quando si crea un account, Tiankii configura automaticamente un Address Lightning funzionante tramite [Breez SDK Liquid](https://sdk-doc-liquid.breez.technology/guide/about_breez_sdk_liquid.html), un'implementazione nodeless che utilizza la rete Liquid in background. È possibile ricevere e inviare immediatamente pagamenti senza alcuna configurazione tecnica. Questa soluzione semplifica drasticamente l'accesso ai principianti, pur rimanendo auto-custode.



**Per gli esercenti (Merchant Dashboard)** : La connessione di un wallet Lightning esistente è obbligatoria per utilizzare i terminali del punto vendita e ricevere pagamenti. Tiankii supporta molte soluzioni: portafogli mobili (Blink, Strike), soluzioni self-hosted (LNbits, nodo LND/CLN) o portafogli web (Alby). Le guide dettagliate alla connessione sono disponibili nella sezione Risorse di questo tutorial.
...
Andate su accounts.ibi.me per creare il vostro conto. In questa pagina è possibile scegliere tra due tipi di account: "Uso personale" per uso individuale o "Uso commerciale" per uso commerciale. Selezionate "Uso personale" per accedere agli strumenti per la ricezione e l'invio di pagamenti nel Bitcoin.
...
Dopo aver selezionato "Uso personale", sarete automaticamente reindirizzati a go.ibi.me per completare la registrazione. La registrazione può essere effettuata tramite e-mail, numero di telefono o utilizzando il proprio account Google, Microsoft o Twitter. Una volta creato, potrete accedere immediatamente alla vostra interfaccia IBI, con il vostro Lightning Address già operativo.
...
L'interfaccia di IBI visualizza il saldo in satoshi e in valuta locale (USD). Tre pulsanti consentono di interagire: "Ricevi" per ricevere pagamenti, "Scansiona" per scansionare un codice QR e "Invia" per inviare satoshis.
...
Per ricevere i satoshi, premere "Ricevi". L'applicazione genera automaticamente un codice QR e visualizza il vostro Address Lightning personalizzato (formato nom@ibi.me). Condividete questo indirizzo o il codice QR con il mittente: i fondi arriveranno immediatamente sul vostro conto IBI.
...
Per inviare i satoshi, premere "Invia". È possibile scansionare un codice QR Lightning o inserire direttamente una destinazione Address Lightning.
...
Inserire l'importo desiderato in satoshi, verificare l'importo equivalente in valuta locale e confermare il pagamento.
...
Andate su accounts.ibi.me per creare il vostro conto. Selezionate "Uso commerciale" per accedere agli strumenti del commerciante. Questo tipo di conto sblocca l'accesso alla Merchant Dashboard e ai terminali dei punti vendita.
...
Passo fondamentale per l'attivazione del punto vendita: collegare il wallet Lightning per ricevere i pagamenti. L'interfaccia offre diverse opzioni di connessione. Si noti che alcune opzioni (Bitcoin Onchain e Lightning Network) sono ancora in fase di sviluppo e appaiono in grigio sull'interfaccia.
...
Per questa esercitazione, utilizzeremo la connessione Lightning Address, compatibile con Chivo, Blink Wallet e Strike. Inserite il vostro Lightning Address (formato nom@domaine.com), ad esempio da LN Markets, Alby o qualsiasi altro fornitore compatibile.
...
Una volta effettuato l'accesso, l'account è operativo. È ora possibile accedere al POS o tornare al cruscotto per configurare altre impostazioni.
...
Per accettare pagamenti in negozio, accedere al menu "Terminale di vendita" da "Strumenti di pagamento". Questa sezione vi permette di creare e gestire i vostri terminali POS (il numero di terminali dipende dal vostro piano, vedi i piani Freemium e Business). Fare clic su "Apri" per aprire l'interfaccia POS nel browser.
...
Il terminale POS visualizza un tastierino numerico per l'inserimento dell'importo della vendita. Inserire l'importo nella valuta locale (ad esempio, 500 sats corrispondono a 630,74 ARS), quindi premere "OK" per confermare.
...
L'applicazione genera istantaneamente un codice QR Lightning e una Address Lightning per il pagamento. I clienti possono scansionare il codice QR con il loro wallet o utilizzare la carta Bolt su un terminale NFC.
...
Tutte le transazioni sono registrate nel vostro Merchant Dashboard. È possibile tenere traccia completa dei ricavi per periodo, del numero totale di vendite e dello storico dettagliato per la contabilità.



L'interfaccia delle impostazioni offre diverse schede di configurazione. La scheda "Impostazioni generali" consente di gestire il profilo del commerciante e le notifiche. La scheda "Utenti" consente di aggiungere e gestire l'accesso alla Merchant Dashboard per il proprio team (gestione multiutente in base al proprio piano). La scheda "Spazio di lavoro per lo sviluppo" dà accesso alle chiavi API per le integrazioni avanzate, mentre "Abbonamento" consente di gestire l'abbonamento.
...
**Autotutela**: Mantenete le vostre chiavi private e il pieno controllo dei vostri fondi. Nessun rischio di blocco del conto o di fallimento della piattaforma di terzi.



**Semplicità**: Lightning Address come indirizzo e-mail, pagamenti NFC con un semplice tocco, interfaccia intuitiva senza necessità di competenze tecniche.



**Ecosistema completo**: Strumenti per tutti i profili (privati, rivenditori, sviluppatori) con integrazioni modulari per soddisfare le vostre esigenze.
...
**Dipendenza da SaaS**: Se Tiankii non è disponibile, la generazione di fatture è temporaneamente impossibile (i vostri fondi rimangono al sicuro).
...
Tiankii illustra come un'infrastruttura ben progettata possa rimuovere le barriere tecniche che impediscono l'adozione di massa del Bitcoin come valuta quotidiana. Combinando la potenza del Lightning Network con un approccio non custodiale e strumenti accessibili, l'ecosistema offre un percorso equilibrato tra sovranità finanziaria e facilità d'uso.



Per gli esercenti, Tiankii rappresenta un'opportunità per ridurre drasticamente i costi delle transazioni e accedere a una nuova base di clienti. Per i consumatori, Lightning Address e le carte NFC trasformano il Bitcoin in una moneta pratica, senza complessità tecniche.



Sebbene l'adozione diffusa del Bitcoin rimanga una sfida che richiede formazione e tempo, infrastrutture come Tiankii dimostrano che gli strumenti tecnici esistono già. La missione di semplificare i pagamenti Bitcoin non è più una visione lontana, ma una realtà accessibile a chiunque sia disposto a fare il grande passo.
...
- [Cruscotto commercianti](https://pay.tiankii.com)
...
- [Blink Wallet](https://blink.sv) - Raccomandata la compatibilità con il fulmine Wallet
