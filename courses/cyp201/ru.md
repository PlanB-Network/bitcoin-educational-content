---
name: Архитектура Bitcoin-кошелька
...
Обучение будет продолжено с изучением фразы-пароля [BIP39](https://planb.academy/resources/glossary/bip0039), начальной фразы (не путать с мнемонической фразой), основного кода цепи и мастер-ключа. Мы подробно рассмотрим, что представляют собой эти элементы, их соответствующие роли и как они рассчитываются.
...
Эта таблица предлагает вам перевод основных английских терминов, используемых для облегчения понимания схем и технических документов, применяемых в рамках курса CYP 201.

| Английский      | Перевод / Объяснение                                                                               |
| --------------- | -------------------------------------------------------------------------------------------------- |
| *pubkey hash*   | Хэш публичного ключа (используется для генерации Bitcoin-адреса).                                  |
| *public key*    | Публичный ключ (используется для получения средств, выводится из приватного ключа).                 |
| *signature*     | Цифровая подпись (криптографическое доказательство того, что сообщение исходит от владельца приватного ключа). |
| *scriptPubKey*  | Скрипт блокировки (определяет условия для траты выхода).                                            |
| *scriptSig*     | Скрипт разблокировки (предоставляет данные для выполнения *scriptPubKey*).                          |
| *Stack*         | Стек (структура данных, используемая *Bitcoin Script*).                                             |
| *input*         | Вход транзакции (ссылка на предыдущий выход, используемый как источник).                            |
| *output*        | Выход транзакции (определяет получателя и сумму).                                                   |
| *transaction*   | Транзакция Bitcoin (набор входов и выходов, подтверждающих перевод).                                |
| *XOR*           | Логический оператор "исключающее ИЛИ", используется в некоторых криптографических схемах.           |
| *HMAC*          | Код аутентификации сообщения, основанный на хэше и секретном ключе.                                 |
| *ECDSA*         | Алгоритм цифровой подписи на эллиптических кривых.                                                 |
| *hash*          | Хэш (уникальный и фиксированный отпечаток данных).                                                 |
| *SigHash*       | Тип хэша подписи (определяет, какие части транзакции подписываются).                                |
| *HD Wallet*     | Иерархический детерминированный кошелёк (генерирует несколько ключей из одного сида).               |
| *Random Number* | Случайное число (используется для генерации безопасных приватных ключей).                           |
| *State*         | Состояние (промежуточное значение в криптографическом процессе).                                    |
| *Entropy*       | Энтропия (мера случайности, используемая для генерации сидов кошелька).                             |
| *Mnemonic*      | Мнемоника (последовательность слов, упрощающая резервное копирование и восстановление сида).        |
| *Wordlist*      | Список слов (предопределённый набор, используемый для генерации BIP39-мнемоник).                    |
| *Seed*          | Сид (начальное значение, позволяющее вывести все ключи HD-кошелька).                                |
| *Address*       | Адрес Bitcoin (читаемый идентификатор для получения средств, выводится из публичного ключа).       |
| *Leaf*          | Лист (конечный узел в дереве деривации).                                                           |
...
Первый тип криптографических алгоритмов, используемых в Bitcoin, включает в себя хеш-функции. Они играют существенную роль на разных уровнях протокола, но также и внутри Bitcoin кошельков. Давайте вместе узнаем, что такое [хеш-функция](https://planb.academy/resources/glossary/hash-function) и для чего она используется в Bitcoin.
...
Например, хеш-функция [SHA256](https://planb.academy/resources/glossary/sha256) производит хеш фиксированной длины 256 бит. Таким образом, если мы используем вход "_PlanB_", сообщение произвольной длины, сгенерированный хеш будет следующим 256-битным отпечатком:
...
Эта функция используется во многих аспектах Bitcoin. На уровне протокола она участвует в механизме Proof-of-Work, где применяется двойное хеширование для поиска частичного столкновения между заголовком кандидата [блока](https://planb.academy/resources/glossary/block), созданного майнером, и целевым значением сложности. Если это частичное столкновение найдено, кандидат блок становится действительным и может быть добавлен в [блокчейн](https://planb.academy/resources/glossary/blockchain).

SHA256 также используется при построении дерева Меркла, которое является аккумулятором, используемым для записи транзакций в блоках. Эта структура также найдена в протоколе [Utreexo](https://planb.academy/resources/glossary/utreexo), который позволяет уменьшить размер набора UTXO. Кроме того, с введением [Taproot](https://planb.academy/resources/glossary/taproot) в 2021 году, SHA256 используется в [MAST](https://planb.academy/resources/glossary/mast) (_Merkelised Alternative [Script](https://planb.academy/resources/glossary/script) Tree_), что позволяет раскрывать только условия расходования, действительно использованные в скрипте, не раскрывая другие возможные варианты. Она также используется в расчете идентификаторов транзакций, при передаче пакетов по P2P-сети, в электронных подписях... Наконец, и это особенно интересно в рамках этого обучения, SHA256 используется на уровне приложений для построения кошельков Bitcoin и производства адресов.

Чаще всего, когда вы сталкиваетесь с использованием SHA256 в Bitcoin, это будет фактически двойной хеш SHA256, обозначаемый как "**[HASH256](https://planb.academy/resources/glossary/hash256)**", который просто состоит из двукратного последовательного применения SHA256:
...
Другая хеш-функция, доступная в языке Script и используемая для получения адресов, — это функция [RIPEMD160](https://planb.academy/resources/glossary/ripemd160). Эта функция производит 160-битный хеш (таким образом, короче, чем SHA256). Обычно она сочетается с SHA256 для формирования функции HASH160:
...
Наконец, только на уровне приложений иногда также используется функция [SHA512](https://planb.academy/resources/glossary/sha512), которая косвенно играет роль в производстве ключей для кошельков. Эта функция очень похожа на SHA256 в своей работе; обе принадлежат к одной семье SHA2, но SHA512 производит, как указывает ее название, 512-битный хеш, в сравнении с 256 битами для SHA256. Мы подробно рассмотрим ее использование в следующих главах.
...
- Результат был непредсказуемым: Любая предсказуемость может быть использована для более быстрого нахождения коллизий, чем при атаке методом [грубой силы](https://planb.academy/resources/glossary/brute-force-attack). Функция обеспечивает, что каждый бит выходных данных зависит нетривиальным образом от входных данных. Другими словами, функция разработана так, чтобы каждый бит конечного результата имел независимую вероятность быть 0 или 1, даже если на практике эта независимость не абсолютна.
...
SHA256, хотя и использует тот же тип конструкции, теоретически устойчив к этому типу атаки, в отличие от SHA1 и MD5. Это может объяснить загадку двойного хеширования, реализованного в Bitcoin [Сатоши Накамото](https://planb.academy/resources/glossary/nakamoto-satoshi). Чтобы избежать этого типа атаки, Сатоши мог предпочесть использовать двойной SHA256:
...
Термин "_кошелек_" в Bitcoin может быть довольно путанным для новичков. Действительно, то, что называется кошельком Bitcoin, это программное обеспечение, которое не хранит ваши биткойны напрямую, в отличие от физического кошелька, который может содержать монеты или купюры. Биткойны являются просто единицами учета. Эта единица учета представлена **[UTXO](https://planb.academy/resources/glossary/utxo)** (_Unspent Transaction Outputs_, непотраченные выходы транзакций), которые являются непотраченными выходами транзакций. Если эти выходы не потрачены, это означает, что они принадлежат пользователю. UTXO, таким образом, являются частями биткойнов переменного размера, принадлежащими пользователю.
...
Математическая связь между публичным и приватным ключом, а также возможность выполнения подписи для доказательства владения приватным ключом без его раскрытия, становятся возможными благодаря алгоритму цифровой подписи. В протоколе Bitcoin используются 2 алгоритма подписи: **[ECDSA](https://planb.academy/resources/glossary/ecdsa)** (_Elliptic Curve Digital Signature Algorithm_) и **схема подписи [Schnorr](https://planb.academy/resources/glossary/schnorr-protocol)**. ECDSA является цифровым протоколом подписи, используемым в Bitcoin с самого начала. Schnorr более новый в Bitcoin, так как был введен в ноябре 2021 года с обновлением Taproot.
Эти два алгоритма довольно схожи по своим механизмам. Оба они основаны на [криптографии эллиптических кривых](https://planb.academy/resources/glossary/elliptic-curve). Основное различие между этими двумя протоколами заключается в структуре подписи и некоторых специфических математических свойствах. Поэтому мы изучим функционирование этих алгоритмов, начиная с самого старого: ECDSA.
...
Если вы хотите узнать больше о математических основах современной криптографии, я также рекомендую проконсультироваться с этим другим курсом на Plan ₿ Academy:
...
Как было показано ранее, алгоритмы цифровой подписи в Bitcoin основаны на паре [приватного](https://planb.academy/resources/glossary/private-key) и [публичного ключей](https://planb.academy/resources/glossary/public-key), которые математически связаны. Давайте вместе исследуем, в чем состоит эта математическая связь и как они генерируются.
...
Эта идея была изначально сформулирована Джозефом Пуном и Таддеусом Драйджой в Белой книге по [Lightning Network](https://planb.academy/resources/glossary/lightning-network). До переименования этот флаг sighash назывался `SIGHASH_NOINPUT`.
...
Современные HD-кошельки (детерминированные и иерархические) опираются на единственный начальный элемент информации, называемый "энтропией", для детерминированной генерации всего набора ключей кошелька. Эта [энтропия](https://planb.academy/resources/glossary/entropy) представляет собой псевдослучайное число, уровень хаоса которого частично определяет безопасность кошелька.
...
- **[Chain Code](https://planb.academy/resources/glossary/chain-code)**: Уникальный 32-байтовый код для получения дочерних ключей.
...
**Глубина 1: Цель ([BIP43](https://planb.academy/resources/glossary/bip0043))**
Цель определяет логическую структуру вывода. Например, адрес P2WPKH будет иметь $/84'/$ на глубине 1 (согласно [BIP84](https://planb.academy/resources/glossary/bip0084)), в то время как адрес P2TR будет иметь $/86'/$ (согласно [BIP86](https://planb.academy/resources/glossary/bip0086)). Этот слой обеспечивает совместимость между кошельками, указывая индексные номера, соответствующие номерам BIP.
...
**Глубина 2: Тип валюты ([BIP44](https://planb.academy/resources/glossary/bip0044))**
...
Запись `/<0;1>/*` указывает, что дескриптор может генерировать адреса как из внешней цепочки (`0`), так и из внутренней (`1`). Символ подстановки (`*`) в конце пути означает, что из этой позиции можно последовательно производить все негарденированные («*unhardened*») дочерние ключи, будь то внешние или внутренние адреса. Эта синтаксис не подразумевает напрямую понятие *gap limit*, которое относится к специфическому механизму кошельков для обнаружения адресов, а служит здесь только для указания, что все возможные производные в этой точке учитываются.
...
| Стандарт             | _scriptPubKey_ | _scriptSig_ | _redeem script_ | _[witness](https://planb.academy/resources/glossary/scriptwitness)_ |
| ---------------------- | ----------------------------------------------------------- | --------------------------------- | ------------------- | -------------------------------------------- |
| P2PK                 | <*pubkey*> OP_CHECKSIG | <*signature*> | | |
| P2PKH                | OP_DUP OP_HASH160 <*pubKeyHash*> OP_EQUALVERIFY OP_CHECKSIG | <*signature*> <*public key*> | | |
| [P2SH](https://planb.academy/resources/glossary/p2sh)                 | OP_HASH160 <*scriptHash*> OP_EQUAL | <*data pushes*> <*redeem script*> | Произвольные данные | |
...
Адреса P2WPKH используют кодировку _[bech32](https://planb.academy/resources/glossary/bech32-and-bech32m)_ и всегда начинаются с `bc1q`. Этот тип скрипта соответствует выходам SegWit версии 0.
