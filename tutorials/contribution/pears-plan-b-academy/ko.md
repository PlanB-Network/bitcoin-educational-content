---
name: 지도 ₿ 아카데미 - Pears 앱
description: Pears에서 Plan ₿ Academy 애플리케이션을 설치하고 사용하려면 어떻게 하나요?
---
...
아시다시피 Plan ₿ Academy는 오픈 라이선스에 따라 게시된 강좌, 튜토리얼 및 수천 개의 리소스를 한데 모은 최대 규모의 Bitcoin 전용 교육 데이터베이스입니다. 원래 Plan ₿ Academy는 웹사이트였습니다. 하지만 검열 등의 이유로 더 이상 정상적으로 액세스할 수 없게 된다면 어떻게 될까요?



이 튜토리얼에서는 **홀펀치**가 개발하고 **테더**가 지원하는 피어투피어(P2P) 기술인 **Pears**를 통해 **플랜 ₿ 아카데미** 플랫폼을 정말 저렴하게 운영하는 방법을 배워보겠습니다.



Pears는 중앙 집중식 웹사이트에 의존하지 않고도 Plan ₿ Academy 플랫폼을 실행할 수 있는 소프트웨어입니다. 이 튜토리얼에서는 컴퓨터에 Pears를 설치하여 Pears를 통해 Plan ₿ Academy에 액세스하는 방법을 설명합니다.



서버, 호스트, 중개자 등 중앙 집중식 인프라에 의존하지 않고도 웹 애플리케이션을 배포하고 사용할 수 있도록 하는 것이 Pears의 목표입니다. 즉, 클라우드 제공업체가 문을 닫거나 한 국가가 도메인을 차단하더라도 애플리케이션은 네트워크의 피어들 사이에서 계속 사용할 수 있습니다. 이러한 접근 방식 덕분에 교육 플랫폼인 Plan ₿ Academy는 단일 장애 지점 없이 전 세계 어디에서나 계속 액세스할 수 있습니다.
...
- 다음 명령을 실행하여 요금제 ₿ 아카데미 애플리케이션을 실행합니다:
...
홀펀치는 마티아스 부스(Mathias Buus)와 파올로 아르도이노(Tether의 CEO이자 비트파이넥스의 CTO)가 설립한 회사로, P2P 로직을 Bitcoin 이상으로 확장한다는 사명을 가지고 개발 중입니다. 이들의 야망은 모든 애플리케이션이 승인 없이, 서버 없이, 중개자 없이 실행될 수 있는 "P2P 인터넷"을 구축하는 것입니다. 홀펀치는 이미 완전한 P2P 화상 회의 및 메시징 애플리케이션인 **Keet**의 뒤를 잇고 있습니다.
...
### 1.2 - Linux(Debian)에 Pears를 설치하려면 어떻게 하나요?



Debian 시스템에 Pears를 설치하는 것은 비교적 간단하지만 몇 가지 전제 조건이 필요하므로 이 섹션에서 자세히 설명하겠습니다.
...
Pears는 베어 자바스크립트 런타임에서 사용하는 `libatomic1`을 비롯한 여러 시스템 라이브러리에 의존합니다. 다음 명령어로 설치하세요:
...
#### 1.2.3 NVM을 통해 Node.js 및 npm 설치하기
...
### 1.3 - Windows에 Pears를 설치하려면 어떻게 하나요?
...
*Linux를 사용 중이고 이미 Pears를 설치한 경우 2단계로 바로 진행할 수 있습니다



#### 1.3.1. 관리자 모드에서 PowerShell 열기
...
### 1.4. MacOS에 Pears를 설치하는 방법은 무엇인가요?
...
*Linux 또는 Windows를 사용 중이고 이미 Pears를 설치한 경우 2단계로 바로 진행할 수 있습니다
...
## 2. 배에서 요금제 ₿ 아카데미는 어떻게 사용하나요?



Pears가 설치되어 실행 중이면 P2P 네트워크를 통해 **Plan ₿ Academy** 플랫폼을 직접 실행할 수 있습니다. 터미널에서 다음 명령을 실행하기만 하면 됩니다(Linux, Windows 및 macOS에서 동일한 명령어):
...
업로드가 완료되면 Plan ₿ Academy가 Pears 환경에서 열리며, 중앙 서버에 종속되지 않고 원래 웹 사이트에서와 같이 사용할 수 있습니다.
