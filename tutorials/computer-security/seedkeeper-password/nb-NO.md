...
Seedkeeper er et smartkort utviklet av Satochip, et belgisk selskap som spesialiserer seg på maskinvareløsninger for håndtering og beskyttelse av digitale hemmeligheter. Satochip er kjent for sitt utvalg av smartkort for Bitcoin-økosystemet, og Seedkeeper ble utviklet som et alternativ til tradisjonelle metoder for lagring av Mnemonic-fraser og andre digitale hemmeligheter.



Seedkeeperen har form av et multifunksjonelt, EAL6-sertifisert smartkort med en sikker prosessor og sabotasjesikkert minne (dvs. en "*secure element*"). Som navnet antyder, er kortets rolle å lagre Bitcoin-porteføljens mnemoteknikk og passord på en kryptert og beskyttet måte. Med Seedkeeper kan du generate, importere, organisere og lagre hemmelighetene dine direkte i kortets sikre komponent.
...
I denne veiledningen tar vi kun for oss bruken av SeedKeeper for tradisjonelle passord, på samme måte som en passordbehandler. Hvis du ønsker å bruke SeedKeeper til å lagre Bitcoin Wallet Mnemonic fraser, kan du se denne andre veiledningen:
...
Velg "generate hemmelighet*". Med alternativet "*Importer en hemmelighet*" kan du lagre en eksisterende hemmelighet (for eksempel et passord du har opprettet tidligere).
...
Tildel denne hemmeligheten en "*Label*", slik at den lett kan identifiseres hvis du lagrer flere opplysninger i Seedkeeper. Du kan også legge til et brukernavn knyttet til passordet og nettadressen til nettstedet.
...
Velg lengde og parametere for passordet ditt, og klikk deretter på "*generate*" og deretter "*Importer*".
...
Dette tar deg til en liste over alle dine registrerte hemmeligheter. I dette eksempelet ønsker jeg å vise passordet for Plan ₿ Network-kontoen min, så jeg klikker på det.
...
Tast inn PIN-koden til reservekortet (Seedkeeper nr. 2).
...
Det er alt som trengs! Nå vet du hvordan du bruker Seedkeeper til å lagre passordene dine. I en fremtidig veiledning vil vi se på hvordan du bruker Seedkeeper til å sikkerhetskopiere en Bitcoin-portefølje. Jeg inviterer deg også til å oppdage hvordan Seedkeeper kan brukes sammen med SeedSigner :
