...
이러한 간단한 단계가 너무 어렵게 느껴진다면 GrapheneOS 소프트웨어가 사전 설치된 (there are companies that sell them) Pixel을 구입하는 것도 고려해 볼 수 있습니다. 단, 공급업체에 약간의 신뢰가 필요하다는 점에 유의하세요.
...
- [Signal](https://signal.org/)은 강력한 실적과 다양한 기능을 갖춘 가장 인기 있는 종단간 암호화(E2EE) 메신저 중 하나입니다. Signal은 가입 시 전화번호가 필요하므로 전화번호를 모르는 사람과 채팅할 계획이라면 다른 대안을 살펴보는 것도 좋습니다 **_[or hide your number in your profile settings](https://support.signal.org/hc/en-us/articles/6712070553754-Phone-Number-Privacy-and-Usernames)_**. Signal은 Aurora Store를 통해 다운로드해야 합니다.
...
- [Mercurygram](https://f-droid.org/en/packages/it.belloworld.mercurygram/)는 안드로이드용 공식 텔레그램 앱의 비공식 FOSS Fork입니다. 텔레그램에는 E2EE '비밀대화'가 있지만, 기본 옵션은 비공개가 아닙니다. Mercurygram는 F-Droid에서 다운로드 받을 수 있습니다.
...
- [Syncthing Fork](https://f-droid.org/en/packages/com.github.catfriend1.syncthingfork/)은 파일 동기화 프로그램의 Fork입니다. 두 대 이상의 기기 간에 파일을 실시간으로 동기화하며, 외부의 시선으로부터 안전하게 보호합니다. 내 데이터는 내 데이터이므로 저장 위치, 제3자와의 공유 여부, 인터넷을 통한 전송 방법을 선택할 권리가 있습니다. 동기화는 F-Droid를 통해 사용할 수 있습니다.
...
- [Breezy Weather](https://f-droid.org/en/packages/org.breezyweather/)는 Material 3 Expressive 인터페이스와 50개 이상의 소스를 갖춘 오픈 소스 날씨 앱으로, F-Droid를 통해 이용할 수 있습니다. 또한 다양한 크기의 위젯을 지원하므로 홈 화면에서 바로 선택한 위치의 날씨를 확인할 수 있습니다.
...
- [크립토메이터](https://f-droid.org/en/packages/org.cryptomator.lite/)는 데이터를 로컬에서 암호화하여 선호하는 클라우드 서비스에 안전하게 업로드할 수 있는 무료 크로스 플랫폼 서비스입니다 (_Android에서는 Google Play를 통해서만 유료_). 크립토메이터는 F-Droid를 통해 다운로드할 수 있습니다.
