...
Give your portfolio a name (e.g. "Wallet for tuto") and select the address type. Select "Segwit" to use native bech32 addresses that optimize transaction costs.
...
One of Specter Desktop's major assets is its ability to simplify the management of multi-signature portfolios. A multisig wallet requires several signatures to authorize a transaction, thus eliminating the single point of failure. A 2-on-3 configuration, for example, requires two signatures from three separate hardware wallets to validate any expenditure.
...
**Please note**: backing up a multisig portfolio is fundamentally different from backing up a single portfolio. Your recovery phrases (seed phrases) alone are not sufficient to restore a multisig portfolio. You must also back up the **output descriptor** (output descriptor), which contains the configuration information for your multisig portfolio.
