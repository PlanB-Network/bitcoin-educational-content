...
Eine Empfangsadresse wird verwendet, um Bitcoins zu empfangen. Sie wird aus einem öffentlichen Schlüssel generiert, indem dieser gehasht und einem spezifischen Format unterzogen wird. Dadurch wird eine neue Ausgabebeschränkung für eine Münze erstellt, um den Besitzer zu ändern.
...
Darüber hinaus haben Sie sicherlich schon von einem erfahrenen Bitcoin-Nutzer gehört, dass Empfangsadressen nur einmal verwendet werden sollten und dass Sie daher für jede neue eingehende Zahlung in Ihre Brieftasche eine neue generieren sollten. Aber warum?
Grundsätzlich gefährdet die Wiederverwendung von Adressen Ihre Gelder nicht direkt. Durch die Verwendung von Kryptographie auf elliptischen Kurven können Sie dem Netzwerk nachweisen, dass Sie im Besitz eines privaten Schlüssels sind, ohne diesen Schlüssel preiszugeben. Sie können also mehrere verschiedene UTXOs (Unspent Transaction Outputs) auf derselben Adresse sperren und sie zu verschiedenen Zeitpunkten ausgeben. Wenn Sie den privaten Schlüssel, der mit dieser Adresse verbunden ist, nicht preisgeben, kann niemand auf Ihre Gelder zugreifen. Das Problem bei der Wiederverwendung von Adressen betrifft eher die Privatsphäre.
...
Heutzutage ist die Verwendung von leeren Adressen nicht mehr eine zusätzliche Firewall, sondern eine Verpflichtung für jeden, der seine Privatsphäre auch nur ein wenig schützen möchte, aufgrund der Anwesenheit von Blockchain-Analyseunternehmen und der Entwicklung von KYC (Know Your Customer).
...
Ein Benutzer kann seinen Zahlungscode frei kommunizieren (in sozialen Netzwerken, auf seiner Website...) ohne das Risiko des Verlusts der Privatsphäre, im Gegensatz zu einer herkömmlichen Empfangsadresse oder einem öffentlichen Schlüssel.
...
3. Alice verbindet ihre PayNym mit der von Bob ("Follow" auf Englisch). Dieser Vorgang erfolgt außerhalb der Blockchain und ist vollständig kostenlos.

4. Alice verbindet ihre PayNym mit der von Bob ("Connect" auf Englisch). Dieser Vorgang erfolgt "on chain". Alice muss die Transaktions-Mining-Gebühren sowie eine feste Gebühr von 15.000 Sats für den Service auf Samourai bezahlen. Die Servicegebühren werden auf Sparrow angeboten. Dieser Schritt wird als Benachrichtigungstransaktion bezeichnet.
...
Das Durchführen der Benachrichtigungstransaktion, d.h. das Verbinden ihrer PayNym, ist ein obligatorischer Vorlauf, um BIP47-Zahlungen durchzuführen. Sobald dies jedoch erfolgt ist, kann der Absender mehrere Zahlungen an den Empfänger (genau 2^32) durchführen, ohne erneut eine Benachrichtigungstransaktion durchführen zu müssen.
...
* Das Verbinden von zwei PayNyms ("follow") ist völlig kostenlos. Dies hilft dabei, verschlüsselte "off chain" -Kommunikation herzustellen, insbesondere um die kollaborativen Transaktionswerkzeuge von Samourai (Stowaway oder StonewallX2) zu nutzen. Diese Operation ist spezifisch für PayNym. Sie wird nicht im BIP47 beschrieben.

* Das Verbinden von zwei PayNyms ist kostenpflichtig. Dies beinhaltet das Durchführen der Benachrichtigungstransaktion, um die Verbindung herzustellen. Die Kosten umfassen mögliche Servicegebühren, Transaktions-Mining-Gebühren und 546 Sats, die an die Empfangsbenachrichtigungsadresse des Empfängers gesendet werden, um ihn über die Öffnung des Tunnels zu informieren. Diese Operation ist mit BIP47 verbunden. Sobald sie durchgeführt wurde, kann der Absender mehrere BIP47-Zahlungen an den Empfänger durchführen.

Um zwei PayNyms verbinden zu können, müssen sie bereits verknüpft sein.
...
Nachdem die Wallet initialisiert wurde, falls Sie es noch nicht getan haben, fordern Sie Ihren PayNym an, indem Sie auf das Pluszeichen (+) unten rechts und dann auf "PayNym" klicken.
...
Wie in Teil 2 dieses Papiers erklärt, befindet sich der wiederverwendbare Zahlungscode in Tiefe 3 der HD Wallet. Er ist letztendlich in seiner Platzierung, Struktur und Funktion etwas vergleichbar mit einem xpub.
...
Schließlich berechnen wir die Prüfsumme dieses Zahlungscodes mit HASH256, das heißt, wir führen eine doppelte Hash-Funktion mit der SHA256-Funktion durch. Wir nehmen die ersten vier Bytes dieses Hash-Werts und fügen sie am Ende hinzu (in Rosa).

Der Zahlungscode ist bereit und muss nur noch in Base 58 konvertiert werden:

Wie Sie sehen können, ähnelt diese Konstruktion stark der Struktur eines erweiterten öffentlichen Schlüssels vom Typ "xpub".

Während dieses Prozesses, der zu unserem Zahlungscode führt, haben wir einen komprimierten öffentlichen Schlüssel und einen Chain-Code verwendet. Diese beiden Elemente sind das Ergebnis einer deterministischen und hierarchischen Ableitung von der Wallet-Seed, indem wir den folgenden Ableitungspfad verwenden: m/47'/0'/0'/
...
>Kredit: Ursprüngliche Idee: A.J. Han VinckVektorversion: FlugaalÜbersetzung: Dereckson, Public domain, via Wikimedia Commons. https://commons.wikimedia.org/wiki/File:Diffie-Hellman_Key_Exchange_(fr).svg

In dieser Erklärung repräsentiert die Farbe Braun das Geheimnis, das zwischen Alice und Bob geteilt wird. Man muss sich vorstellen, dass es für den Angreifer in Wirklichkeit unmöglich ist, die Farben Orange und Himmelblau zu trennen, um die geheimen Farben von Alice oder Bob wiederherzustellen.
...
Der Diffie-Hellman-Schlüsselaustausch zwischen Alice und Bob funktioniert folgendermaßen:

* Alice und Bob wählen zwei gemeinsame Zahlen: p und g. p ist eine Primzahl. Je größer diese Zahl p ist, desto sicherer wird Diffie-Hellman sein. g ist eine primitive Wurzel von p. Diese beiden Zahlen können unverschlüsselt über ein unsicheres Netzwerk kommuniziert werden, sie entsprechen der gelben Farbe in der obigen Erklärung. Alice und Bob müssen einfach genau die gleichen Werte für p und g haben.
...
* Diese Zahlen A (entspricht der orangefarbenen Farbe) und B (entspricht der himmelblauen Farbe) werden zwischen den beiden Parteien ausgetauscht. Der Austausch kann unverschlüsselt über ein unsicheres Netzwerk erfolgen.

* Alice, die nun B kennt, berechnet den Wert von z wie folgt:

> z ist gleich B hoch a modulo p:
z = B^a % p
* Zur Erinnerung, B = g^b % p. Wir haben also:
> z = B^a % p
> z = (g^b)^a % p
>
...
* Bob, der jetzt über A informiert ist, wird auch den Wert von z berechnen:
...
Dank der Distributivität des Modulo-Operators finden Alice und Bob genau den gleichen Wert für z. Diese Zahl repräsentiert ihr gemeinsames Geheimnis, das Äquivalent zur Farbe Braun in der vorherigen Erläuterung. Sie können dieses gemeinsame Geheimnis verwenden, um eine Kommunikation zwischen ihnen auf einem unsicheren Netzwerk zu verschlüsseln.

![Schema der technischen Funktionsweise von Diffie-Hellman](assets/14.png)

Ein Angreifer, der im Besitz von p, g, A und B ist, wird nicht in der Lage sein, a, b oder z zu berechnen. Diese Operation umzukehren würde bedeuten, die Exponentiation rückgängig zu machen. Diese Berechnung ist unmöglich, außer man versucht alle Möglichkeiten einzeln auszuprobieren, da wir in einem endlichen Körper arbeiten. Dies würde bedeuten, den diskreten Logarithmus zu berechnen, also das Inverse der Exponentialfunktion in einer endlichen zyklischen Gruppe.

Solange wir also ausreichend große Werte für a, b und p wählen, ist Diffie-Hellman sicher. Typischerweise ist mit Parametern von 2.048 Bits (eine Zahl mit 600 Dezimalstellen) das Testen aller Möglichkeiten für a und b illusorisch. Mit Zahlen dieser Größe gilt der Algorithmus heute als sicher.

Genau hier liegt jedoch der Hauptnachteil des Diffie-Hellman-Protokolls. Um sicher zu sein, muss der Algorithmus große Zahlen verwenden. Daher bevorzugen wir heute den Einsatz des ECDH-Algorithmus, einer Variante von Diffie-Hellman, der eine algebraische Kurve verwendet, genauer gesagt eine elliptische Kurve. Dadurch können wir mit viel kleineren Zahlen arbeiten und gleichzeitig eine äquivalente Sicherheit gewährleisten, was die Ressourcen für Berechnung und Speicherung reduziert.
...
Wir werden diese Eigenschaft nutzen, um unseren Diffie-Hellman-Algorithmus anzupassen. Der Funktionsweise von ECDH ist wie folgt:
...
Sie erhalten das gleiche gemeinsame Geheimnis, denn:

> (x,y) = ka·Kb = ka·kb·G = kb·ka·G = kb·Ka

Ein potenzieller Angreifer, der das unsichere öffentliche Netzwerk beobachtet, kann nur die öffentlichen Schlüssel beider Parteien und die Parameter der gewählten Kurve erhalten. Wie zuvor erklärt, reichen diese beiden Informationen allein nicht aus, um die privaten Schlüssel zu bestimmen, und daher kann der Angreifer nicht auf das Geheimnis zugreifen.
...
Wie wir im vorherigen Abschnitt gesehen haben, ist ECDH eine Variante des Diffie-Hellman-Austauschs, bei dem Schlüsselpaare auf einer elliptischen Kurve erstellt werden. Das passt gut, denn Schlüsselpaare, die diesem Standard entsprechen, haben wir in unseren Bitcoin-Hierarchical-Deterministic-Wallets!

Die Idee besteht darin, die Schlüsselpaare der hierarchischen deterministischen Bitcoin-Wallets beider Parteien zu verwenden, um gemeinsame und ephemere Geheimnisse zwischen ihnen herzustellen. Im BIP47 wird daher ECDHE (Elliptic Curve Diffie-Hellman Ephemeral) verwendet.
...
Es wäre also möglich, BIP47 ohne diese Benachrichtigungstransaktion zu verwenden, aber jeder Benutzer müsste eine Sicherungskopie der Zahlungscodes seiner Kontakte erstellen. Diese Situation bleibt jedoch untragbar, solange es keine einfache und robuste Möglichkeit gibt, diese Sicherungskopien zu erstellen, zu speichern und aktuell zu halten. Die Benachrichtigungstransaktion ist daher im aktuellen Zustand der Dinge nahezu obligatorisch.

Neben der Funktion als Sicherungskopie der Zahlungscodes spielt diese Transaktion, wie der Name schon sagt, auch eine Benachrichtigungsrolle für den Empfänger. Sie signalisiert dem Client, dass ein Tunnel geöffnet wurde.
...
Der Zahlungscode selbst stellt kein direktes Datenschutzrisiko dar. Im Gegensatz zum herkömmlichen Bitcoin-Modell, das es ermöglicht, den Informationsfluss zwischen der Benutzeridentität und den Transaktionen zu unterbrechen, insbesondere durch die Verwendung anonymer öffentlicher Schlüssel, kann der Zahlungscode direkt mit einer Identität verknüpft werden. Das ist natürlich keine Verpflichtung, aber diese Verbindung ist nicht gefährlich.
...
Ein einzelner Zahlungscode stellt daher kein direktes Datenschutzrisiko dar, da nur die Benachrichtigungsadresse daraus abgeleitet wird. Man kann daraus einige Informationen ableiten, aber normalerweise kann man nicht wissen, mit wem Sie Transaktionen durchführen.

Es ist daher wichtig, eine strikte Trennung zwischen den Zahlungscodes der Benutzer aufrechtzuerhalten. Zu diesem Zweck ist der initiale Kommunikationsschritt des Codes ein kritischer Moment für die Zahlungsdatenschutz, und dennoch ist er für das reibungslose Funktionieren des Protokolls erforderlich. Wenn einer der beiden Zahlungscodes öffentlich abgerufen werden kann (z. B. auf einer Website), darf der zweite Code, d.h. der des Absenders, nicht mit dem ersten in Verbindung gebracht werden.
...
* Bevor ich ihnen eine Transaktion senden kann, muss ich sicherstellen, dass sie von meinem persönlichen Zahlungscode wissen, der auch mit meiner Identität verbunden ist, da ich ihn verwende, um Transaktionen von meinen sozialen Netzwerken zu erhalten.
...
Die Benachrichtigungstransaktion ist sicherlich nicht die einzige Lösung, um den Zahlungscode des Absenders geheim zu übermitteln, aber sie erfüllt derzeit perfekt diese Funktion, indem sie mehrere Sicherheitsebenen anwendet.
...
Natürlich gilt das klassische Bitcoin-Vertraulichkeitsmodell immer noch für die ephemeren öffentlichen Schlüssel, die aus der Kombination der beiden Zahlungscodes abgeleitet werden. Die beiden Modelle sind voneinander abhängig. Ich möchte hier nur darauf hinweisen, dass im Gegensatz zur herkömmlichen Verwendung eines öffentlichen Schlüssels zum Empfangen von Bitcoins der Zahlungscode einer Identität zugeordnet werden kann, da die Information "Bob führt eine Transaktion mit Alice durch" zu einem anderen Zeitpunkt unterbrochen wird. Der Zahlungscode wird verwendet, um Zahlungsadressen zu generieren, aber allein durch Beobachtung der Blockchain ist es unmöglich, eine BIP47-Zahlungstransaktion den verwendeten Zahlungscodes zuzuordnen.

### Aufbau der Benachrichtigungstransaktion.

Lassen Sie uns nun betrachten, wie diese Benachrichtigungstransaktion funktioniert. Stellen wir uns vor, Alice möchte mit BIP47 Geld an Bob senden. In meinem Beispiel fungiert Alice als Absender und Bob als Empfänger. Letzterer hat seinen Zahlungscode auf seiner Website veröffentlicht. Alice kennt also bereits Bobs Zahlungscode.
...
2. Alice konvertiert ihren persönlichen Zahlungscode in Binärform.

3. Sie verwendet diesen Verblindungsfaktor als Schlüssel, um eine symmetrische Verschlüsselung auf den Payload ihres Zahlungscodes durchzuführen. Der verwendete Verschlüsselungsalgorithmus ist ein einfaches XOR. Die durchgeführte Operation ist vergleichbar mit dem Vernam-Chiffre, auch bekannt als "One-Time Pad":
...
>
c>' = c XOR f2
* Alice ersetzt die echten Werte der x-Koordinate des öffentlichen Schlüssels und des Chain Codes (c) in ihrem Zahlungscode durch die verschlüsselten Werte (x') und (c').

Bevor wir mit der technischen Beschreibung dieser Benachrichtigungstransaktion fortfahren, wollen wir uns einen Moment mit der XOR-Operation befassen. XOR ist ein logischer Bit-Operator, der auf der Booleschen Algebra basiert. Ausgehend von zwei Bit-Operanden gibt er 1 zurück, wenn die Bits gleicher Rangordnung unterschiedlich sind, und er gibt 0 zurück, wenn die Bits gleicher Rangordnung gleich sind. Hier ist die Wahrheitstabelle für XOR in Abhängigkeit von den Werten der Operanden D und E:
...
- Symmetrie:
 ...
 ->  D ⊕ L = E
Dann ähnelt diese Verschlüsselungsmethode sehr dem Vernam-Chiffre (One-Time Pad), dem einzigen bekannten Verschlüsselungsalgorithmus, der eine bedingungslose (oder absolute) Sicherheit bietet. Damit das Vernam-Chiffre diese Eigenschaft aufweist, muss der Verschlüsselungsschlüssel perfekt zufällig sein, die gleiche Länge wie die Nachricht haben und nur einmal verwendet werden. Bei der hier verwendeten Verschlüsselungsmethode für BIP47 ist der Schlüssel tatsächlich gleich lang wie die Nachricht, der Verblindungsfaktor hat genau die gleiche Länge wie die Verkettung der x-Koordinate des öffentlichen Schlüssels mit dem Chain-Code der Zahlungscodekette. Dieser Verschlüsselungsschlüssel wird tatsächlich nur einmal verwendet. Allerdings stammt dieser Schlüssel nicht aus einem perfekten Zufall, da es sich um ein HMAC handelt. Er ist eher pseudozufällig. Es handelt sich also nicht um ein Vernam-Chiffre, aber die Methode kommt ihm nahe.
Kehren wir zu unserem Aufbau der Benachrichtigungstransaktion zurück:

4. Alice hat jetzt ihren Zahlungscode mit einer verschlüsselten Nutzlast. Sie wird eine Transaktion erstellen und verbreiten, bei der ihr öffentlicher Schlüssel "A" als Eingabe, eine Ausgabe an die Benachrichtigungsadresse von Bob und eine OP_RETURN-Ausgabe bestehend aus ihrem Zahlungscode mit der verschlüsselten Nutzlast beteiligt sind. Diese Transaktion ist die Benachrichtigungstransaktion.
...
![Diagramm: Alice sendet ihren maskierten Zahlungscode an Bobs Benachrichtigungsadresse](assets/16.png)
...
Wenn wir dieses Diagramm mit dem vergleichen, was ich Ihnen zuvor beschrieben habe:
...
* "Masked Payment Code" entspricht dem verschleierten Zahlungscode, d.h. mit verschlüsseltem Payload: x' und c'.

* "Notification Transaction" ist die Transaktion, die den OP_RETURN enthält.

Ich fasse die Schritte zusammen, die wir gerade gesehen haben, um eine Benachrichtigungstransaktion durchzuführen:
...
* Sie verwendet diesen geheimen Punkt, um einen HMAC zu berechnen, der als Verblindungsfaktor dient.

* Sie verwendet diesen Verblindungsfaktor, um den Payload ihres persönlichen Zahlungscodes zu verschlüsseln.
...
![BIP47-Benachrichtigungstransaktion](assets/17.png)

Quelle: https://blockstream.info/

Wenn wir uns diese Transaktion ansehen, können wir bereits erkennen, dass sie eine einzige Eingabe und 4 Ausgaben hat:
...
* Die dritte Ausgabe von 15.000 Sats repräsentiert die Servicegebühren, da ich Samourai Wallet verwendet habe, um diese Transaktion zu erstellen.

* Die vierte Ausgabe von zwei Millionen Sats repräsentiert das Wechselgeld, d.h. den verbleibenden Unterschied meines Eingangs, der zu einer anderen mir gehörenden Adresse zurückkehrt.
...
Unter den OP-Codes können wir 0x6a erkennen, was OP_RETURN und 0x4c, was OP_PUSHDATA1 bedeutet. Das Byte nach diesem letzten OP-Code gibt die Größe der folgenden Nutzlast an. Es ist 0x50, also 80 Bytes.
...
Schließlich kann man sehen, dass die Abszisse des öffentlichen Schlüssels (in Blau) und der Zeichenkette-Code (in Rot) verschlüsselt wurden. Dies bildet die Nutzlast des Zahlungscodes.
...
Warum funktioniert das? Wie kann Bob den gleichen blinden Faktor wie Alice bestimmen und somit ihren Zahlungscode entschlüsseln? Lassen Sie uns genauer betrachten, wie ECDH in dem beschriebenen Szenario funktioniert.
...
Dieser blinde Faktor wird erhalten, indem HMAC-SHA512 auf zwei Werte angewendet wird: die Abszisse eines geheimen Punktes und die verbrauchte UTXO (Unspent Transaction Output) als Eingabe der Transaktion. Bob muss also über diese beiden Informationen verfügen, um die Nutzlast des Zahlungscodes von Alice zu entschlüsseln.

Für die Eingabe-UTXO kann Bob sie einfach beobachten und aus der Benachrichtigungstransaktion abrufen. Für den geheimen Punkt muss Bob ECDH verwenden.
...
![Schema zur Generierung eines gemeinsamen Geheimnisses mit ECDHE](assets/19.png)
...
Der nächste Schritt verteilt die Indizes wie folgt:

* Das erste normale Kindpaar (nicht verstärkt) wird verwendet, um die Benachrichtigungsadresse zu generieren, über die wir in dem vorherigen Teil gesprochen haben: m/47'/0'/0'/0/.
...
Sobald Alice diese Empfangsadresse "K0" von Bob hat, kann sie eine herkömmliche Bitcoin-Transaktion erstellen, indem sie eine UTXO auswählt, die ihr in einem anderen Zweig ihrer HD-Wallet gehört, und diese an Bobs Adresse "K0" sendet.

![Alice sendet Bitcoins mit BIP47 an Bob](assets/21.png)

Quelle: Wiederverwendbare Zahlungscodes für hierarchische deterministische Wallets, Justus Ranvier. https://github.com/bitcoin/bips/blob/master/bip-0047.mediawiki
Wenn wir dieses Schema mit dem vergleichen, was ich Ihnen zuvor beschrieben habe:
...
* Alice wählt den ersten abgeleiteten Kind-Privatkey aus, der von ihrem persönlichen Zahlungscode stammt.

* Sie berechnet einen geheimen Punkt auf der elliptischen Kurve mit ECDH aus dem ersten unbenutzten abgeleiteten Kind-Pubkey, der von Bobs Zahlungscode stammt.
...
* Sie addiert diesen neuen geheimen Punkt mit Bobs öffentlichem Schlüssel.
...
Wenn sie eine zweite Zahlung durchführen möchte, wiederholt sie die oben genannten Schritte, außer dass sie den zweiten abgeleiteten öffentlichen Schlüssel aus Bobs Zahlungscode auswählt. Das heißt, den nächsten unbenutzten Schlüssel. Sie hat dann eine zweite Empfangsadresse, die zu Bob gehört, "K1".
...
Sie kann so weitermachen und bis zu 2^32 leere Adressen ableiten, die zu Bob gehören.

Aus externer Sicht, beim Beobachten der Bitcoin-Blockchain, ist es theoretisch unmöglich, eine BIP47-Zahlung von einer normalen Zahlung zu unterscheiden. Hier ist ein Beispiel für eine BIP47-Zahlungstransaktion im Testnet:
...
Es sieht aus wie eine normale Transaktion mit einem verbrauchten Input, einer Zahlungsausgabe von 210.000 Sats und einer Wechselgeldausgabe:
...
* Bob berechnet den geheimen Punkt "S", indem er Addition und Verdoppelung von Punkten auf der elliptischen Kurve anwendet und seinen privaten Schlüssel "b" auf den öffentlichen Schlüssel von Alice "A" anwendet. Hier verwenden wir ECDH, das uns garantiert, dass dieser Punkt "S" für Bob und Alice gleich ist.
...
* Bob addiert seinen abgeleiteten Kind-Privatschlüssel "b" zu seinem persönlichen Zahlungscode. Er ist der einzige, der den Wert von "b" erhalten kann. Dann addiert er "b" mit dem gemeinsamen Geheimnis "s", um k0, den privaten Schlüssel von K0, zu erhalten:
...
* Er berechnet einen geheimen Punkt auf der elliptischen Kurve mit ECDH aus dem ersten abgeleiteten öffentlichen Schlüssel aus Alice' Chain-Code.
...
Da Bob den wiederverwendbaren Zahlungscode von Alice kennt, hat er bereits alle erforderlichen Informationen, um ihr eine Rückerstattung zu senden. Er muss Alice nicht erneut kontaktieren, um nach Informationen zu fragen. Er muss sie lediglich mit einer Benachrichtigungstransaktion informieren, insbesondere damit sie ihre BIP47-Adressen mit ihrem Seed wiederherstellen kann, und dann kann er ihr bis zu 2^32 Zahlungen senden.
Bob kann Alice dann auf die gleiche Weise zurückerstatten, wie sie ihm Zahlungen geschickt hat. Die Rollen kehren sich um:

![Bob erstattet Alice mit BIP47](assets/27.png)
...
Im Gegensatz zur Verwendung von BIP47 müssen diese gemeinsamen Transaktionen keine Benachrichtigungstransaktion durchführen. Es reicht aus, die PayNym zu verknüpfen, um diese Werkzeuge zu verwenden. Es ist nicht erforderlich, sie zu verbinden.
...
Natürlich ist der Hauptnachteil von BIP47 die Transaktionsbenachrichtigung. Der Benutzer muss Gebühren für das Mining dieser Transaktion bezahlen, was für einige lästig sein kann. Auf der anderen Seite ist das Argument des "Spams" der Bitcoin-Blockchain absolut unannehmbar. Jeder, der die Gebühren für seine Transaktion bezahlt, sollte in der Lage sein, sie in das Register einzutragen, unabhängig von ihrem Zweck. Das Gegenteil zu behaupten, bedeutet, sich für Zensur auszusprechen.
Es ist möglich, dass in Zukunft kostengünstigere Lösungen gefunden werden, um den Zahlungscode des Absenders an den Empfänger zu übermitteln und dieser ihn sicher zu speichern. Aber im Moment ist die Transaktionsbenachrichtigung die Lösung mit den geringsten Kompromissen.
...
In Bezug auf die Privatsphäre ist BIP47 sehr interessant. Wie bereits im Abschnitt über die Transaktionsbenachrichtigung erklärt, enthüllt der Zahlungscode keine Informationen über abgeleitete temporäre Adressen. Dadurch wird der Informationsfluss zwischen der Bitcoin-Transaktion und der Empfänger-ID unterbrochen, im Gegensatz zur herkömmlichen Verwendung einer Empfangsadresse.
...
Hoffentlich werden diese wiederverwendbaren Zahlungscodes in Zukunft von den Akteuren des Ökosystems übernommen, in Wallet-Software implementiert und von Bitcoin-Nutzern verwendet.

Jede wirklich positive Lösung für die Privatsphäre des Benutzers muss diskutiert, vorangetrieben und verteidigt werden, damit Bitcoin nicht zum Spielplatz von Unternehmen wird und zum Überwachungsinstrument der Regierungen wird.
...
* Verständnis und Verwendung von CoinJoin auf Bitcoin.

* Verständnis der Derivationspfade einer Bitcoin-Brieftasche.
