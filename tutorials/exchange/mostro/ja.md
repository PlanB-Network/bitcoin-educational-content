---
name: モストロ
description: LightningとNostrによるKYCフリーBitcoin P2P Exchangeプラットフォーム
---
...
**Mostro P2P**は、Lightning Network、Nostrプロトコル、ホールドインボイスを組み合わせた分散型オルタナティブを提供します。Exchange期間中、ビットコインはあなたの管理下に置かれ、盗難、Exchangeの破産、恣意的な没収のリスクを排除します。
...
Mostro P2Pは、2021年に開始された人気のテレグラムボット**lnp2pbot**の開発者である**grunch**によって作成されたオープンソースのBitcoin Exchangeプロトコルである。lnp2pbotはすでにLightning経由でKYCフリーのP2P交換を可能にしていたが、大きな脆弱性を提示していた： **テレグラムは政府による検閲の影響を受けやすい集中障害点**を構成する。
...
- daemon Mostrod**：ユーザーとLightning Network間の交換を調整する。
...
**分散型コミュニケーション**：すべての注文は暗号署名されたイベントを通じてNostrに公開されます。プライベートな交渉はエンドツーエンドで暗号化されたメッセージで送信され、絶対的な機密性が保証されます。
...
**分散型仲裁**：紛争が発生した場合、公平な仲裁人が証拠を検証し、透明性の高い基準に基づいて決定を下す。各紛争は、トレーサビリティのために一意のtokenを生成する。



**支払いの柔軟性**：yadio.ioのExchangeレートサービスを介して多くの不換紙幣をサポートしています。SEPA送金、PayPal、Revolut、現金、および当事者間で合意されたあらゆる方法を受け入れます。
...
Mostro CLI**は、最も成熟した機能的なクライアントです。Rustで書かれ、Interfaceのコマンドライン経由でMostroの全機能を提供します。macOS、Linux、Windowsと互換性があります。
...
- リリース`：エスクローを解除し、Exchangeを確定する。
- getdm`：ダイレクトメッセージを表示する
- レート `rate` : Exchange後にカウンターパーティを評価する。
...
アルファ版の**モバイル・アプリ**は、スマートフォンから直接P2P取引が可能です。直感的なグラフィカルInterfaceは、タブ化されたナビゲーション、注文表示、高度なフィルター、取引相手とコミュニケーションするための統合チャットを備えています。
...
### Mostro-web - Interface web (開発中)



Interface現在開発中の先進的なJavaScriptウェブ・アプリケーション。広範な取引とポートフォリオ管理機能を備えた完全なユーザー体験を提供するように設計されています。インストール不要でブラウザからアクセスできます。
...
- 十分な流動性のあるLightning Network** Wallet (またはLightning互換機)
...
- オプションだが推奨**：IP AddressをマスクするためのVPNまたはTor接続
...
**CLI/Mobile** の同期に重要です：CLIとモバイルアプリで同じ注文にアクセスするには、両方のクライアントで**同じMostro Pubkey**と**同じNostr relays**を使用する必要があります。モバイルアプリの設定でこれらの設定を確認してください。
...
- 少額から**：Sats 10,000円以下からスタート可能。
- 複数のリレー**：地理的に異なる3～5台のリレーを構成する
- ネットワーク保護**：VPNまたはTorを有効にしてIP Addressを隠す
- Walletセキュア**：Walletライトニングの自動ロックを有効にする
...
このセクションでは、実際のユースケースに従って、Mostro CLIを介したビットコインの購入を実証する。
...
Mostroは、BTCを受け取るために**Lightning Invoice**を提供するように要求します。正確な金額はサトシで表示されます（ここでは4715 Sats）。
...
### ステップ3：ライトニングInvoiceの提供



generate Wallet（Phoenix、Breezなど）からライトニングInvoiceを正確な金額で購入し、それを送る：
...
システムは出荷を確認し、エスクローを有効にする前に売り手がInvoiceホールドを支払うのを待つよう通知する。
...
売り手がフィアットの受領を確認し、`release`コマンドでエスクローを解除するとすぐに、あなたは即座にビットコインをあなたが提供したライトニングInvoiceで受け取ります。
...
**CLI/モバイル同期**のために重要です：CLIとモバイルアプリの両方を使用する場合、両方のクライアントで**全く同じNostrリレーとMostro Pubkey**を設定してください。この同一の設定を行わないと、両方のインターフェイスで利用可能な同じ注文が表示されません。設定(上のスクリーンショット)に表示されているリレーとMostro Pubkeyは、CLIの`.env'ファイルにあるものと一致していなければなりません。
...
右下のGreen **"+"**ボタンを押し、**SELL**（赤いボタン）を選択します。
...
**重要なお知らせです：あなたのビットコイン（ここでは1-5ユーロで4674 Sats）をエスクローにロックするために表示されたInvoiceホールド**を支払う必要があります。Exchangeがキャンセルされます。



Invoiceのホールドをコピーし、Walletのライトニング（Phoenix、Breezなど）から支払う。
...
あなたのアカウントでフィアットペイメントの**受領**を確認したら、Green **RELEASE**ボタンを押して、バイヤーにSatssを送信することを確認します。
...
ビットコインはLightning Invoiceを通じて即座に買い手に送金される。
...
4.ライトニングInvoice（Walletから生成されたもの）を提供する。
...
9.Lightning Invoiceでビットコインを即座に受け取る
...
**金融主権**：Invoiceのホールド・メカニズムにより、あなたのビットコインがあなたの直接管理下から離れることはなく、Exchangeの破産や海賊行為のリスクを排除します。



**検閲に強い**：いかなる権力もネットワークを遮断したり、あなたの命令を検閲することはできません。Nostrのリレーが稼働している限り、システムは機能する。
...
Mostro P2Pは、Lightning Network、Nostr、自動エスクローを組み合わせた真に分散化された取引のための中央集権的な取引所に代わる有望な選択肢です。開発初期で流動性が限られているにもかかわらず、このプラットフォームはすでに金融主権、検閲耐性、デフォルトの匿名性を提供している。



自律性と機密性を好むビットコイナーにとって、Mostroは漸進的な探求に値する実行可能な選択肢である。その非中央集権的なアーキテクチャは、商業的な進化よりもむしろコミュニティを保証し、真に自由なBitcoin取引所の未来への道を開く。
...
- [カスタマーCLI](https://github.com/MostroP2P/mostro-CLI)
