...
Voor het overboeken van fondsen tussen Bitcoin On-Chain en Lightning Network is over het algemeen ofwel het handmatig openen van Lightning-kanalen (technisch en duur) nodig, of het gebruik van gecentraliseerde swapplatforms met KYC. SwapMarket biedt een alternatief: Trustless atomaire swaps via concurrerende aanbieders, zonder KYC.



Innovatie: hoewel aanbieders tussenpersonen zijn, garandeert HTLC (*Hash Time Locked Contracts*) wiskundig dat uw geld onder uw controle blijft. De samenvoeging van verschillende aanbieders (Boltz, ZEUS Swaps, Eldamar, Middle Way) creëert prijsconcurrentie. Interface web open-source zelf te hosten.
...
**Provider-ontdekking** : Hard-gecodeerde lijst in `src/configs/Mainnet.ts`. Nieuwe providers toegevoegd via Pull Request of e-mail.



**Onafhankelijke backends**: Elke aanbieder heeft zijn eigen Boltz backend. Interface bevraagt de API's in realtime om offertes direct te vergelijken.
...
Interface toont alle actieve aanbieders: naam van de aanbieder, toegepaste kosten (percentage en/of vast), minimum/maximum beschikbare bedragen en ondersteunde swaptypes. De applicatie bevraagt direct de API's van elke aanbieder waarnaar in het configuratiebestand wordt verwezen om in realtime koersen op te vragen. Concurrentie tussen aanbieders garandeert optimale tarieven, over het algemeen rond de 0,5% voor standaard swaps.
...
**Swap-in (On-Chain → Lightning)**: On-Chain BTC's omzetten in Lightning satoshis. Gebruiksmogelijkheid: een mobiele Wallet Lightning van stroom voorzien, binnenkomende capaciteit op een knooppunt verkrijgen of onmiddellijke liquiditeit hebben.



**Swap-out (Lightning → On-Chain)**: Lightning-satoshis omzetten in On-Chain BTC. Use case: Wallet Lightning dumpen in Cold opslag of liquiditeit tussen lagen opnieuw in evenwicht brengen.
...
**Trustless atoomruil: HTLC garandeert dat Exchange volledig wordt voltooid of dat elke partij haar inzet terugkrijgt. Het tegenpartijrisico wordt wiskundig geëlimineerd.



**Inwisselmechanisme**: Elke swap heeft een vervaldatum (TIMELOCK). Als de swap mislukt, wordt het geld na afloop automatisch terugbetaald. De gebruiker behoudt altijd de optie om zijn bitcoins terug te vorderen.
...
De applicatie zal toegankelijk zijn op `http://localhost:3000`. Zelf hosten garandeert totale controle over Interface, elimineert het risico van censuur van het officiële domein en maakt het mogelijk om de broncode te controleren voordat deze wordt uitgevoerd.
...
**Wallet Lightning**: Zorg ervoor dat je een operationele Wallet Lightning hebt (Phoenix, Zeus, BlueWallet, enz.). Voor inruilen betaal je generate en een Lightning Invoice. Voor ruilen betaal je een Lightning Invoice.



**Wallet On-Chain**: Voor inruil heb je een Wallet Bitcoin On-Chain nodig om geld te versturen. Voor ruilen maak je een Bitcoin klaar die Address ontvangt.
...
Klik op de hoofdpagina van de SwapMarkt op het tandwielpictogram (⚙️) rechtsboven op de Interface, naast het ruilformulier.
...
- Denominatie**: Keuze tussen BTC of Sats
...
- LIGHTNING** (bovenste veld): Voer het bedrag in dat je wilt verzenden in Sats Lightning (voorbeeld: 30.000 Sats)
- Bitcoin** (onderste veld): Het bedrag dat je ontvangt wordt automatisch weergegeven nadat de kosten zijn afgetrokken (voorbeeld: Sats 29.320)



Plak in het onderste veld je **ontvangende Bitcoin Address** waar je het geld wilt ontvangen. Controleer deze Address zorgvuldig.



De standaard provider is meestal Boltz Exchange. Netwerkkosten en providerkosten worden duidelijk weergegeven.
...
**Let op**: De provider selectie Interface toont niet de **minimumbedragen** voor elke provider. Deze informatie verschijnt alleen in de Interface voor het maken van swaps, nadat een aanbieder is geselecteerd. Minimum- en maximumbedragen kunnen per aanbieder verschillen en kunnen na verloop van tijd veranderen. **Controleer deze limieten altijd op het moment van je swap**: als het bedrag dat je wilt ruilen buiten de limieten van een provider valt, kun je een andere selecteren die geschikter is voor je transactie.
...
Klik op de gele **"CREATE ATOMIC SWAP"** knop. SwapMarket zal generate een **Lightning Invoice** (BOLT11) voor je klaarzetten om te betalen vanaf je Wallet Lightning.
...
Betaal deze Invoice uit je Wallet Lightning (Phoenix, Zeus, BlueWallet, etc.). Het exacte bedrag dat moet worden betaald, wordt weergegeven (voorbeeld: 30.000 Sats).
...
Zodra de Lightning-betaling is bevestigd, ontvangt SwapMarket onmiddellijk uw betaling en zendt de provider de Bitcoin transactie naar uw Address.



De status verandert in **"Invoice.settled"** (Invoice betaald) en er verschijnt een bevestigingsbericht.



Uw On-Chain bitcoins zijn beschikbaar zodra de transactie is bevestigd (meestal binnen een paar minuten tot een paar uur, afhankelijk van de Mining kosten die de provider heeft gekozen).
...
U kunt klikken op **"OPEN CLAIM TRANSACTION"** om de Bitcoin transactie te bekijken op een Blockchain verkenner.
...
- Bitcoin** (bovenste veld): Vul het bedrag in dat je in Sats Bitcoin wilt versturen (voorbeeld: 63.400 Sats)
...
Plak in het onderste veld een Lightning** Invoice (BOLT11) gegenereerd van je Wallet Lightning, of gebruik je LNURL Address als je Wallet dat ondersteunt.
...
Selecteer het eerder gedownloade Rescue Key-bestand. Na succesvolle verificatie gaat de Interface automatisch verder met de volgende stap.



**Stap 3: Bitcoin** stort Address



SwapMarket genereert nu een **unieke Bitcoin Address** met daarin de HTLC Contract die gekoppeld is aan je Lightning Invoice.
...
- Status** : "Invoice.set" (Invoice ingesteld, wacht op betaling Bitcoin)
- QR-code**: Bitcoin depot Address
- Bitcoin** Address: Begint meestal met "bc1p..." (voorbeeld: bc1p5mvtwxapjkds...9d4n9f)
- Waarschuwing in geel** : "Zorg ervoor dat je transactie binnen ~24 uur na het aanmaken van deze swap bevestigt!"



Deze periode van ~24 uur is de **timeout** van de HTLC Contract. Als uw Bitcoin transactie niet binnen dit tijdsbestek wordt bevestigd, mislukt de ruil en moet u uw Rescue Key gebruiken om uw geld terug te krijgen.
...
Je kunt de Address kopiëren door op de **"Address"** knop te klikken, of scan de QR-code rechtstreeks vanaf je Wallet On-Chain.
...
Van je Wallet Bitcoin On-Chain, stuur **exact** het aangegeven bedrag (bijv. 63.400 Sats) naar de Address die is gegenereerd.



**Belangrijk**: Gebruik de juiste Mining vergoedingen om een snelle bevestiging te garanderen. Als de vergoeding te laag is en de transactie in Mempool blijft na de timeout (~24h), zal de ruil mislukken.



Zodra de transactie is verzonden, detecteert SwapMarket dat deze in Mempool is en geeft :




- Status** : "transactie.Mempool"
- Bericht**: "Transactie is in Mempool - Wachten op bevestiging om de ruil te voltooien"
...
Zodra de Bitcoin-transactie de eerste bevestiging ontvangt, betaalt de provider automatisch uw Lightning Invoice. Je ontvangt direct de satoshis op je Wallet Lightning.
...
Je Lightning satoshi's zijn onmiddellijk beschikbaar in je Wallet.
...
**Vertrouwelijkheid**: Geen KYC, Interface 100% client-side (geen overdracht van persoonlijke gegevens), compatibel met Tor Browser.
...
**Verlooptijd**: Time-out van 24 tot 48 uur. Als On-Chain transactie niet bevestigd is voor afloop, handmatig herstel vereist.
...
**On-Chain sporen**: HTLC scripts kunnen mogelijk geïdentificeerd worden door geavanceerde Blockchain analyse.
...
**Gebruik Tor Browser**: Voor maximale vertrouwelijkheid opent u SwapMarket via Tor Browser om uw IP-adres Address te verbergen.
...
**Hou Mempool in de gaten**: Controleer Mempool.space voor een swap-in. Kies tijden met weinig activiteit om de Mining kosten te minimaliseren.



**Controleer adressen**: Voor verwisselingen, controleer zorgvuldig je ontvangen Address. Gebruik kopiëren en plakken en controleer de eerste 5 en laatste 5 tekens.
...
**Documenteer je swaps**: Noteer de ID, Address aflossing en vervaldatum van elke swap. Deze informatie vergemakkelijkt het traceren en herstellen in het geval van een technisch probleem.
...
**Breng uw cashflow in balans**: Gebruik SwapMarket om uw verdeling tussen On-Chain (sparen, langetermijnzekerheid) en Lightning (dagelijkse uitgaven, directe betalingen) aan te passen aan uw werkelijke behoeften.
...
**Aanbeveling**: Gebruik Boltz Exchange voor eenvoud of SwapMarket om kosten te optimaliseren door concurrentie. Beide zijn gelijkwaardig in veiligheid (HTLC niet-custodiaal).
...
SwapMarket faciliteert Bitcoin/Lightning uitwisselingen door meerdere aanbieders samen te voegen in één Interface. De HTLC architectuur garandeert de niet-custodiale aard van swaps, de afwezigheid van KYC beschermt de vertrouwelijkheid en de open-source zelf-hostbare code versterkt de weerstand tegen censuur.



Concurrentie tussen aanbieders verbetert tarieven en vermenigvuldigt bronnen van liquiditeit. Om het beheer van twee Layer te optimaliseren (On-Chain besparingen, Lightning-kosten), is SwapMarket een praktisch hulpmiddel dat de financiële soevereiniteit en vertrouwelijkheid bewaart.
...
- [Boltz Exchange](https://boltz.Exchange) - Originele atoomwissel service
- [ZEUS Swaps](https://zeusln.com) - Aanbieder van Lightning swaps