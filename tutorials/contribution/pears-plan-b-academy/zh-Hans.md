...
*如果您使用的是 Linux 系统，并且已经安装了 Pears，则可以直接跳到**步骤 2**。


#### 1.3.1.以管理员身份打开 PowerShell
...
*如果您使用的是 Linux 或 Windows 系统，并且已经安装了 Pears，则可直接跳至**步骤 2**。
...
在 Pears 网络中，*seed* 应用程序意味着将其从自己的计算机重新分配给其他同行。实际上，当你使用 seed Plan ₿ Academy 时，你的计算机就成了一个数据源，允许其他用户下载应用程序，而无需依赖中央服务器。
...
感谢您为《梨上的₿计划学院》的分散式传播做出贡献，并帮助 Bitcoin 教育真正实现抗审查！