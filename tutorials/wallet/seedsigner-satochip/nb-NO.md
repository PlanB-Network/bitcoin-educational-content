...
*Takk til [Crypto Guide] (https://www.youtube.com/@CryptoGuide/) for Fork av SeedSigner-fastvaren for smartkortstøtte, som vi vil bruke i denne opplæringen



---

Satochip er en Hardware Wallet i smartkortformat, med et EAL6+-sertifisert sikkerhetselement, en av de høyeste sikkerhetsstandardene. Det er designet og produsert av det belgiske selskapet med samme navn: Satochip.
...
- Til slutt må du sikkerhetskopiere seed på et fysisk medium og kontrollere sikkerhetskopien nøye.
...
### 5.2. Lagring av seed på en Seedkeeper
...
Hvilken sikkerhetskopistrategi du velger her, avhenger av dine preferanser, men det er viktig å ha minst én kopi av Mnemonic-frasen din, enten på fysiske medier (papir eller metall) eller, som her, i en Seedkeeper. Du kan også multiplisere antall sikkerhetskopier etter behov. Hvis du vil ha mer informasjon om strategier for sikkerhetskopiering av porteføljer, foreslår jeg at du leser denne veiledningen :
...
Velg en `Label` for å enkelt identifisere de ulike hemmelighetene som er lagret på Seedkeeper. Du kan for eksempel bare beholde Wallets fingeravtrykk eller eksplisitt angi `seed`. Valget avhenger av dine preferanser og risiko.
...
Seedkeeper PIN-koden bør være så lang og tilfeldig som mulig, for å forhindre brute force-forsøk i tilfelle fysisk kompromittering av kortet. Du bør også ta en sikkerhetskopi av denne PIN-koden, lagret et annet sted enn i Seedkeeper. Uten denne PIN-koden vil du ikke kunne få tilgang til Mnemonic som er lagret i Seedkeeper, og bitcoinsene dine vil gå tapt for alltid.



### 5.3. Lagre seed på Satochip



Nå som porteføljen din er generert, lagret og kontrollert, skal vi overføre den til Satochip. For å gjøre dette må du sørge for at seed er lastet inn i SeedSigners RAM. Gå deretter til `Verktøy > Smartkortverktøy > Satochip-funksjoner`.
...
Sett Satochip inn i smartkortleseren, og velg deretter `Initialiser med seed`.
...
Deretter kan du stille inn Satochip PIN-koden, fra 4 til 16 tegn. For å styrke sikkerheten til din Wallet bør du velge en lang, tilfeldig kode: Det er den eneste beskyttelsen mot fysisk tilgang til Mnemonic-frasen din.



Husk å lagre PIN-koden så snart den er opprettet, enten i en sikker passordbehandler eller på et fysisk medium, avhengig av din personlige strategi. I sistnevnte tilfelle må du sørge for å aldri oppbevare mediet som inneholder PIN-koden på samme sted som Satochip, ellers blir beskyttelsen ubrukelig. Det er viktig å ha en sikkerhetskopi: **uten denne PIN-koden vil du ikke lenger ha tilgang til seed, og bitcoinsene dine vil gå tapt permanent**.
...
Sett inn microSD-kortet som inneholder operativsystemet, og slå deretter på SeedSigner. For øyeblikket kan den ikke gjøre noe, siden den ennå ikke kjenner til seed. Du må begynne med å sette Satochip inn i smartkortleseren, siden det er den som inneholder seed.
...
Sørg for at Sparrow wallet-programvaren er installert på datamaskinen din. Hvis du ikke vet hvordan du laster den ned, sjekker ektheten og installerer den riktig, kan du se vår komplette veiledning om emnet :
...
Gi porteføljen et navn, og klikk deretter på `Opprett Wallet`. Sparrow vil deretter be deg om å angi et passord for å låse lokal tilgang til denne Wallet. Velg et sterkt passord: det beskytter dataene dine i Sparrow (offentlige nøkler, adresser, etiketter og transaksjonshistorikk). Dette passordet er imidlertid ikke nødvendig for å gjenopprette Wallet i fremtiden: det er bare Mnemonic-frasen din (og muligens passphrase) som er nødvendig.
...
SeedSigner vil deretter be deg om å skanne en tilfeldig mottakende Address fra din Sparrow wallet for å bekrefte gyldigheten av importen.
...
Satochip (via SeedSigner) og Sparrow wallet er nå sikkert koblet sammen. Sparrow fungerer som en komplett administrasjon av Interface, mens Satochip forblir den eneste enheten som kan signere transaksjonene dine. Du er nå klar til å motta og sende bitcoins i en fullstendig luftgapet konfigurasjon.
...
En ny Bitcoin Address vises. Du finner :




- Address i tekstformat (begynner med `bc1q...` hvis du bruker P2WPKH, som i dette eksempelet) ;
- Den tilhørende QR-koden ;
- Et `Label`-felt, nyttig for å spore transaksjonene dine.



Jeg anbefaler på det sterkeste at du legger til en etikett på hver Bitcoin-kvittering i Wallet. Dette vil hjelpe deg med å enkelt identifisere opprinnelsen til hver UTXO og bedre administrere personvernet ditt. Hvis du vil vite mer om dette viktige emnet, kan du ta en titt på den dedikerte opplæringen på Plan ₿ Academy :
...
Før du kommuniserer din mottatte Address til betaleren, er det viktig å kontrollere at den tilhører din seed. Dette trinnet sikrer at Satochip kan signere transaksjoner som er knyttet til denne Address. Det forhindrer også potensielle angrep der Sparrow viser en falsk Address. Husk at Sparrow kjører i et usikkert miljø (datamaskinen din), der angrepsflaten er langt større enn for Satochip-en din, som er helt isolert. Derfor bør du aldri stole blindt på adressene som vises i Sparrow før du har sjekket dem på din Hardware Wallet.



I Sparrow klikker du på QR-koden til Address for å forstørre den: Den vil da vises i fullskjerm.
...
Hvis den skannede Address stemmer overens med den som stammer fra din seed, vil SeedSigner vise meldingen: `Address Verified`.
...
Da kan du være sikker på at Address tilhører porteføljen din.
...
Du kan nå sende denne Address i tekstform eller via QR-koden til den personen eller tjenesten som skal sende deg Satss. Når transaksjonen har blitt sendt i nettverket, vil den vises i fanen `Transaksjoner` i Sparrow wallet.
...
I Sparrow wallet kan du opprette en transaksjon ved å klikke på `Send`-fanen i venstre sidefelt. Jeg foretrekker imidlertid å bruke fanen `UTXOs`, som lar deg praktisere *Coin Control*. Denne metoden gir deg nøyaktig kontroll over UTXO-ene som brukes, slik at du kan begrense informasjonen som avsløres under transaksjonene dine.
...
- I `Betal til` limer du inn mottakerens Address eller skanner QR-koden deres ved hjelp av kameraikonet ;
...
Nå som transaksjonen er signert og validert, gjenstår det bare å kringkaste den i Bitcoin-nettverket slik at en Miner kan inkludere den i en BLOCK. I Sparrow klikker du på `Scan QR`.
...
Vis QR-koden som vises på din SeedSigner (den som inneholder den signerte transaksjonen) til webkameraet. Sparrow vil da vise alle transaksjonsdetaljene. Gjør en siste kontroll av at all informasjonen er korrekt, og klikk deretter på "Broadcast Transaction" for å kringkaste den på Bitcoin-nettverket.
...
- Ved å spille inn Mnemonic-frasen på et fysisk medium;
...
Hvis du fortsatt har Satochipen din, men SeedSigner er ødelagt eller mistet, er situasjonen ganske enkel å håndtere, siden Wallet fortsatt befinner seg i Satochipen.
...
For å gjøre dette trenger du et USB-smartkort eller en NFC-leser. Åpne Wallet som du ønsker å gjenopprette i Sparrow, gå deretter til fanen "Innstillinger" og klikk på "Erstatt".
...
- Skriv inn Mnemonic-frasen manuelt i menyen `Seeds > Enter 12-word seed`.
...
- Eller last inn seed fra en Seedkeeper via menyen `Seeds > From SeedKeeper` (dette er metoden jeg bruker i denne veiledningen). Du trenger bare å taste inn PIN-koden til Seedkeeper og velge hemmeligheten som skal brukes som seed på SeedSigner.
