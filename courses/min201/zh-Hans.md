...
连接完成后，我们将使用格栅和旧螺丝安装新的Noctua风扇了。盒子里的新螺丝将稍后重复使用。请确保将其放置在正确的方向。您会注意到风扇一侧有一个箭头，指示气流方向。重要的是要将风扇定位，使这个箭头指向机箱内部。然后重新连接风扇。
...
5. 最后一步：连接风扇，并用电源风扇盒中未使用的螺丝在顶部固定格栅。您只有4个螺丝，但每个格栅对角线上的2个就足够了。如需，您也可以在五金店寻找类似的螺丝。
