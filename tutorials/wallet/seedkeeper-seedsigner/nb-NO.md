...
For å overkomme denne svakheten kan SeedSigner kombineres med [**Seedkeeper**] (https://satochip.io/product/seedkeeper/), et smartkort utviklet av Satochip. Dette gjør det mulig å lagre Mnemonic-fraser (eller andre hemmeligheter) i en secure element som er beskyttet med en PIN-kode. Seedkeeper-appleten er åpen kildekode, og secure element har EAL6+-sertifisering. Sammen med SeedSigner tilbyr den en svært interessant sikkerhetsfunksjon: Nøklene dine administreres helt offline, du signerer transaksjonene dine på en betrodd skjerm, og seed er fysisk beskyttet i et smartkort som er motstandsdyktig mot fysiske angrep.
...
- Hvis du ennå ikke har en Bitcoin Wallet tilknyttet din SeedSigner, må du følge trinn **5** og **6** i veiledningen som er nevnt nedenfor. Disse avsnittene forklarer hvordan du generate en Mnemonic-frase med SeedSigner, lagrer den via en *SeedQR*, og deretter kobler denne Wallet til Sparrow wallet for å administrere den. Jeg vil ikke gå inn på disse prosedyrene her, og **Jeg antar at du allerede har en fungerende Bitcoin Wallet, konfigurert med Sparrow og din SeedSigner**.
...
Nå som Seedkeeperen din er i drift, kan du lagre din Bitcoin Wallet Mnemonic på smartkortet. For å begynne, slå på SeedSigner som vanlig, og skann deretter Wallets *SeedQR* for å laste den inn i enheten. Når seed er importert, velger du bare `Done`.
...
Velg nå din Seedkeepers nye PIN-kode, mellom 4 og 16 tegn. Velg en lang, tilfeldig kode for ekstra sikkerhet: Det er den eneste barrieren som beskytter fysisk tilgang til Mnemonic-frasen din.
...
Du kan deretter definere en `Label` knyttet til Mnemonic-frasen din. Denne etiketten er nyttig hvis du lagrer flere hemmeligheter på Seedkeeper, slik at du enkelt kan identifisere dem.
...
Mnemonic-frasen din er nå lagret på smartkortet.
...
- En andre kopi av din ukrypterte Mnemonic-frase, som fungerer som den ultimate sikkerhetskopien av porteføljen din, og som kun skal brukes i tilfelle tap eller tyveri av Seedkeeper. Ettersom denne versjonen er ukryptert, må den oppbevares på et separat, sikrere sted for å forhindre samtidig kompromittering av de to sikkerhetskopiene.



Avhengig av beskyttelsesstrategi og risikoprofil kan du også duplisere seed på flere forskjellige Seedkeepers, eller opprette flere fysiske kopier av Mnemonic. Hvis du vil lære mer om disse fremgangsmåtene, kan du ta en titt på følgende veiledning:
...
Du kan nå bruke din Seedkeeper til å laste inn Mnemonic-frasen din i SeedSigner ved oppstart, og dermed signere Bitcoin-transaksjonene dine. For å begynne, slå på din SeedSigner ved å plugge den inn, og åpne deretter `Seeds`-menyen.
...
Din seed er nå lastet inn. Kontroller at dette er riktig Wallet ved å sammenligne fingeravtrykket som vises på skjermen, med det som er angitt i Sparrow wallet-innstillingene. Dette fingeravtrykket ble også oppgitt da Wallet først ble opprettet.
...
- Ta også vare på en tydelig kopi av Mnemonic og passphrase, på papir eller metall. Dette er din siste utvei hvis du mister Seedkeeper eller PIN-koden. Sørg for å oppbevare disse kopiene på separate steder, slik at de ikke kan kompromitteres samtidig.



I denne konfigurasjonen vil noen som får tak i klarteksten Mnemonic alene, ikke kunne stjele noe som helst uten å kjenne passphrase (forutsatt, selvfølgelig, at den er sterk nok til å motstå et brute-force-angrep). Omvendt, hvis noen oppdager klarteksten din passphrase, vil den forbli ubrukelig uten den tilsvarende Mnemonic frasen.



Til slutt, hvis noen klarer å få fysisk tilgang til Seedkeeperen din som inneholder seed og passphrase, vil de ikke kunne hente ut noe som helst uten å kjenne PIN-koden. I motsetning til passphrase kan ikke denne koden avlures, ettersom smartkortet automatisk låser seg selv etter fem ugyldige forsøk.
...
- En **passphrase strong**: den må være lang, tilfeldig og inneholde et bredt utvalg av tegn. Kompleksiteten er ikke noe problem for deg, siden du bare trenger å skrive det inn én gang på tastaturet under initialiseringen, og deretter vil det bli overført av Seedkeeper;
...
Klikk deretter på `Backup seed`, sett Seedkeeper inn i stasjonen, og velg `To SeedKeeper`.
...
Skriv inn PIN-koden din for å låse opp Seedkeeper, og tildel deretter en etikett til denne hemmeligheten. Du kan la fingeravtrykket være etiketten for å opprettholde en form for plausibel benektelse, eller du kan for eksempel eksplisitt oppgi `passphrase Wallet`.
...
- I menyen `SeedKeeper-funksjoner` kan du :
 ...
 - Lagre eller last inn deskriptorene dine (nyttig funksjon for Multisig-mapper).
