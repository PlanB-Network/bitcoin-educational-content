...
- [ハッシュ関数](https://planb.academy/resources/glossary/hash-function)、暗号鍵、デジタル署名の原理を理解する。
---
...
$n$ビットのハッシュ関数における衝突耐性は、誕生日攻撃に従って、$2^{\frac{n}{2}}$のオーダーです。例えば、[SHA256](https://planb.academy/resources/glossary/sha256)（$n = 256$）の場合、衝突を見つける複雑さは$2^{128}$試行のオーダーです。実際には、$2^{128}$異なるメッセージを関数を通すと、衝突が見つかる可能性があります。
...
私たちの簡略化された例に戻りましょう。[プライベートキー](https://planb.academy/resources/glossary/private-key) $k = 4$ で、関連する公開キーを計算するために、次の操作を行います：
