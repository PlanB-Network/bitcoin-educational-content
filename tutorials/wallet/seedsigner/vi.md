---
name: SeedSigner
description: Một Hardware Wallet tự làm, không có quốc tịch, giá cả phải chăng và hoàn toàn cách ly không khí
---
...
SeedSigner là một Hardware Wallet Bitcoin mã nguồn mở mà bất kỳ ai cũng có thể tự chế tạo bằng các linh kiện điện tử đa năng, giá rẻ. Không giống như các sản phẩm thương mại như Ledger, Coldcard hay Trezor, đây không phải là một thiết bị sẵn sàng sử dụng do một công ty sản xuất: nó là một dự án cộng đồng cho phép mọi người tự tạo thiết bị của riêng mình và kiểm soát mọi bước.



SeedSigner được thiết kế để hoạt động 100% ***không kết nối mạng***: nó không bao giờ kết nối Internet, không có Wi-Fi hoặc Bluetooth (trong trường hợp của Raspberry Pi Zero v1.3) và không bao giờ được kết nối với máy tính để truyền dữ liệu Exchange. Giao tiếp chỉ được thực hiện thông qua hệ thống mã QR Exchange. Cụ thể, phần mềm quản lý danh mục đầu tư của bạn (chẳng hạn như Sparrow wallet) sẽ hiển thị giao dịch cần ký dưới dạng mã QR; bạn quét chúng bằng camera của SeedSigner, sau đó thiết bị sẽ ký giao dịch bằng khóa riêng tư được lưu trữ tạm thời trong RAM. Cuối cùng, nó tạo ra mã QR chứa giao dịch đã ký, bạn quét mã QR này bằng phần mềm để gửi đến mạng Bitcoin.
...
Một lựa chọn khác để lưu trữ seed và sử dụng nó với SeedSigner là sử dụng thẻ thông minh *SeedKeeper* kết hợp với đầu đọc tương thích. Điều này cung cấp cho bạn một *secure element* rất mạnh mẽ để lưu trữ seed, đồng thời sử dụng màn hình SeedSigner để ký giao dịch. Tuy nhiên, cấu hình cụ thể này sẽ được trình bày trong một hướng dẫn chuyên sâu khác. Ở đây, chúng ta sẽ tập trung vào cách sử dụng SeedSigner cơ bản:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

Dự án SeedSigner chiếm một vị trí quan trọng trong hệ sinh thái Bitcoin, vì nó mang đến cho mọi người, ở mọi nơi trên thế giới, cơ hội được hưởng lợi từ hệ thống bảo mật tiên tiến để bảo vệ bitcoin của họ. Ưu điểm chính của nó nằm ở khả năng tiếp cận: phần cứng cần thiết có thể được mua với giá dưới 50 đô la. Hơn nữa, nó cho phép những người sống ở các quốc gia bị hạn chế tự xây dựng Hardware Wallet của riêng họ từ các linh kiện máy tính tiêu chuẩn, dễ tìm và ít bị ràng buộc bởi các quy định.



Nhưng ngay cả bên ngoài những bối cảnh cụ thể này, SeedSigner vẫn có thể là một lựa chọn thú vị dành cho bạn: nó là mã nguồn mở, hoạt động không trạng thái và không có khoảng cách, đồng thời giảm các vectơ tấn công liên quan đến chuỗi Supply của Hardware Wallet của bạn.
...
- Raspberry Pi Zero
 ...
 - Phiên bản W và v2 cũng tương thích, nhưng tích hợp chip Wi-Fi/Bluetooth. Do đó, bạn nên vô hiệu hóa nó bằng cách tháo module radio ra khỏi card. Thao tác này tương đối đơn giản, nhưng đòi hỏi độ chính xác cao (kìm nhỏ là đủ cho Zero W, trong khi bút Hot là cần thiết cho v2 để tháo tấm kim loại che module). Tôi sẽ không đi sâu vào chi tiết trong hướng dẫn này, nhưng bạn sẽ tìm thấy tất cả hướng dẫn trong tài liệu này: *[Vô hiệu hóa WiFi/Bluetooth bằng phần cứng](https://github.com/DesobedienteTecnologico/rpi_disable_wifi_and_bt_by_hardware)*.
 ...
 - Đừng quên mang theo bộ nguồn micro-USB Supply.
...
- Màn hình Waveshare 1,3" (240×240 px)** (bằng tiếng Pháp)
    ...
    - Màn hình bao gồm ba nút bấm và một cần điều khiển mini đóng vai trò là Interface của người dùng.
...
- Nhà ở (tùy chọn, nhưng được khuyến nghị)** (tùy chọn, nhưng được khuyến nghị)** (tùy chọn, nhưng được khuyến nghị)** (tùy chọn, nhưng được khuyến nghị)** (khuyến nghị)
...
Sau đó, PowerShell sẽ tính toán Hash SHA256 của tệp hình ảnh. So sánh Hash này với giá trị tương ứng trong `seedsigner.0.8.6.sha256.txt`.
...
Xác định vị trí đầu nối ribbon camera trên Raspberry Pi Zero: đó là một dải đen mỏng ở cạnh bo mạch, có thể nhấc nhẹ lên để mở. Hãy nhấc nó lên một cách cẩn thận, không dùng lực mạnh: nó chỉ cần nghiêng một vài mm.
...
Sau đó, đặt mô-đun camera vào lỗ thích hợp trên vỏ. Tùy thuộc vào kiểu máy, mô-đun có thể được gắn trực tiếp hoặc cần một dải keo nhỏ để giữ cố định. Ống kính phải được căn chỉnh hoàn hảo, hướng ra ngoài.
...
Nếu bạn đã thay đổi hướng camera hoặc muốn thay đổi các cài đặt khác (chẳng hạn như ngôn ngữ Interface) sau này, bạn sẽ cần bật tính năng lưu trữ cài đặt trên thẻ nhớ microSD. Nếu không, cài đặt của bạn sẽ trở về mặc định mỗi khi bạn khởi động lại, vì Raspberry Pi Zero không có bộ nhớ lưu trữ.
...
Phiên bản firmware hiện tại hỗ trợ một số gói phần mềm: Sparrow, Spectre, BlueWallet, Nunchuk và Keeper. Trong trường hợp của tôi, tôi sử dụng **Sparrow wallet**, tôi đặc biệt khuyên dùng vì tính đơn giản và nhiều chức năng của nó.
...
- gW-20 (satoshi, hoặc 1/100.000.000 BTC)
...
- Mạng gW-22`: chỉ được sửa đổi nếu bạn muốn sử dụng SeedSigner trên Testnet.
...
- `Xpub export`: bật hoặc tắt tính năng xuất khóa công khai mở rộng (`xpub`, `ypub`, `zpub`) sang phần mềm quản lý danh mục đầu tư thông qua mã QR (một chức năng chúng ta sẽ sử dụng sau, vì vậy hãy để nó bật ở đây).
...
Cuối cùng, trong menu `Ngôn ngữ`, bạn có thể thay đổi ngôn ngữ của Interface theo sở thích của mình.
...
seed (hoặc cụm từ Mnemonic) tạo thành nền tảng cho danh mục đầu tư Bitcoin của bạn. Nó được sử dụng để lấy khóa riêng và địa chỉ của bạn, đồng thời cung cấp quyền truy cập vào quỹ của bạn. SeedSigner cung cấp một số phương pháp để tạo ra nó, chúng ta sẽ tìm hiểu trong phần này.
...
- Cụm từ này cho phép bạn truy cập đầy đủ, không hạn chế vào tất cả bitcoin của mình.** Bất kỳ ai sở hữu cụm từ này đều có thể đánh cắp tiền của bạn, ngay cả khi không có quyền truy cập trực tiếp vào SeedSigner của bạn;
- Thông thường, một cụm từ 12 từ được sử dụng để khôi phục Wallet nếu Hardware Wallet bị mất hoặc bị đánh cắp. Tuy nhiên, vì SeedSigner là thiết bị *không trạng thái*, nó không bao giờ đăng ký seed của bạn. Vì vậy, bản sao lưu vật lý của bạn không chỉ là bản sao lưu, mà còn là **cách duy nhất để sử dụng Wallet**. Nếu bạn làm mất những bản sao lưu này, bitcoin của bạn sẽ bị mất vĩnh viễn. Vì vậy, hãy sao lưu chúng cẩn thận, trên nhiều phương tiện lưu trữ và ở những nơi an toàn;
- Nếu bạn mới bắt đầu, tôi thực sự khuyên bạn nên đọc hướng dẫn này để hiểu rõ hơn về những rủi ro liên quan đến việc quản lý cụm từ Mnemonic:
...
Sau đó chọn độ dài cho cụm từ Mnemonic của bạn. 12 từ đã mang lại mức độ bảo mật đủ tốt, vì vậy đây là lựa chọn tôi khuyên bạn nên làm.
...
Cẩn thận viết lại các từ trong cụm từ Mnemonic của bạn trên một vật dụng hỗ trợ phù hợp (giấy hoặc kim loại).
...
Nếu bạn nhập sai, SeedSigner sẽ thông báo và bạn sẽ phải bắt đầu lại, nhớ ghi lại cụm từ Mnemonic khi được gửi. Bước xác minh này đảm bảo bản sao lưu của bạn chính xác và đầy đủ. Sau khi xác thực, màn hình sẽ hiển thị `Sao lưu đã được xác minh`.
...
SeedSigner là một thiết bị không có bộ nhớ vĩnh viễn. Điều này có nghĩa là seed của bạn không bao giờ được lưu trữ bên trong thiết bị (không giống như Ledger, Trezor hay Coldcard chẳng hạn). Ngay khi bạn tắt nguồn, seed sẽ biến mất hoàn toàn khỏi RAM. Khi SeedSigner khởi động lại, nó sẽ trở về trạng thái trống: bạn sẽ cần phải nhập lại seed để ký các giao dịch.



Điều này cung cấp khả năng bảo vệ thiết yếu. Không giống như các ví phần cứng khác, SeedSigner dựa trên Raspberry Pi Zero mà không có bất kỳ lớp bảo vệ vật lý nào, bao gồm cả *secure element*. Tuy nhiên, vì không lưu trữ dữ liệu nhạy cảm, ngay cả một thiết bị bị xâm nhập vật lý cũng không thể cho phép kẻ tấn công trích xuất khóa riêng tư hoặc tiêu thụ bitcoin của bạn.



Mặt khác, kiến trúc này cũng đi kèm với một trách nhiệm bổ sung: nếu không có bản sao lưu, tiền của bạn chắc chắn sẽ bị mất. Đó là lý do tại sao tôi khuyên bạn nên sử dụng **bản sao lưu kép**. Bạn đã có cụm từ khôi phục: đây là bản sao lưu dài hạn chính của bạn, cần được lưu giữ ở nơi an toàn. Bây giờ, chúng ta sẽ tạo một bản sao của cụm từ này dưới dạng **mã QR**.
...
Tôi cũng khuyên bạn nên lưu trữ mã QR và cụm từ Mnemonic ở hai nơi riêng biệt để tránh mất mọi thứ khi xảy ra khiếu nại.
...
Sau khi xác minh hoàn tất, SeedSigner sẽ hiển thị dấu vân tay của khóa chính Wallet của bạn. Dấu vân tay này sẽ nhận dạng Wallet của bạn và đảm bảo bạn sử dụng đúng cụm từ khôi phục trong tương lai. Nó không tiết lộ bất kỳ thông tin nào về khóa riêng của bạn, vì vậy bạn có thể lưu trữ an toàn trên phương tiện kỹ thuật số. Chỉ cần đảm bảo bạn giữ một bản sao có thể truy cập được và không bao giờ làm mất nó.
...
Cũng ở giai đoạn này, bạn có thể thêm **passphrase BIP39** để tăng cường bảo mật cho Wallet. Tùy chọn này có thể đáng giá, tùy thuộc vào chiến lược sao lưu của bạn, nhưng cũng tiềm ẩn rủi ro: nếu bạn mất passphrase, quyền truy cập vào bitcoin của bạn sẽ bị mất vĩnh viễn.



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

Nếu bạn chưa quen với khái niệm passphrase, tôi mời bạn đọc hướng dẫn toàn diện về chủ đề này:
...
Để thực hiện việc này, bạn cần một tờ giấy trắng hoặc mã QR kim loại tương ứng với độ dài của cụm từ Mnemonic. Nếu bạn đã mua trọn bộ SeedSigner, các mẫu thường được bao gồm. Nếu không, bạn có thể tải xuống và in (hoặc sao chép thủ công) tại đây:
...
Từ màn hình seed, chọn `Sao lưu seed`.
...
Cẩn thận sao chép từng chấm đen trên bảng lưu của bạn, sau đó sử dụng cần điều khiển để chuyển sang BLOCK tiếp theo. Hãy từ từ: chỉ cần một sai lệch nhỏ cũng có thể khiến mã QR không sử dụng được.
...
Xin chúc mừng, danh mục Bitcoin của bạn hiện đã được thiết lập và chạy! Bây giờ chúng ta sẽ nhập các thành phần công khai của danh mục này vào **Sparrow wallet** để quản lý dễ dàng.
...
Hãy đảm bảo bạn đã cài đặt phần mềm Sparrow wallet trên máy tính. Nếu bạn không biết cách tải xuống, kiểm tra và cài đặt đúng cách, vui lòng xem hướng dẫn đầy đủ của chúng tôi về chủ đề này:
...
Đặt tên cho danh mục đầu tư của bạn, sau đó nhấp vào `Tạo Wallet`. Sau đó, Sparrow sẽ yêu cầu bạn đặt mật khẩu để khóa quyền truy cập cục bộ vào Wallet này. Hãy chọn một mật khẩu mạnh: mật khẩu này bảo vệ quyền truy cập vào dữ liệu danh mục đầu tư của bạn trong Sparrow (khóa công khai, địa chỉ, nhãn và lịch sử giao dịch). Không cần mật khẩu này để khôi phục danh mục đầu tư sau này: chỉ cần cụm từ Mnemonic (và có thể cả passphrase) cho mục đích này.
...
SeedSigner và Sparrow wallet của bạn hiện đã được liên kết an toàn. Sparrow hoạt động như một Interface quản lý hoàn chỉnh, trong khi SeedSigner vẫn là thiết bị duy nhất có khả năng ký các giao dịch của bạn. Giờ đây, bạn đã sẵn sàng nhận và gửi bitcoin trong cấu hình air-gapped hoàn toàn.
...
#### 7.1.1 Tạo tín hiệu thu Address
...
Một Bitcoin Address mới sẽ được hiển thị. Bạn sẽ thấy:




- Văn bản Address (bắt đầu bằng `bc1p...` nếu bạn sử dụng P2TR như tôi),
- Mã QR tương ứng,
- Trường `Nhãn` để theo dõi giao dịch của bạn.



Tôi thực sự khuyên bạn nên thêm nhãn vào mỗi biên lai Bitcoin trên Wallet. Điều này sẽ giúp bạn dễ dàng xác định nguồn gốc của từng UTXO và cải thiện việc quản lý quyền riêng tư. Để tìm hiểu sâu hơn về chủ đề quan trọng này, bạn có thể xem chương trình đào tạo chuyên sâu trên Plan₿ Academy:
...
Address của bạn hiện được liên kết với nhãn này trong tất cả các phần Sparrow.
...
Trước khi chia sẻ mã Address nhận được, điều rất quan trọng là phải kiểm tra xem mã này có thuộc về mã seed của bạn hay không. Bước này đảm bảo SeedSigner của bạn có thể ký các giao dịch liên quan đến mã Address này. Nó cũng bảo vệ bạn khỏi các cuộc tấn công có thể xảy ra khi Sparrow hiển thị mã Address giả mạo. Hãy nhớ rằng Sparrow chạy trên một môi trường không an toàn (máy tính của bạn), nơi có bề mặt tấn công lớn hơn nhiều so với SeedSigner của bạn, vốn hoàn toàn bị cô lập. Đó là lý do tại sao bạn không bao giờ nên tin tưởng mù quáng vào bất kỳ mã Address nhận nào được hiển thị trên Sparrow cho đến khi bạn đã xác minh nó bằng Hardware Wallet của mình.



Trên Sparrow, hãy nhấp vào mã QR của Address để phóng to: sau đó mã sẽ được hiển thị toàn màn hình.
...
Nếu Address được quét trùng khớp với mã seed của bạn, màn hình SeedSigner sẽ hiển thị thông báo: `Address đã xác minh`.
...
Điều này xác nhận rằng Address thuộc về Wallet của bạn và bạn có thể tự tin nhận bitcoin từ nó.
...
Bây giờ bạn có thể gửi Address này (dưới dạng văn bản hoặc mã QR) cho người hoặc bộ phận cần gửi Satss cho bạn. Sau khi giao dịch được phát trên mạng, nó sẽ xuất hiện trong tab `Giao dịch` của Sparrow wallet.
...
- Trong `Pay to`, hãy dán mã Address của người nhận hoặc nhấp vào biểu tượng máy ảnh để quét mã QR;
- Trong `Nhãn`, thêm nhãn để theo dõi khoản chi phí này;
- Trong `Số tiền`, nhập số tiền cần gửi;
- Cuối cùng, hãy chọn mức phí dựa trên điều kiện thị trường hiện tại (có thể tham khảo ước tính trên [Mempool.space](https://Mempool.space/)).
...
Nhấp vào `Xem lại Chi tiết` và kiểm tra cẩn thận tất cả thông tin trực tiếp trên màn hình SeedSigner. Các mục quan trọng nhất cần kiểm tra là số tiền đã gửi, mã Address của người nhận và số tiền của bất kỳ khoản phí nào được áp dụng.
...
Bây giờ giao dịch đã hợp lệ, nó cần được phát trên mạng Bitcoin để đến được Miner, người sẽ thêm giao dịch đó vào BLOCK.
...
Trình bày mã QR được SeedSigner của bạn hiển thị (mã của giao dịch đã ký) cho webcam. Sparrow sẽ giải mã chữ ký và hiển thị đầy đủ chi tiết giao dịch. Kiểm tra lại lần cuối để đảm bảo tất cả thông tin đều chính xác, sau đó nhấp vào Phát Giao dịch để phát trên mạng Bitcoin.
