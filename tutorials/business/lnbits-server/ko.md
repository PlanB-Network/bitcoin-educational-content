---
name: LNbits 서버
description: PHOENIXD을 사용하는 Ubuntu VPS 또는 Umbrel에 자체 호스팅된 LNbits 서버 설치 및 구성
---
...
LNbits는 오픈 소스 Interface 웹 애플리케이션으로, 모든 라이트닝 백엔드(LND, 코어 라이트닝, PHOENIXD)를 완전한 서비스 플랫폼으로 변환합니다. 이 자체 호스팅 솔루션을 사용하면 자금에 대한 완전한 통제권을 유지하면서 여러 라이트닝 포트폴리오를 개별적으로 관리하고, POS를 배포하고, 기부 시스템 또는 청구 서비스를 만들 수 있습니다.



이 튜토리얼에서는 두 가지 설치 방법을 다룹니다: *gW-4가 포함된 *VPS 우분투**(전체 Bitcoin 노드가 없는 경량 솔루션)와 **엄브렐**(기존 LND 노드와 통합). 개념과 확장을 다루는 Plan B Network의 일반 LNbits 튜토리얼과 달리 이 가이드는 단계별 기술 설치 절차에 중점을 둡니다.
...
LNbits는 기존 백엔드(LND, 코어 라이트닝, PHOENIXD)에 연결되는 파이썬(FastAPI)으로 개발된 라이트닝 어카운팅 시스템입니다. 기존 라이트닝 노드와 달리 LNbits는 액세스 가능한 Interface을 제공하므로 자체 API 키를 사용하여 여러 개의 격리된 포트폴리오를 관리할 수 있습니다. 가족, 직원 또는 프로젝트를 위한 하위 계정을 만들 수 있으며, 이들에게 모든 자금에 대한 액세스 권한을 부여하지 않아도 됩니다.
...
대시보드**에는 실시간 잔액, 거래 내역, 청구 도구가 표시됩니다. 각 Wallet에는 API 키가 포함된 고유 URL이 있어 기존 로그인 없이도 액세스할 수 있습니다. 3단계 API 키 시스템**(관리자, Invoice, 읽기 전용)은 안전한 통합을 위해 세분화된 권한 제어 기능을 제공합니다.
...
**엄브렐**: 앱 스토어에서 쉽게 설치할 수 있습니다. 전제 조건: 동기화된 LND 및 개방형 채널을 갖춘 기능적인 Umbrel 노드. 자동 구성.
...
서버가 안전해지면 PHOENIXD를 설치하고 구성해야 합니다. Plan B Network는 설치, seed 생성 및 시스템 서비스 구성을 다루는 완전한 전용 튜토리얼을 제공합니다:



https://planb.academy/tutorials/node/lightning-network/phoenixd-beb86edd-f9c0-4bec-ad36-db234c88e7b1

PHOENIXD이 실행되면 (`./Phoenix-CLI getinfo`로 확인) `~/.Phoenix/Phoenix.conf`에 **HTTP 비밀번호**를 기록해 두세요 - LNbits를 PHOENIXD에 연결할 때 필요합니다.
...
슈퍼유저 관리 Interface에서 '설정'으로 이동한 다음 '사용자 관리' 섹션으로 이동합니다. "새 사용자 만들기 허용" 옵션을 찾을 수 있습니다.
...
- 공격자는 사용자 모르게 계정을 생성하고 LIGHTNING NODE의 유동성을 사용할 수 있습니다
- Interface 슈퍼유저에서 사용자 계정을 수동으로 생성해야 합니다
...
일단 구성이 완료되면 슈퍼유저 관리자만 Interface "사용자"를 통해 새 사용자 계정을 만들 수 있습니다. 이 접근 방식은 누가 라이트닝 인프라에 액세스하고 자금을 사용할 수 있는지에 대한 완전한 제어를 보장합니다.
...
PHOENIXD은 자동 유동성을 통해 채널을 자동으로 관리합니다. generate는 LNbits에서 ~30,000 Sats의 라이트닝 Invoice을 받고 다른 Wallet에서 지불합니다. PHOENIXD은 ACINQ에 채널을 자동으로 개설합니다. 개설 수수료(~20~23만 Sats)가 차감되고, On-Chain 확인 후 잔액(~7~10만 Sats)이 표시됩니다.
...
엄브렐은 필요한 종속성을 자동으로 확인합니다. LNbits를 실행하려면 LIGHTNING NODE(LND)이 필요합니다. LIGHTNING NODE이 이미 작동 중인 경우 "LNbits 설치"를 클릭하여 확인합니다.
...
Umbrel은 Docker 이미지를 다운로드하고 LND와의 연결을 자동으로 구성한 후 컨테이너를 시작합니다(2~5분). 설치는 전적으로 백그라운드에서 이루어집니다.
...
처음 실행하면 LNbits에서 슈퍼유저 관리자 계정을 만들라는 메시지가 표시됩니다. 사용자 이름을 입력하고 보안 비밀번호를 설정하여 Interface 관리 시스템에 대한 액세스를 보호합니다.
...
계정을 생성하면 자동으로 Interface 기본 관리 영역으로 이동합니다. 엄브렐은 이미 LND를 자금 출처로 설정해 두었으므로 모든 라이트닝 결제는 기존 채널을 통해 이루어집니다.
...
- LNbits 잔액**: LNbits 시스템에 할당된 자금(초기 0 Sats)
...
**중요 사항**: 각 사용자는 자신의 비밀번호로 완전히 독립적으로 로그온할 수 있습니다. 슈퍼유저 관리자는 Interface 관리 도구를 통해 모든 권한을 보유합니다.
...
이제 "Satoshi" 사용자가 생성되었으므로 그에게 Wallet 라이트닝을 할당해야 합니다. 해당 사용자의 Wallet 아이콘(두 번째 아이콘)을 클릭한 다음 "새 Wallet 만들기"를 클릭합니다.
...
이제 두 개의 기존 지갑, 즉 자동으로 생성된 기본 Wallet "LNbits Wallet"와 새로운 "Wallet Of Satoshi"가 표시됩니다. 사용자 환경을 단순화하기 위해 삭제 아이콘(빨간색 휴지통)을 클릭하여 기본 Wallet를 삭제할 수 있습니다.
...
이제 "Satoshi" 사용자는 명확하게 식별되는 하나의 Wallet을 갖게 됩니다. 각 사용자 Wallet은 완전히 자율적으로 작동하며 기본 LND 노드의 유동성을 사용합니다.



**핵심 개념**: 이 모든 지갑은 엄브렐 노드의 글로벌 유동성을 공유합니다. 각 Wallet에 대해 새로운 라이트닝 채널을 만들지 않고, LNbits는 기존 라이트닝 인프라 내에서 자금 할당을 관리하는 지능형 회계 Layer 역할을 합니다. 이것이 바로 LNbits의 멀티 Wallet 시스템의 힘입니다.
...
슈퍼유저 계정(오른쪽 상단 아이콘)에서 로그아웃하고 LNbits 로그인 페이지로 돌아갑니다. 이제 새 사용자의 자격 증명으로 로그인할 수 있습니다.
...
이전에 정의한 사용자 이름("Satoshi")과 비밀번호를 입력한 다음 "로그인"을 클릭합니다. 사용자는 관리 Interface과 완전히 분리된 개인 Wallet에 직접 액세스할 수 있습니다.



### Wallet 사용자의 Interface



연결이 완료되면 사용자는 Wallet Lightning에서 Interface에 액세스합니다.
...
- 주요 작업**: "요청 붙여넣기"(결제할 청구서 붙여넣기), "Invoice 생성"(영수증 generate), QR 아이콘(빠른 스캔)
...
LNbits는 스마트폰에서 Wallet를 사용할 수 있는 여러 가지 방법을 제공합니다:
...
- 모바일 앱으로 LndHub QR 코드를 스캔하여 Wallet을 연결합니다
...
이 다중 사용자 구성의 주요 사용 사례는 **가족 또는 가까운 지인들과 지갑을 공유하는 경우**입니다. 전용 Wallet(예시에서는 "Satoshi")로 사용자를 생성한 후에는 신뢰할 수 있는 가족 구성원과 이러한 로그인 자격 증명을 공유할 수 있습니다.
...
이 이중 Layer 보호(네트워크 액세스 + 사용자 인증) 덕분에 엄브렐에서는 "새 사용자 생성 허용" 옵션이 덜 중요해집니다. 네트워크 또는 VPN에 이미 액세스 권한이 있는 사람만 Interface 로그인에 도달할 수 있습니다.



**일반적인 시나리오**: "아빠" 계정, "엄마" 계정, "비즈니스" 계정 등을 생성합니다. 각 가족 구성원은 각자의 독립된 Wallet Lightning을 사용하면서 엄브렐 노드의 공유 유동성을 활용할 수 있습니다. 사용자 이름과 비밀번호를 공유하기만 하면 로컬 네트워크의 모든 기기에서 또는 Tailscale VPN을 통해 연결할 수 있습니다. 자세한 내용은 Tailscale 전용 튜토리얼을 참조하세요:
...
Interface 슈퍼유저로 돌아가 왼쪽 패널의 '확장 프로그램' 메뉴에 액세스하여 전체 LNbits 확장 프로그램 에코시스템을 살펴보세요.
...
각 확장 프로그램은 이 Interface에서 클릭 한 번으로 활성화됩니다. '무료'로 표시된 확장 프로그램은 무료이며, 일부는 '유료' 버전으로 제공됩니다. 카탈로그를 살펴보고 비즈니스, 가족 관리, Lightning Network의 기능 실험 등 필요에 맞는 확장 기능을 찾아보세요.
...
**혜택**: 재무 주권(자금/키/데이터에 대한 완전한 제어), 아키텍처 유연성(무손실 VPS→Full node 마이그레이션), 전문 확장 시스템, 직관적인 Interface.
...
**백업**: seed PHOENIXD/자격증명 LND, LNbits 데이터베이스, '.env' 파일. 매일 자동화하고, 프로덕션 서버에서 분리하여 암호화합니다. 정기적으로 복원을 테스트합니다.
...
- 엄브렐**에서: App Store에서 새 버전을 자동으로 알려줍니다. "확장 프로그램 관리" > "모두 업데이트"를 통해 확장 프로그램을 동기화합니다. Umbrel 자동 백업에 SQLite 데이터베이스가 포함되어 있는지 확인하세요.
...
- [PHOENIXD 깃허브](https://github.com/ACINQ/PHOENIXD)
