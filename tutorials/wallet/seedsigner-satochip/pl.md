...
Satochip to Hardware Wallet w formacie karty inteligentnej, z certyfikowanym elementem zabezpieczającym EAL6+, jednym z najwyższych standardów bezpieczeństwa. Została zaprojektowana i wyprodukowana przez belgijską firmę o tej samej nazwie: Satochip.



Wyceniony na około 25 euro, Satochip wyróżnia się na tle konkurencji doskonałym stosunkiem jakości do ceny. Dzięki bezpiecznemu chipowi oferuje odporność na ataki fizyczne. Co więcej, kod źródłowy jego apletu jest całkowicie open-source, na licencji *AGPLv3*.
...
Wybierz `Label`, aby łatwo zidentyfikować różne sekrety przechowywane w Seedkeeper. Możesz na przykład po prostu zachować odcisk palca Wallet lub wyraźnie wskazać `seed`. Wybór zależy od preferencji i ryzyka.
...
Kod PIN Seedkeeper powinien być tak długi i losowy, jak to możliwe, aby zapobiec próbom brutalnej siły w przypadku fizycznego naruszenia karty. Należy również zachować kopię zapasową tego kodu PIN, przechowywaną w innym miejscu niż Seedkeeper. Bez tego kodu PIN nie będzie można uzyskać dostępu do Mnemonic przechowywanych w Seedkeeper, a bitcoiny zostaną utracone na zawsze.
...
Teraz, gdy portfel został wygenerowany, zapisany i sprawdzony, przeniesiemy go do Satochip. Aby to zrobić, upewnij się, że seed jest załadowany do pamięci RAM SeedSigner. Następnie przejdź do `Tools > Smartcard Tools > Satochip Functions`.
...
Upewnij się, że oprogramowanie Sparrow wallet jest zainstalowane na komputerze. Jeśli nie wiesz, jak je pobrać, sprawdzić jego autentyczność i poprawnie zainstalować, zapoznaj się z naszym kompletnym samouczkiem na ten temat:
...
Przypisz nazwę do portfela, a następnie kliknij `Create Wallet`. Sparrow poprosi o ustawienie hasła w celu zablokowania lokalnego dostępu do tego Wallet. Wybierz silne hasło: chroni ono dane w Sparrow (klucze publiczne, adresy, etykiety i historię transakcji). Hasło to nie jest jednak wymagane do przywrócenia Wallet w przyszłości: wymagana będzie jedynie fraza Mnemonic (i ewentualnie passphrase).
...
Teraz sprawdź, czy `Master fingerprint` wyświetlany w Sparrow wallet jest zgodny z tym znalezionym wcześniej w SeedSigner.



Następnie SeedSigner poprosi o zeskanowanie losowego odbiorczego Address z Sparrow wallet w celu potwierdzenia ważności importu.
...
Pojawia się nowy Bitcoin Address. Znajdziesz :




- Address w formacie tekstowym (zaczynającym się od `bc1q...`, jeśli używasz P2WPKH, jak w tym przykładzie);
- Powiązany kod QR ;
- Pole `Label`, przydatne do śledzenia transakcji.



Zdecydowanie zalecam dodanie etykiety do każdego paragonu Bitcoin w Wallet. Pomoże to łatwo zidentyfikować pochodzenie każdego UTXO i lepiej zarządzać swoją prywatnością. Aby dowiedzieć się więcej na ten ważny temat, zapoznaj się z dedykowanym szkoleniem w Plan ₿ Academy :
...
Przed przekazaniem płatnikowi otrzymanego Address ważne jest, aby sprawdzić, czy należy on do seed. Ten krok gwarantuje, że Satochip będzie w stanie podpisać transakcje powiązane z tym Address. Zapobiega to również potencjalnym atakom, w których Sparrow wyświetlałby fałszywy Address. Należy pamiętać, że Sparrow działa w niezabezpieczonym środowisku (na komputerze użytkownika), którego powierzchnia ataku jest znacznie większa niż Satochip, który jest całkowicie odizolowany. Dlatego nigdy nie należy ślepo ufać adresom wyświetlanym w Sparrow przed sprawdzeniem ich na Hardware Wallet.
...
Jeśli zeskanowany Address pasuje do seed, SeedSigner wyświetli komunikat: `Address Verified`.
...
Dzięki temu możesz mieć pewność, że Address należy do Twojego portfolio.
...
Możesz teraz przesłać ten Address w formie tekstowej lub za pośrednictwem kodu QR do osoby lub usługi, która musi wysłać Ci Satss. Gdy transakcja zostanie przesłana do sieci, pojawi się w zakładce "Transakcje" w Sparrow wallet.
...
- W `Pay to`, wklej Address odbiorcy lub zeskanuj jego kod QR za pomocą ikony aparatu ;
- W polu `Label` dodaj etykietę, aby śledzić ten wydatek;
- W polu `Amount` wprowadź kwotę do wysłania;
- Na koniec należy wybrać szybkość ładowania zgodnie z bieżącymi warunkami sieciowymi (szacunki są dostępne na stronie [Mempool.space](https://Mempool.space/)).
...
Teraz, gdy transakcja została podpisana i zweryfikowana, pozostaje tylko rozgłosić ją w sieci Bitcoin, aby Miner mógł dołączyć ją do BLOCK. W Sparrow kliknij `Scan QR`.
...
Przedstaw kod QR wyświetlony na urządzeniu SeedSigner (zawierający podpisaną transakcję) kamerze internetowej. Sparrow wyświetli wszystkie szczegóły transakcji. Sprawdź ostatecznie, czy wszystkie informacje są prawidłowe, a następnie kliknij przycisk "Broadcast Transaction", aby rozgłosić transakcję w sieci Bitcoin.
...
Transakcja jest teraz przesyłana do sieci. Możesz śledzić jej potwierdzenie w zakładce "Transakcje" w Sparrow wallet.
...
- Zapisując frazę Mnemonic na nośniku fizycznym;
...
Jeśli nadal posiadasz Satochip, ale Twój SeedSigner jest uszkodzony lub zagubiony, sytuacja jest dość prosta do opanowania, ponieważ Twój Wallet jest nadal w Satochip.
...
Jeśli nie chcesz go odbudowywać, możesz również używać swojego Satochipa w klasyczny sposób, tj. bezpośrednio z komputera, bez przechodzenia przez SeedSigner. Ta metoda działa doskonale, ale znacznie zmniejsza bezpieczeństwo Bitcoin Wallet: tracisz izolację "*air-gapped*" i teraz musisz podpisywać na ślepo, ponieważ SeedSigner działał jako zaufany ekran. Może to być jednak tymczasowe rozwiązanie w sytuacjach awaryjnych lub jeśli nie jesteś w stanie odbudować SeedSigner.



Aby to zrobić, będziesz potrzebować karty inteligentnej USB lub czytnika NFC. Otwórz Wallet, który chcesz przywrócić w Sparrow, a następnie przejdź do zakładki `Ustawienia` i kliknij `Zastąp`.
...
Na koniec wprowadź kod PIN karty inteligentnej, aby ją odblokować. Będziesz wtedy mógł uzyskać dostęp do Wallet, tworzyć transakcje i podpisywać je bezpośrednio za pomocą podłączonego Satochip.
...
- Wprowadź ręcznie frazę Mnemonic w menu `Seeds > Enter 12-word seed`.
...
- Możesz też załadować seed z Seedkeepera za pomocą menu `Seeds > From SeedKeeper` (jest to metoda, której używam w tym poradniku). Wystarczy wprowadzić kod PIN Seedkeepera i wybrać sekret, który ma być używany jako seed w SeedSigner.
...
Teraz wiesz, jak używać Satochip do bezpiecznego zarządzania portfelem Bitcoin w połączeniu z SeedSigner.
