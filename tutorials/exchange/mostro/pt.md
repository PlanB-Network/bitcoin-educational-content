---
name: Mostro
description: Plataforma Bitcoin P2P Exchange sem KYC via Lightning e Nostr
---
...
Como é que adquire ou vende bitcoins sem comprometer a sua soberania financeira? As plataformas centralizadas impõem procedimentos KYC invasivos que expõem os seus dados pessoais, com a possibilidade de congelamento arbitrário da conta ou vigilância estatal.



*o *Mostro P2P** oferece uma alternativa descentralizada que combina o Lightning Network, o protocolo Nostr e a retenção de facturas. A sua principal inovação: um sistema de caução automatizado em que os seus bitcoins permanecem sob o seu controlo durante todo o Exchange, eliminando o risco de roubo, falência do Exchange ou confisco arbitrário.
...
**Flexibilidade de pagamento**: Suporte para muitas moedas fiduciárias através do serviço de taxas Exchange da yadio.io. Aceita transferências SEPA, PayPal, Revolut, dinheiro e qualquer método acordado entre as partes.
...
A Mostro oferece dois clientes operacionais principais para as suas bolsas P2P.
...
O **Mostro CLI** é o cliente mais maduro e funcional. Escrito em Rust, ele oferece toda a gama de recursos do Mostro através de uma linha de comando Interface. Compatível com macOS, Linux e Windows.
...
- libertar: Libertar o depósito de garantia e finalizar o Exchange
- `getdm`: Ver mensagens diretas
- `rate` : Avaliar a sua contraparte após uma Exchange
...
- Opcional mas recomendado**: Ligação VPN ou Tor para mascarar o seu IP Address
...
- Negocie Bitcoin livremente - sem KYC** : Negoceie sem verificação de identidade graças à Nostr
...
- Proteção de rede**: Ativar VPN ou Tor para ocultar o seu IP Address
...
A Mostro pedir-lhe-á que forneça um **Lightning Invoice** para receber as BTC. O montante exato em satoshis será indicado (aqui: 4715 Sats).
...
### Passo 3: Fornecer o seu Lightning Invoice



generate um Lightning Invoice do seu Wallet (Phoenix, Breez, etc.) para o montante exato, depois envie-o :
...
O sistema confirma o envio e informa-o de que deve aguardar que o vendedor pague a retenção Invoice antes de ativar o depósito de garantia.
...
Assim que o vendedor confirmar o recebimento da moeda fiduciária e liberar o depósito com o comando `release`, você receberá instantaneamente seus bitcoins no Lightning Invoice que você forneceu.
...
**Importante para a sincronização CLI/Mobile**: Se utilizar tanto o CLI como a aplicação móvel, configure **exatamente os mesmos relés Nostr e Mostro Pubkey** em ambos os clientes. Sem esta configuração idêntica, não verá as mesmas encomendas disponíveis em ambas as interfaces. Os relés e a Mostro Pubkey visíveis em Settings (screenshot acima) devem corresponder aos do arquivo `.env' do CLI.
...
Prima o botão **"+"** do Green no canto inferior direito e, em seguida, selecione **VENDER** (botão vermelho).
...
**Instruções importantes**: Deve agora **pagar o Invoice hold** apresentado para bloquear os seus bitcoins (aqui 4674 Sats por 1-5 EUR) no depósito de garantia. Tens **15 minutos no máximo**, caso contrário o Exchange será cancelado.



Copiar a retenção Invoice e pagá-la a partir do seu Wallet Lightning (Phoenix, Breez, etc.).
...
Depois de ter **confirmado a receção** do pagamento em moeda fiduciária na sua conta, prima o botão **RELEASE** do Green e confirme o envio das Satss para o comprador.
...
Os Bitcoins são transferidos instantaneamente para o comprador através do seu Lightning Invoice.
...
4. Forneça o seu Lightning Invoice (gerado a partir do seu Wallet)
...
9. Receber bitcoins instantaneamente no seu Lightning Invoice
...
**Soberania financeira**: Os seus bitcoins nunca deixam o seu controlo direto graças ao mecanismo de retenção Invoice, eliminando o risco de falência ou pirataria Exchange.
