...
Uno dei punti di forza di Matrix è la sua capacità di fungere da **ponte tra diversi sistemi di comunicazione**. Grazie ai _bridge_ (ponti), è possibile collegare:
- Telegram
- WhatsApp
- Signal
- Messenger
...
La sincronizzazione della stanza avviene in tempo reale tra i server partecipanti, garantendo un'esperienza senza soluzione di continuità e mantenendo la decentralizzazione.
...
### Riprendi il controllo delle tue comunicazioni
...
Uno dei principali punti di forza di Matrix è la sua capacità di **estendere** gli scambi grazie a bridge (ponti) in grado di collegare:
- WhatsApp
- Telegram
- Signal
- Slack
...
- **SchildiChat**: Variante di Element con miglioramenti ergonomici.
...
Ora hai un account e puoi utilizzare la versione Web di Element.
...
Per invitare gli amici alla chat di gruppo, fai clic sul cerchio "i" nell'angolo in alto a destra. Si apre il pannello di destra. Clicca su "Persone" per visualizzare l'elenco dei membri della stanza: al momento dovreste essere gli unici presenti.

![cover-kali](assets/fr/12.webp)

Fai clic su "Invita in questa stanza" nella parte superiore dell'elenco delle persone e si aprirà una richiesta per invitare gli amici a unirsi a te su Matrix. Se hanno già effettuato l'accesso a Matrix, inserisci il loro ID Matrix. Se non lo sono, inserisci il loro indirizzo e-mail e saranno invitati a unirsi a te.
...
**Tre modi per entrare a far parte di una stanza**:
1. **Tramite un link di invito** (spesso sotto forma di URL `matrix.to`).
2. **Cercare il nome della stanza** nell'applicazione.
3. **Inserendo l'ID completo della stanza**, ad esempio:

`#bitcoin:matrix.org`

`#communauté-bénin:monsrv.bj`

Una volta entrato, la chatroom si comporta come un classico newsgroup, con cronologia, crittografia, file, reazioni e chiamate audio/video, a seconda del client utilizzato.
