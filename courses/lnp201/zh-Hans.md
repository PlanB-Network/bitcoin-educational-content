...
本课程旨在为您提供对闪电网络 (Lightning Network) 的深入技术理解，这是一个设计用于快速且通常成本较低地进行比特币交易的覆盖网络。您将逐步了解控制这一系统的基本概念，从支付通道的打开到路由技术和流动性管理。

**部分 1：基础知识**  
我们将从闪电网络的一般介绍开始，建立关于比特币、其地址、UTXO 和交易工作原理的基本概念。这一基本回顾是理解闪电网络如何依赖区块链的底层机制来安全运行的必要步骤。

**部分 2：通道的打开与关闭**  
在这一部分中，我们将探讨通道的开启过程，这是闪电网络的基石。您将了解如何创建承诺交易、撤销密钥在安全性中的作用，以及通道如何以协作或单方面的方式关闭。每一个步骤都会以精确且技术性的方式进行解释，以帮助您理解所有的细节。

**部分 3：流动性网络**  
闪电网络不仅限于单个通道；它是一个真正的支付网络。我们将看到如何通过使用 HTLC 在中介节点之间路由交易。本部分还将介绍入站和出站流动性的问题。

**部分 4：闪电网络工具**  
本部分介绍了闪电网络的实用工具，如 *Invoices*、*LNURL* 和 *Keysend*。您还将学习如何管理通道的流动性，这是确保支付流畅并最大化您的闪电网络交易效率的重要方面。

**部分 5：深入探索**  
最后，我们将通过回顾所涉及的概念来结束课程，同时为那些希望深入了解闪电网络的学习者打开更高级主题的大门。

准备好探索闪电网络的技术机制了吗？让我们开始吧！

---

*以下是您将在英文课程图中遇到的一些术语，以及它们的翻译，以帮助您更好地用自己的语言理解它们：*
...
| *scan QR code*     | 扫描二维码                    |
| *Show QR code*     | 显示二维码                    |
| *Asks the invoice* | 请求发票                      |
| *Give the invoice* | 提供发票                      |
| *Payment*          | 支付                          |
| *Preimage*         | 原像                          |
...
[闪电网络](https://planb.academy/resources/glossary/lightning-network)是建立在比特币协议之上的[支付通道](https://planb.academy/resources/glossary/payment-channel)网络，旨在实现快速和低成本的交易。它允许参与者之间创建支付通道，在这些通道内，交易几乎可以即时完成，且费用最小，而无需在[区块链](https://planb.academy/resources/glossary/blockchain)上单独记录每笔交易。因此，闪电网络寻求改善比特币的[可扩展性](https://planb.academy/resources/glossary/scalability)，并使其适用于低价值支付。
...
在我们的例子中，Alice在通道的一侧有100,000 [satoshis](https://planb.academy/resources/glossary/satoshi-sat)，而Bob有30,000，总共130,000 satoshis，这构成了**[通道容量](https://planb.academy/resources/glossary/lightning-channel-capacity)**。
...
比特币地址是从**[公钥](https://planb.academy/resources/glossary/public-key)**派生出来的一系列字符，而公钥本身是从**[私钥](https://planb.academy/resources/glossary/private-key)**计算出来的。正如你肯定知道的，它用于锁定比特币，这等同于在我们的钱包中接收它们。
...
在比特币上，一笔交易涉及从一个地址发送资金到另一个地址。以爱丽丝向鲍勃发送0.002比特币为例。爱丽丝使用与她的地址关联的私钥**[签名](https://planb.academy/resources/glossary/digital-signature)**交易，从而证明她确实有能力花费这些资金。但这笔交易背后究竟发生了什么？比特币地址上的资金被一个**[脚本](https://planb.academy/resources/glossary/script)**锁定，这是一种小程序，它规定了花费资金的某些条件。
...
- **[点对点](https://planb.academy/resources/glossary/peertopeer-p2p)通信（闪电协议）**：这些是[闪电节点](https://planb.academy/resources/glossary/lightning-node)互相发送的消息，用于通信。我们将在我们的图表中用虚线黑线表示这些消息。
- **支付通道（闪电协议）**：这些是在闪电上交换资金的路径，我们将用实线黑线表示。
- **比特币交易（比特币协议）**：这些是在[链上](https://planb.academy/resources/glossary/onchain)进行的交易，我们将用橙色线表示。
...
- **存款交易**：Alice准备一个比特币交易，将资金存入这个多签名地址。例如，她可能决定向这个多签名地址发送**130,000 satoshis**。这个交易是**已构建但尚未在[区块](https://planb.academy/resources/glossary/block)链上发布**。
...
一旦存款交易被包含在一个比特币区块中，并且达到了一定的[确认](https://planb.academy/resources/glossary/confirmation)深度（后续区块的数量），通道就被视为开启。
...
当Alice在通道内进行交易，向Bob发送资金时，会创建一个新的比特币交易来反映资金分配的变化。这个被称为**[承诺交易](https://planb.academy/resources/glossary/commitment-transaction)**的交易没有在区块链上发布，但代表了闪电交易后通道的新状态。
...
- **[时间锁](https://planb.academy/resources/glossary/timelock)**：每个承诺交易都包括Alice资金的时间锁。时间锁是一种[智能合约](https://planb.academy/resources/glossary/smart-contract)原语，它设置了一个交易必须满足的时间条件才能被添加到区块中。这意味着，如果Alice发布了其中一个承诺交易，她必须等待一定数量的区块过去后才能恢复她的资金。这个时间锁从承诺交易确认开始适用。其持续时间通常与通道的大小成正比，但也可以手动配置。
- **撤销密钥**：如果Bob拥有**撤销密钥**，他也可以立即花费Alice的资金。这个密钥由Alice持有的一个秘密和Bob持有的一个秘密组成。注意，每个承诺交易的这个秘密都是不同的。
   感谢这两种结合的机制，Bob有时间检测到Alice试图作弊的企图，并通过使用撤销密钥取回他的[输出](https://planb.academy/resources/glossary/output)，对Alice进行惩罚，对Bob来说，这意味着回收频道中的所有资金。我们的新承诺交易现在将如下所示：
...
- 每个人都拥有对方上一个交易的秘密，这允许他们在其中一方试图通过在比特币节点的[内存池](https://planb.academy/resources/glossary/mempool)中发布一个旧状态的交易来作弊时使用撤销密钥。确实，为了惩罚另一方，必须持有双方的秘密和对方的承诺交易，其中包括签名的[输入](https://planb.academy/resources/glossary/input)。没有这个交易，单独的撤销密钥是无用的。获取这个交易的唯一方法是从内存池（等待确认的交易）中检索它，或者在时间锁定期间从区块链上的已确认交易中检索它，这证明另一方试图作弊，无论是有意还是无意。
...
- Alice和Bob一起协商**关闭交易**的[费用](https://planb.academy/resources/glossary/transaction-fees)。这些费用通常基于关闭时比特币费用市场计算。重要的是要注意，**始终是开启通道的人**（在我们的例子中是Alice）支付关闭费用。
...
在本章中，我们将探讨即使支付受款人没有通过支付通道直接连接，闪电网络上的支付如何能够到达接收者。事实上，闪电网络是一个**支付通道网络**，它允许通过其他参与者的通道将资金发送到远端节点。我们将发现支付是如何在网络中路由的，[流动性](https://planb.academy/resources/glossary/liquidity-lightning)如何在通道之间移动，以及交易费用是如何计算的。

### 支付通道网络

在闪电网络上，一笔交易对应于两个节点之间的资金转移。正如前几章所见，进行闪电交易需要与某人开设一个通道。这个通道允许在关闭它以回收链上余额之前进行几乎无限数量的[链下](https://planb.academy/resources/glossary/offchain)交易。然而，这种方法的缺点是需要与另一方直接开设通道才能接收或发送资金，这意味着每个通道都需要一个开启交易和一个关闭交易。如果我计划与这个人进行大量支付，开启和关闭通道变得划算。相反，如果我只需要执行几次闪电交易，直接开启通道就不太有优势，因为这将花费我2次链上交易，而链下交易的数量有限。例如，当想要在商家处使用闪电支付而没有计划返回时，可能会发生这种情况。
...
**创建秘密**：Bob生成一个随机的秘密，记为*s*（原像），并使用[哈希函数](https://planb.academy/resources/glossary/hash-function)（记为*h*）计算其哈希值，记为*r*。我们有：
...
为了保持他们的网络地图最新，节点通过一种称为“**_[gossip](https://planb.academy/resources/glossary/gossip)_**”的算法定期交换消息。这是一种分布式算法，用于以流行病方式将信息传播到网络中的所有节点，从而允许在几个通信周期内交换和同步通道的全球状态。每个节点将信息传播给一个或多个随机或非随机选择的邻居，这些邻居又将信息传播给其他邻居，依此类推，直到达到全球同步状态。
...
- `Alice → 1 → 2 → 5 → Bob`，因为这是最短且容量最大的路由。
...
在本章中，我们将更仔细地研究闪电网络**发票**的操作，即接收节点发送给发送节点的支付请求。目标是理解如何在闪电网络上支付和接收支付。我们还将讨论两种替代传统发票的方法：[LNURL](https://planb.academy/resources/glossary/lnurl)和Keysend。
...
然后，发票以**[bech32](https://planb.academy/resources/glossary/bech32-and-bech32m)**编码，与比特币[SegWit](https://planb.academy/resources/glossary/segwit)地址（以`bc1`开头的格式）相同。
...
- **付款人（The Payer）**：这是进行支付的人。他们需要[外向流动性](https://planb.academy/resources/glossary/outbound-capacity)才能将资金转移给其他用户。例如，这可以是一个消费者。
- **卖家（或收款人，The Seller or Payee）**：这是接收支付的人。他们需要[进入流动性](https://planb.academy/resources/glossary/inbound-capacity)才能接受对他们节点的支付。例如，这可以是一个商家或在线商店。
...
![LNP201](assets/en/076.webp) 2. **通道中的交易**：在这个通道中，然后可以进行多次交易而无需将它们发布到区块链上。每一笔闪电交易都会创建通道的一个新状态，反映在承诺交易中。
