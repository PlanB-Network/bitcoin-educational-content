...
Il Satochip è un Hardware Wallet in formato smart card, con un elemento di sicurezza certificato EAL6+, uno dei più alti standard di sicurezza. È progettato e prodotto dall'omonima azienda belga: Satochip.
...
Per iniziare, aprire il menu `Strumenti > Strumenti Smartcard` sul SeedSigner.
...
La strategia di backup scelta dipende dalle vostre preferenze, ma è indispensabile avere almeno una copia della vostra frase Mnemonic, sia su supporto fisico (carta o metallo) sia, come in questo caso, in un Seedkeeper. È inoltre possibile moltiplicare il numero di backup in base alle esigenze. Per ulteriori informazioni sulle strategie di backup del portafoglio, vi suggerisco di leggere questo tutorial:



https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

Per eseguire il backup del seed su Seedkeeper, andare direttamente al menu `Backup seed`.
...
Scegliere una `etichetta` per identificare facilmente i diversi segreti memorizzati su Seedkeeper. Si può, ad esempio, mantenere semplicemente l'impronta digitale del Wallet o indicare esplicitamente `seed`. La scelta dipende dalle vostre preferenze e dai vostri rischi.
...
Il codice PIN di Seedkeeper deve essere il più lungo e casuale possibile, per evitare tentativi di forza bruta in caso di compromissione fisica della carta. È inoltre necessario conservare una copia di backup di questo codice PIN, memorizzata in un luogo separato da Seedkeeper. Senza questo PIN, non sarà possibile accedere al Mnemonic memorizzato in Seedkeeper e i bitcoin andranno persi per sempre.



### 5.3. Salvare il seed sul Satochip



Ora che il portafoglio è stato generato, salvato e controllato, lo trasferiremo a Satochip. A tale scopo, assicurarsi che il seed sia caricato nella RAM del SeedSigner. Quindi andare su `Strumenti > Strumenti Smartcard > Funzioni Satochip`.
...
È quindi possibile impostare il codice PIN Satochip, da 4 a 16 caratteri. Per rafforzare la sicurezza del vostro Wallet, scegliete un codice lungo e casuale: è l'unica protezione contro l'accesso fisico alla vostra frase Mnemonic.



Ricordate di salvare il PIN appena creato, in un gestore di password sicuro o su un supporto fisico, a seconda della vostra strategia personale. In quest'ultimo caso, assicuratevi di non conservare mai il supporto contenente il PIN nello stesso luogo in cui si trova il vostro Satochip, altrimenti la protezione diventerà inutile. È importante avere una copia di backup: **Senza questo PIN, non sarà più possibile accedere al proprio seed e i bitcoin andranno definitivamente persi**.
...
Il seed è ora importato in Satochip.
...
Ora che il vostro portafoglio è attivo e funzionante, importeremo le sue informazioni pubbliche (il "*keystore*") in Sparrow wallet o in un altro software di gestione del portafoglio. Questo software verrà utilizzato per creare, distribuire e tenere traccia delle transazioni. Tuttavia, non sarà in grado di firmarle, poiché solo Satochip (ed eventuali backup) detiene le chiavi private necessarie per questa operazione.
...
Inserire la scheda microSD contenente il sistema operativo, quindi accendere il SeedSigner. Per il momento non può fare nulla, poiché non conosce ancora il vostro seed. Dovrete iniziare inserendo il Satochip nel lettore di smart card, poiché è quello che contiene il vostro seed. Dovrete iniziare inserendo lo Satochip nel lettore di smart card, poiché è quello che contiene il vostro seed.
...
Si passa poi alla scelta dello standard di scripting. Scegliere il più recente: `Native SegWit`.
...
Assicuratevi che il software Sparrow wallet sia installato sul vostro computer. Se non sapete come scaricarlo, verificarne l'autenticità e installarlo correttamente, consultate il nostro tutorial completo sull'argomento:
...
Assegnare un nome al portafoglio, quindi fare clic su "Crea Wallet". Sparrow chiederà quindi di impostare una password per bloccare l'accesso locale a questo Wallet. Scegliete una password forte: protegge i vostri dati nel Sparrow (chiavi pubbliche, indirizzi, etichette e cronologia delle transazioni). Tuttavia, questa password non è necessaria per ripristinare il Wallet in futuro: lo sarà solo la frase del Mnemonic (ed eventualmente del passphrase).
...
Il SeedSigner chiederà quindi di scansionare un Address ricevente a caso dal proprio Sparrow wallet per confermare la validità dell'importazione.
...
Il vostro Satochip (tramite SeedSigner) e il Sparrow wallet sono ora collegati in modo sicuro. Il Sparrow funge da gestione completa del Interface, mentre lo Satochip rimane l'unico dispositivo in grado di firmare le transazioni. Ora siete pronti a ricevere e inviare bitcoin in una configurazione totalmente air-gapped.
...
Sul computer, aprire il Sparrow wallet e sbloccare il `Satochip-SeedSigner` Wallet utilizzando la password. Verificare che il software sia connesso a un server (indicatore in basso a destra). Quindi, nella barra laterale, fare clic su "Ricevi".
...
Appare un nuovo Bitcoin Address. Troverete :




- Il Address in formato testo (iniziando con `bc1q...` se si utilizza il P2WPKH, come in questo esempio) ;
- Il codice QR associato ;
- Un campo `Label`, utile per tracciare le transazioni.



Vi consiglio vivamente di aggiungere un'etichetta a ogni ricevuta Bitcoin del vostro Wallet. Questo vi aiuterà a identificare facilmente la provenienza di ogni UTXO e a gestire meglio la vostra privacy. Per saperne di più su questo importante argomento, consultate la formazione dedicata su Plan ₿ Academy :
...
Prima di comunicare il Address ricevuto all'ordinante, è importante verificare che esso appartenga al seed. Questo passo garantisce che il Satochip sia in grado di firmare le transazioni associate a questo Address. Questa fase garantisce che il vostro Satochip sia in grado di firmare le transazioni associate a questo Address. Inoltre, previene potenziali attacchi in cui il Sparrow mostrerebbe un Address fraudolento. Tenete presente che Sparrow funziona in un ambiente insicuro (il vostro computer), la cui superficie di attacco è di gran lunga superiore a quella del vostro Satochip, che è totalmente isolato. Per questo motivo non bisogna mai fidarsi ciecamente degli indirizzi visualizzati nel Sparrow prima di averli verificati sul proprio Hardware Wallet.



Nel Sparrow, fare clic sul codice QR del Address per ingrandirlo: verrà quindi visualizzato a schermo intero.
...
Quindi confermare il tipo di script utilizzato (in questo caso, `Native SegWit`), inserire il codice PIN Satochip per sbloccarlo e convalidare le informazioni `xpub`.
...
Se il Address scansionato corrisponde a quello derivato dal seed, il SeedSigner visualizzerà il messaggio: `Address verificato`.
...
Potrete così essere certi che il Address faccia parte del vostro portafoglio.
...
È ora possibile trasmettere questo Address in forma di testo o tramite il suo codice QR alla persona o al servizio che deve inviarvi Satss. Una volta che la transazione è stata trasmessa in rete, apparirà nella scheda "Transazioni" di Sparrow wallet.
...
- Firma di questa transazione sul Satochip, tramite il SeedSigner ;
- Infine, la transazione viene trasmessa in rete dal Sparrow.
...
- In `Pagare a`, incollare il Address del destinatario o scansionare il suo codice QR utilizzando l'icona della fotocamera;
- In `Etichetta`, aggiungere un'etichetta per tracciare questa spesa;
- In "Importo", inserire l'importo da inviare;
- Infine, selezionare la velocità di carica in base alle condizioni attuali della rete (le stime sono disponibili su [Mempool.space](https://Mempool.space/)).
...
Accendere SeedSigner e inserire Satochip come di consueto. Dalla schermata iniziale, selezionare `Scan`, quindi scansionare il codice QR visualizzato su Sparrow.
...
Il SeedSigner rileva che si tratta di un PSBT e visualizza un riepilogo della transazione:
...
Ora che la transazione è firmata e convalidata, non resta che trasmetterla sulla rete Bitcoin in modo che un Miner possa includerla in un BLOCK. Nel Sparrow, fare clic su "Scansione QR".
...
Presentare il codice QR visualizzato sul SeedSigner (quello contenente la transazione firmata) alla webcam. Sparrow visualizzerà tutti i dettagli della transazione. Verificare che tutte le informazioni siano corrette, quindi fare clic su "Trasmetti transazione" per trasmetterla alla rete Bitcoin.
...
- Registrando la frase del Mnemonic su un supporto fisico;
...
### 8.1. Recuperare il proprio Wallet con Satochip



Se avete ancora il vostro Satochip ma il vostro SeedSigner è rotto o è andato perso, la situazione è abbastanza semplice da gestire, poiché il vostro Wallet è ancora nel Satochip.
...
Se non volete ricostruirne uno, potete anche usare il vostro Satochip nel modo classico, cioè direttamente dal vostro computer, senza passare per il SeedSigner. Questo metodo funziona perfettamente, ma riduce notevolmente la sicurezza del vostro Bitcoin Wallet: perdete l'isolamento "*air-gapped*" e ora dovete firmare alla cieca, poiché il SeedSigner fungeva da schermo affidabile. Tuttavia, questa può essere una soluzione temporanea in caso di emergenza o se non si è in grado di ricostruire un SeedSigner.



A tale scopo, è necessario un lettore di smart card USB o NFC. Aprire il Wallet che si desidera ripristinare nel Sparrow, quindi andare alla scheda "Impostazioni" e fare clic su "Sostituisci".
...
- Oppure caricare il seed da un Seedkeeper, tramite il menu `Seeds > From SeedKeeper` (questo è il metodo che sto usando in questa guida). Sarà sufficiente inserire il codice PIN del Seedkeeper e selezionare il segreto da utilizzare come seed sul SeedSigner.
