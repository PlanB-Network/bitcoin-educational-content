...
Прежде чем объяснять этот процесс, я считаю важным напомнить об индексах, с которыми мы в настоящее время работаем. Путь производной платежного кода описывается следующим образом: `m/47'/0'/0'`. Следующая глубина распределяет индексы таким образом:
