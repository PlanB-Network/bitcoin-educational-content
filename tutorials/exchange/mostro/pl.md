---
name: Mostro
description: Wolna od KYC platforma Bitcoin P2P Exchange za pośrednictwem Lightning i Nostr
---
...
**Mostro P2P** oferuje zdecentralizowaną alternatywę łączącą Lightning Network, protokół Nostr i przechowywanie faktur. Jego główną innowacją jest zautomatyzowany system depozytowy, w którym bitcoiny pozostają pod kontrolą użytkownika przez cały Exchange, eliminując ryzyko kradzieży, bankructwa Exchange lub arbitralnej konfiskaty.
...
- daemon Mostrod**: koordynuje wymianę między użytkownikami a Lightning Network
...
**Poufność w założeniu**: Dwa tryby dostępne w zależności od potrzeb. Tryb reputacji** łączy twoją reputację z kluczem Nostr, aby budować trwałe zaufanie. Tryb w pełni prywatny** zapewnia całkowitą anonimowość dzięki jednorazowym kluczom efemerycznym.
...
**Elastyczność płatności**: Obsługa wielu walut fiducjarnych za pośrednictwem usługi kursu yadio.io Exchange. Akceptuje przelewy SEPA, PayPal, Revolut, gotówkę i wszelkie metody uzgodnione między stronami.
...
Mostro CLI** jest najbardziej dojrzałym i funkcjonalnym klientem. Napisany w Rust, oferuje pełen zakres funkcji Mostro za pośrednictwem linii poleceń Interface. Kompatybilny z systemami macOS, Linux i Windows.
...
- `release`: Zwolnij depozyt i sfinalizuj Exchange
- `getdm`: Wyświetl bezpośrednie wiadomości
- `rate` : Oceń swojego kontrahenta po Exchange
...
Interface zaawansowana aplikacja internetowa JavaScript w fazie aktywnego rozwoju. Zaprojektowana tak, aby oferować pełne doświadczenie użytkownika z rozbudowanymi funkcjami handlowymi i zarządzania portfelem. Dostęp przez przeglądarkę bez konieczności instalacji.
...
- Opcjonalne, ale zalecane**: Połączenie VPN lub Tor w celu zamaskowania adresu IP Address
...
**Krok 3**: Postępuj zgodnie z ekranami onboardingu, które prezentują funkcjonalności:
...
- Małe kwoty**: Zacznij od mniej niż Sats 10,000 na początek
- Wiele przekaźników**: Konfiguracja 3-5 geograficznie zróżnicowanych przekaźników
- Ochrona sieci**: Aktywuj VPN lub Tor, aby ukryć swoje IP Address
- Wallet secure**: Aktywuj automatyczną blokadę na Wallet Lightning
...
W tej sekcji zademonstrowano zakup bitcoinów za pośrednictwem Mostro CLI na podstawie rzeczywistego przypadku użycia.
...
Mostro poprosi o podanie **Lightning Invoice**, aby otrzymać BTC. Dokładna kwota w satoshis zostanie podana (tutaj: 4715 Sats).
...
### Krok 3: Dostarczenie urządzenia Lightning Invoice



generate a Lightning Invoice z Wallet (Phoenix, Breez itp.) za dokładną kwotę, a następnie wyślij ją :
...
System potwierdza wysyłkę i informuje, że przed aktywacją escrow należy poczekać, aż sprzedawca zapłaci Invoice hold.
...
Gdy tylko sprzedawca potwierdzi otrzymanie fiat i zwolni depozyt za pomocą polecenia `release`, natychmiast otrzymasz swoje bitcoiny na dostarczonym przez ciebie Lightning Invoice.
...
Naciśnij przycisk Green **"+"** w prawym dolnym rogu, a następnie wybierz **Sprzedaż** (czerwony przycisk).
...
**Ważna instrukcja**: Musisz teraz **opłacić wyświetlony Invoice hold**, aby zablokować swoje bitcoiny (tutaj 4674 Sats za 1-5 EUR) w escrow. Masz **maksymalnie 15 minut**, w przeciwnym razie Exchange zostanie anulowany.



Skopiuj blokadę Invoice i opłać ją z Wallet Lightning (Phoenix, Breez itp.).
...
Po **potwierdzeniu otrzymania** płatności fiat na swoim koncie, naciśnij przycisk Green **RELEASE** i potwierdź wysłanie Satss do kupującego.
...
Bitcoiny są natychmiast przesyłane do kupującego za pośrednictwem jego Lightning Invoice.
...
4. Podaj swój piorun Invoice (wygenerowany z Wallet)
...
9. Natychmiastowe otrzymywanie bitcoinów na Lightning Invoice
...
**Suwerenność finansowa**: Twoje bitcoiny nigdy nie opuszczają Twojej bezpośredniej kontroli dzięki mechanizmowi Invoice hold, eliminując ryzyko bankructwa Exchange lub piractwa.



**Odporny na cenzurę**: Żadna władza nie może wyłączyć sieci ani cenzurować poleceń użytkownika. System działa tak długo, jak długo działają przekaźniki Nostr.
...
**Elastyczność płatności**: Możliwa jest każda wzajemnie akceptowana metoda płatności (przelewy, aplikacje mobilne, gotówka, barter).
