...
قبل از راه‌اندازی Raspberry Pi با حافظه فرمت شده، باید آن را در یک کامپیوتر قرار دهیم و دو فایل ایجاد کنیم که به ما اجازه می‌دهند به صورت از راه دور متصل شویم. با استفاده از دستور `touch`، یک فایل خالی در پارتیشن /boot ایجاد می‌کنیم که اتصال SSH را در اولین بوت کارت SD تازه فرمت شده فعال می‌کند.
...
با استفاده از دستور nano فایل `wpa_supplicant.conf` را ایجاد کرده و مستقیماً شروع به ویرایش آن می‌کنیم. در این فایل، باید تنظیمات وای‌فای را کپی کنیم، متن بین START و END را کپی کرده و SSID و رمز عبور وای‌فای که می‌خواهید به آن متصل شوید را تغییر دهید.
...
سپس، کارت SD را در Raspberry Pi قرار می‌دهیم و Pi را به منبع تغذیه متصل می‌کنیم تا سیستم‌عامل راه‌اندازی شود. باید آن را در شبکه شناسایی کنیم و پروتکل mDNS احتمالاً نام raspberrypi.local را به آن اختصاص می‌دهد. بیایید سعی کنیم از طریق SSH متصل شویم.
...
اگر کار نمی‌کند، باید شبکه را پیدا کنیم. بیایید بفهمیم به IP Address که متصل هستیم، کدام است.
...
به عنوان مثال، اگر 192.168.0.0 باشد، از nmap برای پیدا کردن Pi استفاده کنید.
...
#### 12. فایل پیکربندی LND را ایجاد کنید.
...
اکنون می‌توانید btc را به Address ارسال کنید که توسط LND بازگردانده شده است.
...
برای پرداخت یک Invoice رعد و برق:
...
برای مشاهده اطلاعات درباره گره من:
