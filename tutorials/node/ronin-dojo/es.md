...
description: Instalar y utilizar tu propio nodo Bitcoin RoninDojo.
---
...
_Estamos siguiendo de cerca la evolución de este caso así como los desarrollos relacionados con las herramientas asociadas. Ten la seguridad de que actualizaremos este tutorial a medida que esté disponible nueva información._
...
Ejecutar y utilizar tu propio nodo es esencial para participar realmente en la red de Bitcoin. Aunque ejecutar un nodo Bitcoin no proporciona ninguna ventaja económica al usuario, le permite preservar su privacidad, actuar de manera independiente y tener control sobre su confianza en la red.

En este artículo, examinaremos en detalle RoninDojo, una excelente solución para ejecutar tu propio nodo Bitcoin.
...
Si no estás familiarizado con el funcionamiento y el papel de un nodo Bitcoin, te recomendamos que comiences leyendo este artículo: El nodo Bitcoin - Parte 1/2: Conceptos técnicos.

![Samourai] (assets/1.webp)

## ¿Qué es RoninDojo?

Dojo es un servidor de nodo completo de Bitcoin desarrollado por el equipo de Samouraï Wallet. Puedes instalarlo libremente en cualquier máquina.
...
El código de RoninDojo es de código abierto, por lo que también es posible instalar esta solución en tu propio hardware. Esta opción es económica, pero requiere un poco más de manipulación, que es lo que haremos en este artículo.

RoninDojo es un Dojo, por lo que permite integrar fácilmente Whirlpool CLI en tu nodo Bitcoin para tener la mejor experiencia posible de CoinJoin. Con Whirlpool CLI, no solo podrás mezclar tu Bitcoin las 24 horas del día, los 7 días de la semana sin tener que dejar encendida tu computadora personal, sino que también podrás mejorar significativamente tu privacidad.

RoninDojo integra muchas otras herramientas que se basarán en tu Dojo, como el calculador Boltzmann que permite determinar el grado de privacidad de una transacción, el servidor Electrum para conectar tus diferentes billeteras Bitcoin a tu nodo, o el servidor Mempool para observar tus transacciones de forma privada.
En comparación con otra solución de nodo como Umbrel, que presentamos en este artículo, RoninDojo se enmarca en una línea de desarrollo profundamente orientada hacia soluciones "On Chain" y herramientas que optimizan la privacidad de los usuarios. Por lo tanto, RoninDojo no permite interactuar con la Lightning Network.
...
Descarga la imagen de disco que corresponda a tu hardware. En este caso, descargamos la imagen "MANJARO-ARM-RONINOS-RPI4-22.03.IMG.XZ":

![Descargar imagen de disco RoninOS] (assets/2.webp)

Una vez descargada la imagen, verifica su integridad utilizando el archivo .SHA256 correspondiente. Te explicamos detalladamente cómo hacer esto en este artículo: ¿Cómo verificar la integridad de un software de Bitcoin en Windows?
...
Deja que la instalación se realice tranquilamente. En nuestro caso, esto tomó aproximadamente 2 horas. Mantén tu terminal abierto durante la operación.
...
Si deseas cambiar la contraseña de usuario por una más segura, puedes hacerlo ahora desde la pestaña "Configuración". En RoninDojo, no hay una capa de seguridad adicional, así que te recomendamos que elijas una contraseña realmente fuerte y que cuides su respaldo.
...
La primera utilidad de tu nodo recién instalado y sincronizado será difundir sus transacciones a la red de Bitcoin. Por lo tanto, seguramente querrás conectar tus diferentes software de gestión de carteras.
Puede hacer esto a través de Electrum Rust Server (electrs). La aplicación normalmente viene preinstalada en el nodo RoninDojo. Si no es así, puedes instalarla manualmente desde la interfaz RoninCLI.

Simplemente ve a:

> Aplicaciones > Administrar aplicaciones > Instalar Electrum Server

Para obtener la dirección Tor de tu Servidor Electrum, desde el menú RoninCLI, ve a:

> Credenciales > Electrs

Solo tendrás que ingresar el enlace .onion en el software de tu billeteraa. Por ejemplo, en Sparrow Wallet, simplemente ve a la pestaña:

> Archivo > Preferencias > Servidor

En el tipo de servidor, selecciona "Private Electrum", luego ingresa la dirección Tor de tu Servidor Electrum en el campo correspondiente. Finalmente, haz clic en "Probar conexión" para probar y guardar tu conexión.

![Interfaz de conexión de Sparrow Wallet a un electrs] (assets/23.webp)

### Conectar tu software de billeteras a Samourai Dojo.

En lugar de usar Electrs, también puedes usar Samourai Dojo para conectar tu billetera de software compatible al nodo RoninDojo. Por ejemplo, Samourai Wallet ofrece esta opción.

Para hacer esto, simplemente escanea el código QR de conexión de tu Dojo. Para acceder a él desde RoninUI, haz clic en la pestaña "Panel de control" y luego en el botón "Administrar" en el cuadro de su Dojo. Luego podrás ver los códigos QR de conexión a tu Dojo y a BTC-RPC Explorer. Para mostrarlos en texto claro, haz clic en "Mostrar valores".

![Obtener el código QR de conexión al Dojo] (assets/24.webp)

Para conectar tu cartera Samourai Wallet a tu Dojo, deberás escanear este código QR durante la instalación de la aplicación:

![Conexión a Dojo desde la aplicación Samourai Wallet] (assets/25.webp)

### Utilizar tu propio explorador de Mempool.

Herramienta esencial para los usuarios de Bitcoin, el explorador permite verificar diferente información sobre la cadena de bloques de Bitcoin. Con Mempool, por ejemplo, podrás verificar en tiempo real las tarifas aplicadas por otros usuarios para ajustar las tuyas. También puedes verificar el estado de confirmación de tus transacciones o ver el saldo de una dirección.

Estas herramientas de explorador pueden exponerte a riesgos de pérdida de privacidad y te obligan a confiar en la base de datos de un tercero. Cuando utilizas esta herramienta en línea sin pasar por tu propio nodo:

- Corres el riesgo de filtrar información sobre tu cartera.

- Confías en el administrador del sitio web para la cadena de prueba de trabajo que aloja.
...
Simplemente ingresa tu dirección Mempool Tor en el navegador Tor para disfrutar de tu propia instancia de Mempool, basada en tus propios datos. Te recomendamos agregar esta dirección Tor a tus favoritos para poder acceder a ella más rápidamente. También puedes crear un acceso directo en tu escritorio.
...
WhirlpoolCLI se utiliza junto con WhirlpoolGUI, una interfaz gráfica que se instala en una computadora personal para facilitar la gestión de Coinjoins. En este artículo, explicaremos en detalle cómo configurar Whirlpool CLI en tu propio dojo: https://www.pandul.fr/post/comprendre-et-utiliser-le-coinjoin-sur-bitcoin#:~:text=dans%20cette%20partie.-,Tutoriel%20%3A%20Whirpool%20CLI%20sur%20Dojo%20et%20Whirlpool%20GUI.,-Si%20vous%20souhaitez
Para obtener más información sobre Coinjoin en general, lo explicamos todo en este artículo: https://www.pandul.fr/post/comprendre-et-utiliser-le-coinjoin-sur-bitcoin

### Usar Whirlpool Stat Tool.

Después de realizar CoinJoins con Whirlpool, es posible que quieras saber concretamente cuál es el nivel de privacidad de sus UTXO mezclados. Whirlpool Stat Tool te permite hacer esto fácilmente. Con esta herramienta, podrás calcular la puntuación prospectiva y la puntuación retrospectiva de sus UTXO mezclados. Para obtener más información sobre el cálculo de estos conjuntos anónimos y cómo funcionan, recomendamos que leas esta parte: https://www.pandul.fr/post/comprendre-et-utiliser-le-coinjoin-sur-bitcoin#:~:text=perdre%20en%20confidentialit%C3%A9.-,Anon%20Sets.,-Comme%20expliqu%C3%A9%20pr%C3%A9c%C3%A9demment

La herramienta está preinstalada en tu RoninDojo. Por el momento, solo está disponible desde RoninCLI. Para iniciarlo desde el menú principal, ve a:

> Samourai Toolkit > Whirlpool Stat Tool

Las instrucciones de uso se mostrarán. Una vez que haya terminado, presiona cualquier tecla para acceder a las líneas de comando:

![Página de inicio de Whirlpool Stats Tool] (assets/28.webp)

Verás que se muestra el terminal:

> wst#/tmp>

Para salir de esta interfaz y volver al menú RoninCLI, simplemente escribe el comando:

> quit

Para comenzar, vamos a configurar el proxy en Tor para poder extraer los datos de OXT de manera confidencial. Escribe el comando:

> socks5 127.0.0.1:9050

Luego, descarga los datos del pool que contiene tu transacción:

> download 0001
>
> Reemplaza 0001 con el código de denominación del pool que te interese.
...
Una vez que se hayan descargado los datos, cargalos con el comando:
...
Deja que se complete la carga, esto puede llevar unos minutos. Después de cargar los datos, escribe el comando "score" seguido de tu TXID (identificador de transacción) para obtener tus conjuntos Anon:
...
WST mostrará primero la puntuación retrospectiva (métricas retrospectivas) y luego la puntuación prospectiva (métricas prospectivas). Además de las puntuaciones de los conjuntos Anon, WST también proporcionará la tasa de difusión de tu salida en el pool en función del conjunto Anon.

Ten en cuenta que la puntuación prospectiva de su UTXO se calcula a partir de la TXID de su mezcla inicial, no de su última mezcla. Por otro lado, la puntuación retrospectiva de un UTXO se calcula a partir de la TXID del último ciclo.

Una vez más, si no entiendes estos conceptos de conjuntos Anon, te recomendamos que leas esta parte de nuestro artículo sobre Coinjoin en el que lo explicamos todo en detalle con diagramas: https://www.pandul.fr/post/comprendre-et-utiliser-le-coinjoin-sur-bitcoin#:~:text=perdre%20en%20confidentialit%C3%A9.-,Anon%20Sets.,-Comme%20expliqu%C3%A9%20pr%C3%A9c%C3%A9demment

### Utilizando la calculadora Boltzmann.

La calculadora Boltzmann es una herramienta que te permite calcular fácilmente diversas métricas avanzadas sobre una transacción de Bitcoin, incluido su nivel de entropía. Todos estos datos permitirán poner números al nivel de confidencialidad de una transacción y detectar posibles errores. Esta herramienta está preinstalada en tu nodo RoninDojo.

Para acceder a ella desde RoninCLI, conéctate a través de SSH y ve al menú:

> Samourai Toolkit > Boltzmann Calculator

Antes de explicar cómo usarlo en RoninDojo, explicaremos qué representan estas métricas, cómo se calculan y para qué sirven.
...
> Si no estás familiarizado con el término "UTXO", te recomendamos que leas este breve artículo: Mecanismo de una transacción de Bitcoin: UTXO, inputs y outputs.
...
Ahora que hemos estudiado los indicadores, veamos cómo calcularlos utilizando este software. Desde RoninCLI, ve al menú:

> Samourai Toolkit > Boltzmann Calculator

![Página de inicio del software Boltzmann Calculator] (assets/35.webp)

Una vez que se haya iniciado el software, ingresa el identificador de la transacción que deseas estudiar. Puedes ingresar varias transacciones separándolas con una coma y luego presionar Enter:

![Ingresar un TXID para estudiar en Boltzmann Calculator] (assets/36.webp)

El calculador te devolverá todos los indicadores que hemos visto anteriormente:

![Impresión de los datos de Boltzmann Calculator para este TXID] (assets/37.webp)

Escribe el comando "Quit" para salir del software y volver al menú de RoninCLI.

Para obtener más información sobre el calculador Boltzmann, te recomeendamos que leas estos artículos:
...
Bisq es una plataforma de intercambio que permite realizar compras y ventas de Bitcoin de igual a igual. Se utiliza con un software de escritorio que se ejecuta a través de Tor y que permite intercambiar Bitcoin sin necesidad de proporcionar tu identidad.
...
Para familiarizarte con el software, te recomeendamos que leas esta página: https://bisq.network/getting-started/
...
Por ejemplo, a veces puede suceder que tu billetera conectada a tu RoninDojo no encuentre el Bitcoin que te pertenece. El saldo está en 0 aunque estés seguro de que tienes Bitcoin en esa billetera. Hay muchas posibles causas a considerar, como un error en las rutas de derivación, y una de ellas es que tu nodo no esté observando tus direcciones.
...
Verifica que todas las transacciones aparezcan correctamente. También verifica que el tipo de derivación coincida con el de tu billetera. Aquí podemos ver que el nodo interpreta esta xpub como una derivación BIP44. Si este tipo de derivación no coincide con el de tu billetera, haz clic en el botón "Retype" y selecciona BIP44/BIP49/BIP84 según tu elección:

![Cambiar el tipo de derivación de la xpub estudiada desde RoninUI] (assets/43.webp)

Si tu xpub no está rastreada por tu nodo, verás aparecer esta pantalla que te invita a importarla:

![Importar una xpub con la herramienta XPUB Tool en RoninUI] (assets/44.webp)

También puedes utilizar otras herramientas de mantenimiento:

- Transaction Tool: Te permite observar los detalles de una transacción específica.

- Address Tool: Permite verificar que una dirección específica esté siendo rastreada por tu Dojo.

- Rescan Blocks: Obliga a tu nodo a volver a escanear un rango de bloques seleccionado.

También encontrarás en RoninUI la herramienta "Push Tx". Te permite difundir una transacción firmada a la red de Bitcoin. Esta debe ingresarse en formato hexadecimal:

![Herramienta de difusión de transacciones firmadas desde RoninUI] (assets/45.webp)

## Conclusión.

Hemos podido ver cómo instalar y familiarizarnos con esta magnífica herramienta que es RoninDojo. Es una excelente opción para ejecutar tu propio nodo de Bitcoin. Es una solución estable que integra y mantiene actualizadas todas las herramientas esenciales para un Bitcoiner.
...
Para obtener más información sobre RoninDojo, te recomendamos que consultes los enlaces en mis recursos externos a continuación.
