...
비트코인 블록체인은 가능한 한 많은 사람이 허가 없이, 그리고 신뢰에 의존하지 않고 검증할 수 있도록 설계되었습니다. 이러한 요구사항은 구조적인 한계를 수반합니다. 즉, 블록은 독립적인 노드가 과도하게 비싼 하드웨어에 투자하지 않고도 다운로드, 저장, 검증할 수 있을 만큼 합리적인 크기를 유지해야 하므로, 블록체인은 무제한의 거래를 처리할 수 없습니다. 이는 흔히 트릴레마로 요약됩니다: 탈중앙화, 보안, 확장성. 블록체인을 기반으로 한 시스템은 이 세 가지 속성을 동시에 극대화할 수 없습니다. 비트코인은 탈중앙화와 보안을 우선시하며, 그 결과 온체인에서 처리 가능한 거래 처리량은 자연스럽게 제한됩니다.
...
라이트닝은 양방향 결제 채널에 기반합니다. 채널이란 두 참여자 간의 기술적 관계로, 각 결제를 블록체인에 기록하지 않고도 오프체인에서 사토시(sats)를 교환할 수 있게 해줍니다.
...
라이트닝은 중개자를 신뢰하지 않고도 결제를 전달할 수 있도록 설계되었습니다. 이를 위해 프로토콜은 HTLC(Hashed Time-Locked Contracts)라 불리는 스마트 컨트랙트를 사용합니다. 모든 세부 사항을 다루지는 않지만, 일반적인 메커니즘은 다음과 같습니다.
...
이 장에서는 비트코인 위에서 각각 새로운 가능성을 제공하는 네 가지 중요한 프로토콜을 소개합니다. 사이드체인(특히 Liquid), Ark, RGB, 그리고 Taproot Assets입니다.
...
이 세컨드 레이어 프로토콜의 개념은 2023년 5월 Burak에 의해 공개되었습니다. 라이트닝 네트워크와 마찬가지로 Ark는 비트코인 메인 체인 위에 배포되는 시스템입니다. 이는 빠르고, 익명적이며, 저렴한 비용으로 오프체인 비트코인 결제를 가능하게 합니다. 라이트닝과 비교했을 때, Ark는 결제를 받기 위해 인바운드 유동성을 필요로 하지 않아 사용자 경험을 크게 개선합니다. 또한 코인조인 거래에 가까운 프라이버시를 제공합니다. 더 나아가 비트코인에 커버넌트가 추가된다면, Ark는 비대화형(non-interactive)으로도 작동할 수 있습니다.
