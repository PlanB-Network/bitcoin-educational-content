---
name: Jahe Wallet
description: Perangkat lunak Bitcoin Wallet sumber terbuka dan mandiri, Fork dari Wasabi Wallet, yang mengintegrasikan Coinjoins
---
...
CoinJoin** adalah jenis tertentu dari struktur transaksi Bitcoin yang menyatukan beberapa peserta dalam satu transaksi kolaboratif. Mekanisme ini menggabungkan entri dari beberapa pengguna yang berbeda ke dalam sebuah transaksi bersama, sehingga sangat sulit - bahkan tidak mungkin dilakukan dengan benar - untuk melacak dana. Sebagai hasilnya, hampir tidak mungkin bagi pengamat luar untuk mengidentifikasi secara pasti asal dan tujuan bitcoin yang terlibat, tidak seperti pada transaksi Bitcoin konvensional.



Bagi Anda, pengguna, CoinJoin membantu menjaga kerahasiaan Anda. Sebagai contoh, jika Anda menerima donasi sebesar 10.000 Sats pada Bitcoin Address, pengirim dapat melacak dana ini dan, dalam beberapa kasus, menyimpulkan bahwa Anda memiliki jumlah bitcoin yang lebih besar, atau mengamati aktivitas Anda. Dengan melakukan CoinJoin setelah donasi 10.000 Sats ini, Anda mematahkan pelacakan: pengirim tidak lagi dapat memperoleh informasi apa pun tentang Anda dari pembayaran ini.



Chaumian CoinJoin menawarkan tingkat keamanan yang tinggi, karena dana tetap berada di bawah kendali eksklusif pengguna setiap saat. Bahkan operator dari server koordinator tidak dapat mengalihkan bitcoin peserta dalam keadaan apa pun. Baik pengguna maupun koordinator tidak perlu saling mempercayai: masing-masing tetap memegang kendali atas kunci pribadinya, dan hanya memiliki wewenang untuk memvalidasi transaksi. Oleh karena itu, tidak ada pihak ketiga yang dapat mengambil bitcoin Anda selama CoinJoin, atau membuat hubungan langsung antara input dan output Anda.
...
Ginger secara otomatis mengaktifkan **CoinJoin** default ketika Anda membuat portofolio. Anda akan diberitahu mengenai hal ini dan kemudian dapat menyesuaikan pengaturan agar sesuai dengan kebutuhan Anda.
...
Setelah Anda membuat portofolio pertama Anda, Anda akan dibawa ke Interface Wallet milik Ginger.
...
Ginger mendukung penggunaan portofolio perangkat keras melalui standar Bitcoin core Interface, meskipun integrasi langsung dari atau ke portofolio perangkat keras belum tersedia.
...
Pada tab ini, Anda dapat mengubah bahasa, mata uang, dan unit tampilan biaya (BTC/Satoshi), antara lain.
...
Tab ini memungkinkan Anda mengaktifkan Bitcoin Knots untuk berjalan ketika aplikasi dimulai, memilih jaringan Anda (Main/RegTest), dan penyedia tarif pengisian daya Anda (Mempool Space/BLOCKSTREAM info/Full node), dll.
...
- selama CoinJoin, koordinator dapat menghubungkan input dan output Anda ke IP Address;
- ketika menyiarkan transaksi, simpul berbahaya yang terhubung dengan Anda dapat mengaitkan transaksi Anda dengan IP Anda.



Jangan lupa untuk menekan **Done** (di bagian kanan bawah Coin) setiap kali, untuk menyimpan pengaturan Anda. Beberapa pengaturan mengharuskan Ginger Wallet dihidupkan ulang agar dapat diterapkan.
...
- CoinJoin** (di mana Anda dapat menyesuaikan pengaturan CoinJoin portofolio ini);
...
- Masukkan nama sumber yang ingin Anda kaitkan dengan Address. Ini adalah pelabelan untuk melacak pembayaran Anda. Ini tidak memiliki implikasi On-Chain; ini hanyalah informasi penelusuran yang disimpan secara lokal dalam aplikasi Anda;
...
- klik panah kecil di sebelah kiri **generate** untuk memilih format Address Anda (**SegWit** / **Taproot**), lalu klik **generate**, ke generate, Address, dan kode QR.
...
Address atau kode QR ini akan digunakan oleh pengirim untuk mengirimkan bitcoin kepada Anda.
...
Video tutorial tentang cara mengirim melalui Ginger Wallet.
...
- Tekan tombol **Kirim**;
- masukkan Address penerima, jumlah yang akan dikirim dan label;
...
Coin kanan atas layar dengan label **Semua** memungkinkan Anda untuk menyaring penawaran berdasarkan metode pembayaran. Metode pembayaran yang Anda pilih akan ditetapkan secara default, tetapi dapat diubah kapan saja.
...
- Sekarang Anda perlu menyelesaikan transaksi:
 ...
 - Pada titik tertentu, Anda akan menerima Address penerima dan jumlah yang tepat untuk dikirim;
 - Kemudian kembali ke Ginger Wallet untuk melanjutkan proses;
 - Setelah kembali ke Ginger Wallet, kotak dialog akan muncul, dan Anda dapat melanjutkan dengan mengklik **Kirim**.



Ini akan membuka layar **Kirim** dengan Address penerima dan jumlah yang telah diisi sebelumnya. Anda juga dapat menggunakan tombol **Kirim** pada layar beranda. Meskipun Anda dapat mengirim transaksi secara manual, kami sarankan Anda menyelesaikannya melalui kotak dialog untuk proses yang lebih optimal.



## Buatlah CoinJoin di atas Jahe Wallet
...
Lindungi kerahasiaan bitcoin Anda dengan **CoinJoin**, yang terintegrasi langsung ke dalam Ginger Wallet. Wallet menggunakan **WABISABI**, protokol Chaumian CoinJoin yang dirancang untuk memfasilitasi koin bersama yang lebih mudah diakses dan efisien.



Terserah Anda untuk memilih strategi CoinJoin (otomatis atau manual) yang paling cocok untuk Anda.



Ginger CoinJoin siap digunakan segera setelah Anda mengunduhnya (tidak perlu langkah tambahan). Secara otomatis, Ginger CoinJoin berjalan di latar belakang untuk melindungi privasi Anda pada setiap transaksi. Dalam praktiknya, pembaca CoinJoin akan muncul setiap kali Anda memiliki saldo yang dapat dianonimkan.



Untuk memulai CoinJoin secara manual, ini adalah operasi satu klik. Mulai putaran dan tunggu hingga transaksi CoinJoin dibuat dan dikonfirmasi. Anda akan melihat skor anonimisasi di Interface.
...
Ginger lebih memilih coinjoin dengan lebih banyak peserta daripada putaran yang lebih kecil dan lebih cepat. Coinjoin yang lebih besar menawarkan lebih banyak anonimitas, biaya yang lebih rendah, dan efisiensi ruang BLOCK yang lebih besar.
...
- Jika Anda kehilangan komputer atau mencurigai adanya akses yang tidak sah, segera buat Wallet baru. Transfer dana Anda ke portofolio baru ini dan hapus portofolio lama;
- Gunakan Address yang berbeda untuk setiap penerimaan untuk menghindari penggunaan ulang alamat;
...
Jika Anda merasa tutorial ini bermanfaat, silakan tinggalkan jempol Green di bawah ini. Jangan ragu untuk membagikan artikel ini melalui platform media sosial Anda. Terima kasih banyak!
