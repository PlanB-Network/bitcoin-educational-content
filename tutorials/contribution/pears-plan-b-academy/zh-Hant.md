...
Pears 是點對點應用程式的運行環境、開發工具和部署平台。這個開放原始碼的工具讓您可以在沒有伺服器或基礎架構的情況下，直接在使用者之間建立、分享及執行軟體。在實際應用上，這意味著每個使用者不再將應用程式寄存在中央伺服器上，而是成為網路中的一個節點：他們與其他對等使用者分享部分應用程式和資料。整個系統形成了一個分散式網路，其中每個實例相互合作以保持服務的可存取性。
...
*如果您使用 Linux 且已經安裝 Pears，您可以直接跳到 **步驟 2**.* 。


#### 1.3.1.以管理員身分開啟 PowerShell
...
*如果您使用 Linux 或 Windows 並已安裝 Pears，您可以直接跳到 **步驟 2***.*
...
Pears 透過 *npm*（*Node.js* 套件管理程式）發行。雖然 Pears 並不直接依賴 *Node.js* 執行，但在安裝時必須使用 *Node.js*。在 macOS 上安裝 *Node.js* 的建議方法是 *NVM* (*Node Version Manager*)，它允許你同時管理多個 Node 版本。
...
在 Pears 網路中，*seed* 一個應用程式意味著從您自己的電腦將它重新分配給其他對等電腦。實際上，當您 seed Plan ₿ Academy 時，您的電腦就會成為一個資料來源，讓其他使用者可以下載應用程式，而不需要仰賴中央伺服器。
...
感謝您為 Pears 上的 Plan ₿ Academy 的分散式散佈做出貢獻，並協助 Bitcoin 教育真正抵擋審查！