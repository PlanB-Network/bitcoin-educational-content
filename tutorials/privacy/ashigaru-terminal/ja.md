...
足軽ターミナルではビットコインを直接使うことはできません。足軽ターミナルと足軽アプリで同じwalletを同期させるか（このチュートリアルでやります）、`Mix to`オプション（次のチュートリアルで見ます）を使って、設定されたミックスサイクルの後に自動的にwalletハードウェアに資金を送ることができます。
...
*注意*: `Watch-only` オプションを使うと、wallet ハードウェアの `xpub` を保存して、後で `Mix to` 機能を使うことができます。しかし、このタイプのwalletは明らかにコインジョイントに参加できません。この機能については、次のチュートリアルで詳しく説明します。
