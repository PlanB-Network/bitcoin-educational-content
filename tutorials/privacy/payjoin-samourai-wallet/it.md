...
> *"Costringi le spie della blockchain a riconsiderare tutto ciò che pensano di sapere."*
...
Questi scambi crittografati richiedono l'instaurazione di una connessione e l'autenticazione tra i partecipanti Cahoots. Le comunicazioni di Soroban si basano quindi sui Paynym degli utenti. Se non sei familiare con i Paynym, ti invito a consultare questo articolo per ulteriori dettagli: [BIP47 - PAYNYM](https://planb.academy/tutorials/privacy/on-chain/paynym-bip47-a492a70b-50eb-4f95-a766-bae2c5535093)


Per semplificarlo, un Paynym è un identificatore unico collegato al tuo wallet che consente varie funzionalità, tra cui la messaggistica crittografata. Il Paynym è presentato sotto forma di un identificatore e un'illustrazione che rappresenta un robot. Ecco un esempio del mio su Testnet. ![paynym samourai wallet](assets/it/1.webp)
...
- _Paynym_ = identificatore unico di un wallet che consente la comunicazione con un altro utente su Soroban, al fine di effettuare una transazione Cahoots.

[**-> Scopri di più sulle transazioni Payjoin e la loro utilità**](https://planb.academy/tutorials/privacy/on-chain/payjoin-848b6a23-deb2-4c5f-a27e-93e2f842140f)

## Come stabilire una connessione tra Paynym?
Per effettuare una transazione remota Cahoots, nello specifico un Payjoin (Stowaway) tramite Samourai, è necessario "Seguire" l'utente con cui si intende collaborare, utilizzando il suo Paynym. Nel caso di un Stowaway, ciò significa seguire la persona a cui si desidera inviare bitcoin.

**Di seguito la procedura per stabilire questa connessione.**

Per iniziare, è necessario ottenere il codice di pagamento del Paynym del destinatario per il Payjoin. Nell'applicazione Samourai Wallet, il destinatario deve toccare l'icona del suo Paynym (il piccolo robot) situata in alto a sinistra dello schermo e quindi fare clic proprio nickname Paynym, che inizia con `+...`. Ad esempio, il mio è `+namelessmode0aF`. Se il tuo collaboratore utilizza Sparrow Wallet, ti invito a consultare il nostro tutorial dedicato cliccando qui.

![connexion paynym samourai](assets/notext/2.webp)

Il tuo collaboratore verrà reindirizzato alla propria pagina Paynym. Da lì, potrà condividere le proprie credenziali Paynym con te o il proprio codice QR da scansionare.  Per farlo, deve cliccare sull'icona "condividi" situata in alto a destra dello schermo.
...
Puoi quindi incollare il codice di pagamento del tuo collaboratore selezionando `COLLER LE CODE DE PAIEMENT`(INCOLLA IL CODICE DI PAGAMENTO), o aprire la fotocamera per scansionare il suo codice QR premendo `SCANNEZ LE CODE QR` (SCANNERRIZA IL QR CODE).![paste paynym identifier](assets/notext/5.webp)
...
Il software ti offrirà quindi un pulsante `SE CONNECTER`(CONNETTI). Non è necessario fare clic su questo pulsante per il nostro tutorial. Questo passaggio è richiesto solo se hai intenzione di effettuare pagamenti all'altro Paynym nell'ambito del BIP47, che non è correlato al nostro tutorial.
![connect paynym](assets/notext/8.webp)
Una volta che il tuo Paynym segue il Paynim del destinatario, ripeti questa operazione in direzione opposta in modo tale che anche il destinatario ti segua. Puoi quindi effettuare un Payjoin.
