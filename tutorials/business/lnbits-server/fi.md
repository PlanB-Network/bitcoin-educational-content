...
description: Itse isännöidyn LNbits-palvelimen asennus ja konfigurointi Ubuntu VPS:llä PHOENIXD:lla tai Umbrelilla
---
...
LNbits on avoimen lähdekoodin Interface-verkkosovellus, joka muuttaa minkä tahansa Lightning-taustapalvelun (LND, Core Lightning, PHOENIXD) täydelliseksi palvelualustaksi. Tämän itse isännöidyn ratkaisun avulla voit hallita useita Lightning-salkkuja erillään, ottaa käyttöön myyntipisteitä, luoda lahjoitusjärjestelmiä tai laskutuspalveluita säilyttäen samalla varojen täydellisen hallinnan.



Tämä opetusohjelma kattaa kaksi asennustapaa: **VPS Ubuntu with PHOENIXD** (kevyt ratkaisu ilman täyttä Bitcoin-solmua) ja **Umbrel** (integrointi olemassa olevaan LND-solmuun). Toisin kuin Plan B Networkin yleisessä LNbits-oppaassa, jossa käsitellään käsitteitä ja laajennuksia, tässä oppaassa keskitytään teknisiin asennustoimenpiteisiin askel askeleelta.
...
LNbits on Python-kielellä (FastAPI) kehitetty Lightning-kirjanpitojärjestelmä, joka on yhteydessä olemassa olevaan backendiin (LND, Core Lightning, PHOENIXD). Toisin kuin perinteiset Lightning-solmut, LNbits tarjoaa käytettävissä olevan Interface:n, jonka avulla voit hallita useita erillisiä salkkuja, joilla on omat API-avaimet. Voit luoda alatilejä perheellesi, työntekijöille tai projekteille antamatta heille pääsyä kaikkiin varoihisi.
...
LNbits tarjoaa monipuolisen **laajennusjärjestelmän**: LNbNs tarjoaa: TPoS (myyntipiste), Paywall (sisällön rahaksi muuttaminen), Events (lipunmyynti), LndHub (palvelin BlueWalletille), Bolt Cards (NFC-maksut), Split Payments (automaattinen jakelu) ja User Manager (käyttäjähallinta todennuksella).
...
## Asennus Ubuntu VPS:ään PHOENIXD:lla
...
Kun palvelimesi on suojattu, sinun on asennettava ja konfiguroitava PHOENIXD. Plan B Network tarjoaa täydellisen oppaan, joka kattaa asennuksen, seed:n luomisen ja systemd-palvelun konfiguroinnin:



https://planb.academy/tutorials/node/lightning-network/phoenixd-beb86edd-f9c0-4bec-ad36-db234c88e7b1

Kun PHOENIXD on toiminnassa (tarkista `./Phoenix-CLI getinfo`), huomioi **HTTP-salasana** tiedostossa `~/.Phoenix/Phoenix.conf` - tarvitset sitä LNbitsin yhdistämiseen PHOENIXD:een.
...
PHOENIXD-taustapalvelun konfigurointi:
...
Testaa `uv run lnbits --host 0.0.0.0 --port 5000` ja luo sitten systemd-palvelu `Wants=PHOENIXD.service`.
...
Aktivoi Interface-ylläpitäjä tiedostossa `.env` :
...
Siirry SuperUserin hallintakoneesta Interface kohtaan "Asetukset" ja sen jälkeen kohtaan "Käyttäjien hallinta". Löydät vaihtoehdon "Salli uusien käyttäjien luominen".
...
- Hyökkääjä voi luoda tilejä ja käyttää LIGHTNING NODE:n maksuvalmiutta tietämättäsi
- Käyttäjätilit on luotava manuaalisesti Interface SuperUser -ohjelmasta
...
Kun se on määritetty, vain SuperUser-ylläpitäjä voi luoda uusia käyttäjätilejä Interface:n "Users"-osion kautta. Tämä lähestymistapa takaa täydellisen valvonnan sen suhteen, kuka voi käyttää Lightning-infrastruktuuria ja varoja.
...
PHOENIXD hallinnoi kanavia automaattisesti automaattisen likviditeetin avulla. generate Lightning Invoice ~30 000 Sats LNbitsistä ja maksaa sen toisesta Wallet:stä. PHOENIXD avaa automaattisesti kanavan ACINQ:lle. Avausmaksu (~20-23k Sats) vähennetään, loppusaldo (~7-10k Sats) näkyy On-Chain:n vahvistuksen jälkeen.
...
Umbrel tarkistaa automaattisesti tarvittavat riippuvuudet. LNbits vaatii toimiakseen LIGHTNING NODE:n (LND). Jos LIGHTNING NODE on jo toiminnassa, vahvista se napsauttamalla "Install LNbits".
...
Umbrel lataa Docker-kuvan, määrittää automaattisesti yhteydet LND:n kanssa ja käynnistää kontin (2-5 minuuttia). Asennus tapahtuu kokonaan taustalla.
...
Ensimmäisellä käynnistyskerralla LNbits pyytää sinua luomaan SuperUser-ylläpitäjätilin. Anna käyttäjänimi ja aseta turvallinen salasana Interface:n hallintajärjestelmän suojaamiseksi.
...
Kun olet luonut tilin, pääset automaattisesti Interface:n päähallinta-alueelle. Umbrel on jo perustanut LND:n rahoituslähteeksesi - kaikki Lightning-maksut menevät nykyisten kanaviesi kautta.



### Pääsy Interface:n ylläpitäjälle
...
**Tärkeä kohta**: Jokainen käyttäjä voi kirjautua sisään täysin itsenäisesti omalla salasanallaan. SuperUser-ylläpitäjä säilyttää täyden hallinnan Interface-hallintatyökalun kautta.
...
Satoshi-käyttäjällä on nyt yksi, selvästi tunnistettava Wallet. Kukin käyttäjän Wallet toimii täysin itsenäisesti, mutta käyttää taustalla olevan LND-solmun likviditeettiä.



**Konsepti**: Kaikki nämä lompakot jakavat Umbrel-solmusi globaalin likviditeetin. Et luo uusia Lightning-kanavia jokaista Wallet:ää varten - LNbits toimii älykkäänä kirjanpito-Layer:nä, joka hallinnoi varojen jakamista olemassa olevassa Lightning-infrastruktuurissasi. Siinä on LNbitsin multi-Wallet-järjestelmän voima.
...
Kirjaudu ulos SuperUser-tililtä (kuvake oikealla ylhäällä) ja palaa LNbitsin kirjautumissivulle. Voit nyt kirjautua sisään uuden käyttäjän tunnuksilla.
...
Syötä aiemmin määritetty käyttäjänimi ("Satoshi") ja salasana ja napsauta sitten "LOGIN". Käyttäjä saa suoran pääsyn henkilökohtaiseen Wallet:een, joka on täysin eristetty hallintalaitteesta Interface.
...
Kun yhteys on muodostettu, käyttäjä käyttää Interface:taan Wallet Lightningista.
...
Interface:n ominaisuudet:




- Nykyinen saldo**: Näytetään Sats:nä ja valittuna valuuttana (tässä esimerkissä CUC)
- Tärkeimmät toimet**: "PASTE REQUEST" (liitä maksettava lasku), "CREATE Invoice" (generate kuitti), QR-kuvake (pikaskannaus)
...
LNbits tarjoaa useita tapoja käyttää Wallet:ää älypuhelimessa:
...
- Aktivoi **LndHub**-laajennus LNbitsissä tätä Wallet:tä varten
- Skannaa LndHubin QR-koodi mobiilisovelluksella Wallet:n yhdistämiseksi
...
- Skannaa tämä QR-koodi älypuhelimellasi avataksesi Wallet suoraan mobiiliselaimessasi
- Lisää sivu aloitusnäyttöön nopeaa käyttöä varten



**Tärkeä turvallisuus**: Tämä URL-osoite sisältää API:n avaimet, joiden avulla pääset käyttämään Wallet:tä. Älä koskaan jaa sitä julkisesti. Käsittele tätä QR-koodia kuten Bitcoin:n yksityisiä avaimia - kuka tahansa, joka skannaa tämän QR-koodin, saa täyden pääsyn Wallet:een.
...
Tämän monikäyttäjäkokoonpanon tärkein käyttötapaus on **lompakoiden jakaminen perheen tai lähipiirin kanssa**. Kun olet luonut käyttäjän, jolla on oma Wallet-tunnus (esimerkiksi "Satoshi" esimerkissämme), voit jakaa nämä kirjautumistiedot luotettavien perheenjäsenten kanssa.
...
Tämä kaksinkertainen Layer-suojaus (verkkoyhteys + käyttäjän todennus) tekee "Salli uusien käyttäjien luominen" -vaihtoehdon vähemmän kriittiseksi Umbrelissa. Vain henkilöt, joilla on jo pääsy verkkoon tai VPN:ään, pääsevät Interface-kirjautumiseen.
...
Palaa Interface SuperUseriin ja avaa vasemmanpuoleisen paneelin "Laajennukset"-valikko tutustuaksesi koko LNbits-laajennusten ekosysteemiin.
...
- Tukiliput**: Maksullinen tukijärjestelmä (saat Satssin vastaamaan kysymyksiin)
...
- SMTP**: Mahdollistaa käyttäjille sähköpostien lähettämisen ja ansaita Satssia
...
Jokainen laajennus aktivoidaan yhdellä napsautuksella tästä Interface:sta. Laajennukset, joissa on merkintä "ILMAINEN", ovat maksuttomia, kun taas jotkut ovat saatavilla maksullisina versioina. Tutustu luetteloon ja etsi tarpeisiisi sopivat laajennukset - olipa kyse sitten liiketoiminnasta, perheenhallinnasta tai Lightning Network:n ominaisuuksien kokeilemisesta.
...
**Edut**: Interface: Taloudellinen riippumattomuus (varojen/avainten/tietojen täysi hallinta), arkkitehtuurin joustavuus (häviötön VPS→Full node-siirtyminen), ammattimainen laajennusjärjestelmä, intuitiivinen Interface.



**Rajoitukset** : Ohjelmisto on beta-versiossa (varovaisuus määrien suhteen), tietoturva ylläpitäjän vastuulla, URL-osoitteet, jotka sisältävät arkaluonteisia API-avaimia (HTTPS pakollinen), usean käyttäjän hallinta edellyttää huoltajan vastuuta.
...
**Backups**: seed PHOENIXD/credentials LND, LNbits-tietokanta, `.env`-tiedostot. Automatisoidaan päivittäin, pidetään poissa tuotantopalvelimelta, salattu. Testaa palautukset säännöllisesti.
...
LNbitsin omahosting tarjoaa konkreettisen tien Lightningin taloudelliseen riippumattomuuteen. VPS+PHOENIXD tarjoaa kevyen ratkaisun nopeisiin palveluihin, ja Umbrel integroituu täysin olemassa olevaan Bitcoin-solmuun. Skaalautuva arkkitehtuuri mahdollistaa kehityksen yksinkertaisesta monikäyttäjä-Wallet:stä kehittyneisiin liiketoimintakäyttötapauksiin.
