...
Olvid 的安全模型基于一个重要原则：在建立数字身份的过程中完全不依赖可信的第三方。与大多数依赖集中式目录来管理用户身份的加密消息服务不同，Olvid 不依赖任何集中式基础设施来保障通信的完整性。这种架构消除了因目录被攻破而带来的风险。

尽管如此，Olvid 仍然使用一个中心服务器进行消息分发，但该服务器仅承担后勤角色：异步传输加密消息。该服务器不参与任何加密过程，不知道用户的真实身份，也不了解消息内容或元数据（除了用于路由的接收方公钥）。因此，该服务器默认可以视为不可信，但不会影响整体安全性。即使它被攻破，也无法访问通信内容。Olvid 出于效率和服务质量考虑而采用集中式分发机制，但安全性仍独立于这一基础设施。
...
⚠️ **注意**：从 Olvid 3.7 版本开始，备份您的个人资料和联系人的程序已被新程序替换。本教程仍然介绍旧版本。您可以在他们的常见问题中了解新版本：[💾 备份您的个人资料](https://www.olvid.io/faq/sauvegarder-vos-profils/)
