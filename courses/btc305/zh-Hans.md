...
BTCPay Server可让您通过两种方式启用2FA，其可以通过一些2FA应用（Authy、Google、Microsoft认证器）或通过安全设备（FIDO2或LNURL Auth）完成。
...
商店所有者可以选择是否使用LNURL。闪电网络URL，或LNURL，是一种提议的标准，用于闪电付款方和收款方之间的互动。简而言之，LNURL是一个以lnurl为前缀的bech32编码url。闪电钱包可对URL进行解码，联系URL，并等待一个包含进一步指令的JSON对象，最值得注意的是定义knurl操作方式的标签。

- 启用LNURL
- LNURL经典模式
  - 为了钱包兼容性，比特币地址的编码格式有两种：Bech32 编码（经典）与明文 URL（即将推出）。
- 允许收款方传递评论。
