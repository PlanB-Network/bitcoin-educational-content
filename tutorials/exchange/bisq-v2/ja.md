...
このガイドは「Bisq Easy」にのみ焦点を当てています。これは現在利用可能な唯一のプロトコルです。Bisq Easyは新しいビットコインユーザーのために特別に設計されました。このプロトコルにより、ユーザーは分散型のピアツーピアプラットフォーム上で法定通貨とビットコインを売買できます。取引は600米ドル（最低6米ドル）相当までに制限されており、取引の安全性はBTC販売者の評判に依存しています。Bisq Easyには取引手数料や保証金の要件はありません。Bisq Easyは、600米ドル（または同等額）未満の法定通貨取引においてBisq 1を置き換える予定です。
