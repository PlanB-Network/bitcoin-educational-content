...
Seedkeeper是Satochip公司开发的一款智能卡，Satochip是一家比利时公司，专门从事管理和保护数字机密的硬件解决方案。Satochip 公司以其 Bitcoin 生态系统智能卡系列而闻名，Seedkeeper 的设计理念是替代传统的 Mnemonic 短语和其他数字机密存储方法。



具体来说，Seedkeeper 是一种多功能、通过 EAL6 认证的智能卡，带有安全处理器和防篡改存储器（即 "*secure element*"）。顾名思义，它的作用是以加密和受保护的方式存储 Bitcoin 组合的助记符和密码。有了 Seedkeeper，您可以直接将 generate、导入、组织和保存您的秘密到卡的安全组件中。
...
在本教程中，我们只介绍如何使用 SeedKeeper 以密码管理器的方式保存传统密码。如果您想使用 SeedKeeper 保存 Bitcoin Wallet Mnemonic 短语，请参阅本教程：
...
系统会要求您输入种子管家的 PIN 码。由于这是一张新卡，因此尚未定义 PIN 码。请输入任意代码跳过这一步，然后点击 "*下一步*"。
...
选择 "generate secret*"。通过 "*导入密文*"选项，您可以保存现有密文（例如，您过去创建的密码）。
...
为该密文指定一个 "*标签*"，以便在 Seedkeeper 中存储多条信息时可以轻松识别。您还可以添加与密码和网站 URL 相关联的用户名。
...
选择密码长度和参数，然后点击 "*generate*"，再点击 "*导入*"。
...
这将带您进入所有已注册秘密的列表。在本例中，我想显示 Plan ₿ Network 账户的密码，因此点击了它。
...
这将带您进入主页，上面有您的秘密列表。点击 Interface 右上方的三个小圆点。
...
这就是全部内容！现在你知道如何使用 Seedkeeper 来存储密码了吧。在今后的教程中，我们将介绍如何使用 Seedkeeper 备份 Bitcoin 投资组合。我还会邀请你来了解它与 SeedSigner 的结合使用：
