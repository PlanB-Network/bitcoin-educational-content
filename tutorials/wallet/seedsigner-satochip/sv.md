...
Satochip är en Hardware Wallet i smartkortformat med ett EAL6+-certifierat säkerhetselement, en av de högsta säkerhetsstandarderna. Den är designad och producerad av det belgiska företaget med samma namn: Satochip.



Satochip kostar ca 25 euro och skiljer sig från konkurrenterna genom sitt utmärkta värde för pengarna. Tack vare sitt säkra chip är den motståndskraftig mot fysiska attacker. Dessutom är källkoden för appleten helt öppen källkod och licensierad enligt *AGPLv3*.
...
- Satochip kostar cirka 25 euro, och eftersom appleten är öppen källkod kan du installera den själv på ett tomt smartkort. Sedan tillkommer kostnaden för SeedSigner-komponenterna och tillägget för läsning av smartkort: beroende på var man köper hårdvaran blir summan mellan 70 och 100 euro.
...
Nu när du har fått all din hårdvara och mjukvara att fungera korrekt kan du gå vidare med att skapa din Bitcoin-portfölj. För att göra detta ansluter du din SeedSigner och sedan generate din seed som med en vanlig SeedSigner, antingen genom att kasta tärningen eller genom att ta ett foto:
...
Vilken backup-strategi som väljs här beror på dina preferenser, men det är absolut nödvändigt att ha minst en kopia av din Mnemonic-fras, antingen på fysiska medier (papper eller metall) eller, som här, i en Seedkeeper. Du kan också multiplicera antalet säkerhetskopior efter behov. För mer information om strategier för säkerhetskopiering av portföljer föreslår jag att du läser denna handledning :
...
Välj en `Label` för att enkelt identifiera dina olika hemligheter som lagras på Seedkeeper. Du kan t.ex. helt enkelt behålla Wallet:s fingeravtryck eller uttryckligen ange `seed`. Valet beror på dina preferenser och risker.
...
PIN-koden för Seedkeeper bör vara så lång och slumpmässig som möjligt för att förhindra försök till brute force i händelse av att kortet fysiskt äventyras. Du bör också ha en säkerhetskopia av denna PIN-kod, lagrad på en annan plats än Seedkeeper. Utan denna PIN-kod kommer du inte att kunna komma åt Mnemonic som lagras i Seedkeeper, och dina bitcoins kommer att gå förlorade för alltid.
...
Nu när din portfölj har genererats, sparats och kontrollerats ska vi överföra den till Satochip. För att göra detta, se till att seed är laddad i SeedSigners RAM. Gå sedan till `Tools > Smartcard Tools > Satochip Functions`.
...
Sätt in din Satochip i smartkortläsaren och välj sedan `Initialisera med seed`.
...
Du kan sedan ställa in Satochip PIN-koden, från 4 till 16 tecken. För att förstärka säkerheten för din Wallet, välj en lång, slumpmässig kod: det är det enda skyddet mot fysisk åtkomst till din Mnemonic-fras.



Kom ihåg att spara denna PIN-kod så snart den skapats, antingen i en säker lösenordshanterare eller på ett fysiskt medium, beroende på din personliga strategi. I det senare fallet ska du se till att aldrig förvara mediet som innehåller PIN-koden på samma plats som din Satochip, annars blir skyddet värdelöst. Det är viktigt att ha en säkerhetskopia: **utan denna PIN-kod kommer du inte längre att kunna komma åt din seed, och dina bitcoins kommer att gå förlorade permanent**.
...
Om du vill använda en passphrase BIP39 för att förbättra säkerheten för din Wallet, se del 6 av denna handledning:
...
Slutligen väljer du `Coordinator`, dvs. det program för portföljförvaltning som du vill använda. Här kommer vi att använda Sparrow wallet.
...
Ett varningsmeddelande visas: detta är helt normalt. Den utökade publika nyckeln (`xpub`) gör att du kan se alla adresser som härrör från din seed (på det första kontot). Den ger dock inte tillgång till dina medel: dess avslöjande skulle äventyra din integritet, men inte säkerheten för dina bitcoins. Med andra ord kan du se dina saldon, men inte spendera dem.
...
Öppna Sparrow wallet på datorn och klicka sedan på `File → Import Wallet` i menyraden.
...
Tilldela ett namn till din portfölj och klicka sedan på `Create Wallet`. Sparrow kommer sedan att be dig att ange ett lösenord för att låsa lokal åtkomst till denna Wallet. Välj ett starkt lösenord: det skyddar dina data i Sparrow (offentliga nycklar, adresser, etiketter och transaktionshistorik). Detta lösenord krävs dock inte för att återställa Wallet i framtiden: endast din Mnemonic-fras (och eventuellt din passphrase) kommer att göra det.
...
Kontrollera nu att det `Master fingerprint` som visas i Sparrow wallet stämmer överens med det som tidigare fanns på din SeedSigner.



SeedSigner kommer sedan att be dig att skanna en slumpmässig mottagande Address från din Sparrow wallet för att bekräfta importens giltighet.
...
Din Satochip (via SeedSigner) och Sparrow wallet är nu säkert anslutna. Sparrow fungerar som en komplett hantering av Interface, medan Satochip förblir den enda enheten som kan signera dina transaktioner. Du är nu redo att ta emot och skicka bitcoins i en helt luftgapad konfiguration.
...
#### 7.1.1 Skapa en mottagning Address
...
En ny Bitcoin Address dyker upp. Du kommer att hitta :




- Address i textformat (börjar med `bc1q...` om du använder P2WPKH, som i det här exemplet) ;
...
Din Address är nu associerad med denna etikett i alla Sparrow-avdelningar.
...
Innan du kommunicerar din mottagande Address till betalaren är det viktigt att kontrollera att den tillhör din seed. Detta steg säkerställer att din Satochip sedan kommer att kunna signera transaktioner som är associerade med denna Address. Det förhindrar också potentiella attacker där Sparrow skulle visa en bedräglig Address. Tänk på att Sparrow körs i en osäker miljö (din dator), vars attackyta är mycket större än den för din Satochip, som är helt isolerad. Därför bör du aldrig lita blint på de adresser som visas i Sparrow innan du har kontrollerat dem på din Hardware Wallet.



I Sparrow, klicka på QR-koden för Address för att förstora den: den kommer då att visas i helskärm.
...
Om den skannade Address matchar den som härrör från din seed, kommer SeedSigner att visa meddelandet: `Address verifierad`.
...
Då kan du vara säker på att Address hör hemma i din portfölj.
...
Du kan nu sända denna Address i textform eller via dess QR-kod till den person eller tjänst som behöver skicka dig Satss. När transaktionen har sänts ut i nätverket kommer den att visas på fliken `Transaktioner` i Sparrow wallet.
...
- I `Pay to` klistrar du in mottagarens Address eller skannar deras QR-kod med hjälp av kameraikonen ;
- I `Label`, lägg till en etikett för att spåra denna kostnad;
- I `Amount` anger du det belopp som ska skickas;
- Välj slutligen laddningshastighet enligt aktuella nätverksförhållanden (uppskattningar finns på [Mempool.space](https://Mempool.space/)).
...
Nu när transaktionen är signerad och validerad återstår bara att sända den i Bitcoin-nätverket så att en Miner kan inkludera den i en BLOCK. I Sparrow klickar du på `Scan QR`.
...
Presentera QR-koden som visas på din SeedSigner (den som innehåller den signerade transaktionen) för webbkameran. Sparrow kommer då att visa alla transaktionsdetaljer. Gör en sista kontroll av att all information är korrekt och klicka sedan på "Broadcast Transaction" för att sända ut den i Bitcoin-nätverket.
...
- Genom att spela in Mnemonic-fras på ett fysiskt medium;
...
Om du fortfarande har din Satochip men din SeedSigner är trasig eller försvunnen är situationen ganska enkel att hantera, eftersom din Wallet fortfarande finns i Satochip.
...
Slutligen anger du PIN-koden för ditt smartkort för att låsa upp det. Du kommer då att kunna komma åt din Wallet, skapa transaktioner och signera dem direkt med den anslutna Satochip.
...
Det andra, mer känsliga scenariot är när du förlorar tillgången till din Satochip som innehåller seed: oavsett om den är trasig, borttappad, stulen eller om du har glömt PIN-koden. Om din Satochip har stulits eller förkommit rekommenderar vi starkt att du omedelbart överför dina bitcoins till en helt ny Wallet, genererad med en annan seed, så snart du har fått tillgång till dina pengar igen. Detta säkerställer att en potentiell angripare aldrig kan få tillgång till dina Satss.



För att återfå tillgång till din portfölj och flytta dina pengar laddar du bara din seed i SeedSigner. Beroende på vilken backupmedia du använde har du flera alternativ:





- Ange din Mnemonic-fras manuellt i menyn `Seeds > Enter 12-word seed`.
...
- Eller ladda din seed från en Seedkeeper, via menyn `Seeds > From SeedKeeper` (det är den metod jag använder i den här handledningen). Du behöver bara ange PIN-koden för Seedkeeper och välja den hemlighet som ska användas som seed på SeedSigner.
