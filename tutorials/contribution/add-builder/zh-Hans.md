...
- 首先，您需要拥有一个GitHub账户。如果您不知道如何创建账户，我们制作了[一个详细的教程来指导您](https://planb.academy/tutorials/contribution/others/create-github-account-a75fc39d-f0d0-44dc-9cd5-cd94aee0c07c)。
- 转到[PlanB专用于数据的GitHub仓库](https://github.com/PlanB-Network/bitcoin-educational-content/tree/dev/resources/projects)的`resources/project/`部分：
...
- 在这个文件夹中，您将创建一个名为`project.yml`的第一个YAML文件：
...
- 返回到 `resources/projects/` 文件夹，并选择您在上一个提交中刚创建的业务文件夹：
...
- 保留默认的提交标题，并确保选中了 `Commit directly to the patch-1 branch` 框，然后点击 `Commit changes`： ![project](assets/23.webp)
- 返回到 `assets` 文件夹：
...
![project](assets/30.webp)- 在文件上，点击右上角的三个小点，然后点击`删除文件`：
