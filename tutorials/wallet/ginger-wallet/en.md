...
Once you've created your first portfolio, you'll be taken to Ginger's Interface Wallet.
...
You can create multiple portfolios on Ginger Wallet. Simply click on **Add a portfolio**.


![screen](assets/fr/16.webp)


Ginger supports the use of hardware portfolios via the Bitcoin core standard Interface, although direct integration from or to a hardware portfolio is not yet available.
...
This tab lets you enable Bitcoin Knots to run when the application starts, choose your network (Main/RegTest), and your charge rate provider (Mempool Space/BLOCKSTREAM info/Full node), etc.
...
- For two-factor authentication, make sure your authentication application supports the SHA256 protocol and 8-digit codes. Ginger Wallet requires an 8-digit 2FA code for enhanced security. This longer format makes the code much harder to guess or compromise, offering greater protection against unauthorized access.
...
Don't forget to press **Done** (in the lower right-hand Coin) each time, to save your settings. Some settings require Ginger Wallet to be restarted to take effect.
...
- CoinJoin** (where you can customize this portfolio's CoinJoin settings) ;
...
Video tutorial on how to send via Ginger Wallet.
...
The top right Coin of the screen with the **All** label allows you to filter offers by payment method. Your selected payment method will be set by default, but can be changed at any time.
...
## Make a CoinJoin on Ginger Wallet
...
Ginger CoinJoin is ready to use as soon as you download it (no additional steps required). Automatically, Ginger CoinJoin runs in the background to protect your privacy with every transaction. In practice, the CoinJoin reader will appear whenever you have a balance that can be anonymized.


As for manual start-up of CoinJoin, it's a one-click operation. Start the round and wait for the CoinJoin transaction to be built and confirmed. You'll see the anonymization score in Interface.
