...
Il **server Graylog** gira su **Debian 12**, ma l'installazione è possibile eseguirla su altre distribuzioni, anche tramite Docker. La macchina virtuale è dotata di **8 GB di RAM** e **256 GB di spazio su disco**, in modo da avere risorse sufficienti per tutti i componenti (altrimenti questo può avere un impatto significativo sulle prestazioni). Tuttavia, questo dato è indicativo, poiché **il dimensionamento dell'architettura di Graylog dipende dalla quantità di informazioni da elaborare**. Graylog può elaborare 30 MB o 300 MB di dati al giorno, così come 300 GB di dati al giorno... È una soluzione **scalabile** in grado di gestire **terabyte di registri** (vedi [questa pagina](https://go2docs.graylog.org/current/planning_your_deployment/planning_your_deployment.html?tocpath=Plan%20Your%20Deployment%7C_____0)).

![Image](assets/fr/032.webp)

_Fonte: Graylog_

Prima di iniziare la configurazione, assegna un indirizzo IP statico alla macchina Graylog e installa gli ultimi aggiornamenti. Assicurati di impostare il fuso orario della macchina locale e di definire un server NTP per la sincronizzazione di data e ora. Ecco il comando da eseguire:
...
Inizia aggiornando la cache dei pacchetti e installando gli strumenti necessari per il futuro.
...
Una volta fatto questo, inizia a installare MongoDB. Scarica la chiave GPG corrispondente al repository MongoDB:

```
curl -fsSL https://www.mongodb.org/static/pgp/server-6.0.asc | sudo gpg -o /usr/share/keyrings/mongodb-server-6.0.gpg --dearmor
```

Aggiungi quindi il repository di MongoDB 6 alla macchina Debian 12:

```
echo "deb [ signed-by=/usr/share/keyrings/mongodb-server-6.0.gpg] http://repo.mongodb.org/apt/debian bullseye/mongodb-org/6.0 main" | sudo tee /etc/apt/sources.list.d/mongodb-org-6.0.list
```

Successivamente, aggiorna la cache dei pacchetti e cerca di installare MongoDB:
...
MongoDB non può essere installato perché manca una dipendenza: **libssl1.1**. Dovri installare questo pacchetto manualmente prima di poter procedere, perché Debian 12 non lo ha nei suoi repository.
...
Scarica il pacchetto DEB denominato "**libssl1.1_1.1.1f-1ubuntu2.23_amd64.deb**" (ultima versione) con il comando **wget**, quindi lo installerai con il comando **dpkg**. Questo produce i due comandi seguenti:
...
Riavvia l'installazione di MongoDB:

```
sudo apt-get install -y mongodb-org
```

Quindi riavvia il servizio MongoDB e abilitalo all'avvio automatico quando viene lanciato il server Debian.
...
Con MongoDB installato, puoi passare all'installazione del componente successivo.

#### B. Installazione di OpenSearch

Passa all'installazione di OpenSearch sul server. Il comando seguente aggiunge la chiave di firma per i pacchetti OpenSearch:

```
curl -o- https://artifacts.opensearch.org/publickeys/opensearch.pgp | sudo gpg --dearmor --batch --yes -o /usr/share/keyrings/opensearch-keyring
```

Aggiungi quindi il repository OpenSearch, in modo da poter scaricare il pacchetto con **apt** in seguito:
...
Quindi **installa OpenSearch**, facendo attenzione a **definire la password predefinita per l'account Admin** dell'istanza. Qui la password è "**IT-Connect2024!**", ma sostituisci questo valore con una password forte. **Evita password deboli** come "**P@ssword123**" e usa almeno **8 caratteri** con almeno un carattere di ogni tipo (minuscolo, maiuscolo, numero e carattere speciale), altrimenti si verificherà un errore alla fine dell'installazione. **Questo è un prerequisito da OpenSearch 2.12.**
...
Una volta terminato, prenditi un momento per eseguire la configurazione minima. Apri il file di configurazione in formato YAML:
...
- `network.host: 127.0.0.1`: questa configurazione assicura che OpenSearch sia hostato nell'interfaccia ricorsiva, che è sufficiente dato che si trova sullo stesso server di Graylog.
...
È necessario configurare la macchina virtuale Java utilizzata da OpenSearch per regolare la quantità di memoria che il servizio può utilizzare. Modifica il seguente file di configurazione:

```
sudo nano /etc/opensearch/jvm.options
```

Con la configurazione qui implementata, **OpenSearch inizierà con 4 GB di memoria allocata e potrà crescere fino ad 4 GB**, quindi non ci saranno variazioni di memoria durante il funzionamento. Qui la configurazione tiene conto del fatto che la macchina virtuale ha un totale di **8 GB di RAM**. Entrambi i parametri devono avere lo stesso valore. Ciò significa sostituire le righe:
...
Infine, abilita l'avvio automatico di OpenSearch e lancia il servizio associato.
...
Se visualizzi lo stato del sistema, dovresti vedere un processo Java con 4 GB di RAM.
...
Per **installare Graylog 6.1** nella sua ultima versione, esegui i seguenti 4 comandi per **scaricare e installare Graylog Server**:
...
Comincia a configurare queste due opzioni:

- **password_secret**: questo parametro serve a definire una chiave utilizzata da Graylog per proteggere la memorizzazione delle password degli utenti (nello spirito di una chiave di salt). Questa chiave deve essere **unica** e **casuale**.
...
Inizia generando una chiave di 96 caratteri per il parametro **password_secret**:
...
Copia il valore restituito, quindi aprire il file di configurazione di Graylog:

```
sudo nano /etc/graylog/server/server.conf
```

Incolla la chiave nel parametro **password_secret**, in questo modo:

![Image](assets/fr/027.webp)

Salva e chiudi il file.

Successivamente, è necessario impostare la password per l'account "**admin**" creato di default. Nel file di configurazione deve essere memorizzato l'Hash della password, il che significa che deve essere calcolato. L'esempio seguente fornisce l'Hash della password "**LogsWells@**": adattare il valore alla propria password.
...
Copia il valore ottenuto come output (senza il trattino alla fine della riga).

Apri nuovamente il file di configurazione di Graylog:

```
sudo nano /etc/graylog/server/server.conf
```

Incolla il valore nell'opzione **root_password_sha2** in questo modo:

![Image](assets/fr/026.webp)

Nel file di configurazione, imposta l'opzione "**http_bind_address**". Specifica "**0.0.0.0:9000**" in modo che l'interfaccia web di Graylog sia accessibile sulla porta **9000**, attraverso qualsiasi server IP Address.

![Image](assets/fr/024.webp)

Quindi imposta l'opzione "**elasticsearch_hosts**" su `http://127.0.0.1:9200` per dichiarare la nostra istanza OpenSearch locale. Questo è necessario, perché non stiamo usando un **Graylog Data Node**. Senza questa opzione, non sarà possibile andare avanti...
...
Una volta fatto questo, prova a connetterti a Graylog da un browser. Immettere l'indirizzo IP (o il nome) del server e la porta 9000.
...
**Benvenuti nell'interfaccia di Graylog!**

![Image](assets/fr/019.webp)

#### E. Graylog: crea un nuovo account amministratore

Invece di utilizzare l'account amministratore creato in modo nativo da Graylog, è possibile creare il proprio account amministratore personale. Fare clic sul menu "**Sistema**", quindi su "**Utenti e team**" e cliccare sul pulsante "**Crea utente**". Compila quindi il modulo e assegna il ruolo di amministratore al proprio account.

![Image](assets/fr/020.webp)

Un account personalizzato può contenere informazioni aggiuntive, come nome e cognome e indirizzo e-mail, a differenza di un account amministratore nativo. Inoltre, questo garantisce una migliore tracciabilità quando ogni persona lavora con un account nominato.
...
In questa seconda parte, imparerai a configurare una macchina Linux per inviare i suoi log a un server Graylog. Per farlo, installa e configura Rsyslog sul sistema.

### II. Configurazione di Graylog per ricevere i log di Linux

Inizierai configurando Graylog. Ci sono tre passaggi da completare:
...
Accedi all'interfaccia di Graylog, fai clic su "**Sistema**" nel menu e poi su "**Ingressi**". Nell'elenco a discesa, selezionare "**Syslog UDP**" e fai clic sul pulsante "**Lancia nuovo ingresso**". È anche possibile creare un ingresso Syslog TCP, ma questo richiede l'uso di un certificato TLS: un vantaggio per la sicurezza, ma non è trattato in questo articolo.

![Image](assets/fr/001.webp)

Sullo schermo apparirà una procedura guidata. Inizia dando un nome a questo input, ad esempio "**Graylog_UDP_Rsyslog_Linux**" e scegli una porta. Per impostazione predefinita, la porta è "**514**", ma è possibile personalizzarla. Qui è stata selezionata la porta "**12514**".

![Image](assets/fr/016.webp)

È anche possibile selezionare l'opzione "**Store full message**" per memorizzare l'intero messaggio di log in Graylog. Fai clic su "**Lancio dell'input**".
...
Da Graylog, fai clic su "**Sistema**" nel menu, quindi su "**Indici**". Nella nuova pagina visualizzata, fai clic su "**Create index set**".

![Image](assets/fr/005.webp)

Nomina questo indice, ad esempio "**Indice Linux**", aggiungi una descrizione e un prefisso, prima di confermare. In questo caso, **immagazzineremo tutti i registri di Linux in questo indice**. È anche possibile creare indici specifici per memorizzare solo determinati registri, registri di servizi Web, ecc.

![Image](assets/fr/006.webp)

Ora devi creare un nuovo flusso per indirizzare i messaggi a questo indice.

#### C. Creare un flusso

Per creare un nuovo flusso, fai clic su "**Flussi**" nel menu principale di Graylog. Quindi fai clic sul pulsante "**Create stream**" a destra. Nella finestra che appare, dai un nome al flusso, ad esempio "**Flusso Linux**" e scegli l'indice "**Indice Linux**" per il campo "**Index Set**". Conferma la scelta.
...
Il nuovo flusso dovrebbe apparire nell'elenco e dovrebbe essere nello stato "**Running**". La larghezza di banda dei messaggi mostra "**0 msg/s**", poiché al momento non viene inviato alcun log all'ingresso UDP di Rsyslog. Per visualizzare i registri di un flusso, è sufficiente fare clic sul suo nome.
...
Accedi alla macchina Linux, in locale o in remoto, e utilizza un account utente con i permessi di elevare i suoi privilegi (tramite sudo). Altrimenti, utilizza direttamente l'account "root".

#### A. Installazione del pacchetto Rsyslog

Inizia aggiornando la cache dei pacchetti e installando il pacchetto denominato "**rsyslog**".
...
Quindi controlla lo stato del servizio. Nella maggior parte dei casi, è già in funzione.
...
Inoltre, la directory "**/etc/rsyslog.d/**" è usata per memorizzare altri file di configurazione per Rsyslog. Nel file di configurazione principale è presente una cartella "Include" per importare tutti i file "**.conf**" presenti in questa directory. In questo modo è possibile avere file aggiuntivi per la configurazione di Rsyslog senza modificare il file principale.
...
In questo file, inserisci questa riga:
...
- `@`: indica che il trasferimento viene eseguito in UDP. È necessario specificare "**@@**" per passare a TCP.
- `192.168.10.220:12514`: indica l'indirizzo del server Graylog e la porta su cui vengono inviati i log (corrispondente all'Input).
- `RSYSLOG_SyslogProtocol23Format`: corrisponde al formato dei messaggi da inviare a Graylog.

Al termine, salva il file e riavvia Rsyslog.
...
Da Graylog, è possibile fare clic su "**Streams**" e selezionare il nuovo stream per visualizzare i messaggi correlati. In alternativa, fai clic su "**Ricerca**", seleziona il tuo Steam e avvia una ricerca.

Ecco alcune caratteristiche principali dell'interfaccia:

**1** - Seleziona il periodo per il quale visualizzare i messaggi. Per impostazione predefinita, vengono visualizzati i messaggi degli ultimi 5 minuti.

**2** - Seleziona il flusso o i flussi da visualizzare.

**3** - Attiva l'aggiornamento automatico dell'elenco dei messaggi (ad esempio, ogni 5 secondi). Altrimenti, l'elenco sarà statico e dovrai aggiornarlo manualmente.

**4** - Fai clic sulla lente di ingrandimento per avviare la ricerca dopo aver modificato il periodo, il flusso o il filtro.
...
Il punto di forza di Graylog è la capacità di indicizzare i log e di effettuare ricerche in base a vari criteri: macchina di origine, Timestamp, contenuto del messaggio, ecc... Potrai cercare di identificare le connessioni SSH fallite.

Da una macchina remota (il server Graylog, per esempio), prova a connettersi al server Linux su cui è stato appena configurato Rsyslog. Ad esempio:

```
ssh [email protected]
```

Quindi inserisci deliberatamente un **nome utente** e una **password** errati, al fine di ottenere **errori di connessione generate**. Nel file "**/var/log/auth.log**", questo farà sì che generate registri messaggi simili ai seguenti:

```
Failed password for invalid user it-connect from 192.168.10.199 port 50352 ssh2
```

Dovresti trovare questi messaggi su Graylog.

Su Graylog, utilizza il seguente filtro di ricerca per visualizzare solo i messaggi corrispondenti:

```
message:Failed password AND application_name:sshd
```

Se disponi di più server e desideri analizzare i registri di un server specifico, specifica il suo nome in aggiunta a:
...
I tentativi di connessione non riusciti vengono effettuati dalla macchina con indirizzo IP "**192.168.10.199**". Per saperne di più sull'attività di questo host, è possibile **ricercare questo IP Address**. Graylog produrrà tutti i log in cui si fa riferimento a questo indirizzo IP, su tutti gli host (per i quali è configurato l'invio di log).
...
Otteni altri risultati (non sorprendenti, dato che non filtriamo l'applicazione SSH):
