...
Our online exchange platform is designed for an intuitive user experience, allowing you to execute instant swaps with ease. We support multiple languages, including **(a)** English, Vietnamese, and Russian. For small swaps, no account is required; however, you can **(c)** sign up to significantly increase your daily transaction limits. The platform currently lists over 99 digital assets, which you can conveniently convert to either Bitcoin (BTC)—the hardest asset—or directly to Vietnamese Dong (VND), with various payment options available.
...
In the "You send" section – the big square with red VND icon, choose your preferred payment methods to pay for BTC from the dropdown. In this instance, VND Bank Transfer (pay VND via local banks) is selected. We support **67 local banks** in Vietnam.

### Step 2: Choose Receiving Currency

In the "You receive" section - the big square with Bitcoin logo, select BTC (Bitcoin blockchain/mainchain) — it’s usually the default or easy to find in the drop-down list.

### Step 3: Enter VND Amount and Review Deal Details
...
### Step 4: Review & Enter Your Payment Details

Select one of 67 supported local banks from the dropdown list then fill in your full Account holder name, Account number, add a note if desired.

### Step 5: Send the Asset & Receive Your Payment

Click on the Swap button to show BitcoinVN's BTC deposit address and a QR code. Send exactly the specified BTC amount from your wallet to the address has been shown by scanning the QR code or copying the Bitcoin address.
