...
*Вот несколько терминов, с которыми вы встретитесь в схемах курса на английском языке, вместе с их переводом, чтобы помочь вам лучше понять их на вашем языке:*

| Английский         | Перевод - объяснение          |
| ------------------ | ----------------------------- |
| *timelock*         | Временная блокировка          |
| *Revocation Key*   | Ключ отзыва                   |
| *invoice*          | Счет / запрос на оплату       |
| *sig* (signature)  | Подпись                       |
| *secret*           | Секрет                        |
| *amount*           | Сумма                         |
| *scan QR code*     | Сканировать QR-код            |
| *Show QR code*     | Показать QR-код               |
| *Asks the invoice* | Запрашивает счет              |
| *Give the invoice* | Предоставляет счет            |
| *Payment*          | Платеж                        |
| *Preimage*         | Прообраз                      |
...
[Сеть Молния](https://planb.academy/resources/glossary/lightning-network) - это сеть [платежных каналов](https://planb.academy/resources/glossary/payment-channel), построенная поверх протокола Биткоина, целью которой является обеспечение быстрых и недорогих транзакций. Она позволяет создавать платежные каналы между участниками, в рамках которых транзакции могут осуществляться почти мгновенно и с минимальными комиссиями, без необходимости записывать каждую транзакцию индивидуально в [блокчейн](https://planb.academy/resources/glossary/blockchain). Таким образом, Сеть Молния стремится улучшить [масштабируемость](https://planb.academy/resources/glossary/scalability) Биткоина и сделать его пригодным для микроплатежей.
...
В нашем примере, Алиса имеет 100,000 [сатоши](https://planb.academy/resources/glossary/satoshi-sat) на своей стороне канала, а Боб - 30,000, что в сумме составляет 130,000 сатоши, что и является **[емкостью канала](https://planb.academy/resources/glossary/lightning-channel-capacity)**.
...
Другой способ представить распределение средств — представить себе **курсор**, который показывает, где находятся средства в канале. Изначально, при 100 000 сатоши у Алисы и 30 000 у Боба, курсор больше на стороне Боба, так как у Алисы гораздо больше средств. После транзакции на 40 000 сатоши курсор немного сдвинется в сторону Алисы, у которой теперь 60 000 сатоши.
...
Bitcoin-адрес представляет собой серию символов, полученных из **[публичного ключа](https://planb.academy/resources/glossary/public-key)**, который, в свою очередь, вычисляется из **[приватного ключа](https://planb.academy/resources/glossary/private-key)**. Как вы наверняка знаете, он используется для блокировки биткоинов, что эквивалентно их получению в наш кошелек.
...
- **[Peer-to-peer](https://planb.academy/resources/glossary/peertopeer-p2p) коммуникации (протокол Lightning)**: Это сообщения, которые [узлы Сети Молнии](https://planb.academy/resources/glossary/lightning-node) отправляют друг другу для коммуникации. Мы будем изображать эти сообщения пунктирными чёрными линиями на наших диаграммах.
...
- **Биткоин транзакции (протокол Bitcoin)**: Это транзакции, совершаемые [в блокчейне](https://planb.academy/resources/glossary/onchain), которые мы будем изображать оранжевыми линиями.
...
Канал считается открытым, как только транзакция внесения депозита включена в [блок](https://planb.academy/resources/glossary/block) Bitcoin и достигла определенной глубины [подтверждений](https://planb.academy/resources/glossary/confirmation) (количество последующих блоков).
...
Когда Алиса совершает транзакцию в канале для отправки средств Бобу, создается новая транзакция Bitcoin, отражающая это изменение в распределении средств. Эта транзакция, называемая **[транзакцией обязательства](https://planb.academy/resources/glossary/commitment-transaction)**, не публикуется в блокчейне, но представляет новое состояние канала после транзакции Lightning.
...
- **Ключ отзыва**: Средства Алисы также могут быть немедленно потрачены Бобом, если у него есть **ключ отзыва**. Этот ключ состоит из секрета, который хранит Алиса, и секрета, который хранит Боб. Обратите внимание, что этот секрет отличается для каждой транзакции обязательства.
   Благодаря этим двум объединенным механизмам, Боб имеет время обнаружить попытку Алисы обмануть и наказать ее, извлекая свой [выход](https://planb.academy/resources/glossary/output) с помощью ключа отзыва, что для Боба означает восстановление всех средств канала. Наша новая транзакция обязательства теперь будет выглядеть так:
...
- Каждый имеет секрет другого для предыдущей транзакции, что позволяет им использовать ключ отзыва только в случае, если один из них пытается обмануть, опубликовав транзакцию со старым состоянием в [мемпулах](https://planb.academy/resources/glossary/mempool) узлов Bitcoin. Действительно, чтобы наказать другую сторону, необходимо иметь оба секрета и транзакцию обязательства другого, которая включает подписанный [вход](https://planb.academy/resources/glossary/input). Без этой транзакции ключ отзыва сам по себе бесполезен. Единственный способ получить эту транзакцию - извлечь ее из мемпулов (в транзакциях, ожидающих подтверждения) или в подтвержденных транзакциях на блокчейне во время таймлока, что доказывает, что другая сторона пытается обмануть, намеренно или нет.
...
- Алиса и Боб вместе договариваются о [комиссии](https://planb.academy/resources/glossary/transaction-fees) за **транзакцию закрытия**. Эти комиссии обычно рассчитываются на основе рынка комиссий Bitcoin на момент закрытия. Важно отметить, что **всегда тот, кто открыл канал** (Алиса в нашем примере), платит комиссии за закрытие.
...
$$ f_{\text{Кэрол-Боб}} = \text{базовая комиссия} + \left(\frac{\text{ppm} \times \text{сумма}}{10^6}\right) $$
$$ f_{\text{Кэрол-Боб}} = 1 + \frac{1 \times 40000}{10^6} = 1 + 0.04 = 1.04 \text{ сат} $$

- **Сьюзи** вычитает 8 сатоши в виде комиссий на канале с Кэрол:
$$ f_{\text{Сьюзи-Кэрол}} = \text{базовая комиссия} + \left(\frac{\text{ppm} \times \text{сумма}}{10^6}\right) $$
$$ f_{\text{Сьюзи-Кэрол}} = 0 + \frac{200 \times 40001.04}{10^6} = 0 + 8.0002 \approx 8 \text{ сат} $$
...
**Создание секрета**: Боб генерирует случайный секрет, обозначенный как _s_ (прообраз), и вычисляет его хеш, обозначенный как _r_, с использованием [хеш-функции](https://planb.academy/resources/glossary/hash-function), обозначенной как _h_. У нас получается:
...
В этой главе мы более внимательно рассмотрим работу счетов-фактур Lightning, то есть запросов на оплату, отправляемых получающим узлом отправляющему узлу. Цель состоит в том, чтобы понять, как осуществлять платежи и получать платежи в сети Lightning. Мы также обсудим 2 альтернативы классическим счетам-фактурам: [LNURL](https://planb.academy/resources/glossary/lnurl) и Keysend.
...
Счета-фактуры затем кодируются в **[bech32](https://planb.academy/resources/glossary/bech32-and-bech32m)**, том же формате, что и для адресов Bitcoin [SegWit](https://planb.academy/resources/glossary/segwit) (формат, начинающийся с `bc1`).
...
- **Плательщик**: Это тот, кто совершает платежи. Ему нужна [исходящая ликвидность](https://planb.academy/resources/glossary/outbound-capacity), чтобы иметь возможность переводить средства другим пользователям. Например, это может быть потребитель.
- **Продавец (или Получатель платежа)**: Это тот, кто получает платежи. Ему нужна [входящая ликвидность](https://planb.academy/resources/glossary/inbound-capacity), чтобы иметь возможность принимать платежи на свой узел. Например, это может быть бизнес или интернет-магазин.
...
![LNP201](assets/en/076.webp) 2. **Транзакции в канале**: В этом канале затем можно провести множество транзакций без необходимости публиковать их в блокчейне. Каждая транзакция Lightning создает новое состояние канала, отраженное в транзакции обязательства.
...
Теперь, когда вы поняли теоретические принципы работы Lightning Network, вы можете перейти к практике и развернуть свой первый узел Lightning, чтобы обрести большую автономию в его использовании. Для этого пройдите курс LNP 202:
