...
**Provider discovery** : Hard-coded list in `src/configs/Mainnet.ts`. New providers added via Pull Request or email.


**Independent backends**: Each provider operates its own Boltz backend. Interface queries the APIs in real time to compare quotes instantly.
...
**Swap-out (Lightning → On-Chain)**: Convert Lightning satoshis into On-Chain BTC. Use case: dump Wallet Lightning into Cold storage or rebalance liquidity between layers.


### Safety and recovery


**Trustless Atomic Swaps: HTLC guarantees that either the exchange is completed in full, or each party recovers its stake. Counterparty risk is mathematically eliminated.


**Redemption mechanism**: Each swap has an expiry date (TIMELOCK). If the swap fails, the funds are automatically refundable after expiration. The user always retains the option of reclaiming his bitcoins.
...
The application will be accessible at `http://localhost:3000`. Self-hosting guarantees total control over Interface, eliminates the risk of censorship of the official domain, and allows source code to be audited before execution.
...
- Denomination**: Choice between BTC or Sats
...
- Bitcoin** (bottom field): The amount you will receive is automatically displayed after fees have been deducted (example: Sats 29,320)
...
The status changes to **"Invoice.settled "** (invoice paid), and a confirmation message appears.


Your On-Chain bitcoins will be available as soon as the transaction has been confirmed (usually within a few minutes to a few hours, depending on the mining fees chosen by the provider).


![Confirmation swap-out](assets/fr/06.webp)


You can click on **"OPEN CLAIM TRANSACTION "** to view the Bitcoin transaction on a Blockchain explorer.
...
Select the previously downloaded Rescue Key file. After successful verification, the Interface automatically moves on to the next step.
...
Once the transaction has been sent, SwapMarket detects that it is in Mempool and displays :
...
**Confidentiality**: No KYC, Interface 100% client-side (no transmission of personal data), Tor Browser compatible.
...
**Keep an eye on Mempool**: Check Mempool.space before a swap-in. Choose times of low activity to minimize mining costs.
...
SwapMarket facilitates Bitcoin/Lightning exchanges by aggregating multiple providers into a single Interface. The HTLC architecture guarantees the non-custodial nature of swaps, the absence of KYC preserves confidentiality, and the open-source self-hostable code reinforces resistance to censorship.
...
- [ZEUS Swaps](https://zeusln.com) - Provider of Lightning swaps