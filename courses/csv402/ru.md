...
Распределенные вычисления, отдельная отрасль компьютерной науки, изучает протоколы, используемые для циркуляции и обработки информации в сети [узлов](https://planb.academy/resources/glossary/node). Вместе эти узлы и правила протоколов образуют так называемую распределенную систему. Среди основных свойств, характеризующих такую систему, можно назвать следующие:
...
- **хронологический порядок** операций, чтобы данные были надежно [привязаны по времени](https://planb.academy/resources/glossary/timestamp) и существовал консенсус относительно последовательности событий (последовательности состояний).
...
- Согласование порядка этих изменений состояния, что делает невозможным переписать или отменить подтвержденные операции апостериори (это также известно в Биткойне как "защита от [двойной траты](https://planb.academy/resources/glossary/double-spending-attack)").

Первая функциональная реализация механизма распределенного консенсуса без права доступа была представлена Сатоши Накамото в биткойне благодаря совместному использованию структуры данных блокчейн и алгоритма [Proof-of-Work](https://planb.academy/resources/glossary/proof-of-work) (PoW). В этой системе достоверность истории блока зависит от вычислительной мощности узлов ([майнеров](https://planb.academy/resources/glossary/miner)). Таким образом, биткойн является важным и историческим примером распределенной системы консенсуса, открытой для всех (*permissionless*).
...
В контексте Биткойна вы, несомненно, знакомы с принципами майнинга, децентрализации и окончательности транзакций в блокчейне, а также с тем, как работают [платежные каналы](https://planb.academy/resources/glossary/payment-channel). В RGB мы вводим новую парадигму под названием **Client-side Validation**, которая, в отличие от блокчейна или Lightning, заключается в локальном (на стороне клиента) хранении и проверке переходов состояния [смарт-контракта](https://planb.academy/resources/glossary/smart-contract). Она также отличается от других "DeFi" техник (_rollups_, _plasma_, _ARK_ и т.д.) тем, что Client-side Validation опирается на блокчейн для предотвращения двойных трат и систему временных меток, сохраняя реестр состояний и переходов вне цепи только у соответствующих участников.
...
Для смарт-контрактов типа RGB мы разбиваем на шарды в соответствии с самими контрактами. Каждый контракт является независимым _шардом_. Например, если вы держите только токены USDT, вам не нужно хранить или подтверждать всю историю другого токена, например USDC. В биткойне блокчейн не делает _шардинга_: у вас есть глобальный набор [UTXO](https://planb.academy/resources/glossary/utxo). При валидации на стороне клиента каждый участник сохраняет только те данные контракта, которые он держит или использует.
...
Коммитмент" - это криптографическое обязательство, обычно _хэш_ (например, [SHA-256](https://planb.academy/resources/glossary/sha256)), вставляемый в транзакцию Биткойна, который доказывает, что в нее были включены приватные данные, не раскрывая их.
...
- **Криптографическое обязательство (хэш)**: С помощью [хэш-функции](https://planb.academy/resources/glossary/hash-function) вы можете взять на себя обязательство хранить часть данных (число), опубликовав его хэш. Данные остаются секретными до тех пор, пока вы не раскроете предварительный образ, но вы можете доказать, что знали их заранее;
...
Каждый контракт представляет собой изолированный _осколок_: USDT и USDC, например, не обязаны делиться своими историями. [Атомарные обмены](https://planb.academy/resources/glossary/atomic-swap) по-прежнему возможны, но это не предполагает слияния их реестров. Все делается с помощью криптографических обязательств, без раскрытия всего графа истории каждому участнику.
...
В этой главе мы рассмотрим реализацию [клиентской валидации](https://planb.academy/resources/glossary/client-side-validation) и [одноразовых печатей](https://planb.academy/resources/glossary/single-use-seal) в блокчейне Биткойна. Мы представим основные принципы работы **уровня коммитмента** (уровень 1) RGB, уделив особое внимание схеме **TxO2**, которую RGB использует для определения и закрытия печати в транзакции Bitcoin. Далее мы обсудим два важных момента, которые еще не были подробно рассмотрены:


- Детерминированные обязательства биткоина;
- [Многопротокольные обязательства](https://planb.academy/resources/glossary/multi-protocol-commitment).
...
- Используйте [открытый ключ](https://planb.academy/resources/glossary/public-key) или адрес
...
- Используйте [вывод](https://planb.academy/resources/glossary/output) транзакций **Bitcoin**

Это означает, что _одноразовая пломба_ определяется как точная _точка выхода_ (пара [TXID](https://planb.academy/resources/glossary/txid-transaction-identifier) + номер выхода). Как только эта _выходная точка_ будет израсходована, пломба будет закрыта.
...
- Определите печать через значение открытого ключа и закройте ее в _[входе](https://planb.academy/resources/glossary/input)_ ;
...
| PkI            | Значение открытого ключа | Вход транзакции         | Только [Taproot](https://planb.academy/resources/glossary/taproot) & несовместимо с устаревшими кошельками           | Идентификация на базе Bitcoin | Sigtweak, witweak                |
...
В день, когда он хочет закрыть печать (чтобы сигнализировать о событии или закрепить конкретное сообщение), он тратит этот UTXO в новой транзакции (эту транзакцию часто называют "_[свидетельской транзакцией](https://planb.academy/resources/glossary/witness-transaction)_" (не связанной с _сегвитом_, это просто термин, который мы ему дали). Эта новая транзакция будет содержать _коммитмент_ к сообщению.
...
- **Входе** через :
- **Sigtweak** (изменяет `r` компонент [подписи](https://planb.academy/resources/glossary/digital-signature) [ECDSA](https://planb.academy/resources/glossary/ecdsa), аналогично принципу "Sign-to-contract");
...
Более ранняя схема предполагала использование случайной части подписи (ECDSA или [Schnorr](https://planb.academy/resources/glossary/schnorr-protocol)) для встраивания _обязательства_: эта техника известна как "**Подпись к контракту**". Вы заменяете случайно сгенерированный [nonce](https://planb.academy/resources/glossary/nonce) на хэш, содержащий данные. Таким образом, подпись неявно раскрывает ваши обязательства, не занимая дополнительного места в транзакции. Этот подход имеет ряд преимуществ:
...
- [Multisig](https://planb.academy/resources/glossary/multisig) перед Taproot: когда у вас есть несколько подписантов, вам нужно решить, какая подпись будет нести _обязательство_. Подписи могут быть расставлены по-разному, и если подписант отказывается, вы теряете контроль над результатом _обязательства_;
...
На практике **sig tweak** также не очень совместим с существующим оборудованием ([аппаратными кошельками](https://planb.academy/resources/glossary/hardware-wallet)) и форматами (Lightning и т. д.). Так что эту замечательную идею трудно реализовать на практике.

***Ключевой твик (оплата по контракту) :***

В **key tweak** используется историческая концепция _pay-to-contract_. Мы берем открытый ключ `X` и изменяем его, добавляя значение `H(message)`. В частности, если `X = x * G` и `h = H(message)`, то новый ключ будет `X' = X + h * G`. Этот модифицированный ключ скрывает приверженность к `сообщению`. Владелец оригинального [закрытого ключа](https://planb.academy/resources/glossary/private-key) может, добавив `h` к своему закрытому ключу `x`, доказать, что у него есть ключ для расходования выходных данных. В теории это элегантно, потому что :
...
- Вам нужно адаптировать свои [скрипты](https://planb.academy/resources/glossary/script), дескрипторы и т.д.
...
- Далее идет 32-байтный _коммитмент_ (обычно это [корень Меркла](https://planb.academy/resources/glossary/merkle-root) из **MPC**), к которому мы добавляем 1 байт **Nonce** (всего 33 байта для этой второй части).
...
- `G`: порождающая точка [эллиптической кривой](https://planb.academy/resources/glossary/elliptic-curve) [secp256k1](https://en.bitcoin.it/wiki/Secp256k1).
...
- Состояние **контракта**, указывающее на текущую конфигурацию контракта (права, остатки, переменные и т.д.);
...
- **blinding**: случайное число из 8 байт, для усиления конфиденциальности и предотвращения попыток [грубой силы](https://planb.academy/resources/glossary/brute-force-attack) для идентификации UTXO;
...
- **Fungible**: представляет собой количество, которое можно менять (как жетоны). В открытой форме у нас есть `сумма` и `вязка`. В скрытой форме у нас есть единственное обязательство *[Педерсена](https://planb.academy/resources/glossary/pedersen-commitment)*, которое скрывает количество и ослепление;
...
- Это нельзя сравнить с классическим [хардфорком](https://planb.academy/resources/glossary/hard-fork) блокчейна, так как RGB работает в режиме проверки на стороне клиента и не влияет на общую совместимость блокчейна;
...
Откат означает, что ранее действовавшее правило становится недействительным. Таким образом, это "ужесточение" правил, но, строго говоря, не [софтфорк](https://planb.academy/resources/glossary/soft-fork):
