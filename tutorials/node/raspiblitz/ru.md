...
Raspiblitz — это отличная система для запуска Bitcoin-узла и связанных приложений. Я рекомендую его вместе с узлом MyNode большинству пользователей (идеально иметь два узла для избыточности). Одним из главных преимуществ является то, что узел Raspiblitz является «Free Open Source Software», в отличие от MyNode или Umbrel. [Почему это важно? Объясняет Влад Коста.](https://bitcoin-takeover.com/why-bitcoin-free-open-source-software-matters/amp/?__twitter_impression=true) Вы также можете запускать Raspiblitz через WiFi-соединение, а не через ethernet – вот [дополнительное руководство](https://armantheparman.com/headless-wifi/) для этого. (Я не нашел способа сделать это с MyNode).

Вы можете купить готовую ноду с прикрепленным мини-экраном, или можете собрать ее самостоятельно (экран не нужен).

[Руководство на странице GitHub](https://github.com/rootzoll/raspiblitz) отличное, но, возможно, слишком детальное для пользователя со средним опытом. Мои инструкции будут более краткими и, надеюсь, проще для выполнения.

По сути, процесс очень похож на процесс настройки [узла MyNode](https://armantheparman.com/mynode-bitcoin-node-easy-setup-guide-raspberry-pi/) с Raspberry Pi 4. Руководство Raspiblitz предлагает купить монитор, но он действительно не нужен, и я бы этого не рекомендовал. Вам даже не нужна дополнительная клавиатура или мышь. Просто получите доступ к терминальному меню устройства через компьютер в той же домашней сети и используйте команду ssh в терминале. Это возможно в Linux/Mac (легко) и немного сложнее в Windows.
...
Перейдите на [сайт GitHub Raspiblitz](https://github.com/rootzoll/raspiblitz) и найдите ссылку «download image»:
![image](assets/4.webp)

Sha-256 хэш загруженного файла указан на сайте. Он будет изменяться с каждым обновлением. Если вы не понимаете, о чём идёт речь, вам следует разобраться, поэтому я написал [руководство, которое вы можете прочитать здесь.](https://armantheparman.com/gpg/)
...
Вот [полезное видео для Linux, но оно также подходит для Mac](https://youtu.be/id3DGvljhT4?list=PLtK75qxsQaMLZSo7KL-PmiRarU7hrpnwK).

Для Windows вот [простой учебник](https://www.youtube.com/watch?v=MBBWVgE0ewk&t=1s).
_ОБНОВЛЕНИЕ: Проверка pgp/gpg теперь доступна. Вам понадобится открытый ключ Openoms. [Вот он](http://parman.org/downloadable/openoms.txt) (возможно, потребуется режим инкогнито, чтобы ссылка работала – http, не https)_
Mac/Linux
...
Для Windows необходимо установить [putty](http://putty.org/), чтобы подключиться к Pi через ssh. Введите ту же команду, что и выше.
