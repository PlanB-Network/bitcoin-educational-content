...
objectives:
- 理解支撐比特幣的核心概念和技術
- 獲得比特幣安全、軟體開發和網路治理方面的實用技能
- 掌握閃電網路及其相關協議
- 探索支撐交易安全和網路完整性的密碼學原理
---
# 踏上 CUBO+ 比特幣開發課程之旅！
...
在整個課程中，學員將利用關鍵工具來幫助他們理解並提升學習體驗。使用 Linux、命令列 Interface、GitHub 和 Docker 將是提供 Bitcoin 開發實作方法不可或缺的一環。這些工具將有助於使用 Bitcoin 軟體堆疊、管理開發環境，以及在實際環境中進行專案協作。
...
儘管 Bitcoin 具備潛力，但仍面臨著重大的挑戰。在非洲和印度等地區，電力和網際網路存取等基本基礎設施往往缺乏，限制了技術的採用。此外，**數位包容** - 確保所有年齡層和教育程度的人都能使用技術 - 仍是一大障礙。
...
- 稀有性：** Bitcoin 的固定 Supply 可確保其長期價值。
- 時間偏好：** 鼓勵為未來儲蓄，而非立即花費。
...
Bitcoin在**Cypherpunk運動**和**奧地利經濟學**的基礎上，使其成為一種獨特且革命性的貨幣形式。了解這些原則有助於掌握 Bitcoin 的創立原因以及其現今的運作方式。如需進一步閱讀，請考慮**Saifedean Ammous**所著的**Bitcoin Standard**。
...
- _Proof-of-Work (PoW)：_ 參與者必須解決計算謎題以驗證交易並保護網路安全的系統。
...
在第四模組的第一講中，我們探討了 Bitcoin 的架構和 Bitcoin core 節點的安裝。
...
歡迎回來！在上一節中，我們介紹了 Bitcoin 架構背後的基本概念，包括其加密基礎和對等網路結構。今天，我們將透過示範如何安裝和設定 Bitcoin 節點，從理論走向實踐。
...
- 強化網路**：透過參與網路，您可以協助保持網路的分散性，讓 Bitcoin 更能抵擋攻擊。
- 修剪 Blockchain**：只保留最近的交易，減少儲存需求，如果您的磁碟空間有限，這是最理想的選擇。
- 使用先進的 Wallet 功能**：以隱私與安全的方式管理您的 Bitcoin、離線使用 generate 私密金鑰，以及安全地簽署交易。
- 直接與 Bitcoin 網路互動**：使用 Bitcoin core，您可以直接與網路互動，無須經過中介，確保您能獲得最精確的資料。
- 受益於增加的隱私**：身為 Full node 的操作者，您不必信任外部服務，可保護您的交易隱私不受外部監控。


運行 Bitcoin 節點的好處對任何專注的比特幣玩家來說都是巨大的。您不僅可以幫助保護網路安全並加強其分散性，還可以提高您的隱私，確保您自己交易的完整性，並在 Bitcoin 生態系統中發揮積極作用。運行一個節點是實現金融主權和完全接受 Bitcoin 分散性的關鍵一步。
...
首先，下載最新的 Bitcoin core 版本，並驗證檔案的哈希值，以確保沒有發生篡改。
...
C-Lightning 也稱為 Core-Lightning，是一種 Layer 2 協定，可使用 off-chain 通道促進更快速、更便宜的交易。它的突出之處在於其模組化和開發人員友善的架構，可透過外掛程式進行廣泛的客製化。
...
**在 Testnet 上設定 C-Lightning 以連線 Bitcoin core**
...
按照這些步驟，您就可以將功能完整的 C-Lightning 設定連接到 Bitcoin core 節點，準備進行 Testnet 交易。


#### 結論與問題


總括而言，今天我們介紹了重新安裝 Bitcoin core 的基本步驟，接下來是安裝和設定 C-Lightning 的詳細步驟。如果您有任何問題，歡迎現在提出，或準備在下一課中進一步說明。請記住，實際操作經驗是非常重要的，因此請使用我們討論過的 Testnet 設定來獲得更多的啟發。
...
Ledger 裝置是廣受歡迎的 Hardware Wallet，因其強大的安全性而備受稱頌。然而，Ledger 背後的公司卻因為各種關於使用者資料隱私的爭議而備受審查。了解實體裝置的安全性與該公司的作法之間的差異，對於知情使用而言至關重要。


**安全模式：multi-sig 錢包和多樣化硬體的重要性**


Bitcoin 安全性的一個關鍵方面是利用多重簽章 (multi-sig) 電子錢包。multi-sig 電子錢包需要多個私人密碼匙來授權交易，大大提高了安全性。此外，使用不同類型的硬體錢包可分散風險並加強安全模式。
...
安裝過程的第一步是從 Specter 的官方資源庫下載 Specter。驗證下載的完整性以避免軟體受損是非常重要的。下載完成後，在桌面上安裝 Specter 並啟動應用程式。


**設定 Specter 與 Bitcoin core 或 Electrum 伺服器連線**


要配置 Specter，需要將其連接到 Bitcoin core 或 Electrum 伺服器。這些伺服器為 Wallet 的操作提供必要的 Blockchain 資料。配置包括在 Specter 的設定中設定伺服器 Address，並確保連線穩定。


**解釋衍生路徑和公開金鑰檢索**


瞭解衍生路徑對 Wallet 管理非常重要。衍生路徑定義密鑰如何從主密鑰產生。在 Specter 中，您可以透過連線 Hardware Wallet（或模擬器）並瀏覽 Wallet Interface，擷取公開金鑰。請確保記錄這些路徑，以供日後參考。


**實用示範：使用 Ledger 模擬器**


我們現在要使用 Ledger 模擬器來取得金鑰。這包括將模擬器連接至 Specter，導覽至金鑰管理部分，並選擇適當的金鑰以建立 Wallet。
...
遇到問題是不可避免的。實際的除錯工作包括檢查使用者權限、驗證資料目錄存取權限，以及查看日誌是否有錯誤。例如，確保 Specter 具有存取 Bitcoin core 資料目錄的必要權限，以避免作業中斷。


**範例問題：資料目錄存取**


常見的問題是資料目錄存取不正確。請確認 Specter 設定中 Bitcoin core 資料目錄的路徑是否正確。這可確保 Specter 能存取必要的 Blockchain 資料，以執行 Wallet 作業。


**下一步與整合**


正如我們所總結的，接下來的步驟包括將 Specter 與 Lightning Network 整合。這樣就可以從 Specter 向 Lightning 節點發送資金，促進更快更便宜的交易。未來的課程將詳細介紹此整合，強化您的 Bitcoin 交易能力。


**lock timing variability**


瞭解區塊時序的變化至關重要。Bitcoin 區塊可能會以不同的間隔進行挖掘，從而影響交易確認時間。在所有配置和 Wallet 作業中都必須考慮到這種可變性。
...
擁抱自給自足的哲學。實用技能和自學是最重要的，往往超越正規教育的效益。參與實際操作，加深您對 Bitcoin 安全性的瞭解。


**隱私權考量**


避免使用會追蹤或記錄交易的服務，以維護隱私。匿名性對於安全的 Bitcoin 作業至關重要，謹慎選擇服務有助於保護您的身份和交易歷史。
...
十多年來，Bitcoin 已經證明是金融界的轉型創新，在全球範圍內成功運作，並為數位經濟開啟了新的可能性。然而，它仍然面臨挑戰，需要具創意的合作解決方案。Bitcoin 的持續演進為有興趣塑造分散式金融未來的人提供了獨特的機會。
...
儘管 Bitcoin 已經存在超過十年，但仍然面臨重大的可用性挑戰。用戶可使用的工具和介面往往缺乏較傳統金融系統的成熟性和易用性。這一點在薩爾瓦多等地區尤為明顯，在這些地區，Bitcoin 的採用得到了政府的認可。這方面的主要問題是需要更好的抽象，以簡化使用者體驗，使 Bitcoin 即使是技術知識極少的個人也能使用。
...
保護 Bitcoin 資產安全是一項複雜的任務，充滿挑戰。通常用於日常交易的 Hot 錢包，會構成重大的安全風險，尤其是對那些操作 Lightning 節點的人而言。此外，Bitcoin 資產的繼承規劃仍然是一個迂迴曲折且經常不安全的過程。這些安全措施的複雜性會讓潛在使用者卻步，也讓廣泛採用變得複雜。


#### 隱私權的公開問題


隱私權是 Bitcoin 生態系統中的另一個關鍵問題。雖然隱私權對於安全性而言至關重要，但 Bitcoin 目前的架構所提供的隱私權功能有限。On-Chain 交易很容易被追蹤，對使用者的匿名性構成風險。雖然 Lightning Network 有提升隱私權的潛力，但仍需大幅改善。透明度與隱私權之間的平衡非常微妙，需要創新的解決方案來確保使用者的安全性與隱私權。


#### 彈性方面的未解決問題


Bitcoin 通訊協定的彈性對於改善隱私、安全性和擴充性是必要的。然而，過高的彈性也可能成為漏洞，可能成為攻擊媒介並威脅網路的分散性。取得適當的平衡對於維持 Bitcoin 通訊協定的完整性和彈性至關重要。
...
增強 Bitcoin 可用性的努力往往是以犧牲安全性和隱私為代價的。例如，Satoshi 的 Wallet 等用戶友好型保管錢包提供了可使用的 Interface，但在安全性和隱私性方面卻大打折扣。簡化的系統可能會增加可用性，但卻可能導致 Address 重複使用等問題，破壞隱私。因此，任何可用性的改善都必須小心權衡潛在的安全性與隱私權的取捨。
...
在 Bitcoin 中，不同的隱私權處理方法都有各自的取捨。隱私權模糊化 (Privacy by obfuscation) 是指加入更多的資訊來模糊相關的資料，可以提高隱私權，但可能會使網路變得複雜。例子包括 Monero 和 Zcash。另一方面，透過省略來隱私，目的是減少 On-Chain 資訊，如 Lightning Network 所見，可以同時改善隱私和擴充性。每種方法都有其優點和缺點，因此必須採用細微的方法來增強隱私性。
...
在 Bitcoin Wallet 開發過程中遵守標準化實踐對於確保易用性和安全性至關重要。目前許多錢包並不遵循既定標準，導致分散和潛在漏洞。標準化可以顯著改善用戶體驗和 Bitcoin 交易的整體安全性。


傳統的12字備份詞組雖然對基本的Bitcoin使用有效，但在容納off-chain通訊協定（如Lightning Network）方面卻有不足。未來的備份標準需要不斷演進，為這些進階功能提供更好的安全性與可用性，確保使用者能在 Bitcoin 生態系統的不同層級安全地管理資產。


透過統一的通訊協定來簡化付款流程，對於提升使用者體驗是非常重要的。BIP70、BIP78 和 Payneem 等現有通訊協定提供了各種解決方案，但仍有進一步創新的空間。更精簡、更易於使用的支付通訊協定可以促進更廣泛的採用和易用性。


開發更好的工具和硬體對於改善 Bitcoin 的可用性和安全性至關重要。硬體錢包（如 Ledger 和 Trezor）等創新產品提供了強大的安全解決方案，但必須持續發展，以應對 Address 新興的威脅。改良後的工具可以讓更多人更容易使用 Bitcoin，也更安全。


降低與 Hardware Wallet 發佈相關的風險並確保其完整性至關重要。Supply 連鎖攻擊對這些裝置的安全性構成重大威脅。實施嚴格的安全措施並確保生產和分銷過程的透明度，有助於降低這些風險。


簡化使用者與 Bitcoin 和 Lightning Network 的互動，同時維持安全性和效率，是我們的主要目標。更好的使用者經驗抽象可以讓 Bitcoin 更容易被非技術使用者使用，在不影響安全性的前提下，促進更廣泛的採用。
...
基礎 Layer (Layer 1) 的創新具有挑戰性，但對於 Bitcoin 的長期演進卻至關重要。Layer 2 解決方案（如 Lightning Network）允許更多的實驗性變更，並能更靈活地 Address 擴展性和隱私權問題。這兩層在 Bitcoin 的持續發展中都扮演著關鍵的角色。


** 協調共識**


Bitcoin 協定的變更需要大量的協調與社群共識。Bitcoin 的分散性使得這個過程本身就充滿挑戰。有效的協調和明確的溝通對於處理複雜的通訊協定變更和確保成功採用改善措施是非常重要的。


**可擴充性挑戰**


達成全球共識和管理複雜的次要層 (例如 Lightning Network)，對擴充性提出了挑戰。這些問題必須加以解決，以確保 Bitcoin 能容納不斷增加的交易量，同時維持其安全性與分散性的核心原則。


總而言之，持續解決這些開放性問題，並在 Bitcoin 生態系統中進行創新，對於其演進至關重要。可用性、安全性、隱私性和可擴充性之間的平衡需要謹慎的考量和共同努力。透過對這些發展的貢獻，參與者可協助塑造 Bitcoin 的未來及其在全球金融版圖中的角色。
...
雖然安全性與可靠性的目標都是維護系統的完整性，但兩者的方法卻有很大的不同。可靠度工程處理的是隨機事件造成系統故障的可能性，通常會運用統計方法來預測並降低這些故障。另一方面，安全性必須考慮到攻擊的蓄意與智慧性，因此需要採用稱為「深度防禦」的多層防禦策略。
...
在危地馬拉，為選舉結果加上時間戳記，對於確保選舉程序的完整性發揮了關鍵作用。透過使用加密方法對 Timestamp 資料進行加密，選舉官員可以提供竄改過的證據，證明選舉結果的真實性，從而阻止受重大經濟誘因影響的潛在操縱者。
...
自由與開放源碼軟體 (FLOSS) 的使用在 Bitcoin 的生態系統中至關重要。Peter Todd 探討了 FLOSS 對 Bitcoin 的重要性，探索了 FLOSS 的歷史，並檢視了 Github 如何讓我們協同建立像 Bitcoin 這樣的開放原始碼軟體。
...
管理軟體的法律架構主要圍繞著作權法。這些法律賦予軟體創造者使用、修改及散佈其作品的專屬權利。然而，開放原始碼授權條款提供了一個在特定條件下與大眾分享這些權利的機制。這個法律結構對於了解軟體散佈與修改的動態是非常重要的。
...
Bitcoin core 就是一個成功的例子，它是 Bitcoin 基礎架構的重要部分。在 Bitcoin core 上工作的開發人員通常是透過補助金、捐款，以及從專案成功中獲益的組織所提供的贊助來獲得資金。此模式可讓開發人員專注於軟體的改進，而不受傳統商業資金的限制。
...
**安全特性**：加密 Hash 函式必須遵守幾項安全特性。預映像抵抗可確保從 Hash 輸出逆向工程原始輸入在計算上是不可行的。其次，抗前影像性意味著很難找到不同的輸入來產生相同的 Hash 輸出。碰撞阻抗可確保找不到兩個不同的輸入會產生相同的 Hash 輸出。


**Merkle 樹**：Merkle Tree 是一種資料結構，能夠有效且安全地驗證大型資料集。資料項成對散列，所產生的散列值反覆組合，形成單一根 Hash。在 Bitcoin 中，Merkle 樹在區塊建立和交易驗證中至關重要，特別是對於簡化支付驗證 (SPV) 客戶端和 Taproot (Mast) 中。


![Image](assets/en/016.webp)


**數位簽章 (ECDSA)**：橢圓曲線數位簽章演算法 (ECDSA) 用於確保 Bitcoin 交易的真實性和完整性。它涉及使用私密金鑰產生簽章，而該簽章可使用對應的公開金鑰驗證。主要概念包括瞭解有限域、離散對數和非ces 的重要性。


**橢圓曲線**：橢圓曲線因其效率和安全性而被用於公開密碼匙加密法。橢圓曲線加密法的安全性取決於離散對數問題的難解程度。
...
**公鑰加密**：離散對數法和有限字段是 Bitcoin 密碼計算的基礎。挑戰回應通訊協定用來驗證私人密碼匙的知識，而不會洩露密碼匙。
...
在本講座中，我們探討了 Bitcoin 安全性與功能的基礎密碼概念。從 Hash 函數、Merkle 樹和數位簽章的關鍵作用，到橢圓曲線密碼學的複雜數學，這些 Elements 構成 Bitcoin 分散式網路的骨幹。了解這些概念不僅僅是掌握理論，更重要的是認識到實際世界開發中的實際影響和潛在陷阱。


身為 Bitcoin 開發人員，必須謹慎且精確地進行加密實作。Bitcoin 網路的安全性在很大程度上取決於這些加密原則的正確與安全應用。無論您是驗證交易、設計新功能，或是確保 Blockchain 的完整性，對加密學的深入瞭解將使您能夠在 Bitcoin 生態系統內建立更穩健、更安全、更創新的解決方案。
...
Bitcoin 是一種數位貨幣，以共識通訊協定運作，這是一套由網路參與者同意的規則，以確保統一性和功能性。Bitcoin 的核心是被稱為 Blockchain 的分散式 Ledger，交易由網路節點記錄和驗證。完整節點儲存 Bitcoin Blockchain 的全部歷史，在維護此 Ledger 的完整性上扮演著重要的角色。其他類型的節點，例如存檔節點、pruned 節點和 SPV（簡化支付驗證）節點，也以各種方式為網路做出貢獻。共識協定可確保所有這些節點對 Blockchain 的狀態達成一致意見，使 Bitcoin 穩健地對抗檢查和欺詐。
...
Bitcoin core 及其替代實作都有監督軟體開發與維護的維護人員。這些維護人員負責合併程式碼變更，確保這些變更符合共識規則，並且不會引入弱點。


#### Soft 前叉 vs Hard 前叉


Soft 分叉是收緊 Bitcoin 通訊協定現有規則的變更，使某些先前有效的交易無效。它們是向後相容的，這表示未升級的節點仍能識別新規則。Soft Fork 的一個例子是 2010 年對溢出錯誤的修復，防止憑空創造金錢。


Hard 分叉是鬆動現有規則的變更，允許新類型的交易。這些並非向後相容，意即未升級的節點將無法辨識新規則。2106 年的問題可能需要 Hard Fork 的範例，以確保 Bitcoin 在此日期之後仍能繼續運作。
...
幾個真實案例說明了 Bitcoin 治理的實際運作。2010 年的溢出錯誤修復是 Soft Fork，解決了一個重要的缺陷。2106 年的問題可能需要 Hard Fork 來 Address 其影響。從最長工作鏈到最多工作鏈的過渡反映了一個影響如何達成共識的重要治理決策。
...
運行中的節點可以驗證 Bitcoin 規則並參與網路，即使使用不同的通訊協定，例如 Blockstream Satellite。這強調了 Bitcoin 的共識機制與網路使用的資料通訊方式之間的分離。節點的經濟意義，尤其是那些由大型實體 (如 Binance) 所經營的節點，會影響改變的採用。這些實體在網路中擁有大量的經濟利益，可以透過營運有影響力的節點來左右決策。
...
在 Bitcoin 中，多數規則扮演著重要角色。如果 60% 的礦工採用新規則，他們的區塊就會被運行原始 Bitcoin core 的礦工拒絕，導致分裂。Bitcoin Satoshi's Vision (BSV) 就是一個因缺乏社區支持而導致 Hard Fork 失敗的例子。


讓我們簡單回顧一些重要的概念。


**Forced Soft Fork**：實施限制性規則來變更 Bitcoin 的概念可能會導致進一步的分裂和管理問題。此方法說明 Bitcoin 社群內的複雜性和潛在衝突。


**51% 攻擊**：51% 攻擊描述了一種情況，大多數雜湊能力可以透過 Mining 空區塊攻擊 Bitcoin。除非社群採用新的共識規則來 Address 此攻擊，否則這可能會有效地殺死網路。


**Check-Lock-Time-Verify (CLTV)**：Check-Lock-Time-Verify (CLTV) 是以 Soft Fork 實施治理變更的範例。CLTV 可確保交易僅在特定時間後才有效，這對付款通道和備份金鑰非常有用。這項變更使用之前什麼都不做的操作碼來收緊規則。
...
3. **隱私和控制**：運行您自己的節點可確保您不必依賴外部節點，因為外部節點可能會追蹤您的交易和 Wallet 結餘，從而危及您的隱私。
...
- 保存區塊和最重鏈的歷史**：節點會儲存自己的 Blockchain 複本，這可讓節點驗證交易和區塊的真實性。
- 維護有效候選清單；Mempool**：節點必須在 Mempool 中保留可能的交易候選人清單，以納入區塊中。
...
- RPC (遠端程序呼叫)**：Bitcoin core 包含一個 RPC Interface，允許外部應用程式 (例如錢包) 與節點互動。
- 儲存區塊和鏈狀態**：Bitcoin core 可以儲存整個 Blockchain，也可以不儲存，無論是存檔或 pruned 節點。它也會在磁碟上儲存網路的目前狀態 (The UTXO set)。
...
- Mempool (blocksonly)**：對於希望盡量減少頻寬使用量的使用者，執行「blocksonly」節點是一個解決方案，節點只處理區塊，忽略交易。
...
運行 Bitcoin 節點是參與 Bitcoin 網路的強大方式，可讓您驗證交易、維護隱私，並為 Blockchain 的安全性和分散性做出貢獻。無論您選擇執行 Full node 或透過修剪 Blockchain 或停用某些元件來自訂您的設定，瞭解 Bitcoin 節點的核心功能和安全考量，將使您有能力做出明智的決定，並為 Bitcoin 的持續演進做出貢獻。
...
本講座的主要目的是引導您通過在 Rust 中編寫解析器來解析 Bitcoin 區塊。這包括瞭解 Bitcoin 區塊和交易的結構，並實作必要的邏輯來擷取和詮釋這些資料。


### 解析 Bitcoin 區塊和 Rust 中的交易
...
- 版本**：表示區塊的版本。
- 上一區塊**：參考 Blockchain 中的上一個區塊。
- Merkle Root**：一個 Hash 代表區塊中所有交易的合併 Hash。
- Timestamp**：區塊被挖出的時間。
- 位元**：有效區塊 Hash 的目標臨界值。
...
- 輸出**：包含 Value 和 ScriptPubKey。
  - 價值**：交易值。
  - PubKey script 長度**：PubKey script 的長度。
...
雖然有可能出現惡意的 Mining 空區塊，但尚未觀察到。出現空區塊的主要原因是時間限制，而非惡意意圖。


#### 空區塊的影響


出現空區塊是 Mining 程序的正常現象，主要是由於時序問題。雖然這些區塊不包含交易，但仍會延展 Blockchain，並有助於網路安全。


#### 安全的重要性


Bitcoin Mining 的安全性至關重要。透過遵守最佳實務和使用經過嚴格審核的函式庫，礦工和開發人員可以確保 Blockchain 的完整性，並防止潛在的漏洞。


總而言之，在 Rust 中解析 Bitcoin 區塊和交易，需要了解複雜的結構，並實作有效率的解析技術。處理特殊情況和腳本解析需要謹慎考量，而專注於效率和安全性可確保 Bitcoin 網路的穩健性。
...
Daniela Brozzoni 全面概述 Bitcoin Layer 1 軟體堆疊，解釋構成 Bitcoin 通訊協定基礎的層次 (即 Bitcoin 節點和 Bitcoin 荷包)，以及如何透過介紹 Bitcoin 函式庫和深入研究 Bitcoin 開發套件 (BDK) 來建立 Bitcoin 軟體。
...
- 完整節點**：這些節點儲存整個 Blockchain，並驗證所有交易和區塊。這些節點提供高度的安全性，對於網路的去中心化是不可或缺的。



  - 存檔節點**：歸檔節點是完整節點的子集，可保留所有 Blockchain 資料，對於歷史分析和除錯很有價值。
...
![archival node](assets/en/037.webp)
  ...
  - pruned 節點**：pruned 節點只保留 Blockchain 的一部分，剔除不再需要驗證的舊資料，節省磁碟空間。
...
Bitcoin core 是使用最廣泛的 Full node 實作。它同時執行 Full node 和 Wallet 的雙重功能。Bitcoin core 的主要方面包括



- 可用性**：可透過命令列 Interface (CLI) 和圖形化使用者 Interface (GUI) 使用。
...
- Bitcoin Knots**：提供比 Bitcoin core 更先進的功能，佔用更多空間和記憶體。
- LibBitcoin**：靈活且模組化的實作。
- btcd**：以 Go 撰寫，提供不同的設計哲學。


實施這些替代方案有其本身的風險，尤其是在共識規則方面。偏離既定的驗證規則可能會導致分叉或不一致。Bitcoin Kernel 專案希望透過集中共識程式碼來降低這些風險，以確保各種實作的一致性。
...
1. **保管人與非保管人**：
   ...
   - 保管人錢包**：由第三方管理，提供便利，但需要信任託管人。
   - 非保管人錢包**：由使用者控制，提供更高的安全性和隱私性。
...
3. **On-Chain 對閃電**：
   ...
   - 閃電錢包**：促進更快速、更便宜的交易 off-chain。


4. **Cold 錢包 vs. Hot 錢包**：


   - Cold 錢包**：不連線至網際網路，提供絕佳的安全防護。
   - Hot 錢包**：與網際網路連線，提供更多存取便利性，但安全性較低。


#### Cold Wallet 安全性


Cold 電子錢包因其安全性而備受推崇。由於保持離線狀態，它們天生就能抵禦線上黑客的攻擊。然而，確保透過 Cold 電子錢包進行的交易是安全且準確的，以防止不慎將 Bitcoin 傳送給惡意行為者，這一點至關重要。
...
不論類型為何，每台 Bitcoin Wallet 都能執行三項基本功能：
...
鑒於所有 Bitcoin 錢包都有類似的功能，重複重寫 Wallet 邏輯的效率很低。這就是 Bitcoin 開發套件 (BDK) 發揮作用的地方。
...
BDK 透過提供建構在 Rust Bitcoin 之上的更高階功能，簡化了 Wallet 的建立。它透過綁定支援多種程式語言，包括 Kotlin、Swift 和 Python。
...
- generate 位址**：建立新地址以接收來自 Testnet Faucet 的 Bitcoin。
...
指定並列印 transaction ID（txid）可以在 Mempool.space 等平台上進行監控。使用 `Blockchain.broadcast` 方法可以完成交易的廣播，而驗證交易的詳細資訊和狀態是確保成功傳播的關鍵。


#### BDK 效用與隱私權考量


BDK 對於簡化 Bitcoin Wallet 開發具有無價的價值。若要增強隱私性，建議使用 Electrum、Explora 等工具和個人 Bitcoin core 節點。


#### 程式語言


開發 Bitcoin 專案時，Rust 因其安全性與效率，通常是首選。不過，語言的選擇可能會根據特定的專案需求和開發人員的專業知識而有所不同。
...
透過瞭解這些元件，從 Bitcoin 節點和錢包到 Bitcoin 開發套件 (BDK)，您可以更有信心和能力地瀏覽 Bitcoin 生態系統。這些知識讓您能夠開發穩健且安全的 Bitcoin 應用程式，為這項革命性技術的持續演進做出貢獻。
...
雖然 Bitcoin 的分散性是其核心優勢之一，但也帶來了重大的挑戰。Blockchain 的公開性意味著所有人都能看到所有的交易，這引起了隱私權的疑慮。此外，由於 Mining 的權力集中在少數大型實體手中，因此需要在眾多節點之間達成共識，這可能會導致集中化的壓力。
...
針對 Address 的低效率與隱私權問題，有人提出支付管道作為可行的解決方案。微支付通道允許交易發生 off-chain，減少了整個網路不斷分享資料的需求。這可大幅減輕 Blockchain 的負擔，讓交易更快速、更便宜。


支付通道背後的基本原則是採取交易 off-chain 的概念。當事人可以開啟一個付款通道，在他們之間進行許多交易，而不是將每筆交易廣播到整個網路。只有通道的開啟和關閉才會被記錄在 Blockchain 上，大大提高了效率和隱私性。


儘管支付通道具有 off-chain 的性質，但仍可選擇強制執行交易 On-Chain。如果發生爭議或有一方試圖作弊，就可以將通道的最新狀態廣播給 Blockchain，確保協議的交易得以兌現，並正確分配資金。


支付通道代表了 Blockchain 技術的重大躍進，提供了一種可擴展且安全的交易方法，同時解決了與 Bitcoin 相關的許多基本問題。隨著我們在這些基礎上不斷創新和發展，Blockchain 的未來看起來越來越有希望。


總括而言，瞭解 Bitcoin 的歷史背景與挑戰，以及透過 MHL、Lightning Network 和付款管道提出的創新解決方案，就能全面掌握 Blockchain 技術的現況與未來潛力。
...
在之前的討論中，我們涵蓋了基本付款管道的基本原理。這些通道允許兩個參與者，例如 Alice 和 Bob，直接進行無縫交易。但是，這種模式有一個明顯的限制：Alice 只能與 Bob 進行交易，而不能與其他參與者（如 Charlie）進行交易，除非她與每個參與者建立獨立的通道。由於 Alice 不可能與她需要交易的每個人都開通直接管道，因此必須使用多個管道，這導致了低效率和擴充性的問題。
...
在 HTLC 中，Alice 以秘密「R」的 Hash 鎖定付款。Bob、Carol 和 Diana 各自與後續的中介人建立類似的契約，確保他們只有在提出正確的秘密 "R" 時，才能領回自己的資金。此機制可確保原子性；付款完全完成或失敗，以防部分資金損失。
...
在實際情況中，Alice 的付款流程涉及透過 Bob、Carol 和 Diana 等多個中介向 Eric 付款。此鏈中的每位參與者都負責從前一位參與者抽取資金。


#### 頻道狀態更新


頻道根據參與者之間的相互協議和簽名更新其狀態。例如，Alice 和 Bob 不一定要使用秘密「R」，只要他們同意交易條款，就可以更新他們的頻道狀態。


#### 確保原子性


HTLC 機制透過使用時間鎖和簽章來確保原子性。此保障措施可確保支付通訊協定保證完全成功或完全失敗，防止部分資金損失。
...
Alice 必須瞭解網路拓樸，才能決定付款路由。儘管付款路由經過多個中間人的複雜性，但仍採用來源路由以維護隱私。
...
總而言之，適當的節點操作可確保原子支付，而 Lightning Network 旨在 Address Ripple 等傳統支付系統所面臨的許多問題。透過利用 HTLC 和策略路由，Lightning Network 為分散式支付提供了更具擴展性、更有效率且更安全的解決方案。
...
Bitcoin 網路以 Trustless 價值 Exchange 系統的方式運作，主要作為結算 Layer，交易記錄在公共 Ledger 上。這可確保安全性和不變性，但也有其限制，尤其是在交易速度和費用方面。因此，Bitcoin 對於日常的小額交易可能效率不高。


進入 Lightning Network，它的功能是在 Bitcoin Blockchain 之上的第二個 Layer。此支付網路旨在促進快速且低成本的交易。透過在雙方之間開通支付通道，他們可以在 off-chain 上進行交易，僅在 Bitcoin Blockchain 上記錄初始和最終餘額。這可大幅降低主網路的負載，增強可擴充性，並使微交易變得可行。


為了更好地掌握這個概念，請考慮酒吧帳單的比喻。當您在酒吧開帳時，您可以持續點飲料，但每次點飲料後都不需付款。最後，您會在晚上結束時結清總金額。同樣地，一個 Lightning 通道允許多次交易 off-chain，只有在通道關閉時才結算 On-Chain。另一個類比是機場，通過多個節點進行支付，就好比乘坐轉機航班到達目的地。每個節點 (或「航班」) 都會幫助您將付款引導到它需要去的地方，確保有效率的路由。
...
- 頻道設定訊息**：這些訊息在頻道建立階段非常重要。
- 頻道狀態訊息**：這些訊息處理活動頻道內的更新，確保雙方同步。
...
特定訊息如「Update Fee」會調整 Commitment 交易的 Miner 費用，而「Channel Reestablish」訊息則可確保雙方對等機構在斷線後保持同步。
...
掉期可在不關閉通道的情況下實現 On-Chain 提款和高效的流動性管理。目前正在開發未來的解決方案，例如拼接，以加強這個流程。
...
Lightning Network 中的網路發現功能已從最初使用的 IRC（網路中繼通訊）演進到 Bolt 7 定義的更複雜的通訊協定。 此通訊協定使用特定的 Lightning 訊息（通常稱為 gossip 訊息）來發現和維護網路拓樸。
...
- 公告簽章**：這可確保廣播訊息的真實性。
- 頻道更新**：此訊息傳達有關通道的更新，例如費用結構和最高 HTLC 金額。
...
DNS 和 Bitcoin DNS seed 查詢提供 IP 和節點資訊，有助於發現 Lightning 對等點。這種初始發現機制可幫助節點快速連接到網路。
...
儘管如此，透過 Lightning Network 傳輸語音資料會受到大量壓縮，並受到訊息大小的限制。Sphinx 就是一個應用範例，它探索如何創新使用 Lightning 來傳輸資料。


#### 用例與辯論


Lightning Network 的用途是一個持續爭論的話題。雖然 Lightning Network 主要是為支付而設計，但也在探索其他使用情況，例如資料傳輸，儘管並未獲得普遍接受。社群持續討論潛在的網路應用和通訊協定強化。


#### 社區討論


Lightning Network 社群充滿活力，不斷就用例、協定應用和潛在改進進行辯論和討論。這種協同合作的環境可促進創新，同時確保網路的發展能滿足使用者的需求。
...
Lightning Network (LN) 代表著 Bitcoin 在可擴展性和交易速度方面的重大突破。LN 客戶端（通常稱為 Lightning wallets）是專門的軟體或應用程式，可讓使用者透過 Lightning Network 進行交易。這些錢包是使用者與 LN 之間重要的 Interface，利用 off-chain 路徑促進即時結算、低費用的交易。


Lightning 錢包的設計是為了讓交易過程更人性化，即使只有極少技術知識的人也能從先進的 Bitcoin 功能中獲益。透過實現快速且具成本效益的微交易，這些錢包可大大促進 Bitcoin 在日常交易中的廣泛採用。


![LN Clients](assets/en/052.webp)_Lightning Wallets_


### Bitcoin 錢包 vs. Lightning 錢包


Bitcoin 錢包和 Lightning 錢包在架構和使用案例上有根本的不同，儘管它們有私密金鑰管理的共同特性：
...
- 私密金鑰的關注**：Bitcoin 電子錢包的首要焦點是誰持有私密金鑰。這決定了使用者資金的安全性和控制權。
...
- 這代表使用者可透過其管道接收的 Bitcoin 數量。它同樣重要，因為它確保其他人可以向使用者寄送付款。
...
考慮一個涉及 Alice、Bob、Charlie 和 Dan 的情境 - 典型的 LN 使用者透過各種管道互連：



- Alice 想要支付 Dan，但她與 Bob 的通路缺乏足夠的當地流動資金。
- 如果 Bob 有足夠的餘額，並且與 Charlie 有通道，而 Charlie 與 Dan 有通道，則 Alice 的付款可以經由 Bob 和 Charlie 到達 Dan。
...
#### Lightning Network 中的挑戰：



- 渠道枯竭**：隨著時間的推移，渠道可能會變得不平衡，資金集中在一邊，限制了交易能力。
- 連線問題**：有效率的交易路由需要強大的網路連線，而維護網路連線可能相當困難。
...
- 功能**：與 Breez 相似，利用 LSP 模型來管理流動資金，開放原始碼，著重於使用者的簡單性及有效的流動資金管理。
...
- 功能**：整合 On-Chain 和 Lightning 錢包、支援託管通道、開放源碼且具備進階功能，適合功能強大的使用者。
...
- 保管錢包**：第三方持有私人金鑰，通常提供自動流動性管理。例如 Satoshi 的 Wallet。
- 非保管型錢包**：使用者持有私人金鑰，可能需要手動流動性管理。例如 Breez 和 OBW。
...
總之，瞭解 Lightning Network 客戶的各個方面、其與傳統 Bitcoin 錢包的差異，以及有效管理流動資金的重要性，對於充分發揮 Lightning Network 的潛力至關重要。透過選擇正確的 Wallet 並積極參與生態系統，使用者可以大幅提升 Bitcoin 的交易體驗。
...
在本專題中，Asi0 將探討使用 Lightning Network (LN) 時所面臨的實際挑戰。儘管 Lightning Network 採用革命性的方式來擴充 Bitcoin 交易，但它仍提出了使用者和開發人員必須克服的幾項實際挑戰。我們將特別探討四大挑戰： ***流動資金管理**、**Layer 1/Layer 2 抽象**、**接收離線付款**以及**備份管理**。
...
在 Lightning Network 中，**流動資金**是指支付或收款所需的支付通道中的可用資金。使用者需確保擁有足夠的入站和出站流動資金，才能成功進行交易。例如，如果您想要接收付款，您必須有可用的入站流動資金，這表示另一個節點必須分配其部分餘額給您的通道。同樣地，如果您想發送付款，您的通道中就需要出站流動資金。



- 實際問題**：使用者經常發現很難平衡他們的通路並維持足夠的流動性。此外，重新平衡通道也會產生成本。
- 可能的解決方案**：一些 Lightning 錢包已開始整合自動通道再平衡，但此功能仍在開發中。用戶也依賴**閃電服務供應商（LSP）**來協助進行流動性管理。
...
- **LSP (Lightning Service Provider)**：協助使用者在 Lightning Network 上管理流動資金和通道的服務。
...
對使用者而言，**Layer 1 (L1)** （Bitcoin 的基礎 Layer）與**Layer 2 (L2)** （Lightning Network）之間的互動通常無法完全抽象化。例如，開啟和關閉通道需要 On-Chain Bitcoin 交易 (L1)，使用者必須為這些動作支付 On-Chain 費用。當 Bitcoin 網路擁塞時，這會帶來額外的複雜性和潛在的延遲。



- 實際問題**：用戶在與 Bitcoin base Layer 相對於 Lightning Layer 進行互動時，經常會因難以理解其複雜性而煩惱。這可能會造成費用、交易時間和安全性方面的困惑。
- 可能的解決方案**：改進的 Wallet 設計，可抽象 L1/L2 互動，並在後台管理通道開啟/關閉。有些錢包已經允許用戶根據情況在 On-Chain 和 Lightning 交易之間無縫切換。
...
- 實際問題**：確保使用者免於管理通路和 On-Chain 交易的複雜技術問題，同時在需要時維持透明度。
...
- **L2 (Layer 2)**：Lightning Network，在 Bitcoin 的基礎上運作，使交易更快、更便宜。
...
Lightning Network 的挑戰之一是**在使用者離線時接收付款。與 Bitcoin 的基礎 Layer 不同，Bitcoin 可以隨時接收交易，而 Lightning 則需要付款人和收款人都在線上才能完成交易。這對於許多希望在日常情況下使用 Lightning 付款的用戶來說是一個很大的限制。



- 實際問題**：除非使用者的節點是線上且連線至網路，否則使用者無法接收付款，這對於想要使用 Lightning 作為日常付款方式的人來說很不方便。
- 可能的解決方案**：一些變通方案包括使用託管式錢包，或在收款人節點上線前，仰賴第三方服務作為付款中介。
...
- 可能的解決方案**：目前正在進行**離線付款憑證**的研究，此憑證可讓收款人在重新連線至網路後申請付款。
...
- ** 離線付款**：一方未連接 Lightning Network 時發送或接收的付款。
...
透過解決這些挑戰，Lightning Network 將會持續成熟，成為擴充 Bitcoin 更堅固可靠的解決方案。
...
Bitcoin 經常以蜜獾作為化身，蜜獾以其堅韌和不屈而聞名。這個象徵正好代表 Bitcoin 堅強不屈的天性。就像蜜獾能抵擋毒液的侵襲而繼續茁壯成長一樣，Bitcoin 也在各種逆境中展現了非凡的韌性，包括監管挑戰、市場波動和技術攻擊。


**Bitcoin 的本質：不斷進化**

與一成不變的概念相反，Bitcoin 正處於永續演進的狀態。它的通訊協定和生態系統由全球的開發人員和研究人員社群持續改進和改善。這個演進過程是由增強安全性、可擴展性和功能性的需求所驅動，以確保 Bitcoin 在加密貨幣領域中保持領先地位。


### Lightning Network 的創新


**Lightning Network：快速發展**

Lightning Network 是 Bitcoin 的第二個 Layer 解決方案，用於擴大交易規模和加快交易速度，目前正在快速開發中。這款 Layer 可透過 off-chain 支付通道實現快速、低成本的交易。目前正在整合重要的創新技術，以提高其效率和可用性。


**Dual-funded channels**

傳統上，「閃電」頻道由一方提供資金。但是，雙方供資頻道允許雙方（例如 Alice 和 Bob）為頻道的流動性供資。這種增強功能有助於提高發送和接收容量的靈活性，但需要進行前期溝通，並制定新協議來管理聯合資金。
...
L2 機制引進了一種新方法來使舊通道狀態失效，而無需依賴懲罰機制。此更新依賴 SIGHASH_ANYPREVOUT，此功能需要 Bitcoin Soft Fork。L2 機制有望簡化通道管理並提高安全性。


**Bolt 12**

Bolt 12 可解決 Lightning Network 中使用的現有 Bolt 11 發票的限制。它引入了可重複使用的發票，並將流程自動化，僅在 Lightning Network 內運作，無需 HTTP 和 Web 伺服器。這項創新可簡化交易並提升使用者體驗。
...
Taproot 是一項重大升級，可整合交易複雜性並加強隱私權。Taproot 與 MuSig（多簽署交易協定）和 Schnorr Signatures 相結合，可提高交易效率。這些進步讓 Lightning 交易類似於一般的 Bitcoin 交易，簡化了流程並加強了隱私保護。


**PTLC 路由**

點時間鎖定合約 (PTLC) 是現有 Hash 時間鎖定合約 (HTLC) 的改進。PTLC 使用 Schnorr 簽署，並以公開金鑰取代共用機密，降低付款關聯性和濫用的可能性，從而改善隱私性。


**頻道廠商**

頻道工廠可建立多方頻道 (例如 4 對 4 Multisig)，進而產生新的 2 對 2 付款頻道 off-chain。此系統允許快速、免費地建立和關閉通道，儘管它需要所有參與者的合作。通道工廠增加了 Lightning Network 的整體擴充性和彈性。


**Watchtowers**

Watchtowers 是監控 Blockchain 舊通道狀態的第三方實體。如果偵測到違規行為，它們會公佈懲罰交易，以確保網路安全。雖然 Watchtowers 可以阻止不當行為，從而提高安全性，但它們也會引發有關監控交易的隱私問題。
...
Lightning Service Providers (LSP) 由 Breez Wallet 提出概念，旨在透過即時接收功能改善使用者體驗。LSPs為用戶打開通道，類似於互聯網服務供應商提供連接服務的方式。這項創新簡化了用戶上線流程，並確保在Lightning Network上進行無縫互動。


**保持更新的資源**

要掌握 Bitcoin 和 Lightning Network 的最新技術創新，必須善用寶貴的資源。Bitcoin OpTec 電子報、lightning dev 郵件清單，以及來自業界專家（如 Jason Lopp）的資料，都能提供有關這個快速發展領域的持續進步與研究的深入見解和最新資訊。
...
在深入探討 Lightning 付款的擴充與整合之前，了解 Lightning 付款的基本操作是非常重要的。傳統的 Lightning 付款涉及幾個關鍵部分：**付款人**、**收款人**和**Lightning Network**本身。付款人透過產生**Invoice**，向收款人啟動付款，其中包括要支付的金額和目的地（收款人的節點）等關鍵資訊。


此程序依賴於 **Hash 時間鎖定合約 (HTLC)**，以確保付款只能由合法收款人在指定時間內領取。此機制中兩個重要的 Elements 是 **洋蔥支出** 和 **HTLC 鏈**：



- 洋蔥路由**：透過分層封裝交易資料來提供隱私，確保每個中介只知道其前後的節點，而不知道整個路由。
- HTLC 連鎖**：一系列鎖定資金的合約，直到付款完成或還原為止。


加強 Lightning Network 功能的較新通訊協定是 **Keysend**。傳統方法需要寄件者與收件者事先溝通才能使用 generate 或 Invoice，與此不同的是，Keysend 可讓**寄件者主動付款**，簡化流程並改善使用者體驗。
...
- Bolt 12 提供**：此通訊協定與 LNURL 相似，但利用**洋蔥訊息**增強隱私性。Bolt 12 可讓使用者自動取得發票，無須手動介入，提高隱私性與可用性。
...
- **Bolt 12**：Lightning 的延伸，利用洋蔥訊息來保護隱私，同時自動化 Invoice 抓取。
- **Nostr**：整合 LP 通訊協定與使用個案的分散式平台
- 微交易付款。
- **RGB協定**：可透過 Lightning Network 傳輸資產 (如代用幣) 的通訊協定。
...
Lightning 付款也被整合到**Nostr**等平台和播客服務中，展現出這些協定的多樣性。此外，付款的**前影像**可用作**存取鑰匙**，以解鎖內容或服務，進一步增加 Lightning Network 的實用性。


先進的通訊協定，如 ** 點時間鎖定合約 (PTLC) **，透過啟用更複雜的加密操作，讓 Lightning 更進一步。PTLCs 改善了路由和付款分割，提高了安全性和效率。


**LNURL**和**Bolt 12**等協定可減少手動互動，從而簡化付款程序，確保 Lightning Network 更為人性化，並獲得廣泛採用。
