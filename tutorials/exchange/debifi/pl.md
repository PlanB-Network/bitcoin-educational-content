---
name: Debifi
description: Uzyskaj pożyczkę pozabankową gwarantowaną przez Bitcoin.
---
...
Przez wieki tradycyjne pożyczki umożliwiały finansowanie wielu projektów. Jednak nadal są one powolne, kosztowne i mało inkluzywne, zwłaszcza dla tych, którzy nie mają dostępu do solidnej infrastruktury bankowej.



Powstanie protokołu Bitcoin zapoczątkowało nową erę finansową, niosąc ze sobą szereg wyzwań. Jednym z tych wyzwań było uzyskanie płynności bez konieczności sprzedaży Bitcoin, a tym samym utraty ekspozycji na jego potencjał wzrostu



Rezultatem jest **Debifi**, platforma, która pozycjonuje się jako nowoczesna alternatywa dla banków. Cel jest jasny: uczynić kredyt tak prostym i przejrzystym, jak to tylko możliwe, łącząc zalety tradycyjnego systemu finansowego z wolnością oferowaną przez Bitcoin. Nazwa Debifi odzwierciedla tę wizję: ***Decentralized Bitcoin Finance***, skrót, który ilustruje spotkanie zdecentralizowanych finansów i innowacji Bitcoin.
...
- Wskaźnik Loan-to-Value (LTV)** pożyczki określa, ile gotówki można pożyczyć jako zabezpieczenie Bitcoin. Współczynnik LTV = kwota pożyczki / kwota zabezpieczenia * 100. Na przykład LTV w wysokości 50% oznacza, że wartość pożyczki jest równa 50% wartości zdeponowanego Bitcoin.




Samouczek wideo dotyczący sesji BTC:
...
## Rozpoczęcie pracy z Debifi



Aby rozpocząć korzystanie z Debifi, będziesz potrzebować kilku warunków wstępnych.
...
Zanim będziesz mógł pożyczyć od Debifi, upewnij się, że masz następujące przedmioty:
...
- KYC (*Know Your Customer*): w zależności od wybranej oferty pożyczki może być wymagany proces weryfikacji tożsamości. W Debifi każda oferta wskazuje, czy KYC jest wymagany, czy nie. Należy się więc odpowiednio przygotować. KYC jest przeprowadzany przez wiarygodnych zewnętrznych dostawców usług, takich jak Sumsub.
...
- Aplikacja mobilna Debifi (iOS/Android) jest wymagana do zaciągania pożyczek. Musisz pobrać aplikację, utworzyć konto i "połączyć" swoje urządzenie (rejestrując urządzenie na swoim koncie). Aplikacja Debifi obsługuje uwierzytelnianie dwuskładnikowe (2FA) i bez smartfona nie można potwierdzać transakcji w Debifi.
...
Zainstaluj aplikację zgodnie z typem posiadanego smartfona, a następnie otwórz ją.
...
Po wejściu do aplikacji kliknij menu **Ustawienia**.
...
Kod weryfikacyjny otrzymasz w wiadomości e-mail. Skopiuj i wklej ten kod do aplikacji. Następnie otwórz aplikację Debifi na swoim smartfonie i zaloguj się.
...
Dla bezpieczeństwa Debifi poprosi Cię o wykonanie trzech kroków.
...
- Po pierwsze, musisz ustawić silny kod PIN, aby później uzyskać dostęp do aplikacji.
...
- Następnie skonfiguruj uwierzytelnianie dwuskładnikowe (2FA), aby powiązać swoje urządzenie z kontem za pomocą kodu 2FA.
...
- Na koniec zapisz 12 słów klucza prywatnego, zapisując je na niezawodnym nośniku i przechowując w bezpiecznym miejscu. Ten etap jest niezbędny do odzyskania konta i zarządzania środkami.
...
- W celu zwiększenia bezpieczeństwa można nawet dodać passphrase.
...
Należy pamiętać, że tylko zarejestrowany smartfon będzie mógł otworzyć konto (dodatkowy środek bezpieczeństwa).



Po wykonaniu tych kroków kliknij menu **Oferty**, aby zobaczyć dostępne oferty pożyczek. Kliknięcie oferty spowoduje przekierowanie na stronę Debifi.
...
### Wejdź na stronę internetową i zapoznaj się z ofertami pożyczek



Po podłączeniu urządzenia przejdź na stronę [Debifi website](https://debifi.com/). Zaloguj się, aby ustanowić bezpieczne połączenie między aplikacją mobilną Debifi a platformą internetową. Ułatwia to interakcję z dostępnymi ofertami pożyczek (przejrzysty widok szczegółów każdej oferty) i zarządzanie kontem.
...
Samouczek wideo dotyczący logowania się do konta na platformie:
...
Platforma zapewnia kontakt z płynnością o jakości instytucjonalnej i oferuje szereg opcji dostosowanych do Twoich potrzeb. Przejrzyj, aby dowiedzieć się, co jest dostępne. Masz również możliwość dostosowania parametrów pożyczki do swojej indywidualnej tolerancji ryzyka i sytuacji finansowej.
...
Waluty fiat oferowane obecnie przez Debifi można przeglądać na platformie.
...
### Jasne, elastyczne klauzule umowne



Debifi opiera się na przejrzystych i elastycznych warunkach pożyczki, aby zaspokoić potrzeby każdej ze stron (pożyczkodawcy i pożyczkobiorcy). Kluczowe klauzule obejmują :
...
Ustalanie stawek zależy zasadniczo od wybranego LTV, długości okresu kredytowania, zmienności zabezpieczenia i oceny ryzyka specyficznej dla platformy. Stawki pozostają stałe przez cały okres trwania pożyczki.
...
#### Prawa do likwidacji (wezwania do uzupełnienia depozytu zabezpieczającego)
...
### Rozpoczęcie procesu kredytowego



Aby wybrać ofertę pożyczki, która odpowiada Twoim potrzebom, kliknij ją, aby wyświetlić jej funkcje.
...
1. Nazwa instytucji udzielającej pożyczki ;


2. Zakres kwoty pożyczki;


3. Otrzymasz środki w USDC Ethereum;


4. Okres kredytowania, oprocentowanie i wskaźnik LTV;


5. W przypadku tej oferty wymagany jest KYC;
...
Wróć do aplikacji mobilnej dla ''**Podaj klucz publiczny**''.
...
### Depozyt gwarancji i otrzymanie środków
...
Otrzymasz również to powiadomienie w swojej aplikacji mobilnej.
...
Następnie otrzymasz powiadomienie od Debifi z prośbą o uiszczenie opłat za pożyczkę lub prowizji w celu zmiany statusu pożyczki.
...
Wszystko, co musisz zrobić, to podpisać transakcję, która pozwoli Debifi odliczyć prowizję od gwarancji. Ze swojej strony pożyczkodawca będzie również musiał podpisać transakcję potrącenia opłaty, w przeciwnym razie Debifi nie będzie w stanie otrzymać swojej prowizji.
...
W przypadku wzrostu ceny zabezpieczenia, pożyczkobiorca zachowuje wszelkie zyski kapitałowe z zabezpieczenia. Jest on winien jedynie kwotę pożyczki, która jest z góry określona i niezależna od ceny Bitcoin.




## Spłata i odzyskanie zabezpieczenia



Na koniec uzgodnionego okresu (lub wcześniej, jeśli chcesz) musisz spłacić pożyczkę.
...
Gratulacje! Proces został zakończony.
...
Monitoruj swój stosunek pożyczki do zabezpieczenia (LTV). Jeśli to możliwe, skonfiguruj alerty (cena BTC, LTV). Nie pozwól, aby Twój wskaźnik zbliżył się do 90%. W razie wątpliwości zwiększ zabezpieczenie lub spłać wcześniej.
...
Jeśli to możliwe, zacznij od małych kwot. W przypadku pierwszej pożyczki przetestuj skromną kwotę, aby zapoznać się z procesem.
...
Alternatywnie, jeśli jesteś pożyczkodawcą, ten film instruktażowy poprowadzi Cię przez korzystanie z platformy Debifi. Od wyboru pożyczkobiorców zainteresowanych Twoją ofertą, po dostarczanie kluczy publicznych, podpisywanie umów, przesyłanie stablecoinów i nie tylko.
...
Wiesz już, jak korzystać z platformy Debifi w celu uzyskania pożyczki.



Polecam udział w tym kursie, który dogłębnie analizuje Bitcoin, Stablecoiny i ich wkład w suwerenność.
