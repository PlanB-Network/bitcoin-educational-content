...
Siden covid-19-pandemien har kontaktløse digitale betalinger blitt utbredt, selv i de minste butikkene. I løpet av denne perioden har mange bedrifter oppdaget det praktiske med Bitcoin-kontantløsninger, som gjør det mulig for dem å motta betalinger fra hele verden. Disse løsningene er imidlertid noen ganger vanskelige å bruke eller uegnet for små bedrifter. I denne veiledningen tar vi en titt på Breez betalingsterminal, en løsning som skiller seg ut med sin brukervennlighet, samtidig som den gir deg full kontroll over håndteringen av bitcoins.
...
I menyen "*Point of Sale*" finner du en Interface for innkreving av betalinger. Den er delt inn i to deler:
...
Den første delen er direkte debiteringstastaturet. Denne Interface er praktisk for å samle inn en enkelt betaling når du kjenner kundens totale kjøp, eller når du ikke trenger en fast produktkatalog i virksomheten din (f.eks. frilanstjenester).
...
Den andre delen er produktkatalogen. Denne Interface er ideell når du har en produktkatalog med forhåndsdefinerte priser. Her kan du forhåndskonfigurere produktene dine og deretter bruke dem til generate-fakturaer for å forbedre sporbarheten til kontantkvitteringene dine.
...
Du kan konfigurere hver enkelt vare manuelt fra denne Interface ved å klikke på "**Plus**"-knappen og deretter definere navn, pris og en identifikator for denne varen.
...
I samme seksjon kan du definere gyldighetsperioden for Lightning-fakturaene dine. Fra nå av har kundene `N` sekunder på seg til å betale for alle fakturaene dine, hvis ikke må du generere en ny Lightning Invoice.
...
Som manager kan du styrke sikkerheten til bitcoinsene dine ved å legge til et passord som kreves for alle utgående betalinger fra Wallet. Denne funksjonen er spesielt nyttig når du ikke er den eneste som administrerer uttaket ditt.
...
Du har nå en fullstendig forståelse av salgsstedet som Breez-applikasjonen tilbyr for sømløs integrering av Bitcoin i virksomheten din. Hvis du synes denne veiledningen var nyttig, anbefaler vi vår veiledning om be-BOP, en e-handelsplattform som lar deg ta imot betalinger i bitcoins og tjene penger på virksomheten din.
