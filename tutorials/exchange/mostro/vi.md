---
name: Mostro
description: Nền tảng Bitcoin P2P Exchange không cần KYC thông qua Lightning và Nostr
---
...
**Mostro P2P** cung cấp một giải pháp thay thế phi tập trung kết hợp Lightning Network, giao thức Nostr và lưu trữ hóa đơn. Cải tiến chính của nó: một hệ thống ký quỹ tự động, cho phép bạn kiểm soát bitcoin của mình trong suốt Exchange, loại bỏ nguy cơ bị đánh cắp, phá sản hoặc bị tịch thu tùy tiện.
...
Mostro P2P là giao thức mã nguồn mở Bitcoin Exchange do **grunch**, nhà phát triển bot Telegram phổ biến **lnp2pbot** ra mắt năm 2021, tạo ra. Mặc dù lnp2pbot đã kích hoạt các sàn giao dịch P2P không cần KYC thông qua Lightning, nhưng nó lại có một lỗ hổng bảo mật lớn: **Telegram là điểm lỗi tập trung** dễ bị chính phủ kiểm duyệt.
...
**Linh hoạt thanh toán**: Hỗ trợ nhiều loại tiền tệ fiat thông qua dịch vụ tỷ giá Exchange của yadio.io. Chấp nhận chuyển khoản SEPA, PayPal, Revolut, tiền mặt và bất kỳ phương thức nào được thỏa thuận giữa các bên.
...
Mostro cung cấp hai máy khách hoạt động chính cho các sàn giao dịch P2P của bạn.
...
**Mostro CLI** là máy khách hoàn thiện và đa chức năng nhất. Được viết bằng Rust, nó cung cấp đầy đủ các tính năng của Mostro thông qua dòng lệnh Interface. Tương thích với macOS, Linux và Windows.
...
- `release`: Giải phóng tài khoản ký quỹ và hoàn tất Exchange
- `getdm`: Xem tin nhắn trực tiếp
- `rate`: Đánh giá đối tác của bạn sau Exchange
...
**Ứng dụng di động** phiên bản alpha cho phép giao dịch P2P trực tiếp từ điện thoại thông minh của bạn. Giao diện Interface trực quan với điều hướng theo tab, xem lệnh, bộ lọc nâng cao và trò chuyện tích hợp để giao tiếp với đối tác của bạn.



Có sẵn cho **Android** (thông qua APK trên GitHub), đây là lựa chọn tối ưu cho những người dùng ưa thích sự đơn giản và giao dịch di động thường xuyên.
...
- Một Lightning Network** Wallet đang hoạt động có đủ thanh khoản (hoặc tương thích với Lightning)
...
- Tùy chọn nhưng được khuyến nghị**: Kết nối VPN hoặc Tor để che giấu IP của bạn Address
...
- Số tiền nhỏ**: Bắt đầu với số tiền ít hơn Sats 10.000 để bắt đầu
- Nhiều rơle**: Cấu hình 3-5 rơle ở nhiều vị trí địa lý khác nhau
- Bảo vệ mạng**: Kích hoạt VPN hoặc Tor để ẩn IP của bạn Address
- Wallet secure**: Kích hoạt khóa tự động trên Wallet Lightning của bạn
...
Mostro sẽ yêu cầu bạn cung cấp **Lightning Invoice** để nhận BTC. Số lượng chính xác tính bằng satoshi sẽ được hiển thị (ở đây: 4715 Sats).
...
### Bước 3: Cung cấp Lightning Invoice của bạn



generate a Lightning Invoice từ Wallet (Phoenix, Breez, v.v.) của bạn với số tiền chính xác, sau đó gửi nó:
...
Hệ thống xác nhận việc giao hàng và thông báo cho bạn đợi người bán thanh toán khoản giữ Invoice trước khi kích hoạt ký quỹ.
...
Ngay khi người bán xác nhận đã nhận được tiền pháp định và giải phóng tài khoản ký quỹ bằng lệnh `giải phóng`, bạn sẽ ngay lập tức nhận được bitcoin trên Lightning Invoice mà bạn đã cung cấp.
...
Nhấn nút Green **"+"** ở góc dưới bên phải, sau đó chọn **BÁN** (nút màu đỏ).
...
**Hướng dẫn quan trọng**: Bây giờ bạn phải **thanh toán khoản giữ Invoice** được hiển thị để khóa bitcoin của bạn (ở đây là 4674 Sats với giá 1-5 EUR) trong tài khoản ký quỹ. Bạn có **tối đa 15 phút**, nếu không khoản giữ Exchange sẽ bị hủy.



Sao chép lệnh giữ Invoice và thanh toán bằng Wallet Lightning (Phoenix, Breez, v.v.) của bạn.
...
Sau khi bạn đã **xác nhận biên lai** khoản thanh toán bằng tiền pháp định vào tài khoản của mình, hãy nhấn nút **GIẢI PHÓNG** Green và xác nhận gửi Satss cho người mua.
...
Bitcoin sẽ được chuyển ngay lập tức cho người mua thông qua Lightning Invoice.
...
4. Cung cấp Lightning Invoice của bạn (được tạo từ Wallet của bạn)
...
9. Nhận bitcoin ngay lập tức trên Lightning Invoice của bạn
...
**Chủ quyền tài chính**: Bitcoin của bạn sẽ không bao giờ rời khỏi quyền kiểm soát trực tiếp của bạn nhờ cơ chế giữ Invoice, loại bỏ nguy cơ Exchange phá sản hoặc bị vi phạm bản quyền.
...
Mostro P2P là một giải pháp thay thế đầy hứa hẹn cho các sàn giao dịch tập trung, kết hợp Lightning Network, Nostr và ký quỹ tự động để tạo nên một giao dịch phi tập trung thực sự. Mặc dù mới phát triển và còn hạn chế về tính thanh khoản, nền tảng này đã cung cấp khả năng tự chủ tài chính, khả năng chống kiểm duyệt và tính ẩn danh khi vỡ nợ.
