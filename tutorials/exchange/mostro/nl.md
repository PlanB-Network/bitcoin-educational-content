---
name: Mostro
description: KYC-vrij Bitcoin P2P Exchange platform via Lightning en Nostr
---
...
Hoe koop of verkoop je bitcoins zonder je financiële soevereiniteit in gevaar te brengen? Gecentraliseerde platforms leggen invasieve KYC-procedures op die je persoonlijke gegevens blootleggen, met de mogelijkheid van willekeurige accountbevriezing of staatstoezicht.



**Mostro P2P** biedt een gedecentraliseerd alternatief dat Lightning Network, het Nostr-protocol en holdfacturen combineert. De belangrijkste innovatie: een geautomatiseerd escrow systeem waarbij jouw bitcoins onder jouw controle blijven gedurende de Exchange, waardoor het risico van diefstal, Exchange faillissement of willekeurige inbeslagname geëlimineerd wordt.
...
Mostro P2P is een open source Bitcoin Exchange protocol gemaakt door **grunch**, ontwikkelaar van de populaire Telegram bot **lnp2pbot** gelanceerd in 2021. Hoewel lnp2pbot al KYC-vrije P2P uitwisselingen mogelijk maakte via Lightning, bracht het een grote kwetsbaarheid met zich mee: **Telegram vormt een gecentraliseerd point of failure** dat vatbaar is voor censuur door overheden.
...
**Betalingsflexibiliteit**: Ondersteuning voor veel fiatvaluta's via de yadio.io Exchange tariefservice. Accepteert SEPA-overschrijvingen, PayPal, Revolut, contant geld en elke andere methode die tussen partijen is overeengekomen.
...
De **Mostro CLI** is de meest volwassen en functionele client. Het is geschreven in Rust en biedt het volledige scala aan Mostro-functies via een Interface commandoregel. Compatibel met macOS, Linux en Windows.
...
- vrijgeven`: De escrow vrijgeven en de Exchange afronden
- `getdm`: Directe berichten bekijken
- `rate` : Evalueer uw tegenpartij na een Exchange
...
- Optioneel maar aanbevolen**: VPN of Tor-verbinding om je IP Address te maskeren
...
- Netwerkbeveiliging**: Activeer VPN of Tor om je IP Address te verbergen
- Wallet beveiligd**: Activeer de automatische vergrendeling op uw Wallet Lightning
...
Dit gedeelte demonstreert de aankoop van bitcoins via Mostro CLI volgens een praktijkgeval.
...
Mostro zal je vragen om een **bliksem Invoice** om de BTC te ontvangen. Het exacte bedrag in satoshis zal worden aangegeven (hier: 4715 Sats).
...
### Stap 3: Zorg voor uw Lightning Invoice



generate een Lightning Invoice van je Wallet (Phoenix, Breez, enz.) voor het exacte bedrag, stuur het dan :
...
Het systeem bevestigt de verzending en informeert je dat je moet wachten tot de verkoper de Invoice hold heeft betaald voordat je de escrow activeert.
...
Zodra de verkoper de ontvangst van de fiat bevestigt en de escrow vrijgeeft met het `release` commando, ontvang je direct je bitcoins op de Lightning Invoice die je hebt aangeleverd.
...
Druk op de Green **"+"** knop rechtsonder en selecteer dan **Verkopen** (rode knop).
...
**Belangrijke instructie**: Je moet nu **de Invoice hold** betalen om je bitcoins (hier 4674 Sats voor 1-5 EUR) in de escrow te vergrendelen. Je hebt maximaal **15 minuten** anders wordt de Exchange geannuleerd.



Kopieer de Invoice blokkade en betaal deze vanaf je Wallet Lightning (Phoenix, Breez, enz.).
...
Zodra je **de ontvangst** van de fiatbetaling op je rekening hebt bevestigd, druk je op de Green **RELEASE** knop en bevestig je om de Satss naar de koper te sturen.
...
Bitcoins worden onmiddellijk overgedragen aan de koper via zijn Lightning Invoice.
...
4. Geef je Lightning Invoice (gegenereerd uit je Wallet)
...
9. Direct bitcoins ontvangen op je Lightning Invoice
...
**Financiële soevereiniteit**: Uw bitcoins verlaten nooit uw directe controle dankzij het Invoice houdmechanisme, waardoor het risico van Exchange faillissement of piraterij wordt geëlimineerd.
