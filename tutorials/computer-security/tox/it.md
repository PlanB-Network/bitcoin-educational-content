...
_E2EE = crittografia end-to-end_


## Che cos'è Tox?

Tox è un protocollo di comunicazione decentralizzato e gratuito (open source) che utilizza la tecnologia *Networking and Cryptography Library* (NaCl) e combinazioni di algoritmi di crittografia per garantire la sicurezza e la riservatezza dei suoi utenti. Tox consente di inviare messaggi scambiando del testo, effettuare chiamate audio e video, condividere file e condividere il proprio schermo con gli amici in modo sicuro, decentralizzato e senza intermediari.
...
- qTox: Un client Tox [open source](https://github.com/TokTok/qTox) basato sul Qt Framework (C++) disponibile su Windows, Linux, MacOs.
...
Una volta scaricato, installa il tuo client Tox e crea il tuo profilo.

![qTox-acount](assets/fr/05.webp)

Congratulazioni, sei appena entrato a far parte del protocollo Tox. Nel software qTox, puoi trovare le informazioni del tuo profilo cliccando sul tuo nome utente.

![profil](assets/fr/06.webp)

In particolare, troverai il tuo Tox ID, che potrai salvare come codice QR e condividere con chi vuole mettersi in contatto con te.

Esportai il file del profilo Tox per avere un backup del profilo e delle informazioni di contatto da utilizzare per il ripristino. Fai clic sul pulsante **Esporta**, quindi scegli il percorso del file di backup.
...
Puoi contattarmi tramite questo Tox ID, ad esempio: EBC5604D9386E594CCC32943A03F96A96687FBD46788F1CD4F3337EB703C3C16BE3DC2CA6D9F
...
I tuoi client Tox ti permettono anche di comunicare con un gruppo di persone in modo completamente decentralizzato: si tratta delle cosiddette conferenze. Nel menu **Gruppi**, crea una nuova conferenza o consulta l'elenco degli inviti a partecipare alle conferenze che hai ricevuto.

![conferences](assets/fr/11.webp)

Una volta creata la conferenza, è possibile invitare gli amici a partecipare alla conferenza sul client Tox. Nell'elenco degli amici, fai clic con il tasto destro del mouse sul nome utente dell'amico che si desidera invitare. Fai clic sull'opzione **Invita alla conferenza**, quindi seleziona il nome della conferenza creata. È possibile invitare un amico anche creando implicitamente una conferenza con l'opzione **Crea una nuova conferenza**.

⚠️ I client Tox sono ancora in fase di sviluppo, pertanto è possibile che si verifichino errori durante l'interazione con il software. Le funzioni di conferenza e di videochiamata non sono ancora disponibili sul client Tox per Android (aTox). ⚠️
...
È inoltre possibile personalizzare le configurazioni di condivisione dei file per ogni discussione. Fai clic con il tasto destro del mouse sul nome utente del destinatario e seleziona "Mostra altri dettagli".

![details](assets/fr/14.webp)

Dai dettagli dell'interfaccia, è possibile gestire le autorizzazioni concesse al destinatario per:
...
- Nella sezione **Generale**, modifica la lingua di base del client Tox, definisci i percorsi dei file di backup e la dimensione massima dei file da accettare automaticamente.

![general](assets/fr/16.webp)

- Nella sezione **Interfaccia utente**, modifica i caratteri e le dimensioni dei messaggi. È inoltre possibile modificare il tema del client Tox.
...
Immagina di scrivere al nostro utente `EBC5604D9386E594CCC32943A03F96A96687FBD46788F1CD4F3337EB703C16BE3DC2CA6D9F` che chiameremo **utente B**. Il client Tox individua questo identificatore nella tabella Hash Distributed e recupera l'indirizzo IP associato. Una volta trovato l'indirizzo IP, il client Tox creerà un canale di comunicazione diretto e crittografato con la macchina dell'**utente B**, oppure utilizzerà altri nodi come relè per raggiungere l'**utente B**. Gli algoritmi di crittografia assicurano che, indipendentemente dal canale di comunicazione, solo **Utente B** sarà in grado di leggere il contenuto del messaggio.

Se ti è piaciuto scoprire Tox e sei riuscito a capire come è utile per rafforzare la tua privacy, non esitare a dare un pollice in su a questo tutorial. Ti consigliamo anche il nostro tutorial su Simple Login, uno strumento che consente di ricevere e inviare e-mail in modo anonimo.
