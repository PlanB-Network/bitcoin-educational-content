...
description: Minar Bitcoin fácilmente desde casa.
---
...
el dispositivo parece un paralelepípedo con una pantalla en la parte frontal
...
Y por la parte trasera tenemos: el agujero para la alimentación, espacio para una tarjeta SD (que puede ser necesaria para cualquier actualización), un pequeño botón que dice `IP REPORT` que permite conocer la IP Address del mini Miner, que es necesaria para acceder al dashboard del dispositivo. Una vez pulsado el botón, se muestra la IP Address durante unos 5 segundos, luego desaparece y vuelve a aparecer la pantalla de configuración. Sin embargo, si necesitas cambiar alguna configuración, basta con volver a pulsar el botón en cuestión y la IP Address reaparecerá en la pantalla. Siguiendo con la lista nos encontramos con un puerto Ethernet y un acceso para realizar un reset del dispositivo, para lo cual tendrás que coger un pin y mantenerlo pulsado durante 10 segundos con el fin de resetear todos los ajustes de la mini Miner. Finalmente encontramos dos luces indicadoras, una Green y otra roja, que nos indican el estado del Miner.
...
Será necesario conectar el dispositivo a internet vía ethernet, ten en cuenta que con la nueva versión (BMM 101) esto ya no es necesario. Volviendo a este mini Miner, una vez localizada la ubicación tendremos que conectarlo primero a la línea de internet y después a la corriente: el dispositivo se encenderá automáticamente y mostrará su dirección IP en la pantalla.

## Configuración

Tenemos que abrir un navegador e introducir la dirección IP que nos muestra el mini Miner en la barra de búsqueda. Te recordamos que para encontrar el dispositivo en la red tendrá que ser local, por lo que tendrás que tener el ordenador que estés utilizando conectado a la misma red que el mini Miner. Una vez introducida la dirección IP pulsamos enter y nos aparecerá en pantalla la página de login al sistema operativo del mini Miner, que es Braiins OS.
...
Si vamos a "Pantalla Mini Miner" veremos la configuración de nuestra mini Miner, como la visualización de la pantalla. Podemos elegir si mostrar la hora, o el precio de Bitcoin, o la pantalla con la información del estado de la máquina como el producto Hash, la temperatura, los vatios consumidos, etc. Aquí depende de ti para elegir lo que quieras ver en la pantalla, también podemos cambiar el brillo de la pantalla, establecer el modo nocturno, y mostrar la hora con formato de 12 o 24 horas.
...
Ahora todavía no estamos operativos, porque tenemos que conectarnos a un pool para poder arrancar el minado, así que tenemos que ir a "Configuración"
...
Aquí tendremos que decidir qué poola utilizar. En este tutorial mostraremos dos opciones. La primera es conectarnos a Mining pool Braiins que también es utilizado por mineros profesionales, como puedes ver en este tutorial:
...
Para conectarnos a este pool necesitamos crear una cuenta. Este pool también realiza pagos utilizando Lightning Network, por lo que podremos recibir unos cuantos Sats al día. Para ello necesitamos crear una drección Lightning en la que se recibirán las recompensas. Si no sabes cómo crear una cuenta en braiins pool o cómo configurar tu dirección lightning puedes seguir esta guía:
...
Ahora tienes que volver al panel del pool y hacer clic en "¡Conectado! Volver atrás"

![image](assets/en/15.webp)

¡Hemos conectado nuestro mini Miner al pool de braiins! Ahora puedes verlo en la lista de trabajadores. Si no aparece haz un refresh y espera unos instantes. Una vez que aparezca, comprueba que tiene el estado "OK" con una marca de verificación verde.

![image](assets/en/17.webp)

si volvemos al dashboard deberíamos empezar a ver movimiento en el gráfico y ver el Hashrate de nuestro dispositivo. Esto significa que la pool está recibiendo nuestro trabajo y por lo tanto estamos a todos los efectos minando.
...
aparece una página con toda la información que necesitamos. A continuación, copiamos allí la dirección stratum

![image](assets/en/19.webp)

luego volvemos al dashboard de nuestra mini Miner, vamos a configuración y a pools, pinchamos en añadir nuevo pool (mismo proceso que visto anteriormente) y pegamos la 'dirección stratum bajo pool url.

![image](assets/en/20.webp)

Ahora volvamos a la página del pool y veamos que como nombre de usuario tenemos que introducir una dirección Bitcoin, que será con la que recibiremos la recompensa en caso de que minemnos un bloqueo, luego un punto y después el nombre de nuestro dispositivo, como hicimos anteriormente con Braiins Pool, mientras que la contraseña la podemos elegir nosotros mismos.

![image](assets/en/21.webp)

Volvemos al mini Miner y en nombre de usuario pegamos una dirección Bitcoin seguido de punto y el nombre, pondremos `miniminer`. En la contraseña en su lugar pondremos test, tú puedes poner lo que quieras.

![image](assets/en/22.webp)

Ahora guardamos la configuración y desactivamos la pool Braiins.

![image](assets/en/23.webp)

¡Bien! ¡Ahora somos mineros en la pool pública!
