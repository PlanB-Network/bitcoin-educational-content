...
È possibile scaricare l'ultima versione di Angry IP Scanner dal sito web ufficiale dell'applicazione o da GitHub. Noi utilizzeremo quest'ultima opzione. Fai clic sul link sottostante e scaricate la versione EXE: "**ipscan-3.9.1-setup.exe**".
...
Al primo avvio, leggi le istruzioni della finestra "**Inizio**" per saperne di più sul funzionamento dello strumento. A proposito, ci sono diversi termini da considerare:
...
Per eseguire la scansione di una sottorete IP, è sufficiente inserire **l'indirizzo IP iniziale** e **l'indirizzo IP finale** nel campo "**intervallo IP**" (altrimenti, modifica la tipologia sulla destra). Quindi fate clic sul pulsante "**Avvio**".

![Image](assets/fr/019.webp)

Poche decine di secondi dopo, il risultato sarà visibile nell'interfaccia del software. **Per ogni indirizzo IP nell'intervallo analizzato, si vedrà se Angry IP Scanner ha rilevato un host o meno.** Sullo schermo apparirà anche un riepilogo che indica il numero di host attivi (in questo caso 6) e il numero di host con porte aperte.
...
### C. Aggiungere porte alla scansione
...
Se si desidera avere ulteriori informazioni sulla scansione, fai clic sul menu "**Strumenti**", quindi su "**Fetchers**". In questo modo si aggiungono "funzionalità" alla scansione. È sufficiente selezionare un fetcher e spostarlo a sinistra per attivarlo. In questo modo si aggiungerà una colonna in più al risultato della scansione.
...
Semplice ed efficace: Angry IP Scanner rileva gli host connessi a una rete e consente di configurare la scansione delle porte. In termini di opzioni, non è flessibile come Nmap e non va così in profondità, ma è un buon inizio per una scansione rapida.

Se si desidera utilizzare **Nmap** con una interfaccia grafica, è possibile utilizzare **l'applicazione Zenmap** (vedi panoramica sotto).
