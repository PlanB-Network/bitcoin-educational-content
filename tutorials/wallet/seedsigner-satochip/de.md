...
description: Wie verwende ich einen Satochip mit meinem SeedSigner?
---
...
Der Satochip ist ein Hardware Wallet im Chipkartenformat, mit einem EAL6+ zertifizierten Sicherheitselement, einem der höchsten Sicherheitsstandards. Er wird von dem gleichnamigen belgischen Unternehmen Satochip entwickelt und hergestellt.



Mit einem Preis von rund 25 € hebt sich der Satochip durch sein hervorragendes Preis-Leistungs-Verhältnis von der Konkurrenz ab. Dank seines sicheren Chips bietet er Widerstand gegen physische Angriffe. Darüber hinaus ist der Quellcode seines Applets vollständig quelloffen und unter der *AGPLv3* lizenziert.
...
Jetzt, da Ihre Hard- und Software einwandfrei funktioniert, können Sie Ihr Bitcoin-Portfolio erstellen. Schließen Sie dazu Ihren SeedSigner an, und erstellen Sie dann Ihren generate oder seed wie bei einem herkömmlichen SeedSigner, indem Sie entweder würfeln oder ein Foto machen:
...
- Sichern Sie schließlich die seed auf einem physischen Datenträger und überprüfen Sie die Sicherung sorgfältig.
...
Die hier gewählte Sicherungsstrategie hängt von Ihren Präferenzen ab, aber es ist unerlässlich, mindestens eine Kopie Ihres Mnemonic-Satzes zu haben, entweder auf physischen Medien (Papier oder Metall) oder, wie hier, in einem Seedkeeper. Sie können die Anzahl der Sicherungskopien bei Bedarf auch vervielfachen. Für weitere Informationen über Portfolio-Backup-Strategien empfehle ich Ihnen, dieses Tutorial zu lesen:



https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

Um Ihr seed auf einem Seedkeeper zu sichern, gehen Sie direkt zum Menü "seed sichern".
...
Wählen Sie ein `Label`, um Ihre verschiedenen auf Seedkeeper gespeicherten Geheimnisse leicht zu identifizieren. Sie können z. B. einfach den Fingerabdruck des Wallet beibehalten oder ausdrücklich `seed` angeben. Die Wahl hängt von Ihrer Vorliebe und Ihrem Risiko ab.
...
Der Seedkeeper-PIN-Code sollte so lang und zufällig wie möglich sein, um Brute-Force-Versuche im Falle einer physischen Kompromittierung der Karte zu verhindern. Sie sollten auch eine Sicherungskopie dieses PIN-Codes aufbewahren, die an einem anderen Ort als Seedkeeper gespeichert ist. Ohne diese PIN können Sie nicht auf die in Seedkeeper gespeicherten Mnemonic zugreifen, und Ihre Bitcoins sind für immer verloren.



### 5.3. Speichern Sie den seed auf dem Satochip



Nachdem Ihr Portfolio nun erstellt, gespeichert und geprüft wurde, übertragen wir es auf den Satochip. Stellen Sie dazu sicher, dass der seed in den RAM des SeedSigners geladen ist. Gehen Sie dann zu "Tools > Smartcard Tools > Satochip Functions".
...
Stecken Sie Ihren Satochip in den Chipkartenleser und wählen Sie dann "Mit seed initialisieren".
...
Sie können dann den Satochip-PIN-Code mit 4 bis 16 Zeichen einstellen. Um die Sicherheit Ihres Wallet zu verstärken, wählen Sie einen langen, zufälligen Code: Er ist der einzige Schutz gegen den physischen Zugriff auf Ihren Mnemonic-Satz.



Denken Sie daran, diese PIN zu speichern, sobald sie erstellt ist, entweder in einem sicheren Passwort-Manager oder auf einem physischen Medium, je nach Ihrer persönlichen Strategie. Im letzteren Fall sollten Sie den Datenträger mit der PIN niemals am selben Ort wie Ihren Satochip aufbewahren, da der Schutz sonst nutzlos wird. Es ist wichtig, eine Sicherungskopie zu haben: **Ohne diese PIN können Sie nicht mehr auf Ihren seed zugreifen und Ihre Bitcoins sind dauerhaft verloren**.
...
Der SeedSigner fragt Sie dann, welche seed in den Satochip importiert werden sollen. Wählen Sie denjenigen aus, dessen Fingerabdruck mit dem Portfolio übereinstimmt, das Sie gerade erstellt haben.
...
Nachdem Ihr Portfolio nun eingerichtet ist, importieren wir seine öffentlichen Informationen (den "*Keystore*") in Sparrow wallet oder eine andere Portfolioverwaltungssoftware. Diese Software wird zur Erstellung, Verteilung und Verfolgung Ihrer Transaktionen verwendet. Sie wird jedoch nicht in der Lage sein, diese zu signieren, da nur der Satochip (und etwaige Sicherungskopien) die für diesen Vorgang erforderlichen privaten Schlüssel enthält.
...
Wählen Sie schließlich den "Koordinator", d. h. die Portfoliomanagement-Software, die Sie verwenden möchten. Hier verwenden wir Sparrow wallet.
...
Der SeedSigner erzeugt dann Ihren xpub in Form eines dynamischen QR-Codes, der alle Daten enthält, die Sie für die Verwaltung Ihres Portfolios in Sparrow wallet benötigen. Sie können die Helligkeit des Bildschirms mit dem Joystick einstellen, um das Scannen des QR-Codes zu erleichtern.
...
Stellen Sie sicher, dass die Sparrow wallet-Software auf Ihrem Computer installiert ist. Wenn Sie nicht wissen, wie Sie die Software herunterladen, ihre Echtheit überprüfen und sie korrekt installieren, lesen Sie unsere vollständige Anleitung zu diesem Thema:
...
Überprüfen Sie nun, ob der in Sparrow wallet angezeigte "Master-Fingerabdruck" mit dem zuvor auf Ihrem SeedSigner gefundenen übereinstimmt.



Der SeedSigner fordert Sie dann auf, einen zufällig ausgewählten Address von Ihrem Sparrow wallet zu scannen, um die Gültigkeit des Imports zu bestätigen.
...
Ihr Satochip (über SeedSigner) und Sparrow wallet sind jetzt sicher verbunden. Sparrow dient als komplettes Management Interface, während der Satochip das einzige Gerät bleibt, das Ihre Transaktionen signieren kann. Sie sind nun bereit, Bitcoins in einer vollständig luftgekapselten Konfiguration zu empfangen und zu senden.
...
Öffnen Sie auf Ihrem Computer Sparrow wallet und entsperren Sie Ihren `Satochip-SeedSigner` Wallet mit Ihrem Passwort. Überprüfen Sie, ob die Software mit einem Server verbunden ist (Anzeige unten rechts). Klicken Sie dann in der Seitenleiste auf `Empfangen`.
...
Ein neuer Bitcoin Address erscheint. Sie finden:




- Der Address im Textformat (beginnend mit `bc1q...`, wenn Sie P2WPKH verwenden, wie in diesem Beispiel) ;
- Der zugehörige QR-Code ;
- Ein Feld "Bezeichnung", das für die Rückverfolgung Ihrer Transaktionen nützlich ist.



Ich empfehle Ihnen dringend, jeden Bitcoin-Beleg in Ihrem Wallet mit einem Etikett zu versehen. Auf diese Weise können Sie die Herkunft jedes UTXO leicht identifizieren und Ihre Privatsphäre besser verwalten. Wenn Sie mehr über dieses wichtige Thema erfahren möchten, sehen Sie sich die entsprechende Schulung in der Plan ₿ Academy an:
...
Bevor Sie Ihren Address an den Zahler weitergeben, müssen Sie überprüfen, ob er zu Ihrem seed gehört. Dieser Schritt stellt sicher, dass Ihr Satochip dann in der Lage ist, Transaktionen zu unterzeichnen, die mit diesem Address verbunden sind. Er verhindert auch potenzielle Angriffe, bei denen Sparrow einen betrügerischen Address anzeigen würde. Bedenken Sie, dass Sparrow in einer unsicheren Umgebung (Ihrem Computer) läuft, deren Angriffsfläche viel größer ist als die Ihres Satochip, der völlig isoliert ist. Aus diesem Grund sollten Sie den im Sparrow angezeigten Adressen niemals blind vertrauen, bevor Sie sie nicht auf Ihrem Hardware Wallet überprüft haben.



Klicken Sie im Sparrow auf den QR-Code des Address, um ihn zu vergrößern: Er wird dann bildschirmfüllend angezeigt.
...
Wenn der gescannte Address mit dem von Ihrem seed abgeleiteten übereinstimmt, zeigt der SeedSigner die Meldung an: gW-74 verifiziert".
...
So können Sie sicher sein, dass der Address zu Ihrem Portfolio gehört.
...
Sie können diesen Address nun in Textform oder über seinen QR-Code an die Person oder den Dienst übermitteln, der Ihnen Satss senden muss. Sobald die Transaktion im Netz übertragen wurde, erscheint sie auf der Registerkarte "Transaktionen" des Sparrow wallet.
...
- Schließlich wird die Transaktion vom Sparrow über das Netz verbreitet.
...
- Fügen Sie unter "Bezahlen an" den Address des Empfängers ein oder scannen Sie den QR-Code mit dem Kamerasymbol ein;
...
- Wählen Sie schließlich den Ladestrom entsprechend den aktuellen Netzbedingungen aus (Schätzwerte finden Sie unter [Mempool.space](https://Mempool.space/)).
...
Sobald die Signatur abgeschlossen ist, generiert der SeedSigner einen neuen QR-Code, der die signierte Transaktion enthält und vom Sparrow gescannt werden kann.
...
Nun, da die Transaktion signiert und validiert ist, muss sie nur noch im Bitcoin-Netz verbreitet werden, damit ein Miner sie in einen BLOCK aufnehmen kann. Klicken Sie in Sparrow auf "QR scannen".
...
Halten Sie den auf Ihrem SeedSigner angezeigten QR-Code (der die signierte Transaktion enthält) vor die Webcam. Sparrow zeigt dann alle Details der Transaktion an. Überprüfen Sie abschließend, ob alle Informationen korrekt sind, und klicken Sie dann auf "Transaktion senden", um sie im Bitcoin-Netzwerk zu übertragen.
...
Ihre Transaktion wird nun an das Netz übertragen. Sie können die Bestätigung der Transaktion auf der Registerkarte "Transaktionen" von Sparrow wallet verfolgen.
...
- Durch Aufzeichnung des Mnemonic-Satzes auf einem physischen Medium;
...
Wenn Sie Ihren Satochip noch haben, Ihr SeedSigner aber kaputt oder verloren gegangen ist, ist die Situation recht einfach zu handhaben, da sich Ihr Wallet noch im Satochip befindet.
...
Wenn Sie keinen neuen bauen wollen, können Sie Ihren Satochip auch auf die klassische Weise verwenden, d.h. direkt von Ihrem Computer aus, ohne den SeedSigner zu benutzen. Diese Methode funktioniert einwandfrei, aber sie verringert die Sicherheit Ihres Bitcoin Wallet erheblich: Sie verlieren die "*air-gapped*"-Isolierung und müssen nun blind unterschreiben, da der SeedSigner als vertrauenswürdiger Bildschirm fungierte. Dies kann jedoch eine vorübergehende Lösung für einen Notfall sein, oder wenn Sie nicht in der Lage sind, einen SeedSigner neu zu erstellen.



Dazu benötigen Sie ein USB-Smartcard- oder NFC-Lesegerät. Öffnen Sie das Wallet, das Sie wiederherstellen möchten, in Sparrow, gehen Sie dann auf die Registerkarte "Einstellungen" und klicken Sie auf "Ersetzen".
...
Geben Sie schließlich Ihre Smartcard-PIN ein, um sie zu entsperren. Sie können dann auf Ihr Wallet zugreifen, Transaktionen erstellen und diese direkt mit dem angeschlossenen Satochip unterschreiben.
...
Das andere, heiklere Szenario ist, wenn Sie den Zugang zu Ihrem Satochip mit dem seed verlieren: sei es, dass er kaputt, verloren oder gestohlen wurde oder dass Sie seinen PIN-Code vergessen haben. Wenn Ihr Satochip gestohlen oder verlegt wurde, empfehlen wir Ihnen dringend, Ihre Bitcoins sofort auf einen brandneuen Wallet zu übertragen, der mit einem anderen seed generiert wurde, sobald Sie wieder Zugang zu Ihrem Geld haben. Dadurch wird sichergestellt, dass ein potenzieller Angreifer niemals Zugang zu Ihrem Satss erhalten kann.
...
- Geben Sie Ihren Mnemonic-Satz manuell in das Menü "Saatgut > 12-Wort-seed eingeben" ein.
...
- Oder laden Sie Ihren seed von einem Seedkeeper über das Menü "Seeds > From SeedKeeper" (dies ist die Methode, die ich in diesem Tutorial verwende). Sie müssen lediglich den PIN-Code des Seedkeepers eingeben und das Geheimnis auswählen, das als seed auf dem SeedSigner verwendet werden soll.
