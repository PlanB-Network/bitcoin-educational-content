...
Seedkeeper on nutikaart, mille on välja töötanud Belgia ettevõte Satochip, mis on spetsialiseerunud digitaalsaladuste haldamiseks ja kaitsmiseks mõeldud riistvaralahendustele. Satochip on tuntud oma Bitcoin ökosüsteemi jaoks mõeldud kiipkaartide valiku poolest, kuid Seedkeeper on alternatiiviks Mnemonic fraaside säilitamise traditsioonilistele meetoditele.



Konkreetselt on Seedkeeper multifunktsionaalne EAL6-sertifikaadiga kiipkaart, millel on turvaline protsessor ja võltsimiskindel mälu (st "secure element*"). Nagu nimigi ütleb, on selle ülesanne salvestada Bitcoin Mnemonic fraase ja paroole krüpteeritud ja kaitstud viisil. Seedkeeperiga saate generate importida, organiseerida ja salvestada oma saladusi otse kaardi turvalisse ossa.
...
- Bitcoin** Mnemonic fraaside salvestamine: selle asemel, et kirjutada oma 12 või 24 sõna paberile, saate need sisestada nutikaardile ja kaitsta neid PIN-koodiga.
...
Seedkeeper on väga huvitav ka siis, kui seda kombineerida staatita Hardware Wallet-ga, nagu SeedSigner või Specter DIY. Sellisel juhul ei ole vaja kasutada Satochip'i arvuti- või mobiiliklienti. Seedkeeper hoiab seed oma secure element-s ja seda saab kasutada otse allkirjastamisseadmega, kaotades vajaduse paberil QR-koodi järele. Ma ei hakka seda konkreetset kasutusjuhtumit selles õpetuses arendama, kuna see on teise spetsiaalse õpetuse teema :
...
Selles õpetuses käsitlen ma ainult Bitcoin-ga seotud kasutusjuhtumeid, kuna see ongi see, mida see õpetus käsitleb. Me ei hakka käsitlema paroolide haldamise funktsionaalsust, mis on teise õpetuse teema.



Võrreldes Mnemonic lause lihtsa paberkandjal varundamisega on Seedkeeper'i kasutamisel mitmeid eeliseid:





- Vargusevastane:** seed teie Wallet-s ei ole selge tekstiga kättesaadav. Selle väljavõtmiseks peate teadma seemnepidaja PIN-koodi. Varas, kes seadme kätte saab, ei saa ilma selle koodita sellega midagi teha.





- Riski jaotamine kahe teguri vahel:** võite jagada turvalisuse digitaalse ja füüsilise teguri vahel. Näiteks kui salvestate Seedkeeper PIN-koodi oma paroolihalduris, on seed saamiseks vaja nii juurdepääsu sellele haldurile kui ka kiipkaardi füüsilist valdust (oluliselt väiksem rünnaku tõenäosus).
...
- Sõltuvus üldotstarbelisest arvutiseadmest:** seed sisestamiseks ja haldamiseks on vaja arvutit või nutitelefoni, mis tähendab, et teie Mnemonic läbib masinat, millel on palju laiem ründepind kui Hardware Wallet-l. See võib kujutada endast ohtu, kui tööjaam on ohustatud. Seepärast ei soovita ma kasutada Seedkeeperit Hardware Wallet seed salvestamiseks (välja arvatud staatita kasutamiseks ilma arvutita, nagu SeedSigneriga). Hardware Wallet roll on just seed säilitamine minimalistlikus, väga turvalises keskkonnas. Kui seed sisestatakse käsitsi oma tavalisse arvutisse, ei ole see enam piiratud Hardware Wallet-ga: see satub ka üldotstarbelisse masinasse, mis on avatud mitmetele ründevektoritele. Seega on parem kasutada Seedkeeperit pigem Hot Wallet kui Cold jaoks (v.a SeedSigner / stateless Hardware Wallet).





- PIN-koodiga seotud kadumisrisk:** seed otsene kättesaamatus, erinevalt paberkandjal varundusest, pakub tõepoolest kaitset füüsilise varguse vastu. Kuid nagu alati, on turvalisus tasakaalustatav varguse ja kadumise riski vahel. Kui teie varukoopia nõuab PIN-koodi, muudab selle koodi kadumine võimatuks teie Mnemonic fraasi taastamise ja seega juurdepääsu teie bitcoinidele.



Arvestades neid eeliseid ja puudusi, leian, et Seedkeeperi parimad kasutusalad (peale selle paroolihalduri funktsiooni) on ühelt poolt seemnete salvestamine teie **tarkvaraportfellidest**, kuna need asuvad juba teie telefonis või arvutis, või teie ekraanita riistvara rahakottidest nagu Satochip, ja teiselt poolt selle kasutamine koos staatita Hardware Wallet-ga nagu SeedSigner, kus ta tõeliselt oma rolli mängib.
...
Teilt küsitakse PIN-koodi teie Seedkeeper'i jaoks. Kuna tegemist on uue kaardiga, ei ole PIN-koodi veel kindlaks määratud. Selle sammu vahelejätmiseks sisestage ükskõik milline kood ja vajutage seejärel nuppu "*Järgmine*".
...
## 5. seed registreerimine Seedkeeper'is
...
Valige "Impordi saladus*". Valik "*generate saladus*" võimaldab teil luua uue Mnemonic fraasi otse rakendusest.
...
*Sellel pildil kujutatud Mnemonic on fiktiivne ja ei kuulu kellelegi. See on ainult näide. Ärge kunagi paljastage kellelegi oma Mnemonic, sest muidu varastatakse teie bitcoinid
...
## 6. Juurdepääs oma seed-le Seedkeeper'is



Kui soovite oma Mnemonic fraasi kontrollida, võtke oma Seedkeeper kätte ja vajutage nupule "*Click & Scan*".
...
See viib teid kõigi teie registreeritud saladuste nimekirja. Selles näites tahan ma kuvada seed minu portfellis "*BLOCKSTREAM App*", seega klõpsan sellel.
...
Teie eelnevalt salvestatud Mnemonic fraas kuvatakse nüüd ekraanil.
...
See on kõik, mis vaja! Nüüd te teate, kuidas kasutada Seedkeeperit Mnemonic fraasi Bitcoin Wallet salvestamiseks. Järgmises õpetuses vaatame, kuidas kasutada Seedkeeperit paroolide salvestamiseks. Samuti kutsun teid üles avastama selle kombineeritud kasutamist koos SeedSigneriga :
...
Selles õpetuses mainisime mitu korda ***Descriptors*** teie Bitcoin portfellis. Te ei tea, mis need on? Sellisel juhul soovitan teil läbida meie tasuta CYP 201 koolituskursus, kus käsitletakse põhjalikult kõiki HD portfellide toimimisega seotud mehhanisme!
