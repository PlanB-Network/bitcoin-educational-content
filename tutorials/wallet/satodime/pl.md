...
**Satodime**, opracowana przez **[Satochip](https://satochip.io/fr/)**, jest bezpieczną kartą na okaziciela do przechowywania Bitcoin w namacalny i prosty sposób. Działa ona jako samodzielny Hardware Wallet, w którym użytkownik ma pełną kontrolę nad swoimi kluczami prywatnymi bez zależności od strony trzeciej. Open-source i certyfikat EAL6+, obsługuje do trzech niezależnych sejfów.
...
Satodime, **carte au porteur, należy do każdego, kto fizycznie ją posiada**, bez potrzeby wcześniejszej rejestracji lub identyfikacji. Zaspokaja potrzebę bezpiecznego, przenośnego przechowywania Bitcoin, umożliwiając każdemu posiadaczowi karty korzystanie z niej lub przesyłanie bitcoinów poprzez skanowanie jej za pośrednictwem aplikacji mobilnej w celu przejęcia lub odblokowania sejfów. W przeciwieństwie do papierowego banknotu, wykorzystuje bezpieczny chip do Seal kluczy prywatnych, które są ujawniane dopiero po odpieczętowaniu, co czyni kartę podobną do gotówki, gdzie Ownership jest określany przez fizyczne posiadanie. Idealna do dawania bitcoinów w prezencie, ułatwia bezpieczny transfer bitcoinów z rąk do rąk, podczas gdy aplikacja mobilna wykorzystuje NFC do dostępnej interakcji ze smartfonem.
...
- Ostrzeżenie**: Ten krok jest nieodwracalny. Prosimy o zapoznanie się z [artykułem *Ownership*] (https://satochip.io/satodime-Ownership-explained/).
...
Satodime obsługuje do 3 sejfów. Utwórz jeden do przechowywania Bitcoin:
...
- Zeskanuj kartę do generate i Seal klucz prywatny (nieznany do momentu odpieczętowania).
...
- Skopiuj publiczny Address lub zeskanuj kod QR.
- Wysyłanie środków z innego Wallet.
- Sprawdź saldo po potwierdzeniu na Blockchain.
...
Po odpieczętowaniu wyeksportuj klucz do użycia w innym Wallet :
...
- Zaimportuj go do programu Wallet kompatybilnego z zarządzaniem funduszami (np. Sparrow wallet lub Electrum).
...
- Weź Ownership z karty,
- Utwórz sejf Bitcoin,
- Przenieś tam Satss,
- Przeniesienie karty Ownership: następna osoba, która zeskanuje kartę, staje się jej właścicielem,
- Przekaż kartę Satodime wybranej osobie i poproś ją o pobranie aplikacji, a następnie zeskanowanie karty w celu pobrania Ownership - a tym samym uzyskania dostępu do "przechowywanych" na niej bitcoinów.
...
- Sprawdź status**: Zawsze skanuj, aby potwierdzić Ownership karty i stan zapieczętowania/rozpieczętowania sejfów.
...
- Odzyskiwanie**: Jeśli karta została zgubiona, ale jest własnością, środki znajdują się na Blockchain; użyj klucza prywatnego, jeśli został odpieczętowany.
...
[Plan ₿ Network](https://planb.network/) dla samouczków na temat samoprzechowywania, kluczy prywatnych itp.
...
**Satochip** to belgijska firma, która opracowuje rozwiązania sprzętowe i programowe do zarządzania i przechowywania bitcoinów i innych kryptowalut. Jej flagowy produkt, Satochip Hardware Wallet, to karta NFC wyposażona w secure element z certyfikatem EAL6+. Uzupełniony o Seedkeeper, menedżer fraz i sekretów Mnemonic, oraz Satodime, kartę na okaziciela, Satochip oferuje kompleksową gamę dostosowaną do potrzeb użytkowników. Jej urządzenia, oparte na oprogramowaniu open source, mają na celu demokratyzację bezpieczeństwa na Bitcoin.