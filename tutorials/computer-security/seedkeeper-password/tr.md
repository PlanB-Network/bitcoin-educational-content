...
Bu eğitimde, SeedKeeper'ın yalnızca geleneksel parolalar için, bir parola yöneticisi tarzında kullanımını ele alacağız. SeedKeeper'ı Bitcoin Wallet Mnemonic cümlelerini kaydetmek için kullanmak isterseniz, lütfen bu diğer eğitime bakın:
...
"generate secret*" seçeneğini seçin. "*Bir sırrı içe aktar*" seçeneği mevcut bir sırrı kaydetmenize olanak tanır (örneğin, geçmişte oluşturduğunuz bir parola).
...
Bu sırra bir "*Etiket*" atayın, böylece Seedkeeper'ınızda birkaç bilgi parçası saklıyorsanız kolayca tanımlanabilir. Ayrıca parola ile ilişkili bir kullanıcı adı ve sitenin URL'sini de ekleyebilirsiniz.
...
Parolanızın uzunluğunu ve parametrelerini seçin, ardından "*generate*" ve ardından "*Import*" üzerine tıklayın.
...
Bu sizi tüm kayıtlı parolalarınızın bir listesine götürür. Bu örnekte, Plan ₿ Network hesabımın şifresini görüntülemek istiyorum, bu yüzden üzerine tıklıyorum.
...
Hepsi bu kadar! Artık Seedkeeper'ı şifrelerinizi saklamak için nasıl kullanacağınızı biliyorsunuz. Gelecekteki bir eğitimde, Seedkeeper'ı bir Bitcoin portföyünü yedeklemek için nasıl kullanacağımıza bakacağız. Ayrıca sizi SeedSigner ile birlikte kullanımını keşfetmeye davet ediyorum:
