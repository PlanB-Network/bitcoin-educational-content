...
VirtualBox**のインストールは簡単で、どのバージョンのWindowsでも同じ手順で行えます。ダウンロードした**VirtualBoxの実行ファイルを起動し、"**Next**"をクリックします。
...
左側の「**拡張機能**」をクリックし(1)、右側の「**+**」ボタンをクリックして(2)、ダウンロードしたVirtualBox**拡張機能パック**をロードします(3)。
...
- VMの構成（RAM、CPUなど）に対応する**VBOX**ファイル（XML形式）。
