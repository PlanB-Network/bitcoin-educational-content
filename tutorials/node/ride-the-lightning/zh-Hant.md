...
**如果沒有圖形化的 Interface，Lightning 節點只能透過人性化的 CLI 指令來管理。RTL 使用符合人體工學的 Interface 簡化了這些操作。以下是**主要的應用**：
...
簡而言之，RTL 是一個額外的元件，可透過安全 API 連接到您的節點，需要適當的存取代碼，並提供其本身的安全性 Layer。這種模組化的架構甚至允許您使用一個RTL實例來管理**多個Lightning節點。
...
**Sources :** RTL 官方網站；RTL GitHub；Umbrel App Store；Umbrel 社群；Plan B Network 資源。
