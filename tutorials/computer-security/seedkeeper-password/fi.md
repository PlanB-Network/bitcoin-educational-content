...
Seedkeeper on älykortti, jonka on kehittänyt Satochip, belgialainen yritys, joka on erikoistunut digitaalisten salaisuuksien hallintaan ja suojaamiseen tarkoitettuihin laitteistoratkaisuihin. Seedkeeper on tunnettu Bitcoin-ekosysteemiin tarkoitetuista älykorttivalikoimistaan, ja Satochip on suunnitellut sen vaihtoehdoksi perinteisille Mnemonic-lauseiden ja muiden digitaalisten salaisuuksien tallennusmenetelmille.



Konkreettisesti siemenvartija on EAL6-sertifioitu monitoiminen älykortti, jossa on suojattu prosessori ja väärentämissuojattu muisti (eli *secure element*). Kuten nimestä voi päätellä, sen tehtävänä on tallentaa Bitcoin-salkun muistitietoja ja salasanoja salatulla ja suojatulla tavalla. Seedkeeperillä voit generate, tuoda, järjestää ja tallentaa salaisuutesi suoraan kortin suojattuun komponenttiin.
...
Tässä oppaassa käsittelemme vain SeedKeeperin käyttöä perinteisten salasanojen hallintaan salasanahallinnan tapaan. Jos haluat käyttää SeedKeeperiä Bitcoin Wallet Mnemonic -lausekkeiden tallentamiseen, katso tämä toinen opetusohjelma:
...
Valitse "generate secret*". "*Tuo salaisuus*" -vaihtoehdon avulla voit tallentaa olemassa olevan salaisuuden (esimerkiksi aiemmin luodun salasanan).
...
Määritä tälle salaisuudelle "*Label*", jotta se voidaan helposti tunnistaa, jos tallennat useita tietoja Seedkeeperiin. Voit myös lisätä salasanaan liittyvän käyttäjänimen ja sivuston URL-osoitteen.
...
Tämä vie sinut luetteloon kaikista rekisteröidyistä salaisuuksistasi. Tässä esimerkissä haluan näyttää Plan ₿ Network-tilini salasanan, joten napsautan sitä.
...
Siinä kaikki! Nyt tiedät, miten käyttää Seedkeeperiä salasanojesi tallentamiseen. Tulevassa opetusohjelmassa tarkastelemme, miten Seedkeeperiä käytetään Bitcoin-salkun varmuuskopiointiin. Kutsun sinut myös tutustumaan sen yhdistettyyn käyttöön SeedSignerin kanssa:
