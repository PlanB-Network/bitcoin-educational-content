---
name: Bitcoin Keeper - Plano de herança
description: Configurar e utilizar o Bitcoin Keeper Wallet com os Assinantes de Torneiras para criar um plano de herança
---

![cover](assets/cover.webp)


## Introdução


Este tutorial em vídeo orienta-o na configuração e utilização do Bitcoin Keeper Wallet com Tapsigners para criar um plano de herança. Passamos pelo envio/receção de transacções, gestão de chaves, cópias de segurança e muito mais.


![video](https://youtu.be/tCld_-n2d30)


Para ver o tutorial completo do Bitcoin Keeper Wallet, consulte o tutorial aqui:

https://planb.network/tutorials/wallet/mobile/bitcoin-keeper-7f2a160b-10b6-4cc5-8820-514ee2eb1599

Veja aqui outras ligações relevantes:


- Descarregar a aplicação Bitcoin Keeper [aqui] (bitcoinkeeper.app)
- Tapsigner [Vídeo](youtube.com/watch?v=xKHy1HMw5iU)


**Nota:** Este tutorial é apenas um rascunho em inglês, ainda precisamos de alguém para escrever um guia extenso sobre este assunto. Se fores esse alguém, entra em contacto connosco no nosso [Telegrama](https://t.me/PlanBNetwork_ContentBuilder/325) ou no [GitHub](https://github.com/PlanB-Network/Bitcoin-educational-content)