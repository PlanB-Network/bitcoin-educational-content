...
Satochip on Hardware Wallet kiipkaardiformaadis, millel on EAL6+ sertifitseeritud turvaelement, mis on üks kõrgeimaid turvastandardeid. Selle on projekteerinud ja tootnud samanimeline Belgia ettevõte Satochip.
...
- Satochip maksab umbes 25 eurot ja kuna rakendus on avatud lähtekoodiga, saate selle ise tühjale kiipkaardile paigaldada. Seejärel tuleb lisada SeedSigneri komponentide ja kiipkaartide lugemiseks vajaliku laienduse maksumus: olenevalt sellest, kust te selle riistvara ostate, peaks summa olema 70-100 eurot.
...
Kui seed on genereeritud, on see ainus aeg, mil see asub SeedSigner'i mälus. Minu puhul tahan ma selle salvestada [Seedkeeper](https://satochip.io/product/seedkeeper/), mis on teine Satochipi toode, mis on mõeldud saladuste säilitamiseks. Kasutan seda seadet viimase abinõuna, juhul kui mu Satochip peaks kaduma.



Siin valitud varundusstrateegia sõltub teie eelistustest, kuid kindlasti peab olema vähemalt üks koopia teie Mnemonic fraasist, kas füüsilisel andmekandjal (paber või metall) või, nagu siin, Seedkeeperis. Te võite ka varukoopiate arvu vastavalt vajadusele mitmekordistada. Lisateavet portfelli varundusstrateegiate kohta soovitan lugeda seda õpetust :



https://planb.academy/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

seed varundamiseks Seedkeeperil minge otse menüüsse `Backup seed`.
...
Valige `Silt`, et hõlpsasti identifitseerida oma erinevaid Seedkeeperisse salvestatud saladusi. Võite näiteks lihtsalt hoida Wallet sõrmejälge või märkida selgesõnaliselt `seed`. Valik sõltub teie eelistustest ja riskist.
...
Sisestage oma Satochip kiipkaardilugejasse, seejärel valige "Initsialiseeri seed".
...
Sisestage operatsioonisüsteemi sisaldav microSD-kaart, seejärel lülitage SeedSigner sisse. Hetkel ei saa ta midagi teha, sest ta ei tea veel teie seed. Te peate alustuseks sisestama Satochipi kiipkaardilugejasse, sest just sellel on teie seed.
...
Ilmub hoiatusteade: see on täiesti normaalne. Laiendatud avalik võti (`xpub`) võimaldab teil vaadata kõiki teie seed-st saadud aadresse (esimesel kontol). See ei anna aga juurdepääsu teie rahalistele vahenditele: selle avalikustamine ohustaks teie privaatsust, kuid mitte teie bitcoinide turvalisust. Teisisõnu, see võimaldab teil jälgida oma saldot, kuid mitte kulutada neid.
...
Veenduge, et Sparrow wallet tarkvara on teie arvutisse paigaldatud. Kui te ei tea, kuidas seda alla laadida, selle autentsust kontrollida ja õigesti paigaldada, vaadake meie täielikku õpetust sel teemal :
...
Määrake oma portfellile nimi, seejärel klõpsake nuppu "Loo Wallet". Seejärel palub Sparrow teil määrata parool, et lukustada kohalik juurdepääs sellele Wallet-le. Valige tugev parool: see kaitseb teie andmeid Sparrow-s (avalikud võtmed, aadressid, sildid ja tehingulugu). Siiski ei ole see parool vajalik Wallet taastamiseks tulevikus: seda on vaja ainult teie Mnemonic fraas (ja võimalik, et ka passphrase).
...
Nüüd kontrollige, et Sparrow wallet-s kuvatav "Master fingerprint" vastaks teie SeedSignerist eelnevalt leitud sõrmejäljele.



SeedSigner palub teil seejärel impordi kehtivuse kinnitamiseks skaneerida juhuslik vastuvõttev Address teie Sparrow wallet-st.
...
Teie Satochip (SeedSigneri kaudu) ja Sparrow wallet on nüüd turvaliselt ühendatud. Sparrow toimib Interface täieliku juhtimisega, samas kui Satochip jääb ainsaks seadmeks, mis on võimeline teie tehinguid allkirjastama. Olete nüüd valmis bitcoinide vastuvõtmiseks ja saatmiseks täiesti õhuga kaetud konfiguratsioonis.
...
#### 7.1.1 Vastuvõtu Address genereerimine
...
Ilmub uus Bitcoin Address. Te leiate :




- Address tekstiformaadis (alustades `bc1q...`, kui kasutate P2WPKH, nagu selles näites) ;
- Sellega seotud QR-kood ;
- Väli "Label", mis on kasulik tehingute jälgimiseks.



Soovitan tungivalt lisada sildi igale Bitcoin kviitungile oma Wallet-s. See aitab teil hõlpsasti tuvastada iga UTXO päritolu ja paremini hallata oma privaatsust. Kui soovite selle olulise teema kohta rohkem teada saada, vaadake spetsiaalset koolitust Plan ₿ Akadeemias :
...
Teie Address on nüüd seotud selle märgisega kõigis Sparrow sektsioonides.
...
Enne vastuvõtva Address edastamist maksjale on oluline kontrollida, et see kuulub teie seed juurde. See samm tagab, et teie Satochip saab seejärel allkirjastada selle Address-ga seotud tehinguid. Samuti välditakse sellega võimalikke rünnakuid, mille puhul Sparrow kuvab võltsitud Address. Pidage meeles, et Sparrow töötab ebaturvalises keskkonnas (teie arvutis), mille ründepind on palju suurem kui teie Satochipil, mis on täielikult isoleeritud. Seepärast ei tohiks te kunagi pimesi usaldada Sparrow-s kuvatavaid aadresse enne nende kontrollimist Hardware Wallet-s.



Sparrow-s klõpsake selle suurendamiseks Address QR-koodil: see kuvatakse seejärel täisekraanil.
...
Kui skaneeritud Address vastab teie seed-st saadud Address-le, kuvab SeedSigner teate: "Address kontrollitud".
...
Siis võite olla kindel, et Address kuulub teie portfelli.
...
Nüüd saate edastada selle Address tekstina või selle QR-koodi kaudu isikule või teenusele, kes peab teile saatma Satss. Kui tehing on võrgus edastatud, ilmub see Sparrow wallet vahekaardile "Tehingud".
...
- Lõpuks edastab Sparrow tehingu üle võrgu.
...
Sparrow wallets saate luua tehingu, klõpsates vasakpoolsel külgribal vahekaardil "Saada". Ma eelistan siiski kasutada vahekaarti `UTXOs`, mis võimaldab teil *Coin Control* harjutada. See meetod pakub täpset kontrolli kulutatud UTXO-de üle, et piirata tehingute käigus ilmnevat teavet.
...
- Sisestage jaotises "Makse aadressile" saaja Address või skannige tema QR-kood, kasutades kaamera ikooni ;
- Lisage lahtrisse "Silt" selle kulu jälgimiseks silt;
- Sisestage lahtrisse "Summa" saadetav summa;
- Lõpuks valige laadimiskiirus vastavalt praegustele võrgutingimustele (hinnangud on kättesaadavad aadressil [Mempool.space](https://Mempool.space/)).
...
Lülitage oma SeedSigner sisse ja sisestage Satochip nagu tavaliselt. Valige avaekraanilt "Skaneerimine" ja skannige seejärel Sparrow ekraanil kuvatavat QR-koodi.
...
Kui allkirjastamine on lõpule viidud, genereerib SeedSigner uue QR-koodi, mis sisaldab allkirjastatud tehingut ja on valmis Sparrow poolt skaneerimiseks.
...
Nüüd, kui tehing on allkirjastatud ja valideeritud, on vaja seda ainult edastada Bitcoin võrgus, et Miner saaks selle lisada BLOCK-sse. Sparrow-s klõpsake nuppu `Scan QR`.
...
Esitage veebikaamerale oma SeedSigneril kuvatav QR-kood (see, mis sisaldab allkirjastatud tehingut). Seejärel kuvab Sparrow kõik tehingu üksikasjad. Tehke viimane kontroll, et kõik andmed oleksid õiged, ja seejärel klõpsake "Broadcast Transaction", et edastada tehing Bitcoin võrgus.
...
- Salvestades Mnemonic fraasi füüsilisel andmekandjal;
...
Kui te ei soovi seda ümber ehitada, võite kasutada oma Satochipi ka klassikalisel viisil, st otse arvutist, ilma SeedSigneri kaudu käimata. See meetod töötab suurepäraselt, kuid vähendab oluliselt teie Bitcoin Wallet turvalisust: te kaotate "*õhkhaardistatud*" isolatsiooni ja peate nüüd allkirjastama pimesi, kuna SeedSigner toimis usaldusväärse ekraanina. See võib siiski olla ajutine lahendus hädaolukorras või kui te ei saa SeedSignerit ümber ehitada.



Selleks on vaja USB-kiipkaardi või NFC-lugejat. Avage Wallet, mida soovite taastada, Sparrow-s, seejärel minge vahekaardile "Seaded" ja klõpsake "Asendamine".
...
- Sisestage oma Mnemonic fraas käsitsi menüüs `Seeds > Enter 12-word seed`.
...
Kui seed on laaditud SeedSignerisse, saate ükskõik, millist meetodit kasutate, allkirjastada ühe või mitu skaneerimistehingut, et viia oma bitcoinid uude, kompromiteerimata Wallet-sse. Kuidas seda teha, leiate järgmise õpetuse osast 7.2:
