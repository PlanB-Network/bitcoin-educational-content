---
name: SeedSigner
description: Un Hardware Wallet de bricolaje, apátrida, asequible y con un control total del aire
---
...
El SeedSigner es un Hardware Wallet Bitcoin de código abierto que cualquiera puede construir por sí mismo utilizando componentes electrónicos baratos y de uso general. A diferencia de productos comerciales como Ledger, Coldcard o Trezor, no se trata de un dispositivo listo para usar fabricado por una empresa: es un proyecto comunitario que permite a cualquiera crear su propio dispositivo, controlando cada paso.



El SeedSigner está diseñado para ser 100% ***air-gapped***: nunca se conecta a Internet, no tiene Wi-Fi ni Bluetooth (en el caso de la Raspberry Pi Zero v1.3) y nunca se conecta a un ordenador para Exchange datos. La comunicación se realiza exclusivamente a través de un código QR del sistema Exchange. En concreto, su software de gestión de carteras (como Sparrow wallet) muestra la transacción que debe firmarse en forma de códigos QR; usted los escanea con la cámara del SeedSigner, luego el dispositivo firma la transacción utilizando sus claves privadas almacenadas temporalmente en su memoria RAM. Por último, genera códigos QR que contienen la transacción firmada, que usted escanea con su software para enviarla a la red Bitcoin.
...
Otra opción para almacenar su seed y usarla con el SeedSigner es utilizar una tarjeta inteligente *SeedKeeper* junto con un lector compatible. Esto le proporciona un *secure element* muy robusto para almacenar su seed, mientras utiliza la pantalla del SeedSigner para firmar transacciones. Pero esta configuración en particular es el tema de otro tutorial dedicado. Aquí nos concentraremos en el uso básico del SeedSigner:



https://planb.network/tutorials/wallet/hardware/seedkeeper-seedsigner-45cca4c4-1f22-46bb-87ae-9cddb68aa579

El proyecto SeedSigner ocupa un lugar importante en el ecosistema Bitcoin, ya que ofrece a todos, en cualquier parte del mundo, la posibilidad de beneficiarse de una seguridad avanzada para proteger sus bitcoins. Su principal ventaja reside en su accesibilidad: el hardware necesario puede adquirirse por menos de 50 dólares. Además, permite a las personas que viven en países con restricciones construir su propia Hardware Wallet a partir de componentes informáticos estándar, fáciles de encontrar y menos sujetos a restricciones reglamentarias.



Pero incluso fuera de estos contextos particulares, SeedSigner puede ser una opción interesante para ti: es de código abierto, funciona sin estado y con airgap, y reduce los vectores de ataque vinculados a la cadena Supply de tu Hardware Wallet.
...
- Raspberry Pi Zero
 ...
 - No olvides incluir un micro-USB de alimentación Supply.
...
- Pantalla Waveshare de 1,3" (240×240 px)** (en francés)
    ...
    - La pantalla incorpora tres botones y un minijoystick que sirve de Interface para el usuario.
...
- Alojamiento (opcional, pero recomendado)** (opcional, pero recomendado)** (opcional, pero recomendado)** (opcional, pero recomendado)** (recomendado)
...
Puedes comprar estos componentes por separado o, para mayor simplicidad, optar por paquetes ya preparados que incluyen todo el hardware necesario. Personalmente, encargué mi pack [en este sitio francés](https://bitcoinbazar.fr/), pero también encontrarás una lista de proveedores para cada región del mundo en [la página de hardware del proyecto SeedSigner](https://seedsigner.com/hardware/). Si prefieres comprar los componentes por separado, están disponibles en las principales plataformas de comercio electrónico o en tiendas especializadas.
...
A continuación, PowerShell calcula el Hash SHA256 de su archivo de imagen. Compare este Hash con el valor correspondiente en `seedsigner.0.8.6.sha256.txt`.
...
Si has cambiado la orientación de la cámara o deseas cambiar otros ajustes (como el idioma Interface) más adelante, tendrás que habilitar la persistencia de los ajustes en la microSD. De lo contrario, la configuración volverá a ser la predeterminada cada vez que reinicies, ya que la Raspberry Pi Zero no tiene memoria persistente.
...
La versión actual del firmware es compatible con varios paquetes de software: Sparrow, Specter, BlueWallet, Nunchuk y Keeper. En mi caso, utilizo **Sparrow wallet**, que recomiendo especialmente por su sencillez y su rica funcionalidad.
...
- gW-22 network`: a modificar sólo si se desea utilizar el SeedSigner en Testnet.
...
Por último, en el menú `Idioma`, puede cambiar el idioma del Interface para adaptarlo a sus preferencias.
...
## 5. Crear y guardar el seed



La seed (o frase Mnemonic) constituye la base de su cartera Bitcoin. Se utiliza para derivar sus claves privadas y direcciones, y proporciona acceso a sus fondos. SeedSigner ofrece varios métodos para generarla, que exploraremos en esta sección.
...
- Esta frase te da acceso completo y sin restricciones a todos tus bitcoins.** Cualquiera en posesión de esta frase puede robar tus fondos, incluso sin acceso físico a tu SeedSigner ;
- Normalmente, se utiliza una frase de 12 palabras para restaurar una Wallet en caso de pérdida o robo de una Hardware Wallet. Pero como el SeedSigner es un dispositivo *sin estado*, nunca registra tu seed. Así que tus copias de seguridad físicas no son sólo copias de seguridad, sino **la única manera de utilizar tu Wallet**. Si pierdes estas copias de seguridad, tus bitcoins se perderán permanentemente. Así que haz copias de seguridad con cuidado, en varios soportes y en lugares seguros;
- Si está empezando, le recomiendo encarecidamente que lea este tutorial para conocer en detalle los riesgos que conlleva la gestión de una frase Mnemonic :



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

### 5.1 Acceda a la herramienta de creación de seed
...
### 5.2 Creación de la seed con la foto
...
En este tutorial, usamos el método de **Lanzamiento de Dados**. Haz clic en `Nuevo seed` (con el icono del dado).
...
A continuación, elija la longitud de su frase Mnemonic. 12 palabras ya ofrecen un nivel de seguridad suficiente, así que es la opción que recomiendo.
...
Escriba cuidadosamente las palabras de su frase Mnemonic en un soporte físico adecuado (papel o metal).
...
Si cometes un error, el SeedSigner te informará y tendrás que empezar de nuevo, asegurándote de anotar tu frase Mnemonic cuando te la den. Este paso de verificación asegura que tu copia de seguridad es correcta y completa. Una vez validada, la pantalla mostrará `Backup Verified`.
...
El SeedSigner es un dispositivo sin memoria permanente. Esto significa que su seed nunca se almacena dentro del dispositivo (a diferencia de una Ledger, Trezor o Coldcard, por ejemplo). En cuanto se apaga, la seed desaparece por completo de su memoria RAM. Cuando el SeedSigner se reinicia, vuelve a un estado en blanco: tendrás que darle tu seed de nuevo para firmar tus transacciones.



Esto proporciona una protección esencial. A diferencia de otras billeteras de hardware, SeedSigner se basa en una Raspberry Pi Zero sin protección física, incluyendo *secure element*. Pero como no se almacenan datos sensibles, ni siquiera un dispositivo comprometido físicamente permitiría a un atacante extraer tus claves privadas o gastar tus bitcoins.
...
También le aconsejo que guarde su código QR y su frase Mnemonic en dos lugares distintos, para evitar perderlo todo en caso de siniestro.



Por último, una alternativa más avanzada y segura es utilizar el SeedSigner con un **SeedKeeper**, que almacena el seed en un secure element. Para saber más, consulta este tutorial :
...
Es también en esta fase cuando puedes añadir un **passphrase BIP39** para reforzar la seguridad de tu Wallet. Esta opción puede merecer la pena, en función de tu estrategia de copias de seguridad, pero también entraña riesgos: si pierdes la passphrase, el acceso a tus bitcoins se perderá definitivamente.



https://planb.academy/tutorials/wallet/backup/seedsigner-passphrase-7a61f64d-aa03-4bcf-8308-00c89a74cffe

Si aún no conoce el concepto passphrase, le invito a leer este completo tutorial sobre el tema:
...
### 5.8 Guardar la seed en formato QR (*SeedQR*)



El SeedSigner le permite convertir su seed en un código QR de papel, llamado *SeedQR*. Este método simplifica la recarga de su Wallet, ya que evita tener que volver a escribir cada palabra manualmente.



Para ello, necesitará un papel en blanco o un código QR de metal que se corresponda con la longitud de su frase Mnemonic. Si ha adquirido un paquete completo para su SeedSigner, las plantillas suelen estar incluidas. Si no, puedes descargarlas e imprimirlas (o reproducirlas a mano) aquí :
...
En la pantalla de su seed, seleccione `Backup seed`.
...
Reproduce cuidadosamente cada punto negro en tu hoja de guardado y, a continuación, utiliza el joystick para pasar al siguiente BLOCK. Tómate tu tiempo: un simple desajuste puede inutilizar el código QR.
...
Enhorabuena, ¡tu cartera Bitcoin ya está en marcha! Ahora vamos a importar sus componentes públicos a **Sparrow wallet** para gestionarla fácilmente.
...
Inserta la microSD que contiene el sistema operativo, enciende tu SeedSigner y carga el seed que acabas de crear a partir de tu código QR de copia de seguridad. En la pantalla de inicio, selecciona `Escanear` y escanea tu SeedQR con el SeedSigner.
...
Aparecerá un mensaje de advertencia. Es normal: esta clave pública extendida (`xpub`) le permite ver todas las direcciones derivadas de su seed (en la primera cuenta). No le permite gastar sus fondos, pero revela la estructura de su cartera. Si alguna vez se filtra, es un problema para tu privacidad, pero no para la seguridad de tus bitcoins: te permite verlos, pero no gastarlos.
...
Asigne un nombre a su cartera y, a continuación, haga clic en `Crear Wallet`. A continuación, Sparrow le pedirá que establezca una contraseña para bloquear el acceso local a esta Wallet. Elija una contraseña fuerte: protege el acceso a los datos de su cartera en Sparrow (claves públicas, direcciones, etiquetas e historial de transacciones). Esta contraseña no es necesaria para restaurar la cartera en una fecha posterior: sólo su frase Mnemonic (y posiblemente su passphrase) es necesaria para este propósito.
...
#### 7.1.1 Generación de una recepción Address



En su ordenador, abra Sparrow wallet y desbloquee su SeedSigner Wallet utilizando su contraseña. Asegúrese de que el software está conectado a un servidor (muesca en la parte inferior derecha). En la barra lateral, haz clic en `Recibir`.
...
Aparecerá una nueva Bitcoin Address. Verá :




- El texto Address (que empieza por `bc1p...` si utilizas P2TR como yo),
- El código QR correspondiente,
- Un campo `Label` para el seguimiento de sus transacciones.



Le recomiendo encarecidamente que añada una etiqueta a cada recibo de Bitcoin en su Wallet. Esto le permitirá identificar fácilmente la procedencia de cada UTXO y mejorar su gestión de la privacidad. Para profundizar en este importante tema, puedes consultar la formación dedicada en Plan ₿ Academy :
...
Antes de compartir su Address receptora, es muy importante comprobar que pertenece a su seed. Este paso garantiza que su SeedSigner podrá firmar transacciones asociadas a esta Address. También protege contra posibles ataques en los que Sparrow muestre una Address fraudulenta. Recuerde que Sparrow se ejecuta en un entorno inseguro (su ordenador), que tiene una superficie de ataque mucho mayor que su SeedSigner, que está totalmente aislado. Por eso nunca debes creer ciegamente en ningún destinatario Address mostrado en Sparrow hasta que lo hayas verificado con tu Hardware Wallet.
...
Si el Address escaneado coincide con el derivado de su seed, la pantalla de SeedSigner mostrará el mensaje `Address Verificado`.
...
Esto confirma que la Address pertenece a tu Wallet y que puedes recibir bitcoins de ella con toda confianza.
...
Ahora puede comunicar este Address (en forma de texto o código QR) a la persona o departamento que necesite enviarle Satss. Una vez que la transacción se haya difundido en la red, aparecerá en la pestaña `Transacciones` de Sparrow wallet.
...
- En "Pagar a", pegue el Address del destinatario o haga clic en el icono de la cámara para escanear el código QR;
...
Haga clic en "Revisar detalles" y compruebe cuidadosamente toda la información directamente en la pantalla de SeedSigner. Los elementos más importantes a comprobar son el importe enviado, el Address del destinatario y el importe de los gastos aplicados.
...
Una vez firmada, el SeedSigner genera un nuevo código QR que contiene la transacción firmada, lista para ser escaneada por Sparrow.
...
Ahora que la transacción es válida, hay que difundirla en la red Bitcoin, para que llegue a un Miner que la añadirá a un BLOCK.
