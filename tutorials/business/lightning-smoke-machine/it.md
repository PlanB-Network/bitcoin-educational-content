...
- Server BTCPay configurato. Per ulteriori informazioni sulla creazione di un'istanza del server BTCPay, visitare questo tutorial: https://planb.academy/fr/tutorials/business/point-of-sale/btcpay-server-928eb01e-824b-4b57-a3e8-8727633beddc
...
1. **Scollegare SEMPRE la macchina del fumo dalla rete elettrica** prima di aprire il telecomando o manomettere il cablaggio


2. **Rimuovere la batteria dal telecomando** prima di maneggiarla (rischio di cortocircuito e di danni ai componenti)


3. **Verificare che tutte le connessioni siano isolate** prima di ricollegare qualsiasi cosa
...
Se non siete a vostro agio con questo tipo di manipolazione, portate con voi qualcuno che lo sia.
...
Obiettivo: Collegare il relè al pulsante ON/OFF del telecomando


1. Aprire il telecomando




    - Identificare il pulsante ON/OFF
    - Svitare la custodia per aprire il telecomando


2. Individuare le connessioni




 - Individuare i terminali + e - dell'alimentatore
...
3. Cablaggio dei pulsanti (saldature o connettori)
    ...
    - Saldare un cavo nero al terminale - del pulsante
    - Saldare un cavo rosso al terminale comune +
...
Prima di passare al software, controllare :
...
1 - Accedere all'istanza del proprio *Server BTCPay* con un account di amministratore



2 - Creare la prima tenda



3 - Sul lato sinistro di *BTCPay Server*, scorrere fino in fondo e andare su *"Manage Plugins "*
...
5 - Scorrere l'elenco dei plugin e cliccare su *"Installa "* : *BitcoinSwitch e Blink* (o il wallet disponibile a vostra scelta)
...
6 - Una volta completata l'installazione, riavviare *BTCPay Server* e attendere 1 minuto per il riavvio dell'istanza
...
7 - Quando si torna a *"Gestione plugin "*, verificare che entrambi i plugin siano stati installati
...
- Visitate https://www.blink.sv
- Creare il proprio account. Consultare il tutorial :
...
- Accedere all'interfaccia del API: **[https://www.blink.sv/en/api](https://www.blink.sv/en/api)** e accedere con lo stesso account utilizzato per creare il wallet *Blink*
...
![blink-api](assets/fr/18.webp)
   ...
   - Una volta collegato, andare alla scheda *chiavi API*
...
![blink-api](assets/fr/19.webp)
   ...
   - Cliccare su *" * nell'angolo in alto a destra per accedere alla configurazione della chiave API
...
![blink-api](assets/fr/20.webp)
   ...
   - Assegnare un nome alla chiave API e lasciare le impostazioni predefinite. Poi, nel terzo passo, annotate la vostra chiave API - la vedrete solo una volta: `blink_mZ5KxxxxxxxxxNbmX`
...
**3 - Collegare il *Blink* al *Server BTCPay***




- Aprite il vostro *Server BTCPay*
...
- Fare clic su *Usa un nodo personalizzato*
- Incollare la seguente stringa di connessione:
...
- Sostituire solo :
    - api-key= *con la vostra chiave API Blink*
    - wallet-id= *dal vostro wallet Blink* ID
- Quindi fare clic su *Test connection*, poi su *Save*
...
![btcpay-server](assets/fr/24.webp)
 ...
 - Verificare che la connessione sia stabilita (stato verde)
...
- In BTCPay Server, andare alla scheda *Plugins* e cliccare su *Point of sale*
...
- Date un nome al vostro PoS e cliccate su *Crea*
...
- Configurazione PoS :
    - Scegliere lo stile del punto vendita = *Stampa display*
    - Valuta = *SATS
    - Fare clic su *Salva*
...
- Configurazione del prodotto :
    - Eliminare tutti i prodotti predefiniti
...
- Configurare il prodotto:
    ...
    - Fare clic su *Chiudi* e poi su *Salva* per salvare il nuovo prodotto
...
- Collegare l'ESP32 al computer con il cavo USB/Micro-USB
- Quindi fare clic su *Collegamento al dispositivo*
- Si apre una finestra, selezionare la porta USB dell'ESP32, quindi fare clic su *Connect*
...
- Una volta collegato l'ESP32, si procederà al flash del firmware di BitcoinSwitch. Nella sezione *T-Display*, cliccate su *Upload Firmware* per l'ultima versione disponibile (attualmente: *bitcoinSwitch T-Display v1.0.1*)
...
- Attendere il caricamento; il processo è completo quando i log mostrano *"Leaving...". "*
...
**3 - Controllare l'installazione del firmware di BitcoinSwitch




- Ricarica la pagina: [https://bitcoinswitch.lnbits.com/](https://bitcoinswitch.lnbits.com/)
- Ricollegare l'ESP32 al computer con il cavo USB/Micro-USB
- Quindi fare clic su *Collegamento al dispositivo
- Selezionate la porta USB del vostro ESP32, quindi fate clic su *Connect* come descritto sopra
- Una volta collegato, premere il pulsante **RESET** sull'ESP32
- Controllare nei log che le ultime righe mostrino :
...
- Aprire l'istanza del server BTCPay, quindi accedere al PoS creato in seguito.
- Cliccare quindi su "Visualizza" per aprire il PoS nel browser
...
- Copiare l'URL della pagina, ad esempio :
...
- `XXXXv` → il dominio della vostra istanza del server BTCPay
- `46XXXXXXXXXXXXXXXXwFB` → il vostro identificativo univoco PoS
...
- Sostituire `https://` con `wss://`
- Aggiungere `/bitcoinswitch' alla fine
...
- Ritorno alla pagina: [https://bitcoinswitch.lnbits.com/](https://bitcoinswitch.lnbits.com/) con il vostro ESP32 connesso
- Andare a *Configura dispositivo* → *Impostazioni Wi-Fi*
...
- Nella sezione *LNbits Device URL*, incollare l'URL WebSocket creato nel passaggio precedente
- Fare clic su *Carica la configurazione*
...
- Attendere il completamento del caricamento; i log dovrebbero visualizzare i parametri appena inseriti (SSID, password e URL WebSocket)
...
- Attendere che ESP32 stabilisca la connessione WebSocket. Si dovrebbe vedere :
...
Prima del test finale, controllare :
...
1. Inserire la spina della macchina del fumo (220V) e accenderla


2. Alimentare l'ESP32 (batteria o USB)


3. Aprire il PoS BTCPay nel browser


4. Scansione dell'elemento "macchina del fumo


5. Pagare con un wallet Lightning (Blink o altro wallet)
...
**⚡** Impilare sats, fare fumo, divertirsi, rimanere umili! **⚡**