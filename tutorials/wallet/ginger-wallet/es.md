...
Ginger Wallet es un software de Wallet sólo para ordenadores (sin aplicación móvil).
...
CoinJoin** es un tipo particular de estructura de transacción Bitcoin que reúne a varios participantes en una única transacción colaborativa. Este mecanismo mezcla las entradas de diferentes usuarios en una transacción común, lo que hace extremadamente difícil -si no a menudo imposible si se hace correctamente- rastrear los fondos. Como resultado, resulta casi imposible para un observador externo identificar con certeza el origen y el destino de los bitcoins implicados, a diferencia de lo que ocurre en las transacciones Bitcoin convencionales.



Para usted, el usuario, CoinJoin ayuda a preservar su confidencialidad. Por ejemplo, si recibes una donación de 10.000 Sats en una Bitcoin Address, el remitente puede rastrear estos fondos y, en algunos casos, deducir que posees una mayor cantidad de bitcoins, u observar tus actividades. Al hacer una CoinJoin después de esta donación de 10.000 Sats, rompes la trazabilidad: el remitente ya no podrá deducir ninguna información sobre ti a partir de este pago.



La Chaumian CoinJoin ofrece un alto nivel de seguridad, ya que los fondos permanecen en todo momento bajo el control exclusivo del usuario. Ni siquiera los operadores de los servidores coordinadores pueden desviar los bitcoins de los participantes bajo ninguna circunstancia. Ni los usuarios ni los coordinadores necesitan confiar los unos en los otros: cada uno conserva el control de sus claves privadas y sigue siendo el único autorizado para validar las transacciones. Por tanto, ningún tercero puede apropiarse de sus bitcoins durante un CoinJoin, ni establecer un vínculo directo entre sus entradas y salidas.
...
Desde el principio, Ginger te recuerda los costes que conlleva el proceso CoinJoin.
...
Una vez que hayas creado tu primera cartera, pasarás a la Interface Wallet de Ginger.
...
Puede crear varias carteras en Ginger Wallet. Basta con hacer clic en **Añadir una cartera**.
...
Ginger admite el uso de carteras de hardware a través de Bitcoin core estándar Interface, aunque la integración directa desde o hacia una cartera de hardware aún no está disponible.
...
Esta pestaña te permite activar Bitcoin Knots para que se ejecute cuando se inicie la aplicación, elegir tu red (Main/RegTest), y tu proveedor de tarifas de carga (Mempool Space/BLOCKSTREAM info/Full node), etc.
...
- durante una CoinJoin, el coordinador podría vincular sus entradas y salidas a su IP Address;
- al difundir una transacción, un nodo malicioso al que te conectes podría asociar tu transacción con tu IP.



No olvide pulsar **Hecho** (en la parte inferior derecha Coin) cada vez, para guardar sus ajustes. Algunos ajustes requieren que Ginger Wallet se reinicie para que surtan efecto.
...
- CoinJoin** (donde puede personalizar la configuración CoinJoin de esta cartera) ;
...
- Herramientas** (donde puede consultar su seedphrase, sincronizar de nuevo su cartera o eliminarla).
...
- Introduzca el nombre de la fuente a la que desea asociar la Address. Se trata de un etiquetado para realizar un seguimiento de sus pagos. Esto no tiene implicaciones On-Chain; es simplemente información de trazabilidad almacenada localmente en su aplicación;
...
- haga clic en la flechita a la izquierda de **generate** para elegir su formato Address (**SegWit** /**Taproot**) , luego haga clic en **generate**, para generate un Address y código QR.
...
El remitente utilizará este Address o código QR para enviarte bitcoins.
...
Tutorial en vídeo sobre cómo enviar a través de Ginger Wallet.
...
- introduzca el Address del destinatario, el importe a enviar y una etiqueta;
...
- Seleccione su país, o incluso su estado (en algunas regiones, como Canadá), antes de proceder a la compra de una Bitcoin. De hecho, cuando hagas clic en la función **Comprar** por primera vez, también tendrás que especificar tu región.
...
El Coin superior derecho de la pantalla con la etiqueta **Todos** le permite filtrar las ofertas por método de pago. El método de pago seleccionado se establecerá por defecto, pero se puede cambiar en cualquier momento.
...
- Al igual que con la opción **Comprar**, cuando utilice la función Vender por primera vez, deberá seleccionar su país antes de proceder a la venta de una Bitcoin.
...
- Ahora tienes que finalizar la transacción:
 ...
 - En algún momento, recibirá un Address de destinatario y la cantidad exacta que debe enviar;
 - A continuación, vuelva a Ginger Wallet para continuar el proceso;
 - Una vez de vuelta en Ginger Wallet, aparecerá un cuadro de diálogo que le permitirá continuar haciendo clic en **Enviar**.



Se abrirá la pantalla **Enviar** con la Address del destinatario y el importe rellenados previamente. También puede utilizar el botón **Enviar** de la pantalla de inicio. Aunque puede enviar la transacción manualmente, le recomendamos que la complete a través del cuadro de diálogo para optimizar el proceso.
...
Proteja la confidencialidad de sus bitcoins con **CoinJoin**, integrado directamente en Ginger Wallet. Wallet utiliza **WABISABI**, un protocolo Chaumian CoinJoin diseñado para facilitar coinjoins más accesibles y eficientes.



Usted elige la estrategia CoinJoin (automática o manual) que más le convenga.



Ginger CoinJoin está listo para usar en cuanto lo descarga (no requiere pasos adicionales). Automáticamente, Ginger CoinJoin se ejecuta en segundo plano para proteger su privacidad en cada transacción. En la práctica, el lector CoinJoin aparecerá siempre que tenga un saldo que pueda ser anonimizado.



En cuanto a la puesta en marcha manual de CoinJoin, es una operación de un solo clic. Inicie la ronda y espere a que se construya y confirme la transacción de CoinJoin. Verás el resultado de anonimización en Interface.
...
- Mantenga siempre su seedphrase en un lugar seguro fuera de línea;
- Si pierde su ordenador o sospecha de un acceso no autorizado, cree inmediatamente una nueva Wallet. Transfiere tus fondos a esta nueva cartera y elimina la antigua;
- Utilice un Address diferente para cada recepción para evitar reutilizar direcciones;
...
Si has encontrado este tutorial útil, por favor déjame un Green pulgar abajo. Por favor, siéntase libre de compartir este artículo a través de sus plataformas de medios sociales. Muchas gracias
