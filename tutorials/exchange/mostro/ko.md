---
name: Mostro
description: 라이트닝 및 노스트르를 통한 KYC-free Bitcoin P2P Exchange 플랫폼
---
...
금융 주권을 침해하지 않으면서 어떻게 비트코인을 구매하거나 판매할 수 있을까요? 중앙화된 플랫폼은 사용자의 개인 데이터를 노출하는 침습적인 KYC 절차를 시행하며, 임의적인 계정 동결이나 국가 감시의 가능성이 있습니다.



**모스트로 P2P**는 Lightning Network, 노스트르 프로토콜, 홀드 인보이스를 결합한 탈중앙화 대안을 제공합니다. 주요 혁신은 자동화된 에스크로 시스템으로, 비트코인이 Exchange 기간 내내 사용자의 통제 하에 있어 도난, Exchange 파산 또는 임의 몰수의 위험을 제거합니다.
...
모스트로 P2P는 2021년에 출시된 인기 텔레그램 봇 **lnp2pbot**의 개발자인 **grunch**가 만든 오픈 소스 Bitcoin Exchange 프로토콜입니다. Lnp2pbot은 이미 라이트닝을 통해 KYC가 필요 없는 P2P 교환을 가능하게 했지만, 주요 취약점이 존재했습니다: **텔레그램은 정부의 검열에 취약한 중앙 집중식 장애 지점**을 구성한다는 것입니다.



모스트로는 이러한 개념의 **탈중앙화된 진화**를 나타냅니다: 모스트로는 텔레그램을 **Nostr** 프로토콜(검열 불가능한 분산 릴레이 네트워크)로 대체함으로써, 이러한 시스템적 위험을 제거합니다. 이 프로토콜은 즉각적인 거래를 위한 Lightning Network, 검열 방지 통신을 위한 노스트르, 그리고 **보류 송장**을 결합하여 진정한 비수탁 자동 에스크로를 생성합니다.
...
- daemon Mostrod**: 사용자와 Lightning Network 간의 교환을 조정합니다
...
**분산형 중재**: 분쟁이 발생하면 공정한 중재자가 증거를 검토하고 투명한 기준에 따라 결정을 내립니다. 각 분쟁은 추적을 위해 고유한 token를 생성합니다.



**결제 유연성**: Yadio.io Exchange 요금 서비스를 통해 다양한 법정화폐를 지원합니다. SEPA 송금, PayPal, Revolut, 현금 및 당사자 간에 합의된 모든 방법을 허용합니다.
...
모스트로 CLI**은 가장 성숙하고 기능적인 클라이언트입니다. Rust으로 작성되었으며, Interface 명령줄을 통해 모스트로의 모든 기능을 제공합니다. MacOS, Linux 및 Windows와 호환됩니다.
...
- '피아트': 법정화폐 결제 전송 확인
- '릴리스'를 클릭합니다: 에스크로를 해제하고 Exchange를 마무리합니다
- `getdm`: 쪽지 보기
- `rate` : Exchange 이후 상대방을 평가합니다
...
알파 버전의 **모바일 앱**을 사용하면 스마트폰에서 직접 P2P 거래를 할 수 있습니다. 탭 탐색, 주문 보기, 고급 필터, 거래 상대방과 소통할 수 있는 통합 채팅 기능을 갖춘 직관적인 그래픽 Interface.
...
- 충분한 유동성을 갖춘 작동 중인 Lightning Network** Wallet(또는 라이트닝 호환)
 - 권장: 추천: Phoenix, 브리즈, Satoshi의 Wallet...
...
- 선택 사항이지만 권장**: IP 마스킹을 위한 VPN 또는 토르 연결 Address
...
**CLI/모바일** 동기화에 중요합니다: CLI와 모바일 앱에서 동일한 주문에 액세스하려면 두 클라이언트에서 **동일한 모스트로 펍키**와 **동일한 노스트르 릴레이**를 사용해야 합니다. 모바일 앱의 설정에서 이러한 설정을 확인하세요.
...
**중요**: 복구 문구를 안전한 장소(비밀번호 관리자, 종이 금고)에 적어 두세요.
...
- 소액**: 시작하려면 Sats 10,000 미만부터 시작하세요
- 다중 릴레이**: 지리적으로 다양한 3~5개의 릴레이 구성
- 네트워크 보호**: VPN 또는 Tor를 활성화하여 IP를 숨기세요 Address
- Wallet 보안**: Wallet 라이트닝의 자동 잠금 활성화
...
모스트로는 BTC를 받으려면 **라이트닝 Invoice**를 제공하라고 요청할 것입니다. 정확한 금액은 사토시 단위로 표시됩니다(여기서는 4715 Sats).
...
### 3단계: Lightning Invoice 제공



generate에서 라이트닝 Invoice(Phoenix, 브리즈 등)으로 정확한 금액을 확인한 후 전송합니다:
...
시스템에서 배송을 확인하고 에스크로를 활성화하기 전에 판매자가 Invoice 보류금을 지불할 때까지 기다리라고 알려줍니다.
...
판매자가 법정화폐 수령을 확인하고 '릴리스' 명령으로 에스크로를 해제하면 즉시 제공한 라이트닝 Invoice로 비트코인을 수령할 수 있습니다.
...
- 릴레이**: 복원력을 위해 여러 개의 노스트르 릴레이를 추가합니다(예: `wss://nos.lol`, `wss://relay.damus.io`)
...
*gW-61/모바일 동기화**에 중요**: CLI과 모바일 앱을 모두 사용하는 경우 두 클라이언트에서 **정확하게 동일한 Nostr 릴레이와 모스트로 펍키**를 구성하세요. 이렇게 동일하게 구성하지 않으면 두 인터페이스에서 동일한 주문을 사용할 수 없습니다. 설정(위 스크린샷)에 표시되는 릴레이와 모스트로 펍키가 CLI `.env' 파일에 있는 것과 일치해야 합니다.
...
오른쪽 하단의 Green **"+"** 버튼을 누른 다음 **판매**(빨간색 버튼)를 선택합니다.
...
**중요 지침**: 에스크로에 비트코인을 잠그려면 **표시된 Invoice 보류**를 지불해야 합니다(여기서는 4674 Sats, 1-5 EUR). 최대 **15분**의 시간이 주어지며, 그렇지 않으면 Exchange이 취소됩니다.



Invoice 보류를 복사하여 Wallet 라이트닝(Phoenix, 브리즈 등)에서 결제하세요.
...
계정에서 법정 화폐 결제의 **수령 확인**이 완료되면 Green **RELEASE** 버튼을 누르고 구매자에게 Satss를 전송하도록 확인합니다.
...
비트코인은 라이트닝 Invoice을 통해 구매자에게 즉시 전송됩니다.
...
4. 라이트닝 Invoice(Wallet에서 생성)를 제공하세요
...
9. 라이트닝 Invoice에서 즉시 비트코인을 받으세요
...
**금융 주권**: Invoice 홀드 메커니즘 덕분에 비트코인이 사용자의 직접적인 통제를 벗어나지 않아 Exchange 파산이나 불법 복제의 위험이 없습니다.
...
모스트로 P2P은 진정한 탈중앙화 거래를 위해 Lightning Network, 노스트라와 자동화된 에스크로를 결합한 중앙화 거래소의 유망한 대안입니다. 개발 초기이고 유동성이 제한적이지만, 이 플랫폼은 이미 금융 주권, 검열 저항성, 기본 익명성을 제공합니다.
