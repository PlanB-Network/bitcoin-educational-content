---
name: Đóng góp học liệu giáo dục
description: Làm thế nào để thêm các tài liệu giáo dục mới lên Plan ₿ Academy?
---
...
*Một lưu ý quan trọng là tất cả nội dung được tích hợp vào trang web phải không vi phạm bản quyền hoặc phải tuân thủ đúng giấy phép của tệp nguồn. Ngoài ra, mọi ấn phẩm hình ảnh được đăng tải trên Plan ₿ Academy đều được cung cấp theo giấy phép [CC-BY-SA](https://creativecommons.org/licenses/by-sa/4.0/).*
...
- Truy cập [kho lưu trữ (repository) GitHub của PlanB dành riêng cho dữ liệu](https://github.com/PlanB-Network/bitcoin-educational-content/tree/dev/resources/bet) trong phần `resources/bet/`:
...
- Trong ví dụ này, tôi sẽ tạm để trống các đường dẫn (links), vì tôi dự định sẽ tải trực tiếp tệp PDF lên GitHub:
...
*Nếu bạn không muốn thêm nội dung trực tiếp trên GitHub và bạn đã điền sẵn các liên kết trong tệp `bet.yml` ở các bước trước, bạn có thể bỏ qua phần này và chuyển thẳng đến bước tạo Pull Request.*
...
- Ví dụ, tôi đã thêm tệp PDF về danh sách BIP39:
...
- Quay lại thư mục `/assets` và nhấn vào tệp bạn vừa tải lên:
![event](assets/46.webp)
- Sao chép đường dẫn (URL) trung gian của tệp. Ví dụ, trong trường hợp của tôi, URL là:
...
```
Thao tác này giúp chúng ta xác định trước đường dẫn cố định cho tệp tin của bạn, một khi đề xuất đóng góp được chấp nhận và gộp (merge) vào repository gốc của Plan ₿ Academy.
- Quay lại tệp `bet.yml` của bạn và nhấn vào biểu tượng bút chì: ![event](assets/47.webp)
