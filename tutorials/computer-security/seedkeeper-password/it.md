...
Seedkeeper è una smartcard sviluppata da Satochip, un'azienda belga specializzata in soluzioni hardware per la gestione e la protezione dei segreti digitali. Rinomata per la sua gamma di smartcard per l'ecosistema Bitcoin, Satochip ha concepito il Seedkeeper come alternativa ai metodi tradizionali di memorizzazione delle frasi Mnemonic e di altri segreti digitali.



Concretamente, il Seedkeeper ha la forma di una smart card multifunzionale, certificata EAL6, con un processore sicuro e una memoria a prova di manomissione (cioè un "*secure element*"). Come suggerisce il nome, il suo ruolo è quello di memorizzare le mnemotecniche e le password del portafoglio Bitcoin in modo criptato e protetto. Con Seedkeeper, è possibile generate, importare, organizzare e salvare i propri segreti direttamente nel componente sicuro della scheda.
...
- Memorizzazione delle frasi Bitcoin** Mnemonic: invece di scrivere le vostre 12 o 24 parole su carta, potete importarle nella smartcard e proteggerle con un codice PIN.
...
In questo tutorial, ci occuperemo solo dell'uso di SeedKeeper per le password tradizionali, alla maniera di un gestore di password. Se si desidera utilizzare SeedKeeper per salvare frasi Bitcoin Wallet Mnemonic, consultare quest'altro tutorial:
...
## 3. Inizializzazione del Seedkeeper
...
Posizionare quindi la scheda sul retro dello smartphone. L'applicazione rileverà che Seedkeeper non è ancora stato inizializzato e chiederà di impostare il codice PIN della smart card, di lunghezza compresa tra 4 e 16 caratteri. Per una sicurezza ottimale, scegliete un codice PIN robusto, il più lungo possibile, casuale e composto da un'ampia varietà di caratteri. Questo PIN è l'unica barriera contro l'accesso fisico alle vostre password.
...
## 4. Salvate le password su Seedkeeper
...
Selezionare "generate segreto*". L'opzione "*Importa un segreto*" consente di salvare un segreto esistente (ad esempio, una password creata in passato).
...
Nel nostro caso, vogliamo salvare una password. Fare clic su "*Password*".
...
Assegnare una "*Etichetta*" a questo segreto, in modo da poterlo identificare facilmente se si memorizzano diverse informazioni in Seedkeeper. È inoltre possibile aggiungere un nome utente associato alla password e l'URL del sito.
...
Scegliere la lunghezza e i parametri della password, quindi fare clic su "*generate*", quindi su "*Import*".
...
Se volete controllare la vostra password, prendete il vostro Seedkeeper e fate clic sul pulsante "*Click & Scan*".
...
In questo modo si accede a un elenco di tutti i segreti registrati. In questo esempio, voglio visualizzare la password del mio account Plan ₿ Network, quindi faccio clic su di essa.
...
La password salvata in precedenza appare ora sullo schermo. È possibile copiarla e utilizzarla sul sito web in questione.
...
Per farlo, portate con voi il secondo Seedkeeper (ricordate di identificare uno dei due con un marchio per evitare confusione). Iniziate ad inizializzarlo, come descritto nella fase 3 di questa guida. Ancora una volta, scegliete un codice PIN forte. A seconda della vostra strategia, potete optare per un PIN diverso o mantenere lo stesso.
...
Aprite l'applicazione, fate clic su "*Click & Scan*", inserite il PIN del vostro Seedkeeper n°1 (sorgente), quindi scansionatelo.
...
Questo è tutto! Ora sapete come utilizzare Seedkeeper per memorizzare le vostre password. In un prossimo tutorial, vedremo come utilizzare Seedkeeper per eseguire il backup di un portafoglio Bitcoin. Vi invito anche a scoprire il suo uso combinato con SeedSigner :
