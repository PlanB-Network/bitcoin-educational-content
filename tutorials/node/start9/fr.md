...
Start9 est une société fondée en 2020, principalement reconnue pour avoir développé [**StartOS**,] (https://github.com/Start9Labs/start-os) un système d'exploitation basé sur Linux conçu pour les serveurs personnels. Il permet aux utilisateurs d'auto-héberger facilement une large gamme de services logiciels, tels que les nœuds Bitcoin et Lightning, les applications de messagerie ou les gestionnaires de mots de passe, tout en conservant un contrôle total sur leurs données et sans dépendre des plates-formes technologiques centralisées. StartOS dispose d'une interface conviviale basée sur un navigateur, d'une place de marché pour l'installation de services et d'outils de protection de la vie privée intégrés tels que l'intégration de Tor et le cryptage HTTPS à l'échelle du système. Start9 commercialise également des appareils matériels directement préconfigurés avec le système d'exploitation, bien que le logiciel puisse être installé sur du matériel compatible ou des machines virtuelles (VM).
...
StartOS et Umbrel facilitent tous deux l'installation de services auto-hébergés, mais leur approche diffère au niveau de l'architecture et du fonctionnement. Umbrel s'exécute comme une couche applicative sur les systèmes Debian/Ubuntu ou les machines virtuelles, tandis que Start9 fournit un véritable système d'exploitation dédié qui nécessite une installation directe sur le matériel ou la machine virtuelle. Umbrel propose une interface soignée, inspirée de macOS, tandis que Start9 adopte un design fonctionnel et minimal. Umbrel met à disposition une large [sélection d'applications] (https://apps.umbrel.com/), tandis que le [Start9 Marketplace] (https://marketplace.start9.com/) compense avec des capacités techniques avancées. Start9 facilite l'accès aux paramètres avancés grâce à des formulaires d'interface utilisateur validés, réduisant ainsi le besoin d'interaction avec la ligne de commande. Il se distingue également par sa gestion des sauvegardes avec des sauvegardes cryptées du système en un clic, une fonctionnalité qui fait défaut à Umbrel. StartOS fournit des outils de surveillance intégrés et applique le chiffrement HTTPS pour l'accès au réseau local, ce qui renforce la sécurité. Umbrel utilise HTTP non chiffré localement, bien que les deux plateformes supportent l'accès à distance sécurisé via Tor. Umbrel se distingue par son écosystème d'applications très riche et son interface d'utilisateur soignée. Start9 est un choix solide pour ceux qui apprécient la sécurité, la configurabilité, la fiabilité des sauvegardes et la gestion avancée des services sans avoir besoin d'une expertise en ligne de commande. Pour en savoir plus sur Umbrel et les différences avec Start9, visitez ce cours :
...
Pour commencer l'installation, utilisez un autre ordinateur et rendez-vous sur le [site web Start9] (https://start9.com/), et naviguez jusqu'à la section documentation en cliquant sur `DOCS`. De là, accédez aux `Flashing Guides` pour trouver la version appropriée de StartOS. A ce stade, deux méthodes d'installation s'offrent à vous :
...
Une fois l’image StartOS téléchargée et vérifiée, vous devez la flasher sur une clé USB. le logiciel `BalenaEtcher` est recommandé pour cette tâche. Il s'agit d'un outil libre et gratuit pour écrire des fichiers image de système d'exploitation sur des lecteurs USB et des cartes SD, disponible pour Windows, macOS et Linux. Téléchargez la version appropriée sur le site officiel [Balena Etcher website] (https://www.balena.io/etcher/) et exécutez le programme d'installation. Connectez la clé USB ou la carte SD cible, ouvrez Balena Etcher et cliquez sur "Flash from file" pour sélectionner l'image du système d'exploitation téléchargée. Etcher détectera automatiquement les lecteurs connectés ; sélectionnez la cible correcte s'il y en a plusieurs. Cliquez sur "Flash!`" pour lancer l'écriture. Etcher valide automatiquement le processus d'écriture à la fin. Une fois l'écriture terminée, retirez le lecteur en toute sécurité et utilisez-le pour démarrer votre appareil.
...
Pour la configuration initiale, consultez la page Start9 [documentation] (https://docs.start9.com/0.3.5.x/) sous la rubrique "MANUEL DE L'UTILISATEUR" suivie de "Prise en main - Configuration initiale".  Ce guide officiel doit être consulté pour obtenir les informations les plus récentes.
