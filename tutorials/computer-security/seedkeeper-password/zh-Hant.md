...
Seedkeeper 是由 Satochip 開發的智慧卡，Satochip 是一家比利時公司，專門提供管理和保護數位機密的硬體解決方案。Satochip 以其 Bitcoin 生態系統的智慧卡系列而聞名，Seedkeeper 的構想是取代傳統的 Mnemonic 詞組和其他數位機密的儲存方式。



具體而言，Seedkeeper 採用多功能、經 EAL6 認證的智慧卡形式，具有安全處理器和防篡改記憶體（即 "*secure element*"）。顧名思義，它的作用是以加密和受保護的方式儲存 Bitcoin 組合代號和密碼。有了 Seedkeeper，您可以直接在卡片的安全元件中儲存 generate、匯入、整理和儲存您的秘密。
...
在本教程中，我們只會介紹如何使用 SeedKeeper 以密碼管理器的方式儲存傳統密碼。如果您想使用 SeedKeeper 來儲存 Bitcoin Wallet Mnemonic 詞組，請參閱此其他教學：
...
第一步是在電腦或智慧型手機上安裝軟體。在電腦上，您需要 [下載最新版本的 Satochip-Utils](https://github.com/Toporin/Satochip-Utils/releases)。在手機上，Seedkeeper 應用程式可在 [Google Play 商店](https://play.google.com/store/apps/details?id=org.satochip.seedkeeper) 以及 [Apple App 商店](https://apps.apple.com/be/app/seedkeeper/id6502836060) 下載。
...
選擇「generate secret*」。*Import a secret*」選項可讓您儲存現有的密碼 (例如，您過去建立的密碼)。
...
為這個秘密指定一個「*標籤*」，以便在您的 Seedkeeper 中儲存多項資訊時，可以輕易識別。您也可以加入與密碼相關的使用者名稱以及網站的 URL。
...
選擇密碼的長度和參數，然後按一下「*generate*」，再按「*Import*」。
...
這會帶您進入所有註冊密碼的清單。在這個範例中，我想要顯示 Plan ₿ Network 帳戶的密碼，因此我按一下它。
...
若要執行此步驟，請帶著您的第二個 Seedkeeper（記得在兩個 Seedkeeper 之中的一個做上標記，以免混淆）。依照本教學第 3 步所述，開始初始化它。再一次，選擇一個強大的 PIN 碼。根據您的策略，您可以選擇不同的 PIN 碼或保留相同的 PIN 碼。
...
就是這麼多了！現在您知道如何使用 Seedkeeper 儲存密碼了。在未來的教學中，我們將介紹如何使用 Seedkeeper 備份 Bitcoin 的投資組合。我也邀請您發現它與 SeedSigner 的結合使用：
