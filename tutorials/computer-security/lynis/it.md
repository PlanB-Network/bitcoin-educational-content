...
Il progetto è disponibile a questo indirizzo su **Github**:
- [GitHub - Lynis](https://github.com/CISOfy/lynis)
- [Scarica Lynis da CISOFY](https://cisofy.com/lynis/#download)

In questa guida passo-passo, **utilizzerò una VPS con Debian 12** e mi concentrerò sulla parte SSH, in quanto vorrei verificare la sua configurazione e migliorarla.
...
### B. Scaricalo dal sito web ufficiale
...
```
sudo lynis
# Se tu hai scaricato Lynis compilando i sorgenti, utilizza questa sintassi:
./lynis
```
...
Nel resto dell'articolo, imparerai a utilizzare alcune opzioni.

### B. Esecuzione dell'audit del sistema

Chiederai a **Lynis** di verificare il tuo sistema, evidenziando ciò che è configurato correttamente e ciò che potrebbe essere migliorato. Per farlo, inserisci il seguente comando:
...
Alla fine della verifica, ottieni questo (possiamo vedere che **Lynis** ha identificato correttamente il sistema **Debian 12** e userà il **Debian Plugin** per l'analisi):

![Image](assets/fr/017.webp)

Successivamente, Lynis elencherà una serie di punti corrispondenti a tutto ciò che ha controllato nel nostro sistema. Come vedrai, l'elenco è organizzato per categorie. Vale la pena notare che viene utilizzato un codice colore per evidenziare le raccomandazioni:
- **Rosso** per elementi critici o best practice non rispettate (un pacchetto mancante, per esempio), cioè il tuo server non rispetta questo punto
...
- **Bianco**, quando è neutrale

Qui puoi vedere che Lynis raccomanda di installare **fail2ban**:
...
Quindi, la sezione "**Utenti, gruppi e autenticazione**". Lo strumento ti informa di un avviso sui permessi della directory "**/etc/sudoers.d**". Per il momento non sappiamo altro, ma alla fine dell'analisi potremo vedere quali sono le raccomandazioni.

![Image](assets/fr/049.webp)

Ecco cosa puoi trovare nelle sezioni "**Shells", "Files Systems" e "USB Devices "**. Tra le altre cose, puoi vedere che ci sono suggerimenti sui punti di montaggio e che i dispositivi USB sono attualmente consentiti su questa macchina.

![Image](assets/fr/048.webp)

Poi, le sezioni: ""Name services", "Ports and packages", "Networking". Indicano che i pacchetti non più in uso possono essere eliminati e che non esiste un'utilità in grado di gestire gli aggiornamenti automatici.
...
Puoi vedere che ha trovato i file di configurazione di **Nginx** e che per la parte SSL/TLS, i **ciphers** sono configurati con l'uso di un protocollo che non sarebbe sicuro. D'altra parte, secondo Lynis, la gestione dei log è corretta.

![Image](assets/fr/038.webp)

Il servizio SSH è presente sul tuo server VPS. La sua configurazione è analizzata da Lynis. Va detto che la sicurezza di SSH può essere facilmente migliorata! Torneremo su quest'area in dettaglio una volta ottenute le raccomandazioni.
...
Sei arrivato alla fine dell'analisi... Quest'ultimo punto dimostra che avrai tutto da guadagnare dall'avere ClamAV su questa macchina.
...
- Questo strumento suggerisce di installare **fail2ban** per bloccare automaticamente gli host che non riescono ad autenticarsi (in particolare con la forza bruta).
...
- Questo strumento suggerisce di impostare valori minimi e massimi per il periodo di validità di una password. In questo modo si garantisce, tra l'altro, che le password vengano cambiate regolarmente.
...
Per migliorare la configurazione del sistema, installerai alcuni pacchetti sulla macchina: alcuni raccomandati da Lynis, altri che consiglio personalmente.

Avrai una buona base su cui lavorare, purché dedicherai un po' di tempo alla loro configurazione. A tale scopo, consulta il sito IT-Connect, altri articoli sul Web e la documentazione dello strumento.
...
Diamo un'occhiata più da vicino alla tua attuale configurazione **SSH** in:**/etc/ssh/sshd_config**

![Image](assets/fr/018.webp)

La configurazione suggerita di seguito può ancora essere ottimizzata, ma fornisce una buona base. *Si noti che sono stati rimossi alcuni commenti per una maggiore leggibilità*.

Tu dovrai:
- Cambiare la porta di connessione SSH (dimentica la 22 predefinita)
...
- Attiva la **Modalità di restrizione**
...
- Imposta **MaxAuthtries** su 3

Rappresenta il numero di tentativi di autenticazione falliti prima che la comunicazione venga chiusa.

- Imposta **MaxSessions** su 2

Rappresenta il numero massimo di sessioni simultanee.

- Abilita l'autenticazione a chiave pubblica

```
PubkeyAuthentication yes
```

- Mantieni l'autenticazione con password:

```
PasswordAuthentication yes
```

Vieta le password vuote e l'autenticazione **Kerberos**, così come l'autenticazione **diretta di root**
...
Assicurarti di avere "**PermitRootLogin no"**, se è uguale a **"yes"**, è il **"male assoluto "**.

- Vieta il reindirizzamento delle connessioni TCP
...
- Imposta il timeout di comunicazione tra il client e sshd
...
Dai un'occhiata ai risultati sul tuo secondo server, dove hai dedicato un po' più di tempo all'hardening! Quindi è normale che il punteggio sia più alto (**76**).
...
Creerai anche alcune variabili contestuali che ti aiuteranno a organizzarti meglio, come il nome dell'host e la data per assegnare il nome al report e la marcatura temporale, e il percorso della cartella in cui vogliamo inserire i nostri report.
...
Probabilmente non si applicherà la stessa configurazione per un VPS esposto alla Rete, dove è necessaria una sola connessione SSH (perché si è l'unica persona a connettersi), a differenza di un **bastion** o di uno **scheduler** che avranno bisogno di moltiplicare le connessioni **SSH.**
...
Non dimenticare di comunicare con i tuoi team prima dell'implementazione: devi far capire loro perché stai facendo questi cambiamenti, non solo dirgli che li stai facendo. Alla fine, l'obiettivo sarà quello di trasmettere le buone pratiche, aumentando così le possibilità di successo.
...
**Questo tutorial è terminato, divertiti con Lynis!**
