...
LNbits ist ein in Python (FastAPI) entwickeltes Lightning-Buchhaltungssystem, das sich mit einem bestehenden Backend (LND, Core Lightning, PHOENIXD) verbindet. Im Gegensatz zu herkömmlichen Lightning-Knoten bietet LNbits ein zugängliches Interface, mit dem Sie mehrere isolierte Portfolios mit eigenen API-Schlüsseln verwalten können. Sie können Unterkonten für Ihre Familie, Mitarbeiter oder Projekte einrichten, ohne ihnen Zugriff auf alle Ihre Gelder zu geben.
...
**Ubuntu VPS**: Leichte Lösung ohne vollen Bitcoin-Knoten. Voraussetzungen: 1 vCPU, 1-2 GB RAM, Ubuntu 22.04 LTS, Python 3.10+, Git, UV. HTTPS + Domainname für die öffentliche Darstellung erforderlich (LNURL-Dienste).



**Umbrel**: Einfache Installation aus dem App Store. Voraussetzung: funktionierender Umbrel-Knoten mit synchronisiertem LND und offenen Kanälen. Automatische Konfiguration.
...
Sobald Ihr Server gesichert ist, müssen Sie PHOENIXD installieren und konfigurieren. Plan B Network bietet ein komplettes Tutorial zur Installation, seed-Generierung und systemd-Dienstkonfiguration:
...
Aktivieren Sie den Interface-Administrator in `.env`:
...
Gehen Sie in der SuperUser-Verwaltung Interface auf "Einstellungen" und dann auf den Abschnitt "Benutzerverwaltung". Dort finden Sie die Option "Erstellung neuer Benutzer zulassen".
...
- Ein Angreifer könnte ohne Ihr Wissen Konten erstellen und die Liquidität Ihres LIGHTNING NODE nutzen
- Sie müssen die Benutzerkonten manuell über den Interface SuperUser
...
Einmal konfiguriert, kann nur der SuperUser-Administrator über Interface "Benutzer" neue Benutzerkonten erstellen. Dieser Ansatz garantiert die vollständige Kontrolle darüber, wer auf Ihre Lightning-Infrastruktur zugreifen und Ihre Mittel verwenden kann.
...
PHOENIXD verwaltet automatisch Kanäle über Auto-Liquidität. generate ein Lightning Invoice von ~30.000 Sats von LNbits und bezahlt es von einem anderen Wallet. PHOENIXD eröffnet automatisch einen Kanal zu ACINQ. Die Eröffnungsgebühr (~20-23k Sats) wird abgezogen, der Restsaldo (~7-10k Sats) erscheint nach der Bestätigung von On-Chain.



Prüfen Sie den Status mit `./Phoenix-CLI getinfo`. Dann erwägen Sie die Deaktivierung von auto-liquidity (`auto-liquidity=off` in `Phoenix.conf`), um Kanalöffnungen zu kontrollieren.
...
**Wichtig**: HTTPS obligatorisch für die öffentliche Anzeige (API-Schlüsselsicherheit + LNURL-Kompatibilität). Überspringen Sie diesen Schritt nur für die lokale Nutzung.
...
Umbrel prüft automatisch, ob Abhängigkeiten erforderlich sind. LNbits benötigt LIGHTNING NODE (LND) um zu laufen. Wenn Ihr LIGHTNING NODE bereits betriebsbereit ist, klicken Sie zur Bestätigung auf "LNbits installieren".
...
Beim ersten Start fordert LNbits Sie auf, das SuperUser-Administratorkonto zu erstellen. Geben Sie einen Benutzernamen ein und legen Sie ein sicheres Passwort fest, um den Zugriff auf das Interface-Verwaltungssystem zu schützen.
...
Sobald Sie ein Konto erstellt haben, werden Sie automatisch in den Hauptverwaltungsbereich von Interface weitergeleitet. Umbrel hat LND bereits als Ihre Finanzierungsquelle eingerichtet - alle Blitzzahlungen werden über Ihre bestehenden Kanäle abgewickelt.



### Zugang zum Interface-Administrator
...
**Wichtiger Hinweis**: Jeder Benutzer kann sich völlig unabhängig mit seinem eigenen Passwort anmelden. Der SuperUser-Administrator behält über das Interface-Verwaltungstool die volle Kontrolle.



### Wallet-Benutzerverwaltung



Nachdem der "Satoshi"-Benutzer angelegt wurde, müssen Sie ihm einen Wallet-Blitz zuweisen. Klicken Sie auf das Wallet-Symbol (zweites Symbol) für den betreffenden Benutzer und dann auf "NEUEN Wallet ERSTELLEN".
...
Der "Satoshi"-Benutzer hat jetzt einen einzigen, eindeutig identifizierten Wallet. Jeder Wallet-Benutzer arbeitet völlig autonom, nutzt aber die Liquidität des zugrunde liegenden LND-Knotens.



**Schlüsselkonzept**: Alle diese Wallets teilen sich die globale Liquidität Ihres Umbrel-Knotens. Sie erstellen keine neuen Lightning-Kanäle für jeden Wallet - LNbits fungiert als intelligenter buchhalterischer Layer, der die Zuteilung von Mitteln innerhalb Ihrer bestehenden Lightning-Infrastruktur verwaltet. Das ist die Stärke von LNbits' Multi-Wallet-System.
...
Geben Sie den zuvor festgelegten Benutzernamen ("Satoshi") und das Kennwort ein und klicken Sie dann auf "LOGIN". Der Benutzer erhält direkten Zugang zu seinem persönlichen Wallet, völlig isoliert von der Verwaltung Interface.
...
Sobald die Verbindung hergestellt ist, greift der Benutzer von Wallet Lightning auf seinen Interface zu.
...
Die Merkmale des Interface sind:




- Aktueller Saldo**: Wird in Sats und in der gewählten Währung (in diesem Beispiel CUC) angezeigt
- Wichtigste Aktionen**: "ANTRAG EINFÜGEN" (Einfügen einer Rechnung zum Bezahlen), "Invoice ERSTELLEN" (generate eine Quittung), QR-Symbol (Schnellscan)
...
### Wallet mobiler Zugang



Die rechte Seitenwand bietet eine besonders praktische Funktion: den mobilen Zugriff auf das Wallet. Klappen Sie den Abschnitt "Mobiler Zugriff" aus, um die verfügbaren Optionen zu entdecken.
...
LNbits bietet mehrere Möglichkeiten, den Wallet auf einem Smartphone zu verwenden:
...
- Der QR-Code, der in "Export to Phone with QR Code" angezeigt wird, enthält die vollständige URL des Wallet mit integrierter Authentifizierung
...
Dieser doppelte Layer-Schutz (Netzzugang + Benutzerauthentifizierung) macht die Option "Anlegen neuer Benutzer zulassen" bei Umbrel weniger kritisch. Nur Personen, die bereits Zugang zu Ihrem Netzwerk oder VPN haben, können das Interface Login erreichen.



**Typisches Szenario**: Sie erstellen ein "Vater"-Konto, ein "Mutter"-Konto, ein "Geschäfts"-Konto und so weiter. Jedes Familienmitglied hat seinen eigenen isolierten Wallet Lightning, profitiert aber von der gemeinsamen Liquidität Ihres Umbrel-Knotens. Teilen Sie einfach den Benutzernamen und das Passwort mit - der Benutzer kann sich dann von jedem Gerät in Ihrem lokalen Netzwerk oder über Ihr Tailscale VPN verbinden. Bitte lesen Sie unser Tailscale-Tutorial für weitere Informationen:
...
Kehren Sie zum Interface SuperUser zurück und öffnen Sie das Menü "Erweiterungen" auf der linken Seite, um das gesamte Ökosystem der LNbits-Erweiterungen zu entdecken.
...
- Support-Tickets**: Bezahltes Support-System (erhalten Sie Satss zur Beantwortung von Fragen)
...
- SMTP**: Ermöglicht es den Benutzern, E-Mails zu versenden und Satss zu verdienen
...
Jede Erweiterung wird mit einem einzigen Klick von diesem Interface aus aktiviert. Erweiterungen, die mit "FREE" gekennzeichnet sind, sind kostenlos, während einige als "PAID"-Versionen verfügbar sind. Durchsuchen Sie den Katalog, um die Erweiterungen zu finden, die Ihren Bedürfnissen entsprechen - sei es für Unternehmen, Familienmanagement oder zum Experimentieren mit den Funktionen von Lightning Network.
...
**Vorteile**: Finanzielle Souveränität (volle Kontrolle über Fonds/Schlüssel/Daten), architektonische Flexibilität (verlustfreie VPS→Full node-Migration), professionelles Erweiterungssystem, intuitiver Interface.



**Einschränkungen** : Software in der Beta-Phase (Vorsicht bei den Beträgen), Sicherheit unter der Verantwortung des Administrators, URLs mit sensiblen API-Schlüsseln (HTTPS obligatorisch), Mehrbenutzer-Verwaltung impliziert die Verantwortung des Administrators.
...
**Backups**: seed PHOENIXD/Zugangsdaten LND, LNbits-Datenbank, `.env`-Dateien. Täglich automatisieren, vom Produktionsserver fernhalten, verschlüsselt. Testen Sie Wiederherstellungen regelmäßig.
