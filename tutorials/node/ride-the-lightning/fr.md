...
**Ride The Lightning (RTL)** est une interface web complète permettant de gérer un nœud Lightning Network. Cette application web auto-hébergée offre un **"cockpit" Lightning** accessible depuis votre navigateur. RTL fonctionne avec les principales implémentations Lightning (LND, Core Lightning/CLN et Eclair) et donne un contrôle total sur votre nœud et vos canaux. Open-source (licence MIT) et gratuit, RTL est intégré par défaut dans de nombreuses solutions de nœuds clés en main (RaspiBlitz, MyNode, Umbrel, etc.).

**Pourquoi utiliser RTL ?** Sans interface graphique, la gestion d'un nœud Lightning se fait via des commandes CLI peu intuitives. RTL simplifie ces opérations avec une interface ergonomique. Voici les **cas d'usage principaux** :

**Visualiser ses canaux et son nœud** – Le tableau de bord affiche le solde on-chain, la liquidité Lightning (locale/distante), l'état de synchronisation, l'alias du nœud, etc. Vous pouvez consulter la liste de vos canaux, leur capacité, la répartition local/distante et leur état. RTL propose également des vues adaptées pour analyser l'activité sous différents angles.

**Gérer les canaux Lightning** – L'ouverture ou la fermeture de canaux se fait en quelques clics. RTL permet de se connecter à un pair et d'ouvrir un canal sans utiliser une commande manuelle. Vous pouvez également ajuster les frais de routage, consulter le score d'équilibre, ou initier un rebalance circulaire pour rééquilibrer les fonds entre canaux.

**Suivre et effectuer des paiements** – RTL gère les transactions Lightning : envoi de paiements via factures (invoices), génération de factures pour recevoir, suivi des transactions (paiements, routage) avec historique détaillé. L'interface analyse aussi le routage pour visualiser les paiements transitant par votre nœud.

**Gestion du wallet on-chain et sauvegardes** – L'onglet on-chain permet de générer des adresses et d'envoyer des transactions Bitcoin. RTL simplifie également la sauvegarde des canaux en permettant l'export du  fichier SCB pour LND, automatiquement mise à jour après chaque modification de canal.

En résumé, RTL est un **tableau de bord puissant pour le Lightning Network**, offrant une interface claire et pédagogique pour piloter votre nœud au quotidien. Ce tutoriel vous accompagnera dans son installation et son utilisation afin de gérer vos canaux et paiements en toute simplicité.
...
Avant de passer à la pratique, il est utile de comprendre brièvement **comment RTL interagit avec votre nœud Lightning** du point de vue technique.

**Architecture générale :** RTL est une application web construite avec Node.js (backend) et Angular (frontend). Concrètement, RTL tourne comme un petit serveur web local (sur le port 3000 par défaut) qui communique avec votre implémentation Lightning via les API de celle-ci. Selon le type de nœud :

* Avec **LND**, RTL utilise l'API REST de LND (port 8080) pour exécuter les commandes Lightning. La connexion est sécurisée par TLS et nécessite le fichier **macaroon d'admin** de LND afin de s'authentifier.

**Avec Core Lightning (CLN)**, RTL utilise soit l'API REST fournie par *c-lightning-REST*, soit un **rune d'accès** via le plugin `commando`. Sur certaines distributions comme Umbrel, cette configuration est automatiquement installée et prête à l’emploi.
**Avec Eclair**, RTL se connecte à l'API REST d'Eclair en utilisant le mot de passe d'authentification que vous avez configuré au préalable.

**Configuration et sécurité :** RTL se configure au moyen d'un fichier JSON (`RTL-Config.json`) dans lequel vous renseignez le port web, le mot de passe d'accès, et les informations de connexion à votre nœud. L'interface web est protégée par un login/mot de passe (par défaut `password` à changer) et supporte la 2FA. Par défaut, RTL s'exécute en HTTP local (`http://localhost:3000`), mais pour un accès distant, utilisez toujours une connexion sécurisée (HTTPS via reverse-proxy, Tor, ou VPN).

En résumé, RTL est un composant supplémentaire qui se connecte à votre nœud via des API sécurisées, nécessite des jetons d'accès appropriés, et fourni sa propre couche de sécurité. Cette architecture modulaire permet même de gérer **plusieurs nœuds Lightning avec une seule instance RTL**.

## 3. Installation de RTL

RTL étant distribué en open-source, plusieurs méthodes d’installation du système sont possibles. Dans cette section, nous allons explorer deux approches principales : l'installation manuelle et l'installation via Umbrel.

### Méthode manuelle

L'installation manuelle est idéale si vous souhaitez garder le contrôle précis ou si vous intégrez RTL dans une configuration personnalisée. Les étapes présentées ci-dessous concernent un nœud LND tournant sous Linux (ex: Raspberry Pi ou VPS sous Ubuntu/Debian), mais la procédure reste très similaire pour CLN/Eclair.
...
**Configurer RTL** : Une fois les dépendances installées, préparez le fichier de configuration. Copiez/renommez le fichier `Sample-RTL-Config.json` à la racine du projet en `RTL-Config.json`. Ouvrez-le dans votre éditeur :
...
**(Optionnel) Exécuter RTL en service** : Si vous souhaitez un démarrage automatique, créez un service systemd :
...
- Remplacez `/chemin/vers/RTL/rtl` par l'emplacement réel vers le fichier `rtl` sur votre machine, et `<votre_user>` par votre nom d'utilisateur Linux.
...
RTL lancera désormais automatiquement à chaque redémarrage de la machine. Vous pouvez vérifier son statut avec :
```bash
...
- **Ride The Lightning** (pour LND) : à installer si vous utilisez le nœud Lightning par défaut d'Umbrel (LND).
- **Ride The Lightning (Core Lightning)** : à installer uniquement si vous avez ajouté l'application *Core Lightning* sur Umbrel et souhaitez gérer ce nœud avec RTL.

*Chaque version de RTL est configurée pour fonctionner avec l'implémentation respective (LND ou Core Lightning). Si vous n'avez pas modifié de nœud Lightning, vous pouvez simplement choisir la version classique pour LND .*
...
**Important :** Après l'installation, RTL affiche une page contenant le mot de passe par défaut. **Copiez et sauvegardez ce mot de passe** - car vous en aurez besoin pour vous connecter à l’interface RTL. Ce mot de passe sera affiché à chaque démarrage de RTL jusqu'à ce que vous cochiez l'option "Don't show it again". 
...
Une fois l'installation terminée, l'application apparaît dans votre menu *Apps* sur Umbrel. Cliquez sur **Ride The Lightning** pour la lancer.
...
Sur l'écran de connexion, saisissez le mot de passe que vous avez copié précédemment. Une fois connecté, l'interface web RTL sera accessible directement depuis le dashboard Umbrel, sans configuration supplémentaire nécessaire!

## 4. Présentation et utilisation de l'interface RTL

Maintenant que RTL est installé et opérationnel, découvrons ensemble son interface web et ses principales fonctionnalités. Nous allons passer en revue les différentes sections de l'application afin de vous offrir une vue d'ensemble complète.
...
Dès votre connexion, vous arrivez sur le **tableau de bord principal** qui présente une vue globale de votre nœud Lightning. Cette page regroupe les informations essentielles :
...
L'onglet **On-chain** vous donne accès à la gestion de vos bitcoins directement sur la chaîne principale :
...
L'interface RTL propose un menu latéral dédié au Lightning Network, regroupant toutes les fonctionnalités nécessaires à la gestion de votre nœud. Voici le détail de chaque sous-menu, dans l'ordre indiqué sur la capture d'écran :
...
- Consulter la liste de vos pairs connectés et de vos canaux Lightning ouverts ou en attente.
- Ajouter un nouveau pair (connexion à un autre nœud Lightning).
- Ouvrir, fermer ou gérer les canaux existants.
- Accéder aux détails de chaque canal : capacité, soldes locaux/distants, état, historique des échanges, score d'équilibre, etc.
...
- Effectuer des paiements Lightning (via une invoice).
...
- Les paiements routés par votre nœud pour le compte d'autres utilisateurs du réseau Lightning.
- Les statistiques de routage : nombre de transactions relayées, frais gagnés, erreurs rencontrées.
- Un historique exportable, utile pour analyse avancée.
...
- Des outils permettant d'analyser plus simplement la performance de votre nœud.
...
- La vérification de signatures provenant d’autres nœuds pour authentifier un message.

#### 7. Backup

Ce sous-menu est consacré à la sauvegarde et permet :
- D'exporter le fichier de backup des canaux (SCB pour LND).
- De restaurer de la configuration ou des canaux en cas de besoin.
- D’accéder à des recommandations pour sécuriser vos sauvegardes.
...
- Un aperçu complet de l'état de votre nœud Lightning : alias, version, couleur, état de synchronisation.
- Les statistiques sur les canaux (actifs, en attente, fermés), la capacité totale, la connectivité.
- Des informations concernant le réseau Lightning dans son ensemble ainsi que la place occupée par votre nœud au sein de ce réseau.
...
À l'inverse, le Swap In permet de réinjecter des fonds on-chain dans le réseau Lightning. Cela est particulièrement utile lorsqu'on souhaite restaurer la capacité de sortie de ses canaux. Le déroulement est le suivant :
...
Ces deux mécanismes permettent d'optimiser la liquidité des canaux Lightning tout en vous garrantissant un contôle total et souverain sur ses fonds.
...
Cette interface complète vous permet de gérer efficacement votre nœud Lightning, des opérations basiques aux fonctionnalités avancées, le tout dans une interface intuitive et bien structurée.

## 5. Cas d'usage avancés & sécurité

Dans cette section, voici les points essentiels à retenir pour aller plus loin avec RTL et renforcer la sécurité de votre nœud Lightning :
...
En appliquant ces principes, vous réduisez considérableement les risques et conservez un contrôle total sur votre nœud Lightning.

## 7. Conclusion

**Ride The Lightning** est un outil incontournable pour gérer efficacement un nœud Bitcoin/Lightning, que vous soyez débutant ou utilisateur avancé. Il fournit une interface claire pour administrer vos canaux, vos paiements et l'état de votre nœud, tout en vous aidant à approfondir votre compréhension du Lightning Network.

RTL se démarque grâce à sa compatibilité multi-implémentation, à ses fonctions avancées (rebalancing, swaps, rapports) et à sa pédagogie. Pour des besoins simples, l'interface Umbrel ou un wallet mobile suffisent, mais RTL prend tout son sens pour une gestion active et optimisée de votre nœud.
...
En résumé, RTL vous offre un contrôle complet sur votre nœud Lightning, au sein d'une interface moderne et complète.
