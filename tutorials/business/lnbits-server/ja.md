---
name: LNbitsサーバー
description: PHOENIXDまたはUmbrel上のUbuntu VPSへのLNbitsサーバーのインストールと設定
---
...
LNbitsは、あらゆるLightningバックエンド（LND、Core Lightning、PHOENIXD）を完全なサービスプラットフォームに変換するオープンソースのInterfaceウェブアプリケーションです。このセルフホストソリューションにより、資金を完全に管理しながら、複数のLightningポートフォリオを個別に管理したり、販売ポイントを展開したり、寄付システムや課金サービスを作成したりすることができます。



このチュートリアルでは、2つのインストール・アプローチについて説明します： **VPS Ubuntu with PHOENIXD** (完全な Bitcoin ノードなしの軽量ソリューション) と **Umbrel** (既存の LND ノードとの統合) です。Plan B Networkの一般的なLNbitsチュートリアル（コンセプトと拡張機能をカバー）とは異なり、このガイドはステップバイステップの技術的なインストール手順に重点を置いています。
...
LNbitsは、既存のバックエンド（LND、Core Lightning、PHOENIXD）に接続するPython（FastAPI）で開発されたLightning会計システムです。従来のLightningノードとは異なり、LNbitsはアクセス可能なInterfaceを提供し、独自のAPIキーで複数の独立したポートフォリオを管理することができます。家族、従業員、プロジェクト用のサブアカウントを作成し、すべての資金にアクセスできるようにする必要はありません。



分離されたアーキテクチャは、SQLite（デフォルト）またはPostgreSQL（プロダクション）に情報を保存し、資金はLightningバックエンドによって管理されます。この分離により、PHOENIXDからLNDへユーザーデータを失うことなく移行することができます。
...
ダッシュボード**には、リアルタイムの残高、取引履歴、請求ツールが表示されます。各WalletはAPIキーを含むユニークなURLを持っており、従来のログインなしでアクセスできます。3レベルのAPIキーシステム**（管理者、Invoice、読み取り専用）は、安全な統合のための権限のきめ細かい制御を提供します。



LNbitsは**LNURL**（LNURL-pay、LNURL-Withdraw、LNURL-auth）をネイティブに実装し、**Lightning Address**をサポートしているため、最新のライトニング・ウォレットとの互換性が保証され、プロフェッショナル・サービスの展開が容易になります。
...
サーバーが安全になったら、PHOENIXD をインストールして設定する必要があります。Plan B Network は、インストール、seed の生成、systemd サービスの設定をカバーする完全な専用チュートリアルを提供しています：



https://planb.academy/tutorials/node/lightning-network/phoenixd-beb86edd-f9c0-4bec-ad36-db234c88e7b1

PHOENIXDが起動したら（`./Phoenix-CLI getinfo`で確認）、`~/.Phoenix/Phoenix.conf`の**HTTPパスワード**に注意してください - LNbitsをPHOENIXDに接続するために必要です。
...
管理画面は`http://<IP-VPS>:5000/Wallet?usr=<SuperUserID>`にアクセスしてください。Server "メニューで資金源、拡張機能、ユーザーアカウントを設定できる。
...
SuperUserの管理画面Interfaceから "設定 "を開き、"ユーザー管理 "セクションに移動します。新規ユーザーの作成を許可する」オプションがあります。
...
- 攻撃者は、あなたの知らないうちにアカウントを作成し、あなたのLIGHTNING NODEの流動性を使用することができます。
- Interface SuperUserから手動でユーザーアカウントを作成する必要があります。
...
一度設定されると、SuperUser管理者のみがInterfaceの "Users "から新しいユーザーアカウントを作成することができます。このアプローチは、誰がLightningインフラにアクセスし、資金を使用できるかを完全にコントロールすることを保証します。
...
PHOENIXDは自動流動性によりチャネルを自動管理。generateにLNbitsから～30,000SatsのLightning Invoiceを受け取り、別のWalletから支払う。PHOENIXDは自動的にACINQにチャネルを開設する。開設手数料（～2～3万Sats）が差し引かれ、残金（～7～1万Sats）はOn-Chainの確認後に表示される。



./Phoenix-CLI getinfo`でステータスをチェックする。それから、チャネルのオープンをコントロールするために、自動流動性を無効にする(`Phoenix.conf`の`auto-liquidity=off`)ことを検討する。
...
Umbrel は必要な依存関係を自動的にチェックします。LNbitsの動作にはLIGHTNING NODE(LND)が必要です。LIGHTNING NODEがすでに動作している場合は、「LNbitsをインストールする」をクリックして確認してください。
...
初回起動時、LNbits は SuperUser 管理者アカウントの作成を促します。Interface管理システムへのアクセスを保護するため、ユーザー名を入力し、安全なパスワードを設定してください。
...
**重要**：このSuperUserアカウントは、LNbitsインスタンスの全権限を持ちます。パスワードは厳重に管理してください。



アカウントを作成すると、自動的にInterfaceのメイン管理エリアに移動します。UmbrelはすでにLNDを資金源として設定しており、Lightningの支払いはすべて既存のルートを通じて行われます。
...
**重要なポイント**：重要**：各ユーザーは自分のパスワードで完全に独立してログオンすることができます。SuperUser の管理者は、Interface の管理ツールで完全にコントロールすることができます。
...
Satoshi」ユーザーが作成されたので、彼にWalletライトニングを割り当てる必要がある。Walletのアイコン（2つ目）をクリックし、「CREATE NEW Wallet」をクリックします。
...
ダイアログ・ボックスが表示され、Wallet に名前を付けるよう求められます。わかりやすい名前（例："Wallet Of Satoshi"）を入力し、表示通貨（CUC、USD、EURなど）を選択します。
...
CREATE "をクリックしてください。LNbitsは即座にこのユーザーのためのWallet Lightningを生成します。
...
Satoshi "ユーザーは現在、明確に識別された単一のWalletを持っています。各ユーザーWalletは、LNDノードの流動性を利用しながら、完全に自律的に動作します。



**キーコンセプト**：これらのウォレットはすべて、あなたのUmbrelノードのグローバル流動性を共有します。LNbitsは、既存のLightningインフラ内で資金配分を管理するインテリジェントな会計Layerとして機能します。これがLNbitsのマルチWalletシステムのパワーです。
...
ユーザー名（"Satoshi"）とパスワードを入力し、"LOGIN "をクリックします。ユーザーは、管理用 Interface から完全に分離された個人用 Wallet に直接アクセスできるようになります。
...
一度接続すると、ユーザーはWallet LightningからInterfaceにアクセスする。
...
- 主なアクション「PASTE REQUEST"（支払い請求書を貼り付ける）、"CREATE Invoice"（generateレシートを作成）、QRアイコン（クイックスキャン）
...
右側のサイドパネルには、Walletへのモバイルアクセスという、特に実用的な機能があります。モバイル・アクセス」セクションを開くと、利用可能なオプションが表示されます。
...
LNbitsはこのWalletをスマートフォンで使う方法をいくつか提案している：
...
- このWalletのためにLNbitsの**LndHub**エクステンションを有効にする。
- モバイルアプリでLndHubのQRコードをスキャンし、Walletを接続します。
...
- QRコードでスマホにエクスポート」で表示されるQRコードには、認証機能を統合したWalletの完全なURLが含まれています。
- スマートフォンからこのQRコードをスキャンすると、モバイルブラウザでWalletを直接開くことができます。
- ホーム画面にページを追加して素早くアクセス



**重要なセキュリティ**：この URL には Wallet にフルアクセスするための API キーが含まれています。決して公開しないでください。このQRコードは、Bitcoinのプライベート・キーと同様に扱ってください - このQRコードをスキャンした人は、Walletへのフル・アクセスが可能になります。



このモバイル機能は、あなたのLNbits Umbrelインスタンスを、あなたとあなたの友人のための正真正銘のLightning Walletサーバーに変えます。
...
このマルチユーザー設定の主なユースケースは、**家族や親しいサークル**とウォレットを共有することです。専用のWallet（この例では "Satoshi "など）を持つユーザーを作成したら、このログイン認証情報を信頼できる家族と共有することができます。
...
この二重のLayer保護（ネットワークアクセス＋ユーザー認証）により、Umbrelの「新規ユーザーの作成を許可する」オプションはそれほど重要ではなくなります。Interface ログインにアクセスできるのは、すでにネットワークまたは VPN にアクセスしている人だけです。



**典型的なシナリオ**：お父さん "アカウント、"お母さん "アカウント、"ビジネス "アカウントなどを作成します。家族の各メンバーは、あなたのUmbrelノードの共有流動性の恩恵を受けながら、独自の分離されたWallet Lightningを持っています。ユーザ名とパスワードを共有するだけで、ユーザはローカルネットワーク上のどのデバイスからでも、Tailscale VPN経由でも接続できます。詳しくは、Tailscale専用チュートリアルをご覧ください：
...
Interface SuperUserに戻り、左側のサイドパネルにある "Extensions "メニューにアクセスすると、LNbitsの完全な拡張機能エコシステムが表示されます。
...
各拡張機能は、このInterfaceからワンクリックで有効になります。FREE "と表示されている拡張機能は無料ですが、"PAID "バージョンもあります。ビジネス、家族管理、あるいは Lightning Network の機能を試してみるなど、あなたのニーズに合ったものをカタログからお探しください。
...
**メリット**：財務主権（資金/鍵/データの完全管理）、アーキテクチャの柔軟性（ロスレスVPS→Full node移行）、プロフェッショナルな拡張システム、直感的なInterface。



**制限事項** ：ソフトウェアがベータ版であること（量に注意）、セキュリティが管理者の責任下にあること、機密性の高いAPIキーを含むURL（HTTPS必須）、複数ユーザーの管理は管理者の責任を意味する。
...
**バックアップ**：seed PHOENIXD/credentials LND, LNbits データベース, `.env` ファイル。毎日自動化し、本番サーバーから離して暗号化しておく。定期的にリストアをテスト。
...
LNbitsセルフホスティングは、Lightning金融主権への具体的な道を提供します。VPS+PHOENIXDは、既存のBitcoinノードとの完全な統合により、高速サービスのための軽量ソリューションを提供します。スケーラブルなアーキテクチャは、シンプルなマルチユーザーWalletから洗練されたビジネスユースケースへの進化を可能にします。
