...
![BTC204](assets/ru/003.webp)

**Раздел 4: Понимание транзакций [coinjoin](https://planb.academy/resources/glossary/coinjoin)**
...
Протокол биткойна основан на модели [UTXO](https://planb.academy/resources/glossary/utxo), которая расшифровывается как "Unspent Transaction Output".
...
**Напоминать:** [Сатоши](https://planb.academy/resources/glossary/satoshi-sat), часто сокращаемый до сата, - это наименьшая единица биткойна, сопоставимая с центом в [фиатных](https://planb.academy/resources/glossary/fiat) валютах.

```plaintext
1 BTC = 100 000 000 SATS
```

Теоретически один UTXO может представлять любую стоимость в биткоинах, начиная от сата и заканчивая теоретическим максимумом в 21 миллион BTC. Однако логически невозможно владеть всеми 21 млн биткоинов, и существует нижний экономический порог, называемый "[пылью](https://planb.academy/resources/glossary/dust)", ниже которого UTXO считается экономически невыгодным тратить.

**Вы знали?** Крупнейший UTXO, когда-либо созданный на Bitcoin, имел стоимость `500 000 BTC`. Он был создан платформой MtGox во время операции по [консолидации](https://planb.academy/resources/glossary/consolidation) в ноябре 2011 года: [29a3efd3ef04f9153d47a990bd7b048a4b2d213daaa5fb8ed670fb85f13bdbcf](https://mempool.space/fr/tx/29a3efd3ef04f9153d47a990bd7b048a4b2d213daaa5fb8ed670fb85f13bdbcf)
...
![BTC204](assets/ru/007.webp)

Все UTXO защищены [скриптами](https://planb.academy/resources/glossary/script), которые определяют условия, при которых они могут быть потрачены. Чтобы израсходовать UTXO, пользователь должен продемонстрировать сети, что он или она удовлетворяет условиям, оговоренным в сценарии, защищающем этот UTXO. Как правило, UTXO защищены [открытым ключом](https://planb.academy/resources/glossary/public-key) (или [адресом получения](https://planb.academy/resources/glossary/receiving-address), который представляет этот открытый ключ). Чтобы потратить UTXO, связанный с этим открытым ключом, пользователь должен доказать, что он владеет соответствующим [закрытым ключом](https://planb.academy/resources/glossary/private-key), предоставив [цифровую подпись](https://planb.academy/resources/glossary/digital-signature), сделанную с помощью этого ключа. Именно поэтому мы говорим, что ваш [биткойн-кошелек](https://planb.academy/resources/glossary/wallet) на самом деле не содержит биткойнов, а хранит ваши закрытые ключи, которые, в свою очередь, дают вам доступ к вашим UTXO и, как следствие, к биткойнам, которые они представляют.

![BTC204](assets/ru/008.webp)
...
Биткойн-транзакция - это операция, записанная в [блокчейне](https://planb.academy/resources/glossary/blockchain), которая передает право собственности на биткойны от одного лица к другому. Точнее, поскольку мы работаем по модели UTXO и у нас нет счетов, транзакция удовлетворяет условиям расходования, которые обеспечили один или несколько UTXO, расходует их и эквивалентно создает новые UTXO с новыми условиями расходования. Короче говоря, транзакция перемещает биткоины из удовлетворенного сценария в новый сценарий, предназначенный для их обеспечения.

![BTC204](assets/ru/010.webp)

Таким образом, каждая транзакция Bitcoin состоит из одного или нескольких [входов](https://planb.academy/resources/glossary/input) и одного или нескольких [выходов](https://planb.academy/resources/glossary/output). Входы - это UTXO, потребляемые транзакцией для создания выходов. Выходы - это новые UTXO, которые могут быть использованы в качестве входов для будущих транзакций.

![BTC204](assets/ru/011.webp)

**Вы знали?** Теоретически транзакция биткоина может иметь бесконечное количество входов и выходов. Единственным ограничением является максимальный размер [блока](https://planb.academy/resources/glossary/block).
...
![BTC204](assets/ru/014.webp)

### Комиссионные за транзакции

Интуитивно можно подумать, что [транзакционные издержки](https://planb.academy/resources/glossary/transaction-fees) также представляют собой результат сделки. Но в действительности это не так. Транзакционные издержки представляют собой разницу между суммарными входами и суммарными выходами. Это означает, что после использования части стоимости вводимых ресурсов для покрытия желаемых выходов в сделке, определенная сумма вводимых ресурсов остается неиспользованной. Эта остаточная сумма и составляет транзакционные издержки.

```plaintext
Комиссия = total inputs - total outputs
```

Возьмем пример Алисы, у которой UTXO составляет `10 000 SATS` и которая хочет купить багет по цене `4 000 SATS`. Алиса создает транзакцию со своим UTXO в размере `10 000 SATS` в качестве входа. Затем она генерирует вывод в размере `4 000 SATS`, чтобы пекарь заплатил за багет. Чтобы поощрить [майнеров](https://planb.academy/resources/glossary/miner) интегрировать ее транзакцию в блок, Алиса выделяет `200 SATS` в качестве комиссии. Затем она создает второй вывод, обмен, который будет ей возвращен, на сумму `5 800 SATS`.

![BTC204](assets/ru/015.webp)

Применяя формулу платы, мы видим, что для несовершеннолетних действительно осталось `200 SATS`:

```plaintext
Комиссия = total inputs - total outputs
Комиссия = 10 000 - (4 000 + 5 800)
Комиссия = 10 000 - 9 800
Комиссия = 200
```

Когда майнеру удается подтвердить блок, он получает право взимать эти сборы со всех транзакций, включенных в его блок, посредством так называемой транзакции "[coinbase](https://planb.academy/resources/glossary/coinbase-transaction)".
...
Coinbase - это особый тип транзакции Bitcoin, который уникален для каждого блока и всегда является первым из них. Она позволяет майнеру, нашедшему достоверное [доказательство работы](https://planb.academy/resources/glossary/proof-of-work), получить вознаграждение за блок. Это вознаграждение состоит из двух элементов: **[блочного гранта](https://planb.academy/resources/glossary/block-subsidy)** и **комиссии за транзакцию**, о которых говорилось в предыдущем разделе.
...
![BTC204](assets/ru/016.webp)

Биткоины с поддержкой блока - это новые BTC, созданные с нуля, в соответствии с заранее установленным [графиком эмиссии](https://planb.academy/resources/glossary/limite-demission) в правилах консенсуса. Субсидия на блокчейн уменьшается вдвое каждые 210 000 блоков, то есть примерно раз в четыре года, в процессе, известном как "[халвинг](https://planb.academy/resources/glossary/halving)". Первоначально с каждой субсидией создавалось 50 биткоинов, но постепенно это количество уменьшилось; в настоящее время оно составляет 3,125 биткоина за блок.
...
**Вы знали?** Биткоины, сгенерированные в результате транзакции на coinbase, имеют период зрелости в 100 блоков, в течение которого они не могут быть потрачены майнером. Это правило призвано избежать осложнений, связанных с использованием вновь созданных биткоинов в цепочке, которая впоследствии может устареть.
...
С другой стороны, в силу своей природы [двойная трата](https://planb.academy/resources/glossary/double-spending-attack) не является проблемой для физических валют. Если я даю вам купюру в 10 евро, она безвозвратно покидает мое владение и переходит в ваше, что, естественно, исключает возможность многократной траты денежных единиц, которые она воплощает. Короче говоря, я не смогу потратить эту купюру в 10 евро еще раз.

![BTC204](assets/ru/019.webp)
...
- и [повторное использование адресов](https://planb.academy/resources/glossary/address-reuse).

![BTC204](assets/ru/031.webp)
...
Например, здесь представлена транзакция Bitcoin, в которой используется схема [sweep](https://planb.academy/resources/glossary/sweep-transaction):

```plaintext
35f1072a0fda5ae106efb4fda871ab40e1f8023c6c47f396441ad4b995ea693d
```

![BTC204](assets/ru/036.webp)
...
![BTC204](assets/ru/047.webp)

На этой диаграмме видно, что вход n° 0 разблокирует скрипт [P2WPKH](https://planb.academy/resources/glossary/p2wpkh) (SegWit V0, начинающийся с `bc1q`). Выход n° 0 использует тот же тип скрипта. Выход n° 1, с другой стороны, использует скрипт [P2TR](https://planb.academy/resources/glossary/p2tr) (SegWit V1, начинающийся с `bc1p`). Интерпретация этой особенности заключается в том, что, скорее всего, адрес с той же версией, что и входной, является адресом обмена. Поэтому он всегда будет принадлежать одному и тому же пользователю.
...
![BTC204](assets/ru/048.webp)

Source : [Mempool.space](https://mempool.space/tx/db07516288771ce5d0a06b275962ec4af1b74500739f168e5800cbcb0e9dd578)

На последнем мы видим, что вход № 0 и выход № 1 используют скрипты P2WPKH (SegWit V0), а выход № 0 - другой скрипт [P2PKH](https://planb.academy/resources/glossary/p2pkh) (Legacy).
...
Анализ каналов также включает в себя несколько более общий компонент [OSINT](https://planb.academy/resources/glossary/osint) (*Open Source Intelligence*), включающий поиск информации в Интернете. Именно по этой причине мы не рекомендуем публиковать адреса непосредственно в социальных сетях или на веб-сайтах, будь то псевдонимные или нет.

![BTC204](assets/ru/063.webp)
...
Несколько факторов подтверждают эту гипотезу. Во-первых, тип скрипта, используемого для защиты входа UTXO, - это мультисигмальный скрипт [P2SH](https://planb.academy/resources/glossary/p2sh) 2/3, что указывает на продвинутый уровень безопасности, характерный для биржевых платформ:
...
Адреса SegWit V0 (`P2WPKH` и `P2WSH`) и [Taproot](https://planb.academy/resources/glossary/taproot) / SegWit V1 (`P2TR`) представляют современные форматы. Адреса SegWit начинаются с `bc1q`, а адреса Taproot, представленные в 2021 году, начинаются с `bc1p`.
...
Первый совет, который я могу дать, - это смешаться с самой плотной толпой. В биткойне это означает использование наиболее распространенных шаблонов скриптов. Например, скрипты [P2WSH](https://planb.academy/resources/glossary/p2wsh), часто используемые для мультисиговых конфигураций SegWit V0, очень редки. Они не позволяют скрыть большой набор анонимности. То же самое касается и старых моделей, таких как P2PKH или P2SH. Хотя они широко представлены в наборе UTXO, они все реже используются для новых транзакций.
...
![BTC204](assets/ru/118.webp)

В первом примере мы увидели, как coinjoin может защитить конфиденциальность монеты по отношению к её прошлому, а во втором примере — как он также может защитить историю монеты по отношению к её будущему. Именно поэтому я упомянул, что coinjoin следует воспринимать как разовое событие, которое сегментирует историю монеты в обоих направлениях:

![BTC204](assets/ru/119.webp)
...
Помимо этих трёх исторических реализаций, недавно появилась новая: Joinstr. Я не буду рассматривать её в этой главе, но вы можете найти всю информацию о ней в этом специальном руководстве:
...
![BTC204](assets/ru/139.webp)

С появлением Wabisabi версия 2.0 адаптировала свой подход к coinjoin, приблизив его к модели Whirlpool. Хотя транзакции coinjoin по-прежнему остаются очень крупными, теперь стало возможным последовательно выполнять несколько циклов, следуя модели Whirlpool. Особое внимание было также уделено управлению сдачей: в отличие от Wasabi 1.0, где сдача была напрямую связана с входами пользователей, Wabisabi стремится разделять сдачу на несколько небольших сумм, распределённых в равных номиналах между всеми участниками.
...
![BTC204](assets/ru/143.webp)


- 6 входов и 6 выходов;

![BTC204](assets/ru/144.webp)


- 7 входов и 7 выходов;

![BTC204](assets/ru/145.webp)


- 8 входов и 8 выходов.

![BTC204](assets/ru/146.webp)
...
![BTC204](assets/ru/153.webp)

### Ремиксы

После выполнения первоначального микса UTXO переводятся на счёт **postmix**. Этот счёт объединяет уже смешанные UTXO и те, которые ожидают повторного микширования. Когда клиент Whirlpool активен, UTXO, находящиеся на счёте **postmix**, автоматически доступны для повторного микширования и будут случайным образом выбраны для участия в этих новых циклах.
...
Самое простое и безопасное решение — оставить ваши смешанные UTXO в состоянии покоя на счёте **postmix**, позволяя им продолжать перемешиваться и прикасаться к ним только для их траты. Кошельки Samourai (а теперь и Ashigaru) и Sparrow имеют дополнительные механизмы защиты от всех рисков, связанных с анализом цепочки. Эти меры защиты помогают вам избегать ошибок.
...
- **Консолидировать их через Monero:** Ранее Samourai Wallet предлагал услугу атомарного обмена между BTC и XMR, которая, к сожалению, недоступна после их ареста. Этот подход позволял обрабатывать «токсичные» UTXO, консолидируя их в Monero без ущерба для вашей конфиденциальности через CIOH, а затем возвращать их обратно в сеть Биткоина. Если аналогичный сервис появится вновь, он может быть интересным вариантом. Однако следует помнить, что этот метод может оказаться затратным — как из-за комиссий за майнинг, так и из-за премии, связанной с ограниченной ликвидностью при обратной конвертации в биткоины.
...
Основной принцип работы Whirlpool остаётся неизменным, однако есть некоторые различия по сравнению с использованием в Samourai: в 2025 году на Ashigaru доступны лишь два пула — `0.25 BTC` и `0.025 BTC`, с соответствующими входными комиссиями `0.0125 BTC` и `0.00125 BTC`.
...
![BTC204](assets/ru/177.webp)


Наиболее продвинутой реализацией payjoin, несомненно, является Stowaway, созданный разработчиками Samourai Wallet. После ареста основателей программного обеспечения этот инструмент на Samourai работает лишь частично. Однако он был возобновлён в приложении Ashigaru.

Преимущество Stowaway заключается в том, что это полный и очень простой в использовании протокол, поддерживающий как приём, так и отправку payjoin. Частично подписанные транзакции можно обменивать вручную посредством сканирования нескольких QR-кодов или автоматически через Tor с использованием Soroban.

![BTC204](assets/ru/178.webp)
...
Если вы хотите провести платежную операцию, сохранив при этом определенную степень конфиденциальности, payjoin - хороший вариант. Но, как мы только что увидели, payjoin требует участия получателя. Что же делать, если получатель отказывается участвовать в payjoin или если вы просто предпочитаете не привлекать его? Одна из альтернатив - использовать транзакцию [Stonewall](https://planb.academy/resources/glossary/stonewall) или Stonewall x2. Давайте рассмотрим эти два типа транзакций подробнее.
...
Транзакции Stonewall доступны в Sparrow Wallet и в Ashigaru, тогда как Stonewall x2 в настоящее время поддерживаются только Ashigaru.

![BTC204](assets/ru/189.webp)
...
Идея этого метода рикошета изначально принадлежит командам Samourai Wallet, которые интегрировали его в своё приложение, чтобы автоматизировать работу. Услуга была платной в Samourai, поскольку рикошет требовал фиксированную плату в 100 000 сатоши за обслуживание, к которой добавлялись комиссии за майнинг. То же самое действует сегодня в Ashigaru. Поэтому его использование рекомендуется преимущественно для переводов на крупные суммы.

![BTC204](assets/ru/196.webp)

Приложение Ashigaru предлагает два варианта ricochet (те же, что были ранее в Samourai):


- Усиленный ricochet, или «поштучная доставка», имеет преимущество распределения сервисных комиссий Ashigaru на пять последовательных транзакций. Этот вариант также гарантирует, что каждая транзакция будет транслироваться в отдельный момент времени и записываться в отдельный блок, что позволяет максимально точно имитировать поведение смены владельца. Несмотря на более низкую скорость, этот метод предпочтительнее для тех, кто не спешит, поскольку он повышает эффективность ricochet, усиливая его устойчивость к анализу цепочки;

![BTC204](assets/ru/197.webp)
...
![BTC204](assets/ru/201.webp)

Более того, нет никакой гарантии, что Алиса, получив закрытый ключ $B$ Боба, передаст взамен свой закрытый ключ $A$. Таким образом, этот обмен основан на огромном доверии между сторонами и оказывается неэффективным для обеспечения безопасной секретной передачи права собственности.

![BTC204](assets/ru/202.webp)
...
Один из способов избежать различных атак деанонимизации - использовать предложение [Dandelion](https://planb.academy/resources/glossary/dandelion). Этот широковещательный протокол был формализован в BIP156, но так и не был реализован в Биткойне.
...
Чтобы осуществить транзакцию, обе стороны должны иметь биткоин-кошелёк с реализованным протоколом BIP47, например PayNym на Ashigaru или Sparrow Wallet. Совместное использование их платёжных кодов создаёт между ними секретный канал. Для эффективного установления этого канала отправитель должен выполнить на блокчейне Биткоина специальную транзакцию, известную как «уведомляющая транзакция» (подробности я предоставлю позже).
...
Что касается PayNym Bots — тех ботов, которых иногда можно увидеть в X (Twitter), — это визуальные представления платёжного кода, созданные Samourai Wallet. Теперь, с Ashigaru, они слегка изменились, но принцип остаётся тем же. Они генерируются с помощью хэш-функции, что обеспечивает им практическую уникальность. Они представляют собой небольшую строку символов, начинающуюся с `+` :
...
![BTC204](assets/ru/224.webp)

### Обмен ключами Диффи-Хеллмана на эллиптических кривых (ECDH)

Криптографический протокол, лежащий в основе BIP47, известен под аббревиатурой [ECDH](https://planb.academy/resources/glossary/ecdh), что означает *Elliptic-Curve Diffie-Hellman*. Этот метод является вариантом оригинального обмена ключами Диффи-Хеллмана.
