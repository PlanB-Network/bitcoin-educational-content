...
CoinJoin** on Bitcoin tehingustruktuuri eriliik, mis ühendab mitu osalejat ühte ühistehingusse. See mehhanism segab erinevate kasutajate kirjed ühiseks tehinguks, mis muudab vahendite jälgimise äärmiselt keeruliseks - kui mitte sageli võimatuks, kui seda õigesti teha -. Selle tulemusel on välise vaatleja jaoks peaaegu võimatu kindlalt tuvastada kaasatud bitcoinide päritolu ja sihtkohta, erinevalt tavapärastest Bitcoin tehingutest.



Teie, kasutaja, jaoks aitab CoinJoin säilitada teie konfidentsiaalsust. Näiteks kui saate 10 000 Sats annetuse Bitcoin Address, saab saatja neid vahendeid jälgida ja mõnel juhul järeldada, et teil on suurem kogus bitcoine, või jälgida teie tegevust. Kui teete pärast seda 10 000 Sats annetust CoinJoin, katkestate jälgitavuse: saatja ei saa sellest maksest enam teie kohta mingit teavet tuletada.



Chaumian CoinJoin pakub kõrget turvalisuse taset, kuna raha jääb alati kasutaja ainukontrolli alla. Isegi koordineerivate serverite operaatorid ei saa mingil juhul osalejate bitcoin'e ümber suunata. Ei kasutajad ega koordinaatorid ei pea üksteist usaldama: kumbki säilitab kontrolli oma isiklike võtmete üle ja on ainuüksi volitatud tehingute kinnitamiseks. Ükski kolmas isik ei saa seega CoinJoin ajal teie bitcoin'eid omastada ega luua otsest seost teie sisendite ja väljundite vahel.



Kui soovite rohkem teada saada CoinJoin kohta, vaadake Plan ₿ Academy's BTC 204 kursust :
...
Ginger tuletab kohe alguses meelde CoinJoin protsessiga seotud kulusid.
...
Kui olete loonud oma esimese portfelli, viiakse teid Gingeri Interface Wallet.
...
Ginger Wallet-s saate luua mitu portfelli. Lihtsalt klõpsake **Lisata portfell**.
...
Ginger toetab riistvaraportfellide kasutamist Bitcoin core standardi Interface kaudu, kuigi otsene integreerimine riistvaraportfellist või riistvaraportfellile ei ole veel võimalik.
...
Sellel vahekaardil saate lubada rakenduse käivitamisel käivitada Bitcoin Knots, valida oma võrgu (Main/RegTest) ja oma laadimismäära teenusepakkuja (Mempool Space/BLOCKSTREAM info/Full node) jne.
...
- Kahefaktorilise autentimise puhul veenduge, et teie autentimisrakendus toetab SHA256-protokolli ja 8-kohalisi koode. Ginger Wallet nõuab 8-kohalist 2FA-koodi täiustatud turvalisuse tagamiseks. Selle pikema formaadi tõttu on koodi palju raskem ära arvata või kompromiteerida, pakkudes suuremat kaitset volitamata juurdepääsu eest.
...
- gW-42 ajal võib koordinaator ühendada teie sisendid ja väljundid teie IP Address-ga;
- tehingu edastamisel võib pahatahtlik sõlmpunkt, millega te ühendute, seostada teie tehingu teie IP-ga.



Ärge unustage iga kord vajutada **Done** (parempoolses alumises Coin-s), et salvestada oma seaded. Mõne seadistuse jõustumiseks tuleb Ginger Wallet uuesti käivitada.
...
- CoinJoin** (kus saab kohandada selle portfelli CoinJoin seadeid) ;
...
Et saada bitcoins oma Wallet Ginger Wallet :
...
- Sisestage selle allika nimi, millega soovite Address seostada. See on märgistus maksete jälgimiseks. Sellel ei ole On-Chain mõju; see on lihtsalt teie rakenduses lokaalselt salvestatud jälgitavuse teave;
...
- klõpsake Address formaadi (**SegWit** /**Taproot**) valimiseks klõpsake **generate**, seejärel klõpsake **generate**, et valida generate ja Address ning QR-kood.
...
Seda Address või QR-koodi kasutab teie saatja bitcoinide saatmiseks.
...
Videoõpetus, kuidas saata Ginger Wallet kaudu.
...
- sisestage saaja Address, saadetava summa ja silt;
...
- Enne Bitcoin ostmist valige oma riik või isegi oma riik (mõnes piirkonnas, näiteks Kanadas). Kui klõpsate esimest korda funktsioonil **Ostma**, peate ka oma piirkonna määrama.
...
Ekraani paremal üleval Coin, kus on silt **Kõik**, võimaldab teil filtreerida pakkumisi makseviisi järgi. Teie valitud makseviis on vaikimisi määratud, kuid seda saab igal ajal muuta.
...
- Nagu ka **Ostmise** puhul, peate müümise funktsiooni esmakordsel kasutamisel enne Bitcoin müügi jätkamist valima oma riigi.
...
- Nüüd peate tehingu lõpule viima:
 ...
 - Mingil hetkel saadetakse sulle Address ja täpne summa, mille saadad;
 - Seejärel naaske Ginger Wallet juurde, et jätkata protsessi;
 - Kui olete tagasi Ginger Wallet-s, ilmub dialoogiboks, mis võimaldab teil jätkata, klõpsates **Sendama**.



See avab ekraani **Saatmine**, kus on eelnevalt täidetud saaja Address ja summa. Võite kasutada ka avakuval olevat nuppu **Sendama**. Kuigi te võite saata tehingu käsitsi, soovitame optimaalse protsessi tagamiseks täita selle dialoogiboksi kaudu.



## Tee CoinJoin Ginger Wallet-le
...
Kaitske oma bitcoinide konfidentsiaalsust **CoinJoin** abil, mis on integreeritud otse Ginger Wallet-sse. Wallet kasutab **WABISABI**, Chaumian CoinJoin protokolli, mis on loodud lihtsamaks ja tõhusamaks mündiühenduseks.



Teie otsustada on, milline CoinJoin strateegia (automaatne või manuaalne) teile kõige paremini sobib.



Ginger CoinJoin on kasutusvalmis kohe pärast allalaadimist (täiendavaid samme ei ole vaja teha). Ginger CoinJoin töötab automaatselt taustal, et kaitsta teie privaatsust iga tehingu puhul. Praktikas ilmub CoinJoin lugeja alati, kui teil on anonüümseks muudetav saldo.



CoinJoin käsitsi käivitamine toimub ühe klõpsuga. Käivitage voor ja oodake, kuni CoinJoin tehing on üles ehitatud ja kinnitatud. Näete anonüümsuse skoori Interface.
...
Vaikimisi kasutab Ginger oma koordinaatorit koos kõigi eelkonfigureeritud parameetrite ja garanteeritud tasudega. Rohkem kui 0,03 BTC väärtusega müntide liitmisel tuleb lisaks Mining tasule maksta 0,3% koordinaatoritasu. 0,03 BTC või väiksema väärtusega sisseminekud, samuti remixid, on koordinaatoritasudest vabastatud, isegi pärast ühte tehingut. Seega võimaldab CoinJoin vahenditega tehtud makse nii saatjal kui ka saajal oma münte remixida ilma koordinaatoritasudeta.



Ginger eelistab väiksematele ja kiirematele voorudele rohkemate osalejatega ühisvõistlusi. Suuremad coinjoinid pakuvad suuremat anonüümsust, väiksemaid kulusid ja suuremat tõhusust BLOCK ruumi kasutamisel.
...
- Kasutage iga vastuvõtu jaoks erinevat Address, et vältida aadresside korduvkasutamist;
...
Kui leidsid selle õpetuse kasulikuks, palun jäta mulle Green pöidla alla. Palun jagage seda artiklit oma sotsiaalmeedia platvormide kaudu. Tänan teid väga!
