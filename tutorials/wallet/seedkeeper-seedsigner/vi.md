...
- Thiết bị thông thường cần có cho SeedSigner cổ điển: Raspberry Pi Zero, màn hình Waveshare 1,3", camera tương thích và thẻ nhớ microSD (bạn sẽ tìm thấy thêm thông tin chi tiết trong hướng dẫn SeedSigner bên dưới);
...
- Nếu bạn chưa có Bitcoin Wallet được liên kết với SeedSigner của mình, bạn cần làm theo các bước **5** và **6** của hướng dẫn được đề cập bên dưới. Các phần này giải thích cách generate một cụm từ Mnemonic với SeedSigner, lưu nó qua *SeedQR*, rồi kết nối Wallet này với Sparrow wallet để quản lý. Tôi sẽ không đi sâu vào các quy trình này ở đây và **tôi giả định rằng bạn đã có một Bitcoin Wallet đang hoạt động, được cấu hình với Sparrow và SeedSigner**.
...
Bây giờ Seedkeeper của bạn đã hoạt động, bạn có thể lưu Bitcoin, Wallet, Mnemonic trên thẻ thông minh. Để bắt đầu, hãy bật SeedSigner như bình thường, sau đó quét *SeedQR* của Wallet để tải vào thiết bị. Sau khi nhập seed, chỉ cần chọn `Done`.
...
Khi seed được tải, hãy truy cập vào menu `Sao lưu seed`.
...
Bây giờ, hãy chọn mã PIN mới của Seedkeeper, từ 4 đến 16 ký tự. Để tăng cường bảo mật, hãy chọn một mã dài và ngẫu nhiên: đây là rào cản duy nhất bảo vệ quyền truy cập vật lý vào cụm từ Mnemonic của bạn.
...
Sau đó, bạn có thể định nghĩa một `Nhãn` liên kết với cụm từ Mnemonic của mình. Nhãn này hữu ích nếu bạn lưu trữ nhiều bí mật trên Seedkeeper, giúp bạn có thể dễ dàng nhận dạng chúng.
...
Cụm từ Mnemonic của bạn hiện đã được lưu trên thẻ thông minh.
...
- Bản sao thứ hai của cụm từ Mnemonic chưa được mã hóa của bạn, đóng vai trò là bản sao lưu cuối cùng cho danh mục đầu tư của bạn, chỉ được sử dụng trong trường hợp Seedkeeper bị mất hoặc bị đánh cắp. Vì phiên bản này chưa được mã hóa, nên cần lưu trữ ở một vị trí riêng biệt, an toàn hơn để tránh việc cả 2 bản sao lưu bị xâm phạm đồng thời.



Tùy thuộc vào chiến lược bảo vệ và hồ sơ rủi ro của bạn, bạn cũng có thể sao chép seed trên nhiều Seedkeeper khác nhau hoặc tạo nhiều bản sao vật lý của Mnemonic. Để tìm hiểu thêm về các phương pháp này, hãy xem hướng dẫn sau:
...
Bây giờ bạn có thể sử dụng Seedkeeper để tải cụm từ Mnemonic vào SeedSigner khi khởi động, và từ đó ký các giao dịch Bitcoin. Để bắt đầu, hãy bật SeedSigner bằng cách cắm vào, sau đó mở menu `Seeds`.
...
- Lưu trữ seed và passphrase của bạn trong hộp Seedkeeper, được bảo vệ bằng mã PIN mạnh (điều này rất quan trọng). Bản sao lưu này sẽ cho phép bạn dễ dàng sử dụng Wallet hàng ngày. Nếu muốn, bạn có thể sao chép thông tin này trên một hộp Seedkeeper thứ hai;
- Ngoài ra, hãy giữ một bản sao rõ ràng của Mnemonic và passphrase, trên giấy hoặc kim loại. Đây là phương án dự phòng cuối cùng nếu bạn làm mất Seedkeeper hoặc mã PIN. Hãy đảm bảo lưu trữ các bản sao này ở những nơi riêng biệt để chúng không bị xâm phạm cùng lúc.



Trong cấu hình này, nếu ai đó có được riêng mã Mnemonic dạng văn bản thuần túy của bạn, họ sẽ không thể đánh cắp bất cứ thứ gì nếu không biết mã passphrase (tất nhiên, với điều kiện mã này đủ mạnh để chống lại tấn công brute-force). Ngược lại, nếu ai đó phát hiện ra mã passphrase dạng văn bản thuần túy của bạn, mã này sẽ không thể sử dụng được nếu không có cụm từ Mnemonic tương ứng.
...
- **passphrase mạnh**: mật khẩu phải dài, ngẫu nhiên và chứa nhiều ký tự. Độ phức tạp của mật khẩu không phải là vấn đề đối với bạn, vì bạn chỉ cần nhập mật khẩu một lần trên bàn phím trong quá trình khởi tạo, sau đó mật khẩu sẽ được Seedkeeper truyền đi;
- **Mã PIN mạnh** cho Seedkeeper: cũng ngẫu nhiên và bao gồm 16 ký tự.



Để thiết lập, hãy bắt đầu bằng cách tải passphrase vào SeedSigner theo cách thông thường. Bạn có thể làm theo quy trình được mô tả chi tiết trong hướng dẫn này:
...
Sau đó nhấp vào `Sao lưu seed`, đưa Seedkeeper vào ổ đĩa và chọn `Đến SeedKeeper`.
...
Nhập mã PIN của bạn để mở khóa Seedkeeper, sau đó gán nhãn cho bí mật này. Bạn có thể để dấu vân tay làm nhãn để duy trì một số hình thức phủ nhận hợp lý, hoặc ghi rõ ràng là `passphrase Wallet` chẳng hạn.
...
- Trong menu `Chức năng SeedKeeper`, bạn có thể:
 ...
 - Lưu một bí mật mới;
 - Xóa bí mật hiện có;
 - Lưu hoặc tải mô tả của bạn (chức năng hữu ích cho danh mục đầu tư Multisig).
