...
HTLC出力の保存は、プロトコルが暗号化された**blob**に追加の署名データを含むように拡張できるため、将来のバージョンで展開される予定です。
...
regtest**を使用しているため、他のネットワーク（MainnetやTestnetなど）で見られる中央値10分の遅延を待つことなく、RPCコマンドを発行することでMiner**ブロックが可能です。Minerブロックの詳細については、**bitcoin-cli**のヘルプを参照してください。
