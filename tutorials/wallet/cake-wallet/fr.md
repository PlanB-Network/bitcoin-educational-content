...
- les [**Paiements silencieux (BIP-352)**](https://BIPs.dev/352/) améliorent les précédents [codes de paiement BIP 47](https://silentpayments.xyz/docs/comparing-proposals/bip47/) également appelés "PayNyms" avec des adresses furtives réutilisables. Lorsqu'un expéditeur utilise votre Address de paiement silencieux, sa Wallet obtient une Address unique à l'aide de différentes clés qui seront combinées en une Taproot Address unique. Les enregistrements Blockchain montrent des transactions sans rapport entre elles, ce qui empêche de relier les paiements entrants. Les paiements silencieux offrent toute une série d'avantages, notamment
    - Adresses réutilisables : Il n'est pas nécessaire de créer une nouvelle adresse Address pour chaque transaction, ce qui améliore l'expérience de l'utilisateur et la protection de la vie privée
    - Aucune augmentation des coûts : Les paiements silencieux n'augmentent ni la taille ni le coût des transactions.
    - Anonymat renforcé : Les observateurs extérieurs ne peuvent pas relier les transactions à un paiement silencieux Address.
    ...
    - Des adresses uniques pour chaque paiement : Élimination du risque de réutilisation accidentelle de Address.
...
- PayJoin v2** atténue l'analyse du graphe des transactions en fusionnant les entrées des expéditeurs et des destinataires en une seule transaction. Le gâteau Wallet met en œuvre deux avancées essentielles :
    ...
    - Communication sans serveur** : Aucune des parties n'a besoin d'exécuter un serveur PayJoin, ce qui élimine un obstacle technique majeur.
...
- Swap:** sélectionner `Tor Only` (anonymise le trafic Exchange)
...
Si vous souhaitez ajouter une sécurité supplémentaire Layer, vous pouvez mettre en place une "passphrase".  L'objectif principal d'une passphrase est de fournir une protection supplémentaire contre les attaques physiques. Même si un attaquant trouve la phrase seed, il ne peut pas accéder à votre Wallet sans la passphrase correcte. En d'autres termes, la phrase seed seule représente une Wallet, tandis que la phrase seed plus la passphrase créent une Wallet entièrement différente, sans aucun lien avec l'originale. Cette fonction permet également de créer des "portefeuilles secrets" protégés par la passphrase, et vous donne la possibilité de nier l'existence de la passphrase. Dans une situation de coercition, vous pourriez révéler la phrase seed tout en gardant des actifs plus importants en sécurité dans le Wallet protégé par la passphrase.


Si vous utilisez déjà votre propre nœud, basculez sur `Add New Custom Node` et fournissez votre `Node Address` pour valider les transactions et les blocs au sein de votre propre infrastructure. Une fois terminé, tapez sur `Continue` et `Next` pour créer votre Wallet.
...
Pour connaître les meilleures pratiques pour sauvegarder votre phrase Mnemonic, veuillez consulter ce tutoriel :
...
- Solde disponible
- Silent Payments Card pour commencer à scanner les transactions envoyées à votre Silent Payment Address
- La carte PayJoin permet d'activer la carte PayJoin afin de préserver la vie privée et d'économiser de l'argent
- en bas, il y a des raccourcis vers "Vue d'ensemble du Wallet", "Recevoir", "Échanger" entre le Bitcoin et d'autres devises, "Envoyer" et "Acheter"
...
Ici, nous pouvons reconnecter le Wallet, gérer les nœuds et nous connecter à notre propre nœud (recommandé). L'option `Scanner les paiements silencieux` nous permet de personnaliser le scan en spécifiant soit `Scanner à partir de la hauteur du BLOCK`, soit `Scanner à partir de la date`.
...
Il est possible d'activer la carte Paiements silencieux sur l'écran d'accueil pour afficher cette fonction. L'activation de la fonction "Toujours scanner" permet au Wallet de surveiller en permanence le Blockchain pour détecter les paiements silencieux entrants. Il est possible de spécifier des paramètres de balayage pour adapter le processus de balayage à nos besoins, comme décrit ci-dessus.
...
Pour sécuriser notre Wallet, nous pouvons créer une sauvegarde en suivant les instructions de l'application. Cela nous permettra de disposer d'une copie sûre de nos clés privées et de récupérer notre Wallet en cas de perte ou de vol. En outre, nous pouvons consulter notre phrase seed et nos clés privées, modifier notre code PIN, activer l'authentification biométrique, signer/vérifier et configurer 2FA pour une protection Layer supplémentaire.
...
Plusieurs options et types de Address sont disponibles pour la réception de Bitcoin. `SegWit (P2WPKH)` *(commençant par bc1q....)* est l'option par défaut.  Dans cet exemple, sélectionnons l'option "Paiements silencieux".


Pour recevoir un paiement silencieux, appuyez d'abord sur l'icône "Recevoir" dans le gâteau Wallet. Ensuite, entrez le montant que vous espérez recevoir. Pour spécifier le type de Address, tapez à nouveau sur "Recevoir" en haut de l'écran, puis sélectionnez "Paiements silencieux" dans les options.


Sur l'écran principal, votre code QR réutilisable Silent Payment et votre Address s'affichent. Comme prévu, la Address est assez longue :
...
Maintenant, utilisez un BIP-352 compatible Wallet (tel que le Blue Wallet) pour scanner ce code QR et envoyer le paiement. Vous verrez que le Wallet obtient une destination unique Address à partir de votre Address silencieuse.
...
Comme la Blue Wallet ne peut qu'"envoyer" des paiements silencieux, nous utiliserons une autre BIP 352 Wallet compatible comme destinataire. Ce processus est identique à celui d'une transaction Bitcoin normale.



- Tapez sur "Envoyer" sur l'écran d'accueil
- soit en collant notre Address réutilisable `sp1qq...`, soit en scannant le code QR directement à l'intérieur de l'application.
...
Une fois que nous avons saisi le `sp1qq...` Address, le Wallet dérive automatiquement un `bc1p...` Taproot Address (P2TR) correspondant en arrière-plan, qui sera utilisé pour le paiement silencieux.
...
Une fois la transaction envoyée, il vous sera demandé si vous souhaitez ajouter ce contact à votre annuaire Address.
...
Rappelons ce qu'est le PayJoin (https://docs.cakewallet.com/cryptos/Bitcoin/#PayJoin) :


payjoin v2 est une fonction de préservation de la vie privée et de réduction des frais de Bitcoin qui permet à l'expéditeur et au destinataire d'une transaction de travailler ensemble pour créer une seule transaction. Cette transaction provient *à la fois* de l'expéditeur et du destinataire, ce qui permet de briser les techniques de surveillance les plus courantes contre Bitcoin et d'améliorer l'échelonnement et les économies de frais dans certaines circonstances
...
Pour utiliser la PayJoin, les deux parties doivent disposer d'une Wallet compatible avec la PayJoin, et le destinataire doit avoir au moins une Coin ou une sortie dans sa Wallet. Pour commencer, veuillez suivre les étapes suivantes :


1. Tapez sur le "Menu hamburger" et tapez sur le bouton "Confidentialité"

2. Basculer l'option "Utiliser PayJoin
...
Voici notre avis sur le gâteau Wallet, qui offre une protection pratique de la confidentialité Bitcoin grâce à des fonctions telles que les paiements silencieux (BIP-352) et PayJoin v2.


Les paiements silencieux remplacent les adresses jetables par des adresses furtives réutilisables afin d'empêcher la mise en relation des transactions entrantes. Bien que les problèmes de synchronisation des versions précédentes se soient considérablement améliorés, l'analyse et la détection des paiements silencieux requièrent davantage de ressources et de bande passante.


PayJoin v2 bouleverse l'analyse des chaînes en fusionnant les entrées de l'expéditeur et du destinataire en une seule transaction, sans frais supplémentaires ni coordination centrale. Cela met fin à l'heuristique de l'entrée commune-Ownership, ce qui est un avantage significatif car cela signifie que vous ne pouvez pas supposer que toutes les entrées appartiennent à l'expéditeur.


Pour les utilisateurs qui privilégient l'anonymat financier, Cake Wallet est une option viable. Il intègre les protocoles de confidentialité directement dans ses fonctionnalités de base, les rendant accessibles sans aucune complexité technique. Alors que la surveillance des blockchains publiques augmente, des outils comme celui-ci aident à maintenir la confidentialité des transactions là où elle compte le plus. Une mise en œuvre plus large de ces normes dans le paysage Wallet serait une évolution bienvenue.
