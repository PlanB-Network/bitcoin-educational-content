...
이제 Debian 서버로 돌아가서 **피어**, 즉 Windows PC를 WireGuard 구성에 선언할 차례입니다. 우선, 구성을 수정하기 위해 Interface "wg0"**를 스토핑**해야 합니다:
...
문제가 발생하면 "**로그북**" 탭에서 확인할 수 있습니다. 두 호스트는 연결 상태를 확인하기 위해 정기적으로 Exchange 패킷을 주고받으므로 "*피어 1에서 킵얼라이브 패킷 수신 중*" 메시지가 표시됩니다.
