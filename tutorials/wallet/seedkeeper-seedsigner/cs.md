...
*Děkujeme týmu [Satochip](https://satochip.io/) za souhlas s opětovným použitím [jejich videí](https://www.youtube.com/@satochip/videos) v tomto návodu. Děkujeme také [Crypto Guide](https://www.youtube.com/@CryptoGuide/) za jeho Fork firmwaru SeedSigner, který umožňuje podporu čipových karet
...
K překonání této slabiny lze SeedSigner kombinovat s [**Seedkeeper**](https://satochip.io/product/seedkeeper/), čipovou kartou vyvinutou společností Satochip. Ta umožňuje ukládat fráze Mnemonic (nebo jiná tajemství) na kartu secure element chráněnou kódem PIN. Applet Seedkeeper je open-source a jeho secure element má certifikaci EAL6+. Při použití ve spojení s nástrojem SeedSigner nabízí velmi zajímavou bezpečnostní funkci: klíče zůstávají spravovány zcela off-line, transakce podepisujete na důvěryhodné obrazovce a kód seed je fyzicky chráněn v čipové kartě odolné proti fyzickému útoku.
...
Nyní, když je vaše zařízení Seedkeeper funkční, můžete na čipovou kartu uložit své karty Bitcoin Wallet Mnemonic. Chcete-li začít, zapněte SeedSigner jako obvykle a poté naskenujte *SeedQR* svého Wallet, abyste jej nahráli do zařízení. Jakmile je karta seed importována, jednoduše zvolte `Done`.
...
Po načtení seed přejděte do nabídky `Zálohovat seed`.
...
Poté můžete definovat `Značku` přidruženou k frázi Mnemonic. Tento štítek je užitečný, pokud v nástroji Seedkeeper ukládáte několik tajemství, abyste je mohli snadno identifikovat.
...
Vaše fráze Mnemonic je nyní uložena na čipové kartě.
...
- Druhá kopie nešifrované fráze Mnemonic, která slouží jako konečná záloha vašeho portfolia a bude použita pouze v případě ztráty nebo krádeže nástroje Seedkeeper. Jelikož je tato verze nešifrovaná, musí být uchovávána na odděleném, bezpečnějším místě, aby se zabránilo současnému ohrožení obou záloh.



V závislosti na vaší strategii ochrany a rizikovém profilu můžete také duplikovat jednotku seed na několika různých zařízeních Seedkeepers nebo vytvořit několik fyzických kopií jednotky Mnemonic. Chcete-li se o těchto postupech dozvědět více, podívejte se na následující výukový program:
...
Nyní můžete pomocí nástroje Seedkeeper při spuštění načíst frázi Mnemonic do SeedSigneru a podepsat tak transakce Bitcoin. Chcete-li začít, zapněte svůj SeedSigner tak, že jej připojíte, a poté otevřete nabídku `Seeds`.
...
Seedkeeper může obsahovat několik tajemství, takže SeedSigner vás poté vyzve, abyste si vybrali to, které chcete načíst. Zobrazený štítek odpovídá názvu, který jste definovali v kroku 4. Pokud jste, stejně jako v mém případě, zaregistrovali pouze jeden seed, bude k dispozici pouze jedna možnost.
...
Poté můžete svůj Wallet používat jako obvykle: kontrolovat doručovací adresy a podepisovat transakce stejně jako s klasickým SeedSignerem. Více informací o tom, jak jej používat, naleznete ve specializovaném výukovém kurzu :



https://planb.academy/tutorials/wallet/hardware/seedsigner-2b274bff-6fc8-407a-92d7-f6ec4d1fadfb

## 6. Používání nástroje Seedkeeper se zařízením passphrase BIP39



Používáte k ochraně svého portfolia Bitcoin zařízení passphrase? Můžete si jej zaregistrovat v programu Seedkeeper spolu s seed. Toto řešení vám umožní rychle nahrát Wallet do SeedSigneru, aniž byste museli při každém použití ručně zadávat passphrase na malé klávesnici.



Tuto metodu považuji za obzvláště zajímavou, protože umožňuje využívat výhody zabezpečení passphrase a zároveň eliminuje omezení spojená s jeho každodenním používáním. Zde je příklad konfigurace, kterou bych doporučil:
...
- Uschovejte si také průkaznou kopii dokladů Mnemonic a passphrase, a to na papíře nebo na kovu. Jedná se o zálohu poslední instance, pokud ztratíte svůj nástroj Seedkeeper nebo jeho PIN. Nezapomeňte tyto kopie uchovávat na oddělených místech, aby nemohly být ohroženy současně.



V této konfiguraci, pokud se někdo dostane pouze k vašemu otevřenému textu Mnemonic, nebude schopen nic ukrást, aniž by znal passphrase (samozřejmě za předpokladu, že je dostatečně silný, aby odolal útoku hrubou silou). A naopak, pokud někdo objeví váš otevřený text passphrase, zůstane bez odpovídající fráze Mnemonic nepoužitelný.
...
- A **passphrase strong**: musí být dlouhý, náhodný a obsahovat širokou škálu znaků. Jeho složitost pro vás nepředstavuje problém, protože jej budete muset zadat pouze jednou na klávesnici během inicializace, poté bude přenesen programem Seedkeeper;
- **silný kód PIN** pro Seedkeeper: rovněž náhodný a složený ze 16 znaků.



Chcete-li toto nastavení nastavit, začněte tím, že do SeedSigneru obvyklým způsobem nahrajete soubor passphrase. Můžete postupovat podle postupu podrobně popsaného v tomto návodu:
...
Poté klikněte na `Zálohovat seed`, vložte Seedkeeper do jednotky a vyberte `Do SeedKeeperu`.
...
Zadáním kódu PIN odemkněte aplikaci Seedkeeper a přiřaďte jí štítek. Jako štítek můžete ponechat otisk prstu, abyste zachovali určitou formu věrohodného popření, nebo můžete výslovně uvést například `passphrase Wallet`.
...
- V nabídce `Funkce nástroje SeedKeeper` můžete :
 ...
 - Uložení nebo načtení deskriptorů (užitečná funkce pro portfolia Multisig).
