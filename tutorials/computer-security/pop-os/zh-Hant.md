...
| 工具 | 描述 | 安裝命令 |
| ---------------------------- | ------------------------------------------ | ----------------------------------------------- |
| Firefox | 自由且流行的網頁瀏覽器 | `sudo apt install firefox` |
| Brave | 注重隱私的網頁瀏覽器 | 透過 Pop!_Shop 或官方網站安裝 |
| Visual Studio Code (VS Code) | 功能強大的開發者程式碼編輯器 | `flatpak install flathub com.visualstudio.code` |
| Git | 版本管理器 | `sudo apt install git` |
| Flatpak | 備用套件管理器 | `sudo apt install flatpak` |
| VLC | 多功能多媒體播放器 | `sudo apt install vlc` |
| GNOME Terminal | 預設終端機 | Pop!OS 已預裝 |
| Curl | 線上數據傳輸工具 | `sudo apt install curl` |
| Wget | 透過 HTTP/FTP 下載檔案 | `sudo apt install wget` |
| Docker | 應用程式容器化 | 透過官方腳本或 `apt` 安裝 |
| Node.js | 伺服器端 JavaScript 執行環境 | 透過 `apt` 或 NodeSource 安裝 |
| Python3 | 程式語言 | `sudo apt install python3 python3-pip` |
| GIMP | 進階圖像編輯器 | `sudo apt install gimp` |
| Thunderbird | 郵件用戶端 | `sudo apt install thunderbird` |
| Transmission | 輕量級 BitTorrent 用戶端 | `sudo apt install transmission-gtk` |
| Htop | 互動式系統監視器 | `sudo apt install htop` |
