...
- 到 `resources/project/` 區的 [PlanB 專用於資料的 GitHub 儲存庫](https://github.com/PlanB-Network/Bitcoin-educational-content/tree/dev/resources/builders)：
...
- 在此資料夾中，您將建立第一個 YAML 檔案，名稱為 `project.yml`：
...
- 回到 `resources/projects/` 資料夾，選擇您在之前的提交中剛建立的業務資料夾：
...
- 保留提交標題為預設值，並確認「直接提交到 patch-1 分支」方塊已被勾選，然後點選「提交變更」： ![project](assets/23.webp)
- 回到 `assets` 資料夾：
