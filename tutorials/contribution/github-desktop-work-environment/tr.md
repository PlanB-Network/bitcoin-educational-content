...
Yazma yazılımını yüklemeye geçelim. Burada birkaç seçeneğiniz var. Plan ₿ Academy, deposundaki metin dosyaları için bu biçimi kullandığından, Markdown dosyalarını düzenlemeyi destekleyen bir yazılıma ihtiyacınız olacak.
...
Bir bakıma Plan ₿ Academy kaynak deposunun bir kopyasını oluşturduk. Fork'unuz (kopya) ve orijinal depo artık birbirlerinden bağımsız olarak gelişecek. Orijinal depoda, diğer katkıda bulunanlar yeni veriler ekleyebilirken, siz Fork'unuzda kendi değişikliklerinizi yapmaya devam edeceksiniz.
...
Yapacağınız değişikliklerin çoğu Markdown formatındaki (`.md`) dosyalar üzerinde olacaktır. Bu belgeleri düzenlemek için daha önce bahsettiğimiz Obsidian yazılımını kullanabilirsiniz. Ancak, Plan ₿ Academy diğer dosya biçimlerini kullanır ve bunlardan bazılarını değiştirmeniz gerekecektir.
