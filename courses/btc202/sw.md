...
Kama ilivyofafanuliwa na muundaji wake, Satoshi Nakamoto, Bitcoin inajionyesha kama mfumo wa pesa taslimu wa kielektroniki kati ya wenzao. Sentensi hii rahisi, ambayo ni kichwa cha Karatasi Nyeupe, ina vidokezo vingi vya asili ya Bitcoin:
...
Ni watumiaji hawa haswa wanaohakikisha usalama wa Bitcoin. Kama Eric Voskuil anavyoeleza katika kitabu chake *Cryptoeconomics*, usalama wa Bitcoin hautegemei Blockchain, wala nguvu ya haraka, wala uthibitisho, ugatuaji, cryptography, chanzo huria, wala nadharia ya mchezo. Usalama wa Bitcoin unategemea hasa watu ambao wako tayari kujiweka katika hatari ya kibinafsi. Ugatuaji huruhusu hatari hii kuenea kwa idadi kubwa ya watu binafsi, na ni uwezo wao wa kupinga tu ambao unahakikisha uimara wa mfumo.
...
- bitcoins zilizotumiwa lazima ziwepo katika seti yake ya UTXO (database ya matokeo ambayo hayajatumika);
- saini lazima iwe halali, na masharti yote ya matumizi lazima yatimizwe (hati halali);
- jumla ya kiasi cha matokeo lazima kisichozidi jumla ya kiasi cha pembejeo, ambayo ina maana kwamba gharama haziwezi kuwa mbaya.
...
- gW-402 **blocks** vilivyohifadhiwa kwenye diski,
...
- ukubwa uliotengwa kwa Mempool kupitia parameter ya `maxmempool`: node yenye Mempool kubwa itaweza kushikilia shughuli nyingi zaidi kuliko node yenye Mempool ndogo (isipokuwa mwisho inakuwa tupu);
- Sheria za gW-433: hizi ni sehemu ndogo ya sheria za upeanaji wa node na hufafanua sifa ambazo shughuli ambayo haijathibitishwa lazima ifikie ili kukubalika katika Mempool;
...
- gW-446.conf` ina vigezo vya usanidi wa node yako. Hapa ndipo unaweza kurekebisha sheria za relay. Nitakuambia zaidi kuhusu hili katika sura inayofuata.
- `settings.json` ina vigezo vya ziada vya `Bitcoin.conf`.
- `debug.log` ni kumbukumbu ya maandishi ya uchunguzi, ambayo inaweza kutumika kuelewa shughuli za node iwapo kuna hitilafu.
- gW-448.pid` huhifadhi kitambulishi cha mchakato wakati wa utekelezaji, ikiruhusu programu au hati nyingine kutambua kwa urahisi bitcoind (*Bitcoin daemon*) na kuingiliana nayo inapohitajika. Inaundwa wakati wa kuanza kwa node na kufutwa wakati wa kuzima.
...
- datacarrier=1`: Huruhusu utumaji tena na (ikiwa Mining kupitia node) ujumuishaji wa miamala inayobeba data isiyo ya kifedha kupitia towe la `OP_RETURN` (chaguo-msingi: `1`). Kuzima kigezo hiki hupunguza kidogo eneo la uso wa taka za data zisizo za kifedha, kwa gharama ya kupunguzwa kwa utangamano na matumizi fulani. Katika hali zote, lazima ukubali kuchimbwa `OP_RETURN`.
...
- peerbloomfilters=1`: Washa utumiaji wa vichujio vya Bloom (BIP37) ili kutoa blocks/ miamala iliyochujwa kwa wateja wembamba (chaguo-msingi: `0`). Onyo: hii huongeza mzigo kwenye rasilimali zako.
...
- addresstype=<legacy|P2SH-SegWit|bech32|bech32m>`: Inafafanua umbizo la address zinazozalishwa na Wallet kwa ajili ya mapokezi.
